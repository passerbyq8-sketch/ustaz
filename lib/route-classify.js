...
// THIS MODULE IMPORTS NOTHING, ON PURPOSE. guards/stored-deen-sub-suite.cjs mutates it by copying
// the file alone into a temp directory and importing it there, so a single relative import here
// makes that mutant unloadable and silently disarms the check. The homograph tier below therefore
// lives beside DEEN_WORDS rather than in lib/data/lexicon-ar.js — which is also simply where it
// belongs: it is a TIER OF that vocabulary, not a separate dataset.
...
const DEEN_WORDS = new Set([
  ...
  // THE RITE OF SACRIFICE WAS NEVER ON THIS LIST, AND NOBODY COULD SEE IT.
  // MEASURED while building the ambiguous tier: «كم سعر الأضحية هذي السنة؟» was classified
  // religious — but by «السَّنَة», the calendar year, folding onto «السُّنَّة». Not one word of the
  // rite itself was here. The collision had been silently standing in for the vocabulary, so the
  // gap only became visible when the collision stopped voting. These three are rites with no
  // secular reading at all, which is the same standard every entry above is held to.
  'اضحيه', 'اضاحي', 'عقيقه',
  // قرآن وحديث
...
// Exported so the retrieval side can fold a surface the SAME way the router does before it looks
// a word up. A second copy of this in another module is a second copy that drifts.
export function foldAffixes(tok) {
  return foldToken(tok);
}
...
// Membership is that general property and only it: after folding, the form is shared with a common
// secular word, and the secular reading is the one a reader of this product most often intends.
...
export const AMBIGUOUS_RELIGIOUS_TOKENS = Object.freeze(['كفر', 'سنه', 'عمره']);
...
// equals
//   text.replace(/<source\b[^>]*>[\s\S]*?<\/source>/gi, '').replace(/<source\b[^>]*>?[^<\n]*/gi, '')
//        .replace(/<\/source>/gi, '')
// i.e. byte-identical to what branch (a) produces today. Every non-source byte survives.
//
// THE THIRD REGEX CHANGED WITH ITS TWIN TOO (AA-30). An orphan `</source>` -- a closer with no
// opener in front of it -- is invisible to `indexOf(OPEN)`, because the `/` sits where the `s` is
// looked for. It survived here and in stripUnownedSourceCards() alike and reached the reader as
// raw markup. Both now strip it, and the scan below takes it only when it arrives BEFORE the next
// opener, so a well-formed pair is still consumed whole by the dropping state and its own closer
// is never seen by the orphan arm.
//
// THE SECOND REGEX CHANGED WITH ITS TWIN (أ-٦/١). It used to end `[\s\S]*$` — an unclosed card
// deleted the whole remainder of the answer, here and in lib/finalized-sse-writer.js's
// stripUnownedSourceCards(). Both now stop at the tag's own extent: the next `<` or the next
// newline. The two definitions MUST move together or the streamed answer and the finalised one
// stop being the same bytes, which is the property this contract exists to state. That is why
// the dropping state below now also terminates on a newline.
const OPEN = '<source';
const CLOSE = '</source>';
  ...
  function step() {
    ...
    for (;;) {
      if (dropping) {
        ...
        // A newline ends an UNCLOSED card's extent: whatever follows it was never inside the
        // card. It only counts when it arrives BEFORE a real </source>, otherwise a card that
        // legitimately spans a line would be cut in half and its own markup emitted.
      ...
      // AA-30 -- an ORPHAN CLOSER, taken before the open scan gives up. It is only an orphan when
      // no opener stands in front of it: if `i < j` the opener wins, the dropping state consumes
      // the pair whole, and this arm never sees that closer. Without this the tail-keep below
      // would emit `</source>` to the reader verbatim.
      if (j !== -1 && (i === -1 || j < i)) {
        out += buf.slice(0, j);
      ...
      if (i === -1) {
        // A tag split across chunks may be either shape, so hold back the longer dangle: a chunk
        // ending in `</sou` must not be emitted as prose -- it is no prefix of `<source`.
        const keep = Math.max(danglingPrefixLen(buf, OPEN), danglingPrefixLen(buf, CLOSE));
