...
//
// The age-derived band is always present because safeAge() maps absence and unusable values to
// zero. That floor matters: an absent age must not let a separately supplied `adult` band widen
// either the prompt persona or the source policy.
export function narrowestBand(clientBand, age) {
  ...
  const derived = bandForAge(age);
  return claimed && RANK[claimed] < RANK[derived] ? claimed : derived;
}

// buildSystemPrompt is generated and derives its persona from the numeric age it receives. When
// a client band restricts a real age downward, clamp that prompt input to the nearest age inside
// the effective band. Consistent, shipped-client pairs are returned byte-for-byte unchanged.
function ageWithinBand(age, band) {
  const a = safeAge(age);
  if (band === 'young' && a >= 13) return 12;
  if (band === 'teen' && a >= 18) return 17;
  return a;
}
...
export function readerFromBody(body) {
  ...
  const band = narrowestBand(b.band, b.age);
  return {
    name: safeName(b.name),
    age: ageWithinBand(b.age, band),
    band,
