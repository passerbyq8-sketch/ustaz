...
// م٢-و — the issue, not the words: the resolver before the gathering, the judge after it.
import { resolveIssue, judgeCandidates } from './issue-match.js';
// د-١ (DIAG_TRACE_V1) — records only while a traced turn is open.
import { diagTrace } from './diag-trace.js';
import { encyclopediaArticle } from './lib-nav.js';
...
// ── م٢-هـ: «حسب المذاهب» — EACH MADHHAB FROM ITS OWN TEXT ────────────────────────────────
// The owner's item: «كلُّ مذهبٍ من نصِّه المسترجَع (الموسوعة · «بداية المجتهد» · كتبُ المذهب) بإحالتِه؛
// ومذهبٌ بلا نصٍّ يُقالُ فيه ذلك». MEASURED (F): one call over several books is taken over by one of
// them, so each madhhab gets its OWN call, narrowed to its own reference book; and the catalog has no
// madhhab field, so the four books are named here, by id: بدائع الصنائع (Hanafi), الشرح الكبير
// للدردير وحاشية الدسوقي (Maliki), المجموع (Shafi'i), المغني (Hanbali). Every one of them is in the
// library with print pages (page_citable, numbering 'print').
...
// Four more library calls queue behind each other on the service (it runs one search at a time), so
// a madhhab question is given a longer gathering deadline than the ordinary one.
export const BW_GATHER_MADHHAB_MS = 5000;
...
// م٢-ط — THE COLD INSTANCE. The encyclopedia member alone may wait for the index's first build: measured
// 1.6-2.4 s on this machine and longer under load, against which the gathering's own deadline lost the
// race. A warm search takes milliseconds, so only a cold instance's first question ever waits this long,
// and api/ask.js starts the build at the head of the turn so most of it is already paid.
export const BW_ENCYC_COLD_MS = 6000;
 ...
 */
export async function gatherBeforeWriting({ question, table, ctx, libFlagValue, libToken, deps, madhhabMode = false, deadlineMs = madhhabMode ? BW_GATHER_MADHHAB_MS : BW_GATHER_MS }) {
  const record = { fired: true, fatwaQueries: 0, fatwaRows: 0, encycRows: 0, libRows: 0, libRetried: false, reasons: [], madhhab: madhhabMode ? {} : null };
  const { runTool, searchStoredCorpus, encyclopediaRow } = deps;
  const signal = ctx && ctx.signal;
  const empty = { rows: [], spend: [], degraded: ['deadline'], ok: false };

  // م٢-و — the resolver names the issue and writes the searches in the books' own words; an absent,
  // failed or unreadable resolver leaves the deterministic queries, so it can cost only the matching.
  const plan = typeof deps.ask === 'function' ? await resolveIssue(question, deps.ask) : null;
  record.resolver = typeof deps.ask === 'function' ? (plan ? 'ok' : 'fallback') : 'none';
  const fq = plan && plan.fatwaQueries.length ? plan.fatwaQueries : fatwaQueries(question);
  record.fatwaQueries = fq.length;
  const lq = (plan && plan.libraryQuery) || libraryQuery(question);
  const encTerms = plan && plan.encyclopediaTerms.length ? plan.encyclopediaTerms : [lq];
  const libOn = libFlagValue === 'on' && typeof libToken === 'string' && libToken !== '' && lq !== '';
  diagTrace('gather-plan', () => ({
    resolver: record.resolver, plan, fatwaQueries: fq, libraryQuery: lq, encyclopediaTerms: encTerms, libOn, madhhabMode, deadlineMs,
    fallbackFatwa: !(plan && plan.fatwaQueries.length), fallbackLibrary: !(plan && plan.libraryQuery), fallbackEncyclopedia: !(plan && plan.encyclopediaTerms.length),
  }));

  const fatwaMembers = fq.map((query) => raceDeadline(member(runTool, 'search_fatawa', { query }, ctx), deadlineMs, empty));
  const encycMember = encTerms.some(Boolean)
    ? raceDeadline(
      Promise.resolve()
        .then(() => Promise.all(encTerms.filter(Boolean).map((term) => searchStoredCorpus(term, { limit: 6 }))))
        .then((founds) => {
          const seen = new Set();
          const records = [];
          for (const found of founds) {
            for (const rec of (Array.isArray(found && found.records) ? found.records : [])) {
      ...
      Math.max(deadlineMs, BW_ENCYC_COLD_MS), { records: [], ok: false, reason: 'deadline' })
  ...
  // م٢-هـ — one member per madhhab book, each with the same one narrower retry.
  const madhhabMembers = (madhhabMode && libOn) ? MADHHAB_BOOKS_PRIMARY.map((m) => raceDeadline(
    member(runTool, 'search_library', { query: lq }, ctx, { ...libExtra, bookIds: [m.bookId], resultCap: BW_MADHHAB_ROWS_PER_BOOK }).then(async (first) => {
      if (first.rows.length) return first;
      const narrow = narrowLibraryQuery(question);
      if (!narrow || narrow === lq) return first;
      const second = await member(runTool, 'search_library', { query: narrow }, ctx, { ...libExtra, bookIds: [m.bookId], resultCap: BW_MADHHAB_ROWS_PER_BOOK });
      return { ...second, spend: [...first.spend, ...second.spend], degraded: [...first.degraded, ...second.degraded] };
    }), deadlineMs, empty)) : [];
  const [fatwaOuts, encycOut, libOut, madhhabOuts] = await Promise.all([Promise.all(fatwaMembers), encycMember, libMember, Promise.all(madhhabMembers)]);
  // Every member's spend and notes are the turn's, and each spend row is LABELLED as unrequested.
  for (const out of [...fatwaOuts, libOut, ...madhhabOuts]) {
  ...
  diagTrace('gather-members', () => {
    const rowsOf = (out) => (out && Array.isArray(out.rows) ? out.rows : []).map((r) => ({ id: r.recordId || r.id || r.url, title: r.bookTitle || r.title, volume: r.locatorSpan && r.locatorSpan.volume, page: r.locatorSpan && r.locatorSpan.pageStart, chars: String(r.fullText || r.text || '').length }));
    return {
      fatwa: fatwaOuts.map((out) => ({ ok: out.ok, degraded: out.degraded, spend: out.spend, rows: rowsOf(out) })),
      encyclopedia: { ok: encycOut && encycOut.ok, reason: encycOut && encycOut.reason, ids: (encycOut && encycOut.records || []).map((r) => r.id) },
      library: { ok: libOut.ok, retried: record.libRetried, degraded: libOut.degraded, spend: libOut.spend, rows: rowsOf(libOut) },
      madhhab: madhhabOuts.map((out, i) => ({ key: MADHHAB_BOOKS_PRIMARY[i].key, book: MADHHAB_BOOKS_PRIMARY[i].bookId, ok: out.ok, degraded: out.degraded, spend: out.spend, rows: rowsOf(out) })),
    };
  });

  // م٢-و — THE CANDIDATES FIRST, THE TABLE AFTER THE JUDGE. Every store's rows are gathered as
  // candidates (twice each ceiling, so a judge that drops the adjacent card still leaves the right
  // ones), the judge keeps what answers THE ISSUE, and only then are rows added — in the same fixed
  // order and under the same ceilings as before.
  const fatwaCands = [];
  const seenFatwa = new Set();
  for (const out of fatwaOuts) {
    for (const row of out.rows) {
      const key = row.url || row.recordId || row.title;
      if (seenFatwa.has(key) || fatwaCands.length >= BW_FATWA_MAX_ROWS * 2) continue;
      seenFatwa.add(key);
      fatwaCands.push(row);
    }
  }
  const encCands = (encycOut && Array.isArray(encycOut.records) ? encycOut.records : [])
    .filter((rec) => rec && String(rec.snippet || '').trim() !== '')
    .slice(0, BW_ENCYC_MAX_ROWS * 2)
    .map((rec) => ({ ...encyclopediaRow(rec), fullText: String(rec.text || '').slice(0, BW_ENCYC_CHARS) }));
  const libCands = (libOut.rows || []).slice(0, BW_LIBRARY_MAX_ROWS);
  const madhhabCands = madhhabOuts.map((out, i) => (out.rows || []).slice(0, BW_MADHHAB_ROWS_PER_BOOK)
    .map((row) => ({ ...row, madhhab: MADHHAB_BOOKS_PRIMARY[i].key })));
  const all = [...fatwaCands, ...encCands, ...libCands, ...madhhabCands.flat()];
  let kept = new Set(all);
  if (typeof deps.ask === 'function' && all.length) {
    const judged = await judgeCandidates({ question, issue: plan ? plan.issue : '', candidates: all, ask: deps.ask });
    record.judge = judged.judged ? 'ok' : 'fallback';
    record.judgeDropped = all.length - judged.kept.length;
    kept = new Set(judged.kept);
  }
  const added = [];
  // D-2 E4: hydrate only accepted truncated encyclopedia candidates, before pinning.
  const articleDeadline = Date.now() + 12000;
  await Promise.all(encCands.filter(r => kept.has(r)).slice(0, BW_ENCYC_MAX_ROWS).map(async row => {
    if (String(row.fullText || '').length < BW_ENCYC_CHARS || !libOn) return;
    const term = String(row.title || '').replace(/^الموسوعة الفقهية الكويتية — /u, '');
    const full = await encyclopediaArticle(term, row.part, {
      flagValue: libFlagValue, token: libToken, fetchImpl: ctx.fetchImpl, signal, deadline: articleDeadline,
    });
    if (full) { row.fullText = full.text; row.articleSections = full.sections; row.articleComplete = true; }
    else record.reasons.push('encyclopedia:full_article_unavailable:' + row.recordId);
  }));
  for (const row of fatwaCands.filter((r) => kept.has(r)).slice(0, BW_FATWA_MAX_ROWS)) { added.push(table.add(row)); record.fatwaRows += 1; }
  for (const row of encCands.filter((r) => kept.has(r)).slice(0, BW_ENCYC_MAX_ROWS)) { added.push(table.add(row)); record.encycRows += 1; }
  for (const row of libCands.filter((r) => kept.has(r))) { added.push(table.add(row)); record.libRows += 1; }
  // م٢-هـ — the madhhab books last, in the fixed order of MADHHAB_BOOKS_PRIMARY, each row carrying the
  // madhhab its book belongs to, so the writer and the sentence door can tell whose text it is.
  madhhabCands.forEach((rows, i) => {
    const m = MADHHAB_BOOKS_PRIMARY[i];
    const keptRows = rows.filter((r) => kept.has(r));
    record.madhhab[m.key] = keptRows.length;
    for (const row of keptRows) added.push(table.add(row));
  });
  diagTrace('pinned-table', () => ({
    record,
    rows: added.map((r) => ({
      ref: r.ref, kind: r.kind, madhhab: r.madhhab, title: r.bookTitle || r.title, author: r.author,
      volume: r.locatorSpan ? r.locatorSpan.volume : r.part, page: r.locatorSpan ? r.locatorSpan.pageStart : r.page,
      pageEnd: r.locatorSpan ? r.locatorSpan.pageEnd : undefined, id: r.recordId || r.id, url: r.url,
    })),
  }));
  return { added, record };
}
