...
// ENCYC_V1 (sources order 2026-09-24) -- THE YIELDING BUILD, AND WHY IT EXISTS.
// The ordinary build below is ONE synchronous run: gunzip, JSON.parse and addAll over 3,045
// records, measured 1.6-2.8 s on the first call of a process. While it runs nothing else in the
// process runs -- not the reply to a network call it was meant to overlap, not a timer -- so a
// deadline raced against it can never fire. `warmEncyclopedia()` builds the SAME index from the
// SAME records in the SAME order with the SAME options, but gunzips off the main thread and adds
// the records in chunks that hand the event loop back between them (MiniSearch's own addAllAsync).
// The longest stall is then JSON.parse (~0.1 s). Nothing else changes: `yielding` defaults to
// false, so every caller that does not ask for it builds exactly as before.
const WARM_CHUNK_SIZE = 50;
const gunzipAsync = (buffer) => new Promise((resolve, reject) => {
  gunzip(buffer, (error, out) => (error ? reject(error) : resolve(out)));
});

async function buildIndex({ yielding = false } = {}) {
  const { default: MiniSearch } = await import('minisearch');
  const gzPath = resolveGzPath();
  const gz = readFileSync(gzPath);
  const records = JSON.parse((yielding ? await gunzipAsync(gz) : gunzipSync(gz)).toString('utf8'));
  ...
  if (yielding) await mini.addAllAsync(records, { chunkSize: WARM_CHUNK_SIZE });
  else mini.addAll(records);
  return { mini, recordCount: records.length };
}

function getIndex(options) {
  if (!_indexPromise) {
    _indexPromise = buildIndex(options).catch((e) => {
...
/**
 * ENCYC_V1 -- start the index build now, yielding, and do not make the caller wait for it.
 * Shares getIndex()'s one memo: if a build is already in flight (or done) this starts nothing,
 * and every search awaits whichever build exists. NEVER rejects: resolves true when the index is
 * ready and false when the build failed (the search that follows warns, as it always has).
 */
export function warmEncyclopedia() {
  return getIndex({ yielding: true }).then(() => true, () => false);
}
...
export async function searchStoredCorpus(query, { limit = 18 } = {}) {
  ...
  try {
    ...
    const ranked = store.mini.search(norm, {
      prefix: true,
      fuzzy: 0.2,
      combineWith: 'OR',
      boost: { term: 4 },
    });
    hits = ranked.slice(0, cap);
    diagTrace('encyclopedia', () => ({
      query, norm, limit, cap, matched: ranked.length,
      top: ranked.slice(0, 20).map((hit, i) => ({ rank: i + 1, id: String(hit.id || ''), title: String(hit.term || ''), part: Number(hit.part), score: hit.score, taken: i < cap })),
    }));
  } catch (e) {
