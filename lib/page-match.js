...
export function matchPage({ question = '', terms = null, title = '', text = '' } = {}) {
  ...
  // A question with no pivot term of its own cannot refuse anything. Silence here is the
  // fail-SAFE direction: this module exists to remove wrong pages, not to invent refusals.
  if (!want.length) return out('match', 'no-pivot-terms-in-question', [], 1);
