...
import { violatesTemplate } from '../policy/attribution-grades.js';
...
export function gate3Deterministic(ledger, sentence) {
  const claim = (sentence.claimIds || []).map((id) => ledger.claim(id)).find(Boolean);
  const policy = (claim && claim.claimRelation)
    ? { relation: claim.claimRelation, grade: claim.provenanceGrade || claim.provenanceCap }
    : (ledger.policy ? { relation: ledger.policy.claimRelation, grade: ledger.policy.provenanceCap } : null);

  if (policy && policy.relation && violatesTemplate(sentence.text, policy)) {
