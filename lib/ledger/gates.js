...
import { sha256, sliceByBytes, wrapUntrusted } from './segment.js';
...
// THE SOURCE-CLASS RULE, THE SAME MODULE THE LEGACY SCREEN READS. Not a copy and not a second
// opinion: which persons a result set licenses naming is one question with one answer, and two
// implementations of it is one implementation waiting to disagree with the other.
import { attributionLicence } from '../policy/source-attribution.js';
import { consistencyProblems, PROBLEM } from '../policy/consistency-gate.js';
...
export function gate3Deterministic(ledger, sentence) {
  ...
  // ── DOES ANY PAGE UNDER THIS SENTENCE LICENSE NAMING THE MAN IT NAMES? ────
  //
  // The template check above asks whether the WORDING claims more than the grade allows. It cannot
  // ask whether the person named is a person the evidence ever mentioned — a grade-C transmission
  // is a perfectly well-formed sentence when the man in it appears nowhere on the page, and that
  // is precisely how «داعية وخطيب كويتي معروف من أهل العلم» was served over a khutbah that never
  // named him. So the pages the claims actually rest on are read for who they license, and a
  // sentence crediting anybody else with a صفة or a موقف is dropped.
  //
  // Only ATTRIBUTION_NOT_LICENSED is consulted. The other codes belong to the legacy screen, which
  // judges a whole reply drafted in one call; here every other invariant already has its own gate.
  if (claims.length) {
    const pages = claims.map((c) => {
      const src = ledger.source(c.sourceId);
      if (!src) return null;
      return {
        url: src.canonicalUrl || src.host || '',
        author: src.author || '',
        text: ledger.pageText.get(c.sourceId) || '',
      };
    }).filter(Boolean);
    const licensed = attributionLicence(pages).personIds;
    if (consistencyProblems(sentence.text, { sourceLicence: licensed })
      .includes(PROBLEM.ATTRIBUTION_NOT_LICENSED)) {
      return 'source-class:unlicensed-attribution';
    }
  }
