...
import { needsFollowUp, orderedIssues } from './query-ir.js';
...
import { SOURCE_POLICY_VERSION, primaryOpinionAdapter } from './source-policy.js';
...
export async function runEngine(question, opts = {}) {
  const budget = new Budget({ now: opts.now });
  ...
  // ── ANALYZE_QUERY_IR ───────────────────────────────────────────────────────
  let t0 = clock();
  const planned = await planQuestion(question, { budget, fetchImpl: opts.fetchImpl, tier: opts.tier });
  mark('plan', clock() - t0);
  ...
  // A named scholar with no registered corpus is refused HERE — before a single search — and
  // only that issue is refused. A compound question keeps the half we can answer.
  ...
  for (const issue of orderedIssues(plan)) {
    ...
    const planned2 = planIssueBatches(issue, bandSites, {
      ...
      onlySites: issue.requestedAuthorityId
    ...
    work.push({ issue, batches: planned2.batches });
  }
  ...
  // ── THE LOOP ───────────────────────────────────────────────────────────────
  const queue = [];
  for (const w of work) for (const b of w.batches) queue.push({ issue: w.issue, batch: b });

  let qi = 0;
  while (qi < queue.length) {
    if (budget.deadlineReached()) { ledger.reject(REJECTION.BUDGET_EXHAUSTED, 'deadline'); break; }

    ledger.transition('EXECUTE_BATCH');
    const { issue, batch } = queue[qi++];

    // ── EXECUTE_BATCH: a direct corpus read, costing no provider call ──
    let directPages = null;
    if (batch.direct) {
      t0 = clock();
      directPages = await readDirectCorpus(issue.requestedAuthorityId, issue, {
      ...
      ledger.recordSearchAttempt({
        ...
        chars: 0, words: 0, resultCount: directPages.length, refusedCount: 0, cache: 'n/a', ok: true,
      });
      if (!directPages.length) {
        ledger.reject(REJECTION.NO_SUFFICIENT_DIRECT_EVIDENCE, 'direct-corpus-empty', issue.issueId);
        continue;
      }
    }
    ...
    if (!batch.direct) {
      ...
      if (cached.hit) { results = cached.value; cacheState = 'hit'; cacheHits++; }
        ...
        if (results.length) await cache.putSearch(batch.q, results, { sites: batch.sites });
      }
      ...
      for (const w of wanted.slice(0, MAX_CANDIDATES_PER_BATCH)) {
        ...
        if (w.fresh) {
          ...
          const ck = await cache.getExtraction(w.cand.url, { adapterVersion: undefined });
    ...
    if (ledger.verifiedClaims().some((c) => c.issueId === issue.issueId)) {
      const primary = issue.requiredSlots[0];
    ...
    // Early stop is decided on VERIFIED SLOTS, never on a promising title or snippet.
    const remaining = work.filter((w) => !refusedIssues.has(w.issue.issueId));
    if (remaining.every((w) => ledger.issueComplete(w.issue.issueId))) break;
