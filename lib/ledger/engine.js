...
import { assemble, selectClaimsForAnswer, READER } from './assemble.js';
...
export async function runEngine(question, opts = {}) {
  // Defense in depth at the engine's public boundary. The seam normally supplies this as an own
  // primitive, but direct callers and fixtures can carry prototypes. Check ownership before the
  // read, then keep this immutable request-local value for every model call and retry.
  ...
  const planned = opts.plannerOverride
    ...
    // The handler already reduced authorization to one request-local primitive. The planner is
    // another model call in that same request, so it receives the same tier as extraction,
    // verification and drafting. It never re-derives entitlement from the question or body.
    //
    ...
    : await planQuestion(question, {
      budget, fetchImpl: opts.fetchImpl, bandSites: opts.bandSites, tier,
    });
  mark('plan', clock() - t0);
  ...
  if (needsFollowUp(plan)) {
    // A missing qualifier is useful telemetry, not permission to withhold an answer.
    // Search the common forms and let the evidence-backed assembly distinguish them.
  ...
  while (qi < queue.length) {
    ...
    if (!batch.direct) {
      ...
      if (cached.hit) { results = cached.value; cacheState = 'hit'; cacheHits++; }
        ...
        if (dailyBudget && opts.searchHandlesDailyBudget !== true) {
        ...
        try {
          results = await opts.search(batch.q, batch.sites, {
            dailyBudget: opts.searchHandlesDailyBudget === true ? dailyBudget : null,
            requireDailyBudget: opts.searchHandlesDailyBudget === true,
          });
          providerAttempted = true;
        } catch (error) {
          if (error && ['day_cap_reached', 'caller_cap_reached', 'budget_store_unavailable'].includes(error.reason)) {
            serviceLimited = error.reason;
            ledger.reject(REJECTION.BUDGET_EXHAUSTED, 'daily:' + error.reason, issue.issueId);
            for (const slot of issue.requiredSlots) {
              if (!ledger.slotProof(issue.issueId, slot).searchAttempted) {
                ledger.recordSlotProof(issue.issueId, slot, {});
              }
        ...
        if (providerAttempted) budget.spend('braveCalls', 1, issue.intent);
    ...
    if (admitted.length) {
      ...
      if (!confirmed.length && unsure.length && budget.canAfford('modelCalls', 5)) {
        ...
        const res = await callModel({
          ...
          budget, purpose: 'page_match', tier, fetchImpl: opts.fetchImpl,
        });
    ...
    const extracted = await runExtraction(ledger, issue, fresh, {
      budget, fetchImpl: opts.fetchImpl, tier, cycle: budget.spent.verifiedCycles,
    });
    ...
    t0 = clock();
    await runGate2(ledger, survivedG1, { budget, fetchImpl: opts.fetchImpl, tier });
    mark('gate2', clock() - t0);
  ...
  const drafted = await runDraft(ledger, selected, work.map((w) => w.issue), {
    budget, fetchImpl: opts.fetchImpl, tier,
  });
  mark('draft', clock() - t0);
  ...
  t0 = clock();
  const g3 = await runGate3(ledger, drafted.sentences, { budget, fetchImpl: opts.fetchImpl, tier });
  mark('gate3', clock() - t0);
