 ...
 *
 * Arm 3 is what makes «refused without searching» structurally impossible: it needs no network,
 * no key, no budget and no model, so the only remaining reasons a request does not search are
 * the day's ceiling, an empty question, and a band with no eligible source — each of which is an
 * honest fact about the request rather than an accident of a model's phrasing.
 */
export async function planQuestion(question, { budget, fetchImpl, bandSites } = {}) {
  const floor = () => {
