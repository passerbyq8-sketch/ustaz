...
/**
 * A problem sink that keeps the sentence and the field token side by side.
 * `fields()` is deduped and ordered by first appearance — the first field to break is the one
 * a reader of the record wants named first.
 */
function problemSink() {
  ...
  return {
    ...
    push(field, message) {
      ...
      if (IR_FIELD_SET.has(field) && !seen.includes(field)) seen.push(field);
    },
    /** Fold another sink's tokens in without re-stating its sentences. */
    addFields(list) {
      for (const f of list || []) if (IR_FIELD_SET.has(f) && !seen.includes(f)) seen.push(f);
    },
    fields: () => seen.slice(),
  };
}
...
function strArray(value, field, label, sink, { maxItems = MAX_TERMS_PER_FIELD, maxChars = MAX_TERM_CHARS } = {}) {
  ...
  if (!Array.isArray(value)) { sink.push(field, label + ': not an array'); return []; }
  if (value.length > maxItems) { sink.push(field, label + ': more than ' + maxItems + ' entries'); return []; }
  const out = [];
  for (const v of value) {
    if (!isStr(v)) { sink.push(field, label + ': non-string entry'); continue; }
    const s = v.trim();
    if (!s) continue;
    if (s.length > maxChars) { sink.push(field, label + ': entry longer than ' + maxChars + ' chars'); continue; }
    out.push(s);
  }
  return out;
}
...
function validateIssue(raw, index, questionText) {
  ...
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    const s = problemSink();
    s.push('issues', 'issue[' + index + '] is not an object');
    return { issue: null, problems: s.messages, fields: s.fields() };
  }
  ...
  if (raw.requested_authority_id !== undefined && raw.requested_authority_id !== null) {
    if (!isStr(raw.requested_authority_id) || !ID_RE.test(raw.requested_authority_id)) {
      p('requested_authority_id', 'requested_authority_id is not a bare identifier');
    } else {
  ...
  if (askedSlots.length > MAX_SLOTS_PER_ISSUE) p('required_slots', 'required_slots exceeds ' + MAX_SLOTS_PER_ISSUE);
  const slots = new Set();
  for (const s of askedSlots.slice(0, MAX_SLOTS_PER_ISSUE)) {
    if (!isStr(s) || !SLOT_SET.has(s)) { p('required_slots', 'unknown slot: ' + String(s).slice(0, 24)); continue; }
    slots.add(s);
  }
  ...
  for (const d of deps) if (!ID_RE.test(d)) p('dependencies', 'dependency is not a bare identifier: ' + d.slice(0, 24));

  const temporal = TEMPORAL_SCOPES.includes(raw.temporal_scope) ? raw.temporal_scope : null;
  if (!temporal) p('temporal_scope', 'temporal_scope is not one of ' + TEMPORAL_SCOPES.join('|'));
  ...
  if (problems.length) return { issue: null, problems, fields: sink.fields() };
  return {
    fields: [],
    issue: Object.freeze({
      issueId,
...
export function validateQueryPlan(raw, questionText = '') {
  ...
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    ...
    return { ok: false, plan: null, problems, problemFields: sink.fields(), authorityRefusals: [] };
  }
  ...
  for (const k of Object.keys(raw)) {
    if (!ALLOWED_TOP.has(k)) sink.push('unknown_field', 'unknown top-level field: ' + k);
  }
  ...
  if (!confidence) sink.push('confidence', 'confidence is not one of ' + CONFIDENCE.join('|'));
  ...
  if (Array.isArray(raw.issues)) {
    ...
    raw.issues.slice(0, MAX_ISSUES).forEach((r, i) => {
      const { issue, problems: ip, fields: ifs } = validateIssue(r, i, questionText);
      if (ip.length) {
    ...
    for (const iss of issues) {
      for (const d of iss.dependencies) {
        if (!seen.has(d)) sink.push('dependencies', 'issue ' + iss.issueId + ' depends on unknown issue ' + d);
        if (d === iss.issueId) sink.push('dependencies', 'issue ' + iss.issueId + ' depends on itself');
      }
    }
