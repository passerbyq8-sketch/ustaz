...
import { readEntities, eraOf } from '../policy/entities.js';
...
export function validateQueryPlan(raw, questionText = '') {
  ...
  const policy = buildPolicyBlock(questionText, issues);
  return {
    ok: true,
    plan: Object.freeze({
      issues: Object.freeze(issues.map((i) => Object.freeze({ ...i, policy }))),
...
export function buildPolicyBlock(questionText, issues = []) {
  ...
  // THE MODEL MAY AGREE, OR BE DROPPED. If the deterministic reading says the question is ABOUT a
  // man rather than a request for his position, a `requested_authority_id` the planner supplied is
  // discarded: keeping it would let the planner turn a biography question into an attributed one,
  // which is the exact inversion this engine exists to prevent. Where the two agree, the model's
  // id is used because it went through the registry check in validateIssue().
  const modelAuthority = issues.map((i) => i.requestedAuthorityId).find(Boolean) || null;
  const requestedAuthorityId = ir.claimRelation === 'ABOUT_ENTITY'
    ? null
    : (ir.requestedAuthorityId || modelAuthority);

  const authorityEntity = ir.entities.find((e) => e.canonicalId === requestedAuthorityId);
  const subject = ir.entities.find((e) => e.role === 'subject');
  ...
  return Object.freeze({
    claimRelation: ir.claimRelation,
    targetType,
    era,
