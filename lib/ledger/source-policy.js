...
// ع-١ — the three news rows below are admitted to the FETCHER only while the switch is on; see
// LIVE_WORLD_V2_ROWS at policyFor(). The flag is read through the module that owns it and through
// nothing else, so this file names no environment variable of its own.
...
const ROWS = [
  ...
  // ── THE EXCHANGE-RATE REFERENCE (LIVE_WORLD_V2, 2026-09-19) ────────────────
  //
  // SAME SHAPE AS THE ROW ABOVE AND FOR THE SAME REASON. safeFetch() refuses any host without a
  // row here with `preflight:not-an-admissible-url`, so a domain admitted to the world list and
  // not admitted here is a domain the fetcher will never open — a list entry that reads as
  // coverage and produces nothing. (That is not hypothetical: it is the measured state of three
  // of the four existing world domains, recorded in the round's report.)
  //
  // WHAT IT GRANTS, EXHAUSTIVELY: permission to be fetched through the existing hardened path.
  // `caps: {}` makes every capability ineligible and `searchable: false` keeps it out of
  // domainsForCapability(), so a central bank's rate table can never become evidence for a
  // fatwa, a tafsir, a hadith or a scholar's opinion. It is a reference for ONE kind of fact —
  // the official exchange rate of the dinar — chosen by the owner over any commercial exchange
  // house and over any encyclopedia.
  R({
    domain: 'cbk.gov.kw', ownerId: null, adapterId: 'readability', adapterVersion: 'r1',
    health: 'enabled', searchable: false, caps: {},
    note: 'بنك الكويت المركزي: مرجع أسعار الصرف الرسمي وحده (LIVE_WORLD_V2). لا يصلح لفتوى ولا تفسير ولا حديث ولا قول عالِم — والقدرات فارغة عمدًا.',
  }),

  // ── ع-١: THE THREE WORLD NEWS HOSTS, ADMITTED TO THE FETCHER (LIVE_WORLD_V2, 2026-09-20) ──
  //
  // MEASURED, AND IT IS WHY EVERY NEWS QUESTION CAME BACK EMPTY. aljazeera.net, bbc.com and
  // skynewsarabia.com have been on the world SEARCH list since that list shipped, and not one of
  // them has ever had a row in this table — so safeFetch()'s preflight refused every page they
  // returned with `not-an-admissible-url`, before a socket was opened. The app searched them, got
  // URLs back, and could not open any of them: a list that reads as coverage and produces nothing.
  // cbk.gov.kw's row above closed this for the fifth domain on 19 September; these close it for
  // the three that were on the list all along.
  //
  // WHAT THEY GRANT, EXHAUSTIVELY: permission to be FETCHED through the existing hardened path.
  // The SSRF defence is untouched — same preflight, same https-only rule, same public-DNS check,
  // same redirect re-admission on every hop — and lib/source-page-gates.js still decides which of
  // their URLs is an article rather than a section index.
  //
  // WHAT THEY DO NOT GRANT, and this is the point of the empty `caps`: none of them may back a
  // fatwa, a tafsir, a hadith text, a grading, an explanation, a scholar's opinion or a general
  // article. `policy({})` marks every capability ineligible and `searchable: false` keeps them out
  // of domainsForCapability(), so they are never offered as a place to look for a ruling. That is
  // the same refusal their registry rows state in words — «ممنوع أن يُبنى عليه حكم شرعي» — said
  // here in data, which is where the fetcher can read it.
  //
  // AND THEY ARE BEHIND THE SWITCH, WHICH cbk.gov.kw'S ROW DID NOT HAVE TO BE. The registry admits
  // cbk to the world list ONLY while LIVE_WORLD_V2 is on, so its row could be declared
  // unconditionally and still reach nothing with the flag off. These three are on the world list
  // TODAY, flag or no flag, so an unconditional row would change what the app fetches the moment
  // it landed — and the owner's condition is that the switch off is the app that was measured. The
  // gate is in policyFor() below: one test, one place, and with the flag off it answers `null` for
  // these three exactly as the table did before they existed. The ROWS themselves are declared
  // unconditionally, for the reason the registry states about cbk: a row that disappears with an
  // environment variable is a row nobody can review.
  R({
    domain: 'aljazeera.net', ownerId: null, adapterId: 'readability', adapterVersion: 'r1',
    health: 'enabled', searchable: false, caps: {},
    note: 'الجزيرة نت: إذنُ جلبٍ لصفحات الأخبار وحدَه (LIVE_WORLD_V2). لا تصلح لفتوى ولا تفسير ولا حديث ولا قول عالِم — والقدرات فارغة عمدًا.',
  }),
  R({
    domain: 'bbc.com', ownerId: null, adapterId: 'readability', adapterVersion: 'r1',
    health: 'enabled', searchable: false, caps: {},
    note: 'بي بي سي عربي: إذنُ جلبٍ للخدمة العربية وحدَها (LIVE_WORLD_V2)؛ والمسارُ يحكمُه lib/source-page-gates.js. لا تصلح لفتوى ولا تفسير ولا حديث ولا قول عالِم — والقدرات فارغة عمدًا.',
  }),
  R({
    domain: 'skynewsarabia.com', ownerId: null, adapterId: 'readability', adapterVersion: 'r1',
    health: 'enabled', searchable: false, caps: {},
    note: 'سكاي نيوز عربية: إذنُ جلبٍ لصفحات الخبر وحدَها (LIVE_WORLD_V2). لا تصلح لفتوى ولا تفسير ولا حديث ولا قول عالِم — والقدرات فارغة عمدًا.',
  }),
...
// ع-١ — THE ROWS THAT EXIST ONLY WHILE THE SWITCH IS ON.
//
// ONE TEST, IN THE ONE FUNCTION EVERY READER OF THIS TABLE GOES THROUGH. capabilityEligible(),
// expectedAdapterVersion(), ownerOf(), removableParamsFor(), canonical.js's admissible() and
// safe-fetch.js's preflight() all reach a row through policyFor() — so gating here gates all of
// them at once, and there is no second place that can disagree about what "off" means. The three
// domains are the news hosts added above; cbk.gov.kw is NOT among them, because the registry
// already keeps it off the world list with the flag off and its row can therefore stand.
const LIVE_WORLD_V2_ROWS = new Set(['aljazeera.net', 'bbc.com', 'skynewsarabia.com']);
...
export function policyFor(hostOrUrl) {
  ...
  const row = exact || ROWS.find((r) => d === r.domain || d.endsWith('.' + r.domain)) || null;
  // Read per call, never cached: one process drives both states in the kill-switch gate, and a
  // decision frozen at module load would answer for whichever state imported it first.
  if (row && LIVE_WORLD_V2_ROWS.has(row.domain) && !liveWorldV2Enabled()) return null;
  return row;
}
