...
// unreachable store, a non-internal caller and a malformed trace id alike, and the try/catch is
// there for the case none of those cover. Nothing here can change an answer, because by this
// point there is no answer left to change.
async function emitTelemetry(record, internal) {
  if (!record) return { written: false, reason: 'no-record' };
  try {
    return await telemetry.record(record, { internal });
  } catch (e) {
...
export async function runLedgerTurn(res, opts = {}) {
  ...
  if (!parsed.ok) {
    ...
    const wrote = await emitTelemetry(
      stubRecord(opts.traceId, 'NO_QUESTION', opts.flagState), opts.internalTester);
    return {
  ...
  try {
    ...
    const wrote = await emitTelemetry(
      stubRecord(opts.traceId, 'ENGINE_THREW', opts.flagState), opts.internalTester);
    return {
      outcome: 'SAFE_REJECTION', text: READER.NO_EVIDENCE_GENERAL, cards: [],
  ...
  const wrote = await emitTelemetry(out.telemetry && out.telemetry.record, opts.internalTester);
