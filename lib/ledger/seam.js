...
// ── THE COUNTS LINE: STDOUT, AND IT SHIPS BEFORE res.end() ───────────────────
//
// WHERE IT USED TO BE, AND WHY THAT WAS NOWHERE. It lived in api/ask.js, on the line after
// `await runLedgerTurn(...)` — which is after wire.close() has already called res.end(). On a
// serverless platform the invocation may be frozen the instant the response completes, so the
// line was WRITTEN and never SHIPPED. The first live composite fiqh probe against the opened
// engine returned a refusal with no counts line in the log at all, which left the one question
// the probe existed to answer — was Brave called, or was it never called — with no evidence
// either way. A counts line that cannot be read is not instrumentation.
//
// WHY THIS MOVES AND emitTelemetry() BELOW DOES NOT. They are not the same kind of work. The
// store write is a network round trip, and a round trip in front of the reader's bytes is a
// latency change the owner ruled out. This is a synchronous console.log costing microseconds,
// and it buys the only externally visible record of what a request spent. So the stdout line
// goes in front of the close and the store write stays behind it, deliberately, and
// guards/ledger-telemetry-guard.cjs section F pins BOTH halves of that ordering.
//
// ALL THREE EXITS, because the two that fail are the two worth reading. Logging after the await
// covered the no-question and engine-threw exits as a side effect — including the null trace
// those exits made the `ledger ? ... : null` ternary necessary for. Moving the line to the
// successful exit alone would have quietly dropped the counts for exactly the requests a counts
// line gets read for.
//
// COUNTS AND CODES ONLY. No question, no answer, no page text, no reader identity.
function logCounts(outcome, ledger, budget) {
  const snap = budget.snapshot();
  console.log('[ledger]', {
    trace: ledger ? ledger.traceId : null, outcome,
    model: snap.spent.modelCalls,
    brave: snap.spent.braveCalls,
 ...
 *                        directReader, readerPrefix, now, startedAt, traceId, plannerOverride
 * @returns {Promise<{outcome, text, cards, ledger, budget, cacheHits, cacheMisses}>}
 */
export async function runLedgerTurn(res, opts = {}) {
  // Authorization arrives as one request-local primitive. An inherited value is client-shaped
  // ambient state, not a grant: check ownership before reading it so even an inherited getter is
  // never invoked. Only an own, exact Premium primitive survives this boundary.
  const tier = Object.prototype.hasOwnProperty.call(opts, 'tier') && opts.tier === 'premium'
    ? 'premium' : 'standard';
  const readerPrefix = String(opts.readerPrefix || '').trim();
  const withReaderPrefix = (text) => {
    const body = String(text || '');
    return readerPrefix ? readerPrefix + '\n\n' + body : body;
  };
  const registerFinalizerSources = typeof opts.registerFinalizerSources === 'function'
    ? opts.registerFinalizerSources : () => {};
  let finalizerSourcesRegistered = false;
  const registerOnce = (pages) => {
    if (finalizerSourcesRegistered) return;
    finalizerSourcesRegistered = true;
    registerFinalizerSources(Array.isArray(pages) ? pages : []);
  };
  ...
  const dailyBudget = opts.dailyBudgetMode === 'fixture'
    ? null
    : (opts.dailyBudget || new DailySearchBudget({ now, callerDigests: opts.callerDigests }));
  ...
  if (!parsed.ok) {
    ...
    registerOnce([]);
    wire.delta(withReaderPrefix(READER.NO_EVIDENCE_GENERAL));
  ...
  try {
    out = await runEngine(parsed.question, {
      ...
      search: opts.search,
      searchHandlesDailyBudget: opts.searchHandlesDailyBudget,
  ...
  const cardUrls = new Set((out.cards || []).map((card) => card && card.url).filter(Boolean));
  const finalizerPages = [];
  const seenSourceIds = new Set();
  const seenUrls = new Set();
  if (out.ledger && out.ledger.sources instanceof Map && out.ledger.pageText instanceof Map) {
    for (const [sourceId, source] of out.ledger.sources) {
      const canonicalUrl = source && source.canonicalUrl;
      const passage = String(out.ledger.pageText.get(sourceId) || '');
      if (!sourceId || !canonicalUrl || !passage.trim() || !cardUrls.has(canonicalUrl)
        || seenSourceIds.has(sourceId) || seenUrls.has(canonicalUrl)) continue;
  ...
  registerOnce(finalizerPages);

  // The engine, not the request or a model, owns this outcome. Publish it before the first
  // buffered byte so api/ask's finalizer can distinguish a trusted refusal/follow-up from an
  // ordinary answer. Every other (including malformed or future) outcome stays an answer and
  // therefore keeps the full source/consistency finalizer.
  try { opts.registerFinalizerOutcome?.(out.outcome); } catch {}
  ...
  wire.delta(withReaderPrefix(out.text) + (tags.length ? '\n' + tags.join('\n') : ''));
  // BEFORE close(), never after — the invocation is still serving here, so the line actually
  // reaches the log. It cannot delay the reader: the answer and its cards are already on the
  // socket, and this is a synchronous write to stdout.
  logCounts(out.outcome, out.ledger, budget);
  wire.close();
