...
export const SERVICE_LIMITED_TEXT =
  'بلغنا حدّ البحث المتاح لعزك اليوم، فتعذّر استكمال البحث في المصادر لهذا السؤال. '
  + 'جرّب مرّةً أخرى لاحقًا، وما كان محفوظًا عندنا من القرآن والأذكار والصفات الثابتة يعمل كما هو.';
...
export class DailySearchBudget {
  ...
  async reserve() {
    ...
    const used = await this.store.incr(key);
    if (used === null) {
      this.lastReason = 'store_unavailable';
      return { ok: !!this.failOpen, reason: 'store_unavailable', used: null, limit: this.limit };
    }
    // The expiry is set on the FIRST increment of the day, so the key cannot outlive its day and
    // cannot accumulate across days if a later expire call fails.
    if (used === 1) await this.store.expire(key, secondsUntilUtcMidnight(this.now()));
    if (used > this.limit) {
...
export function fakeStore(initial = 0) {
  ...
  return {
    ...
    async incr(k) {
      ...
      const next = (map.get(k) === undefined ? initial : map.get(k)) + 1;
      map.set(k, next);
      return next;
    },
