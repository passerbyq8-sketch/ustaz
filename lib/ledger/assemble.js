...
// THE WORDING OF AN ABSENCE IS CHOSEN BY THE REASON, NOT BY THE SITUATION LOOKING SIMILAR.
import { wordingFor, negationAllowed } from '../policy/slot-proof.js';
...
/**
 * THE SENTENCE AN ABSENCE IS ALLOWED TO BE, FOR THESE ISSUES.
 *
 * ── THE DEFECT THIS REPLACES ────────────────────────────────────────────────
 * One constant answered every failure: READER.NO_DIRECT_EVIDENCE, «لم أعثر ضمن المصادر المتاحة
 * على نصٍّ مباشر يثبت هذه النسبة». It was emitted for F6 with brave_calls = 0 — a claim about a
 * search that had not happened — and it was emitted identically when four real queries came back
 * empty. A reader could not tell the two apart, and one of them was false.
 *
 * ── WHAT REPLACES IT ────────────────────────────────────────────────────────
 * The unfilled slots' own proof records choose the wording. Budget exhaustion says the
 * operational limit; a real empty search says so, scoped to our sources; ineligible results say
 * that; and evidence that failed Gate 2 says THAT. If no slot's record shows a search was ever
 * attempted, the only sentence available is the operational one — an absence may not be claimed
 * on work that was never done.
 */
export function negationFor(ledger, issues) {
  const proofs = [];
  for (const i of issues || []) {
    for (const s of ledger.slotsFor(i.issueId)) {
      if (s.status !== 'filled') proofs.push(ledger.slotProof(i.issueId, s.slot));
    }
  }
  if (!proofs.length) return READER.NO_EVIDENCE_GENERAL;
  // A slot that WAS searched carries the more informative fact, so it speaks for the group; with
  // none searched, the honest line is the operational one.
  const searched = proofs.filter((p) => negationAllowed(p));
  if (!searched.length) return wordingFor('NOT_SEARCHED_BUDGET');
  // Most specific reason first: entailment failure says more than an empty result set.
  const order = ['EVIDENCE_NOT_ENTAILED', 'RESULTS_INELIGIBLE', 'SEARCHED_NO_RESULTS', 'AMBIGUOUS_ENTITY'];
  for (const code of order) {
    if (searched.some((p) => p.outcome === code)) return wordingFor(code);
  }
  return wordingFor('SEARCHED_NO_RESULTS');
}
...
export function assemble(ledger, survivingSentences) {
  ...
  // An internally inconsistent ledger may never become an answer, whatever survived. Scoped to
  // the claims that actually BACK the reply: a claim the gates already refused backs nothing, and
  // letting it veto the answer would punish the reader for a defect the gates had already caught.
  const integrity = ledger.integrityProblems('answer');
  if (integrity.length) {
  ...
  if (!kept.length) {
    return {
      ...
      text: negationFor(ledger, ledger.issues),
      cards: [],
      reasons: reasons.length ? reasons : [REJECTION.NO_SUFFICIENT_DIRECT_EVIDENCE],
      slotProofs: ledger.issues.flatMap((i) => ledger.slotProofsFor(i.issueId)),
    };
  }
  ...
  //
  // BOTH KINDS OF SHORTFALL GET A SENTENCE, and each gets the sentence its own proof earns. An
  // issue that produced nothing and an issue that answered the ruling but not «وما نسبته إلى
  // الشيخ؟» are different failures, and reporting them with one constant is what let F6 claim a
  // search it never ran.
  const shortfall = unanswered.concat(shortIssues.filter((i) => !unanswered.includes(i)));
  if (shortfall.length) parts.push(negationFor(ledger, shortfall));
  ...
  return {
    ...
    // THE PROOF TRAVELS WITH THE ANSWER. Counters and codes only — no query text, no page text,
    // no reader identity — so telemetry can show what was looked for without retaining anything.
    slotProofs: ledger.issues.flatMap((i) => ledger.slotProofsFor(i.issueId)),
  };
}
