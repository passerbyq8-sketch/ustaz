// lib/ledger/flag.js
// THE SWITCH. The current code default is PUBLIC; explicit floors, modes and the store brake can
// still select legacy without changing or redeploying this module.
//
...
//
// ── FAILURE NEVER GRANTS; ABSENCE DOES NOT CANCEL THE PUBLIC DEFAULT ──────────
// A written malformed/off store value engages the brake. An absent, unreachable or slow store
// cannot grant anything, but under PUBLIC_GO_LIVE it also does not invent a brake: the environment
// remains authoritative. The credential and stored-on value govern only `internal` and the
// rollback `unset` arm restored when PUBLIC_GO_LIVE is false.
//
...
//   * every ledger budget receives the finite default ceiling unless the environment overrides it.
...
// ── THE THREE MODES ──────────────────────────────────────────────────────────
//
// A FALSE «MEASURED» CLAIM STOOD HERE, AND IT WAS LOAD-BEARING (corrected 2026-08-07).
//
// The sentence this replaces read: «MEASURED, NOT ASSUMED: the store credential for this project
// is write-only — every secret reads back empty — so the runtime value this switch was built
// around cannot be written at all.» It was stamped MEASURED and it was never measured. It is the
// premise that carried the go-live out of the store and into the code constant above, and two
// other files copied it verbatim (guards/rfc-v05r2-mode-guard.cjs, recon-audit.cjs) — so one
// unmeasured sentence became three, all of them citing each other.
//
// WHAT WAS ACTUALLY MEASURED, on 2026-08-07, against this project's live store:
//
//   PING                            -> PONG
//   DBSIZE                          -> 66
//   GET lg:flag:ledger_rag_enabled  -> null          (never written, not unreadable)
//   GET lg:t:* · legacy lg:dsb:* · lg:p:*  -> live records
//
// So READS WORK. The credential is not write-only and secrets do not read back empty; the claim
// was false about the only half of it anybody had checked. What remains TRUE is narrower and is
// the whole of it: the runtime value has never been written, and nothing here has ever attempted
// a WRITE — so «can this project write it» is UNMEASURED, not impossible. The `null` above is an
// absence, and the old comment read an absence as an incapacity.
//
// THE OWNER'S DECISION IS UNAFFECTED, and that is why this is a comment change and nothing else.
// PUBLIC_GO_LIVE was an owner decision of 2026-08-05, not an inference from this paragraph; the
// environment carries the activation authority and the store is an optional brake because the
// owner said so. The correction removes a false justification, not the thing it justified.
//
//   unset     public while PUBLIC_GO_LIVE is true; credential + store value when it is false.
...
export async function readRuntimeFlag(now = Date.now()) {
  ...
  // struggling". Public mode keeps the environment in charge; the rollback arm fails closed.
...
export async function decidePath(req, now = Date.now()) {
  ...
  if (mode !== 'unset') {
    // THE MODE PATH. The floor above still governs — a mode alone activates nothing. The daily
    // paid search is capped downstream by the environment-scoped v2 budget and fails closed when
    // either its configuration or backing store is unavailable.
  ...
  // ROLLBACK ARM: reachable only when PUBLIC_GO_LIVE is false. Credential, then store value.
