...
export function searchKey(query, { sites = [], policyVersion = SOURCE_POLICY_VERSION } = {}) {
  const material = normalizeForKey(query) + ' ' + sites.slice().sort().join(',') + ' ' + policyVersion;
...
export async function getSearch(query, opts) {
  ...
  if (!v || typeof v !== 'object' || !Array.isArray(v.results)) return { hit: false, value: null };
  return { hit: true, value: v.results };
}

export async function putSearch(query, results, opts) {
  const k = searchKey(query, opts);
  if (!k) return false;
  return store.setex(k, SEARCH_TTL_SECONDS, { results, v: SOURCE_POLICY_VERSION });
}
