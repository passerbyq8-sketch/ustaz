...
/**
 * Write a record. INTERNAL TESTERS ONLY — the caller decides that, and passes it in; this
 * module never inspects a request. A store outage is not an error: telemetry is never allowed
 * to affect an answer.
 */
export async function record(rec, { internal } = {}) {
  if (!internal) return { written: false, reason: 'not-internal' };
