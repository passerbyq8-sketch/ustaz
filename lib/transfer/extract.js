...
// A frozen, plainly valid islamweb question is 615 characters. The old 600-character ceiling
// silently sliced it and still let the shortened pair transfer. Keep a bounded runaway guard,
// but place it above the measured question and REFUSE overlong pairs instead of rewriting them.
...
// A vocalised label can leave only its final mark and colon after the base letters were consumed:
// almosleh prints «السؤالُ:» / «الإجابةُ:», which previously yielded an answer beginning «ُ:».
// This is deliberately anchored and accepts only Arabic marks/tatweel before the colon, so it
// cannot remove punctuation from the actual answer later in the string.
const stripLabelResidue = (s) => String(s == null ? '' : s)
  .replace(/^[\s\u064B-\u065F\u0670\u0640]*[:：]\s*/u, '');
...
/** Read an HTML fragment supplied as data, preserving its authored block boundaries. */
function htmlFragmentText(raw) {
  let document;
  try { ({ document } = parseHTML('<html><body><div id="transfer-field">' + String(raw || '') + '</div></body></html>')); }
  catch { return ''; }
  return blockTextOf(document.querySelector('#transfer-field'));
}
...
export const EXTRACTORS = Object.freeze({
  // MEASURED on the frozen page: the question and accepted answer are separate `.quest-fatwa`
  // boxes, and the answer text has schema.org's `itemprop="text"`. Reading those boxes is what
  // stops at «والله أعلم.»; a running-text label reader continued into navigation and Indonesia.
  // No generic fallback: a template without these bounds is unreadable, not permission to guess.
  'islamweb.net': (doc) => {
    const boxes = Array.from(doc.querySelectorAll('.mainitem.quest-fatwa'));
    const aBox = boxes.find((el) => String(el.getAttribute('itemprop') || '') === 'acceptedAnswer');
    const qBox = boxes.find((el) => el !== aBox);
    const qChildren = qBox ? Array.from(qBox.children || []) : [];
    const qEl = qChildren.length ? qChildren[qChildren.length - 1] : null;
    const aEl = aBox && aBox.querySelector('[itemprop="text"]');
    if (!qEl || !aEl) return null;
    const question = collapse(qEl.textContent || '');
    const answer = tidy(blockTextOf(aEl));
    if (!question || answer.length < MIN_ANSWER_CHARS) return null;
    return { question, answer };
  },
  ...
  'sh-albarrak.com': (doc) => {
    ...
    if (!post) return null;
    const question = collapse(htmlFragmentText(post.question));
    const answer = tidy(htmlFragmentText(post.content));
    if (!question || answer.length < MIN_ANSWER_CHARS) return null;
    return { question, answer };
  },
  ...
  'almosleh.com': (doc) => {
    ...
    const question = collapse(stripLabelResidue(body.slice(qIdx + qLen, am.index)));
    const answer = tidy(stripLabelResidue(body.slice(am.index + am[0].length)));
    if (!question || answer.length < MIN_ANSWER_CHARS) return null;
    return { question, answer };
  },
});
...
export function extractPair(url, html) {
  ...
  if (!pair) return null;
  const question = collapse(pair.question);
  const answer = tidy(pair.answer);
  // A question that ran away and swallowed the answer is not shortened into a different question.
  // Refuse the pair; the ordinary generated path remains available to the caller.
  if (question.length > MAX_QUESTION_CHARS || !normalizeArabic(question) || answer.length < MIN_ANSWER_CHARS) return null;
