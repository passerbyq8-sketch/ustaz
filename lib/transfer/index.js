 ...
 * @param {{judge?:Function, maxChars?:number, band?:string}} deps
...
export async function considerTransferPair(readerQuestion, page, deps = {}) {
  const { judge = null, maxChars = 2400, band } = deps;
  const no = (reason, extra) => ({ transfer: false, reason, score: 0, flips: [], ...(extra || {}) });

  if (!readerQuestion || !page || !page.url) return no('no-page');

  // Verbatim transfer carries the publisher's complete register. The existing generated path is
  // what adapts a ruling to a younger reader; no content classifier exists here that could prove a
  // particular published page age-suitable. Therefore only the actual adult retrieval band opens
  // transfer. Missing/young/teen values fail closed to generation, without removing the source
  // from retrieval or changing its page gate.
  if (band !== 'adult') return no('audience-ineligible', { band: band || 'unknown' });
  ...
  if (!prepared.text) {
    return {
      transfer: false,
      reason: prepared.rejected ? 'cannot-fit-coherent-transfer' : 'nothing-left-after-trim',
      judged,
      ...base,
    };
  }
