...
export function compareQuestions(readerRaw, publishedRaw) {
  ...
  if (flips.length) {
    return {
      verdict: score >= JUDGE_BAND[0] ? TRANSFER.JUDGE : TRANSFER.NO,
...
// FIRST carries a fiqh qualifier the second does not, which is a question about a specific,
// checkable difference.
export const JUDGE_QUESTION = 'هل في الأوّلِ قيدٌ فقهيٌّ ليس في الثاني؟';
