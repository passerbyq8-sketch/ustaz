...
function lex(text) {
  ...
  const w = new Set();
  const raw = new Set();          // tokens as written, WITHOUT prefix/suffix folding
  for (const tok of t.split(' ')) {
    if (!tok) continue;
    raw.add(tok);
    for (const f of foldForms(tok)) w.add(f);
  }
  return { p: ' ' + t + ' ', w, raw, empty: !t };
}
...
const FATWA_VERDICTS = N([
  ...
  'باطل', 'صحيح شرعا', 'لا يجوز', 'بدعه', 'معصيه', 'اثم', 'ذنب',
]);
// VERDICT WORDS THAT ARE ALSO ORDINARY NOUNS, matched WITHOUT the prefix folding the rest
// of this file uses. «مشروع» is the verdict "legitimate"; «المشروع» is "the project", and
// the folder strips the article, so the two were the same token and «ما أحدث أخبار المشروع»
// classified as a ruling question. Requiring the bare, article-less form separates them:
// the verdict sense is a predicate («هل هذا مشروع؟») and never carries the definite article,
// while the noun sense in this collision almost always does.
//
// Same family as the «الذهب»/«ذهب» defect in lib/attribution.js: a fiqh term that is also an
// everyday word. Other members of that family are noted in the report; they fail towards
// `fatwa`, which only ever costs the scope-restricted sources, so none of them is urgent.
const FATWA_VERDICTS_EXACT = N(['مشروع']);
...
// ── HADITH ───────────────────────────────────────────────────────────────────
//
// TWO FAMILIES, AND THE FIRST VERSION OF THIS FILE ONLY HAD ONE.
//
// (a) AUTHENTICATION / GRADING / TAKHRIJ. This list was copied in spirit from
//     lib/source-intent.js's dorar vocabulary, and that was the mistake: the two files
//     answer DIFFERENT questions. There, the question is "is the reader asking for a
//     GRADING?", so bare «حديث» is deliberately excluded — it would hijack every ordinary
//     question to the takhrij path. Here the question is "is this a hadith question AT
//     ALL?", and inheriting that exclusion left the entire explain-a-hadith family
//     classified as `general`.
//
//     MEASURED: «اشرح حديث إنما الأعمال بالنيات» and «حديث من موقع الشيخ عبدالمحسن العباد»
//     both came out `general`, so the hadith sources were never ranked first and the
//     scope-restricted general sources were never dropped.
//
// (b) THE SHAPE THAT NAMES A HADITH. Added here, and deliberately as PHRASES rather than
//     the bare word. «حديث» is also the ordinary Arabic adjective for "modern", and a bare
//     trigger would classify «هذا جهاز حديث» as a prophetic report. Every entry below pins
//     the noun sense by what sits next to it: a verb of explanation, a request, an
//     attribution to the Prophet, or a following «عن». The adjective sense never appears in
//     those frames, and the plural «أحاديث» has no adjective sense at all.
const HADITH_TERMS = N([
  // (a) authentication / grading / takhrij
  ...
  // (b) the hadith NAMED, explained, quoted or asked for
  'شرح حديث', 'اشرح حديث', 'اشرح لي حديث', 'شرح الحديث', 'اشرح الحديث',
  'معني حديث', 'ما معني حديث', 'معني الحديث', 'ما معني الحديث', 'مامعني حديث',
  'صحه حديث', 'ما صحه حديث', 'درجه حديث', 'ما درجه حديث',
  'حديث عن', 'حديثا عن', 'حديث في', 'حديثا في',
  'اذكر حديث', 'اذكر حديثا', 'اذكر لي حديث', 'هات حديث', 'اريد حديث',
  'حديث من موقع', 'حديث من كتاب', 'حديث النبي', 'حديث رسول الله', 'الحديث النبوي',
  'احاديث', 'الاحاديث', 'حديث شريف', 'الحديث الشريف',
]);
...
export function classifyPurpose(query) {
  ...
  if (hit(p, w, FATWA_ASKS) || hit(p, w, FATWA_VERDICTS) || hit(p, w, FATWA_TOPICS)
    || FATWA_VERDICTS_EXACT.some((t) => raw.has(t))) return 'fatwa';
