...
export function isBareAttributionTitle(raw) {
  return BARE_TITLE_CAPTURE.test(norm(raw));
}
...
function detectorNorm(raw) {
  ...
  // sentinel. Escape a reader's literal spelling first: content must never disappear merely
  // because it happens to equal an internal marker. The appended suffix keeps it one visible
  // content token, so word ordinals remain stable while total-tail proof correctly fails closed.
  const escaped = String(raw == null ? '' : raw).toLowerCase()
    .replaceAll(CLAUSE_SENTINEL, CLAUSE_SENTINEL + 'userword')
    // Quotation delimiters are owned by claim-gate and may wrap a governed name without
    // starting a new attribution clause («ما رأي «صالح»، ...»). Remove their delimiter role
    // before turning the remaining punctuation/symbols into structural sentinels.
    .replace(/[\p{Pi}\p{Pf}"']/gu, ' ');
  return normalizeArabic(norm(escaped
    .replace(/[\p{P}\p{S}]+/gu, ' ' + CLAUSE_SENTINEL + ' ')));
}
...
// AW() already declares that و/ف may coordinate an attribution head. Canonicalize only that
// grammar-owned optional prefix and only when what remains is an exact declared head. This makes
// «وعن» the same relation as «عن» and «وروي» the same narration head as «روي», without suffix
// guesses or another vocabulary.
function canonicalCaptureHead(raw) {
  const token = norm(raw).split(' ')[0] || '';
  if (ATTRIBUTION_SPEECH_HEADS.includes(token)) return token;
  if (token.length > 1 && (token[0] === norm('و') || token[0] === norm('ف'))) {
    const coordinated = token.slice(1);
    if (ATTRIBUTION_SPEECH_HEADS.includes(coordinated)) return coordinated;
  }
  return token;
}

function captureHeadWasCoordinated(raw) {
  const token = norm(raw).split(' ')[0] || '';
  if (token.length <= 1 || (token[0] !== norm('و') && token[0] !== norm('ف'))) return false;
  return ATTRIBUTION_SPEECH_HEADS.includes(token.slice(1));
}
...
function captureRecord(normalizedQuestion, match, priority, intent) {
  ...
  const beforeFrame = normalizedQuestion.slice(0, Math.max(0, match.index)).trim();
  return {
    index: match.index,
    priority,
    intent,
    raw,
    frameText: match[0],
    // This is the head selected by the existing attribution grammar, not a second classifier.
    // In particular, «عن» is a relation to somebody, while «قال/قول» governs the speaker.
    head: canonicalCaptureHead(match[0]),
    headCoordinated: captureHeadWasCoordinated(match[0]),
    // Preserve the immediately adjacent detector token, including its structural punctuation
    // sentinel. Word ordinals intentionally omit that sentinel, so they cannot prove that
    // «رُوي» and «عن النبي» belong to one uninterrupted narration frame.
    previousDetectorToken: canonicalCaptureHead(
      beforeFrame.split(' ').filter(Boolean).at(-1) || ''),
...
export function detectAttribution(messages, context = {}) {
  ...
  const structuredSubject = context && context.claimSubject;
  const quoteCharStart = structuredSubject && structuredSubject.source === 'quote'
    && structuredSubject.specific === true && Number.isInteger(structuredSubject.quoteCharStart)
    ? structuredSubject.quoteCharStart : -1;
  // claim-gate owns the raw quote boundary; attribution converts that byte/character boundary
  // with its own detector normalizer so attached punctuation cannot put the two modules in
  // different word-coordinate systems.
  const quotePrefix = quoteCharStart >= 0
    ? detectorNorm(question.slice(0, quoteCharStart)) : '';
  const quoteWordStart = quoteCharStart >= 0
    ? wordOrdinalAt(quotePrefix, quotePrefix.length) : -1;
  const subjectWordCount = structuredSubject && typeof structuredSubject.subject === 'string'
    ? norm(structuredSubject.subject).split(' ').filter(Boolean).length : 0;
  const quoteWordEnd = quoteWordStart >= 0 ? quoteWordStart + subjectWordCount : -1;
  const hasQuotedClaimSubject = quoteWordStart >= 0 && subjectWordCount >= 2;
  const governingCandidates = [];
  const humanCandidates = [];
  const detectorWords = n.split(' ').filter((word) => word && word !== CLAUSE_SENTINEL);
  for (const capture of captures) {
    if (isVocativeUtteranceCapture(capture)) continue;
    const name = cleanName(capture.raw);
    // Sacred candidates are recorded BEFORE human-name cleanup. Cleanup correctly removes
    // «الله» as a possible human name, but that must not erase a nearer divine governor and let
    // an earlier Prophetic mention claim the later quotation.
    const sacred = isSacredAttributionCapture(capture.raw, name, {
      frameText: capture.frameText,
      question: n,
    });
    if (sacred) {
      const propheticSpan = propheticSubjectSpan(capture.raw);
      const kind = propheticSpan ? 'prophetic'
        : containsDivineSubject(capture.raw) ? 'divine' : 'sacred';
      const previousWord = capture.previousDetectorToken;
      const narrationHead = capture.head === norm('عن') && !capture.headCoordinated
        ? ATTRIBUTION_SPEECH_HEADS.find((head) => head !== norm('عن')
          && relationReportHead(head) && previousWord === head) || ''
        : '';
      // «عن النبي» is a relation, not authorship. It becomes a governing report only when the
      // immediately preceding token is an exact narration head from the existing attribution
      // vocabulary («رُوي عن»). Suffix matching is forbidden here: «المقال» ends in «قال» and
      // «النص» is itself a noun, but neither makes the subject of an about-page its speaker.
      if (capture.intent !== ATTRIBUTION_INTENT.MATERIAL_FROM_SITE
        && (capture.head !== norm('عن') || narrationHead)) {
        governingCandidates.push({
          kind, ...capture,
          head: narrationHead || capture.head,
          frameWordStart: narrationHead ? capture.frameWordStart - 1 : capture.frameWordStart,
          nameWordCount: norm(name || capture.raw).split(' ').filter(Boolean).length,
          // This end belongs to the detector's declared Prophetic subject, not to its broad
          // name capture. It is the proof boundary used below and is exported with the winner.
          subjectWordEnd: propheticSpan
            ? capture.nameWordStart + propheticSpan.wordStart + propheticSpan.wordCount : null,
        });
      }
      continue;
    }
    ...
    if (isBareAttributionTitle(name)) {
      const result = {
        attributed: false, mode: 'unnamedScholarClaim',
        scholarName: '', entity: '', scholar: null, question,
      };
      const candidate = { kind: 'human-title', ...capture, result };
      humanCandidates.push(candidate);
      if (capture.head !== norm('عن')) governingCandidates.push(candidate);
      continue;
    }
    ...
    if (capture.intent === ATTRIBUTION_INTENT.MATERIAL_FROM_SITE) {
      humanCandidates.push({ kind: 'site', ...capture, result: {
    ...
    const candidate = { kind: 'human', ...capture, result };
    humanCandidates.push(candidate);
    // A bare «عن X» describes material about X; it is not evidence that X spoke the quotation.
    // Keep its historical non-quote attribution behavior, but never offer it as a quote governor.
    if (capture.head !== norm('عن')) governingCandidates.push(candidate);
  }

  // Bind the FIRST quoted subject to the nearest existing attribution governor. A Prophetic
  // winner is deliberately narrower than natural-language inference: after the detector-owned
  // subject span, the entire interval up to the quote must reduce to punctuation/spacing plus an
  // honorific already declared in route-classify's formula registry. Any other word makes the
  // frame UNBOUND. This avoids a second verb/conjunction lexicon and, crucially, never turns a
  // vague «quote + DEEN» signal into hadith. Human and divine candidates still join the same
  // nearest-governor choice, so a later recognized governor beats an earlier mention.
  if (hasQuotedClaimSubject) {
    const inScope = (candidate) => {
      if (candidate.frameWordStart >= quoteWordStart || candidate.nameWordStart >= quoteWordStart) {
        return false;
      }
      if (candidate.kind !== 'prophetic') return true;
      return Number.isInteger(candidate.subjectWordEnd)
        && candidate.subjectWordEnd <= quoteWordStart
        && isPropheticSubjectSequence(
    ...
    const selected = governors[0] || null;
    if (selected && selected.kind === 'prophetic') {
      return { ...none, sacredAttribution: Object.freeze({
        kind: selected.kind, intent: selected.intent, head: selected.head,
        frameWordStart: selected.frameWordStart, nameWordStart: selected.nameWordStart,
        nameWordCount: selected.nameWordCount, subjectWordEnd: selected.subjectWordEnd,
        quoteCharStart, quoteWordStart,
      }) };
    }
    if (selected && selected.result) return selected.result;
    // Quotation marks may delimit an earlier example rather than the claim being attributed.
    // A later governed human capture lives after the closing delimiter and retains the detector's
    // historical behavior; a capture whose name begins inside the quoted words remains excluded.
    const laterHuman = humanCandidates.find((candidate) => candidate.frameWordStart >= quoteWordEnd);
    if (laterHuman) return laterHuman.result;
    return none;
  }

  // With no quotation, preserve the detector's original intent-ranked human result. Sacred
  // candidates are only structured context and never become a human identity.
  if (humanCandidates.length) return humanCandidates[0].result;
