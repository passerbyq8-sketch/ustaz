...
export function verifyAttributedReply(reply, detection, sources) {
  ...
  for (const s of owned) {
    ...
    if (cmp.verdict === 'incompatible') {
      problems.push('duration-mismatch:' + (cmp.question[0] || {}).text + ' vs ' + (cmp.source[0] || {}).text);
    } else if (cmp.verdict === 'unknown') {
      problems.push('question-fixes-a-time-the-source-does-not:' + (cmp.question[0] || {}).text);
    }
  }
