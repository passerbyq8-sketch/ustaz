...
const MARKERS = [
  'نفاس', 'حيض', 'دم فساد', 'تصلي', 'تصوم', 'تقضي', 'تترك الصلاه', 'تترك الصوم',
...
export function polarity(text, marker) {
  ...
  let idx = t.indexOf(' ' + mk);
  if (idx === -1) idx = t.indexOf(mk);
  if (idx === -1) return null;
  const head = t.slice(0, idx);
  const clause = head.split(CLAUSE_BREAK).pop() || '';
  const before = clause.split(' ').filter(Boolean).slice(-3);
  for (const w of before) if (NEGATORS.includes(w)) return 'no';
  return 'yes';
}
...
export function verifyAttributedReply(reply, detection, sources) {
  ...
  //    The trigger is strict and the satisfaction is lenient, on purpose. It fires only when the
  //    question states a period outright, and it is satisfied by ANY mention of time in the
  //    source — "في الشهر الثاني", "أثناء الشهرين" — not only by a number and a unit. A source
  //    that discusses periods at all is discussing the reader's case; one that never mentions
  //    time while she asked about eighty days is not. Erring towards accepting the source keeps
  //    this from refusing faithful answers over a phrasing difference.
  const askedDur = durations(String((detection && detection.question) || ''));
  if (askedDur.length && !mentionsTime(corpus)) {
    problems.push('question-fixes-a-time-the-source-does-not:' + askedDur[0]);
  }
