...
//
// THE LIST IS THE MEASUREMENT AND ONLY THE MEASUREMENT. «الله» is the one divine name that
// appeared as a `claimedAuthority` among the 389; nothing unmeasured is added to it. A bare
// tasbīḥ with no name in front of it — «سبحانه وتعالى»، «عز وجل»، «جل جلاله» — names nobody and
// is deliberately absent. The test is anchored at the HEAD of the capture, so «عبيدالله»، «رضي
// الله عنها» and «توجهوا إلى الله سبحانه فسألوه» are not touched by it, and it tolerates
// diacritics between the letters the way the seventh attribution pattern above does, so «الله
// جلَّ وعلا في آل فرعون» is one of the 21 and not a miss.
const DIVINE_AUTHORITY_NAMES = Object.freeze(['الله']);
const DIVINE_AUTHORITY_HEAD_RE = new RegExp(
  '^(?:[وفبكل])?(?:'
    + DIVINE_AUTHORITY_NAMES.map((name) => Array.from(name).join('\\p{M}*') + '\\p{M}*').join('|')
    + ')(?![\\p{Script=Arabic}\\p{M}])',
  'u',
);
