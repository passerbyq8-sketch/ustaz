...
const CALCULATION_RE = /=|[0-9٠-٩]\s*[×÷]|[×÷]\s*[0-9٠-٩]|[0-9٠-٩]\s*\+\s*[0-9٠-٩]|[0-9٠-٩]\s+[-−–*/]\s+[0-9٠-٩]|(?:^|[\s(])[سص]\s*[+\-−–×÷*/]/u;
...
const NAME_HONORIFIC_TAIL = /^\s*(?:رحمه(?:م|ما|ا)?\s+الله(?:\s+تعالى)?|رضي\s+الله\s+عن(?:هما|هم|ها|ه)|حفظه\s+الله|عليه\s+السلام)/u;
...
const DANGLING_HEAD_RE = /(?:^|\s)(?:في|على|إلى|عن|من|مع|لدى|عند|بحسب|قول|رأي|فتوى|حكم|بين|نحو|حول|لدن)$/u;
const DANGLING_CLAIM_RE = /^(?:رحمه|رحمهم|رحمها|رحمهما|رضي|حفظه|تعالى|عليه\s+السلام|الذي|التي|الذين|اللذين)(?:\s|$)/u;
...
function attributionParts(sentence, attribution) {
  ...
  const head = before
    .replace(/(?:^|\s)(?:ثم|كما|أمّا|أما|وأمّا|وأما)(?=[\s،؛:]*$)/u, '')
    .replace(/[\s:]+$/u, '')
    .trim();
...
function removalBreaksSentence(sentence, attribution) {
  ...
  if (head && DANGLING_HEAD_RE.test(head)) return true;
  return Boolean(claim && DANGLING_CLAIM_RE.test(claim));
}
