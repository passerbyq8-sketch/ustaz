...
function khilafTriggerFor(khilafFromSource, khilafFromOpinions) {
...
export function reviewAnswer({
  ...
  for (const run of splitStructure(text)) {
    ...
    for (const part of sentenceParts(run.text)) {
      ...
      if (scopedDomain === 'fiqh') {
        ...
        // not the sentence's. Disagreement is not inferred from this prose; it is answer-level
        // state derived from the supporting excerpt or supplied opinion metadata below.
        let reviewed = part;
        output.push(reviewed);
  ...
  const notices = [];
  const khilafTrigger = khilafTriggerFor(khilafFromSource, normalizedKhilafFromOpinions);
