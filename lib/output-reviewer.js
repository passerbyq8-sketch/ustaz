...
const KHILAF_MARKERS = /(?:(?:\u0641\u064a(?:\s+\u0647\u0630\u0647)?\s+\u0627\u0644\u0645\u0633\u0627\u0644\u0647|\u0641\u064a\u0647\u0627?|\u062d\u0648\u0644\u0647\u0627?)\s+\u062e\u0644\u0627\u0641|\u062e\u0644\u0627\u0641\s+(?:\u0628\u064a\u0646|\u0644\u062f\u064a)\s+(?:\u0627\u0647\u0644\s+\u0627\u0644\u0639\u0644\u0645|\u0627\u0644\u0639\u0644\u0645\u0627\u0621|\u0627\u0644\u0641\u0642\u0647\u0627\u0621)|(?:\u0645\u0633\u0627\u0644\u0647|\u0627\u0635\u0644)\s+\u062e\u0644\u0627\u0641\u064a\u0647|\u0645\u0648\u0636\u0639\s+\u062e\u0644\u0627\u0641|\u0627\u062e\u062a\u0644\u0641(?:\u0648\u0627)?(?:\s+\u0641\u064a\u0647\u0627?)?\s+(?:\u0627\u0647\u0644\s+\u0627\u0644\u0639\u0644\u0645|\u0627\u0644\u0639\u0644\u0645\u0627\u0621|\u0627\u0644\u0641\u0642\u0647\u0627\u0621)|(?:\u0627\u0644)?\u0642\u0648\u0644(?:\u0627\u0646|\u064a\u0646)|(?:\u0641\u064a\s+(?:\u0647\u0630\u0647\s+)?\u0627\u0644\u0645\u0633\u0627\u0644\u0647|(?:\u0644\u0627\u0647\u0644\s+\u0627\u0644\u0639\u0644\u0645|\u0644\u0644\u0639\u0644\u0645\u0627\u0621|\u0644\u0644\u0641\u0642\u0647\u0627\u0621))\s+\u0627\u0642\u0648\u0627\u0644|\u0627\u0642\u0648\u0627\u0644\s+(?:\u0627\u062e\u0631\u064a|\u0645\u062a\u0639\u062f\u062f\u0647|\u0645\u0634\u0647\u0648\u0631\u0647))/u;
// Model prose keeps the vocabulary shipped at d0907cb, while source excerpts keep the
// independently calibrated constructions above. The literal is copied from that blob verbatim.
const KHILAF_PROSE_MARKERS = /(?:خلاف|اختلف|قولان|أقوال|الراجح|الجمهور|بعض\s+أهل\s+العلم)/u;
const BOUNDED_KHILAF_PROSE_MARKERS = new RegExp(
  `(?<![\\p{Script=Arabic}\\p{M}])(?:[وف])?(?:${KHILAF_PROSE_MARKERS.source})(?![\\p{Script=Arabic}\\p{M}])`,
  KHILAF_PROSE_MARKERS.flags,
);
...
function sourceDeclaresKhilaf(evidence) {
  ...
  return Boolean(excerpt && KHILAF_MARKERS.test(normalizeArabic(excerpt)));
}
...
// `quantity` was catalogued beside `polarity` and `named-answer`, and its ONE recorded hit was a
// MISREADING: the deposit reads «أي ركعتين، لا ركعةً واحدة», where the later clause NEGATES the alternative
// inside one coherent sentence instead of ruling a second time. That «…، لا …» phrasing occurs
// THREE times in EZIK-XRAY-CC-ANSWERS-2026-08-17.txt with the diacritics stripped, and each one is
// a negation. So the shape's real record over the 140 deposited answers is 0 hits and 1 false — the
// ركعة/ركعتان false on battery-Q18, where «وهذه ركعةٌ واحدة» describes the part and «فالحاصل:
// ركعتان» the whole — and the acceptance rule the order wrote for these three shapes (a shape that
// lies more than it hits is dropped, and is not kept to make the count three) drops it. A shape that
// fires only on Arabic negation is a negation detector, not a contradiction detector.
//
// The witness it was built on is KEPT and inverted: `b3a-xray-q12-turn-1-negation-not-quantity`
// proves that «two, not one» yields `detected: false`, and the reviewermatrix mutant
// `contradiction-quantity-reintroduced` puts the shape back verbatim and dies on that same witness.
//
 ...
 *   khilafFromOpinions?: boolean|null, opinionCount?: number|null}} input
 * @returns {{text: string, annotations: Array<object>, verdict: object}}
 */
export function reviewAnswer({
  text, evidence, domain, mode, khilafFromOpinions, opinionCount,
} = {}) {
