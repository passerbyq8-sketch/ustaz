...
const ATTRIBUTION_PATTERNS = Object.freeze([
  /(?<![\p{Script=Arabic}\p{M}])(?:[وف])?(?<frame>(?:قال|ذكر|أفتى|أجاب|يرى|تقول|قالت)\s+(?:(?:فضيلة|سماحة)\s+)?(?:(?:الشيخ|الإمام|العلامة|الدكتور)\s+)?(?<name>[\p{Script=Arabic}][\p{Script=Arabic}\p{M}\s]{1,55}?))\s+(?<connector>إن|أن|بأن)\s+/u,
  /(?<![\p{Script=Arabic}\p{M}])(?:[وف])?(?<frame>(?:قال|ذكر|أفتى|أجاب|يرى|تقول|قالت)\s+(?:(?:فضيلة|سماحة)\s+)?(?:(?:الشيخ|الإمام|العلامة|الدكتور)\s+)?(?<name>[\p{Script=Arabic}][\p{Script=Arabic}\p{M}\s]{1,55}?))\s*[:،]\s*/u,
  ...
  /(?<![\p{Script=Arabic}\p{M}])(?:[وف])?(?<frame>(?<name>[\p{Script=Arabic}][\p{Script=Arabic}\p{M}\s]{1,55}?)\s+(?:[وف])?(?:ق\p{M}*ا\p{M}*ل\p{M}*|ذ\p{M}*ك\p{M}*ر\p{M}*|أ\p{M}*ف\p{M}*ت\p{M}*ى\p{M}*|أ\p{M}*ج\p{M}*ا\p{M}*ب\p{M}*|ي\p{M}*ر\p{M}*ى\p{M}*|ت\p{M}*ق\p{M}*و\p{M}*ل\p{M}*|ق\p{M}*ا\p{M}*ل\p{M}*ت\p{M}*))(?:\s+(?<connector>إن|أن|بأن)\s+|\s*[:،]\s*)/u,
]);
...
const DEPENDENT_OPENING_RE = /^(?:لأن|لأنه|لأنها|لأنهم|لكون|إذ|حيث|بل|ثم|فقد|وذلك|مما|لكن|لكنه)(?![\p{Script=Arabic}\p{M}])/u;
...
export function reviewAnswer({
  ...
  if (!suppressAnswerFooter) {
    ...
    if (sawGeneralStable && !sawBookEvidence(sources)) notices.push(NOTICES.GENERAL_STABLE);
  }
