...
const ATTRIBUTION_PATTERNS = Object.freeze([
  /(?<![\p{Script=Arabic}\p{M}])(?:[وف])?(?<frame>(?:قال|ذكر|أفتى|أجاب|يرى|تقول|قالت)\s+(?:(?:فضيلة|سماحة)\s+)?(?:(?:الشيخ|الإمام|العلامة|الدكتور)\s+)?(?<name>[\p{Script=Arabic}][\p{Script=Arabic}\p{M}]*(?:\s+(?!(?:(?:لا|لم|لن|ليس)\s+)?(?:يحرم|يجوز|يجب|يكره|يستحب|يصح|يبطل|قال|ذكر|أفتى|أجاب|يرى|تقول|قالت)(?![\p{Script=Arabic}\p{M}]))[\p{Script=Arabic}][\p{Script=Arabic}\p{M}]*){0,5}?))\s+(?<connector>إن|أن|بأن)\s+|(?<![\p{Script=Arabic}\p{M}])(?:[وف])?(?<frame>وصف\s+(?:(?:فضيلة|سماحة)\s+)?(?:الشيخ|الإمام|العلامة|الدكتور)\s+(?<name>[\p{Script=Arabic}][\p{Script=Arabic}\p{M}]*(?:\s+(?!(?:هذا|هذه|ذلك|تلك|هؤلاء)(?![\p{Script=Arabic}\p{M}]))[\p{Script=Arabic}][\p{Script=Arabic}\p{M}]*){0,3})\s+(?:[\p{Script=Arabic}][\p{Script=Arabic}\p{M}]*(?:\s+[\p{Script=Arabic}][\p{Script=Arabic}\p{M}]*){0,2}?)\s+بأنه)\s+/u,
