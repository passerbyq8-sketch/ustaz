...
export const RULING_REVIEW_SYSTEM = 'أنتَ فاحصُ أمانةٍ فقهيّة. أمامَك نصوصٌ مرقّمة، وجملٌ مرقّمةٌ من جوابٍ كُتِبَ منها. '
  ...
  + '{"exists":true,"a":{"row":n,"quote":"..."},"b":{"row":m,"quote":"..."}} باقتباسين حرفيَّين، وإلّا {"exists":false}. '
  + 'أخرجْ JSON وحدَه بلا أيِّ كلامٍ قبلَه أو بعدَه: {"claims":[{"id":1,"verdict":"supported","row":3,"quote":"..."}],"khilaf":{"exists":false}}';
...
export function khilafFromReading(raw, rows) {
  ...
  const ra = byRef.get(Number(k.a.row));
  const rb = byRef.get(Number(k.b.row));
...
export async function reviewRulings({ text, rows, ask }) {
  ...
  diagTrace('sentence-door', () => ({
    ...
    claims: claims.map((claim, i) => {
      const v = verdicts[i];
      const row = v ? byRef.get(v.row) : null;
