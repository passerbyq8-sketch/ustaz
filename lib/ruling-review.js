...
//  (e) no item is said twice in the line.
...
export function terminalNotFound(text, extraLines = [], options = {}) {
  ...
  // No repetition (R1): an item already said in a longer item of the line is dropped.
  const rendered = parts.map((p) => (typeof p === 'string' ? p : `${p.base} في: ${p.heads.join('، و')}`));
  const final = rendered.filter((p, i) => !rendered.some((q, j) => j !== i && q.length > p.length && coveredBy(p, q)));
