...
import { topicTerms } from './stored-deen.js';
import {
  FATWA_BASE,
...
function topicalScore(record, context) {
  ...
  const matched = terms.filter((term) => title.includes(term) || question.includes(term) || answer.includes(term));
  const need = terms.length === 1 ? 1 : Math.min(2, terms.length);
  const titleHits = terms.filter((term) => title.includes(term)).length;
  const questionHits = terms.filter((term) => question.includes(term)).length;
  const answerHits = terms.filter((term) => answer.includes(term)).length;
  const proximity = (value) => {
    const words = value.split(/\s+/u).filter(Boolean);
    const positions = [];
    for (const term of terms) {
      const at = words.findIndex((word) => word.includes(term));
      if (at >= 0) positions.push(at);
    }
    return positions.length >= need ? Math.max(...positions) - Math.min(...positions) + 1 : Infinity;
  };
  const titleWindow = proximity(title);
  const questionWindow = proximity(question);
...
export async function searchFatwas(context, options = {}) {
  ...
  const query = searchQueryFor(context);
  if (!query) return { verification, query: '', scholar: named, records: [], calls: 0, rejected: 0 };
  ...
  const payload = await readJson(url.pathname + url.search, options);
  const raw = Array.isArray(payload.results) ? payload.results : [];
  const relevanceContext = { ...context, resolvedTopic: query };
  ...
  return {
    ...
    totalMatches: Number(payload?.pagination?.total || 0),
  };
}

export const __fatwaTest = Object.freeze({ cleanText, normalizeRecord, searchQueryFor, topicalScore });
