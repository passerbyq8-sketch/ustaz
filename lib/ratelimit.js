...
// ── RAISED BY OWNER ORDER, جولة «الاستعادة» §٥ (٢٠٢٦-٠٨-١٦) ──────────────────
// 20/min -> 40/min and 300/day -> 400/day. The measured reason is the free path's shape, not a
// complaint about the old numbers: a tool loop spends several provider calls on ONE reader turn,
// and a reader who is answered, asks a follow-up and then rephrases has spent three turns on a
// single question. The per-minute window is the one that bites there — it is what turns "let me
// ask that a better way" into a refusal — which is why it doubles while the day window moves by
// a third.
//
// THESE ARE PER-IP WINDOWS, NOT THE MONEY CEILING. ASK_GLOBAL_DAY below is untouched, and the
// real spend ceiling remains the Anthropic Console monthly cap. Raising a per-reader window
// changes who gets refused mid-conversation; it does not change what the service can spend in a
// day, which is the number an operator would reach for if that were the worry.
...
// PREVIEW DEPLOYMENTS SKIP THE PER-IP DAY WINDOW -- SOURCES ORDER 2026-09-24 §4-5.
//
// A preview deployment sits behind Vercel's deployment protection, so the only people who can
// reach it are signed-in members of the team, and the day window there refuses the very
// measurement the previews exist for (a battery of a hundred questions from one address is a
// normal afternoon on a preview and an abuse pattern in production). The MINUTE window and the
// GLOBAL day window stay on in both environments; production keeps all three exactly as before.
//
// The environment is read the way lib/free-brain/flag.js:54-58 reads it: `VERCEL_ENV` must say
// preview AND `VERCEL_URL` must be set. Guards set VERCEL_ENV='preview' alone as a budget
// fixture, and that fixture must not switch a per-IP window off under them.
export function previewSkipsDayWindow(env = process.env) {
  const isPreview = String((env && env.VERCEL_ENV) || '') === 'preview';
  const onPlatform = String((env && env.VERCEL_URL) || '').trim() !== '';
  return isPreview && onPlatform;
}

export async function checkAskLimit(ip, env = process.env) {
  try {
    const skipDay = previewSkipsDayWindow(env);
    const [m, d, g] = await Promise.all([
      ASK_WINDOWS.min.limit(ip),
      skipDay ? Promise.resolve({ success: true }) : ASK_WINDOWS.day.limit(ip),
...
// ---------------------------------------------------------------------------
// Sign-in throttle: guards api/auth-start.js, api/auth-return.js and api/auth-exchange.js.
// Two per-IP sliding windows, the same shape as REPORT_WINDOWS above -- and ONE DELIBERATE
// INVERSION of every family that precedes it.
//
// AUTH_FAIL_OPEN IS FALSE. The four families above fail OPEN because refusing a child's question
// when Redis blinks is worse than the traffic that slips through. This one is the opposite trade
// and the reason is whose bill it is: every sign-in start mints a state record, and every return
// makes a TOKEN EXCHANGE AGAINST OUR CLIENT CREDENTIALS at Google or Apple. A throttle that
// fails open on this path is an unmetered flood, run at our expense, against an account we do
// not control the quota of -- and the provider's answer to that flood is to rate-limit or
// suspend the client, which signs out every real reader. A reader who cannot sign in for the
// minute Redis is unreachable can press the button again; a suspended OAuth client cannot be
// pressed back into existence. So this family REFUSES when it cannot count, the way
// lib/attempts.js and lib/daycap.js already do.
//
// AND NO GLOBAL WINDOW, for the reason REPORT_WINDOWS has none: a global ceiling here would
// lock every reader out of sign-in at once the moment it tripped, which is a bigger outage than
// the abuse it would stop. The per-IP pair plus the fail-closed rule is the whole design.
const AUTH_FAIL_OPEN  = false;
const AUTH_PER_IP_MIN = 20;
const AUTH_PER_IP_DAY = 200;

const AUTH_WINDOWS = {
  min: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(AUTH_PER_IP_MIN, '1 m'), prefix: 'auth:ip:min' }),
  day: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(AUTH_PER_IP_DAY, '1 d'), prefix: 'auth:ip:day' }),
};

// Blocks if the per-IP minute OR per-IP day window is exceeded. On Redis error obeys
// AUTH_FAIL_OPEN (FALSE) -- see the note above; this is the one family that refuses.
export async function checkAuthLimit(ip) {
  try {
    const [m, d] = await Promise.all([
      AUTH_WINDOWS.min.limit(ip),
      AUTH_WINDOWS.day.limit(ip),
    ]);
    if (!m.success || !d.success) return { ok: false };
    return { ok: true };
  } catch (e) {
    console.warn(`[ratelimit] auth error, fail-${AUTH_FAIL_OPEN ? 'open' : 'closed'}:`, e && e.message ? e.message : e);
    return { ok: AUTH_FAIL_OPEN };
  }
}

// ---------------------------------------------------------------------------
// Public reading throttle: guards api/articles-list.js and api/articles-get.js.
// Two per-IP sliding windows, the same shape as REPORT_WINDOWS and AUTH_WINDOWS above, and
// NO GLOBAL WINDOW for the reason neither of those has one.
//
// WHY A SIXTH FAMILY AT ALL. These two routes used to drink from AUTH_WINDOWS, so a reader
// who has no account spent the same budget as someone trying passwords against the five
// sign-in routes. One bag for two appetites cannot be tuned: every request of headroom
// handed to a reader is a request of headroom handed to a guesser, so the ceiling could only
// ever be set to what the guesser deserved. A separate family is what makes the reader's
// ceiling a question about reading.
//
// READ_FAIL_OPEN IS TRUE, AND THAT IS THE INVERSION BACK. The family above refuses when it
// cannot count because every sign-in it lets through spends OUR client credentials at Google
// or Apple, and the provider's answer to a flood is to suspend the client. Nothing of the
// sort is true of a page of writing: serving one costs a read. The one thing that makes auth
// fail closed is the one thing that must not apply here, and a reader is not made to answer
// for a fault that is not his.
//
// WHAT FAILING OPEN HERE BUYS, AND WHAT IT DOES NOT. The articles live in the SAME Upstash
// instance as these counters -- lib/articles/store.js and the client above are both built
// from KV_REST_API_URL/KV_REST_API_TOKEN. So when the whole store is unreachable this family
// changes nothing a reader can see: the throttle allows the request, lib/articles/store.js
// then cannot answer it, and that is a 503 where it used to be a 429. The gain is the
// NARROWER fault -- the counter itself stumbling while a plain read still answers -- and
// there a reader now gets his page instead of a refusal he did not earn. That is a smaller
// claim than "the reader stops being shut out", and it is the true one.
const READ_FAIL_OPEN  = true;
const READ_PER_IP_MIN = 120;
const READ_PER_IP_DAY = 3000;

const READ_WINDOWS = {
  min: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(READ_PER_IP_MIN, '1 m'), prefix: 'read:ip:min' }),
  day: new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(READ_PER_IP_DAY, '1 d'), prefix: 'read:ip:day' }),
};

// Blocks if the per-IP minute OR per-IP day window is exceeded. No global window. On Redis
// error obeys READ_FAIL_OPEN (TRUE) -- see the note above; this family allows what it cannot
// count. The return shape is checkAuthLimit's exactly, so a route moving onto this family
// needs no new handling at the call site.
export async function checkReadLimit(ip) {
  try {
    const [m, d] = await Promise.all([
      READ_WINDOWS.min.limit(ip),
      READ_WINDOWS.day.limit(ip),
    ]);
    if (!m.success || !d.success) {
      // THE REFUSED READER IS TOLD WHEN TO COME BACK, AND IT COSTS NOTHING TO TELL HIM. Every
      // verdict the limiter returns already carries `reset`, a unix timestamp in milliseconds.
      // It was being thrown away, so a reader who hit the ceiling got a 429 with no Retry-After
      // and no way to know whether to try in a second or an hour. Reading it out of the answer
      // we already have adds no store call, no dependency and no environment variable.
      //
      // ADDITIVE, AND ONLY HERE. checkAuthLimit's shape is unchanged and this is not offered on
      // the allowed path: there is nothing to wait for there, and a field that reads like a wait
      // but is not one is worse than an absent one. When BOTH windows refuse, the LATER reset is
      // the honest answer -- the reader is not free until both are. A limiter that supplied no
      // usable reset simply yields { ok: false }, exactly as before, and the call sites check.
      const resets = [!m.success ? m.reset : null, !d.success ? d.reset : null]
        .filter((v) => typeof v === 'number' && Number.isFinite(v));
      return resets.length ? { ok: false, reset: Math.max(...resets) } : { ok: false };
    }
    return { ok: true };
  } catch (e) {
    console.warn(`[ratelimit] read error, fail-${READ_FAIL_OPEN ? 'open' : 'closed'}:`, e && e.message ? e.message : e);
    return { ok: READ_FAIL_OPEN };
  }
}
