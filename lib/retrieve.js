...
// `pathHint` narrows the operator to a sub-path (e.g. 'site:shamela.ws/book') for a
// targeted pass whose useful content lives under one path. Empty for every ordinary
// pass, so the general search's query is byte-identical to what it has always been.
...
//
// shamela.ws: ask Brave for the book path specifically, ask for more candidates, and
// keep only real book PAGES. The three work together and none is sufficient alone --
//   * the path hint biases the ranker towards /book/... instead of /category/...;
//   * 10 results (still ONE Brave request -- `count` is a page-size, not a call count)
//     make it likely that at least one real /book/{id}/{page} URL is in the list at all;
//   * the filter is the guarantee: even if Brave ignores the path hint entirely, a
//     non-citable URL still cannot reach a fetch slot.
// The fetch fan-out itself is NOT widened: FETCH_PER_CALL is still 2 per wave and the
// targeted pass still runs a single wave, so at most two pages are ever fetched.
