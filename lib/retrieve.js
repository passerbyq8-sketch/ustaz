...
async function runSearchPass(
  ...
  return { text, sources: kept.map((k) => ({ title: k.title, url: k.url, passage: k.passage })) };
}
...
export async function retrieve(query, opts = {}) {
  ...
  if (intent) {
    ...
    // so a minor can never be routed to it however the query is worded.
    ...
    if (!bandList.includes(intent.domain)) {
      ...
      const targeted = await runSearchPass(query, [intent.domain], { ...opts, maxWaves: 1 });
