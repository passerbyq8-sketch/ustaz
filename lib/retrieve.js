...
import { domainsForWorld, filterSitesForPurpose, isVideoAnswerDomain } from './source-registry.js';
...
import { DailySearchBudget, SERVICE_LIMITED } from './ledger/daily-budget.js';
...
async function runSearchPass(
  ...
  for (const group of plan.groups) {
    ...
    if (dailyBudget) {
      let reservation = { ok: false, reason: 'store_unavailable' };
      try { reservation = await dailyBudget.reserve(); }
      catch (error) {
        console.warn(`[retrieve] daily search reservation failed before group ${group.index}: ${error.message}`);
      }
      if (!reservation || !reservation.ok) {
        console.warn(`[retrieve] no daily search unit reserved before group ${group.index} (${reservation && reservation.reason ? reservation.reason : 'store_unavailable'})`);
        if (diagnostics) {
...
async function searchOpen(q, num, safesearch) {
...
export async function retrieveOpenWorld(query, opts = {}) {
  const { band, dailyBudget = null, maxResults = 6 } = opts;

  // ── THE DAY'S CEILING, RESERVED BEFORE THE REQUEST LEAVES ──────────────────
  //
  // «يستهلك من سقف Brave اليومي القائم — لا سقف جديد.» This is that ceiling and not a second one:
  // the same lib/ledger/daily-budget.js, the same global `lg:dsb:YYYY-MM-DD` key, the same limit
  // and the same TTL. Nothing new is counted and no new number is introduced.
  //
  // A MEASURED FACT WORTH RECORDING: that ceiling has been reserved against ONLY by the ledger
  // engine. The legacy path — which is the path this function runs on — has never counted a
  // single Brave call against it. So this is the first legacy search that pays for itself, and
  // the direction is the safe one: the open search is precisely the one with no domain list
  // bounding how often it is worth making.
  //
  // A REFUSED RESERVATION IS A FALL-THROUGH, NOT A REFUSAL. Returning no sources leaves the
  // caller on the ordinary general route, exactly as a missing key or an empty result does.
  // Failing CLOSED on an unreachable store is this module's own default and it is right here for
  // both reasons at once: an uncounted spend is a bill, and the reader loses only the live facts,
  // never the answer.
  if (dailyBudget) {
    let res = { ok: false, reason: 'threw' };
    try { res = await dailyBudget.reserve(); }
    catch (e) { console.warn('[retrieve/open] budget reserve threw:', e.message); }
    if (!res.ok) {
      console.warn(`[retrieve/open] no search unit reserved (${res.reason}) — not searching`);
  ...
  let results = [];
  try {
    results = await searchOpen(query, maxResults, safesearch);
  } catch (e) {
