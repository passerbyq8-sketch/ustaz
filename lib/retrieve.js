...
export async function fetchAndClean(url, timeoutMs) {
  ...
  try {
    res = await fetch(url, {
      headers: {
        // D6أ: was a desktop-Chrome string. A server claiming to be a person at a keyboard
        // takes the allow/throttle/block decision away from the site's operator, which is the
        // one thing the rest of this file is careful never to do. Imported, never retyped.
