...
async function runSearchPass(
  ...
  // its host. Defaults to off, so every ordinary pass is unchanged.
  {
    ...
    // TEMP DIAGNOSTIC (shamela only) -- remove with the block in retrieve()/api/ask.js.
    diagBox = null,
  } = {}
) {
  let results = [];
  ...
  if (diagBox) diagBox.brave = results.length;
  if (candidateFilter) {
    ...
    if (diagBox) diagBox.filtered = results.length;
    if (results.length === 0) {
      console.warn('[retrieve] no citable candidates after filter');
      if (diagBox) diagBox.stop = 'no-citable-candidates';
      return { text: NO_SOURCE_TEXT, sources: [] };
    }
  }
  ...
  const tryWave = async (waveIn) => {
    ...
    for (let i = 0; i < wave.length; i++) {
      ...
      // TEMP DIAGNOSTIC: path + gate verdict only. Never the query, never page content.
      if (diagBox) {
        (diagBox.fetches || (diagBox.fetches = [])).push(
          `${(() => { try { return new URL(finalUrl || r.link).pathname; } catch { return '?'; } })()}=${note}/${(text || '').length}`
        );
      }
...
export async function retrieve(query, opts = {}) {
  ...
  // TEMP DIAGNOSTIC (remove me): records ONLY the routing label plus, for the Shamela
  // path, candidate counts and gate verdicts. No key, no query, no page content.
  const diag = { route: intent ? intent.domain : 'none', why: intent ? intent.reason : '-' };
  const shamelaPath = intent && intent.domain === 'shamela.ws';
  if (intent) {
    ...
    if (!bandList.includes(intent.domain)) {
      ...
      diag.stop = 'breaker-open';
    } else {
      console.warn(`[retrieve] targeted ${intent.domain} (${intent.reason})`);
      const box = shamelaPath ? {} : null;
      const targeted = await runSearchPass(query, [intent.domain], {
        ...opts, maxWaves: 1, ...(TARGETED_TUNING[intent.domain] || {}), diagBox: box,
      });
      if (box) Object.assign(diag, box);
      if (targeted.sources.length > 0) return { ...targeted, diag };
      console.warn(`[retrieve] targeted ${intent.domain} empty — falling back to general search`);
      diag.result = 'targeted-empty';
    }
  }
  ...
  for (let t = 0; t < tiers.length; t++) {
    ...
    if (pass.sources.length > 0) return { ...pass, diag }; // first tier with a real source wins
  }
