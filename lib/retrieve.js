...
import { domainsForWorld, filterSitesForPurpose, isVideoAnswerDomain, findSource } from './source-registry.js';
import { extractPair, transferableDomains } from './transfer/extract.js';

// The hosts lib/transfer/extract.js can read, as a Set, so a page from anywhere else costs one
// lookup rather than a parse. Built once.
const TRANSFERABLE = new Set(transferableDomains());

/** The published {question, answer} of a page, or null. Cheap host test first. */
function publishedPair(url, html) {
  if (!html) return null;
  let host = '';
  try { host = new URL(url).hostname.replace(/^www\./, ''); } catch { return null; }
  if (!TRANSFERABLE.has(host) && ![...TRANSFERABLE].some((d) => host.endsWith('.' + d))) return null;
  try { return extractPair(url, html); } catch { return null; }
}
...
// D6أ: the ONE name every fetcher this app owns sends. See lib/user-agent.js for the four
// different answers that used to exist and why two of them were untrue.
import { EZIK_USER_AGENT } from './user-agent.js';
// D7: the per-host fetch allowance. A measured exception to the shared budget, or the shared
// budget unchanged — see lib/domain-budget.js for the numbers each entry rests on.
import { fetchTimeoutFor } from './domain-budget.js';
...
import { BUDGET_REASON, DailySearchBudget, SERVICE_LIMITED } from './ledger/daily-budget.js';
import { safeFetch } from './ledger/safe-fetch.js';
import { createHash } from 'node:crypto';

export const WORLD_RETRIEVAL_OUTCOME = Object.freeze({
  FOUND: 'FOUND',
...
// The shared runtime allowance for both the provider call and each ordinary page fetch.
// Exported so measurements and guards can compare against the number the request path uses.
export const DEFAULT_FETCH_TIMEOUT_MS = 8000;

class RetrievalDiagnosticError extends Error {
  constructor(reason, message) {
    super(message || reason);
    this.name = 'RetrievalDiagnosticError';
    this.reason = reason;
  }
}

function newWorldDiagnostics() {
  return {
    reasons: new Set(),
...
function diagnosticReason(error, scope = 'PROVIDER') {
  if (error && error.reason) return error.reason;
  if (error && (error.name === 'AbortError' || error.code === 'ETIMEDOUT')) return scope + '_TIMEOUT';
  return scope + '_TRANSPORT';
}

const PAID_BUDGET_REASONS = new Set([
  BUDGET_REASON.DAY, BUDGET_REASON.CALLER, BUDGET_REASON.STORE,
]);

function isPaidBudgetRefusal(error) {
  return !!(error && PAID_BUDGET_REASONS.has(error.reason));
}

async function reservePaidSearchUnit(dailyBudget, diagnostics, label) {
  if (!dailyBudget || typeof dailyBudget.reserve !== 'function') {
    throw new RetrievalDiagnosticError(BUDGET_REASON.STORE, 'paid-search budget unavailable');
  }
  let reservation = null;
  try { reservation = await dailyBudget.reserve(); } catch { reservation = null; }
  if (reservation && reservation.ok) return reservation;
  const reason = reservation && PAID_BUDGET_REASONS.has(reservation.reason)
    ? reservation.reason : BUDGET_REASON.STORE;
  if (diagnostics) {
    diagnostics.serviceLimited = true;
    diagnostics.reasons.add(SERVICE_LIMITED);
    diagnostics.reasons.add(reason);
  }
  console.warn(`[retrieve] no paid-search unit reserved before ${label} (${reason})`);
  throw new RetrievalDiagnosticError(reason, reason);
}

function finishWorldDiagnostics(state, sources) {
  const d = state || newWorldDiagnostics();
  const hasSources = Array.isArray(sources) && sources.length > 0;
  const operationalFailures = d.search.failed + d.pages.failed + d.breakerSkips;
  let outcome = WORLD_RETRIEVAL_OUTCOME.INCONCLUSIVE;
  if (d.serviceLimited) {
    outcome = SERVICE_LIMITED;
  } else if (hasSources) {
    outcome = operationalFailures
      ? WORLD_RETRIEVAL_OUTCOME.INCONCLUSIVE
      : WORLD_RETRIEVAL_OUTCOME.FOUND;
  } else if (d.search.completed === 0 && d.search.failed > 0) {
    outcome = WORLD_RETRIEVAL_OUTCOME.SEARCH_FAILED;
  } else if (d.pages.attempted > 0 && d.pages.completed === 0
    && d.pages.failed === d.pages.attempted) {
    outcome = WORLD_RETRIEVAL_OUTCOME.SEARCH_FAILED;
  } else if (operationalFailures > 0) {
    outcome = WORLD_RETRIEVAL_OUTCOME.INCONCLUSIVE;
  } else if (d.reasons.has('INJECTION_MARKERS')) {
    // A completed search whose only pages were instruction-bearing did not prove that the
    // sources were empty. It proved only that no page was safe to place in model context.
    outcome = WORLD_RETRIEVAL_OUTCOME.INCONCLUSIVE;
  } else if (d.search.completed > 0) {
    outcome = WORLD_RETRIEVAL_OUTCOME.COMPLETED_EMPTY;
  }
  return {
    outcome,
...
export const SITES_MINOR = [
  ...
  'islamweb.net', 'alukah.net', 'iifa-aifi.org',
...
// lib/source-registry.js owns which rows are world sources. Deriving this list keeps query
// planning and registry admission on one set when a world source is legitimately added or removed.
export const SITES_GENERAL = domainsForWorld();
...
async function searchWeb(
  q, num = 3, label = '', timeoutMs = DEFAULT_FETCH_TIMEOUT_MS,
  transport = globalThis.fetch, dailyBudget = null, diagnostics = null,
) {
  if (!process.env.BRAVE_API_KEY) {
    throw new RetrievalDiagnosticError('NO_PROVIDER_KEY', 'BRAVE_API_KEY missing');
  }
  ...
  if (!isSendable(q)) {
    ...
    throw new RetrievalDiagnosticError('QUERY_NOT_SENT', 'query did not satisfy the provider wire limit');
  }
  // Nothing after this await can refuse locally before transport. A successful reservation is
  // therefore exactly one Brave attempt, including provider timeouts and HTTP failures.
  await reservePaidSearchUnit(dailyBudget, diagnostics, label || 'provider transport');
  ...
  try {
    ...
    try {
      ...
      if (timedOut || (error && (error.name === 'AbortError' || error.code === 'ETIMEDOUT'))) {
        throw new RetrievalDiagnosticError('PROVIDER_TIMEOUT', 'provider search timed out');
      }
      throw new RetrievalDiagnosticError('PROVIDER_TRANSPORT', 'provider search transport failed');
    }
    if (!r.ok) {
      console.warn(`[retrieve] Brave HTTP ${r.status}: ${(await r.text().catch(() => '')).slice(0, 200)}`);
      throw new RetrievalDiagnosticError('PROVIDER_HTTP', `provider returned HTTP ${r.status}`);
    }
    let data;
    try { data = await r.json(); } catch {
      throw new RetrievalDiagnosticError('PROVIDER_PROTOCOL', 'provider returned malformed JSON');
    }
    const items = (data.web && data.web.results) || [];
    if (!Array.isArray(items)) {
      throw new RetrievalDiagnosticError('PROVIDER_PROTOCOL', 'provider response has no result list');
    }
...
export async function fetchAndClean(url, timeoutMs, options = {}) {
  // Use the ledger's single hardened page transport: policy and public-DNS checks happen
  // before a socket opens; redirects are manual and each hop is checked; response type and
  // size are bounded. admitUrl narrows this further to the exact Brave source band.
  const fetched = await safeFetch(url, {
    timeoutMs,
    fetchImpl: options.fetchImpl || globalThis.fetch,
    admitUrl: options.admitUrl,
  });
  if (!fetched.ok) {
    const finalUrl = fetched.fetchedUrl || url;
    const status = Number(fetched.status || 0);
    if (status === 403 || status === 429 || status === 503) {
      tripBreaker(hostname(finalUrl) || hostname(url), `HTTP ${status}`);
    }
    const detail = status ? `HTTP ${status}` : fetched.reason;
    return { title: '', text: '', rawLen: 0, note: `fetch-failed ${detail}`, finalUrl };
  }

  const html = fetched.html;
  const rawLen = fetched.bytes;
  const finalUrl = fetched.fetchedUrl;
  ...
  // ── THE PUBLISHED PAIR, WHILE THE HTML IS STILL HERE (قرار ١) ─────────────
  //
  // Transfer mode needs the page's OWN question, and this is the only place it can be had: the
  // extractors read markup — a Next.js data blob on one host, a submission form to skip on
  // another — and by the time this function returns, the HTML is gone. Re-fetching the page
  // later to read it again would double the cost of every transferable page.
  //
  // BOUNDED BY A HOST CHECK FIRST, so a page from any other domain costs one Set lookup and
  // nothing more. `publishedPair` returns null for everything it cannot read, and every consumer
  // treats it as optional — nothing existing reads this key.
  const published = publishedPair(finalUrl || url, html);
  ...
  if (hasPageRules(finalUrl || url)) {
    ...
    // A PAGE THAT FAILED ITS OWN GATE IS NOT TRANSFERABLE EITHER. The gate is the host's own
    // statement that this page is not an answer, and transfer mode may not overrule it.
  ...
  return { title, text, rawLen, note, finalUrl, published };
}
...
async function runSearchPass(
  ...
  {
    maxResults = 5, perFetchTimeoutMs = DEFAULT_FETCH_TIMEOUT_MS,
    searchTimeoutMs = DEFAULT_FETCH_TIMEOUT_MS,
    maxChars = 2500, band, maxWaves = 2, purpose, diagnostics = null,
    dailyBudget = null, transport = globalThis.fetch,
    pageTransport = globalThis.fetch,
  } = {}
) {
  // ── BOUNDED QUERIES ────────────────────────────────────────────────────────
  ...
  if (!plan.groups.length) {
    ...
    diagnostics?.reasons.add('NO_QUERY_PLAN');
  ...
  // #5: Brave's site: filter is only a soft hint. This exact band's allow-list is
  // enforced before the first connection and before every redirect hop by safeFetch;
  // the final-host check below remains defence in depth.
  const allowSites = sites;
  const admittedPageUrl = (candidate) => {
    try {
      const u = new URL(candidate);
      return u.protocol === 'https:' && hostAllowed(u.hostname, allowSites);
    } catch { return false; }
  };
  ...
  // Count rejected instruction-bearing pages across both waves. The count is returned only as
  // telemetry; none of these pages is allowed to become context or a source card below.
  ...
  const tryWave = async (waveIn) => {
    ...
    const wave = waveIn.filter((r) => {
      if (!admittedPageUrl(r.link)) {
        console.warn(`[retrieve] prefetch-off-list ${String(r.link || '').slice(0, 180)}`);
        diagnostics?.reasons.add('PAGE_OFF_LIST');
        return false;
      }
      if (isBreakerOpen(hostname(r.link))) {
        console.warn(`[retrieve] breaker-skip ${hostname(r.link)} — ${r.link}`);
        if (diagnostics) {
          diagnostics.breakerSkips += 1;
          diagnostics.reasons.add('BREAKER_OPEN');
        }
        return false;
      }
    ...
    let unsure = null;
    // قرار ١٠: a domain that answers in VIDEO can produce a card and can never produce evidence.
    // Held in its own slot, BELOW `unsure`, and returned only when the wave found nothing else:
    // it must never displace a page that actually carries a written answer, because a card with
    // no passage behind it cannot support a single sentence of a reply.
    let videoCard = null;
    // D7: one shared budget, except where a host has been MEASURED and the measurement says
    // otherwise. fetchTimeoutFor returns perFetchTimeoutMs unchanged for every host but the one,
    // so this is not a new policy — it is the same policy with one measured exception in it.
    const settled = await Promise.allSettled(
      wave.map((r) => fetchAndClean(r.link, fetchTimeoutFor(r.link, perFetchTimeoutMs), {
        fetchImpl: pageTransport,
        admitUrl: admittedPageUrl,
      }))
    );
    if (diagnostics) diagnostics.pages.attempted += wave.length;
    for (let i = 0; i < wave.length; i++) {
      ...
      if (outcome.status !== 'fulfilled') {
        ...
        if (diagnostics) {
          const reason = diagnosticReason(outcome.reason, 'PAGE');
          diagnostics.pages.failed += 1;
          if (reason === 'PAGE_TIMEOUT') diagnostics.pages.timedOut += 1;
          diagnostics.reasons.add(reason);
        }
        continue;
      }
      const { title, text, note, finalUrl, author, attributionType, published } = outcome.value;
      if (/^fetch-failed/.test(note)) {
        if (diagnostics) {
          diagnostics.pages.failed += 1;
          diagnostics.reasons.add('PAGE_HTTP');
        }
      } else if (/cloudflare|js-challenge/i.test(note)) {
        if (diagnostics) {
          diagnostics.pages.failed += 1;
          diagnostics.reasons.add('PAGE_CHALLENGE');
        }
      } else if (diagnostics) {
        diagnostics.pages.completed += 1;
      }
      ...
      // A fence changes how the model is asked to read untrusted text; it does not make a page
      // containing model-facing instructions eligible evidence. Scan every attacker-controlled
      // surface that would travel with the candidate, and reject this page alone before the
      // video-card path, page matching, or any model context is constructed.
      const pageMarkers = injectionMarkersIn(
        [title || r.title || finalUrl, finalUrl, text, author, attributionType,
          published && published.question, published && published.answer]
          .filter(Boolean).join('\n')
      );
      if (pageMarkers.length) {
        injectionMarkers.push(...pageMarkers);
        diagnostics?.reasons.add('INJECTION_MARKERS');
        console.warn(`[retrieve] injection-marked ${host} — ${finalUrl} `
          + `(${pageMarkers.length} marker(s)); page refused`);
        continue;
      }
      // ── قرار ١٠: THE ANSWER ON THIS PAGE IS NOT TEXT ───────────────────────
      // MEASURED 2026-08-08: mostafaaladwy.com puts its answer in a YouTube iframe and leaves the
      // text div empty. There is no written answer on the page at all — so the extractor returns
      // the 360-character site footer, byte-identical on every fatwa, which clears the host's own
      // declared minText of 20 and would be handed to the model as the shaykh's ruling.
      //
      // IT RUNS ABOVE THE BLOCKED TEST, and that placement is the decision. The page gate in
      // lib/source-page-gates.js already refuses these pages («a published question with no
      // published answer is not a fatwa»), so by the BLOCKED line they are gone — evidence AND
      // card together. But for THIS domain an unextractable answer is not a failure to read the
      // page, it is what the page IS. The registry knows that and the page gate cannot.
      //
      // The passage is refused; the pointer is kept. The reader is shown where the shaykh
      // actually answered, which is a video.
      if (isVideoAnswerDomain(finalUrl || r.link)) {
        // ...unless we never reached the fatwa at all. A dead fetch or a bot challenge says
        // nothing about the answer's format, and a card for a page nobody could open is a
        // pointer to a guess.
        if (/^fetch-failed/.test(note) || /cloudflare|js-challenge/i.test(note)) {
          console.warn(`[retrieve] video-answer unreachable ${host} — ${finalUrl} (${note})`);
          continue;
        }
        console.warn(`[retrieve] video-answer ${host} — ${finalUrl} (card only; ${text.length} chars of page text refused as evidence)`);
        if (!videoCard) {
          videoCard = {
      ...
      const candidate = {
        ...
        author: author || '',
        attributionType: attributionType || '',
        // قرار ١: the page's OWN question and answer, when it publishes them. Optional everywhere.
    ...
    // A written answer, however unsure, always beats a card with nothing behind it.
    return unsure || videoCard;
  };
  ...
  for (const group of plan.groups) {
    let results = [];
    if (diagnostics) diagnostics.search.planned += 1;
    try {
      results = await searchWeb(group.q, maxResults,
        `group ${group.index}/${plan.groups.length} (${group.sites.length} sites)`, searchTimeoutMs,
        transport, dailyBudget, diagnostics);
      if (diagnostics) diagnostics.search.completed += 1;
    } catch (e) {
      if (isPaidBudgetRefusal(e)) break;
      ...
      if (diagnostics) {
        diagnostics.search.failed += 1;
        diagnostics.reasons.add(diagnosticReason(e, 'PROVIDER'));
      }
      continue;
    }
    if (results.length === 0) {
  ...
  if (kept.length === 0) {
    ...
    return injectionMarkers.length
      ? { text: NO_SOURCE_TEXT, sources: [], injectionMarkers }
      : { text: NO_SOURCE_TEXT, sources: [] };
  }
  ...
  //
  // Instruction-bearing pages were already rejected before candidate admission. The wrapper is
  // still required for every clean page because ordinary source prose remains untrusted data.
  const divider = '\n' + '─'.repeat(40) + '\n';
  // قرار ١٠: a card with no passage contributes NOTHING here — not even its own header line.
  // Emitting «المصدر ١: العنوان — الرابط» with nothing under it would still be putting the page
  // into the evidence, and a header with an empty body reads to the model as a source that was
  // consulted and found silent, which is a different and false claim.
  const evidence = kept.filter((k) => k.passage);
  ...
  return {
    // قرار ١٠: a wave whose only survivor was a video card leaves the model with NOTHING, and it
    // must be TOLD nothing rather than handed an empty string — an empty `text` would read as a
    // page that was fetched and found blank. So the material is the same sentence every other
    // empty result uses, while the card below still travels, which is the whole point: the reader
    // is told no written source was found AND pointed at where the shaykh actually answered.
    text: evidence.length ? material : NO_SOURCE_TEXT,
    ...
    sources: kept.map((k) => ({
      ...
      // Carried so a consumer can tell a card that HAS no passage from one whose passage was
      // dropped — the two look identical downstream and mean different things.
      answerFormat: k.answerFormat || 'text',
...
export async function retrieve(query, opts = {}) {
  const injectionMarkers = [];
  const collectMarkers = (pass) => {
    if (pass && Array.isArray(pass.injectionMarkers)) {
      injectionMarkers.push(...pass.injectionMarkers);
    }
    return pass;
  };
  const withCollectedMarkers = (pass) => injectionMarkers.length
    ? { ...pass, injectionMarkers: injectionMarkers.slice() }
    : pass;
  let rawTiers =
  ...
  if (prefer) {
    if (!tiers[0].includes(prefer)) {
      ...
      const first = collectMarkers(await runSearchPass(
        query, [prefer], { ...opts, purpose, maxWaves: 1 }
      ));
      if (first.sources.length > 0) return withCollectedMarkers(first);
  ...
  if (intent) {
    ...
    if (!bandList.includes(intent.domain)) {
      ...
      const targeted = collectMarkers(await runSearchPass(
        query, [intent.domain], { ...opts, purpose, maxWaves: 1 }
      ));
      if (targeted.sources.length > 0) return withCollectedMarkers(targeted);
  ...
  for (let t = 0; t < tiers.length; t++) {
    const pass = collectMarkers(await runSearchPass(query, tiers[t], { ...opts, purpose }));
    if (pass.sources.length > 0) return withCollectedMarkers(pass); // first tier with a real source wins
  }
  return withCollectedMarkers({ text: NO_SOURCE_TEXT, sources: [] });
}
 ...
 *
 * @returns {{text:string, sources:Array, diagnostics:{outcome:string,reasons:string[],
 *   search:{planned:number,completed:number,failed:number},
 *   pages:{attempted:number,completed:number,failed:number,timedOut:number}}}}
 *   Diagnostics distinguish a completed bounded miss from an operational failure; neither text
 *   nor the accepted source shape changes.
 */
export async function retrieveWorld(query, opts = {}) {
  const diagnostics = newWorldDiagnostics();
  // Every world-provider request pays the canonical daily budget before transport. `null` is not
  // an escape hatch: callers may inject the same reserve() contract for deterministic tests, but
  // an absent or malformed injection falls back to the production DailySearchBudget.
  const dailyBudget = opts.dailyBudget && typeof opts.dailyBudget.reserve === 'function'
    ? opts.dailyBudget
    : new DailySearchBudget();
  ...
  if (conflicts.length) {
    ...
    diagnostics.reasons.add('WORLD_LIST_CONFLICT');
    return {
      text: NO_WORLD_SOURCE_TEXT,
      sources: [],
      diagnostics: finishWorldDiagnostics(diagnostics, []),
    };
  }
  ...
  const resultDiagnostics = finishWorldDiagnostics(diagnostics, pass.sources);
  if (!pass.sources.length) {
    ...
    return {
      text: NO_WORLD_SOURCE_TEXT,
      sources: [],
      ...(Array.isArray(pass.injectionMarkers) && pass.injectionMarkers.length
        ? { injectionMarkers: pass.injectionMarkers }
        : {}),
      diagnostics: resultDiagnostics,
    };
  }
  return { ...pass, diagnostics: resultDiagnostics };
}

// ── THE OPEN WORLD SEARCH — NO `site:` FILTER AT ALL (س٦٫٢) ──────────────────
//
// WHAT THE OWNER ASKED FOR, IN HIS WORDS: «الأسئلة الدنيوية الحية — الطقس، الأسعار، صرف
// العملات، نتائج المباريات وأشباهها — مصدرها بحث مباشر من النت بلا قائمة نطاقات، وقيدها الوحيد
// فلتر الشريعة.»
//
// WHY retrieveWorld() ABOVE COULD NOT SIMPLY BE WIDENED. It searches SITES_GENERAL — four
// domains: Wikipedia, al-Jazeera, the BBC and Sky News Arabia. Those four are an excellent
// answer to «ما آخر أخبار غزة؟» and they are the wrong instrument entirely for «كم سعر صرف
// الدولار مقابل الدينار؟»: none of them publishes a live rate on a page a search will surface,
// so the honest outcome of that search is NO_WORLD_SOURCE_TEXT — a correct answer to a question
// nobody asked. A live quantity is on the open web or it is nowhere.
//
// ── AND THIS IS THE POINT WHERE THIS FILE GIVES UP ITS BEST GUARANTEE ────────
//
// Every retrieval path in this repository rests on ONE mechanism: a hard, post-redirect host
// allow-list applied to every page before a byte of it is read. `site:` is a soft hint from the
// provider; the allow-list is the enforcement. An open search has no allow-list BY DEFINITION —
// that is what «بلا قائمة نطاقات» means — so the mechanism this file trusts most is simply not
// available here, and it would be dishonest to dress something up as its replacement.
//
// SO THE PAGES ARE NOT FETCHED. This function reads the PROVIDER'S OWN RESULT LIST — the title,
// the description and the URL Brave returns — and opens a connection to none of them. That is a
// deliberate narrowing of the owner's spec, and here is the reasoning, stated plainly so he can
// overrule it:
//
//   * an open FETCH is a strictly larger step than an open SEARCH. It would mean this server
//     making requests to hosts nobody has vetted, with no path rules (lib/source-page-gates.js
//     knows nothing about them), no robots-derived refusals, and none of the SSRF address checks
//     lib/ledger/safe-fetch.js applies. Admitting an unbounded search and an unbounded fetcher in
//     one change would be two decisions wearing one;
//   * it is enough for the questions this is FOR. A temperature, an exchange rate, a barrel price
//     and a match score are short facts, and the provider's description field carries them — the
//     whole reason a search engine shows a snippet is that the answer is often inside it;
//   * the material is smaller, so binding the model to it is a smaller problem, which is س٦٫٣'s.
//
// WHAT THAT COSTS, SAID OUT LOUD: a fact that appears only in the body of a page and never in its
// snippet is not reachable this way. That is a real limitation, and it is written into the report
// rather than buried here.
//
// THE SHARIA WALL STILL STANDS, AND IS ENFORCED TWICE. The caller has already run
// classifyWorldIntent(), whose rule 1 refuses anything religious outright. This function then
// refuses to carry a RESULT from any domain on a religious list — because the one thing that must
// never happen is a fatwa site being quoted under news rules, with a news card, selected by a
// provider's relevance score rather than by this app's source policy. A religious question is
// answered from the religious lists by retrieve(), with everything that entails, or it is not
// answered here at all.

/** Registrable hosts that may never be carried by an OPEN world result. */
export function shariaHosts() {
  return new Set([...SITES_ADULT, ...SITES_MINOR, ...SITES_MINOR_FALLBACK]
    .map((d) => String(d).toLowerCase()));
}

// ── AND THE HOSTS THAT MAY NOT CARRY A LIVE *NUMBER* (LIVE_WORLD_V2) ─────────
//
// A NARROWER WALL THAN shariaHosts(), AND A DIFFERENT KIND. That one is about what a source may
// never be; this one is about what a source may never be asked. An encyclopedia is a good answer
// to «من هو…» and «ما عاصمة…» and a bad answer to «كم سعر…»: its number is whatever the last
// editor wrote, carries no quotation date of its own, and reads in an answer exactly like a rate
// quoted this morning. MEASURED in production — the dollar came back from an encyclopedia with no
// date on it at all, which is the defect this exists to close.
//
// IT IS DECIDED BY THE REGISTRY'S `kind` COLUMN AND NOT BY A NAME. No host string appears here.
// A second encyclopedia admitted tomorrow is covered by its row; a news site is not caught by
// accident; and «تبقى مسموحةً لغيرِ ذلك» is enforced by the fact that only a live-number question
// passes `liveNumber: true` at all.
const NO_LIVE_NUMBER_KINDS = new Set(['encyclopedia']);

/** May this host be the source of a LIVE NUMBER (a price, a rate, a moving quantity)? */
export function hostMayCarryLiveNumber(host) {
  const row = findSource(String(host || ''));
  return !(row && NO_LIVE_NUMBER_KINDS.has(row.kind));
}

// SAFESEARCH IS DECIDED BY THE BAND, AND THE DEFAULT IS THE STRICT ONE.
// The owner's rule: «safesearch=strict لغير البالغ (band young/teen أو غائب)، والافتراضي
// للبالغ». Note which way the ABSENT case falls, and that it is the opposite of this app's usual
// treatment of an unknown band. lib/policy/age.js resolves `unknown` to ADULT everywhere else,
// deliberately — «treating an unidentified reader as a child silently degrades every adult who
// has not filled in a profile». That reasoning does not transfer here: there, being wrong costs a
// misidentified adult a simpler ANSWER; here, being wrong costs a child an unfiltered view of the
// open web, and costs the adult a search that filtered out pornography he did not ask for. The
// owner resolved it explicitly, and this is the direction he resolved it in.
export function safesearchFor(band) {
  return band === 'adult' ? 'moderate' : 'strict';
}

/** One open Brave call. No `site:` filter is built, and none may be passed. */
async function searchOpen(q, num, safesearch, dailyBudget) {
  if (!process.env.BRAVE_API_KEY) throw new Error('BRAVE_API_KEY missing');
  const m = measureQuery(q);
  if (!isSendable(q)) {
    console.warn(`[retrieve/open] REFUSING over-long query (${m.chars} chars / ${m.words} words) -- not sent`);
    return [];
  }
  await reservePaidSearchUnit(dailyBudget, null, 'open provider transport');
  const url = 'https://api.search.brave.com/res/v1/web/search'
    + `?q=${encodeURIComponent(q)}&count=${num}`
    + `&safesearch=${encodeURIComponent(safesearch)}`;
  console.warn(`[retrieve/open] brave OPEN ${m.chars}c/${m.words}w safesearch=${safesearch}`);
  ...
  if (!r.ok) {
    console.warn(`[retrieve/open] Brave HTTP ${r.status}: ${(await r.text().catch(() => '')).slice(0, 200)}`);
    return [];
  }
...
const OPEN_SNIPPET_MAX = 400;

/**
 * LIVE WORLD RETRIEVAL OVER THE OPEN WEB.
 *
 * @param {string} query the reader's own words
 * @param {object} opts
 *   band        'young'|'teen'|'adult'|'unknown' — decides safesearch, and nothing else
 *   dailyBudget a DailySearchBudget, or null. See the note on the ceiling below.
 *   maxResults  how many provider results to consider
 * @returns {Promise<{text:string, sources:Array}>} — `sources` is EMPTY when nothing was usable,
 *          and the caller must treat that as "no live material", never as an answer.
 */
export async function retrieveOpenWorld(query, opts = {}) {
  // `liveNumber` says the ANSWER to this question is a number that moves — a price, an exchange
  // rate, a temperature. It narrows nothing else and defaults to false, so every caller that
  // does not pass it gets exactly the behaviour it got before the option existed.
  const { band, dailyBudget = null, maxResults = 6, liveNumber = false } = opts;

  const safesearch = safesearchFor(band);
  let results = [];
  try {
    results = await searchOpen(query, maxResults, safesearch, dailyBudget);
  } catch (e) {
    console.warn('[retrieve/open] search failed:', e.message);
    return { text: NO_WORLD_SOURCE_TEXT, sources: [] };
  }

  const banned = shariaHosts();
  const seen = new Set();
  const kept = [];
  const injectionMarkers = [];
  for (const r of results) {
    ...
    let u;
    try { u = new URL(link); } catch { console.warn('[retrieve/open] drop unparseable url'); continue; }
    // https only, for the reason buildSourceTag insists on it: a card is a link the reader is
    // being invited to open.
    if (u.protocol !== 'https:') { console.warn(`[retrieve/open] drop non-https ${u.protocol}`); continue; }
    const host = u.hostname.toLowerCase().replace(/^www\./, '');
    if (banned.has(host)) {
      // THE WALL, IN THE ONE DIRECTION AN OPEN SEARCH CAN BREACH IT.
      console.warn(`[retrieve/open] REFUSING ${host} — a religious source may not be quoted under news rules`);
      continue;
    }
    if (liveNumber && !hostMayCarryLiveNumber(host)) {
      // Dropped for THIS question only. The same host is admitted without comment on the very
      // next question that is not asking for a moving number.
      console.warn(`[retrieve/open] REFUSING ${host} for a LIVE NUMBER — an encyclopedia figure carries no quotation date`);
      continue;
    }
    const dedup = host + u.pathname;
    if (seen.has(dedup)) continue;
    seen.add(dedup);
    const title = String((r && r.title) || '').replace(/\s+/g, ' ').trim();
    const snippet = String((r && r.snippet) || '')
      .replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, OPEN_SNIPPET_MAX);
    if (!title && !snippet) continue;
    const pageMarkers = injectionMarkersIn(`${title}\n${u.href}\n${snippet}`);
    if (pageMarkers.length) {
      injectionMarkers.push(...pageMarkers);
      console.warn(`[retrieve/open] injection-marked ${host} — ${u.href} `
        + `(${pageMarkers.length} marker(s)); result refused`);
      continue;
    }
    kept.push({ url: u.href, title, snippet, host });
  }

  if (!kept.length) {
    console.warn(`[retrieve/open] nothing usable — ${results.length} provider result(s), 0 kept`);
    return injectionMarkers.length
      ? { text: NO_WORLD_SOURCE_TEXT, sources: [], injectionMarkers }
      : { text: NO_WORLD_SOURCE_TEXT, sources: [] };
  }

  // WRAPPED AS UNTRUSTED DATA, exactly as page text is. A provider's snippet is somebody else's
  // words arriving inside our prompt, which is the injection surface lib/ledger/segment.js exists
  // for. It is not treated more kindly for being short.
  const body = kept
    .map((s, i) => `[${i + 1}] ${s.title}\n${s.host} — ${s.url}\n${s.snippet}`)
    .join('\n\n');
  console.warn(`[retrieve/open] kept ${kept.length} of ${results.length} provider result(s)`);
  return { text: wrapUntrusted(body), sources: kept, injectionMarkers };
}
