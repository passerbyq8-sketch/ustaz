...
import { gateSourcePage, hasPageRules, pathRefusal, declaredMinText, pageKindFromPath } from './source-page-gates.js';
...
export function orderRulingCandidates(results) {
  ...
  return list
    .map((r, i) => ({ r, i, rank: RANK[pageKindFromPath(r && r.link)] ?? 1 }))
...
async function runSearchPass(
  ...
  const tryWave = async (waveIn) => {
    ...
    for (let i = 0; i < wave.length; i++) {
      ...
      const m = matchPage({ question: query, title, text });
      if (m.verdict === 'reject') {
...
export async function retrieveWorld(query, opts = {}) {
  ...
  if (!pass.sources.length) return { text: NO_WORLD_SOURCE_TEXT, sources: [] };
  return pass;
}
