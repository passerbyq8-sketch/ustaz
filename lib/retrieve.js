...
// `pathHint` narrows the operator to a sub-path (e.g. 'site:shamela.ws/book'). Empty for
// every ordinary pass, so the general search's query is byte-identical to what it was.
//
// MEASURED, not assumed: a plain `site:shamela.ws` asking the Brave API for 10 results
// came back with 10 URLs of which ZERO were a citable /book/{id}/{page} page -- the API
// surfaces the home page, /category, /search, /authors and book index pages and stops
// there. The deep pages that actually carry text are only reachable by asking for the
// book path. (An earlier revision dropped this hint on the theory that the API ignored
// it; that test was confounded, because routing was broken at the time and the targeted
// pass never ran at all.)
function siteFilterFor(sites, pathHint = '') {
  return sites.map((s) => 'site:' + s + pathHint).join(' OR ');
}
...
async function searchWeb(query, num = 3, sites, pathHint = '') {
  if (!process.env.BRAVE_API_KEY) {
    throw new Error('BRAVE_API_KEY missing');
  }
  const q = `${query} (${siteFilterFor(sites, pathHint)})`;
...
async function runSearchPass(
  ...
  // sitePathHint / candidateFilter: set only by a targeted pass that knows the shape of a
  // citable URL on its host. Both default to off, so every ordinary pass is unchanged.
  {
    maxResults = 5, perFetchTimeoutMs = 8000, maxChars = 2500, band, maxWaves = 2,
    sitePathHint = '', candidateFilter = null,
  } = {}
) {
  let results = [];
  try {
    results = await searchWeb(query, maxResults, sites, sitePathHint);
  } catch (e) {
...
//
// shamela.ws: ask the BOOK PATH for the maximum number of candidates, then keep only
// real book pages. Driven by a measurement, not a guess -- an instrumented production
// request showed a plain `site:shamela.ws` returning 10 results of which 0 were citable,
// because the API only surfaces the shallow pages of the host.
//   * sitePathHint '/book' is what makes the deep /book/{id}/{page} pages reachable;
//   * 20 results is Brave's per-request maximum and is still ONE request (`count` is a
//     page size, not a call count), so it widens the candidate pool at no extra call --
//     and it is also the hedge: if the API ever ignores the path operator, a wider plain
//     result set is the best remaining chance of containing a deep page;
//   * the filter stays the guarantee: a non-citable URL can never reach a fetch slot.
