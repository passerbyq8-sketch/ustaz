 ...
 * @returns {{domain:string, source:object}|null} null when nobody on the list matches.
 */
export function findScholarDomain(nameRaw) {
  ...
  for (const row of SCHOLAR_SITES) {
    for (const a of row.aliases) {
      const f = foldName(a);
      if (!f) continue;
      if (n === f || n.includes(f) || f.includes(n)) {
        const src = findSource(row.domain);
        // A blocked or unlisted row is not an answer: it must never become a search target.
        if (src && src.status === 'active') return { domain: row.domain, source: src };
        return null;
      }
    }
  }
