...
// The one reader of the 2026-09-19 round's switch. It is consulted in exactly one function
// below — worldSources() — and nowhere else in this file.
...
export const SOURCES = Object.freeze([
  ...
  // ── THE FIFTH WORLD SOURCE, AND A FOURTH STATUS (LIVE_WORLD_V2, 2026-09-19) ─
  //
  // WHY THE OWNER NAMED A CENTRAL BANK. The four rows above are an encyclopedia and three news
  // organisations. Not one of them publishes an exchange rate, and the measured consequence was
  // that «كم سعر صرف الدولار مقابل الدينار؟» came back from an ENCYCLOPEDIA, with no date on the
  // number. A rate is an official fact with an official publisher, and the owner's decision is
  // that the publisher is the Central Bank of Kuwait — not a commercial exchange house, whose
  // rate is its own offer, and not an encyclopedia, whose rate is whenever somebody last edited.
  //
  // `status: 'world-v2'` IS A FOURTH STATUS AND IT IS THE KILL SWITCH'S FOOTPRINT HERE. The row
  // is DECLARED unconditionally — a registry that hides a source when an environment variable is
  // unset is a registry that cannot be reviewed — but worldSources() admits it only while
  // LIVE_WORLD_V2 is on. So with the flag off domainsForWorld() returns the same four domains in
  // the same order it returns today, and lib/retrieve.js's SITES_GENERAL is byte-for-byte what it
  // was. Every other reader of this table is unaffected by construction: activeSources(),
  // blockedSources() and the deferred filter all test a status this row does not have, and
  // sourceAllowsPurpose() refuses any row whose status is not 'active' — which this is not.
  //
  // `scopes: []` says the same refusal a second time, exactly as the four rows above do.
  S({ id: 'cbk',           name: 'بنك الكويت المركزي',                 domain: 'cbk.gov.kw',            kind: 'central-bank',   scopes: [],          bands: ['world'],                  status: 'world-v2',
      note: 'مرجع أسعار الصرف الرسمي (LIVE_WORLD_V2). جداول الأسعار وحدها مقبولة في lib/source-page-gates.js؛ وممنوع أن يُبنى عليه حكم شرعي أو فتوى.' }),
...
// 'world-v2' is the same kind of row behind the LIVE_WORLD_V2 switch. The test is on the STATUS
// and never on an id, so admitting a second flagged world source later costs a row and no code —
// which is the property this whole file exists to keep.
export function worldSources() {
  return SOURCES.filter((s) => s.status === 'world'
    || (s.status === 'world-v2' && liveWorldV2Enabled()));
}
...
export const SCHOLAR_SITES = [
  ...
  { domain: 'dr-mutlaq.com', aliases: [
    ...
    // M1 night 2, task 3 (defect 75): one more two-word spelling readers type. A one-word
    // «مطلق» is deliberately NOT here — it would cross MIN_ALIAS_WORDS_FOR_PARTIAL — and
    // «الدكتور مطلق» is not here either: the reviewer's frame eats the honorific and hands
    // resolveScholar the bare «مطلق», which guards/attribution-capture-widen-guard.cjs
    // measures against the alias's own answer (recorded, defect 75 residue).
    'الشيخ الجاسر',
    // M1+M2 night 3, task 3 (defect 75, the last spelling). «الدكتور مطلق» resolves here now.
    // The reviewer's own frame still eats the honorific and hands resolveScholar the bare
    // «مطلق» in the verb-first plain shape — that is name RESOLUTION and this row does not
    // change it, so it is pinned in guards/attribution-capture-widen-guard.cjs rather than
    // hidden. Bare one-word «مطلق» is still NOT an alias: it is reserved to the owner, and it
    // would cross MIN_ALIAS_WORDS_FOR_PARTIAL.
    'الدكتور مطلق',
  ] },
...
const foldName = (s) => String(s == null ? '' : s)
  .replace(/[ً-ْٰـ]/g, '')
  // M1 night 2, task 3 (defect 75): «د. مطلق» is written with a dot after the abbreviation, and the
  // dot made «د.» a different word from the alias word «د». A full stop is not part of any name.
  .replace(/[.\u06d4]/g, ' ')
