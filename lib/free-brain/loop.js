 ...
 */
/**
 * ── ع-٥٣: ONE BOOK IS ONE CARD, AND THE KEY IS THE BOOK'S OWN ID ────────────
 *
 * MEASURED: the right key EXISTED and was being dropped. `makeAtoms` numbers every atom
 * `subjectId:section:slice` and carries `subject_id` beside it, lib/lib-service.js records it in
 * `record.provenance`, and lib/free-brain/tools.js's row was the last place the two were still
 * together — it kept the title and the author and let the id go. With nothing else to group by,
 * the loop below de-duplicated on the WELDED TAG, and api/ask.js writes the locator and the
 * base64 matn INTO that tag. Three slices of one book therefore differ in every tag they
 * produce, and the reader was shown the same book three times.
 *
 * THE TITLE IS NOT THE KEY AND MUST NEVER BECOME ONE. «صحيح البخاري» returned thirty-three
 * matches in the measured corpus: «an exact title match» is not «this is your book». A title
 * key would merge two DIFFERENT books and call it a repair, which is a worse defect than the
 * one it fixes, because a reader cannot see it.
 *
 * AND `atom_id` IS UNTOUCHED. It is the identity of the SLICE — the thing a citation points at
 * — and grouping is a fact about the book, not about the slice.
 *
 * THE ORDER IS THE CITATION ORDER, and it is the FIRST citation that fixes a book's position:
 * `cited` arrives in delivered-text order, and a card that jumped because its book was quoted
 * again later would move under the reader while they read.
 *
 * @param {Array<object>} cited  rows in delivered-text order — the same array pickReaderCards gets
 * @param {number} max          the ceiling on book chips in one reply
 * @param {(rows: object[]) => ({tag: string}|null)} buildCard  api/ask.js's book-card builder,
 *   handed the whole GROUP: one book, its passages and its places, in citation order
 */
export function pickBookCards(cited, max, buildCard) {
  ...
  for (const row of Array.isArray(cited) ? cited : []) {
    ...
    // AN UNKNOWN ID IS NOT A SHARED ONE. A row that cannot say which book it came from gets a
    // group of its own; folding two unidentified atoms together would be the title test
    // arriving by another route.
    if (!bookId) { groups.push([row]); continue; }
    const seen = byBook.get(bookId);
    if (seen) { seen.push(row); continue; }
    const group = [row];
