...
export async function runFreeBrainTurn(options = {}) {
  ...
  let terminalStreamEligible = false;
  let terminalWriteAdded = false;
  let terminalWriteMs = null;
  let terminalStreamResult = null;
  let finishingDraft = null;
  let finishingDraftStop = null;
  let headFreeAtFinish = false;
  ...
  while (rounds < MAX_TOOL_ROUNDS) {
    ...
    const hadTextBeforeRound = written.length > 0;
    ...
    if (payload.stop_reason !== 'tool_use') {
      finished = roundText !== '';
      if (finished) {
        headFreeAtFinish = !hadTextBeforeRound;
        finishingDraft = roundText;
        finishingDraftStop = payload.stop_reason ?? null;
      }
      break;
    }
  ...
  const preDomain = domainOf(ctx.spend, lexicalRoute);
  const rulingWithoutCards = preDomain !== 'general' && table.rows.length === 0;
  const headFreeBeforeTerminal = finished ? headFreeAtFinish : written.length === 0;
  terminalStreamEligible = streaming.enabled && headFreeBeforeTerminal && !rulingWithoutCards;

  if (streaming.enabled && !headFreeBeforeTerminal) {
    ctx.degraded.push('stream_withheld:head_text');
  }
  if (streaming.enabled && rulingWithoutCards) {
    ctx.degraded.push(`stream_withheld:ruling_without_cards:${preDomain}`);
  }

  let needsTerminalWrite = false;
  if (!finished) {
    // Keep the standing E4 seam literal: a silent finishing round still needs the tools-removed
    // write even when an earlier round put prose in `written`.
    needsTerminalWrite = true;
  } else if (terminalStreamEligible) {
    needsTerminalWrite = true;
    terminalWriteAdded = true;
    const removed = written.pop();
    if (removed !== finishingDraft) {
      if (removed) written.push(removed);
      terminalStreamEligible = false;
      terminalWriteAdded = false;
      needsTerminalWrite = false;
      ctx.degraded.push('stream_withheld:finishing_draft_mismatch');
    }
  }

  if (needsTerminalWrite) {
    let liveUnits = null;
    const writeStartedAt = Date.now();
    try {
      // ── ح‑١: ONLY THIS CALL IS EVER STREAMED (§٢) ──────────────────────────
      // The tool rounds above are NOT, and that is a decision rather than an
      // omission: a tool round produces no text for the reader, so streaming it
      // would add a second code path to the request that carries the retrieval and
      // buy nothing. This is the writing call, and it is the only one whose bytes a
      // reader is ever shown.
      //
      // The flag decides which of the two functions runs. Everything after this
      // point reads `payload` and cannot tell them apart.
      //
      // ── §٤ — A RULING WITH ZERO CARDS IS NOT STREAMED ──────────────────────
      //
      // THE CITATION TEST MOVES IN FRONT OF THE GENERATION, which is the whole of the
      // `cite-retry` decision recorded in phase two. Running it afterwards is the one thing
      // the streaming path may not do: a retry adopted after bytes have left REPLACES text
      // the reader already has, and §٦/١ forbids that outright. So the question is asked
      // while it can still be acted on — and it can be, because retrieval precedes
      // generation and `table.rows` is already final here.
      //
      // WHAT IS TESTED IS THE CARDS, NOT THE CITATIONS. Before the first character there
      // are no citations to count; there are rows, or there are none. A fiqh-scope answer
      // with NO rows behind it is the shape the retry existed to catch, and on this path
      // it is caught by not streaming at all: the turn falls to the unstreamed path it
      // takes today, keeps its retry, and the count is recorded under its own name.
      //
      // `mixed` IS INSIDE THE TEST for the same reason it is inside the retry: it is a fiqh
      // scope with a live pass beside it, and a ruling delivered with nothing behind it is
      // the same defect whichever way the turn was routed.
      if (terminalStreamEligible) {
        liveUnits = createTerminalUnitStream({
          domain: preDomain,
          mode,
          degraded: ctx.degraded,
          onUnit: ({ unit, piece, text }) => {
            if (typeof onWriteUnit !== 'function') return false;
            const accepted = onWriteUnit({ unit, piece, text }) === true;
            if (accepted) streamedThisTurn = true;
            return accepted;
          },
        });
      }
      ...
      const payload = terminalStreamEligible
        ? await callProviderStream({
          ...
          onText: (delta) => {
            ...
            liveUnits.push(delta);
          },
        })
        : await callProvider({
          providerUrl, headers, signal, body: { ...writeBody, stream: false },
        });
      terminalWriteMs = Date.now() - writeStartedAt;
      if (liveUnits) terminalStreamResult = liveUnits.end();
      modelCalls += 1;
      ...
      else if (terminalWriteAdded && !streamedThisTurn && finishingDraft) {
        written.push(finishingDraft);
        deliveredStop = finishingDraftStop;
        ctx.degraded.push('terminal_write_fallback:restored_empty_tool_text');
      }
      ...
      // ── §٥ — A CUT STREAM IS ANNOUNCED AS ONE ──────────────────────────────
      //
      // MEASURED, and it is why these three lines exist. A stream that ends without
      // `message_stop` does not throw: the read loop simply sees `done` and returns what it
      // has, and because only `message_delta` ever sets `stop_reason`, the field is still
      // null. `truncatedFrom(null)` is null — «I do not know» — so the answer footer is NOT
      // suppressed and a half-written answer is dressed as a finished one.
      //
      // ON THE UNSTREAMED PATH THAT CANNOT HAPPEN: `callProvider` parses one JSON body and a
      // body without `stop_reason` is not a body the provider sends. So a null here means one
      // thing on this path and one thing only, and it is named as that thing.
      //
      // A thrown cut is handled below from the stable reviewer prefix, not from the incomplete raw
      // provider tail. That keeps what reached the reader as the short answer and never layers a
      // fallback over bytes already accepted.
      if (terminalStreamEligible && deliveredStop === null) {
      ...
      terminalWriteMs = Date.now() - writeStartedAt;
      if (liveUnits && !terminalStreamResult) {
        try { terminalStreamResult = liveUnits.cut(); } catch {}
      }
      failure = `provider_error_write:${String(error?.status || '')}:${String(error?.message || error)}`;
      ctx.degraded.push(failure);
      if (terminalWriteAdded && !streamedThisTurn && finishingDraft) {
        written.push(finishingDraft);
        deliveredStop = finishingDraftStop;
        ctx.degraded.push('terminal_write_fallback:restored_tool_text');
      } else if (streamedThisTurn) {
        // The reader already owns this prefix. Keep that exact, reviewed prefix as the short
        // answer; a fallback or last-resort sentence would be a replacement after the fact.
        written.push(terminalStreamResult?.acceptedPrefix || '');
        deliveredStop = 'stream_cut';
        ctx.degraded.push('stream_cut:provider_error');
      }
    }
    ...
    ctx.degraded.push(
      terminalWriteAdded ? 'terminal_write_after_tool_text'
  ...
  const streamedPrefix = String(terminalStreamResult?.acceptedPrefix || '');
  ...
  const readerUnits = unitsForDelivery({
    streaming: terminalStreamEligible ? streaming : { enabled: false },
  ...
  return {
    ...
    terminalStreamViolations: Array.isArray(terminalStreamResult?.violations)
      ? terminalStreamResult.violations : [],
