...
export async function runFreeBrainTurn(options = {}) {
  ...
  // the refused sentence again. What this door removes is not a piece the reader is owed; it is a
  // piece he must not be given. So the removal happens above the snapshot, is priced in
  // `ctx.degraded` under its own name, and does not travel as a shortfall.
  //
  ...
  //
  //   ١ · ONE REGENERATION, AND THEN THE SENTENCE ALONE IS DROPPED. `rewriteBudget` is the turn's
  //     one rewriting call and `MAX_TURN_REWRITES` is 1, so «إعادةُ توليدٍ واحدةٌ» is enforced by
  //     the budget and not by a counter this door keeps. WHEN A DOOR ABOVE HAS ALREADY SPENT IT
  //     this door gets none, and it goes straight to the drop — which is that same constraint's
  //     own second half, «فإن مسكَتِ الثانيةَ تُسقَطُ الجملةُ وحدَها ويُعرَضُ الباقي», reached by a
  //     shorter road. The refusal is recorded rather than passing as silence.
  //
  //   ٢ · NEVER AN EMPTY SCREEN. «ما فيه شاشةٌ فاضيةٌ أبدًا.» The drop is adopted only if what is
  //     left still carries reader prose, by `carriesReaderProse` — the same measure ق٥٥ §١ uses for
  //     exactly this question. An answer whose ONLY prose is the refused sentence is delivered
  //     UNCHANGED and the fact is logged: this door may not be the reason a reader gets nothing.
  //
        ...
        if (emittedPrefix !== '' && located.some((one) => one.start < emittedPrefix.length)) {
          ...
          if (rewriteBudget.take('ascription_retry')) {
            ...
            if (carriesReaderProse(rewritten)) {
              ...
              // draft stands and the drop below takes it from there. Named under its own outcome
          ...
          // ── §٥/٣ القيد ١, SECOND HALF — THE SENTENCE ALONE, AND THE REST IS SHOWN ──
          if (located.length) {
            const kept = withoutStolenAscriptions(reviewed.text, located);
            if (carriesReaderProse(kept)) {
              ascriptionDropped = located.length;
              reviewed = {
                text: kept, annotations: reviewed.annotations, verdict: reviewed.verdict,
              };
              ctx.degraded.push(`ascription_dropped:${located.length}`
                + `:${String(reviewed.text || '').length}`);
              ascriptionOutcome = ascriptionRetries > 0 ? 'dropped_after_retry' : 'dropped';
            } else {
              // §٥/٣ القيد ٢ — «ما فيه شاشةٌ فاضيةٌ أبدًا». The refused sentence was the only prose
              // in the answer, so dropping it hands the reader nothing at all. The answer goes out
              // as it stands and the fact is priced here rather than being silently traded for a
              // blank screen. It is the one exit of this door that delivers the defect, and it is
              // named so it can be counted.
              ctx.degraded.push(`ascription_kept_no_prose_left:${located.length}`);
              ascriptionOutcome = 'kept_no_prose_left';
            }
          }
        }
      }
