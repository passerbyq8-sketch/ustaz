...
// the reviewer is above it. Everything that can stop the turn early sets `answer` (or leaves it
// empty) and FALLS THROUGH to the same tail. Enumerated, because a list nobody can enumerate is
// a list nobody can check:
//
//   E1 the model wrote prose                stop_reason !== 'tool_use'   -> break -> tail
//   E2 the rounds ran out                   rounds === MAX_TOOL_ROUNDS   -> write -> tail
//   E3 the tool-phase clock ran out         deadline_write               -> write -> tail
//   E4 the model emitted no text block      answer === ''                -> write -> tail
//   E5 a tool round's provider call threw   provider_error_tool_phase    -> write -> tail
//   E6 the final write's provider call threw  provider_error_write       -> tail with no text
//   E7 nothing survived any of the above    answer === '' at the tail    -> reviewer's LAST_RESORT
//
