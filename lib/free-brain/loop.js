...
const FOREIGN_LINE_QUOTATION_MARK_RE = /(?:[\u00AB\u00BB\u201C\u201D"]|[\uFD3F\uFD3E]|<\/?(?:verse|surah|hadith|steps|suggestions|source|board|document|dhikr|worship)\b)/iu;
...
export async function runFreeBrainTurn(options = {}) {
  ...
  // TRUE once `written[0]` is text the reader has already accepted. The head gate is what makes
  // index 0 the only possible place for it. See `joinRoundTextsHeadPinned`.
  ...
  while (rounds < MAX_TOOL_ROUNDS) {
    ...
    //
    // THE HEAD GATE IS THE SAME GATE, evaluated per round instead of once after the loop:
    // `written.length === 0` is «nothing has been written for the reader yet», which is exactly
    // what `headFreeBeforeTerminal` measured. A turn whose finishing round is preceded by prose
    // is therefore withheld here for the same reason and under the same name.
    //
    ...
    const roundStreamEligible = streaming.enabled && roundHeadFree && !roundRulingWithoutCards;
    if (streaming.enabled && !roundHeadFree && !headWithholdNoted) {
      headWithholdNoted = true;
      ctx.degraded.push('stream_withheld:head_text');
    }
    if (streaming.enabled && roundHeadFree && roundRulingWithoutCards && !cardWithholdNoted) {
    ...
    let roundUnits = null;
    let roundAccepted = false;
    try {
      if (roundStreamEligible) {
        roundUnits = createTerminalUnitStream({
          domain: roundDomain,
          mode,
          degraded: ctx.degraded,
          onUnit: ({ unit, piece, text }) => {
            if (typeof onWriteUnit !== 'function') return false;
            const taken = onWriteUnit({ unit, piece, text }) === true;
            if (taken) {
      ...
      if (roundUnits) streamResult = roundUnits.end();
    } catch (error) {
      if (roundUnits && !streamResult) {
        try { streamResult = roundUnits.cut(); } catch { /* a cut that cannot close reports nothing */ }
      }
      if (roundAccepted) {
        ...
        written.push(streamResult?.acceptedPrefix || '');
        readerOwnsHead = true;
    ...
    // §٢ (V3) — A ROUND WHOSE UNITS THE READER ACCEPTED OWNS ITS PLACE IN THE ANSWER. The head
    // gate guarantees `written` was empty when this round started, so the reader's text is
    // `written[0]` and nowhere else. What that pin does — and what it costs — is at the join.
    if (roundText && roundAccepted) readerOwnsHead = true;
