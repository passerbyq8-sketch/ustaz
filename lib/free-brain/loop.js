...
/**
 * ── §٤: A LESSON IS AN ISNAD AND DOES NOT SPEND A PAGE SLOT ─────────────────
 *
 * THE OWNER'S ORDER, VERBATIM: «الدروسُ أبيه يثبت تحت كلِّ إجابة الدروسُ اللي هو كتبها إسنادًا
 * للردّ».
 *
 * WHY A LESSON IS NOT A SOURCE CARD — MEASURED, NOT PREFERRED:
 *   * `MAX_SOURCES = 3` is frozen by owner order. A lesson row DOES carry a URL, so without this
 *     split `pickReaderCards` would accept it on its own terms and one lesson could evict a
 *     published fatwa from one of the three slots. The ceiling means «at most three external
 *     PAGES the answer rests on», and a lesson is not one of those: nothing was read from it.
 *   * A lesson carries NO TEXT at any layer — lib/lessons-service.js drops `snippet` and the
 *     upstream contract says `citation_allowed = 0`, `usage = search_only`. A card whose body can
 *     never be quoted is a different object from a source card, and the reader is owed that
 *     difference rather than a chip that looks identical to one.
 *
 * SO IT TAKES THE ENCYCLOPEDIA'S SHAPE — server-owned, appended after the prose, costing no slot —
 * but it keeps a LINK, because a lesson has a page to open and the encyclopedia has none.
 */
export const LESSON_KIND = 'lesson';
// A tail is a tail. Past three it stops being an isnad and starts being a reading list.
export const LESSON_TAIL_MAX = 3;

/**
 * Split cited rows into the ones that compete for MAX_SOURCES and the ones that do not.
 *
 * PURE, and order-preserving on BOTH halves: `pages` keeps delivered-text order so the ceiling
 * still cuts exactly where the reader's own prose stopped reaching, and `lessons` keeps it so the
 * tail names them in the order the answer leaned on them.
 *
 * A row of an unknown kind is a PAGE, not a lesson. The default has to fall on the side that is
 * already capped and already URL-checked; a new corpus that reached the tail by accident would be
 * a corpus nobody counted.
 *
 * @param {Array<object>} cited  rows in delivered-text order
 * @returns {{pages: Array<object>, lessons: Array<object>}}
 */
export function splitCitedLessons(cited) {
  const rows = Array.isArray(cited) ? cited : [];
  const pages = [];
  const lessons = [];
  for (const row of rows) {
    if (row && row.kind === LESSON_KIND) lessons.push(row);
    else pages.push(row);
  }
...
/**
 * The lesson rows that reach the reader: only those the answer cited, deduplicated by the tag the
 * builder produces, capped at `max`, and only those the builder can encode at all.
 *
 * `buildTag` is passed IN for the same reason `pickReaderCards` takes it: URL safety stays in
 * api/ask.js where `buildSourceTag` already enforces https-only and refuses a hostile host, and
 * this file stays a pure selection rule a mutant can drive.
 *
 * @param {Array<object>} lessons  lesson rows in delivered-text order
 * @param {number} max             LESSON_TAIL_MAX
 * @param {(row: object) => ({tag: string}|null)} buildTag  the lesson tag builder
 */
export function pickLessonCards(lessons, max, buildTag) {
  const out = [];
  for (const row of Array.isArray(lessons) ? lessons : []) {
    if (out.length >= max) break;
    const card = row && row.url ? buildTag(row) : null;
    if (card && card.tag && !out.some((item) => item.tag === card.tag)) out.push(card);
  }
