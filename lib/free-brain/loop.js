...
export async function runFreeBrainTurn(options = {}) {
  ...
  const {
    ...
    onWriteText = null, env = undefined,
  } = options;
  ...
  let rounds = 0;
  let modelCalls = 0;
  // §٤ — did THIS turn take the streaming path? Not the same question as «is the flag on»:
  // the writing call may be withheld from the stream by the test below even when it is.
  ...
  if (!finished) {
    try {
      ...
      streamedThisTurn = streaming.enabled && !rulingWithoutCards;
      ...
      const payload = streamedThisTurn
        ? await callProviderStream({
          providerUrl, headers, signal, body: writeBody, onText: onWriteText,
        })
        : await callProvider({
      ...
      //
      // WHAT IS NOT DONE HERE, deliberately: the text that already reached the reader
      // before a THROWN cut is not recovered into `written`. That needs the partial message
      // carried out on the error, and there is no turn-level harness in this repository to
      // prove such a change end to end. §٦/١ is not at risk meanwhile — the writer's own
      // prefix check keeps what went out and refuses to send a refusal on top of it —
      // so this is named and left, and tools/stream-p1/cut-stream-proof.cjs measures it.
      if (streamedThisTurn && deliveredStop === null) {
  ...
  if (streamedThisTurn && domain !== 'general' && collectCited(answer).length === 0
    && table.rows.length > 0) {
  ...
  const readerUnits = unitsForDelivery({
    streaming, readerText, deliveredText, domain, mode, khilaf, truncated,
