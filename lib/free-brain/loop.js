...
export async function runFreeBrainTurn(options = {}) {
  ...
  if (rejectedFirst > 0 && !rejectGateOpen) {
    ctx.degraded.push('reject_retry:suppressed_on_stream');
  }
  ...
  if (rejectGateOpen
    ...
    if (carriesReaderProse(rewritten)) {
      ...
      deliveredStop = rejectPayload.stop_reason ?? null;
      proposedRows = citedRefs.map((ref) => table.byRef(ref)).filter(Boolean);
      readerText = stripCitations(answer);
      khilaf = khilafSignal(proposedRows);
      truncatedAfterReject = truncatedFrom(deliveredStop);
      reviewed = await reviewAnswer({
        ...
        khilafFromOpinions: khilaf.khilafFromOpinions,
        opinionCount: khilaf.opinionCount,
        truncated: truncatedAfterReject,
      });
    ...
    if (rejectedCount(reviewed.verdict) > 0) {
      ...
      const delivered = soundProse ? sound : '';
  ...
  return {
    ...
    truncated: rejectWithheld ? true : (truncatedAfterReject ?? truncated),
