...
// ── ق٥٥ §٣-ب — HOW MANY REWRITING ROUNDS A CUT BUYS ─────────────────────────
// One, and written the same way and in the same place as the ceiling above it for the same
// reason: the number is a property of this file that a reader can see and a mutant can move.
//
// ONE IS ENOUGH BY MEASUREMENT AND NOT BY TASTE. Of the 223 answers in 3,000 that carry at least
// one cut, 199 — 89.2% — carry exactly one. A single rewrite is the whole remedy in nine cases
// out of ten, and the tenth is what §٣-ج is for.
const MAX_REJECT_RETRIES = 1;
...
export async function runFreeBrainTurn(options = {}) {
  ...
  if (rejectRetries < MAX_REJECT_RETRIES
    ...
    if (rejectedCount(reviewed.verdict) > 0) {
      ...
      reviewed = { text: sound, annotations: reviewed.annotations, verdict: reviewed.verdict };
      ...
      rejectWithheld = sound !== '';
      ctx.degraded.push(`reject_withheld:${sound.length}`);
    }
