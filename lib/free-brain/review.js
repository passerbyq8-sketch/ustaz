...
export async function reviewAnswer(input = {}) {
  ...
  try {
    const out = await reviewAnswerPure({
      // ── E75 — WHO WAS ASKED ABOUT, NORMALISED LIKE EVERYTHING ELSE ───────
      // This seam FORWARDS NOTHING IT DOES NOT NAME, which is why the field is added in two
      // places rather than one — the loop's call site and this list. It is normalised for the
      // reason the two signals below are: a value arriving in a shape the reviewer does not
      // expect is not rejected by it, it is silently misread. Anything that is not an object
      // carrying a non-empty name is `null` — «nobody in particular was asked about» — because
      // the one thing this must never do is hand the reviewer a half-built identity and let it
      // judge an answer against a man the reader never named.
      requestedIdentity: input.requestedIdentity
        && typeof input.requestedIdentity === 'object'
        && typeof input.requestedIdentity.name === 'string'
        && input.requestedIdentity.name.trim()
        ? input.requestedIdentity : null,
      text,
      ...
      // م٢-ز — the speaker policy crosses the seam as one literal or not at all.
      speakerPolicy: input.speakerPolicy === 'keep' ? 'keep' : null,
    });
