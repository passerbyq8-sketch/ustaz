// lib/free-brain/tools.js — THE FIVE TOOLS THE MODEL MAY CALL, AND WHAT THEY ACTUALLY SEARCH.
//
// ── WHAT THIS REPLACES ───────────────────────────────────────────────────────
...
//
// ── WHAT THE FIVE ARE, MEASURED ──────────────────────────────────────────────
//   search_fatawa   lib/fatwa-service.js over lib/fatwa-contract.js's roster — 19 rows summing to
//                   74,769 published records (18,479 ابن باز · 17,875 سعد الماجد · 13,343 ابن
...
//   search_lessons  api/lessons-search.js over the ezik lessons index — MEASURED 2026-09-09 at
//                   index_version ezik-lessons-v6-2026-09-09: 156,423 units, of which 90,091
//                   carry a transcribed matn (the scholars' own recorded words) and 3,772 are
//                   marked citable. A record here is a LESSON, LECTURE OR SERIES EPISODE at a
//                   permanent url — not a fatwa and not a book page.
//
...
// ع-٥٠ (السكوت) §٣-٢ — the ceiling a per-call row cap is clamped to, taken from the library's own
// contract rather than retyped here: a caller may not ask this file to keep rows the service
// never sends. See `cap` in `runLibrary`.
import { LIB_LIMIT_MAX } from '../lib-contract.js';
// ITEM 37/① — THE LESSONS SERVICE IS REACHED THROUGH THE PROJECT'S OWN DOOR AND NEVER AROUND IT.
// api/lessons-search.js is, and stays, the only caller of https://lib.ezik.app/lessons/search in
// this tree; `search_lessons` below imports its outbound call instead of opening a second one, so
// there is no new address here, no new token, and no second copy of the wire contract to drift.
import { fetchLessonsPayload, shapeBrainResponse } from '../../api/lessons-search.js';
// ٢/٢ — THE SAME CLASSIFIER api/ask.js's WORLD BRANCH USES, AND NO SECOND WORD LIST. `search_live`
// has to decide one thing this file could not decide before: is the ANSWER to this search a number
// that moves? lib/world-intent.js owns that question, owns the three reasons it is built from, and
// is already behind LIVE_WORLD_V2 — so `asksLiveNumber` is false here whenever the switch is off
// and this module reads no environment of its own, exactly as it reads none for the library.
import { asksLiveNumber, classifyWorldIntent } from '../world-intent.js';
// ج-٣/١ — THE ORDERING, FROM THE MODULE THAT OWNS IT AND ITS SWITCH. On a general news question
// the outlets this application admits are put at the front of the result set before the per-call
// cap takes its five. It is a SORT and never a filter: nothing is dropped and nothing refused.
// The switch test lives inside that module, not here, for the same reason `asksLiveNumber` does
// — this file reads no environment of its own and keeps no flag of its own in step.
import { orderNewsSourcesFirst } from '../live-news-order.js';
...
// ── AA-63: A PARAGRAPH IN THE BOOK IS A PARAGRAPH ON THE SCREEN ─────────────
//
// `clean` above folds EVERY whitespace run to one space, and that is right for a title, an author
// and a search phrase — a name that arrived across two lines is one name. It is wrong for a
// PASSAGE: a page of the Kuwaiti encyclopedia is written in paragraphs, and folding them delivers
// the reader one unbroken block of a thousand characters.
//
// ── THE CENSUS, OVER THE WHOLE CORPUS, NOT OVER ONE PAGE ────────────────────
// 3,045 records of lib/data/fiqh-search.json.gz hold 342,678 whitespace runs, and they take
// exactly FOUR shapes:
//
//     " "      318,828   93.04%   an ordinary word space
//     "\n\n"    23,794    6.94%   a paragraph break
//     "\n\n "       42    0.01%   the same, with the next line indented
//     "\n"          14    0.00%   a bare line wrap, and there are fourteen of them in the corpus
//
// 2,102 of the 3,045 records — 69% — carry at least one paragraph break, and every record that
// carries a newline at all carries a blank line. So the structure being folded is REAL and it is
// the majority case, and the ragged whitespace `clean` was defending against is not in this
// corpus at all: there are no tabs, no runs of spaces, and no exotic separators.
//
// ── THE RULE, IN ONE LINE ───────────────────────────────────────────────────
// A whitespace run holding TWO OR MORE newlines is a paragraph break and becomes exactly «\n\n».
// Every other run — spaces, tabs, a single newline, anything the source used for layout — becomes
// one space, exactly as before.
//
// The single newline goes to the space side deliberately: fourteen occurrences in 342,678 runs is
// not a structure, and treating a line wrap as a break is how a justified column becomes a poem.
//
// AND THE TAIL IS TRIMMED AFTER THE CUT, which `clean` does not do. The cap falls where it falls;
// a passage that ends on a break announces a paragraph the reader will never be given, which is
// the same hole this function exists to prevent. Leading whitespace is trimmed before the cut, as
// `clean` does, so what is cut is the passage and not its margin.
const cleanPassage = (value, cap) => String(value == null ? '' : value)
...
export const FREE_BRAIN_TOOLS = Object.freeze([
  ...
  {
    name: 'search_lessons',
    description: 'بحثٌ في دروسِ المشايخِ المسجَّلةِ ومحاضراتِهم وسلاسلِهم العلميّة — أكثرُ من ١٥٦٠٠٠ درسٍ لمشايخَ معاصرينَ بأسمائِهم وروابطِهم الثابتة (خالد المصلح، سعد الخثلان، عثمان الخميس، عبدالعزيز الراجحي، ابن باز، ابن عثيمين، المنجد وغيرهم). اطلبْها لتدلَّ السائلَ على درسٍ يستزيدُ منه فيما أجبتَه به: شرحُ بابٍ فقهيٍّ كاملٍ، أو سلسلةٌ في مسألتِه، أو محاضرةٌ تُفصِّلُ ما اختصرتَه. وليست فتوى: ما يردُ منها كلامُ شيخٍ في درسٍ مسجَّل، فلا يُبنى عليه حكمٌ وحدَه دونَ فتوى، والحكمُ يُطلَبُ من search_fatawa. ولا تُطلَبُ فيما ليس مسألةً شرعيّةً أو علمًا شرعيًّا: لغزٌ أو حسابٌ أو خبرٌ أو سؤالٌ عامّ. ومع كلِّ نتيجةٍ درجةُ مطابقتِها، وهي رقمُ البحثِ لا تصحيحُ حديث؛ والدرجةُ تكبرُ بطولِ عبارةِ بحثِك، فقارِنْ نتائجَ الطلبةِ الواحدةِ بعضَها ببعضٍ ولا تقارنْ درجاتِ طلبتَين.',
    input_schema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'ألفاظُ البحثِ بالعربيّة، تكتبُها أنت بما يناسبُ ما انتهيتَ إليه في جوابِك.' },
      },
      required: ['query'],
    },
  },
...
// ── ITEM 37/① — ONE CONDITIONAL LINE ADDED, AND NO EXISTING OUTPUT MOVED ────
// `matchScore` is carried by lessons rows and by nothing else, so every row this function
// rendered before today renders byte-identically after it: the line is dropped by the same
// `filter(Boolean)` that already drops an absent publisher, url or text. That is why the score
// is rendered HERE rather than welded into `title` the way `search_library` welds its locator —
// a welded score would travel into the reader's source card, and this must reach the MODEL only.
//
// AND IT IS NOT A HADITH GRADE. `درجةُ المطابقة` is named in full on purpose: this repo has a
// `hadith_grading` capability and a bare «الدرجة» beside a scholar's name is one careless read
// away from being taken for صحيح/ضعيف. It is the search service's own relevance number.
export function renderRows(added) {
  ...
  return added.map((row) => [
    ...
    row.matchScore ? `درجةُ المطابقة: ${row.matchScore}` : '',
...
// ── ENCYC_V1: ONE ROW SHAPE FOR THE ENCYCLOPEDIA, WHOEVER ASKED ───────────────────────────
//
// lib/free-brain/loop.js now OFFERS encyclopedia rows before the first provider call (the
// first-second offer, ENCYC_V1), and a row the loop offers must be the row `search_sources` hands
// back when the model asks -- the same title, publisher, passage ceiling and marker -- so the shape
// is built HERE, once, and both callers take it. Two copies of one row would drift, and the model
// would be taught two citations for one source.
//
// TWO FIELDS ARE NEW, AND NEITHER IS RENDERED. `renderRows` reads title/publisher/url/text and
// nothing else, and `reviewerEvidence` and `citedDeliveryLedger` in ./loop.js read named fields
// that are not these, so with the switch off every byte the model and the reader receive is what
// it was:
//   part      the volume (juz') the record sits in -- 1..45, present on all 3,045 records --
//             carried and not derived: the reader card (api/ask.js, ENCYC_V1) names it. No record
//             carries a page, so there is no page field and a page is never composed.
//   matnCut   whether `text` is shorter than the stored passage: the same flag, on the same rule,
//             the library row carries, because the card shows this passage letter for letter and
//             a silent cut is the defect either way.
export function encyclopediaRow(record) {
  const part = Number(record && record.part);
  return {
    kind: 'encyclopedia',
    title: `الموسوعة الفقهية الكويتية — ${record.term}`,
    url: '',
    publisher: 'الموسوعة الفقهية الكويتية',
    // AA-63 -- the encyclopedia is written in paragraphs; see the census at `cleanPassage`.
    text: cleanPassage(record.snippet, SNIPPET_CHARS),
    recordId: record.id || '',
    part: Number.isInteger(part) && part > 0 ? part : null,
    matnCut: cleanPassage(record.snippet, Infinity).length > SNIPPET_CHARS,
  };
}
...
async function runSources(input, ctx) {
  ...
  //
  // م٢-أ (BEFORE_WRITING_V1) — EXCEPT ON A HADITH TURN. «وسؤالُ الحديثِ يبقى على طريقِ التخريج، ولا
  // تأتيه كتبُ الفقهِ ولا الموسوعة»: owner test 12 («ما درجة حديث: «تبسمك…»») received the
  // encyclopedia's «ضَحِك» article through exactly this half. `ctx.noEncyclopedia` is set by
  // lib/free-brain/loop.js on a hadith turn under the switch and on no other turn, so every other
  // caller runs this half as before.
  const added = [];
  if (!ctx.noEncyclopedia) {
    try {
      const enc = await searchStoredCorpus(query, { limit: 3 });
      for (const record of (enc.records || []).slice(0, 3)) {
        added.push(ctx.table.add(encyclopediaRow(record)));
      }
    } catch (error) {
...
async function runLive(input, ctx) {
  ...
  // ── ٢/٢ — AN ENCYCLOPEDIA MAY NOT QUOTE A LIVE NUMBER ON THIS PATH EITHER ──
  //
  // THE GAP THIS CLOSES, MEASURED. api/ask.js's world branch has passed `liveNumber` to
  // retrieveOpenWorld since 19 September, and this call — the one the reader's turn ACTUALLY takes,
  // because the free brain answers before that branch is ever reached — did not. So the rule the
  // owner gave («الموسوعةُ لا تُسعِّر») was enforced on a road nobody was driving on, and on the
  // road everybody drives on, an exchange rate could still come back out of an encyclopedia
  // article with no quotation date anywhere on the figure.
  //
  // IT IS DECIDED BY WHAT THIS PATH KNOWS ABOUT THE QUESTION, AND BY NO THIRD LIST. The search
  // phrase is what this runner holds — the model writes it, and it is the thing the provider is
  // actually being asked — so it is the text the classifier reads. `asksLiveNumber` is
  // lib/world-intent.js's own rule (WEATHER · MARKET_PRICE · FX_RATE), which is the same rule the
  // world branch applies, taken from the module that owns it rather than restated here.
  //
  // TRUE ONLY WHEN THE QUESTION IS A MOVING NUMBER, false in everything else — a news search, a
  // match result, a «ماذا حدث في ٢٠٢٦» — so Wikipedia stays fully admitted to every search that is
  // not asking for a figure, which is the owner's «تبقى مسموحةً لغيرِ ذلك» in one boolean.
  const liveNumber = asksLiveNumber(query);
  // FOR THE TRACE BELOW AND FOR NOTHING ELSE. A closed vocabulary of reason names, never `matched`
  // — that field carries the word out of the reader's own sentence that fired the rule.
  const intentReason = classifyWorldIntent(query).reason;
  ...
  try {
    ...
    if (!pass || !(pass.sources || []).length) {
      pass = await retrieveOpenWorld(query, {
        band: ctx.band, dailyBudget: ctx.dailyBudget, liveNumber,
      });
      calls += 1;
      open = true;
    }
    // ج-٣/١ — BEFORE THE SLICE, or it changes nothing: only the first MAX_RESULTS_PER_CALL rows
    // ever become evidence, so an admitted outlet sitting sixth keeps its seat behind the cap
    // unless the ordering happens first. This is the same placement, and the same reasoning, as
    // `orderRulingCandidates` takes above the wave slice in lib/retrieve.js.
    const ordered = orderNewsSourcesFirst(pass?.sources || [], { reason: intentReason });
    for (const source of ordered.slice(0, MAX_RESULTS_PER_CALL)) {
      added.push(ctx.table.add({
  ...
  // ── ٢/٤ — THE ONE LINE THAT SEPARATES «انهارَ» FROM «لم يجدْ» ON THIS PATH ──
  //
  // WHAT THE LOG COULD NOT SAY. A turn that searched and found nothing and a turn whose search
  // THREW produce the same thing from here on — an empty row set, the same prose, the same answer
  // — so in the platform log the second was invisible inside the first, and a dead provider or a
  // broken import read as an honest empty result. The world branch got this trace on 19 September;
  // the free brain is the path the reader actually takes and had none.
  //
  // IT IS A LOG LINE AND NOTHING ELSE. Not a header, not an SSE frame, not a word of the answer.
  //
  // AND IT CARRIES NO READER TEXT. `reason` is one of lib/world-intent.js's declared reason names,
  // `errorName` is the runtime's class name — never `e.message`, which can quote the question
  // verbatim out of the open-search URL, and never `matched`, which is a word lifted from the
  // question itself. Gate `telemetrytext` made that ruling on the world branch's first draft and
  // it is taken here without being asked twice.
  console.info('[free-brain/live]', {
    reason: intentReason,
    liveNumber,
    open,
  ...
  // ── ٢/٣'s INPUT, RECORDED WHERE IT HAPPENS ────────────────────────────────
  // The print contract in api/ask.js needs two facts this function is the only holder of: whether
  // a live search for a MOVING NUMBER ran at all this turn, and which pages it actually brought
  // back. `host` is derived from the row's own URL rather than taken from the publisher string,
  // because that is the field lib/live-number-source.js reads. Guarded with an array test: the
  // takhrij pass in api/ask.js drives `runTool` with a context of its own that carries no such
  // field, and a tool that threw there would be a tool that broke a seal.
  if (Array.isArray(ctx.liveCalls)) {
    ctx.liveCalls.push({
      liveNumber,
      sources: added.map((row) => {
        let host = '';
        try { host = row.url ? new URL(row.url).hostname.toLowerCase().replace(/^www\./u, '') : ''; }
        catch { host = ''; }
        return { host, title: row.title || '' };
      }),
    });
  }
  return { text: renderRows(added), added, calls };
...
async function runLibrary(input, ctx) {
  ...
  // R1 (brain-night 2026-09-24) — THE CAP IS ASKED OF THE SERVICE, NOT ONLY SLICED FROM ITS REPLY.
  // `cap` below slices what came back, but the request never said how many rows it wanted, so
  // lib/lib-service.js sent LIB_LIMIT_DEFAULT (ten) and the slice of thirty was a slice of ten:
  // MEASURED on the twin of the production index with TAKHRIJ_ROWS_PER_CALL already at thirty,
  // the recorded body was still `{ q, limit: 10 }` and the bracket still named البيهقي for
  // «لا ضرر ولا ضرار». A caller with no resultCap sends exactly the body it always sent
  // (`limit` undefined -> LIB_LIMIT_DEFAULT in lib/lib-service.js), so the model path is
  // byte-identical on the wire; only the takhrij path, which sets resultCap, asks for more.
  const askedRows = Number.isInteger(ctx.resultCap) && ctx.resultCap > 0
    ? Math.min(ctx.resultCap, LIB_LIMIT_MAX)
    : undefined;
  let out;
  try {
    out = await searchLibrary(query, {
      ...
      signal: ctx.signal,
      limit: askedRows,
      // ع-٥٠ (الطريق ب) — THE ONE DOOR, NARROWED BY WHOEVER OPENED IT.
      // ADDITIVE AND NOTHING ELSE. `ctx.bookIds` is absent on every caller that existed before
      // today, and lib/lib-service.js sends the byte-identical `{ q, limit }` body when it is
      // absent or empty, so a model-chosen search is unchanged to the byte.
      //
      // WHY IT IS A ctx FIELD AND NOT A SECOND CALL SITE. guards/lib-book-contract-guard.cjs
      // row A7 holds that `searchLibrary` has exactly ONE call site in the tree and that it is
      // this runner. The takhrij pass needs to ask «which of THESE books carries this matn»,
      // and it asks it THROUGH here rather than opening a second door — which is the owner's
      // ruling of 19 September, route (ب), taken in as many words.
      bookIds: ctx.bookIds,
      // LIB_QUOTE_V1 -- ADDITIVE IN THE SAME WAY: only the quotation path (lib/lib-quote.js) sets
      // `ctx.maxCharsPerHit`, so every other caller's body is byte-identical, and the row below is
      // still clipped at SNIPPET_CHARS; the quotation reads its atom, not this row's passage.
      maxCharsPerHit: ctx.maxCharsPerHit,
    });
  } catch (error) {
  ...
  // ── ع-٥٠ (السكوت) §٣-٢ — HOW MANY ROWS THIS ONE CALLER KEEPS ──────────────
  //
  // `MAX_RESULTS_PER_CALL` IS UNTOUCHED AND STAYS FIVE. It is the MODEL's budget: five rows is
  // what a reading model is handed per call, and widening it widens every tool in this file.
  //
  // THE TAKHRIJ PATH IS NOT A READING MODEL. It asks one machine question — «which of these 54
  // books carries this matn» — and then throws the prose away; the cost of a sixth row to it is
  // an id in an array. MEASURED on the live index on 19 September 2026: the service returns TEN
  // rows and the slice threw away the half that answered.
  //   «إنما الأعمال بالنيات، وإنما لكل امرئ ما نوى»  -> أبو داود is row NINE, and its atom
  //      carries the narration. Cut, the answer was «(لا يثبت مرفوعا)»; kept, أبو داود.
  //   «أحي والداك؟…»                                  -> البخاري is row SEVEN.
  // Six of ten rows for the first query are ONE book (إرواء الغليل), because a تخريج volume
  // mentions a matn far more often than the Ṣaḥīḥ that narrates it once — so the five best rows
  // of a 54-book ladder are routinely five rows of one commentary.
  //
  // IT IS OPT-IN AND IT IS BOUNDED. Absent on every caller that existed before today, so their
  // slice is byte-identical; and clamped to what the service itself will return, so this field
  // can never ask for a page nobody has.
  const cap = Number.isInteger(ctx.resultCap) && ctx.resultCap > 0
    ? Math.min(ctx.resultCap, LIB_LIMIT_MAX)
    : MAX_RESULTS_PER_CALL;
  const added = (out.records || []).slice(0, cap).map((record) => {
    ...
    return ctx.table.add({
      ...
      // AA-63 -- and this passage is the one a reader is shown letter for letter, in the
      // book card's panel (app.jsx BookCard renders it under `whiteSpace: pre-wrap`), so a
      // folded page here is a folded page on the screen.
      text: cleanPassage(record.text, SNIPPET_CHARS),
      recordId: record.id || '',
      // ع-٥٠ (الطريق ب) — THE LIBRARY'S OWN SUBJECT ID, CARRIED AND NOT DERIVED.
      // `provenance.subject_id` is one of lib/lib-service.js's PROVENANCE_IDS and is already on
      // the record; this row was dropping it. A reader never sees it — api/ask.js's card builder
      // reads named fields and this is not one of them — but WITHOUT it nothing downstream can
      // ask whether a hit is on the takhrij ladder, and a ladder matched on a book's printed
      // TITLE instead is the mistake shamela-match-45.md was written about: four of five books
      // that looked missing were present under a different name. So the id travels, and the
      // match is made on the id.
      subjectId: String(prov.subject_id || ''),
      ...
      matnCut: record.truncated === true || cleanPassage(record.text, Infinity).length > SNIPPET_CHARS,
      // م٢-أ (BEFORE_WRITING_V1) — THE WHOLE ATOM, BESIDE THE CLIPPED ONE. Only the before-writing
      // gathering (lib/before-writing.js) sets `ctx.keepFullText`; it pins this for the writer and
      // for the sentence-level door, which must read the madhhab line that sits past SNIPPET_CHARS.
      // Absent on every other caller, so their rows carry exactly the fields they carried.
      ...(ctx.keepFullText ? { fullText: cleanPassage(record.text, Infinity) } : {}),
    });
  });
...
// -- search_lessons ----------------------------------------------------------
// ITEM 37/①. WRITTEN OFF `runLibrary` ABOVE, AND OFF NOTHING ELSE: the same refusal before any
// call when the turn was not handed a key, the same one-line failure text per class, the same
// `ctx.degraded` note, the same `MAX_RESULTS_PER_CALL` slice, the same `ctx.table.add` row, the
// same `renderRows` render, and the same never-throws promise.
//
// THE TWO PLACES IT DIFFERS FROM THE LIBRARY, AND WHY EACH:
//
//   1. A LESSON HAS A URL AND A BOOK HAS NONE. `runLibrary` leaves `url` empty on purpose,
//      because a printed page is attribution and not a link. A lesson IS a link — a permanent
//      address the reader can open and listen to — so the url travels, and a hit without one is
//      dropped rather than carded, exactly as app.jsx's own reader drops it.
//
//   2. THE MATN IS THE MODEL'S AND NOT THE SCREEN'S. `snippet` is the field api/lessons-search.js
//      deletes at its HTTP edge, and it still does: the reader's card is three fields and holds
//      no lesson text. This path reads the tenth field through `shapeBrainResponse`, which exists
//      for this one caller. MEASURED BY THE ROUND THAT BUILT THE DOOR: the service fills it on
//      2.4% of hits and sends it empty on 97.6%, so on most calls the row carries a title, a
//      scholar, a url and a score and no text at all — `renderRows` then prints no text line, and
//      that absence is the honest one. This session could not re-measure the fill rate: reading a
//      raw `snippet` needs SEARCH_API_TOKEN, and the token is not readable from the machine the
//      item was executed on (see the report).
async function runLessons(input, ctx) {
  const query = clean(input.query, 240);
  if (!query) return { text: 'لا نتيجة', added: [], calls: 0 };
  if (!ctx.lessonsToken) {
    return { text: 'الدروس غير متاحة في هذا الطور', added: [], calls: 0 };
  }

  let out;
  try {
    out = await fetchLessonsPayload(query, {
      token: ctx.lessonsToken,
      limit: MAX_RESULTS_PER_CALL,
      fetchImpl: ctx.fetchImpl,
      signal: ctx.signal,
    });
  } catch (error) {
    ctx.degraded.push(`lessons:${String(error?.message || error)}`);
    return { text: 'تعذّر الوصول إلى الدروس', added: [], calls: 0 };
  }

  if (!out || out.ok !== true) {
    ctx.degraded.push(`lessons:${String(out && out.reason ? out.reason : 'unknown')}`);
    // `calls` counts PROVIDER CALLS THAT HAPPENED. A refusal that never reached the wire —
    // an empty token, a query this function itself rejected — is zero; anything the service
    // answered, including a 401 or a 500, is one.
    const reached = out && out.reason !== 'search_api_token_missing' && out.reason !== 'bad_request';
    return { text: 'لا نتيجة من الدروس', added: [], calls: reached ? 1 : 0 };
  }

  const hits = shapeBrainResponse(out.payload).hits || [];
  const added = hits.slice(0, MAX_RESULTS_PER_CALL).map((hit) => {
    const url = typeof hit.url === 'string' ? hit.url.trim() : '';
    return ctx.table.add({
      kind: 'lesson',
      title: clean(hit.title || '', 200),
      // Only an http(s) address travels. A hit whose url is absent or is some other scheme is
      // carried with an empty one, and `renderRows` then prints no link line — the row is still
      // real material, it just has nothing for a reader to open.
      url: /^https?:\/\//i.test(url) ? url : '',
      // `scholar_id` IS NOT AN IDENTIFIER — it is the scholar's Arabic display name, and the two
      // files of the door that opened this service say so at their heads. It is printed as the
      // publisher and is never joined on, slugged or looked up.
      publisher: clean(hit.scholar_id || '', 120),
      text: cleanPassage(hit.snippet, SNIPPET_CHARS),
      recordId: hit.unit_id || '',
      // One decimal, and a STRING, because it is a label the model reads and not a number
      // anything computes with. `renderRows` drops the line when this is absent or zero.
      matchScore: Number.isFinite(Number(hit.score)) ? Number(hit.score).toFixed(1) : '',
    });
  });
  return { text: renderRows(added), added, calls: 1 };
}
...
const RUNNERS = {
  search_fatawa: runFatawa,
  search_sources: runSources,
  search_live: runLive,
  search_library: runLibrary,
  search_lessons: runLessons,
};
...
// The two whitespace rules, exported for a guard and for nothing else. The same idiom
// lib/lib-service.js uses for `__libTest`: AA-63's claim is that ordinary prose comes out
// byte-identical and only a paragraph break changes, and that claim is about the FUNCTION.
// Driving three thousand corpus records through a fetch stub to reach it would have measured
// the runner instead, and a guard that cannot see the rule cannot kill a mutant of it.
export const __toolsTest = Object.freeze({ clean, cleanPassage });
