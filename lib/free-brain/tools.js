...
import { searchLessons } from '../lessons-service.js';
...
export const FREE_BRAIN_TOOLS = Object.freeze([
  ...
  {
    name: 'search_duroos',
    description: 'ابحثْ في دروسِ المشايخِ ومحاضراتِهم المسجَّلة (٣٣ شيخًا). وهذه الأداةُ تختلفُ عن أخواتِها اختلافًا جوهريًّا: لا تُعطيك نصًّا ألبتّة، إنّما عنوانَ الدرسِ واسمَ الشيخِ ورابطَه. فلا تنقلْ منها حرفًا، ولا تبنِ عليها حكمًا، ولا تنسبْ إلى الشيخِ قولًا لم تقرأْه — إنّما تُسنِدُ بها إسنادًا: «وللشيخِ فلانٍ درسٌ في هذه المسألة». ولا تذكرْ درسًا إلّا إن كانَ عنوانُه يخصُّ ما كتبتَه أنت فعلًا؛ وعنوانٌ قريبُ اللفظِ بعيدُ المعنى لا يُذكَر.',
    input_schema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'ألفاظُ البحث بالعربيّة، تكتبُها أنت بما يناسبُ المسألة.' },
      },
      required: ['query'],
    },
  },
...
// ── search_duroos ────────────────────────────────────────────────────────────
// THE ONE TOOL WHOSE ROWS CARRY NO TEXT, AND THAT IS THE CONTRACT RATHER THAN A GAP.
//
// The lessons store answers with a title, a scholar's DISPLAY NAME and a link. `snippet` is empty
// on 97.6% of hits and is dropped in lib/lessons-service.js in every case, empty or filled. So
// `citation_allowed = 0` and `usage = search_only` are enforced HERE BY SHAPE: the row is added
// with `text: ''`, `renderRows` therefore prints no «النص:» line, and the model has nothing to
// quote even if it wanted to. A lesson is an ISNAD, never a matn.
//
// `publisher` CARRIES `scholarId`, WHICH IS A NAME AND NOT A KEY. The upstream calls the field
// that way inside its own contract; it is printed and never joined on.
async function runDuroos(input, ctx) {
  const query = clean(input.query, 240);
  if (!query) return { text: 'لا نتائج.', added: [], calls: 0 };

  let out;
  try {
    out = await searchLessons(
      { query, limit: MAX_RESULTS_PER_CALL },
      { signal: ctx.signal, fetchImpl: ctx.fetchImpl },
    );
  } catch (error) {
    // searchLessons is written never to throw; this is the belt for the day that changes, and it
    // reports rather than losing the turn — the same rule runFatawa keeps above.
    ctx.degraded.push(`duroos:${String(error?.message || error)}`);
    return { text: 'تعذّر الوصولُ إلى الدروسِ الآن.', added: [], calls: 0 };
  }
  if (out && out.degraded) ctx.degraded.push(`duroos:${out.degraded}`);

  const added = ((out && out.records) || []).slice(0, MAX_RESULTS_PER_CALL).map((record) => ctx.table.add({
    kind: 'lesson',
    title: clean(record.title, 200),
    url: record.url || '',
    publisher: clean(record.scholarId, 120),
    // EMPTY ON PURPOSE — see the head of this runner. Not an oversight, and not a field to fill in
    // later: there is no lesson text in this application, at any layer, by the owner's ruling.
    text: '',
  }));
  return { text: renderRows(added), added, calls: 1 };
}
const RUNNERS = { search_fatawa: runFatawa, search_sources: runSources, search_live: runLive, search_duroos: runDuroos };
