...
async function runLibrary(input, ctx) {
  ...
  const added = (out.records || []).slice(0, MAX_RESULTS_PER_CALL).map((record) => {
    ...
    const volume = citable && prov.volume ? `ج${prov.volume}` : '';
    const start = citable && prov.page_start ? String(prov.page_start) : '';
    const end = citable && prov.page_end ? String(prov.page_end) : '';
    const page = start && end && start !== end ? `ص${start}-${end}` : (start ? `ص${start}` : '');
    const where = [volume, page].filter(Boolean).join(' · ');
    return ctx.table.add({
      ...
      locator: where,
      // ع-٥٣ — THE BOOK'S OWN IDENTITY, AND IT WAS BEING DROPPED HERE. `makeAtoms` gives every
      // atom `atom_id = subjectId:section:slice` and carries `subject_id` beside it;
      // lib/lib-service.js records it in `record.provenance` (PROVENANCE_IDS), and this row —
      // the last place the two are still together — kept the title and the author and let the
      // id go. Downstream had nothing left to group by but the welded tag, so three slices of
      // ONE book arrived at the reader as three separate cards.
      //
      // AND IT IS THE ID AND NEVER THE TITLE. «صحيح البخاري» matched thirty-three subjects in
      // the measured corpus: an exact title match is not «this is your book», so a title key
      // would merge two different books and call it a repair. An absent id stays absent — a row
      // that cannot say which book it is from is its own card, not a shared one.
