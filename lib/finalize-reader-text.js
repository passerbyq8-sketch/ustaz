...
import { finishReaderProse } from './reader-prose.js';
// م٣-ب (FULL_ANSWER_V1) — the switch, held for one call of the seat (see finalizeReaderText).
import { withBracketAfterQuote } from './takhrij-lock.js';
...
export function finalizeReaderText(input = {}) {
  // م٣-ب — with `bracketAfterQuote` (api/ask.js, FULL_ANSWER_V1) the lock and the grade rule below
  // both carry a ladder bracket that stands right after its matn. Absent, nothing changes.
  return withBracketAfterQuote({ bracketAfterQuote: !!input && input.bracketAfterQuote === true },
    () => finalizeReaderTextOnce(input));
}
function finalizeReaderTextOnce(input = {}) {
  ...
  // D4 S2-S4: this is after every prose-adding/deleting pass, including the shared seal.
  if (input.bracketAfterQuote === true) {
    const beforeCleanup = text;
    text = finishReaderProse(text);
    dropped('reader-prose', 'mechanism-or-ordinal', beforeCleanup, text);
  }
