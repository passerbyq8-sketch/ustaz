...
//
// `removed` is the span that differs between the two texts — the common head and tail taken off
// — so for one cut it is exactly what went, and for several cuts in one stage it runs from the
// first to the last. Capped at 200 characters here, so no caller can print more.
...
function removedBetween(before, after) {
  ...
  return a.slice(head, a.length - tail).trim().slice(0, DROP_REMOVED_MAX);
}
...
export function finalizeReaderText(input = {}) {
  ...
  const dropped = (stage, kind, before, after) => {
    if (before !== after) drops.push({ stage, kind, removed: removedBetween(before, after) });
  };
