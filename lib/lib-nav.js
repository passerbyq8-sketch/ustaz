...
export async function navCall(route, body, opts = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      // D-2 E4: this encyclopedia's measured TOC is 4,530,880 bytes. Only its article
      // reader requests the bounded 8 MiB allowance; ordinary navigation keeps 4 MiB.
      const maxBytes = route === 'toc' && body.book_id === 'FC-003910' && opts.encyclopediaToc === true
        ? 8 * 1024 * 1024 : LIB_NAV_MAX_JSON_BYTES;
      if (Buffer.byteLength(text, 'utf8') > maxBytes) return fail('nav_body_too_large', { status: response.status });
...
// D-2 E4. Complete encyclopedia headwords, bounded by the next headword in its TOC.
// Reuses the sole navigation door and never reaches /search, /next or the live origin.
let encyclopediaToc = null;
const encyclopediaArticles = new Map();
export async function encyclopediaArticle(term, volume, opts = {}) {
  if (opts.flagValue !== 'on' || !opts.token) return null;
  const key = foldArabic(term), part = Number(volume);
  const cacheKey = part + ':' + key;
  const cached = !opts.fetchImpl && encyclopediaArticles.get(cacheKey);
  if (cached) return cached;
  const nav = { ...opts, baseUrl: LIB_NAV_ORIGINS[1], encyclopediaToc: true,
    deadline: opts.deadline || Date.now() + 12000 };
  let toc = !opts.fetchImpl && encyclopediaToc;
  if (!toc) {
    const got = await navCall('toc', { book_id: 'FC-003910' }, nav);
    if (!got.ok || got.payload.book_id !== 'FC-003910') return null;
    toc = got.payload;
    if (!opts.fetchImpl) encyclopediaToc = toc;
  }
  const flat = [];
  const walk = (nodes) => { for (const node of nodes || []) { flat.push(node); walk(node.children); } };
  walk(toc.headings);
  const index = flat.findIndex(n => n.level === 2 && Number(n.volume) === part && foldArabic(n.heading) === key);
  if (index < 0) return null;
  const start = flat[index];
  const end = flat.slice(index + 1).find(n => n.level <= start.level);
  if (!end || Number(end.volume) !== part || !posInt(start.page) || !posInt(end.page)
    || end.page < start.page || end.page - start.page > 256) return null;
  const atoms = new Map();
  // Four read-only pages at a time, one deadline for the complete article. An incomplete
  // read is never labelled full and leaves the authentic stored record as the fallback.
  for (let page = start.page; page <= end.page; page += 4) {
    const batch = await Promise.all(Array.from({ length: Math.min(4, end.page - page + 1) }, (_, i) =>
      navCall('page', { book_id: 'FC-003910', volume: part, page: page + i }, nav)));
    for (const got of batch) {
      if (!got.ok || got.payload.book_id !== 'FC-003910' || !Array.isArray(got.payload.atoms)) return null;
      for (const atom of got.payload.atoms) {
        if (typeof atom.text !== 'string' || Number(atom.volume) !== part || !Array.isArray(atom.heading_path)) continue;
        if (atom.heading_path.some(h => foldArabic(h) === key)) atoms.set(atom.atom_id, atom);
      }
    }
  }
  ...
  for (const atom of atoms.values()) {
    ...
    if (previous?.heading === heading) previous.text += '\n' + atom.text;
    else sections.push({ heading, text: atom.text });
  }
  const out = { complete: true, text: [...atoms.values()].map(a => a.text).join('\n'), sections };
  if (!opts.fetchImpl) {
    if (encyclopediaArticles.size >= 30) encyclopediaArticles.delete(encyclopediaArticles.keys().next().value);
    encyclopediaArticles.set(cacheKey, out);
  }
  return out;
}
...
export async function answerPageRequest(text, deps = {}) {
  ...
  switch (plan.outcome) {
    ...
    case 'no_book': return { outcome: 'page_no_book', text: noBookReply(plan.titleText, plan.authorText || '', { form: true }) };
    case 'blocked': return { outcome: 'page_blocked', text: blockedReply(plan.book) };
    case 'ask_which': return { outcome: 'page_ask_which', text: askWhichReply(plan.books, { form: true }) };
  ...
  return {
    outcome: 'page_quoted', text: composed.text, cursor: { bid: owner.id, at: composed.lastAtomId },
    // م٤-ج — the page this reply quotes, for the seal when the reader asked for it to be summarised.
    page: { title: owner.title, passage: joinedText(envelope) || '' },
  };
}
...
// ── م٤-ج · THE QUOTATION BEHIND AN EXPLANATION, RE-READ ───────────────────────────
// MEASURED (C-explain.md, case J): the seal licenses a credit only against pages read IN THIS REQUEST,
// so an explanation that repeated the book's own «رواه البخاري ومسلم» had it cut. The quotation in the
// history is the client's copy and is not evidence; it is re-read from its printed page, and kept only
// when the re-read atoms carry the quoted words.
export async function licensedQuote(cursor, place, deps = {}) {
  if (!cursor || !place || !Number.isInteger(place.page)) return null;
  const book = catalogBook(cursor.bid);
  if (!book || book.blocked || book.auto) return null;
  const body = { book_id: book.id, page: place.page };
  if (Number.isInteger(place.volume)) body.volume = place.volume;
  const got = await navCall('page', body, { ...deps, deadline: Date.now() + LIB_NAV_CALL_MS });
  const env = got.ok ? got.payload : null;
  if (!env || env.book_id !== book.id || !Array.isArray(env.atoms) || !env.atoms.length) return null;
  const passage = env.atoms.map((a) => String((a && a.text) || '')).join('\n');
  const probe = foldArabic(place.quoted).replace(/ …$/u, '').slice(0, 120);
  if (!probe || !foldArabic(passage).includes(probe)) return null;
  return { title: book.title, passage };
}

export const __navTest = Object.freeze({ titleReadings, joinedText, headingByAtom, chapterAtomIn, topHeadings });

// ── م٥ (LIB_NAV_V1) · «المكتبة»: READING A BOOK IN THE SECTION ───────────────────────
//
// THE OWNER'S ITEM: «أبوابُ الكتاب · القراءةُ صفحةً صفحةً بالتالي والسابق · البحثُ داخلَ الكتاب … قواعدُ
// النقلِ نفسُها». So the section's rules ARE the quote rules, enforced HERE on every operation, because a
// rule the client keeps is a rule a request can skip:
//   heritage and fatwa books are read in pages (whole atoms, the real range on the card);
//   an auto-numbered book is read by its chapters, and no page number of it is ever shown;
//   a modern book is not read at all beyond what a quotation may show: one paragraph, with its attribution,
//   found by a search inside it;
//   a withheld book does not exist here.
// api/lib-nav.js is the section's server door; it hands every book-level operation to `readOp`.

/** How the section may show one book: 'pages' · 'chapters' · 'paragraph' · null (withheld or unknown). */
export function readingMode(book) {
  if (!book || book.blocked) return null;
  if (book.cls === 'modern') return 'paragraph';
  return book.auto ? 'chapters' : 'pages';
}

const ATOM_ID_RE = /^[A-Za-z0-9_.:-]{1,80}$/;
const TOC_MAX_NODES = 1500;
const TOC_MAX_DEPTH = 6;
/** Hits a search inside a heritage book returns; a modern book returns one paragraph whatever this is. */
export const LIB_NAV_FIND_ROWS = 10;
const FIND_MAX_CHARS = 200;

const printedAtom = (a) => !!a && a.page_citable === true && a.numbering === 'print';

function shapeHeadings(nodes, budget, depth = 0) {
  const out = [];
  if (!Array.isArray(nodes) || depth >= TOC_MAX_DEPTH) return out;
  for (const n of nodes) {
    if (budget.left <= 0) break;
    if (!n || typeof n.heading !== 'string' || !ATOM_ID_RE.test(String(n.atom_id || ''))) continue;
    const heading = headingOf({ heading_path: [n.heading] });
    if (!heading) continue;
    budget.left -= 1;
    const printed = n.page_citable === true && n.numbering === 'print';
    out.push({
      heading,
      atom_id: n.atom_id,
      volume: printed && posInt(n.volume) !== null ? n.volume : null,
      page: printed && posInt(n.page) !== null ? n.page : null,
      children: shapeHeadings(n.children, budget, depth + 1),
    });
  }
  return out;
}

function shapeAtom(a) {
  const printed = printedAtom(a);
  return {
    atom_id: String(a.atom_id || ''),
    text: String(a.text || '').replace(/\r\n?/g, '\n'),
    heading: headingOf(a),
    volume: printed ? posInt(a.volume) : null,
    page_start: printed ? posInt(a.page_start) : null,
    page_end: printed ? posInt(a.page_end) : null,
  };
}

// A pointer the client may follow: the atom always; the volume and page only where they are print.
function pointerOf(p, book, mode) {
  if (!p || p.book_id !== book.id || !ATOM_ID_RE.test(String(p.atom_id || ''))) return null;
  const pages = mode === 'pages';
  return { atom_id: p.atom_id, volume: pages ? posInt(p.volume) : null, page: pages ? posInt(p.page) : null };
}

function readingOf(book, mode, envelope, atoms) {
  return {
    mode,
    card: navCard(book, atoms),
    atoms: atoms.map(shapeAtom),
    next: pointerOf(envelope.next, book, mode),
    prev: pointerOf(envelope.prev, book, mode),
  };
}

const failWith = (got) => {
  const known = ['numbering_auto', 'numbering_unavailable', 'volume_required', 'page_not_found'];
  return { ok: false, code: known.includes(got.code) ? got.code : 'unavailable' };
};

/**
 * One section operation on one book. Never throws; reads no environment (deps carry the switch, the
 * token and the base, as api/lib-nav.js read them).
 *   'toc'  {book_id}                 → { mode, headings:[{heading, atom_id, volume, page, children}] }
 *   'page' {book_id, page, volume?}  → { mode, card, atoms, next, prev }                  (pages)
 *   'next' {book_id, atom_id}        → what follows that atom, the atom itself never again (pages, chapters)
 *   'at'   {book_id, atom_id}        → reading from that atom, INCLUSIVE: a chapter's first atom, or the
 *                                      previous page of a chapters book; `lead_missing` when the atom
 *                                      itself could not be reached and the reading starts after it
 *   'find' {book_id, q}              → { mode, hits:[{card, text, atom_id, volume, page, cut}] }
 * → { ok:true, body } | { ok:false, code } — code: not_found · not_pages · bad_request · numbering_auto ·
 *   numbering_unavailable · volume_required · page_not_found · end · too_broad · unavailable.
 */
export async function readOp(op, input, deps = {}) {
  const book = catalogBook(String((input && input.book_id) || ''));
  const mode = readingMode(book);
  if (!mode) return { ok: false, code: 'not_found' };
  const nav = { ...deps, deadline: Date.now() + (Number.isInteger(deps.turnMs) ? deps.turnMs : LIB_NAV_TURN_MS) };
  // Every envelope is checked for the book it names and for its own text before a word of it is shown.
  const trusted = (got) => got.ok && got.payload && got.payload.book_id === book.id && joinedText(got.payload) !== null;
  const atomsOf = (env) => (Array.isArray(env.atoms) ? env.atoms : []).filter((a) => a && typeof a.text === 'string');
  const ownAtom = (id) => ATOM_ID_RE.test(id) && id.startsWith(book.id + ':');

  if (op === 'toc') {
    const got = await navCall('toc', { book_id: book.id }, nav);
    if (!got.ok) return failWith(got);
    if (!got.payload || got.payload.book_id !== book.id) return { ok: false, code: 'unavailable' };
    return { ok: true, body: { mode, headings: shapeHeadings(got.payload.headings, { left: TOC_MAX_NODES }) } };
  }
  if (op === 'page') {
    if (mode !== 'pages') return { ok: false, code: 'not_pages' };
    const page = posInt(input.page);
    if (page === null) return { ok: false, code: 'bad_request' };
    const body = { book_id: book.id, page };
    if (posInt(input.volume) !== null) body.volume = input.volume;
    const got = await navCall('page', body, nav);
    if (!got.ok) return failWith(got);
    if (!trusted(got)) return { ok: false, code: 'unavailable' };
    const atoms = atomsOf(got.payload);
    if (!atoms.length) return { ok: false, code: 'page_not_found' };
    return { ok: true, body: readingOf(book, mode, got.payload, atoms) };
  }
  if (op === 'next' || op === 'at') {
    if (mode === 'paragraph') return { ok: false, code: 'not_pages' };
    const atomId = String(input.atom_id || '');
    if (!ownAtom(atomId)) return { ok: false, code: 'bad_request' };
    if (op === 'next') {
      const got = await navCall('next', { atom_id: atomId, unit: 'page' }, nav);
      if (!got.ok) return failWith(got);
      if (!trusted(got)) return { ok: false, code: 'unavailable' };
      const atoms = atomsOf(got.payload).filter((a) => a.atom_id !== atomId);
      if (!atoms.length) return { ok: false, code: 'end' };
      return { ok: true, body: readingOf(book, mode, got.payload, atoms) };
    }
    // 'at'. The contract has no route that returns an atom by its id, and /next leaves out the atom it is
    // given; so the reading is asked from the atom BEFORE it, which the page pointer of the atom after it
    // names (C:/EZIK-LIB/service/src/navigation.mjs `next`: prev = the page before the first returned
    // atom), and cut to start at the atom itself.
    const probe = await navCall('next', { atom_id: atomId, unit: 'atom' }, nav);
    if (!probe.ok) return failWith(probe);
    if (!trusted(probe)) return { ok: false, code: 'unavailable' };
    const before = probe.payload.prev;
    if (before && before.book_id === book.id && ownAtom(String(before.atom_id || '')) && before.atom_id !== atomId) {
      const got = await navCall('next', { atom_id: before.atom_id, unit: 'page' }, nav);
      if (got.ok && trusted(got)) {
        const all = atomsOf(got.payload);
        const at = all.findIndex((a) => a.atom_id === atomId);
        if (at !== -1) return { ok: true, body: readingOf(book, mode, got.payload, all.slice(at)) };
      }
    }
    // Not reachable (the book's first atom, or an atom alone on its page): read from after it, and SAY so.
    const got = await navCall('next', { atom_id: atomId, unit: 'page' }, nav);
    if (!got.ok) return failWith(got);
    if (!trusted(got)) return { ok: false, code: 'unavailable' };
    const atoms = atomsOf(got.payload).filter((a) => a.atom_id !== atomId);
    if (!atoms.length) return { ok: false, code: 'end' };
    return { ok: true, body: { ...readingOf(book, mode, got.payload, atoms), lead_missing: true } };
  }
  if (op === 'find') {
    const q = String((input && input.q) || '').replace(/\s+/g, ' ').trim().slice(0, FIND_MAX_CHARS);
    if (!q) return { ok: false, code: 'bad_request' };
    if (!deps.search || typeof deps.search.runTool !== 'function') return { ok: false, code: 'unavailable' };
    // The one door to /search (lib/free-brain/tools.js runTool → search_library), narrowed to this book.
    const got = await searchInBook(q, book.id, deps.search, mode === 'paragraph' ? QUOTE_ROWS : LIB_NAV_FIND_ROWS);
    if (got.failed) return { ok: false, code: 'unavailable' };
    if (!got.rows.length && got.refused) return { ok: false, code: 'too_broad' };
    const hits = [];
    for (const row of got.rows) {
      const h = got.hits.find((x) => x && `lib:${x.atom_id}` === row.recordId && x.subject_id === book.id);
      if (!h || typeof h.text !== 'string' || !h.text.trim()) continue;
      const a = shapeAtom(h);
      if (mode === 'paragraph') {
        // A modern book gives what a quotation gives: ONE paragraph, cut as a quotation is cut.
        const text = modernParagraph(h.text, q, h.truncated === true);
        if (!text) continue;
        hits.push({ card: navCard(book, [h]), text, atom_id: null, volume: null, page: null, cut: false });
        break;
      }
      hits.push({ card: navCard(book, [h]), text: a.text, atom_id: a.atom_id, volume: a.volume, page: a.page_start, cut: h.truncated === true });
    }
    return { ok: true, body: { mode, hits } };
  }
