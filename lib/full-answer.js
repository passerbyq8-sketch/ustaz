...
// continued draft, first asks whether it still fits. ج٣ (order C): the rewrites ask on every turn.
...
// ج٣ (order C) — the fastest speed a plan may assume: the faster production witness, 6144 tokens written in
// 60,894 ms (program-2026-09-24/01-item32/prod-log-extract.txt). A short call's time is mostly its latency,
// so a speed read off it is noise in either direction, and a plan that believed a fast one could start a
// rewrite it cannot finish.
export const FULL_ANSWER_FASTEST_RATE = 6144 / 60894;

/**
 * ج٣ (order C) — the writing speed of the call that wrote a draft, for the doors' clock on a turn nothing was
 * continued on: its tokens over its time LESS the time to first token, which the clock adds back itself (so it
 * is not counted twice), and held between the slowest planning speed and the fastest measured one. A short
 * draft therefore plans neither slower than FULL_ANSWER_FALLBACK_RATE nor faster than the fastest witness.
 */
export function draftRate(outTokens, ms) {
  const out = Number(outTokens);
  const took = Number(ms);
  if (!(out > 0) || !(took > 0)) return FULL_ANSWER_FALLBACK_RATE;
  const speed = out / Math.max(1, took - FULL_ANSWER_TTFT_MS);
  return Math.min(FULL_ANSWER_FASTEST_RATE, Math.max(FULL_ANSWER_FALLBACK_RATE, speed));
}
...
/**
 * Whether a whole-answer rewrite (citation, reject, empty, ascription) still fits — after a continued draft,
 * and since ج٣ (order C) on every turn. Such a rewrite may write up to the full budget again (its call is
 * sent with it), so it is timed at the full budget.
 *
 * @param {{startedAt:number, now:number, rate:number, maxTokens:number, reserveMs?:number}} input
