...
function stancesIn(value) {
  ...
  // «كبيرة» IS BOUNDED AND THE REST ARE NOT, and the asymmetry is the point: it is the only
  // member of this alternation that is a proper substring of an unrelated prayer word. Raw,
  // it fires inside «تكبيرة» and reports the opening takbir as a prohibition — which then
  // pairs with any «يجوز» in the same passage and forges a documented_disagreement out of a
  // sentence that has no خلاف in it at all. The leading group admits the Arabic proclitics
  // (ال و ف ب ل ك) so «مرتكب الكبيرة» still scores, while «تكبيرة» — whose ت is a stem
  // letter, not a clitic — no longer can.
  if (/حرام|محرم|يحرم|تحريم|منع|بطلان|باطل|واجب|يجب|وجوب|فرض|(?<![\p{Script=Arabic}\p{M}])(?:[وفبك]?ال|[وفبكل])?كبير[هة](?![\p{Script=Arabic}\p{M}])/u.test(rest)) forbid = true;
...
// ── ع-٧٣: THE ROLE IN THE PRAYER IS THE QUESTION, NOT A KEYWORD IN IT ───────
//
// MEASURED: a «منفرد» probe in front of a card titled «المأموم» was ACCEPTED, at score 28. The
// score is word overlap and nothing else, and الصلاة, السهو, التكبيرة and الركوع are shared by
// both roles — so the number was high BECAUSE the two cases look alike, which is exactly the
// situation in which the answer is wrong. And at the moment of ATTACHMENT no relevance test ran
// at all, so a card the ranking had already accepted was never asked again.
//
// THE ROLES ARE MUTUALLY EXCLUSIVE AND THAT IS THE WHOLE OF WHY THIS IS NOT A THRESHOLD. No
// amount of shared vocabulary makes a ruling about the المأموم into a ruling about the المنفرد;
// the sahw one carries is borne by his imam and the other has no imam. A score cannot express
// that, so raising the score would trade one wrong answer for a smaller number of them.
//
// DIACRITICS ONLY ARE FOLDED HERE AND THE HAMZA IS KEPT ON PURPOSE. `normalizeArabic` folds
// أ إ آ ٱ to a bare alef, which makes «إمام» — the man leading — and «أمام» — in front of —
// the same token, and would hand this rule a role the text never named. So the unpointed «امام»
// resolves to NO ROLE. That is the safe direction: the rejection below fires only on an
// EXPLICIT naming of the opposing role, never on a fatwa that simply did not say.
const ROLE_MARKS_RE = /[\u064B-\u0670\u0640\u06D6-\u06ED]/gu;
const AR_EDGE = String.raw`(?<![\p{Script=Arabic}\p{M}])`;
const AR_END = String.raw`(?![\p{Script=Arabic}\p{M}])`;
const roleWord = (stem) => new RegExp(AR_EDGE + '(?:[وف]?(?:لل|[بكل]?ال|[بكل]))?(?:' + stem + ')'
  + '(?:ات|ان|ين|ون|ها|هم|[هةيا])?' + AR_END, 'u');
const PARTICIPANT_ROLES = Object.freeze([
  ['munfarid', roleWord('منفرد')],
  ['mamum', roleWord('مأموم')],
  ['imam', roleWord('إمام')],
]);

/** Which of the mutually exclusive prayer roles this text NAMES. Order is not significant. */
function participantRoles(value) {
  const text = String(value ?? '').replace(ROLE_MARKS_RE, '');
  return PARTICIPANT_ROLES.filter(([, re]) => re.test(text)).map(([role]) => role);
}

/**
 * TRUE when the question asked about one role and the material answers about ANOTHER, alone.
 *
 * THREE WAYS OUT, AND EVERY ONE OF THEM IS A KEEP. The question named no role — then any role is
 * an answer to it. The material named none — a general fatwa covers the المنفرد and the المأموم
 * together and must not be thrown away for being general, which is the declared risk of this
 * rule. Or the material carries the role that was asked for, whether it rules on it alone or
 * COMPARES it with another: a fatwa that says «سواء كان منفردًا أو مأمومًا» has answered.
 *
 * AND NAMING TWO OTHER ROLES IS NOT A COMPARISON. «المأموم يحمله الإمام» names the مأموم and the
 * إمام and is a ruling about congregational prayer; in front of a «منفرد» question it is not a
 * broader answer, it is a different one. The escape is the ASKED role being present, never the
 * count of roles present — and that distinction is what the measured witness turns on.
 */
function roleConflict(context, ...material) {
  const asked = participantRoles([context?.currentQuestion || '', context?.resolvedTopic || ''].join(' '));
  if (!asked.length) return false;
  const found = participantRoles(material.filter(Boolean).join(' '));
...
function evidenceRelevance(text, title, context) {
  ...
  // THE THRESHOLD AND THE SCORE ARE UNTOUCHED, and the score is still reported as measured: a
  // rejected card keeps the number it earned, because the number is a fact about the overlap
  // and the rejection is a fact about the roles. Only `accepted` is the role gate's to say.
  const overlapped = matched.length >= need;
  return {
    accepted: overlapped && !roleConflict(context, title, text),
    score: matched.length * 3 + headHits * 4,
  };
}
...
function quoteAddressesTopic(quote, context, evidence = null) {
  ...
  // ع-٧٣ — AND IT IS ASKED HERE TOO, because this is the moment of ATTACHMENT and nothing was
  // asking it. `evidenceRelevance` gates what enters the ranking; this gates what gets stuck to
  // an answer, and a card can reach the second without passing the first (a stored pack, a
  // fallback order, a span chosen off a title). One rule, both doors.
  if (roleConflict(context, quote, evidence && evidence.title)) return false;
...
export async function runHybridDeenTurn(options = {}) {
  ...
  // ── ع-٤٣ · ONE AUTHORITY FOR WHAT THE ANSWER CLAIMS ────────────────────────
  // MEASURED, twice, as two different wrong answers with one shape underneath: the used set was
  // frozen from the FIRST accepted claims, and then everything downstream — the cards, the record
  // whose «نص الفتوى» is printed, the ids the telemetry reports — kept pointing at it while the
  // claims moved on. The majority/tarjīḥ gate can swap the whole validation object, or rebuild
  // its valid list deterministically, and nothing rebuilt the used set: a probe came back with a
  // summary drawn from evidence B, a card for evidence A, and A’s fatwa text beneath it. And the
  // deterministic fallback was spliced in with no answer-contract test at all, so a bare
  // «ما حكم استعمال السواك؟» over a pack with no ruling in it shipped ANSWER and one card with
  // explicit_ruling REQUIRED and answerContractSatisfied FALSE.
  //
  // So there is ONE authority and it is `finalClaims`. Every model regeneration goes through the
  // verifier — the majority retry used to skip it entirely — every candidate list is held to the
  // answer contract, and a list that fails becomes EMPTY rather than half-used. Only once it has
  // settled are the used evidence, the cited records, the fatwa blocks and the cards derived FROM
  // it. Empty takes the road that was already here: NO_HYBRID_EVIDENCE, no card, nothing claimed.
  const contractOr = (claims) => (answerContractSatisfied(claims, pack, context) ? claims : []);
  const verifiedClaims = async (claims) => {
    if (!claims.length) return [];
    modelCalls++;
    const checked = parsedObject(await verify(verifierRequest(context, claims)));
    const supported = new Set(Array.isArray(checked?.supported_ids) ? checked.supported_ids.map(String) : []);
    return contractOr(claims.filter((item) => supported.has(item.evidence.id)));
  };
  // Everything the reader is shown, derived from one claim list and from nothing else. Returns
  // null when this list cannot honestly become an answer, which is the same thing as empty.
  const bundleFor = (claims, holdToContract) => {
    const usedEvidence = [];
    ...
    // A claim whose evidence is not among the records the reader is shown is not a claim this
    // answer may make — including one pushed past MAX_CARDS, which used to keep drafting into the
    // summary while owning no card at all.
    const shown = new Set(usedEvidence.map((entry) => entry.id));
    const finalClaims = claims.filter((item) => shown.has(item.evidence.id));
    if (!finalClaims.length || !usedEvidence.length) return null;
    if (holdToContract && !answerContractSatisfied(finalClaims, pack, context)) return null;
    ...
    return {
      ...
      directFound,
      lead,
      cards: cardsFor(usedEvidence),
      citedRecords: usedEvidence.map(fatwaRecordFrom).filter(Boolean),
      summary: composeSummary(finalClaims, context, lead),
    };
  };
  const noHybridEvidence = () => ({
    ...base, outcome: 'NO_HYBRID_EVIDENCE', text: NO_HYBRID_EVIDENCE, cards: [], usedEvidence: [],
  ...
  try {
    ...
    validation.valid = contractOr(validation.valid);
    ...
    validation.valid = await verifiedClaims(validation.valid);
  } catch (error) {
    degraded.push(`synthesis:${String(error?.message || error)}`);
    validation.valid = [];
  }

  // ⚠ THE DETERMINISTIC HALF OF ع-٤٣ IS NOT LANDED, AND THIS IS WHY (MEASURED HERE, TODAY).
  //
  // The order asks that the deterministic drop be held to `answerContractSatisfied` like every
  // model list — it is what let a bare «ما حكم استعمال السواك؟» ship ANSWER + one card with
  // explicit_ruling REQUIRED and the contract UNSATISFIED. Enforcing it here refuses the gold
  // fatwa 220120 instead: its ruling span reads «فالجمهور على بطلان العقد وحرمته، والقول
  // الثاني على جوازه», and RULING_OUTCOME carries حرام/محرم and يجوز but neither حرمته nor
  // جوازه — so statesRuling() says NO RULING about a fatwa that rules twice in one sentence.
  // fallbackSentence() above records the size of that gap from its own sample: 14 of 24 answers
  // carry no sentence in the ruling lexicon at all, which is why it has a marked-excerpt branch
  // in the first place. RULING_OUTCOME is out of bounds for this commit, so switching the gate on
  // here would refuse the MAJORITY case, not an edge. That trade is the owner’s call to make.
  //
  // Everything else in ع-٤٣ does land: `finalClaims` is the one authority, every MODEL list is
  // held to the contract before and after narrowing, every model regeneration now passes the
  // verifier, and the used evidence, cited records, fatwa blocks, cards and ids are re-derived
  // from the claims that survived — never from a list that has since been replaced.
  ...
  let final = bundleFor(validation.valid, !deterministic);
  if (!final) return noHybridEvidence();
  if (final.dropped) degraded.push(`uncarded-evidence-dropped:${final.dropped}`);
  let comparison = validation.comparison;

  // ── LAYER 1 · THE RULING SUMMARY ───────────────────────────────────────────
  // `final.summary` is composed from `final.finalClaims`, and recomposed whenever they change.
  ...
  let majorityOffenders = unsupportedMajorityClaims(final.summary, final.citedRecords);
  if (majorityOffenders.length && final.citedRecords.length) {
    try {
      modelCalls++;
      ...
      const retryClaims = await verifiedClaims(contractOr(revalidated.valid));
      const retry = retryClaims.length ? bundleFor(retryClaims, true) : null;
      if (retry && !unsupportedMajorityClaims(retry.summary, retry.citedRecords).length) {
        if (retry.dropped) degraded.push(`uncarded-evidence-dropped:${retry.dropped}`);
        final = retry;
        comparison = revalidated.comparison;
        majorityOffenders = [];
      }
    } catch (error) {
    ...
    if (majorityOffenders.length) {
      ...
      const rebuilt = fallbackClaims(pack, context)
        .filter((item) => !unsupportedMajorityClaims(item.sentence, final.citedRecords).length);
      const whole = rebuilt.length ? bundleFor(rebuilt, false) : null;
      degraded.push(`majority-unsupported:${majorityOffenders.length}`);
      // The rebuild is deterministic, so it is not held to the contract either — same reason as
      // above. What it IS held to is the authority: adopting it re-derives the used records, the
      // cards and the ids, which is the half of ع-٤٣ that made a summary from B sit over a card
      // for A. When there is nothing to rebuild from, the failure stands recorded and unchanged.
      if (whole) {
        if (whole.dropped) degraded.push(`uncarded-evidence-dropped:${whole.dropped}`);
        final = whole;
      }
    }
  }
  const { finalClaims, usedEvidence, summary, directFound } = final;
  ...
  const spansFor = (evidence) => finalClaims
  ...
  if (!text || !cards.length) return noHybridEvidence();
  return {
    ...base,
    outcome: 'ANSWER', text, cards, usedEvidence,
    // The ids the reader can go and check are the ids the ANSWER claims, deduped in citation
    // order. Reading them off usedEvidence instead is how a swapped claim list went unobserved.
    validatedUsedEvidenceIds: [...new Set(finalClaims.map((item) => item.evidence.id))],
    comparison,
...
export const __hybridTest = Object.freeze({
  ...
  liveQueries, documentedCases, participantRoles, roleConflict, quoteAddressesTopic,
});
