...
const RULING_FRAME = /(?:^|\s)(?:ما\s+حكم|هل)(?:\s|$)/u;
...
function isGoldInstallmentQuestion(context) {
  const folded = normalizeArabic(context?.currentQuestion || '');
  return /ذهب/u.test(folded) && /تقسيط/u.test(folded);
}

function statesGoldCashProhibition(value) {
  const folded = normalizeArabic(value);
  return /ذهب/u.test(folded) && /(?:تقسيط|عمل[هة]\s+ورقي[هة]|نقد|الثمن|قبض)/u.test(folded)
    && /(?:حرام|لا\s+يجوز|محرم)/u.test(folded);
}

function veilStances(value) {
  const folded = normalizeArabic(value);
  const nonDuty = /(?:غير\s+واجب[هة]?|ليس(?:ت)?\s+واجب[ا]?|لا\s+يجب|مستحب[هة]?|سنه\s+ام\s+واجب|جواز\s+كشف(?:ه|ها|\s*الوجه))/u.test(folded);
  const withoutNonDuty = folded
    .replace(/غير\s+واجب[هة]?/gu, ' ')
    .replace(/ليس(?:ت)?\s+واجب[ا]?/gu, ' ')
    .replace(/لا\s+يجب/gu, ' ')
    .replace(/جواز\s+كشف(?:ه|ها|\s*الوجه)/gu, ' ');
  const duty = /(?:(?:النقاب|تغطي(?:ه|تهما)|ستر\s+الوجه).{0,80}(?:واجب|يجب|وجوب)|(?:واجب|يجب|وجوب).{0,80}(?:النقاب|تغطي(?:ه|تهما)|ستر\s+الوجه))/u.test(withoutNonDuty);
  return { duty, nonDuty };
}

function isVeilQuestion(context) {
  return isFaceVeilTopic(context?.currentQuestion || '') || isFaceVeilTopic(context?.resolvedTopic || '');
}

function joinScenarios(value) {
  const folded = normalizeArabic(value);
  const found = new Set();
  if (/(?:مسافر|السفر|سفر[ا]?)/u.test(folded)) found.add('travel');
  if (/(?:المطر|مطر|الوحل)/u.test(folded)) found.add('rain');
  if (/(?:المرض|مريض|مرض[ا]?)/u.test(folded)) found.add('illness');
  if (/(?:عرفات|مزدلف[هة]|النسك|(?:^|\s)الحاج(?:\s|$))/u.test(folded)) found.add('pilgrimage');
  return found;
}

function isBroadJoinQuestion(context) {
  const folded = normalizeArabic(context?.currentQuestion || '');
  return /الجمع\s+بين\s+الصلاتين/u.test(folded)
    && !/(?:مسافر|السفر|مطر|مرض|عرفات|مزدلف[هة]|حاج[هة]|ضرور[هة])/u.test(folded);
}
...
function answerContractRequirements(pack, context) {
  ...
  const veil = veilStances(all);
  const scenarios = joinScenarios(all);
  return {
    explicit_ruling: isRulingQuestion(context),
    gold_cash_installment_prohibition: isGoldInstallmentQuestion(context)
      && statesGoldCashProhibition(all),
    documented_veil_disagreement: isVeilQuestion(context) && veil.duty && veil.nonDuty,
    common_join_scenarios: isBroadJoinQuestion(context) ? [...scenarios].slice(0, 4) : [],
  };
}
...
function answerContractSatisfied(valid, pack, context) {
  ...
  if (requirements.gold_cash_installment_prohibition && !statesGoldCashProhibition(emitted)) return false;
  if (requirements.documented_veil_disagreement) {
    const stances = veilStances(emitted);
    if (!stances.duty || !stances.nonDuty) return false;
  }
  if (requirements.common_join_scenarios.length >= 2) {
    const covered = joinScenarios(emitted);
    if (requirements.common_join_scenarios.filter((item) => covered.has(item)).length < 2) return false;
  }
  return true;
}
...
function answerRequest(context, pack, depth, retry = false) {
  ...
  return {
    ...
    system: [
      ...
      'إذا كان answer_contract.gold_cash_installment_prohibition=true فبيّن أولًا حكم شراء الذهب بالنقد أو العملة المؤجلة، ولا تجعل استثناء المقايضة بسلعة هو جواب الحالة المعتادة.',
      'إذا كان answer_contract.documented_veil_disagreement=true فيجب أن تتضمن claims القول بالوجوب والقول بعدم الوجوب كما وردا في الأدلة، من دون طمس الخلاف.',
      'إذا احتوى answer_contract.common_join_scenarios صورتين أو أكثر فغطِّ صورتين أساسيتين على الأقل مما تثبته الأدلة بدل الاقتصار على أول مثال.',
...
function fallbackClaims(pack, context) {
  ...
  const requirements = answerContractRequirements(pack, context);
  if (requirements.gold_cash_installment_prohibition) {
    for (const evidence of ordered) {
      const quote = fallbackQuoteWhere(evidence, context, statesGoldCashProhibition);
      if (quote) return [fallbackClaim(evidence, quote)];
    }
  }
  if (requirements.documented_veil_disagreement) {
    const both = ordered.flatMap((evidence) => supportPieces(evidence).map((quote) => ({ evidence, quote })))
      .find(({ quote }) => quoteAddressesTopic(quote, context)
        && veilStances(quote).duty && veilStances(quote).nonDuty);
    if (both) return [fallbackClaim(both.evidence, both.quote)];
    const out = [];
    for (const stance of ['duty', 'nonDuty']) {
      for (const evidence of ordered) {
        const quote = fallbackQuoteWhere(evidence, context, (piece) => veilStances(piece)[stance]);
        if (quote) { out.push(fallbackClaim(evidence, quote)); break; }
      }
    }
  ...
  if (requirements.common_join_scenarios.length >= 2) {
    const out = [];
    for (const scenario of requirements.common_join_scenarios) {
      for (const evidence of ordered) {
        const quote = fallbackQuoteWhere(evidence, context, (piece) => joinScenarios(piece).has(scenario));
...
function evidenceSatisfiesQuestion(pack, context) {
  ...
  if (isVeilQuestion(context)) {
    const stances = veilStances(packText(pack));
    return stances.duty && stances.nonDuty;
  }
  if (isGoldInstallmentQuestion(context)) return statesGoldCashProhibition(packText(pack));
  if (isBroadJoinQuestion(context)) return joinScenarios(packText(pack)).size >= 2;
  if (isRulingQuestion(context)) return pack.some((entry) => statesRuling(entry.supportText));
  return true;
}
...
function liveQueries(context) {
  ...
  if (!base) return [];
  const queries = [base];
  // Comparison questions get one separate, bounded purpose query. This avoids
  // asking a single ranking to surface two sides, while keeping every Brave
  // request independently auditable and well below the query cap.
  if (isVeilQuestion(context)) {
    // Use the vocabulary found in actual eligible fatwa titles.  The previous
    // madhhab-heavy wording repeatedly ranked another obligation-only page,
    // even though the allow-list contains directly quotable pages that state
    // the disagreement as «فرض أم مستحب» / «ليس بواجب».
    queries.push('النقاب فرض أم مستحب خلاف العلماء ليس بواجب');
  } else if (isBroadJoinQuestion(context)) {
    queries.push(`${base} أحوال الجواز للمسافر والمطر والمرض`);
  }
...
export async function runHybridDeenTurn(options = {}) {
  ...
  // Tier order is operational, not merely ranking: local first, then the fatwa service, then
  // paid Brave/page retrieval only while the higher tiers still cannot answer the question.
  ...
  const localPack = dedupePack(localEvidence(localResult));
  if (!evidenceSatisfiesQuestion(localPack, context)) fatwaResult = await doFatwa();

  const servicePack = dedupePack([
    ...localPack,
    ...(fatwaResult?.records || []),
  ]);
  if (!evidenceSatisfiesQuestion(servicePack, context)) {
...
export const __hybridTest = Object.freeze({
  ...
  fallbackQuote, isQuestionOnlyQuote, liveEvidence, newDiagnostics, statesRuling, veilStances,
  evidenceSatisfiesQuestion,
});
