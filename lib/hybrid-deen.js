...
function statesRuling(value) {
  return RULING_OUTCOME.test(normalizeArabic(value));
}
...
function answerContractSatisfied(valid, pack, context) {
  ...
  if (requirements.explicit_ruling && !statesRuling(emitted)) return false;
...
function supportPieces(evidence) {
  return String(evidence.supportText || '').split(/(?<=[.!؟؛:])\s*|\n+/u)
...
function liveQuery(context) {
  const base = clean(context?.resolvedTopic || context?.currentQuestion || '', 300);
  // The mandatory veil case is a comparison question in substance even when
  // phrased yes/no.  Ask the live source tier for the disagreement explicitly
  // instead of letting a single top-ranked opinion erase the documented split.
  if (normalizeArabic(base).includes('النقاب')) {
    return clean(`${base} خلاف الفقهاء في ستر الوجه والكفين`, 360);
  }
  return base;
}
