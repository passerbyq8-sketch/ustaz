...
// A book carries no URL, so externalCard can never build its card and X-020 would drop
// the record with every claim drafted over it. A linkless card is not a new idea
// here: storedSourceCards already ships one for the local corpus. ATTRIBUTION, NOT
// TRANSFER: title, author, and the volume/page ONLY when the record itself carries
// them, because an auto-numbered atom has no printed page and a page nobody can look
// up is never invented.
function bookCard(evidence) {
  const source = (evidence && evidence.libRecord && evidence.libRecord.provenance) || {};
  const q = '\u0022';
  const strip = (value, max) => clean(String(value == null ? '' : value), max).replace(/[\u0022\u0027<>]/gu, '');
  const book = strip(source.book_title, 120);
  if (!book) return null;
  const author = strip(source.author, 90);
  const volume = strip(source.volume, 12);
  const start = strip(source.page_start, 12);
  const end = strip(source.page_end, 12);
  const page = start && end && start !== end ? start + '-' + end : start;
  const parts = [];
  ...
  return {
    ...
    host: 'lib-book',
    tag: '<source site=' + q + book + q + '>' + parts.join(' \u00b7 ') + '</source>',
  };
}
...
function cardsFor(used) {
  ...
  for (const item of used) {
    if (item.kind === 'lib_corpus') {
      const card = bookCard(item);
      if (card) cards.push(card);
    } else if (item.kind === 'local_encyclopedia') {
...
export async function runHybridDeenTurn(options = {}) {
  ...
  const base = {
    ...
    libReason: localResult?.libReason || null,
    libAttempts: Number(localResult?.libAttempts || 0),
    libRecordCount: (localResult?.libRecords || []).length,
    degraded,
    diagnostics: {
