...
function veilStances(value) {
  const folded = normalizeArabic(value);
  const nonDuty = /(?:غير\s+واجب[هة]?|ليس(?:ت)?\s+واجب[ا]?|لا\s+يجب|مستحب[هة]?)/u.test(folded);
  const withoutNonDuty = folded
    .replace(/غير\s+واجب[هة]?/gu, ' ')
    .replace(/ليس(?:ت)?\s+واجب[ا]?/gu, ' ')
    .replace(/لا\s+يجب/gu, ' ');
  const duty = /(?:(?:النقاب|تغطي(?:ه|تهما)|ستر\s+الوجه).{0,80}(?:واجب|يجب)|(?:واجب|يجب).{0,80}(?:النقاب|تغطي(?:ه|تهما)|ستر\s+الوجه))/u.test(withoutNonDuty);
  return { duty, nonDuty };
}

function isVeilQuestion(context) {
  return /(?:النقاب|ستر\s+الوجه)/u.test(normalizeArabic(context?.currentQuestion || ''));
}
...
function fallbackQuoteWhere(evidence, context, predicate = () => true) {
  ...
  const direct = supportPieces(evidence).filter((piece) => {
    ...
    const hits = keys.filter((key) => words.includes(key));
    return hits.length >= (keys.length === 1 ? 1 : Math.min(2, keys.length)) && predicate(piece);
  }).sort((a, b) => {
...
function fallbackQuote(evidence, context) {
  return fallbackQuoteWhere(evidence, context)
    || clean(evidence.supportText, Math.min(MAX_QUOTE, 500));
}
...
export async function runHybridDeenTurn(options = {}) {
  ...
  if (scholar) {
    ...
    const [fatwa, live] = await Promise.all([
      doFatwa(), Promise.all(queries.map((searchQuery) => doLive(searchQuery, ''))),
    ]);
    fatwaResult = fatwa;
    liveResult = mergeLiveResults(live);
  }
...
export const __hybridTest = Object.freeze({
  ...
  fallbackQuote, liveEvidence, newDiagnostics, statesRuling, veilStances,
});
