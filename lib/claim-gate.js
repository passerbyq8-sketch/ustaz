...
// A quoted span is the strongest possible signal that the reader means THESE words.
...
// Returns { specific, subject, source: 'quote'|'marker'|'anaphora', anaphoric }
export function detectSubject(questionRaw) {
  ...
  const qs = quoted(raw).filter((s) => norm(s).split(' ').length >= 2);
  if (qs.length) {
    const words = stripTrailingVerdict(norm(qs[0]).split(' ').filter(Boolean));
    if (words.length >= 2) return { specific: true, subject: words.join(' '), source: 'quote', anaphoric: false };
  }
  ...
  for (const mk of SUBJECT_MARKERS) {
    ...
    if (words.length >= need) return { specific: true, subject: words.join(' '), source: 'marker', anaphoric: false };
  }
