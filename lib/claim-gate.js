...
export function detectSubject(questionRaw) {
  ...
  if (qs.length) {
    ...
    const fz = frozenOf(qs[0]);
    if (releasedByCorpus(fz, raw)) {
      return { specific: false, subject: '', source: 'quote', anaphoric: false, frozen: fz, phrase: qs[0] };
    }
    const words = stripTrailingVerdict(norm(qs[0]).split(' ').filter(Boolean));
    if (words.length >= 2) {
      return { specific: true, subject: words.join(' '), source: 'quote', anaphoric: false, ...(fz ? { frozen: fz } : {}) };
    }
  }
