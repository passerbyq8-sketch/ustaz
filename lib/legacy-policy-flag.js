...
// THE HISTORICAL ROLLOUT DECISION FOR LEGACY POLICY REPAIRS. Its own decision default remains OFF,
// but api/ask.js now applies the defect corrections unconditionally and records this result only
// as rollout telemetry.
//
// ── WHY THIS EXISTS ──────────────────────────────────────────────────────────
// This module was introduced when the RFC v0.5-R2 repairs were treated as new rollout behaviour.
// That role is now historical: staging a measured bug fix behind a default-off value kept serving
// the defect. The helper remains so telemetry can describe the former rollout state without
// pretending that state still decides whether a correction is applied.
//
...
export async function decideLegacyPolicy(req, now = Date.now()) {
  ...
  if (mode !== 'unset') {
    // THE MODE PATH reports the former rollout decision. The handler records it, but the repaired
    // planning policy is unconditional there now.
