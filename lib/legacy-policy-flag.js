...
// are being activated by one decision, and two force variables would make it possible to activate
// half of it — the child policy on with the ledger off, or the reverse — which is a state nobody
// asked for and nobody would be watching.
//
// Same shape as the floor above, same fail-off: absent or unrecognised means every reader keeps
// the shipped behaviour. Turning this on takes the floor as well, so it is two deliberate acts.
...
export async function decideLegacyPolicy(req, now = Date.now()) {
  ...
  // PUBLIC ACTIVATION, BY THE OWNER'S ORDER. With both floors present the repairs apply to every
  // reader — which is the point of them: the cold refusal a seven-year-old got is in the SHIPPED
  // path, and it goes on being served until this is on.
  if (envForcesPublic()) return { enabled: true, reason: 'env_forced_public' };
