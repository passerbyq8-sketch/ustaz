...
// [111-close-5d] — MEASURED on the preview of 70e12db (this order's second battery, battery Q10): the seal dropped three
// sentences (`attribution,grade,attribution,attribution`, the view that weakened the hadith), and the answer went on
// «…بين تصحيحٍ وتضعيف:» ⟶ «في المقابل، ذهب آخرون إلى تصحيحه» — the other side of a contrast the reader never saw.
const CONTRAST_RE = /^(?:و?لكن|ومع ذلك|مع ذلك|بينما|غير ان|و?في المقابل|و?بالمقابل)(?= |$)/u;
...
export function repairOrphanedOpeners(original, text) {
  ...
  for (const v of sentenceUnits(out)) {
    ...
    if (CONTRAST_RE.test(key)) {
      const n = /^(?:ومع ذلك|مع ذلك|غير ان|و?في المقابل)(?= |$)/u.test(key) ? 2 : 1;
