  ...
  for (const event of events) {
    ...
    if (event.type === 'message_start') {
      ...
      if (!messageStarted || messageDeltaSeen || open.size !== 0 || !Number.isInteger(event.index) || event.index < 0
        || event.index !== nextBlockIndex || !event.content_block || event.content_block.type !== 'text'
      ...
      if (!messageStarted || messageDeltaSeen || !open.has(event.index) || !event.delta
        || event.delta.type !== 'text_delta' || typeof event.delta.text !== 'string') return { valid: false, complete: true };
    } else if (event.type === 'content_block_stop') {
      if (!messageStarted || messageDeltaSeen || !open.has(event.index)) return { valid: false, complete: true };
      open.delete(event.index);
    } else if (event.type === 'message_delta') {
      if (!messageStarted || messageDeltaSeen || open.size || used.size === 0
      ...
      if (!messageStarted) return { valid: false, complete: true };
  ...
  const flush = () => {
    ...
    try {
      ...
      const separated = context.allowWireOwnedCards === false
        ? { prose: wireText, suffix: '' }
        : separateOwnedCardSuffix(wireText, sourceCards);
