...
const HOLDINGS = '(?:هذه النصوص|النصوص التي بين يدي|ما بين يدي|(?:النصوص|الماده|المادة|المصادر|المراجع) (?:المتاحه|المتاحة|المسترجعه|المسترجعة))';
...
export function cleanRetrievalProse(text) {
  ...
  return outsideQuoted(source, chunk => {
    ...
    out = out.replace(/(^|\n)[ \t]*[وف]?[،؛.][ \t]*(?=\n|$)/gu, '$1');
    return out === chunk ? chunk : out.replace(/[ \t]{2,}/gu, ' ').replace(/ +([،؛.؟!])/gu, '$1');
  });
}
