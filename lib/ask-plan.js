...
export function planAsk(messages, opts = {}) {
  ...
  // ── WORLD KNOWLEDGE ABOUT THE NAME, WHEN THE REGISTRIES HAVE RUN OUT ──────
  //
  // «ما رأي خالد عبدالرحمن في قصر الصلاة؟» is the shape of a request for a scholar's position and
  // the name of a singer. No registry can settle that — they answer "is this one of OURS", and the
  // set of people who are not scholars is everybody else. So the caller may supply a verdict from
  // open world knowledge, and a confident `non_scholar` takes the attribution path away.
  //
  // IT ONLY EVER NARROWS. `non_scholar` can turn a request for somebody's fatwa into an ordinary
  // question; nothing here can turn an ordinary question into an attribution, and no value of this
  // option can widen what may be credited to anybody. Absent or `unknown` — the default — leaves
  // every existing decision exactly as it was.
  const entityWorldKnowledgeType = opts.entityWorldType === 'scholar' || opts.entityWorldType === 'non_scholar'
    ? opts.entityWorldType : 'unknown';
  if (entityWorldKnowledgeType === 'non_scholar' && mode === 'namedScholarOpinion') {
    mode = 'none';
  }
  ...
  return {
    ...
    policyEnabled,
    // What open world knowledge said this name is. 'unknown' unless a caller supplied a verdict.
    entityWorldKnowledgeType,
...
export function ambiguousScholarPrompt(candidates) {
  const names = [...new Set((candidates || [])
    .map((d) => { const s = findSource(d); return (s && s.name) || String(d || ''); })
    .filter(Boolean))];
