...
export function planAsk(messages, opts = {}) {
  const detectedClaimSubject = detectSubjectInThread(messages);
  // claim-gate owns quote parsing. Attribution receives that structured boundary instead of
  // maintaining another quote-character parser that could disagree with it.
  const attribution = detectAttribution(messages, { claimSubject: detectedClaimSubject });
  const question = attribution.question || '';
  const purpose = classifyPurpose(question);
  const detectedIr = readEntities(question);
  // A quoted religious subject is not inherently a hadith: it may be Qur'an or a human saying.
  // The kind is added only when attribution's existing capture machinery bound a prophetic
  // subject to a real attribution frame. Whole-question presence is insufficient, and the
  // trusted frozen/purpose structures veto Qur'anic text before the topic classifier sees it.
  const trustedPropheticQuote = detectedClaimSubject
    ...
    && purpose !== 'tafsir';
  const claimSubject = trustedPropheticQuote
    ? Object.freeze({ ...detectedClaimSubject, religiousKind: 'prophetic' })
    : detectedClaimSubject;
  // The quote governor is also the entity/relation governor. Once the structured attribution
  // parser has bound these quoted words to the Prophet, an entity from an earlier clause cannot
  // remain the requested authority in a parallel IR. Keep the raw reader wording in `attribution`,
  // but expose one coherent plan for the current quoted claim.
  const ir = trustedPropheticQuote
    ? Object.freeze({
      ...detectedIr,
      claimRelation: 'NONE',
  ...
  // ── ع-٧٥ · THE LAYER THAT KNOWS WHO HE IS DID NOT GET TO SAY SO ───────────
  //
  // MEASURED, all four spellings of one registered man, `dr-mutlaq.com`:
  //
  //   «ما رأي مطلق الجاسر…»    -> namedEntity «مطلق الجاسر»  resolved    dr-mutlaq.com
  //   «ما رأي الشيخ الجاسر…»   -> namedEntity «الجاسر»       resolved    dr-mutlaq.com
  //   «ما رأي الدكتور مطلق…»   -> namedEntity «مطلق»         UNRESOLVED  «»
  //   «ما رأي د. مطلق…»        -> namedEntity «»             n/a         «»
  //
  // The last two are the same man and the entity layer says so in both: surface «الدكتور مطلق»
  // / «د. مطلق», canonicalId mutlaq-aljasir, role authority, resolutionStatus RESOLVED. The
  // registry agrees — resolveScholar('الدكتور مطلق') and resolveScholar('د. مطلق') both return
  // dr-mutlaq.com, because both are listed aliases. Nothing was missing from either roster.
  //
  // WHAT DECIDED WAS THE WRONG STRING. `attribution.scholarName` is the LEXICAL capture, taken
  // after the frame has eaten the honorific, so «الدكتور مطلق» arrives here as the bare «مطلق»
  // — one common word, which resolveScholar refuses on purpose — and «د. مطلق» arrives as
  // nothing at all. An abbreviated name was handed the identity decision while the layer that
  // had already made it correctly, in all four spellings, was not consulted.
  //
  // SO THE SURFACE AND THE CANONICAL ID ARE CONSUMED TOGETHER, or neither is. The surface is
  // what the registry is asked about; the canonicalId is what `requestedAuthorityId` reports
  // downstream. Taking one without the other is how the two rosters disagreed in the first
  // place.
  //
  // THIS DOES NOT WIDEN WHAT MAY BE CREDITED TO ANYBODY. It hands the name to the SAME resolver
  // that already answers for it, and adopts the entity only when that resolver says `resolved`.
  // Where it moves `mode` it moves it to namedScholarOpinion, which is the FAIL-CLOSED mode —
  // his position may then be answered only from a page of his — so a question that was silently
  // treated as nobody's becomes MORE constrained about crediting him, not less.
  //
  // THREE CONTRACTS ARE LEFT EXACTLY WHERE THEY WERE, and each is excluded by its own clause:
  // materialFromScholarSite asks for MATERIAL and not a position; `ambiguous` is two men and
  // choosing is guessing, so only `resolved` is adopted; and a HISTORICAL man has no official
  // site to be resolved to — his contract is the era branch further down, not this one.
  //
  // NOT BEHIND `policyEnabled`, deliberately, and this is the one judgement call here. The two
  // flagged blocks above are VETOES that take an answer away; this one only replaces an empty
  // domain with the domain the registry already holds for a spelling it already lists. Gated,
  // it would be inert in production — the flag is default-off — and the defect would be
  // recorded as fixed while every reader still met «لم أتبيّنْ أيَّ شيخٍ تقصد».
  const authorityTarget = (() => {
    if (relation !== 'BY_ENTITY' || mode === 'materialFromScholarSite') return null;
    // THE ENTITY LAYER IS CONSULTED ONLY WHERE THE LEXICAL NAME ANSWERS NOBODY. Measured: taking
    // it whenever one resolved person was present broke two things it had no business touching.
    // «ما رأي الشيخ ابن عثيمين…» — the capture already resolves him, and the entity surface
    // carries the honorific, so namedEntity became «الشيخ ابن عثيمين» and the post-search note
    // read «للشيخ الشيخ ابن عثيمين». And «ما رأي خالد المصلح خالد السبت…» — TWO registered men in
    // one question, which the registry reads as ambiguous and the entity layer splits into one
    // authority and one subject; adopting the authority picked a man the reader never singled out.
    // So the gap this fills is exactly «unresolved» — nobody at all. A capture that already names
    // a man decides as it always did, and a capture that already names two keeps its ambiguity.
    const lexical = resolveScholar(attribution.scholarName || '');
    if (lexical.status !== 'unresolved') return null;
    // AND ONLY WHERE IT IS THE SAME MAN, WITH HIS TITLE PUT BACK. Measured: «قال ابن باز إن
    // القصر سنة، فما رأي فلان الفلاني في قصر الصلاة؟» captures «فلان الفلاني», whom no registry
    // knows, while the entity layer holds one resolved authority — ابن باز, named in the OTHER
    // clause. Adopting him there answers a man the reader did not ask about, so the entity is
    // taken only when its surface still contains the captured name; an empty capture is the case
    // where the frame swallowed the name whole («د. مطلق»), and there is nothing to contradict.
    const capturedKey = normalizeArabic(attribution.scholarName || '');
    const resolvedPeople = (ir.entities || []).filter((entity) => entity
      && entity.role === 'authority' && entity.targetType === 'person'
      && entity.resolutionStatus === 'resolved' && entity.era !== 'historical');
    if (resolvedPeople.length !== 1) return null;
    const target = resolvedPeople[0];
    if (capturedKey && !normalizeArabic(target.surface || '').includes(capturedKey)) return null;
    const resolution = resolveScholar(target.surface || '');
    if (resolution.status !== 'resolved') return null;
    return Object.freeze({
      surface: String(target.surface || ''),
      canonicalId: String(target.canonicalId || ''),
      resolution,
    });
  })();
  if (authorityTarget) mode = 'namedScholarOpinion';

  const namedEntity = mode === 'namedScholarOpinion'
    ? (authorityTarget ? authorityTarget.surface : (attribution.scholarName || ''))
  ...
  // The resolution the target already carries — the same call, not a second one, so the surface
  // that was adopted and the domain that answers for it can never come apart.
  const res = authorityTarget ? authorityTarget.resolution
    : namedEntity ? resolveScholar(namedEntity) : { status: 'unresolved' };
  ...
  return {
    ...
    // When the target above was adopted, its OWN canonicalId is what is reported — the same
    // entity the surface was taken from, so the domain and the id can never name two men.
    requestedAuthorityId: authorityTarget ? (authorityTarget.canonicalId || null)
      : authorityEntity && authorityEntity.resolutionStatus === 'resolved'
