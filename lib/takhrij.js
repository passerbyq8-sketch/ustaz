...
// which is a lottery and not a rule. The floor is now read on the DIACRITIC-STRIPPED text, where
// \u00AB\u0627\u0644\u062D\u0645\u0648 \u0627\u0644\u0645\u0648\u062A\u00BB is 11 either way, and it is set below that length.
//
// WHY THERE IS A FLOOR AT ALL: a two-letter quotation is a word being discussed, not a matn being
// narrated. The prophetic-frame test below is the real gate; this only keeps the shortest
// fragments out of a library call. It is NOT the \u00A7\u0665-\u0665 item of the design draft \u2014 that one is about
// a two-word matn the SERVICE cannot phrase-match, and it is still open.
const MIN_MATN_BARE_CHARS = 10;
...
export function findMatns(text) {
  ...
  for (const span of proseSpans(value)) {
    ...
    while ((match = QUOTE_RE.exec(chunk)) !== null) {
      ...
      // The floor, on the bare letters — see MIN_MATN_BARE_CHARS.
      if (bareArabic(inner).length < MIN_MATN_BARE_CHARS) continue;
...
export function hadithCards(text) {
  ...
  while ((match = HADITH_CARD_RE.exec(value)) !== null) {
    ...
    if (bareArabic(matn).length < MIN_MATN_BARE_CHARS) continue;
    out.push({
      matn,
      quoted: match[0],
      start: match.index,
 ...
 *
 * The two can never overlap — `proseSpans` steps over every card block — so one sort by offset is
 * the whole merge.
 */
export function findTargets(text) {
  return [...findMatns(text), ...hadithCards(text)].sort((a, b) => a.start - b.start);
}
...
export function runnerLookup(runTool, ctx) {
  return async (matns, options = {}) => {
    ...
    return Promise.all((Array.isArray(matns) ? matns : []).map(async (matn) => {
      let out;
      try {
        out = await runTool('search_library', { query: matn }, { ...ctx, bookIds });
      } catch {
...
export async function applyTakhrij(text, input = {}) {
  ...
  // The prose matns AND the `<hadith>` cards, in the reader's own order.
  ...
  for (let index = 0; index < targets.length; index += 1) {
    // ── «ظهر في النتائج» IS READ ON THE RESULTS, AND THAT IS THE OWNER'S WORD ──
    // His rule fires «متى ظهر أحد الشيخين في النتائج ولم يظهر الآخر». The RESULTS are the
    // rows the library handed back, which is not the same set as the rows this pass went on
    // to confirm — a hit is dropped when its own text does not carry the matn.
    //
    // AND THE DIFFERENCE IS NOT ACADEMIC. MEASURED on ezik-shamela-20260820 for «إنما الأعمال
    // بالنيات وإنما لكل امرئ ما نوى»: صحيح البخاري comes back as the FIRST row, and the atom
    // it comes back with is the CHAPTER HEADING, which does not carry the matn. Confirmed
    // Shaykhs: zero. So a trigger read on the confirmed set does not fire, and the most
    // established hadith in the corpus ships as «(لا يثبت مرفوعا)» — a false sentence about
    // a hadith البخاري ومسلم both have. Read on the results it fires, the narrowed request
    // returns the narration itself, and the answer is «(البخاري)».
    if (oneShaykhOnly(states[index].seen)) rechecking.push(targets[index].matn);
  }
  ...
  for (let index = targets.length - 1; index >= 0; index -= 1) {
    ...
    const named = parenthetical.sourced ? agreed : { name: '', prayer: '', books: 0 };
...
/**
 * True when exactly one of the two Shaykhs is among the ids the library RETURNED — the one
 * position where «(متفق عليه)» may be owed and the first page of rows could not say so. Both
 * of them, or neither, is an answered question and costs no second call.
 */
function oneShaykhOnly(seen) {
  const rows = new Set();
