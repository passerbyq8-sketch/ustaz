...
import {
  NOT_RAISED, TAKHRIJ_LADDER_IDS, composeParenthetical, ladderRowFor,
...
// لا بتضييق النطاق». The seam below IS that one call: this pass calls `lookup` exactly once
// per answer and hands it every matn at once.
//
// AND THE ADAPTER BELOW IS WHERE TODAY'S SERVICE SHOWS ITS LIMIT. /search takes ONE `q`
// (C:\EZIK-LIB\service\src\contract.mjs:18 — q, limit, filters, max_chars_per_hit, phrase,
// author), so a literal single HTTP request cannot carry four different matns. The adapter
// therefore issues them TOGETHER, in parallel, capped at TAKHRIJ_MAX_MATNS — which costs one
// round-trip of wall-clock, the thing the owner's reason names. Closing it to one request
// needs a batch arm on the service and is written up as such in the report.
/**
 * @param {Function} searchLibrary  lib/lib-service.js's export, injected.
 * @param {object} options          flagValue, token, fetchImpl, signal — handed straight down.
 * @returns {Function} lookup(matns) -> Promise<Array<{matn, subjectIds, atoms}>>
 */
export function libraryLookup(searchLibrary, options = {}) {
  return async (matns) => Promise.all(matns.map(async (matn) => {
    let result;
    try {
      // THE SEARCH IS NARROWED TO THE LADDER, AND THAT IS NOT AN OPTIMISATION.
      // MEASURED on ezik-shamela-20260820, 2026-09-19: an unfiltered top-ten phrase search
      // for «إنما الأعمال بالنيات» returns commentaries and fatwa pages and NOT ONE ladder
      // book, so an unnarrowed lookup would answer «(لا يثبت مرفوعا)» for the single most
      // established hadith in the corpus. The ladder is 54 ids and the service takes 100
      // (C:\EZIK-LIB\service\src\contract.mjs:8), so the whole ladder rides in one request.
      result = await searchLibrary(matn, { ...options, limit: 10, bookIds: TAKHRIJ_LADDER_IDS });
    } catch {
    ...
    for (const record of records) {
      const id = record && record.provenance ? record.provenance.subject_id : null;
      subjectIds.push(id ? String(id) : '');
      atoms.push(typeof record?.text === 'string' ? record.text : '');
    }
    return { matn, subjectIds, atoms };
  }));
}
