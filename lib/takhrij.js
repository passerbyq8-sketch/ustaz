...
import { MISSING_ASKED_HADITH, terminalMissingHadith } from './reader-prose.js';
...
function unprovedSpeechFrame(text, target, floor, proved = false) {
  ...
  // D4 S1: these are already speech-governing introductions. Do not insert a second frame.
  const governing = new RegExp('(?:' + tolerant('لقوله') + '(?:\\s+' + D3A_PRAYER + ')?'
    + '|' + tolerant('حيث قال') + ')\\s*[:：]\\s*$', 'u').exec(lead);
  if (governing) return { start: floor + governing.index, end: target.start, replacement: governing[0] };
...
export async function applyTakhrij(text, input = {}) {
  ...
  if (!found.targets.length && !found.duplicates.length) {
    if (bracketAfterQuote && asksGradeOrSource(input.question)) {
      return { text: terminalMissingHadith(original + '\n\n' + MISSING_ASKED_HADITH), applied: true,
        problems: [], entries: [], reason: 'no_matn', gradingHead: '', missingText: MISSING_ASKED_HADITH };
    }
  ...
  if (head) out = head + '\n\n' + out;
  if (bracketAfterQuote) out = terminalMissingHadith(out);
