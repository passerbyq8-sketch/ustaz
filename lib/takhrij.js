...
function unprovedSpeechFrame(text, target, floor, proved = false) {
  ...
  const match = D3A_FRAME_RE.exec(lead) || new RegExp(tolerant('نص الحديث') + '\\s*[:：]\\s*$', 'u').exec(lead);
  if (!match) return null;
  const connection = /^[وف](?:قال|سمعت)/u.exec(bareArabic(match[0]));
  const replacement = (connection ? connection[0][0] : '') + (proved ? 'قال رسول الله ﷺ: ' : 'نص الحديث: ');
...
export async function applyTakhrij(text, input = {}) {
  ...
  if (bracketAfterQuote) {
    for (const entry of entries) {
      ...
      if (provenBooks.length) entry.proseProof = [...new Set(provenBooks)];
      const grade = ids.some(isShaykhayn) ? 'صحيح' : entry.grade;
      if (provenBooks.length && grade) entry.authenticityProof = { matn: entry.matn, books: [...new Set(provenBooks)], grade };
    }
  }
