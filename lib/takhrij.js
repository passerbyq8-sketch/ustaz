...
const PROSE_GRADE_WORD = '(?:حسن صحيح غريب|حسن صحيح|حسن غريب|صحيح غريب|صحيح|حسن|ضعيف جدا|ضعيف|موضوع|منكر|لا يصح|لا يثبت)';
...
const D3A_FRAME_RE = new RegExp('(?<![\\u0621-\\u064A])(?:'
  ...
  + ')\\s*[:：]?\\s*$', 'u');
function unprovedSpeechFrame(text, target, floor) {
  const lead = String(text).slice(floor, target.start);
  const match = D3A_FRAME_RE.exec(lead);
  if (!match) return null;
  const connection = /^[وف](?:قال|سمعت)/u.exec(bareArabic(match[0]));
  const replacement = match.groups.noun ? 'رواية الحديث: ' : match.groups.that ? 'أن في الرواية: '
    : (connection ? connection[0][0] : '') + 'ورد في الرواية: ';
...
export async function applyTakhrij(text, input = {}) {
  ...
  const frames = frameTargets.map((target, index) => bracketAfterQuote && (!states[index]?.propheticSpeech || states[index]?.narratorSpeech)
    ? unprovedSpeechFrame(original, target, index > 0 ? frameTargets[index - 1].end : 0) : null);
  ...
  for (let step = edits.length - 1; step >= 0; step -= 1) {
    ...
    // ج٤ (order C) — «لا يُسمّى صحابيٌّ إلّا إذا لم يطابقِ المقطعُ … إلّا حديثًا واحدًا»: when the atoms that carry
    // the quoted words are narrated by more than one man, they are more than one hadith, and nobody is named.
    // The parentheses and the match behind them are not touched.
    if (b2 && b2.name && narratorGroups(state.narrators).length > 1) {
      problems.push(TAKHRIJ_COMPANION_MANY_HADITHS);
...
export function headRestatedBy(head, body) {
  ...
  if (raised) tokens = [[raised[1]]];
    ...
    else if (!rest.includes('،')) tokens = [rest === 'البخاري ومسلم' ? RESTATED_AGREED : [rest]];
  }
...
function absorbAnswer(state, matn, answer) {
  ...
  const atomCarriesMatn = state && state.headTail === true ? atomCarriesMatnHeadTail : STRICT_CARRIER;
