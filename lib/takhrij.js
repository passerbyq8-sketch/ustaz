...
export async function applyTakhrij(text, input = {}) {
  ...
  const call = async (matns, options, n) => {
    ...
    try {
      ...
      if (!Array.isArray(answers)) {
        callFailures.push({ call: n, status: 'not-a-list', error: typeof answers, q: String(matns[0] || '').slice(0, 80) });
        return [];
      }
      for (const answer of answers) {
        if (answer && answer.failure) {
          callFailures.push({ call: n, status: String(answer.failure.status), error: String(answer.failure.error), q: String(answer.matn || '').slice(0, 80) });
        }
      }
      return answers;
    } catch (error) {
      callFailures.push({ call: n, status: 'threw', error: String((error && error.message) || error).slice(0, 120), q: String(matns[0] || '').slice(0, 80) });
      return [];
    }
  };
  ...
  return { text: out, applied: out !== original, problems: [...new Set(problems)], entries, trace, callFailures, reason: 'applied' };
}
