...
export function carriedInTheProphetsVoice(atomText, matn) {
  const hay = foldArabic(atomText);
  const needle = foldArabic(matn);
  if (!hay || !needle) return false;
  if (ownWordCount(needle.split(' ').filter(Boolean)) >= ANCHOR_MIN_OWN_WORDS) return true;
  const padded = ' ' + hay + ' ';
  // Every occurrence, with the joining «و»/«ف» `carriesWords` accepts on the first word.
  const starts = [];
  for (const lead of [' ', ' و', ' ف']) {
    for (let at = padded.indexOf(lead + needle + ' '); at >= 0; at = padded.indexOf(lead + needle + ' ', at + 1)) starts.push(at);
  }
  for (const at of starts.sort((a, b) => a - b)) {
    ...
    if (last < 0) continue;
    const gap = ' ' + window.slice(last) + ' ';
    if (COMPANION_VOICE_RE.test(gap) || OTHER_SPEAKER_GAP_RE.test(gap)) continue;
    return true;
  }
  return false;
}
...
function absorbAnswer(state, matn, answer, door = false) {
  ...
  for (let i = 0; i < ids.length; i += 1) {
    ...
    if (!carriedInTheProphetsVoice(atom, matn)) continue;
