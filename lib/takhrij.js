...
import {
  NOT_RAISED, SHAYKHAYN_IDS, TAKHRIJ_LADDER_IDS, composeParenthetical, isShaykhayn, ladderRowFor,
...
export async function applyTakhrij(text, input = {}) {
  ...
  for (let index = targets.length - 1; index >= 0; index -= 1) {
    ...
    const at = named.name ? Math.max(target.leadStart, floor) : target.start;
