...
// B2 — ...and one that was KEPT, because the rest of its lead-in points back at him («قال له»).
export const TAKHRIJ_MODEL_COMPANION_KEPT = 'TAKHRIJ_MODEL_COMPANION_KEPT';
// A pronoun in the lead-in that points back at the narrator the model named.
const B2_BACK_PRONOUN_RE = /(?:^|[\s،:])(?:له|لها|لهما|لهم)(?=[\s،:.؛]|$)/u;
...
const B2_LINK_RE = /(?:^| )[وف]?(?:حدثنا|حدثني|[اأ]خبرنا|[اأ]خبرني|[اأ]نب[اأ]نا|ثنا|عن|سمعت|أن) (?!(?:رسول|النبي|الرسول)(?: |$)|\uFDFA)[^ ]+/u;
...
export async function applyTakhrij(text, input = {}) {
  ...
  const states = targets.map((target) => {
    ...
    if (bracketAfterQuote) state.fullAnswer = true;
  ...
  if (bracketAfterQuote) {
    ...
    for (let index = 0; index < targets.length; index += 1) {
      ...
      // ...but not where the rest of the lead-in points back at him («أن النبي ﷺ قال له:»): the name
      // would leave and «له» would point at nobody. Measured on a saved answer (order B, ب٢ verification);
      // the model's name then stays, and the log says so.
      if (opener && B2_BACK_PRONOUN_RE.test(bareArabic(original.slice(opener.end, t.start)))) {
