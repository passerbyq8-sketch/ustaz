...
// ── [111-b4b-52] · A KINSMAN IS A SPEAKER, AND HIS WORDS ARE NOT THE PROPHET'S ﷺ ───────────────
// MEASURED at eca359e: «وكان النبي ﷺ … وقال ابنُه: «…».» — [b43] stopped «قال ابنُه» being read as the
// collector «قال ابن», and nothing was put in its place, so the son's own words were read as a matn of the
// Prophet ﷺ named earlier in the sentence and the parentheses were asked for them. A frame naming a
// kinsman is a HUMAN credit like «قال الشيخ»: he is the last speaker named, and what he says is his.
// Item 21 is untouched: a Companion's words INSIDE a quotation of the Prophet's ﷺ are that rule's, not this.
const KIN_SPEAKER_WORDS = ['ابنه', 'ابنها', 'ابنته', 'ابنتها', 'أبوه', 'أبوها', 'أمه', 'أمها',
  'جده', 'جدها', 'جدته', 'أخوه', 'أخته', 'عمه', 'عمته', 'خاله', 'خالته', 'زوجته', 'زوجها',
  'والده', 'والدته', 'مولاه', 'مولاها'];
const KIN_CREDIT_WORDS = [];
for (const verb of ['قال', 'قالت', 'يقول', 'تقول']) {
  for (const kin of KIN_SPEAKER_WORDS) KIN_CREDIT_WORDS.push(verb + ' ' + kin);
}
HUMAN_CREDIT_WORDS.push(...KIN_CREDIT_WORDS);
...
// [111-b4b-48] — the frame in front of a matn names its Companion already: his prayer, or his name.
const FRAME_PRAYER_RE = /رضي\s+الله\s+عن(?:هما|هم|ها|ه)(?![\u0621-\u064A])/u;
function companionNamedInFrame(value, target, floor, name) {
  const anchor = target.leadStart;
  let from = lastIndexOfPattern(value.slice(0, anchor), BOUNDARY_RE) + 1;
  if (!value.slice(from, anchor).trim()) {
    const prev = value.slice(0, from).replace(/\s+$/u, '');
    if (/[:\uFF1A]$/u.test(prev)) from = lastIndexOfPattern(prev.slice(0, -1), BOUNDARY_RE) + 1;
  }
  const frame = value.slice(Math.max(from, Number.isInteger(floor) ? floor : 0), anchor);
  const bare = bareArabic(frame);
  return FRAME_PRAYER_RE.test(bare) || foldArabic(frame).includes(foldArabic(name));
}
...
// [111-b4b-47] — and the forms the prose writes that this pass did not read: the two Shaykhs' agreement
// as a noun («لاتفاق الشيخين عليه»), the book named with no verb at all («عند مسلم أيضا»), and the active
// verb in the head of its sentence («روى البخاري أن…»). MEASURED at eca359e (S-W6a, S-W7a, T2-rawa): the
// pass read no prose attribution in them, so it was free to write parentheses beside a credit the prose
// had already made.
const ATTRIBUTION_PHRASE_WORDS = ['متفق عليه', 'في الصحيحين', 'رواه الشيخان', 'أخرجه الشيخان',
  'اتفاق الشيخين', 'لاتفاق الشيخين', 'اتفق عليه الشيخان'];
const ATTRIBUTION_BOOK_WORDS = ['البخاري', 'مسلم', 'الترمذي', 'النسائي', 'أحمد', 'مالك', 'الحاكم',
  'الطبراني', 'البيهقي', 'الدارقطني', 'الدارمي', 'أبي داود', 'ابن ماجه', 'ابن حبان', 'ابن خزيمة'];
const ATTRIBUTION_ACTIVE_WORDS = ['روى', 'أخرج', 'خرج'];
const ATTRIBUTION_AT_WORDS = ['عند', 'في'];
...
const ATTRIBUTION_RE = new RegExp(
  ...
  // [111-b4b-47] — «روى البخاري أن…»، «عند مسلم أيضا»، «في صحيح مسلم».
  + '|(?:' + alternation(ATTRIBUTION_ACTIVE_WORDS) + ')\\s+(?:' + alternation(ATTRIBUTION_BOOK_WORDS) + ')' + WORD_END
  + '|(?:' + alternation(ATTRIBUTION_AT_WORDS) + ')\\s+(?:صحيح\\s+)?(?:' + alternation(ATTRIBUTION_BOOK_WORDS) + ')' + WORD_END
  + ')',
  'u',
);
 ...
 */
// ── [111-close-7] · THE OWNER'S ROW 70: THE LEAD-IN LINE IS THE MATN'S SENTENCE ─────────────────────────
// MEASURED in the owner's own trial (order of 23 Sep, row 70): «رواه مسلم: «صلّى رسولُ الله…» (مسلم)» and «وبحديثِ أبي
// ذرٍّ عند مسلمٍ… «المسبل إزاره» (البخاري)» — the parentheses written beside a matn whose collector the prose had
// already named. Reproduced here, no model: the window reads back only to the last «.»/«؟»/newline, and the answer
// set the matn on a line of its own under a lead-in that ends on «:» — «ويدل على ذلك ما رواه مسلم:⏎«…»». The newline
// ended the window one line short of the credit, and `statedAttributionNear` returned ''. The contract's sixth rule:
// «القوسانِ لا يزاحمانِ نسبةً كتبَها النثر». So where nothing but blank space stands between the last boundary and the
// matn, and the line before ends on a colon (or on the lead-in's own words), that line is the matn's sentence and is
// read too. Nothing else about the window moves.
const LEAD_IN_LINE_END_RE = /[:：،]\s*$/u;
export function statedAttributionNear(text, target, floor = 0, ceiling = -1) {
  const value = String(text == null ? '' : text);
  const head = value.slice(0, target.start);
  let boundary = lastIndexOfPattern(head, BOUNDARY_RE);
  // [111-close-7] — a lead-in line ending on «:» above a matn that stands alone is read with it.
  while (boundary >= 0 && !/\S/u.test(head.slice(boundary + 1).replace(/[«"“]/gu, ''))
    && LEAD_IN_LINE_END_RE.test(head.slice(0, boundary))) {
    const before = head.slice(0, boundary);
    const prev = lastIndexOfPattern(before, BOUNDARY_RE);
    boundary = prev;
    if (/\S/u.test(before.slice(prev + 1))) break;
  }
...
// ── [111-roots-69] · THE ATOM CARRIES THE MATN BY ITS WORDS — NOT BY ITS LETTERS, NOR BY ITS FORMULA ──
// MEASURED on the preview of 20308f8 (roots round 1, Q5 «مفصّل»): «ما أسفلَ من الكعبين ففي النار» went out as
// «(متفق عليه)». مسلم's atom was the muḥrim's khuffs — «وليقطعهما أسفل من الكعبين» — and it passed because the
// comparison was on LETTERS: the anchor «ما اسفل من الكعبين» sat inside «وليقطعهما اسفل من الكعبين», its «ما»
// the tail of another word. And the b4b battery: «إماطة الأذى عن الطريق صدقة» proved by an atom without «صدقة»,
// «أن النبي ﷺ احتجم وهو صائم» and «مسح رسول الله ﷺ على العمامة والموقين» by anchors that were the Prophet's
// formula and nothing else. THE OWNER'S RULE (order of 23 Sep, row 69): an atom that does not carry the quoted
// matn proves no parentheses, and the anchor is never the formula alone. So: words are compared as words;
// a matn of fewer than eight words of its own (the formula's words not counted) is carried whole or not at
// all; and a longer one's anchor holds four words of its own besides the formula.
const MATN_FRAME_WORDS = new Set(['ان', 'انه', 'النبي', 'رسول', 'الله', 'صلي', 'عليه', 'وسلم', 'ﷺ', 'قال', 'قالت', 'يقول', 'كان', 'عن']);
const ANCHOR_MIN_OWN_WORDS = 8;
const ownWordCount = (words) => words.filter((w) => !MATN_FRAME_WORDS.has(w)).length;
const carriesWords = (haystack, needle) => (' ' + haystack + ' ').includes(' ' + needle + ' ')
  // the book joins the quotation to what it follows: «وما أعطي أحد…» carries «ما أعطي أحد…» (البخاري 1469)
  || (' ' + haystack + ' ').includes(' و' + needle + ' ') || (' ' + haystack + ' ').includes(' ف' + needle + ' ');

export function atomCarriesMatn(atomText, matn) {
  const haystack = foldArabic(atomText);
  const needle = foldArabic(matn);
  if (!haystack || !needle) return false;
  if (carriesWords(haystack, needle)) return true;
  // A quotation is often the matn's opening clause. Anchor on its first words.
  const words = needle.split(' ').filter(Boolean);
  if (words.length < MIN_ANCHOR_WORDS) return false;
  if (ownWordCount(words) < ANCHOR_MIN_OWN_WORDS) return false; // [111-roots-69]
  const cut = Math.max(MIN_ANCHOR_WORDS, Math.ceil(words.length * 0.6));
  if (ownWordCount(words.slice(0, cut)) < MIN_ANCHOR_WORDS) return false; // [111-roots-69]
  const anchor = words.slice(0, cut).join(' ');
  if (!carriesWords(haystack, anchor)) return false;
...
// ── [111-b4b-20] · THE FOURTH DOOR, AND IT IS NARROW ────────────────────────────────────────
// THE OWNER'S DECISION (22 September, «مع توصيتك»): a fourth tolerance in the matching rule — ONE letter
// of five (ب · و · ف · ل · ك) joined to the start of ONE word, present on one side and absent on the other,
// and the rest of the words letter for letter. MEASURED (batch 4, the twin): «أوصاني خليلي … بثلاث: صيام
// ثلاثة أيام من كل شهر…» left as «(البخاري)» because مسلم 721 writes «بصيام»; the atom came back and failed
// on that one letter. CLAUDE'S BOUNDS: one word; the letter extra on one side and missing on the other;
// everything else compared exactly as `atomCarriesMatn` compares it (the same folding, the same whole-
// needle-or-opening-anchor rule, the same Companion's-voice refusal); never together with the
// singular/plural door (it is asked only of the matn as written, not of a variant); never for a matn
// under four words; and — being a tolerance of MATCHING, not a search by near wording — only where some
// ladder book already carries the matn exactly. See `absorbAnswer`.
const PREFIX_DOOR_LETTERS = new Set(['ب', 'و', 'ف', 'ل', 'ك']);
function prefixTwin(a, b) {
  if (a.length === b.length + 1) return PREFIX_DOOR_LETTERS.has(a[0]) && a.slice(1) === b && b.length >= 2;
  if (b.length === a.length + 1) return PREFIX_DOOR_LETTERS.has(b[0]) && b.slice(1) === a && a.length >= 2;
  return false;
}
/** Does the atom carry `needle` (folded words) with exactly one word through the fourth door? The index, or -1. */
function prefixDoorAt(hay, needle) {
  for (let i = 0; i + needle.length <= hay.length; i += 1) {
    let door = 0;
    let k = 0;
    for (; k < needle.length; k += 1) {
      if (hay[i + k] === needle[k]) continue;
      if (door === 0 && prefixTwin(hay[i + k], needle[k])) { door = 1; continue; }
      break;
    }
    if (k === needle.length && door === 1) return i;
  }
  return -1;
}
// ── [111-close-1b] · …AND THE DOOR CARRIES BY THE SAME RULES AS THE EXACT COMPARISON ───────────────────────────
// MEASURED on the preview of this round's tip (battery Q21 «مفصّل»): «أَنَّ النَّبِيَّ ﷺ احْتَجَمَ وَهُوَ صَائِمٌ» went out
// «(متفق عليه)» — مسلم does not narrate it. Reproduced on the twin: no مسلم atom carries it, and one came in through this
// door: the matn has three words of its own, so the door took its opening six — «أن النبي صلى الله عليه وسلم», the
// formula alone — and matched it one prefix letter away («فأن النبي…»). [111-roots-69] had closed exactly that for the
// exact comparison (a short matn is carried whole or not at all; an anchor holds four words besides the formula), and
// the door was never told. It is told now, word for word.
export function atomCarriesMatnByOnePrefix(atomText, matn) {
  const hay = foldArabic(atomText).split(' ').filter(Boolean);
  const words = foldArabic(matn).split(' ').filter(Boolean);
  if (!hay.length || words.length < MIN_ANCHOR_WORDS) return false;
  if (prefixDoorAt(hay, words) >= 0) return true;
  if (ownWordCount(words) < ANCHOR_MIN_OWN_WORDS) return false; // [111-close-1b]
  const cut = Math.max(MIN_ANCHOR_WORDS, Math.ceil(words.length * 0.6));
  if (ownWordCount(words.slice(0, cut)) < MIN_ANCHOR_WORDS) return false; // [111-close-1b]
  if (cut >= words.length || prefixDoorAt(hay, words.slice(0, cut)) < 0) return false;
  return !COMPANION_VOICE_RE.test(' ' + words.slice(cut).join(' ') + ' ');
}
...
// ── [111-roots-69] · A SHORT MATN IS THE PROPHET'S ﷺ IN THE ATOM, OR IT IS NOT CARRIED ──────────
// MEASURED on the preview of 20308f8 (roots round 1, Q6 «مفصّل»): «ليس في الحلي زكاة», framed by the answer as
// «رُوي عن النبي ﷺ», went out «(الترمذي · لم يوقف على حكم)». The Tirmidhī atom has those four words — as the
// saying of Companions: «وقال بعض أصحاب النبي ﷺ منهم: ابن عمر، وعائشة، وجابر…، وأنس بن مالك: ليس في الحلي
// زكاة». Every word carried, and not one of them the Prophet's. A short matn (under eight words of its own) is
// therefore confirmed only where the atom gives it to him: the Prophet is named in the words before it, and no
// other speaker stands between his name and it — or it opens a numbered entry of a book of entries.
// ...nor a new entry or a new chain between them: an entry number, «حدثنا», «وعن» — the Prophet named before them
// belongs to the entry before (الدارقطني 1954 → 1955 «عن جابر: قال «ليس في الحلي زكاة»»).
const OTHER_SPEAKER_GAP_RE = /(?:^| )(?:منهم|اصحاب|التابعين|بعض اهل العلم|قالوا|\d+|حدثنا|حدثني|اخبرنا|اخبرني|ثنا|وعن)(?= |$)/u;
// «حديث: «…»» and «في الحديث: «…»» name what follows as a hadith, as the Prophet's name does.
const PROPHET_MARK_RE = /(?:^| )(?:النبي|رسول الله|صلي الله عليه وسلم|ﷺ|(?:و|ف)?(?:ال)?حديث)(?= |$)/gu;
const VOICE_WINDOW_WORDS = 80;
// ── [111-close-1] · …AND A LONG ONE TOO: A CHAPTER HEADING IS NOT A HADITH ────────────────────────
// MEASURED on the preview of efcdcf4 (roots phase 3, battery Q13, «طالب علم» and «مفصّل»): «صَوْمُ ثَلَاثَةِ
// أَيَّامٍ مِنْ كُلِّ شَهْرٍ صَوْمُ الدَّهْرِ كُلِّه» went out «(ابن خزيمة · صحيح)». The confirming atom FC-000685 is
// Ibn Khuzayma's own comment and then a chapter heading — «(182) بَاب ذِكْر الدَّلِيلِ عَلَى أَنَّ صَوْمَ ثَلَاثَةِ
// أَيَّامٍ مِنْ كُلِّ شَهْرٍ يَقُومُ مَقَامَ صِيَامِ الدَّهْرِ» — with no word of the Prophet ﷺ in it. The anchor held,
// and the voice was never asked because the matn had eight words. THE OWNER'S DECISION (order of 23 Sep, row 1):
// the voice is asked of every matn however long, so a heading, an author's words or a narrator's saying
// proves no parentheses. A long matn is looked for whole, then by the anchor `atomCarriesMatn` accepted it
// by; and one that names the Prophet ﷺ in its own opening («أن النبي ﷺ…»، «كان رسول الله ﷺ…») is his
// report by its own words, unless a heading or an entry stands right before it.
const HEADING_GAP_RE = /(?:^| )(?:باب|ذكر|كتاب)(?= |$)/u;
const PROPHET_NAMED_RE = /^(?:\S+ ){0,3}(?:النبي|رسول الله|صلي الله عليه وسلم|ﷺ)(?= |$)/u;
// «عن أبي هريرة روايةً»، «يبلغ به»، «يرفعه» — the muhaddithin's words for «from the Prophet ﷺ» (البخاري 5889).
const MARFU_MARK_RE = /(?:^| )(?:روايه|يرفعه|رفعه|مرفوعا|يبلغ به)(?= |$)/gu;
const HEADING_LOOKBACK_WORDS = 8;
// A long matn's narration carries footnote marks («مسكتان 2 غليظتان») and page marks («ومن يستغن ص 123 يغنه
// الله», البخاري 1469); neither is an entry number.
const FOOTNOTE_RE = /(?:^| )(?:ص \d+|\d)(?= |$)/gu;
export function carriedInTheProphetsVoice(atomText, matn, { listBook = false } = {}) {
  const hay = foldArabic(atomText);
  const needle = foldArabic(matn);
  if (!hay || !needle) return false;
  const words = needle.split(' ').filter(Boolean);
  const long = ownWordCount(words) >= ANCHOR_MIN_OWN_WORDS;
  // A list of entries (صحيح الجامع، ضعيف الجامع) is a list of his sayings, as it was before [111-close-1].
  if (long && listBook) return true;
  const probes = [needle];
  if (long) probes.push(words.slice(0, Math.max(MIN_ANCHOR_WORDS, Math.ceil(words.length * 0.6))).join(' '));
  const padded = ' ' + hay + ' ';
  // Every occurrence, with the joining «و»/«ف» `carriesWords` accepts on the first word.
  const starts = [];
  for (const probe of probes) {
    for (const lead of [' ', ' و', ' ف']) {
      for (let at = padded.indexOf(lead + probe + ' '); at >= 0; at = padded.indexOf(lead + probe + ' ', at + 1)) {
        if (!starts.includes(at)) starts.push(at);
      }
    }
  }
  // «أن النبي ﷺ…»، «كان رسول الله ﷺ…»، «نهى رسول الله ﷺ…» — a report of him by its own opening words.
  const namesHim = long && PROPHET_NAMED_RE.test(needle);
  for (const at of starts.sort((a, b) => a - b)) {
    const before = padded.slice(0, at + 1).trim();
    if (/(?:^| )\d+$/u.test(before)) return true; // a numbered entry opens on it
    // [111-close-1] — an atom that opens on a long matn gives no context against it, as before.
    if (long && !before) return true;
    const window = before.split(' ').slice(-VOICE_WINDOW_WORDS).join(' ');
    let last = -1;
    for (const m of window.matchAll(PROPHET_MARK_RE)) last = m.index + m[0].length;
    if (long) for (const m of window.matchAll(MARFU_MARK_RE)) last = Math.max(last, m.index + m[0].length);
    if (last >= 0) {
      let gap = ' ' + window.slice(last) + ' ';
      if (long) gap = gap.replace(FOOTNOTE_RE, ' ');
      if (!COMPANION_VOICE_RE.test(gap) && !OTHER_SPEAKER_GAP_RE.test(gap) && !(long && HEADING_GAP_RE.test(gap))) return true;
    }
    if (namesHim && !HEADING_GAP_RE.test(' ' + before.split(' ').slice(-HEADING_LOOKBACK_WORDS).join(' ') + ' ')) return true;
  }
  return false;
}
...
// [111-roots-isnad] — the link between the two «عن» is a man, and a man is not followed by a colon or a comma:
// MEASURED on the preview of 20308f8 (roots R2 Q12, all three modes), «رُفِعَ القَلَمُ عَنْ ثَلَاثَةٍ: عَنِ النَّائِمِ…»
// read as a chain, and the pass refused the parentheses (TAKHRIJ_ISNAD_REFUSED) for a hadith the library carries.
...
export async function applyTakhrij(text, input = {}) {
  ...
  if (!found.targets.length && !found.duplicates.length) {
    ...
    // MEASURED at eca359e: «ما درجة حديث «…»؟» answered in prose that quotes nothing left this pass at
    // `no_matn`, before [b27] is ever reached, so the question went unanswered and no head was written.
    // [b27b] byte for byte: the head is for the text the reader is asking about and for nothing else —
    // here that text is his own quotation, asked of the library on its own, and the head says what it
    // proved or says plainly that it proved nothing, never denying that the hadith exists.
    const askedAlone = askedText(input.question);
    if (askedAlone && asksGradeOrSource(input.question) && typeof input.lookup === 'function') {
      const alone = await applyTakhrij('قال رسول الله صلى الله عليه وسلم: «' + askedAlone + '».',
        { env: input.env, lookup: input.lookup });
      const said = gradingHead(input.question, alone.entries || [], alone.callFailures || []);
      if (said) {
        return { text: said + '\n\n' + original, applied: true, problems: [], entries: [],
          callFailures: alone.callFailures || [], reason: 'no_matn', gradingHead: said };
      }
    }
    return { text: original, applied: false, problems: [], entries: [], reason: 'no_matn', askedProof: await proofForAsked(input, []) };
  }
  ...
  const states = targets.map((target) => {
    ...
    absorbAnswer(state, target.matn, first.get(target.matn), true);
    return state;
  });
  ...
  if (rechecking.length) {
    ...
    for (const bookId of SHAYKHAYN_IDS) {
      ...
      for (const one of rechecking) absorbAnswer(states[one.index], one.query, second.get(one.query), one.query === targets[one.index].matn);
    }
  }
  ...
  if (ruling.length) {
    ...
    for (const one of ruling) absorbAnswer(states[one.index], one.query, third.get(one.query), one.query === targets[one.index].matn);
  }
  ...
  for (let step = edits.length - 1; step >= 0; step -= 1) {
    ...
    // MEASURED at eca359e (e55-nothing-proved-whole): «حديث «اطلبوا العلم ولو بالصين» رواه البخاري ومسلم…»
    // — the grader's ruling «(لا يثبت مرفوعا)» was WITHHELD because the prose had named books, the seal then
    // cut that credit (no page proves it), and what reached the reader was «حديث «اطلبوا العلم ولو بالصين».»:
    // a fabricated matn framed as a hadith with nothing said about it. §٢'s rule is about not ARGUING with a
    // credit the prose already gave; «لا يثبت مرفوعا» credits no book — it is the library's own ruling that
    // the matn is not the Prophet's ﷺ (b34), and it is written even where the prose named books.
    if (stated && parenthetical.text === NOT_RAISED) {
    ...
    // MEASURED at eca359e, fixed draft through this pass: «ومن ذلك حديث علي رضي الله عنه أن النبي ﷺ قال: «…»»
    // left as «…حديث علي رضي الله عنه أن النبي عن علي رضي الله عنه: «…»» — the opener put a second time
    // and «ﷺ قال» eaten with the lead-in it replaced. The rule: where the prose already names the
    // Companion in this matn's own frame — its sentence, or the line that ends on a colon and
    // introduces it — no opener is added and the lead-in stays as written.
    const namedInFrame = !denied && agreed.name && companionNamedInFrame(original, target, floor, agreed.name);
    const named = denied || namedInFrame ? { name: '', prayer: '', books: 0 } : agreed;
  ...
  if (head) out = head + '\n\n' + out;
  const askedProof = await proofForAsked(input, entries); // [111-close-2]
  return { text: out, applied: out !== original, problems: [...new Set(problems)], entries, trace, callFailures, reason: 'applied', gradingHead: head, askedProof };
}
...
export function gradingHead(question, entries, callFailures = []) {
  ...
  if (p === AGREED_UPON) return 'تخريج الحديث: البخاري ومسلم.';
  if (p === 'البخاري' || p === 'مسلم') return e.head ? '' : 'تخريج الحديث: ' + p + '.';
  if (p === NOT_RAISED) {
  ...
  if (grade === NO_RULING) return 'تخريج الحديث: ' + outlet + '، ' + (split || 'ولم تُثبت المكتبة حكمًا عليه') + '.';
  if (!graders.length) return '';
  return 'تخريج الحديث: ' + outlet + '، والحكم عليه في ' + books(graders) + ': ' + grade + '.';
}
...
// MEASURED on the preview of efcdcf4 (roots phase 3, battery Q3 — and the owner's own witness in row 64): «ما صحة
// حديث النظافة من الإيمان؟» was answered «حديثٌ ضعيفٌ لا يصحّ…، رواه الترمذيُّ بإسنادٍ ضعيف، كما بيّن ذلك ابنُ باز».
// The library has no Tirmidhī row for it, and the credit passed because the sentence quotes nothing: the seal ties a
// credit to the matn QUOTED beside it ([111-roots-64]) and had none to tie this one to. THE OWNER'S DECISION (order
// of 23 Sep, row 2): the question's own text is the sentence's matn — the text asked about, as in [b27b] — so the
// credit is held to the books the library's atoms prove for it, pruned on [e55], and the ruling stays.
// The asked text is the question's quotation, or, where it quotes nothing, the words after «حديث» up to its end
// mark. This pass asks the library for it only where no matn of the answer is that text already, and it hands the
// seal the books and the text; it writes nothing into the answer.
const ASKED_TAIL_WORDS = new Set(['ثابت', 'صحيح', 'ضعيف', 'موضوع', 'حسن', 'ثابته', 'صحيحه']);
export function askedMatnOf(question) {
  const quoted = askedText(question);
  if (quoted) return quoted;
  const q = String(question == null ? '' : question);
  const m = q.match(/(?:^|[\s\u060C,])(?:[وف]?ب?)?(?:ال)?حديث\s+([^؟?.!\n«»"]+)/u);
  if (!m) return '';
  const words = m[1].trim().split(/\s+/u).filter(Boolean);
  while (words.length && ASKED_TAIL_WORDS.has(foldArabic(words[words.length - 1]))) words.pop();
  return words.length >= 2 && words.length <= 20 ? words.join(' ') : '';
}
async function proofForAsked(input, entries) {
  const asked = askedMatnOf(input && input.question);
  if (!asked || typeof (input && input.lookup) !== 'function') return null;
  const booksOf = (entry) => {
    if (!entry || entry.parenthetical === NOT_RAISED) return [];
    const out = [];
  ...
  const same = (Array.isArray(entries) ? entries : []).find((e) => e && sameMatn(e.matn, asked));
  if (same) return { matn: asked, books: booksOf(same) };
  const alone = await applyTakhrij('قال رسول الله صلى الله عليه وسلم: «' + asked + '».', { env: input.env, lookup: input.lookup });
  return { matn: asked, books: booksOf((alone.entries || [])[0]) };
}
...
// [111-close-1] — the two lists of entries, every entry a saying of the Prophet ﷺ (lib/takhrij-ladder.js row 25).
const LIST_ENTRY_IDS = new Set(['FC-000788', 'FC-000791']);
...
// MEASURED on the preview of 70e12db (battery Q1 «طالب علم»): «جعل رسول الله ﷺ ثلاثة أيام ولياليهن للمسافر، ويومًا وليلة
// للمقيم، إذا تطهّر فلبس خفّيه أن يمسح عليهما» went out «(مسلم)». Its opening is مسلم 276; its tail «إذا تطهّر فلبس خفّيه
// أن يمسح عليهما» is from the hadith of Abū Bakra (ابن ماجه 556), and مسلم's atom carries the opening and not the tail: the
// anchor (the first 60% of the words) proved a bracket for a quotation of which 40% is another hadith. THE OWNER'S
// RULE (the voice check is asked of every matn however long; order of 23 Sep, row 69's remainder, the long matn): an
// atom proves the parentheses only where the whole quoted matn is in it, with the matching tolerances already in force
// — the same folding, the joining «و/ف» of `carriesWords`, and the fourth door where it takes the whole matn. Where no
// atom does, the parentheses are withheld on the path that already withholds them; the matn is not touched.
// The prose's own credit follows it: [e55] reads its proof off `state.confirmed`, so «رواه مسلم» after such a quotation is
// pruned as the parentheses are withheld (measured on the real handler, twin library). `atomCarriesMatn` itself is not
// changed: `rulingWrittenFor` still reads it as before. Re-proved on the twin: of the 41 live brackets, the 34 whose atom
// carries the whole matn are untouched; the four anchor-only ones move (see the commit).
function carriesTheWholeMatn(narration, matn) {
  const hay = foldArabic(narration);
  const needle = foldArabic(matn);
  return Boolean(hay && needle) && carriesWords(hay, needle);
}
function carriesTheWholeMatnByOnePrefix(narration, matn) {
  const hay = foldArabic(narration).split(' ').filter(Boolean);
  const words = foldArabic(matn).split(' ').filter(Boolean);
  return hay.length > 0 && words.length >= MIN_ANCHOR_WORDS && prefixDoorAt(hay, words) >= 0;
}
 ...
 */
function absorbAnswer(state, matn, answer, door = false) {
  const throughDoor = []; // [111-b4b-20]
  ...
  for (let i = 0; i < ids.length; i += 1) {
    ...
    // [111-b4b-20] — an atom the exact comparison refuses is held for the door, which opens only after this pass.
    if (door && !isShortQuote(matn) && !atomCarriesMatn(narrationOf(atom), matn)
      && atomCarriesMatnByOnePrefix(narrationOf(atom), matn)
      && carriesTheWholeMatnByOnePrefix(narrationOf(atom), matn)) throughDoor.push(i); // [111-complete-6]
    if (!atomCarriesMatn(narrationOf(atom), matn)) continue;
    if (!carriesTheWholeMatn(narrationOf(atom), matn)) continue; // [111-complete-6]
    // [111-roots-69] — and a short one only where the atom gives it to the Prophet ﷺ.
    if (!carriedInTheProphetsVoice(atom, matn, { listBook: LIST_ENTRY_IDS.has(ids[i]) })) continue;
  ...
  // [111-b4b-20] — THE FOURTH DOOR, only where a ladder book already carries this matn exactly. A book
  // through it is a book, nothing more: no ruling is read off its atom and no Companion either.
  if (!throughDoor.length || !state.confirmed.length) return;
  for (const i of throughDoor) {
    const row = ladderRowFor(ids[i]);
    if (!row || ENTRY_GRADED_IDS.has(ids[i]) || row.grader) continue;
    if (!state.confirmed.includes(ids[i])) state.confirmed.push(ids[i]);
    const through = state.throughPrefixDoor || (state.throughPrefixDoor = []);
    through.push({ id: ids[i], matn, atom: String(atoms[i] || '') });
  }
}
...
// ── [111-close-13] · THE OWNER'S DECISION 6: THE ENTRY'S RULING IS READ WHERE THE ENTRY WRITES IT ──────────────
// MEASURED over the roots rounds (round 1 Q12 «طالب علم», C-b34-late-ruling): «إن الله يبعث لهذه الأمة على رأس كل مائة
// سنة من يجدد لها دينها» went out «(أبو داود · لم يوقف على حكم)» with السلسلة الصحيحة 599 among its confirming atoms. The
// entry opens «599 - "<المتن>". أخرجه أبو داود…» and closes on al-Albānī's own verdict — «…قلت: …، والسند صحيح رجاله
// ثقات رجال مسلم» — two hundred characters past the matn, where the forty-character window never reached. THE OWNER'S
// DECISION (order of 23 Sep, row 13, option أ): the entry's ruling is read wherever it stands in that entry. So, for an
// entry that OPENS on the matn (its number, then the matn), the entry's text up to the next numbered entry is read for
// the grader's own concluding verdict — «والسند/وإسناده/وهذا إسناد/فالحديث/والحديث + صحيح|حسن|ضعيف…» — and it is that
// entry's ruling only where every such verdict in it agrees. A discussion of another route that disagrees leaves it
// unread, as before; and nothing here reads an entry that does not open on this matn.
const ENTRY_VERDICT_RE = /(?:^| )(?:[وف]?(?:السند|الاسناد|اسناده|سنده|الحديث)|وهذا (?:اسناد|سند)|فهذا (?:اسناد|سند)) (صحيح|حسن|ضعيف(?: جدا)?|موضوع|باطل|منكر)(?= |$)/gu;
const ENTRY_SPLIT_RE = /(?:^|\s)\d{1,5}\s+-\s+/u;
function entryVerdictFor(atomText, matn) {
  const needle = foldArabic(matn);
  if (!needle) return '';
  // The book's own entries: «599 - "…"», each up to the next «<number> - ».
  const chunks = String(atomText || '').split(ENTRY_SPLIT_RE);
  for (const chunk of chunks.slice(1)) {
    const hay = foldArabic(chunk);
    if (!hay.startsWith(needle)) continue;
    const found = new Set([...hay.slice(needle.length).matchAll(ENTRY_VERDICT_RE)].map((m) => m[1]));
...
export function rulingWrittenFor(atomText, matn) {
  ...
  return entryVerdictFor(raw, matn); // [111-close-13]
}
