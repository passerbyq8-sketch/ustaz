...
import { bareGradeSpans, carriesGeneralSpeakerGrade, dropUnsourcedGrades, lockTakhrij } from './takhrij-lock.js';
import { COLON_RE } from './colon-preamble.js';
import { findTargets } from './takhrij.js';
 ...
 */
/** THIRD ORDER, STEP 7 — would the grade rule change this unit if it were a line of the answer? */
const GRADE_CARRIER = 'ا\n';
export function wouldDropGrade(unit) {
  // The rule can change a unit only through a grade span or a <hadith> card's ruling; with neither,
  // it is not run at all. MEASURED: running it on every unit cost the first delta ~5 ms (median) on
  // the forty real answers; asked this way the unit it could change is still asked in full.
  const value = String(unit == null ? '' : unit);
  // BATCH 4 [b12] — and a grade a general speaker carries («وقال بعض أهل العلم: صحيح.»), which the
  // rule now takes whatever stands beside it.
  if (!/<hadith/iu.test(value) && bareGradeSpans(value).length === 0 && !carriesGeneralSpeakerGrade(value)) return false;
  const probe = GRADE_CARRIER + value;
  return dropUnsourcedGrades(probe).text !== probe;
}

// ── BATCH 4 [b18] · THE UNIT THAT CARRIES A MATN WAITS FOR ITS PARENTHESES ────────────────────
// MEASURED at 92d3c7d: on a streamed turn api/ask.js stood the takhrij pass down whole
// (`takhrij:inside_emitted_bytes`), so a streamed answer carried NO parentheses at all — not late,
// not held: none (live: turn 179b2141, 2026-09-22 21:12:43Z, `streamedThisTurn: true`). The owner's
// contract: the parentheses of a streamed answer are the parentheses of the same answer unstreamed,
// letter for letter, and the stream is not held whole for them — the unit carrying a matn is held
// until its parentheses are judged, exactly as a grade unit is held above, and what comes before
// it goes. So a unit the takhrij pass would TARGET — a quoted matn behind its frame, a `<hadith>`
// card, a quotation opening the answer — ends the streamed prefix, and it and everything after it
// reach the reader through the finish, after the pass has run on the whole answer. Nothing here
// rewrites anything; it only declines to send what the pass could still change.
// `holdMatn` is set by the caller only when the pass WILL run (api/ask.js knows: the flag, the
// band, the library). `holdFromStart` is set when the reader asked for the grade or the source:
// the pass then writes a head IN FRONT of the answer ([b27]), and no byte may leave before it.
export function carriesTakhrijTarget(unit) {
  const found = findTargets(String(unit == null ? '' : unit));
  return found.targets.length > 0 || found.duplicates.length > 0;
}

export function createSentenceStream({
  evidence, domain, mode, khilafFromOpinions, opinionCount, truncated, sources,
  holdUnsettled = true, holdRefDrop = true, holdLeadIn = true, holdMatn = false, holdFromStart = false,
} = {}) {
  ...
  // `sources` may be a function, read again at every unit: the pages an answer may lean on can
  // grow while it is written (the rows it has cited so far), and a list captured once would judge
  // a late card against an early list. Every use of `pages()` only ever asks whether a phrase is
  // ON a page, and adding a page can only turn «not on it» into «on it», so a unit this lets go
  // is one the same lock would also keep against any longer list — the seal's included.
  const pages = typeof sources === 'function'
    ? () => { const list = sources(); return Array.isArray(list) ? list : []; }
    : ((list) => () => list)(Array.isArray(sources) ? sources : []);
  ...
  const admit = (unit, settled) => {
    ...
    // BATCH 4 [b18] — the head the takhrij pass writes in front of a grade question's answer.
    if (holdFromStart) {
      streaming = false;
      held += 1;
      matnHolds += 1;
    ...
    // ── THIRD ORDER, STEP 7 · THE GRADE RULE IS ASKED HERE, BEFORE THE UNIT LEAVES ─────────
    //
    // MEASURED in the second order (step6/stream-lock-witness.mjs, at main, a23a737 and 030da4b):
    // «وهذا حديث صحيح.» and «والحديث الصحيح في هذا الباب يدل على التحريم.» passed every test above
    // and went on the wire, because `dropUnsourcedGrades` ran only in the finalizer, after the bytes
    // had left. The finalizer then took the grade, what it shipped was no longer a prefix of what
    // was sent, the writer closed on `early-release:not-a-prefix`, and the rest of the answer was
    // lost with the unsourced grade still on the reader's screen. The owner's decision 1: the lock is
    // completed INSIDE the stream, and the stream is not turned off.
    //
    // It is asked exactly as the lock is asked above, and for the same reason: a unit the rule WOULD
    // change is not sent, and neither is anything after it. Nothing is rewritten here. The held unit
    // and the rest travel to the finalizer, which applies the same rule to them exactly as it does on
    // a turn that never streamed — so the text the reader gets is the text he would have got, and
    // what was sent before it is never refused because of a grade. The rule reads its source in the
    // unit's own block and needs no pages; a unit is a narrower block than its line, so it can only
    // hold MORE than the finalizer would cut, never send what the finalizer would take.
    // The unit is judged on a line of its own under a neutral carrier line: the rule refuses to
    // empty a whole text (it hands back an input it would leave with no Arabic in it), and a unit that
    // IS the grade sentence would otherwise read as untouched. Lines are the rule's blocks, so the
    // carrier changes nothing about how the unit itself is judged.
    if (wouldDropGrade(unit)) {
      streaming = false;
      held += 1;
      gradeHolds += 1;
      return [];
    }
    // BATCH 4 [b18] — a unit the takhrij pass would target waits for its parentheses.
    if (holdMatn && carriesTakhrijTarget(unit)) {
      streaming = false;
      held += 1;
      matnHolds += 1;
      return [];
    }
    // AND THE STRAY CLOSER. The finalizer removes a bare «</source>» (AA-30) from the whole text; a
    // unit carrying one would be refused after it left, so it waits for the finalizer too.
    if (/<\/source>/iu.test(unit)) {
      streaming = false;
      held += 1;
      return [];
    }
