...
// to hold and not for a screen to show. It sends max_chars_per_hit ONLY when a caller
// asks for it (the quotation path, LIB_QUOTE_V1, lib/lib-quote.js); every other caller's
// body is byte-identical to what it always was, and the service answers them at its default.
...
// ── ع-٥٠ · NARROWING A SEARCH TO A NAMED SET OF BOOKS ───────────────────────
//
// WHY IT WAS ADDED, MEASURED. A top-ten phrase search for «إنما الأعمال بالنيات» over the
// whole index returns commentaries, fatwa collections and rebuttals, and NOT ONE of the
// books the takhrij ladder is made of — measured on ezik-shamela-20260820 on 2026-09-19.
// So a caller that must answer «which of THESE books carries this matn» cannot answer it by
// reading ten globally-best rows, and no `limit` this service allows would change that.
//
// IT IS THE SERVICE'S OWN DOOR, NOT A NEW ONE. C:\EZIK-LIB\service\src\contract.mjs:19
// accepts `filters.book_ids`, an array of 1..MAX_BOOK_IDS (100) ids.
//
// AND IT IS ADDITIVE, WHICH IS THE WHOLE OF ITS SAFETY. Absent or empty, the body is the
// byte-identical `{ q, limit }` every existing caller has always sent: no current caller
// passes `bookIds`, so nothing that works today changes shape.
const LIB_MAX_BOOK_IDS = 100;

function withBookIds(body, bookIds) {
  if (!Array.isArray(bookIds) || bookIds.length === 0) return body;
  const ids = [];
  for (const raw of bookIds) {
    const id = String(raw == null ? '' : raw).trim();
    if (id && id.length <= 128 && !ids.includes(id)) ids.push(id);
    if (ids.length >= LIB_MAX_BOOK_IDS) break;
  }
  if (!ids.length) return body;
  return { ...body, filters: { book_ids: ids } };
}

// LIB_QUOTE_V1 -- A QUOTATION ASKS FOR THE WHOLE ATOM. The service cuts every hit at its default
// (1200) unless told otherwise; measured on the twin, 72 of 160 atoms of the quotation key came
// back `truncated`. ADDITIVE exactly as `withBookIds` is: absent, zero or garbage leaves the body
// untouched, and a value above the service's own ceiling is clamped to it rather than refused.
function withMaxChars(body, maxChars) {
  const n = Number(maxChars);
  if (!Number.isInteger(n) || n <= 0) return body;
  return { ...body, max_chars_per_hit: Math.min(n, LIB_MAX_CHARS_PER_HIT_CEILING) };
}
...
export async function searchLibrary(query, options = {}) {
  ...
  try {
    response = await fetchImpl(url, {
      ...
      body: JSON.stringify(withMaxChars(
        withBookIds({ q, limit: clampLimit(options.limit) }, options.bookIds), options.maxCharsPerHit)),
    });
  } catch {
