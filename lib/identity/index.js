...
import {
  containsPropheticOrDivineSubject,
  isBareSacredLabel,
} from '../policy/sacred-attribution.js';
import { isBareAttributionTitle } from '../attribution.js';
...
/** Cache and lookup accept only a human-name candidate, never a sacred or title-only capture. */
export function isHumanIdentityCandidate(nameRaw) {
  const name = String(nameRaw || '').trim();
  return !!identityKey(name)
    && !containsPropheticOrDivineSubject(name)
    && !isBareSacredLabel(name)
    && !isBareAttributionTitle(name);
}
...
export async function identityFor(nameRaw, deps = {}) {
  ...
  // Identity lookup and its cache are for human names. A detector bug, stale caller, or direct
  // unit invocation must not turn «رسول», «النبي», or a bare sacred/title label into a person.
  // This check precedes every injected effect, including a cache read.
  if (!key || !isHumanIdentityCandidate(name)) return unknown;
...
export function identityFactBlock(result, { question = '' } = {}) {
  ...
  // Product-level defence: even a synthetic or stale UNKNOWN verdict cannot turn a prophetic or
  // divine label into an unidentified human. This does not rely on the upstream detector veto.
  if (r.kind === IDENTITY.UNKNOWN && containsPropheticOrDivineSubject(who)) return '';
