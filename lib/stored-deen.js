...
import { ambiguousReligiousHits, isRulingFrame } from './route-classify.js';
...
//
// MEASURED, row 14 of the matrix: a reader asked whether one prayer can be intended for both the
// mosque-greeting and the ʿasr sunnah. The phrase «فكيف أصلي» alone flipped the whole turn to
// LOCAL_WORSHIP and it returned the wudūʾ/salāh template before searching anything; deleting that
// one phrase, and nothing else, gave STORED_FIQH. The template cannot answer him, and it is
// printed with the confidence of an answer.
//
// The frozen subject/manner test upstream (lib/policy/referral-tail.js) already vetoes an
// explicit ruling word. What it cannot see is the OTHER two ways a procedural sentence stops
// being a template request, and both are grammatical families rather than topics:
//
//   نية      the root ن-و-ي — «هل أنوي…؟» asks what to INTEND, which is a fiqh question about
//            the act, not a question about its form. No template has an answer for it.
//   تداخل    one performance offered for two obligations — «تكفي عن», «تجزئ عن», «تغني عن»,
//            «في … واحدة». That is the classic overlap question and it has a ruling, not steps.
//
// The ruling frame is re-tested here too, from the router's own shared predicate, so «هل يجوز…»
// cannot enter through a wording the upstream list happens not to carry.
const INTENT_CLAUSE = /(?:^|\s)(?:انوي|تنوي|ينوي|ننوي|نويت|نيه|بنيه|بنيتين|النيه)(?:\s|$)/u;
const SERVES_TWO = /(?:تكفي|يكفي|تجزي|يجزي|تغني|يغني)\s+عن|(?:في|ب)\s*(?:صلاه|ركعه|ركعتين|نيه)\s+واحده|بنيتين|باثنتين|بسنتين|باسنتين/u;

export function isPureMannerRequest(question) {
  const folded = norm(question);
  if (!folded) return false;
  if (INTENT_CLAUSE.test(folded)) return false;
  if (SERVES_TWO.test(folded)) return false;
  return !isRulingFrame(question);
}
...
function plannedReligiousIndependently(question, plan) {
  if (!plan || !['fatwa', 'tafsir', 'hadith'].includes(plan.purpose)) return false;
...
export function classifyReligiousRuntime(question, plan, lexicalRoute) {
  ...
  const namedOpinion = !!(plan && (plan.attributionMode === 'namedScholarOpinion'
    || plan.claimRelation === 'BY_ENTITY'));
  if (!namedOpinion && isFrozenWorshipQuestion(question) && isPureMannerRequest(question)) return 'LOCAL_WORSHIP';
  if ((plan && plan.purpose === 'hadith')
    || /(?:صحه|تخريج|درجه|شرح|معني)\s+(?:هذا\s+)?(?:ال)?حديث|^\s*حديث(?:\s|$)/u.test(folded)) return 'HADITH';
  const plannedReligious = plannedReligiousIndependently(question, plan);
  ...
  if (!plannedReligious && !authority && !fatwaScholar && identity && worldLookupAllowed(identity)) return 'GENERAL';
  ...
  if (lexicalRoute !== 'DEEN' && !isReligiousText(question) && !plannedReligious) return 'GENERAL';
  return 'STORED_FIQH';
}
