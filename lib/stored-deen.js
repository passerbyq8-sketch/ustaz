...
export async function retrieveStoredFiqhEvidence(options = {}) {
  ...
  // SHAMELA_BRAIN, item 7 path 3. The local corpus answers first; the library is asked
  // only when that corpus did not fill the evidence slots. That is the owner's rule: extra
  // reach when the corpus falls short, never a second opinion competing with it. The
  // threshold is MAX_EVIDENCE_RECORDS, a constant that already governs this path, so no new
  // quorum number is invented here.
  //
  // Eligibility, the flag and the token are all decided by the caller and handed down. This
  // module reads no environment of its own, so a guard can drive it with a fixture and no
  // process at all. searchLibrary refuses before it builds any request unless the flag it
  // was handed is on, so a mistake here cannot light up a network call. It never throws.
  let libRecords = [];
  let libReason = 'lib_not_attempted';
  let libAttempts = 0;
  if (options.libEligible === true && searchQuery && accepted.length < MAX_EVIDENCE_RECORDS) {
    const libSearch = options.libSearch || searchLibrary;
    libAttempts = 1;
    const lib = await libSearch(searchQuery, {
      flagValue: options.libFlagValue,
      token: options.libToken,
      fetchImpl: options.libFetchImpl,
      signal: options.signal,
    });
    libReason = lib.ok ? 'lib_ok' : lib.reason;
    libRecords = lib.ok ? lib.records : [];
  }
  return {
    context,
    searchQuery,
...
export async function runStoredFiqhTurn(options = {}) {
  ...
  const base = {
    ...
    libReason: local.libReason,
    libAttempts: local.libAttempts,
    libRecordCount: (local.libRecords || []).length,
    ...STORED_DEEN_METRICS,
  };
  if (!accepted.length) {
    return {
