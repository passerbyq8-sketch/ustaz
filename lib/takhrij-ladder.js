...
export function composeParenthetical(subjectIds) {
  ...
  if (!rows.length) return { text: NOT_RAISED, rows: [], grade: null, sourced: false, ruled: false };
  ...
  if (hasBukhari && hasMuslim) return { text: AGREED_UPON, rows, grade: null, sourced: true, ruled: true };
  if (hasBukhari) return { text: rows.find((r) => r.n === 1).display, rows, grade: null, sourced: true, ruled: true };
  if (hasMuslim) return { text: rows.find((r) => r.n === 2).display, rows, grade: null, sourced: true, ruled: true };
  ...
  if (outlets.length) {
    ...
    // BELOW THE SHAYKHAYN NOTHING LEAVES BARE. Either the grade a grader stated, or the sentence
    // that says no grader was found — never the collection's name on its own.
    return {
      text: `${outlet.display} · ${grade || NO_RULING}`, rows, grade, sourced: true, ruled: !!grade,
    };
  }
  ...
  if (!speaking.length) return { text: NOT_RAISED, rows, grade: null, sourced: false, ruled: false };
  const outlet = speaking.reduce((a, b) => (a.n <= b.n ? a : b));
  return { text: `${outlet.display} · ${grade}`, rows, grade, sourced: true, ruled: true };
}
