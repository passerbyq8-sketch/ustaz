...
export function composeParenthetical(subjectIds) {
  ...
  if (outlets.length) {
    ...
    return {
      ...
      ruled: !!grade,
      silent: false,
      ruledBy: grade ? rows.filter((r) => r.grade === grade).map((r) => r.display).join('، ') : '',
    };
  }
  ...
  // ── AND HERE IS THE ONE DOOR «(لا يثبت مرفوعا)» COMES THROUGH (§١) ──────
  // No ladder متن book narrates this matn, and a ladder GRADER states a ruling that means it
  // does not stand. That is the owner's first row: «بحثنا فوجدنا حاكما يقول موضوع / لا أصل له».
  // The grader's own name does not go between the parentheses — his RULING is the whole content
  // of them — but it travels in `ruledBy` so the record beside the answer names who ruled, which
  // is what the guard's new row reads.
  if (NOT_ESTABLISHED_GRADES.includes(grade)) {
