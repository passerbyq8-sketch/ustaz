...
const SHAMELA_REQUEST = N([
  ...
  // How people actually phrase "quote me the text", observed in live testing. The
  // earlier list missed "انقل لي نصاً من كتاب ..." because it only had "نص كتاب".
  'انقل لي', 'انقل نص', 'انقل نصا', 'اذكر نص', 'أورد نص', 'هات نص', 'اقتباس',
  'نص من كتاب', 'نصا من كتاب', 'نص من', 'نصا من', 'نص قول', 'نص حديث في كتاب',
]);
 ...
 * @param {{band?:string, depth?:string, userText?:string}} opts
 *        userText: the user's OWN question. The Shamela decision is made on this rather
 *        than on `query`, because wanting a book's wording is a property of what the
 *        person asked, and the model's search query routinely drops it -- it rewrites
 *        "انقل لي نصاً من كتاب صحيح مسلم" into a topical query about the hadith, at which
 *        point nothing is left to recognise. dorar/dr-mutlaq keep reading `query`, whose
 *        subject-matter terms survive rewriting intact.
 * @returns {{domain:string, reason:string}|null}
 */
export function classifySourceIntent(query, opts = {}) {
  const { p, w, empty } = lex(query);
  if (empty) return null;
  // The user's own words, falling back to the model's query when absent.
  const U = lex(opts.userText && String(opts.userText).trim() ? opts.userText : query);
  const shamelaAllowed = opts.band === 'adult';
  const askedForBookText = shamelaAllowed && !U.empty && hit(U.p, U.w, SHAMELA_REQUEST);
  const namedABook = shamelaAllowed && !U.empty && hit(U.p, U.w, SHAMELA_BOOKS);
  ...
  // An EXPLICIT request for a book's wording outranks the weak grading signal below.
  // "انقل لي نصاً من كتاب صحيح مسلم في حديث النية" is a text request that merely mentions
  // a hadith; sending it to hadith-authentication answers a question nobody asked. It does
  // NOT outrank the decisive terms above, so "ما درجة هذا الحديث وما إسناده؟" still goes to
  // dorar even when the asker names the collection it is in.
  if (askedForBookText) return { domain: 'shamela.ws', reason: 'book-text-request' };
  ...
  if (namedABook) return { domain: 'shamela.ws', reason: 'named-book' };
