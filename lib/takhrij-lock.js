 ...
 * @returns {boolean}  true — a statement stands, so only the grading word goes (today's rule)
 *                     false — a credit frame stands alone, so the whole sentence goes
 */
...
export function dropUnsourcedGrades(textRaw, { followedByCard = false } = {}) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    for (const sp of spans) {
      ...
      cuts.push(leavesAStatement(rest) ? { start: sp.start, end: sp.end }
