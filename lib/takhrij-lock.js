  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    if (isAttrib || isGrade) {
      ...
      while (j + 1 < toks.length && j - i < MAX_NAME_WORDS && contiguous(j, j + 1) && !verbAt(j + 1)
        && !BACK_REFERENCES.has(toks[j + 1].bare)) j++;
      if (j > i) {
...
/** Does this sentence also authenticate — a sound grade word, or a verb of authentication? */
function authenticates(body) {
  return tokenize(body).some((t) => {
    const w = withoutConjunction(t.bare);
    return SOUND_GRADE_WORDS.has(w) || AUTHENTICATING_VERBS.has(w);
  });
}
