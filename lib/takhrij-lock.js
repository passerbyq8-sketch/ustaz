...
import { repairOrdinalLabels } from './reader-prose.js';
...
export function dropUnsourcedGrades(textRaw, options = {}) {
  return withBracketAfterQuote(options, () => dropUnsourcedGradesOnce(textRaw, options || {}));
}
function dropUnsourcedGradesOnce(textRaw, { followedByCard = false } = {}) {
  const s = String(textRaw == null ? '' : textRaw);
...
function haystack(sources) {
  return ' ' + (Array.isArray(sources) ? sources : [])
    ...
    .filter((x) => !(x && typeof x === 'object' && x.proseProof))
...
// D3C Q1: a narrative credit can say «رواه جابر في صلاة ...» while the full
// source says «هذه الصفة رواها جابر». The old three-word name reader swallowed
// «في صلاة». This is not a collector proof or a matn match: require the same
// explicitly credited name and three contextual words in that source passage.
function supportedNarrativeCredit(span, body, hay) {
  if (!bracketAfterQuoteInCall || span.kind !== 'attribution' || /[«»]/u.test(body)) return false;
  const match = /^(?:رواه|رواها) (.+?) في (.+)$/u.exec(span.phrase);
  if (!match) return false;
  const name = match[1];
  if (collectorAfter(tokenize('رواه ' + name), 0)) return false;
  const claim = distinctiveWords(norm(body));
  for (const word of name.split(' ')) claim.delete(word);
  for (const verb of ['رواه', 'رواها']) {
    for (const at of occurrences(hay, verb + ' ' + name)) {
      const context = hay.slice(Math.max(0, at - 100), at + SAHIHAYN_WINDOW_CHARS);
      const shared = new Set(context.split(' ').filter(word => claim.has(word)));
      if (shared.size >= SAHIHAYN_SHARED_WORDS) return true;
    }
  }
  return false;
...
const CREDIT_MATN_WINDOW_CHARS = SAHIHAYN_WINDOW_CHARS;
const CREDIT_MATN_ANCHOR_WORDS = 4;
function creditTiedToMatn(sp, s, sen, body, hay) {
  if (sp.kind !== 'attribution') return true;
  const p = norm(sp.phrase);
  const verb = p.split(' ')[0];
  if (!ATTRIB_VERBS.has(verb) && !NARRATION_VERBS.has(verb)) return true;
  if (!containsAsWord(hay, p)) return true; // unsupported anyway: the rules below judge it
  const words = matnOfCredit(s, sen, body, sp.start).split(' ').filter(Boolean);
  if (words.length < 2) return true;
  const anchor = words.slice(0, CREDIT_MATN_ANCHOR_WORDS).join(' ');
  const anchors = [];
  for (let i = hay.indexOf(anchor); i !== -1; i = hay.indexOf(anchor, i + 1)) anchors.push(i);
  for (const at of occurrences(hay, p)) {
    if (anchors.some((a) => Math.abs(a - at) <= CREDIT_MATN_WINDOW_CHARS)) return true;
  }
  return false;
}
...
// م٣-ب (FULL_ANSWER_V1) — with the switch on, the takhrij pass writes a governed matn's bracket right
// after its closing mark, in the middle of the sentence (lib/takhrij.js `parentheticalSlot`). The
// [b25] salvage above looked for the library's bracket only at the sentence's END, so a condemned
// sentence would lose the one thing the library proved about that matn: the [b25] defect, back by
// another road (03-answer/measure/B-bracket.md §4b). With the switch on, a ladder-shaped bracket
// standing right after the quotation is carried too. The switch reaches the salvage through
// `bracketAfterQuoteInCall`, which is set only for the length of one synchronous call of
// `lockTakhrij` or `dropUnsourcedGrades` that asked for it, so the salvage's own signature and its
// call sites (all four pinned by name in the guards) stay as they are.
const PARENTHESES_AFTER_QUOTE_RE = /^\s*\(([^()«»\d]{2,80})\)/u;
let bracketAfterQuoteInCall = false;
/** Runs `run` with the switch set when `options.bracketAfterQuote` asks for it; a nested call inherits. */
export function withBracketAfterQuote(options, run) {
  const before = bracketAfterQuoteInCall;
  bracketAfterQuoteInCall = before || (!!options && options.bracketAfterQuote === true);
  try { return run(); } finally { bracketAfterQuoteInCall = before; }
}
  ...
  while ((quoted = QUOTED_RUN_RE.exec(body)) !== null) {
    ...
    if (framed) {
      ...
      const paren = PARENTHESES_AT_END_RE.exec(body.slice(after))
        || (bracketAfterQuoteInCall ? PARENTHESES_AFTER_QUOTE_RE.exec(body.slice(after)) : null);
...
function matnOfCredit(s, sen, body, at) {
  ...
  // D3C C2d: the authenticity sentence can introduce the quotation immediately after it.
  if (bracketAfterQuoteInCall) {
    const tail = s.slice(sen.end);
    const next = new RegExp(QUOTED_RUN_RE.source, 'u').exec(tail);
    const bridge = next ? tail.slice(0, next.index) : '';
    if (next && bridge.length <= 400 && !takhrijSpans(bridge).length) return foldedWords(next[1]);
  }
  return '';
}

function confirmedAuthenticitySentence(s, sen, body, spans, sources) {
  if (!bracketAfterQuoteInCall || !spans.length) return null;
  const grades = [...foldedWords(body).matchAll(/(?:^| )(صحيح|حسن|ضعيف|موضوع|منكر|جيد|ثابت)(?= |$)/gu)].map(m => m[1]);
  if (!grades.length) return null;
  const matn = matnOfCredit(s, sen, body, spans[0].start);
  if (!matn) return null;
  for (const source of sources || []) {
    const proof = source?.authenticityProof;
    if (!proof || foldedWords(proof.matn) !== matn || !proof.books?.length) continue;
    const grade = foldedWords(proof.grade);
    if (!grades.every(word => grade.split(' ').includes(word) || (word === 'ثابت' && grade === 'صحيح'))) continue;
    // A confirmed collector is proved by its own matn, not by synthesizing the
    // writer's attribution verb in a page. Keep the sentence's exact words.
    const groups = proof.separate ? proof.books.map(book => [book]) : [proof.books];
    if (spans.every(span => matnOfCredit(s, sen, body, span.start) === matn)) {
      for (const books of groups) {
        const named = prunedProseCredit(s, sen, body, spans, books.map(book => ({ key: proofKey(book), matn })));
        if (named && !named.cuts.length) return [];
      }
    }
    if (groups.some(books => {
      const proven = haystack(books.map(book => ({ title: book, passage: book + ' ' + proof.matn })));
      return spans.every(span => span.kind === 'attribution' && supported(span.phrase, proven, body));
    })) return [];
    const creditStart = Math.min(...spans.map(span => span.start));
    const comma = Math.max(body.lastIndexOf('،', creditStart), body.lastIndexOf(',', creditStart));
    const prefix = body.slice(0, comma >= 0 ? comma : creditStart).trim().replace(/[،,؛:]+$/u, '').trim();
    // Owner's Q18 ruling: cut the unsupported credit tail as whole clauses. Never
    // pluck a collector or change a dual inside a kept clause. Only a complete,
    // independently confirmed grade can stand; otherwise the whole sentence goes.
    if (/^(?:نعم )?(?:هذا )?(?:الحديث|حديث) (?:صحيح|حسن|ثابت)(?: (?:صحيح|حسن|ثابت))*$/u.test(foldedWords(prefix))) {
      return [{ start: sen.start, end: sen.end, insert: prefix + '.' + (/\s+$/u.exec(body)?.[0] || ' ') }];
    }
    return [{ start: sen.start, end: sen.end, insert: '' }];
  }
  return null;
}
 ...
 */
function prunedProseCredit(s, sen, body, unsupported, proofs) {
  ...
  for (const sp of unsupported) {
    ...
    const matn = matnOfCredit(s, sen, body, toks[i].start);
...
export function lockTakhrij(text, sources, options = {}) {
  return withBracketAfterQuote(options, () => lockTakhrijOnce(text, sources));
}
function lockTakhrijOnce(text, sources) {
  const s = String(text == null ? '' : text);
  ...
  for (const sen of sentences(s)) {
    ...
    const authenticity = confirmedAuthenticitySentence(s, sen, body, spans, sources);
    if (authenticity) {
      cuts.push(...authenticity);
      if (authenticity.length) prunedCredits += 1;
      continue;
    }
    ...
    const prune = prunedProseCredit(s, sen, body, unsupported, proseProofs);
    if (prune) {
  ...
  out = reconcileAnnouncedCounts(s, out);
  if (bracketAfterQuoteInCall) out = repairOrdinalLabels(out);
  return { text: out, removed, droppedSentences, outcome: 'REBUILT', degraded, repairAttempted: true, salvagedMatns }; // [111-c56]
}
