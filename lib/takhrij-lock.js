...
export function takhrijSpans(text) {
  ...
  for (let i = 0; i < toks.length; i++) {
    ...
    // [111-complete-7] — MEASURED on the preview of 70e12db (round 5 Q6 «مفصّل»: «…، وعند مسلمٍ عن أبي هريرة مرفوعًا: «جُزُّوا
    // الشوارب وأرخوا اللحى، خالفوا المجوس»»; Q4 «مفصّل»: «…، وعند الحاكمِ بلفظِ «سيدُ القومِ في السفرِ خادمُهم»»): no atom
    // carried either and both reached the reader. Neither «مرفوعًا» nor «بلفظ» nor the tanween kept them out — the «و» did:
    // «عند مسلمٍ» alone opens this span, «وعند مسلمٍ» opened none. «وعند» is «عند» with its conjunction, as «ورواه» is «رواه» —
    // read here only where the books are followed by «عن…» or «بلفظ…», the shape [111-complete-7]'s cut below takes, so no
    // other «وعند» sentence meets the whole-sentence rules it never met before.
    const indaWithWaw = b === norm('وعند') && (() => {
      const k7 = collectorAfter(toks, i);
      const next7 = k7 && toks[i + k7 + 1];
      return Boolean(next7 && contiguous(i, i + k7 + 1) && INDA_NEXT_RE.test(next7.bare));
    })();
    if ((b === norm('عند') || b === norm('في') || indaWithWaw) && toks[i + 1] && !(toks[i - 1] && toks[i - 1].bare === norm('عليه'))) {
    ...
    // [111-close-10] — …but not inside a COMMENTARY's title. MEASURED over the roots rounds (C-67-hole, 5 turns; round 3
    // Q7 «موجز»): «…ابن قدامة في المغني، والنووي في شرح صحيح مسلم، وابن تيمية.» reached the reader as «والنووي في شرح،»
    // — the seal read «صحيح مسلم» after «شرح» as a credit no page carried and cut it alone; and round 1 Q11 left «أنّ هذا
    // اللفظ: «…».» the same way. «شرح صحيح مسلم»، «فتح الباري شرح صحيح البخاري»، «شرح النووي على صحيح مسلم» name the
    // commentary, not the matn's source: the book named after «شرح» (or «على» after a «شرح» in the three words before)
    // is read as the title it is, and nothing about a real credit («في صحيح مسلم»، «رواه مسلم») moves.
    const inCommentaryTitle = () => {
      const prev = toks[i - 1] ? withoutConjunction(toks[i - 1].bare) : '';
      if (prev === norm('شرح') || prev === norm('بشرح')) return true;
      if (prev === norm('على') || prev === norm('علي')) {
        for (let q = Math.max(0, i - 4); q < i - 1; q += 1) if (withoutConjunction(toks[q].bare) === norm('شرح')) return true;
      }
      return false;
    };
    if (b === 'صحيح' && toks[i + 1] && SAHIH_BOOKS.has(toks[i + 1].bare) && contiguous(i, i + 1) && inCommentaryTitle()) {
      i += 1; continue;
    }
...
// [111-close-3c] — «…وحديثٍ يُنسبُ للنبيِّ ﷺ بغير سندٍ صحيح.» (battery Q28, `[finalize/drop] grades · chain`): «بغير» negates it.
const GRADE_NEGATORS = new Set(['لم', 'ليس', 'ليست', 'لا', 'غير', 'ولا', 'وليس', 'فليس', 'ولم', 'بغير', 'وبغير', 'بدون', 'بلا']);
...
// [111-close-3b] — and without the modal verb: «فلا يُنسَبُ هذا اللفظُ إليه ﷺ على أنّه حديثٌ ثابتٌ، بل يُذكَرُ حكمةً…» went at
// `[finalize/drop] grades` on this round's preview (round 5 Q4 «موجز»); «لا يُنسَب … على أنّه» is the same ruling.
// [111-close-3c] — MEASURED on the preview of 70e12db (round 5 Q4 «موجز», `[finalize/drop] grades`): «فالأحوط ألّا يُنسب إلى
// النبي ﷺ على أنه حديثٌ ثابتٌ عنه ما لم يُتحقّق من تخريجه…» — the same ruling, its negation written «ألّا» / «أن لا».
// [111-complete-1] — MEASURED on the preview of 27dfa58 (battery Q27 «موجز», `[finalize/drop] grades`): «فلا يصحّ أن يُنسب هذا
// اللفظ إلى النبيّ ﷺ، ولا يجوز أن يُستشهد به على أنّه حديثٌ ثابت.» — «يُستشهد» is the same ruling. Its siblings were
// measured on the corpus and every saved battery: «ولا يجوزُ الاحتجاجُ به على أنّه حديثٌ نبويٌّ ثابت» and «فلا يصحّ الاحتجاجُ
// بلفظه على أنّه…» appeared (and stood only because «نبويّ» pushed the grade out of reach); «يُحتجّ» and «يُستدلّ … على أنّه»
// appeared nowhere, so they are not written here.
const FORBIDDEN_ASCRIPTION_RE = /(?:^| )(?:[وف]?(?:لا|ليس|الا)|لكن لا|فلا|ولا|ان لا) (?:(?:يجوز|يصح|ينبغي|يحل|يسوغ|يجوز لاحد)(?: \S+){0,3} (?:ان )?)?(?:ينسب|تنسب|ينسبه|نسبته|نسبتها|نسبه|نسبه|يعزي|يعزا|يروي|يحدث|يقال|يذكر|يستشهد|الاحتجاج)(?: \S+){0,14} (?:علي |ب)?(?:انه|انها|كونه|كونها)(?: \S+){0,2}$/u;
...
// MEASURED on the preview of 70e12db (battery Q16 «طالب علم», `[finalize/drop] grades`): «القولُ الثالث: أنّه منسوخ، أي أنّ
// الحديثَ صحيحٌ لكنّ حكمَه نُسخ بأدلّةٍ أخرى.» went whole, after «القول الأول: أنه حديث ضعيف…» and «القول الثاني: أنه حديث
// شاذ.» had stayed — the answer's third view gone, and the ruling it carries (the hadith is abrogated) with it.
// THE OWNER'S DECISION (order of 23 Sep, row 2): «فإذا كان في خلاف عليها وفي علماء يقولون انه صحيح وعزك استعمله، هني عزك
// ماجاب شي من عنده ولاخرف». A sentence that states one view of two or more the answer counts is the scholars' grade, not the
// answer's: it stays whole with its grade. The labels are the forms measured on the corpus and the batteries — «القول/
// المذهب/الوجه» with an ordinal or «الآخر» (القول الأول ٤٤ · الثاني ٤٤ · الثالث ٩ · الآخر ١ · المذهب ١+١ · الوجه ١+١) — at the
// head of the sentence. Not a view: a sentence that names «الراجح» or speaks in the answer's own voice («الصحيح»، «نرى»…),
// and a label the answer does not count twice.
const COUNTED_VIEW_LABEL_RE = /^\s*(?:[-•*]\s*)?(?:(?:[0-9]+|[٠-٩]+)\s*[.)-]\s*)?(?:[وف])?(?:ال)?(?:قول|مذهب|وجه)\s+(?:ال)?(أول|اول|ثاني|ثالث|رابع|خامس|آخر|اخر)(?![\p{Script=Arabic}])/u;
const COUNTED_VIEW_OWN_VOICE_RE = /(?:^|[\s،:(])[وف]?(?:ال)?(?:راجح|أرجح|ارجح|صحيح عندي|صواب|مختار|أصح|اصح)(?![\p{Script=Arabic}])|(?:^|[\s،:(])[وف]?(?:نرى|أرى|نرجح|أرجح|نختار|أختار|اختار|رجحه|نرجحه)(?![\p{Script=Arabic}])/u;
const countedViewDiacritics = (v) => String(v).replace(/[\u064B-\u0652\u0670\u0640]/gu, '');
function countedViews(lines) {
  const ordinals = new Set();
  for (const line of lines) {
    for (const sentence of countedViewDiacritics(line).split(/(?<=[.؟!])\s+/u)) {
      const m = sentence.match(COUNTED_VIEW_LABEL_RE);
      if (m) ordinals.add(m[1] === 'اول' ? 'أول' : m[1] === 'اخر' ? 'آخر' : m[1]);
    }
  }
  return ordinals.size;
}
function statesACountedView(block, sp, viewCount) {
  if (viewCount < 2) return false;
  const sen = sentenceAround(block, sp.start);
  const sentence = countedViewDiacritics(block.slice(sen.start, sen.end));
  return COUNTED_VIEW_LABEL_RE.test(sentence) && !COUNTED_VIEW_OWN_VOICE_RE.test(sentence);
}
...
export function dropUnsourcedGrades(textRaw, { followedByCard = false } = {}) {
  ...
  const viewCount = countedViews(arrived); // [111-complete-2]
  let lastNonEmpty = -1;
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // [111-complete-2] — nor one of two or more counted views in a khilaf (see above).
    for (let k = spans.length - 1; k >= 0; k -= 1) if (statesACountedView(block, spans[k], viewCount)) spans.splice(k, 1);
...
function creditTiedToMatn(sp, s, sen, body, hay, asked = '') {
  ...
  // [111-close-2b] — «في الصحيحين/متفق عليه» over a matn that is neither quoted nor asked is tied to nothing an atom
  // proves either; a page naming both Shaykhs near its words proves it no more than it proves «رواه الترمذي» (row 64).
  if (words.length < 2 && !asked && SHAYKHAYN_PHRASES.has(p) && PROPHET_REPORT_RE.test(norm(body))) return false;
...
const DANGLING_HEAD_END = new Set(['من', 'الى', 'الي', 'عن', 'على', 'علي', 'في', 'ب', 'ل', 'ك',
  ...
  // [111-close-5b] — «…من حديث عثمان أيضًا، وقد، لكنّي…» (round 5 Q1 «موجز»): «قد» waits for its verb.
  'قد', 'وقد', 'فقد', 'لقد']);
// [111-close-5b] — a relative pronoun waits for its clause.
// [111-close-5c] — and so does «ما» in «…والدليلُ على النسخ ما رواه النسائيُّ عن أبي سعيدٍ أنّ النبيَّ ﷺ رخّصَ…»: MEASURED on the
// preview of 70e12db (battery Q21) it left «والدليلُ على النسخ ما، والرخصةُ لا تكونُ إلا بعد عزيمة».
const RELATIVE_END = new Set(['الذي', 'التي', 'الذين', 'اللذين', 'اللتين', 'اللاتي', 'اللواتي', 'ما', 'مما', 'فيما', 'بما'].map((w) => norm(w)));
// [111-complete-3] — MEASURED on the preview of 27dfa58 (battery Q27 «موجز», `unsupportedRemoved 2`): «فقد جاء من طريق أنس بن
// مالك رضي الله عنه، وفي إسناده، حتى إنّ بعض المحدّثين حكم عليه بالوضع…» — the cut took both credits and left «وفي إسناده»: a
// fronted predicate with no subject, a frame with nothing in it. The draft before the lock is not logged; the shape is given
// back offline by «…، وفي إسناده رواه ابن عدي، وأخرجه العقيلي، حتى…». The family's rule as it stands (5 · 5b · 5c · 5d): no
// prepositional phrase is left hanging with nothing to hang on. A unit whose kept words are only a preposition and what
// it governs — one word, or a word and the definite word it is annexed to («في إسناد الحديث») — is a frame the cut
// emptied, and it goes from its own comma as 5b's relative clause does; the credits go, and no ruling goes with them.
const BARE_FRAME_PREPOSITIONS = new Set(['في', 'من', 'عن', 'على', 'علي', 'عند', 'الى', 'الي', 'مع', 'لدى', 'لدي', 'بين'].map((w) => norm(w)));
function isBareFrame(words) {
  if (words.length < 2 || words.length > 3) return false;
  const head = words[0].bare.replace(/^[وف](?=\S{2,})/u, '');
  if (!BARE_FRAME_PREPOSITIONS.has(head)) return false;
  return words.length === 2 || /^ال/u.test(words[2].bare);
}
/**
...
function listItemCut(s, sen, body, unsupported) {
  ...
  let kept = body.slice(unit.from, cutFrom);
  let keptWords = tokenize(kept);
  // [111-close-5b] — MEASURED on this round's preview (round 5 Q11 «موجز»): «…وقد قال النبي ﷺ في الحديث الذي رواه
  // البخاري:» lost its credit and left «…في الحديث الذي.» — a relative clause with nothing to relate. The clause that
  // introduced the credit goes with it, back to its own comma, and the item keeps what it said before it.
  if (keptWords.length && (RELATIVE_END.has(keptWords[keptWords.length - 1].bare) || isBareFrame(keptWords))) { // [111-complete-3]
    const commas = topLevelCommas(kept);
    if (commas.length) {
      cutFrom = unit.from + commas[commas.length - 1];
      kept = body.slice(unit.from, cutFrom);
      keptWords = tokenize(kept);
    } else {
      // the unit is that clause alone: it goes from the comma that opened it, and what stood before it stays
      const opener = body.slice(0, unit.from).match(/[،,]\s*$/u);
      if (!opener || !tokenize(body.slice(0, unit.from - opener[0].length)).length) return null;
      return { cuts: [{ start: sen.start + unit.from - opener[0].length, end: sen.start + cutTo, insert: '' }], matn: '' };
    }
  }
...
// ── [111-complete-7] · «عند» AND THE BOOKS AFTER IT, WHEN THE LIBRARY PROVED NONE OF THEM ─────────────────────
// [e55] keeps of a credit the books the library proved for its matn, and «عند/في» stays where the model wrote it; where it
// proved none, [e55] kept nothing and handed the sentence to the rules below. For «عند <books>» those rules take too much:
// measured offline on the two round-5 witnesses once «وعند» was read, the whole line of Q6 went with «أَنْهِكُوا الشَّوَارِبَ
// وَأَعْفُوا اللِّحَى» in it (a matn), and the whole sentence of Q4 with «وكلُّ هذه الطرقِ ضعيفةٌ لا تصحُّ» (the verdict). So
// here, [e55]'s own pruning is carried to its end: every book goes, as it would go one by one, and «عند» — which names
// nothing once they are gone — goes with them. The «و» before it, the narrator («عن أبي هريرة مرفوعًا») or the wording
// («بلفظِ «…»») after it, and the quotation stay exactly as written: «وعند مسلمٍ عن أبي هريرة مرفوعًا:» ⟶ «وعن أبي هريرة
// مرفوعًا:»، «وعند الحاكمِ بلفظِ «…»» ⟶ «وبلفظِ «…»». Its rules are [e55]'s: the closed list of names, no book added or
// swapped, no quotation under three words, and only a sentence whose every unsupported span is such a credit — and it
// applies only where what follows the books is «عن…» or «بلفظ…», so the cut leaves a frame on the matn and never a bare one.
// The matn such a credit speaks of is the quotation it introduces — the first after it, in its sentence or the next
// («وعند مسلمٍ عن أبي هريرة مرفوعًا:⏎«…»»، «وعند الحاكمِ بلفظِ «…»») — and the proof is read against that one.
const INDA_NEXT_RE = /^(?:عن|عنه|عنها|عنهما|عنهم|بلفظ|بلفظه|بلفظها)$/u;
function matnAfterCredit(s, sen, body, at) {
  const after = [...body.matchAll(new RegExp(QUOTED_RUN_RE.source, 'gu'))].find((m) => m.index >= at);
  if (after) return foldedWords(after[1]);
  const next = sentences(s).find((x) => x.start >= sen.end && s.slice(x.start, x.end).trim());
  const there = next ? [...s.slice(next.start, next.end).matchAll(new RegExp(QUOTED_RUN_RE.source, 'gu'))] : [];
  return there.length ? foldedWords(there[0][1]) : '';
}
function indaCreditCut(s, sen, body, unsupported, proofs) {
  if (!unsupported.length || !unsupported.every((sp) => sp.kind === 'attribution')) return null;
  const toks = tokenize(body);
  const cuts = [];
  for (const sp of unsupported) {
    const i = toks.findIndex((t) => t.end > sp.start);
    if (i < 0 || withoutConjunction(toks[i].bare) !== norm('عند')) return null;
    let k = i + 1;
    const names = [];
    while (k < toks.length && /^\s*$/u.test(body.slice(toks[k - 1].end, toks[k].start))) {
      let first = toks[k].bare;
    ...
    if (!names.length || !toks[k] || !/^\s+$/u.test(body.slice(toks[k - 1].end, toks[k].start))) return null;
    if (!INDA_NEXT_RE.test(toks[k].bare)) return null;
    const matn = matnAfterCredit(s, sen, body, toks[k].start);
    if (matn.split(' ').filter(Boolean).length < PROSE_PROOF_MIN_WORDS) return null;
    // [e55]'s own pruning: the books the library proved for this matn stay, in the prose's letters and order.
    const proved = new Set(proofs.filter((p) => sameFoldedMatn(p.matn, matn)).map((p) => p.key));
    const kept = names.filter((n) => proved.has(n.key));
    if (kept.length === names.length) continue;
    const rawOf = (n) => body.slice(toks[n.from].start + (n.conj ? 1 : 0), toks[n.to].end);
    if (kept.length) {
      cuts.push({ start: sen.start + toks[names[0].from].start, end: sen.start + toks[names[names.length - 1].to].end,
        insert: kept.map((n, q) => (q ? 'و' : '') + rawOf(n)).join(' ') });
      continue;
    }
    const from = toks[i].start + (toks[i].bare === norm('عند') ? 0 : 1);
    cuts.push({ start: sen.start + from, end: sen.start + toks[k].start, insert: '' });
  }
  return cuts;
}
...
// [111-close-9b] — MEASURED on this round's preview (battery Q26): «…وأصلُه في صحيح مسلم…، بلفظِ:⏎«الدين النصيحة…» (مسلم)» —
// the seal cut a credit after the matn, and the bracket came back under a lead-in line that still credits مسلم. The
// credit forms now include «في صحيح X»، «عند X»، «الشيخان», and the window reads the sentence before the matn's too.
const WITHHELD_CREDIT_VERB_RE = /(?:^|[\s،,؛(])[وف]?(?:رواه|رواها|رواهما|أخرجه|اخرجه|أخرجها|اخرجها|أخرجاه|خرجه|متفق عليه|في الصحيحين|الشيخان|(?:في\s+صحيح|عند)\s+(?:الإمام\s+)?(?:البخاري|مسلم|البخار|مسلمٍ))(?![\p{Script=Arabic}])/u;
function restoreWithheldBrackets(before, after, sources) {
  ...
  let out = after;
  let count = 0;
  const around = (text, from, to, stripBrackets = false) => {
    const head = text.slice(0, from);
    const stopAt = (t) => Math.max(t.lastIndexOf('.'), t.lastIndexOf('؟'), t.lastIndexOf('!'), t.lastIndexOf('\n'));
    let start = stopAt(head) + 1;
    // [111-close-9b] — and the sentence before it (the matn's lead-in line), past the blank space between them
    start = stopAt(head.slice(0, Math.max(0, start - 1)).replace(/[\s.؟!]+$/u, '')) + 1;
    // the rest of this sentence, and the whole of the next one
    const m = text.slice(to).match(/^[^.؟!\n]*[.؟!\n]?[\s.؟!]*[^.؟!\n]*/u);
    // a bracket is no prose credit — least of all another matn's, now that the window reaches the sentence before
    const w = (text.slice(start, from) + ' ' + (m ? m[0] : '')).replace(/«[^«»]*»/gu, ' ');
    return stripBrackets ? w.replace(/\([^()\n]{2,60}\)/gu, ' ') : w;
  };
  for (const row of rows) {
    ...
    if (WITHHELD_CREDIT_VERB_RE.test(around(out, now.index, now.index + now[0].length, true))) continue; // a credit still stands
...
// ── [111-close-2b] · «في الصحيحين» THAT NOTHING PROVES GOES — THE TWO WORDS, NOT THE SENTENCE ─────────────────────
// MEASURED on this round's preview (battery Q16 «طالب علم»): «…وممّن قال بهذا الشيخُ ابنُ باز…، ومنها ما ثبتَ في الصحيحين من
// أنّ النبيَّ ﷺ كان يصومُ يومَ السبتِ ويومَ الأحد ويقول إنّهما عيدُ المشركين…» — a hadith neither Ṣaḥīḥ has (Aḥmad, al-Nasāʾī
// in al-Kubrā, Ibn Khuzayma), credited to both off a page. The sentence carries a ruling (Ibn Bāz's permission); so where
// every unsupported span of it is «في الصحيحين» standing right after «ثبت/جاء/ورد/صحّ/رُوي» (or «متفق عليه» after its own
// comma), that phrase alone goes — «ومنها ما ثبتَ من أنّ النبيَّ ﷺ…» — and nothing else in the sentence moves.
// …over a report of the Prophet ﷺ in the prose: «أنّ النبيَّ ﷺ كان/قال/نهى/أمر…»، «أنّ رسولَ الله ﷺ…»، «من نهي النبيّ ﷺ عن…».
const PROPHET_REPORT_RE = /(?:^|\s)(?:(?:ان|أن)\s+(?:النبي|رسول\s+الله)(?:\s+صلي\s+الله\s+عليه\s+وسلم|\s+ﷺ)?\s+(?:كان|قال|نهي|امر|فعل|صام|صلي|قضي|جعل|رخص|احتجم|هم)|[وفب]?(?:نهي|امر|قول|فعل|قضاء)\s+(?:النبي|رسول\s+الله))(?:\s|$)/u;
const SHAYKHAYN_AFTER_RE = /(?:^|\s)[وف]?(?:ثبت|ثبتت|جاء|جاءت|ورد|وردت|صح|صحت|روي|رويت)\s*$/u;
function shaykhaynPhraseCut(sen, body, unsupported) {
  if (!unsupported.length || !unsupported.every((sp) => sp.kind === 'attribution' && SHAYKHAYN_PHRASES.has(norm(sp.phrase)))) return null;
  // only the case it was built for: a sentence that quotes nothing and reports the Prophet ﷺ in its own words
  if (new RegExp(QUOTED_RUN_RE.source, 'u').test(body) || !PROPHET_REPORT_RE.test(norm(body))) return null;
  const out = [];
  for (const sp of unsupported) {
    const before = norm(body.slice(0, sp.start));
    if (norm(sp.phrase) === norm('متفق عليه')) {
      const lead = body.slice(0, sp.start).match(/[،,]\s*$/u);
      if (!lead || /\S/u.test(body.slice(sp.end).replace(/^[\s.؟!]*/u, '').slice(0, 1))) return null;
      out.push({ start: sen.start + sp.start - lead[0].length, end: sen.start + sp.end, insert: '' });
      continue;
    }
    if (!SHAYKHAYN_AFTER_RE.test(before)) return null;
    let from = sp.start;
    while (from > 0 && /[ \t]/u.test(body[from - 1])) from -= 1;
    out.push({ start: sen.start + from, end: sen.start + sp.end, insert: '' });
  }
...
export function lockTakhrij(text, sources) {
  ...
  for (const sen of sentences(s)) {
    ...
    // [111-complete-7] — «عند <books>»: the books the library did not prove go, and «عند» with them when none is left.
    const indaCuts = indaCreditCut(s, sen, body, unsupported, proseProofs);
    if (indaCuts) {
      for (const c of indaCuts) { cuts.push(c); keeps.push({ start: c.start, end: c.end }); }
      if (indaCuts.length) {
        prunedCredits += 1;
        for (const sp of unsupported) removed.push({ kind: 'attribution', phrase: sp.phrase });
      }
      continue;
    }
    ...
    // [111-close-2b] — «ثبت/جاء/ورد في الصحيحين» with nothing proving it, and no rule above took it: the two words go.
    const shaykhaynCut = shaykhaynPhraseCut(sen, body, unsupported);
    if (shaykhaynCut) {
      cuts.push(...shaykhaynCut);
      droppedSentences.push({ text: body.trim(), spans: unsupported.map((x) => x.phrase), cut: 'shaykhayn-phrase' });
      for (const sp of unsupported) removed.push({ kind: sp.kind, phrase: sp.phrase });
      continue;
    }
