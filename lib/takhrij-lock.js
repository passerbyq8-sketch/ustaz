...
//   * the attribution or grade is STRIPPED and the matn stands — the narration is not the lie,
//     the credit is; or
//   * the SENTENCE is dropped, when the takhrij was its whole content and stripping it would
//     leave a stub.
// Nothing is repaired, nothing is re-attributed, and no correct attribution is ever supplied from
// this module's own knowledge. Removing a false credit is the only safe operation available.
//
...
export function lockTakhrij(text, sources) {
  const s = String(text == null ? '' : text);
  if (!s.trim()) return { text: s, removed: [], droppedSentences: [] };
  ...
  for (const sen of sentences(s)) {
    ...
    // Would the sentence survive losing them?
    let left = body;
    for (const sp of [...unsupported].sort((a, b) => b.start - a.start)) {
      left = left.slice(0, sp.start) + ' ' + left.slice(sp.end);
    }
    const wordsLeft = norm(left).split(' ').filter(Boolean).length;

    if (wordsLeft < MIN_WORDS_AFTER) {
      // The takhrij was the sentence. Dropping it is the only honest option: what remains would be
      // «والحديث .», which is not a sentence and is not evidence of anything.
  ...
  if (!cuts.length) return { text: s, removed, droppedSentences };
  ...
  return { text: out, removed, droppedSentences };
}
