...
const COUNT_ITEM_MARK_RE = /^\s*(?:[-•*]|[0-9٠-٩]+\s*[.)-]|(?:ال)?(?:أول|ثاني|ثالث|رابع|خامس)\s*:)/u;
...
export function reconcileAnnouncedCounts(beforeText, afterText) {
  ...
  const lines = String(afterText == null ? '' : afterText).split('\n');
  let changed = false;
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (items.length === 1 && count.n === 2) {
      ...
      const noun = (lead ? lead[0] : '') + COUNT_DUAL_NOUNS[words[at].replace(/^[وف]/u, '')] ;
      let k = 0;
      const adjectives = [];
  ...
  return changed ? lines.filter((line) => line !== null).join('\n') : String(afterText == null ? '' : afterText);
}
