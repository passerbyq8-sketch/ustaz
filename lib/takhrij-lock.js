...
function confirmedAuthenticitySentence(s, sen, body, spans, sources) {
  ...
  for (const source of sources || []) {
    ...
    const prefix = body.slice(0, Math.min(...spans.map(span => span.start))).trim().replace(/[،,؛:]+$/u, '').trim();
    // Retain only the independently confirmed authenticity statement. Its unproved credit
    // cannot take that statement with it or turn into a combined citation through this proof.
