...
function confirmedAuthenticitySentence(s, sen, body, spans, sources) {
  if (!bracketAfterQuoteInCall || !spans.length) return false;
  ...
  for (const source of sources || []) {
    ...
    const proven = haystack(proof.books.map(book => ({ title: book, passage: book + ' ' + proof.matn })));
    if (spans.every(span => span.kind === 'attribution' && supported(span.phrase, proven, body))) return true;
  }
  return false;
}
...
function lockTakhrijOnce(text, sources) {
  ...
  for (const sen of sentences(s)) {
    ...
    if (confirmedAuthenticitySentence(s, sen, body, spans, sources)) continue;
