...
//
// MEASURED (program-2026-09-24/04-quote/measure/C-explain.md): the quotation already reaches the
// model byte for byte on the next turn. Two things did not work. «اشرح لي نص كلام النووي في المجموع
// عن …» was CAPTURED by the quote seat and quoted again, with no explanation. And «انقل … ثم لخّصه»
// was answered with the quotation alone: the tail was recognised and thrown away.

// The verbs of explaining, as the first word of the reader's message.
const EXPLAIN_RE = /^(?:[وف])?(?:اشرح|وضح|فسر|لخص|اختصر|بسط)(?:ه|ها|هما|هم)?(?: |$)/u;
/** The reader asks to have the quotation right before explained or summarised, not quoted again. */
export function explainsPreviousQuote(text) {
  return EXPLAIN_RE.test(norm(text));
}

// «… ثم لخّصه / ثم اشرحه / ولخّصه» — what the reader wants done with the quotation after it.
const TAIL_RE = /(?:^| )(?:ثم (?:لخصه|لخصها|لخص|اختصره|اختصرها|اشرحه|اشرحها|اشرح|وضحه|وضحها|فسره|فسرها)|ولخصه|ولخصها|واشرحه|واشرحها|ووضحه)(?: |$)/u;
/** 'summarise' | 'explain' | null. */
export function quoteTail(text) {
  const m = TAIL_RE.exec(norm(text));
  if (!m) return null;
  return /لخص|اختصر/u.test(m[0]) ? 'summarise' : 'explain';
}

/** What the model is asked, after the quotation, on a «ثمّ لخّصه» / «ثمّ اشرحه» turn. */
export const TAIL_PROMPTS = Object.freeze({
  summarise: 'لخّصِ النصَّ المنقولَ أعلاه في نقاطٍ قصيرةٍ بلغةٍ واضحة، ولا تُعِدْ نقلَه، ولا تَزِدْ عليه ما ليس فيه.',
  explain: 'اشرحِ النصَّ المنقولَ أعلاه شرحًا واضحًا، ولا تُعِدْ نقلَه كاملًا، ولا تنسبْ إليه ما ليس فيه.',
});

/** The history the model is given on a tail turn: the question, the quotation as the answer, the tail. */
/** A quotation's text without its cursor marker. */
export function bareQuote(text) {
  return String(text || '').replace(new RegExp(MARKER_RE.source, 'g'), '');
}

export function withQuoteTail(messages, quoteText, tail) {
  const list = Array.isArray(messages) ? messages : [];
  const quote = bareQuote(quoteText);
  return [...list, { role: 'assistant', content: quote }, { role: 'user', content: TAIL_PROMPTS[tail] || TAIL_PROMPTS.summarise }];
}

/** The assistant turn right before the reader's message, as text (markers already stripped or not). */
export function previousAssistantText(messages) {
  const list = Array.isArray(messages) ? messages : [];
  const prev = list.length >= 2 ? list[list.length - 2] : null;
  return prev && prev.role === 'assistant' ? textOf(prev.content) : '';
}

/**
 * A quotation reply's place and words: { volume, page, quoted } from its card line («… · ج1 · ص275»)
 * and its blockquote. null when the text is not a quotation reply.
 */
export function quotedPlace(text) {
  const lines = String(text || '').split('\n');
  if (!/^\u{1F4D6} «/u.test(lines[0] || '')) return null;
  const card = lines[0];
  const page = /(?:^| )ص(\d+)/u.exec(card);
  const volume = /(?:^| )ج(\d+)/u.exec(card);
  const quoted = lines.filter((l) => l.startsWith('>')).map((l) => (l === '>' ? '' : l.slice(2))).join('\n').trim();
  return quoted ? { volume: volume ? Number(volume[1]) : null, page: page ? Number(page[1]) : null, quoted } : null;
}
