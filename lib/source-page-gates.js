...
const RULES = {
  ...
  // ── بنك الكويت المركزي (LIVE_WORLD_V2, 2026-09-19) ─────────────────────────
  //
  // THE RATE TABLES AND NOTHING ELSE. cbk.gov.kw is a whole central bank's site — press
  // releases, circulars, regulations, tenders, annual reports — and exactly three paths on it
  // publish the fact this source was admitted for. The allow-list form is used rather than a
  // blocklist for the reason stated at pathMustMatch's own check: a section the bank adds
  // tomorrow is refused by default instead of admitted by default.
  //
  // MEASURED 2026-09-19 through this repo's own fetchAndClean():
  //   /ar/monetary-policy/market-operations/exchange-rates                 620 clean chars
  //   /ar/monetary-policy/market-operations/exchange-rates/usd             526 clean chars
  //   /ar/monetary-policy/market-operations/gcc-currencies-exchange-rates  366 clean chars
  //
  // THE FLOOR IS 150, NOT 400, AND THAT IS NOT A RELAXATION. The other rules here guard PROSE
  // pages, where four hundred characters is the difference between an article and a navigation
  // strip. This page is a TABLE: its whole content is thirty currency names and thirty numbers,
  // and the shortest of the three admitted pages is 366 characters of pure answer. A 400 floor
  // would refuse a page that is entirely the fact we came for.
  'cbk.gov.kw': {
    pathMustMatch: /^\/(?:ar|en)\/monetary-policy\/market-operations\/(?:exchange-rates(?:\/[a-z]{3})?|gcc-currencies-exchange-rates)\/?$/i,
    minText: 150,
  },
...
export function pathRefusal(url, finalUrl) {
  ...
  // ── THE HOST-INDEPENDENT REFUSALS RUN FIRST, AND FOR EVERY HOST ───────────
  //
  // THE MEASURED DEFECT. A reply cited «Home — موقع د. مطلق الجاسر»: the site ROOT of
  // dr-mutlaq.com, presented as though it were a fatwa. Every gate passed it, and the reason was
  // structural rather than an oversight about one domain — both refusals below used to sit AFTER
  // `if (!found) return null`, so a host with no per-host rules received neither of them. That was
  // most of the allow-list. binbaz.org.sa's own root was admissible by the same hole.
  //
  // NEITHER RULE EVER NEEDED THE PER-HOST TABLE. "The front page is a digest of other pages" and
  // "/category/… is an archive" are facts about how the web is built, not about who is hosting it,
  // and a rule that is true for every host must not be conditional on that host having been
  // enumerated. What stays per-host below is exactly what genuinely varies: which SECTIONS a
  // particular site uses, and which single path shape counts as its article.
  ...
  // is always a digest of other pages, so it is refused for every host on the list.
  ...
  // as though it were an article. dr-mutlaq.com's did exactly that, in production.
  if (p === '/' || p === '') return 'site-root';

  // AND THE GENERIC INDEX SHAPES — /category/…, /tag/…, /archive/…, ?page=N. Same argument: a
  // taxonomy archive is a list of links on every content-management system ever shipped, so the
  // refusal belongs to the shape and not to the domain.
  const gAny = genericListing(target);
  if (gAny) return gAny;
...
// ── WHAT KIND OF PAGE DOES ITS OWN PATH SAY THIS IS? ─────────────────────────
//
// MEASURED. «ما رأي طارق العلي في أحكام العدة؟» was answered over a card from alukah.net titled
// «أحكام العدة للمرأة (خطبة)». A sermon is a fine source for an exhortation, and it is not the
// evidence behind a ruling on waiting periods — but the DOMAIN carrying it is entirely
// legitimate, so the existing per-domain scope filter has nothing to say about the page. The
// lever has to sit on the PAGE, and a site's own URL structure is the one thing about a candidate
// that is knowable before it is fetched.
//
// THREE ANSWERS, AND THE THIRD ONE IS DELIBERATE. `fatwa` and `khutbah` are what a path can say
// out loud; '' is what most paths say, and it must stay a real answer rather than a guess. A page
// at «?p=123» or «/sharia/0/1234/» is not a sermon and is not a ruling — it is unknown, and it
// keeps its ordinary weight. That is a graceful degradation, accepted with its eyes open: the
// alternative is inventing a classification and reordering a real answer out of reach on it.
const FATWA_PATH = /(?:^|\/)(?:fatwa|fatwas|fataawa|fatawa|ftawa|fatawy|answers|iftaa|ifta)(?:\/|$)/i;
const KHUTBAH_PATH = /(?:^|\/)(?:khutbah|khutab|khotab|khutba|dars|doros|droos|dros|lesson|lessons|lecture|lectures|mohadrat|muhadarat|explanations)(?:\/|$)/i;

/**
 * @param {string} url
 * @returns {'fatwa'|'khutbah'|''}
 */
export function pageKindFromPath(url) {
  let p;
  try { p = decodeURIComponent(new URL(String(url)).pathname); }
  catch { return ''; }
  if (!p || p === '/') return '';
  // Fatwa wins a page that somehow says both: the ruling is what the reader is being shown.
  if (FATWA_PATH.test(p)) return 'fatwa';
  if (KHUTBAH_PATH.test(p)) return 'khutbah';
  return '';
}

// ── AND WHAT DOES THE PAGE'S OWN TITLE SAY IT IS? ────────────────────────────
//
// MEASURED, batch 5. The «حادثة العدة» page sits at «/sharia/0/NNNN/» — opaque, so the path lever
// above earns it nothing and it kept its full weight in a ruling question. Its TITLE says
// «(خطبة)» out loud, and that title arrives WITH the search result, before a byte is fetched. The
// one fact that would have queued it behind the rulings was printed on the candidate and nothing
// read it.
//
// SAME THREE ANSWERS, SAME RESTRAINT. A title that says neither is '' and keeps its ordinary
// weight; nothing is ever excluded on this, only ordered.
//
// WORD BOUNDARIES, NOT SUBSTRINGS, and they are load-bearing: «درس» sits inside «مدرسة» and
// «مدروس», and a substring test would file a treatise on the Nizamiyya madrasa as a lesson. The
// class is Arabic letters, so a bracket, a dash or a space ends a word — which is exactly how
// «(خطبة)» is written on the measured page.
const AR = '\\u0621-\\u064A';
const TITLE_FATWA = new RegExp(`(?:^|[^${AR}])(?:ال)?(?:فتوي|فتاوي)(?![${AR}])`, 'u');
const TITLE_KHUTBAH = new RegExp(
  `(?:^|[^${AR}])(?:ال)?(?:خطبه|خطب|محاضره|محاضرات|دروس|درس)(?![${AR}])`, 'u');

/** Fold to the forms the two patterns above are written in. Harakāt, tatwīl, alif and tā' marbūṭa. */
 ...
 * @returns {'fatwa'|'khutbah'|''}
 */
export function pageKindFromTitle(title) {
  const t = foldTitle(title);
  if (!t) return '';
  // Fatwa wins a title that somehow says both, for the same reason the path does.
  if (TITLE_FATWA.test(t)) return 'fatwa';
  if (TITLE_KHUTBAH.test(t)) return 'khutbah';
  return '';
}

/**
 * WHAT KIND OF PAGE IS THIS, reading everything knowable about the candidate before the fetch.
 *
 * THE PATH SPEAKS FIRST, and that is not arbitrary: a URL segment is the SITE's own filing of its
 * own material, chosen by the publisher and stable across every page in the section. A title is
 * one editor's sentence. So the title is consulted exactly where the path says nothing — which is
 * the case the measured incident lives in, and the majority of the corpus besides.
 *
 * @returns {'fatwa'|'khutbah'|''}
 */
export function pageKind(url, title) {
  const byPath = pageKindFromPath(url);
  return byPath || pageKindFromTitle(title);
}

/**
 * THE FLOOR THIS HOST DECLARED FOR ITSELF, or null when it declared none.
 *
 * WHY IT IS `null` AND NOT 400. `ruleFor()` below reports the floor a rule OPERATES under, and
 * defaults it to 400 for a host that has rules but named no number. That is the right answer to
 * "what does this gate enforce" and the wrong answer to "did anybody declare a floor here" —
 * which is what lib/retrieve.js needs to ask before applying its own generic default.
 *
 * MEASURED, and this is the whole reason the distinction matters: mostafaaladwy.com declares
 * `minText: 20` because its fatwas really are that short, /fatwa/49996 extracts to 110 characters
 * of genuine question-and-answer, it cleared this file's gate — and lib/retrieve.js's generic
 * 200-character floor then threw it away. The host's declaration bought nothing, and the site
 * looked like a broken extractor when the extractor was working.
 *
 * @returns {number|null}
 */
export function declaredMinText(hostOrUrl) {
  // Sub-domain aware, and tolerant of a bare host, exactly like hasPageRules() — the two must
  // agree about which rule a URL falls under or the floor consulted is not the floor enforced.
  const found = rulesFor(hostOrUrl);
  const rules = found ? found.rules
    : (() => {
      const h = String(hostOrUrl || '').toLowerCase().replace(/^www\./, '');
      const d = Object.keys(RULES).find((k) => h === k || h.endsWith('.' + k));
      return d ? RULES[d] : null;
    })();
  if (!rules) return null;
  return typeof rules.minText === 'number' ? rules.minText : null;
}
