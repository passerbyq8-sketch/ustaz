...
/**
 * COMBINING MARKS ARE REMOVED, NOT SEPARATED -- and this is the whole of the harakat fix.
 *
 ...
 *
 * U+0640 TATWEEL IS NOT STRIPPED HERE, AND THAT IS MEASURED RATHER THAN ASSUMED. It is category
 * Lm, so \p{L} already accepts it and it has NEVER produced a hyphen: a stretched word slugs to
 * one piece today and did so before this change. Only a mark that SHATTERS a word is removed
 * here; a decorative letter that does not shatter anything is left where the writer put it.
 * (The fatwa normaliser strips tatweel when MATCHING text, which is a different job from minting
 * a URL that has to survive being typed back in.)
 */
const COMBINING_MARKS = /\p{Mn}+/gu;

/**
 * Title -> slug base. Letters and digits of ANY script are kept, combining marks are dropped, and
 * every run of anything else becomes one hyphen. Returns '' when the title held nothing that
 * survives, and the caller falls back to the id -- an article without a typeable title still gets
 * a stable URL.
 */
export function slugify(title) {
  const s = String(title === null || title === undefined ? '' : title)
    .trim().toLowerCase().replace(COMBINING_MARKS, '');
