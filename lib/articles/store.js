...
/**
 * A slug may carry any script's letters and digits and the hyphen, and nothing else -- no ':',
 * no whitespace, no control characters, no '/'. An Arabic title therefore keeps its own letters
 * rather than being flattened to an empty string; the browser percent-encodes them in a URL and
...
export function safeSlug(v) {
  ...
  return /^[\p{L}\p{N}][\p{L}\p{N}-]*$/u.test(s) ? s : null;
}
