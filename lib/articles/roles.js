...
/**
 * THE THREE FACTS A CALLER MAY LEARN ABOUT HIS OWN ACCOUNT, OR null.
 *
 * WHAT THIS IS FOR. Every reason for refusing a writer returns the same 401 with the same two
 * fields, and that is correct for a stranger -- it is what stops the door being used to map who
 * holds what. The price is that the OWNER cannot tell "you have no grant" from "your session is
 * dead" from "your address was never proved" from "the board row is misspelled", and that price
 * was paid in full on 2026-09-07: three rounds and a wasted deployment to narrow one refusal to
 * four candidates.
 *
 * 🔴 IT ANSWERS null FOR ANYBODY WHO IS NOT ALREADY PROVEN TO BE THAT PERSON, AND THAT IS THE
 * WHOLE OF THE DESIGN. No session in the body, a session that is not live, a record with no
 * account key, a key that does not parse -- all of them return null, and the caller of this
 * function then answers exactly what it answered before this function existed, byte for byte. A
 * stranger, a signed-out reader and somebody guessing session strings therefore see no change at
 * all; only the holder of a LIVE session learns anything, and what he learns is three facts about
 * the account that session already names.
 *
 * AND THE ABSENCE OF THE ANSWER IS ITSELF THE FOURTH READING. A holder who is looking at his own
 * session key and gets nothing back has learnt that the key is dead -- which is the one candidate
 * that could not otherwise be told apart, and it is told apart WITHOUT a field that would have
 * had to be returned to people who presented no session at all.
 *
 * WHAT IT DISCLOSES, EXACTLY. `account` says whether the record behind the session could be read
 * at all -- a store fault and an unproved address are different facts and were being confused
 * for each other. `addressProved` is that record's own emailVerified over a non-empty address.
 * `digest` is the sha256 of the caller's OWN stored address, which is what the board row holds
 * and therefore the one value that separates "no row for me" from "a row that does not match me":
 * he compares one hex string with his own board and is done. It discloses nothing he cannot
 * already compute -- Settings shows him the address, and sha256 is public -- and nothing
 * whatsoever about any other account, about who else holds a role, or about how many owners exist.
 *
 * IT NEVER THROWS AND IT NEVER WIDENS ANYTHING. It reads; it grants nothing; and a fault anywhere
 * inside it is null, which is the answer that says the least.
 */
export async function selfFacts(request) {
  try {
    const body = (request && request.body && typeof request.body === 'object') ? request.body : {};
    const session = typeof body.session === 'string' ? body.session : '';
    if (session.length === 0) return null;

    // The SAME liveness rule resolveActor() uses, through the same function, so this cannot come
    // to disagree with the door about what a live session is.
    const record = await touchSession(session);
    if (!record || typeof record.accountKey !== 'string') return null;
    const key = safeAccountKey(record.accountKey);
    if (!key) return null;
    ...
    if (!account || typeof account !== 'object') {
      return { account: 'unreadable', addressProved: false, digest: '' };
    }
    const email = typeof account.email === 'string' ? account.email : '';
    const proved = account.emailVerified === true && email.trim().length > 0;
    // The digest is emitted ONLY on a proved address, because that is the only address the board
    // is ever matched against -- provedDigest() above refuses an unproved one for the same reason
    // and this must not be a second, softer copy of that rule.
    return { account: 'read', addressProved: proved, digest: proved ? emailDigest(email) : '' };
  } catch (e) {
    return null;
  }
}
