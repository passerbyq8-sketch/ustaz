...
// A SUBJECT plus a MANNER word is not enough. It also describes every qualified case in the
// XC-10 family: the sick worshipper, a missed rakʿah, fear prayer, an exception, a condition, or
// an incident followed by “what do I do?”. The local card owns only a whole, general request for
// the canonical procedure. Anchoring the grammar is the important part: any extra state-bearing
// clause makes the question continue to the ordinary answer path.
const DAILY_PRAYER = '(?:الفجر|الظهر|العصر|المغرب|العشاء)';
const GENERAL_WORSHIP_NOUN = '(?:الصلاه|صلاه|الوضوء|وضوء|الغسل|غسل|التيمم|تيمم)';
const GENERAL_WORSHIP_QUALIFIER = '(?:الصحيح|الصحيحه|كامل|كامله|من\\s+الجنابه)';
const GENERAL_ADHKAR_SET = '(?:الاذكار|اذكار|ذكر\\s+الصباح|اذكار\\s+(?:الصباح|المساء|النوم))';
const GENERAL_WORSHIP_DESCRIPTION = new RegExp(
  '^(?:'
    + 'كيف\\s+(?:اصلي(?:\\s+(?:(?:الصلاه|صلاه)(?:\\s+' + DAILY_PRAYER + ')?|' + DAILY_PRAYER + '))?|اتوضا|اغتسل|اتيمم)'
    + '|(?:(?:ما(?:\\s+(?:هي|هو))?|علمني|علميني|اشرح\\s+لي|اعطني|هات|اذكر\\s+لي|اريد)\\s+)?'
      + '(?:صفه|كيفيه|طريقه|خطوات)\\s+' + GENERAL_WORSHIP_NOUN + '(?:\\s+' + GENERAL_WORSHIP_QUALIFIER + ')?'
    + '|(?:ما\\s+هي|علمني|علميني|اعطني|هات|اذكر\\s+لي)\\s+' + GENERAL_ADHKAR_SET + '(?:\\s+(?:كامله|مختصره))?'
    + '|كم(?:\\s+عدد)?\\s+ركعات\\s+(?:صلاه\\s+)?' + DAILY_PRAYER + '\\s+وكيف\\s+(?:اوديها|اصليها)'
  + ')$',
  'u',
);

function worshipDescriptionShape(value) {
  return fold(value || '')
...
export function isFrozenWorshipQuestion(question) {
  ...
  const shape = worshipDescriptionShape(t);
