...
// Contemporary scholars are NOT re-listed here. lib/source-registry.js already owns that roster
// with its whole-word rule and its ambiguity refusal; a second copy is the drift this RFC's
// policy-core guard exists to catch. Their owner id comes from the domain the registry resolves.
const DOMAIN_TO_OWNER = Object.freeze({
  'binbaz.org.sa': 'ibn-baz',
  'binothaimeen.net': 'ibn-uthaymeen',
  'sh-albarrak.com': 'al-barrak',
  'almosleh.com': 'al-mosleh',
  'al-badr.net': 'abdurrazzaq-albadr',
  'othmanalkhamees.com': 'othman-alkhamees',
  'ferkous.com': 'ferkous',
  'saleh.af.org.sa': 'saleh-al-sheikh',
  'ibn-jebreen.com': 'ibn-jebreen',
  'mostafaaladwy.com': 'mostafa-aladwy',
  'almunajjid.com': 'almunajjid',
  'al-abbaad.com': 'al-abbaad',
  'khaledalsabt.com': 'khaled-alsabt',
  'shkhudheir.com': 'al-khudayr',
});
...
export function readEntities(questionRaw) {
  ...
  for (const name of candidateNames) {
    ...
    const ownerId = DOMAIN_TO_OWNER[r.domain];
    if (!ownerId) continue;
  ...
  // The authority is the FIRST entity in an authority role that actually resolved. An ambiguous
  // or unresolved name never becomes an authority id: that is what the clarification is for.
  const auth = entities.find((e) => e.role === 'authority' && e.targetType === 'person'
  ...
  else if (auth) claimRelation = 'BY_ENTITY';
...
export function eraOf(canonicalId) {
  ...
  if (e) return e.era;
  return Object.values(DOMAIN_TO_OWNER).includes(canonicalId) ? 'contemporary' : 'unknown';
}
