...
export const TOPIC_CLASSES = Object.freeze([
  ...
  // danger
  'hazardous_chemistry', 'self_harm', 'weapons_explosives', 'electrical_hazard',
  // the app itself
  'app',
]);
...
// refuses rather than inventing one — the guard inventory fails the build if the table is not total.
//
...
// WHOLE-WORD FORM OF THE SAME TEST, AND THE MEASURED REASON IT EXISTS.
// Substring matching is right for a NOUN, because Arabic glues «ال» and «ب» on the front and the
// noun still means itself: «بالكهرباء» is electricity. It is wrong for a short VERB stem, because
// the same letters turn up inside unrelated words — measured: «لمست» sits inside «المستخدمة», and
// «ألمس» folds to «المس» which sits inside «المسألة». Both made an ordinary question about copper
// wire, and one about following the way of the scholars, into grave hazards.
//
// So a verb is matched as a word, with the proclitics that can precede it spelled out — «بالعبث»
// is still the verb, «المستخدمة» is not.
...
// ELECTRICITY. The hazard a child actually meets in the house, and the one this triage did not
// see: «وش يصير لو لمست السلك الكهربائي؟» came back `general_knowledge` / ALLOW on every route.
//
// Same shape as the rules above — a CONTACT and an ELECTRICAL THING together — so knowing about
// electricity stays allowed and touching it does not. «كيف تعمل الكهرباء؟» and «ما حكم سرقة
// الكهرباء؟» name the thing and no contact, so neither is caught.
//
// Single-token stems, not phrases: Arabic glues «ال» on, and the matcher is substring-on-folded
// text, so «سلك» finds «السلك» while «سلك كهربائي» would miss «السلك الكهربائي» entirely.
// `fold()` maps ئ to ي, so «كهربائ» is written here as it is written by a reader and folds to
// the same stem the text folds to.
const ELECTRIC_THINGS = ['كهرباء', 'كهربائ', 'مقبس', 'قابس', 'فيش', 'سلك', 'اسلاك'];
// The surface forms a reader actually writes, matched as words by `hasWord` above — so «ملموس»
// is not a touch and «المستخدمة» is not a touch either.
const ELECTRIC_CONTACT = [
  'مس', 'لمس', 'المس', 'لمست', 'تلمس', 'يلمس', 'نلمس', 'لمسه', 'يلمسه',
  'امسك', 'مسكت', 'يمسك', 'احط', 'حطيت', 'يحط', 'ادخل', 'دخلت', 'يدخل',
  'اصبع', 'اصبعي', 'اصابع', 'اصابعي',
  'العب', 'يلعب', 'نلعب', 'لعبت', 'عبث', 'اعبث', 'يعبث', 'العبث',
  'اقص', 'اقطع', 'قطعت', 'اشبك', 'اعض', 'عضيت', 'شورت',
];
 ...
 * @param {object} [ir] optional structured IR, so an intent/entity/quoted subject is classified
 *                      by that fact rather than by a surface word
 * @param {string} [effectiveRoute] the route already decided by the request path
 * @returns {string} one of TOPIC_CLASSES
 */
export function classifyTopic(text, ir = null, effectiveRoute = '') {
  const t = fold(text);
  if (!t) return 'general_knowledge';

  // ── WHAT USED TO STAND HERE, AND WHAT ITS REMOVAL COSTS ───────────────────
  //
  // A caller could pass a verdict from open world knowledge, and a confident `non_scholar`
  // re-typed the identity shape «من هو …؟» from `biography` — a RELIGIOUS row in the matrix below
  // — to `general_knowledge`. It was added because asking who a footballer is put the reader on
  // the closed sharia source policy, decided by two words.
  //
  // IT IS GONE BECAUSE ITS ONLY SUPPLIER IS GONE. The verdict came from one model call asking «is
  // this name a scholar?», and that call was removed: only its «no» branch was ever hardened, and
  // the measured failure was a wrong «yes» that nothing doubted («طارق العلي … من أهل العلم»,
  // over a khutbah page that never named him).
  //
  // SO «من هو …؟» IS `biography` AGAIN, FOR EVERYBODY, and that is the conservative direction
  // rather than an oversight. There is no deterministic way to tell an actor from a companion —
  // the roster holds scholars, not every figure of the religion — so the choice was between
  // answering «من هو أبو هريرة؟» from a model's memory and answering «من هو [an actor]؟» from
  // vetted Islamic sources. In an app whose rule is «ناقلٌ لا مفتٍ», the second is the cost worth
  // paying: it is a usability cost, and the first is a correctness one.
  ...
  if (has(t, ELECTRIC_THINGS) && hasWord(t, ELECTRIC_CONTACT)) return 'electrical_hazard';

  // 2. STRUCTURED HADITH INTENT WINS OVER VOCABULARY. Rephrasing or explaining a report does
  //    not turn it into general knowledge merely because the surface request says «أعد الصياغة».
  //    These are signals the request path already produced: its purpose/intents, or a positive
  //    structured prophetic subject. Quote + DEEN alone is intentionally insufficient: a Qur'anic
  //    verse or a scholar's saying can have exactly that surface shape without being a hadith.
  const structuredIntents = Array.isArray(ir && ir.intents) ? ir.intents : [];
  const structuredPropheticSubject = effectiveRoute === 'DEEN'
    && ir && ir.claimSubject && ir.claimSubject.religiousKind === 'prophetic';
  const unboundQuotedSubject = ir && ir.claimSubject
    && ir.claimSubject.source === 'quote' && ir.claimSubject.specific === true;
  if ((ir && ir.purpose === 'hadith')
    || structuredIntents.some((intent) => String(intent || '').startsWith('hadith_'))
    || structuredPropheticSubject) return 'hadith';

  // 3. THE ENTITY IR WINS OVER VOCABULARY. A named authority makes this a scholar-position
  //    question whatever else the sentence contains.
  ...
  if (ir && ir.purpose === 'tafsir') return 'tafsir';
  // An explicit religious quotation whose speaker could not be bound is still a closed-source
  // verification problem. “Not proven Prophetic” must never demote it to general/open evidence.
  if (effectiveRoute === 'DEEN' && unboundQuotedSubject) return 'quote_verification';

  // 4. HEALTH, most specific first. Dosage outranks everything else in health.
  ...
  // Once the planner has identified a concrete quoted subject, lexical overlap is not evidence
  // that its speaker is the Prophet. In particular, «حديثه» can mean a scholar's discourse.
  // True reports already returned above through purpose, intent, or the bound Prophetic subject;
  // keep the old lexical fallback only where no quoted claim is waiting for that binding.
  if (!unboundQuotedSubject && has(t, HADITH_MARKERS)) return 'hadith';
...
export function graveHazard(text) {
  ...
  return (t === 'self_harm' || t === 'weapons_explosives' || t === 'hazardous_chemistry'
    || t === 'electrical_hazard') ? t : '';
}
...
export const CHILD_SAFETY_RUBRIC = Object.freeze({
  ...
  // DETAIL THAT IS SOUND FOR AN ADULT AND NOT FOR A CHILD.
  //
  // This is the case RFC v0.5-R2 §12 names last and it is the easiest to miss: a claim that is
  // correct, verified, well-sourced and religiously appropriate, whose DETAIL is not appropriate
  // to the band being answered. A fatwa page on purity is a legitimate source for a seven-year-old
  // asking how to be clean for prayer, and the paragraph on intercourse on that same page is not.
  //
  // Deliberately NARROW. «الغسل», «الجنابة» and «الاحتلام» are absent: a child asking about ghusl
  // is asking a question the app answers, and blocking the vocabulary of the ruling would be the
  // keyword-blocking this whole policy exists to replace. What is listed is explicit description.
  adultOnlyDetail: Object.freeze([
    'الجماع', 'جماع', 'المجامعه', 'مجامعه', 'ممارسه الجنس', 'الايلاج', 'ايلاج',
    'وصف تفصيلي', 'تفاصيل جنسيه', 'العوره المغلظه',
  ]),
