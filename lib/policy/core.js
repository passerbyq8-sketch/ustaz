 ...
 * @param {object} [opts] `entityWorldType` — what open world knowledge says the named entity is.
 *                      'non_scholar' stops a WORLDLY person's biography being typed as religious.
 * @returns {string} one of TOPIC_CLASSES
 */
export function classifyTopic(text, ir = null, opts = {}) {
  const t = fold(text);
  if (!t) return 'general_knowledge';

  // ── WHO THE PERSON IS OUTRANKS THE PHRASE THAT NAMED HIM ──────────────────
  //
  // `biography` is a RELIGIOUS row in the matrix below, and it was reached by the words «من هو»
  // alone. So «من هو» + a footballer routed a reader into the closed sharia path, and a child
  // asking who a singer is was handed the religious source policy — decided by two words, before
  // anything asked who the man was.
  //
  // A confident `non_scholar` verdict says the subject is a worldly public figure, which makes the
  // question an ordinary one. It NARROWS nothing away from the reader and adds no permission: the
  // hazard triage above still runs first, every religious VOCABULARY test below still runs, and a
  // prophet, a companion or any figure of the religion is classified `scholar` by the world check
  // itself and never reaches this line.
  if (opts.entityWorldType === 'non_scholar' && !has(t, POLEMIC) && !has(t, SHARIA_MARKERS)
    && !has(t, TAFSIR_MARKERS) && !has(t, HADITH_MARKERS)) {
    // Only the identity shape is re-typed. A ruling question that merely mentions him is still a
    // ruling question, and is still answered from the sources.
    if (has(t, BIOGRAPHY)) return 'general_knowledge';
  }
