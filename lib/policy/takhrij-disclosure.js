...
// ── R5 (brain-night, 24 September 2026) · THE HEAD THE LADDER PASS WRITES ──────────────────
// MEASURED on the delivered exam answers H01/H02/H04: line 1 read «تخريج الحديث: ابن حبان، والحكم
// عليه في صحيح الجامع: صحيح.» — a grade lib/takhrij.js proved from the ladder's own grading books —
// and eleven lines down the sentence above told the reader that grade was «لا من كتب التخريج
// نفسها». The sentence was written for the D8 state (a grade copied from a fatwa page, no primary
// corpus); the ladder pass IS the takhrij books, and it was never told so. This is the prefix
// lib/takhrij.js gradingHead() opens every ladder head with; it is defined here, once, and imported
// there, so the composer and this stand-down cannot drift apart.
export const TAKHRIJ_LADDER_HEAD = 'تخريج الحديث: ';
const LADDER_HEAD_LINE_RE = new RegExp('^' + TAKHRIJ_LADDER_HEAD, 'mu');
...
export function takhrijDisclosureOnce(draft, disclosure) {
  ...
  // R5 — the draft carries the ladder's own head: its grade came FROM the takhrij books, and a
  // sentence saying otherwise contradicts the first line the reader read. A line-head only: the
  // same words inside a sentence of the model's are not the head.
  if (LADDER_HEAD_LINE_RE.test(d)) return '';
