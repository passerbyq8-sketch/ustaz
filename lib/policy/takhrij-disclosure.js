...
const GRADE_IN_PROSE_RE = new RegExp('(?:^|[^\\p{L}])(?:[وف]?(?:حديث|الحديث|احاديث|الاحاديث|اسناده|اسنادها|اسناد|الاسناد|سنده|السند|روايه|الروايه|الاثر|اثر)(?:\\s+\\S+){0,2}?\\s+[وف]?' + GRADE_WORD + '|(?:وهو|فهو|هو)\\s+' + GRADE_WORD + '|لا\\s+يصح|لم\\s+يصح|لا\\s+يثبت|لم\\s+يثبت|لا\\s+اصل\\s+له|بالوضع|(?<!(?:الف|الاف|عشر|مايه|سبعين|سبعمايه|اضعاف|ضعف|كل|بكل|له|لها)\\s)[وف]?(?:صححه|حسنه|ضعفه|صححها|حسنها|ضعفها))(?![\\p{L}])', 'u');
// [111-close-12] — C-49b. MEASURED over the roots rounds (round 1 Q1 «طالب علم»): the disclosure «وهذا النقلُ لدرجةِ الحديث…»
// stood under «إنما الأعمال بالنيات» (البخاري) with no grade anywhere in the answer: «ولو كان في ظاهره ألف حسنة» — the NOUN
// «حسنة», folded «حسنه», read as the verb «حسّنه». A verb of grading after a count or a quantity («ألف»، «عشر»، «كل»،
// «له») is the noun, and grades nothing.
...
// MEASURED at eca359e (S3-book-nocomma-silsila, S-W8c-both-lines, and 14 more of the harness texts): the
// sentence «وهذا النقلُ لدرجةِ الحديثِ من المصدرِ المذكور، لا من كتب التخريج نفسِها» was appended under
// «والحكم عليه في السلسلة الضعيفة: موضوع» and «(لا يثبت مرفوعا)» — grades the LIBRARY wrote out of a
// book of takhrij. «Not from the takhrij books themselves» is then false. So where the text carries a
// grade the library wrote — its head («…والحكم عليه في <كتاب>: <حكم>»), «(لا يثبت مرفوعا)», or a
// ladder bracket with a grade («(الترمذي · حسن)») — the disclosure is not written: the grade beside it
// names its own takhrij book. «لم يوقف على حكم» is no grade. Everything else is as it was.
const LIBRARY_GRADE_RE = /والحكم عليه في [^:\n]{2,80}: |\(لا يثبت مرفوعا\)|\([^()\n]{2,60} · (?!لم يوقف على حكم)[^()\n]{2,30}\)/u;
export function takhrijDisclosureOnce(draft, disclosure) {
  ...
  if (LIBRARY_GRADE_RE.test(d)) return ''; // [111-b4b-49]
