...
// ── A FILTER DECIDES WHAT IS KEPT, NOT HOW IT IS SHAPED (AA-85/P5) ───────────
//
// The rejoin at the foot of `screenDraft` used to be `kept.join(' ')`, and a space is not what
// the source put between those sentences. An answer that arrived as three LINES left this
// function as ONE, and every reader downstream then saw a single block: lib/colon-preamble.js:41
// reads the last codepoint of a LINE, so a lead-in that had been the last line was now mid-block
// and no detector could point at it. The seat that closes a dangling lead-in
// (lib/finalize-reader-text.js) was handed one block, and removing the one block there is would
// leave nothing, so it correctly refused. The reader got an answer that ended on a colon.
//
// The cause is not the seat and not the detector. It is that a filter re-shaped what it kept.
// This function still decides EXACTLY what is kept — not one decision below changes — and now
// puts the kept pieces back with the separator the source itself used between them.
//
// WHEN A DROPPED SENTENCE SAT BETWEEN TWO KEPT ONES there is more than one separator to choose
// from. The one carrying the most line breaks wins, so that two sentences the source put on
// different lines are never merged onto one, and two it put on one line never gain a break they
// never had. A paragraph break survives the same way.
const SENTENCE_SPLIT_KEEPING_SEPARATORS = /((?<=[.!؟?])\s+|\n+)/u;
const lineBreaks = (sep) => (String(sep).match(/\n/gu) || []).length;

/**
 * Split into units that remember the separator that FOLLOWED them, so what is kept can be put
 * back as it arrived. `units.filter((u) => u.text.trim()).map((u) => u.text)` is element for
 * element the old `raw.split(SENTENCE_SPLIT).filter((s) => s.trim())`.
 *
 * @returns {Array<{text:string, sep:string}>}
 */
function splitKeepingSeparators(raw) {
  const parts = String(raw).split(SENTENCE_SPLIT_KEEPING_SEPARATORS);
  const units = [];
  for (let i = 0; i < parts.length; i += 2) {
    units.push({ text: parts[i], sep: i + 1 < parts.length ? parts[i + 1] : '' });
  }
  return units;
}

/**
 * Put the kept units back with the source's own separators. `kept` holds indices into `units`.
 */
function rejoinKept(units, kept) {
  let out = '';
  for (let k = 0; k < kept.length; k += 1) {
    if (k > 0) {
      const from = kept[k - 1];
      const to = kept[k];
      let sep = units[from].sep;
      for (let j = from + 1; j < to; j += 1) {
        if (lineBreaks(units[j].sep) > lineBreaks(sep)) sep = units[j].sep;
      }
      out += sep;
    }
    out += units[kept[k]].text;
  }
  return out;
}
...
export function screenDraft(text, ctx = {}) {
  ...
  // The units carry their own separators; `sentences` is the indices of the non-empty ones, in
  // order — the same strings the old split-and-filter produced, and in the same order.
  const units = splitKeepingSeparators(raw);
  const sentences = [];
  for (let i = 0; i < units.length; i += 1) if (units[i].text.trim()) sentences.push(i);
  ...
  const degraded = dropped.length || dropWhole
    ...
    : [];
  return {
    text: dropWhole ? '' : rejoinKept(units, kept),
