...
// ── A NAME IS AS MUCH AN ATTRIBUTION AS A TITLE ──────────────────────────────
// MEASURED GAP: these two patterns matched a speech verb followed by a TITLE and nothing else, so
// «قال العالم بجوازه» was caught at grade C while «قال ابن تيمية بجوازه» — the sentence a model
// actually writes — went straight through. The whole point of the C ceiling is that a summary may
// report a position and may not report speech; a checker that only sees «العالم» enforces that
// against a phrasing nobody uses.
//
// The names come from the roster rather than a list of their own, so a scholar added there cannot
// become quotable here by omission. Aliases are already folded; display forms are folded to match.
const ENTITY_NAMES = Object.freeze([...new Set(
  ROSTER.filter((e) => e.targetType === 'person')
    .flatMap((e) => [...e.aliases, fold(e.display)])
    .filter(Boolean)
    // Longest first: «ابن حجر العسقلاني» must win over «ابن حجر», or the shorter alternative
    // matches and leaves the rest of the name dangling outside the capture.
    .sort((a, b) => b.length - a.length),
)]);
const NAME_ALT = ENTITY_NAMES.map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
const SPOKEN_BY = '(?:الشيخ|العلامه|الامام|الدكتور|الفقيه|المفتي|العالم|شيخ|' + NAME_ALT + ')';

const ATTRIBUTING = new RegExp(
  '(?:قال|يقول|ذكر|افتي|رجح|اختار|يري|راي|ذهب|نص|قرر|صرح|اجاز|منع)\\s*' + SPOKEN_BY,
  'u',
);
// The grade-A shape specifically: his own speech, reported directly.
const DIRECT_SPEECH = new RegExp(
  '(?:قال|يقول|صرح|نص)\\s*' + SPOKEN_BY,
  'u',
);
 ...
 */
export function classifyProvenance({ source, evidenceText, evidenceSpans, policy } = {}) {
  const s = source || {};
  const p = policy || {};
  // ── B IS A PROPERTY OF ONE SPAN, NEVER OF A UNION ─────────────────────────
  //
  // The caller used to hand in the claim's spans JOINED into a single string, and the three
  // things grade B requires were then looked for anywhere in that string. A quotation in one
  // span, the name of a book in a second and «(٢٢/٤١)» in a third produced a citation that
  // nobody wrote: the locator may belong to an entirely different quotation on the same page,
  // and the reader would be shown a wording "confirmed" against a place it never came from.
  //
  // So the spans are kept SEPARATE and each is graded on its own. Being on the same page is not
  // enough, and neither is being in the same answer unit — Gate 1 already requires that much for
  // the claim to exist at all, which makes these two defences independent rather than layered.
  const spans = (Array.isArray(evidenceSpans) && evidenceSpans.length)
    ? evidenceSpans.map((x) => String(x == null ? '' : x))
    : [String(evidenceText || '')];
  ...
  // Each span is asked the question on its own. The FIRST span that carries all three earns B;
  // no span carrying all three means no B, however much the union appears to contain.
  if (era === 'historical') {
    for (const span of spans) {
      const q = detectQuotedRun(span);
      const b = detectBook(span);
      const loc = detectLocator(span);
      if (q && b && loc) return out('B', 'verbatim_with_locator', { quoted: q, book: b, locator: loc });
    }
  }

  // ── C ── AN ELIGIBLE SUMMARY, and ONLY for the historical record.
  //
  // The reported quoted/book/locator here are DESCRIPTIVE only — they say what was seen across
  // the evidence, and the grade has already refused to treat them as a citation.
  if (era === 'historical') {
    return out('C', 'eligible_summary', {
      quoted: detectQuotedRun(text), book: detectBook(text), locator: detectLocator(text),
    });
  }
