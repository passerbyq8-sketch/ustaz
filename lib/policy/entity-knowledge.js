 ...
 * It handed the model `identityAr` — a one-line biography the MODEL had just produced from open
 * world knowledge — and instructed it to open the reply by «تُبيِّنُ هُويّتَه الحقيقيّةَ»: state
 * who this person really is. The intention was kindness, and the effect was measured:
 *
 *   «الشيخ مطلق الجاسر — رحمه الله — إعلامي سعودي محترم»
 *
 * He is alive, he is not a broadcaster, and no page anywhere said either thing. The whole
 * apparatus around this module exists because a model's recollection of a RULING is not evidence;
 * its recollection of a PERSON is not evidence either, and it arrives in the app's own voice with
 * no source card to warn the reader.
 *
 * ── WHAT IT SAYS NOW ─────────────────────────────────────────────────────────
 * One fact, and it is a fact about US rather than about him: this name is not among those we take
 * fatwa from in our sources. That is true by construction — the registries were consulted and he
 * is not in them — and it needs no biography to be said. Then the ruling, from the sources.
 *
 * `identityAr` is deliberately still ACCEPTED and deliberately unused: the world check that
 * produces it is what establishes he is not a mufti, and dropping the parameter would invite a
 * caller to re-plumb it back in. It is not written to the reader, and the second parameter's
 * presence in the signature is a reminder that it must not be.
 */
export function nonScholarDraftingNote(name, _identityArNotForTheReader) {
  return [
    'تنبيهٌ داخليٌّ للصياغة (لا تنقلْه حرفيًّا):',
    'سأل القارئُ عن رأيِ «' + name + '»، وهذا الاسمُ ليس ممّن تُؤخَذ عنهم الفتوى في مصادرنا المعتمدة.',
    '- ابدأْ بجملةٍ واحدةٍ لطيفةٍ محترمةٍ تقول إنّ الفتوى لا تُؤخذ عن هذا الاسم في مصادرنا، بلا سخريةٍ ولا تنقُّصٍ ولا موعظة.',
    '- ممنوعٌ منعًا باتًّا وصفُه بأيِّ شيء: لا مهنةَ ولا جنسيّةَ ولا صفةَ علميّةَ ولا سنَّ ولا بلدَ، '
      + 'ولا «رحمه الله» ولا «المرحوم» ولا أيَّ إشارةٍ إلى وفاتِه أو حياتِه. نحن لا نعرفُ عنه شيئًا من مصدر.',
