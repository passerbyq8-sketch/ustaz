...
//   * It is NOT a crawler. Search and fetch-by-id attempts share the existing per-request page
//     ceiling, retries included. It never walks the library, follows links, or downloads a corpus.
...
import { FATWA_SCHOLARS } from './fatwa-contract.js';
import { compareDurations, durationAcceptable, durationTerms } from './duration.js';
import { MAX_PAGES_FETCHED } from './ledger/budgets.js';
import { RESERVE_FOR_FETCH } from './ledger/direct-corpus.js';
...
// §9: READ FROM THE SCHOLARS REGISTRY, NOT PINNED HERE. These were two module constants, which
// gave the one scholar who happens to have a bespoke adapter a bespoke identity — a shape no
// other scholar could acquire without somebody writing them an adapter too. They are columns in
// lib/fatwa-contract.js now, so giving any other scholar an official publisher name is a data
// edit and nothing more. The exported names are unchanged, so every consumer is untouched.
const IBN_UTHAYMEEN_ROW = FATWA_SCHOLARS.find((entry) => entry.canonicalId === 'ibn-uthaymeen');
export const IBN_UTHAYMEEN_SCHOLAR = IBN_UTHAYMEEN_ROW.formalName;
export const IBN_UTHAYMEEN_PUBLISHER = IBN_UTHAYMEEN_ROW.officialPublisher;
...
export async function retrieveIbnUthaymeen(question, opts) {
  const o = opts || {};
  // The Ledger path supplies its shared request budget. The legacy attributed path supplies no
  // `io`, so it gets a request-local counter derived from that SAME governing ceiling. Capacity
  // is held back for the lesson read exactly as direct-corpus does; every retry passes through
  // allow(), and no counter is shared between concurrent readers.
  let localAttempts = 0;
  const localIo = {
    maxSearches: Math.max(1, MAX_PAGES_FETCHED - RESERVE_FOR_FETCH),
    allow() {
      if (localAttempts >= MAX_PAGES_FETCHED) return false;
      localAttempts++;
      return true;
    },
  };
  const io = o.io || localIo;
  ...
  try {
    ...
    const addTerms = (items) => {
      for (const t of items) if (t && !terms.includes(t)) terms.push(t);
    };
    if (durTerms.length) {
      // Keep the strongest period query first, but do not let several spelling-equivalent period
      // aliases consume the whole search allowance before the first topical phrase and word. The
      // remaining aliases still run inside the same cap if room remains. Questions with no period
      // retain their established phrase-then-word order below.
      addTerms(durTerms.slice(0, 1));
      addTerms(phrases.slice(0, 1));
      addTerms(byLength.slice(0, 1));
      addTerms(durTerms.slice(1));
      addTerms(phrases.slice(1));
      addTerms(byLength.slice(1));
    } else {
      addTerms(phrases);
      addTerms(byLength);
    }
    ...
    let strong = false;
    let done = 0;
    // HOW MANY SEARCHES THIS CALL MAY SPEND. The shipped attributed route uses the request-local
    // cap above; the Ledger caller passes its shared request allowance. Both reserve room for the
    // FETCH phase, because a search phase that consumes the whole ceiling leaves a shortlist
    // nobody can read. MEASURED: with a five-request ceiling and six search terms, all five once
    // went to searches and the lesson GET never started, so the adapter could never succeed
    // inside the ceiling however good its shortlist was.
