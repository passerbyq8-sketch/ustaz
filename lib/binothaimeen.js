...
import { compareDurations } from './duration.js';
...
async function httpJson(url, init, timeoutMs) {
  ...
  for (let attempt = 0; attempt <= RETRIES; attempt++) {
    ...
    try {
      const r = await scheduled(() => fetch(url, {
        ...init,
        signal: ctl.signal,
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'ezik-app/1 (+https://ezik.app; contact via site)',
      ...
      clearTimeout(timer);
      lastErr = e;
      // Transport failure or timeout: one more try, then give up.
...
async function searchOnce(term, mode) {
...
export async function fetchLesson(id) {
  ...
  const hit = cacheGet(key);
  if (hit !== undefined) return hit;
  const json = await httpJson(SHOW_URL(id), { method: 'GET' }, SHOW_TIMEOUT_MS);
...
export function acceptsCandidate(questionTokens, text) {
  ...
  const substantive = Array.from(new Set(questionTokens))
    .some((t) => t.length >= MIN_SUBSTANTIVE && hay.includes(t));
...
export async function retrieveIbnUthaymeen(question, opts) {
  ...
  try {
    ...
    const terms = [];
    for (const t of [...phrases, ...byLength]) if (t && !terms.includes(t)) terms.push(t);

    const seen = new Set();
    const pool = [];
    let strong = false;
    for (const term of terms.slice(0, MAX_SEARCHES)) {
      const rows = await searchOnce(term, 'exact');
      for (const r of rows) {
      ...
      // A title that already matches the question this well will not be improved on by casting a
      // wider net; stop paying the site for searches that cannot change the outcome.
      if (strong) break;
    }
    if (!pool.length) { console.warn('[binothaimeen] no candidates'); return []; }

    pool.sort((a, b) => b.pre - a.pre || a.id.localeCompare(b.id));
    ...
    if (picked === null || picked === '') {
      // An explicit "none of these answers it" is information, and the honest reading of it is
      // that there is no documented source — not that we should fall back on second best.
      console.warn('[binothaimeen] ranker found no matching page among', shortlist.length);
      return [];
    }
    const hit = picked && shortlist.find((c) => c.id === picked);
    if (hit) {
      ordered = [hit, ...shortlist.filter((c) => c !== hit)];
    } else if (shortlist.length > 1 && shortlist[0].pre === shortlist[1].pre) {
      // Nothing chose, and the question does not distinguish the top two by itself. This is also
      // the path a ranker OUTAGE lands on, and it must behave like every other outage in this
      // file: ambiguity is not a match, so refuse rather than guess between two of his pages.
      console.warn('[binothaimeen] ambiguous top candidates, refusing');
      return [];
    }

    for (const c of ordered.slice(0, MAX_FETCHES)) {
      const lesson = await fetchLesson(c.id);
      if (!lesson) continue;
      if (!isTargetedFatwa(lesson.title, lesson.exactText)) {
        console.warn('[binothaimeen] not a targeted fatwa, rejected', c.id);
        continue;
      }
      // THE RANKER IS NOT THE GUARANTEE. Whatever chose this page — a model, an overlap score,
      // or luck — the period it rules on has to be the period the reader asked about. A real
      // fatwa of his on the sixth month is still the wrong answer for a question about eighty
      // days, and this is the check that says so without consulting anyone. Refusing here rather
      // than at verification also means the model is never called for a page we already know is
      // about somebody else's case.
      const dur = compareDurations(question, lesson.title, lesson.exactText);
      if (dur.verdict !== 'compatible') {
        console.warn('[binothaimeen] period mismatch (' + dur.verdict + '), rejected', c.id);
        continue;
      }
      ...
      console.log('[binothaimeen] verified source', { id: c.id, overlap: Number(verdict.score.toFixed(2)) });
      return [{ ...lesson, overlap: verdict.score }];
    }
    console.warn('[binothaimeen] no candidate cleared the gates');
    return [];
  } catch (e) {
