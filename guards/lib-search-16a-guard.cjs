...
(async function main() {
  ...
  // ROUND 28 -- THE VIEW WAS REMOVED, AND WITH IT EVERY ASSERTION WHOSE SUBJECT IT
  // WAS. There is no library block in app.jsx to cut out any more: the sheet, its
  // entry, its state, its seven style keys and its home tile were taken out of
  // app.jsx in this round. THE PIPES WERE NOT TOUCHED -- api/lib-search.js,
  // lib/lib-source-card.js and guards/fixtures-lib-search-16a.json are byte for byte
  // what they were -- so every assertion below whose subject is the server or the
  // card builder is still here, still driven against the real function, and still
  // green. The lifted ones are listed by name and reason in the round report.
  ...
  // == F3. WHAT THE SERVER PUTS ON THE BODY ================================
  // ROUND 28: the view half of this section went with the view. ezLibViewState,
  // ezLibSentence and ezLibCardLine are no longer written in app.jsx, so nothing is
  // cut out of it and evaluated here any more. What remains is what this section
  // always ALSO measured, driven through the real shapeSearchResponse: which sentence
  // the SERVER chooses, and what it refuses to put beside it.
  ...
  // 1. refused -- the sentence is the MODULE's, and the SERVER is what put it there.
  ...
  // 2. degraded -- the shortfall is said out loud, and it too is the module's sentence.
  ...
  // 3. 503 -- no token. The sentence is the SERVER's, and it names no environment
  // variable. RE-BASED onto the body the server sent: the view that used to relay this
  // sentence is gone, the sentence it relayed is not, and that is the half that matters.
  const unavailableText = unavailableBody.error.message;
  check("...and it names no environment variable",
    unavailableText.length > 0 && !/[A-Z][A-Z0-9_]{6,}/.test(unavailableText)
    && !unavailableText.includes("TOKEN"), unavailableText);

  // 4. 502 / a dead network -- and neither of them names a host.
  check("...and neither names a host", !upstreamBody.error.message.includes(LIB_HOST));
  ...
  // == F6. THE ROUTE IS REACHED FROM NOWHERE IN THE CLIENT =================
  // ROUND 28 RE-BASES THIS ONE RATHER THAN LIFTING IT. Three assertions here had the
  // view as their subject -- the single call site, its place inside the sheet, and
  // the tap that renders the sheet -- and they went with it. The fourth said "no code
  // OUTSIDE the library block holds the address or the call". There is no block left,
  // so it now says it of the WHOLE file: the stronger form of the same sentence, and
  // the measurement that PROVES the removal rather than merely permitting it.
  ...
  // == G. THE SHEET, RENDERED -- LIFTED WHOLE IN ROUND 28 ==================
  // This section booted the shipped bundle in a linkedom document, mounted the sheet
  // and drove every state through its own handlers. Its subject was the screen, and
  // the screen is gone: there is no LibrarySheet in app.jsx or in app.js to mount.
  // Its twenty-seven assertions are listed by name in the round report. NOTHING here
  // measured api/lib-search.js or lib/lib-source-card.js that is not still measured
  // above -- sections A to F8 drive the real function and the real card builder.
