...
async function runSuite() {
  ...
  try {
    ...
    // emptiness is a MEASUREMENT: every candidate filling was tried against the deposited fatwa
    // set and every one of them lied in the reader's own reply. The gate freezes it, and it does
    // so by running the probe rather than by reading it, so a filling that answers `true` from
    // any path at all goes red on the spot.
    //
    // ITEM 36 (b) PHASE TWO: 113 -> 114, same rule again -- the gate
    // `prophetascriptionwiring`, guards/prophet-ascription-wiring-guard.cjs. It holds the round
    // that moved the rule of the gate below OUT of the battery and INTO the answer path: the
    // detection now lives in lib/prophet-ascription.js as a VERBATIM copy of the sealed guard's
    // own region, and that new gate compares the two byte for byte and then runs both over every
    // fixture. A copy nobody checks is how the reader gets the old rule back in silence.
    // ITEM 36 (b): 112 -> 113, same rule again -- the gate `prophetascription`,
    // guards/prophet-ascription-guard.cjs. Fifth stop reached deliberately this time: the
    // count here is the only census whose file name says nothing about gates, and it is the
    // one the previous two rounds forgot first.
    ...
    // not from a red gate; adding two at once costs the same five edits, once.
        // ITEM 50-ب (2026-09-20): 114 -> 115, same rule again -- the gate `takhrijcontract`,
    // guards/takhrij-contract-guard.cjs. The guard was on disk and unregistered, so it ran
    ...
    // D3B F1 (2026-09-26): d3bquotes registered 165 -> 166 on local/d3b-20260926; at the merge onto D3C
    // (2026-09-26) renumbered openly 166 -> 169.
    // D3B F2 (2026-09-26): d3bdepends registered 166 -> 167 on local/d3b-20260926; at the merge onto D3C
    // (2026-09-26) renumbered openly 167 -> 170.
    // D3B F3 (2026-09-26): d3breviewer registered 167 -> 168 on local/d3b-20260926; at the merge onto D3C
    // (2026-09-26) renumbered openly 168 -> 171.
    // D3B F4 (2026-09-26): d3bnotfound registered 168 -> 169 on local/d3b-20260926; at the merge onto D3C
    // (2026-09-26) renumbered openly 169 -> 172.
    // D3B F5 (2026-09-26): d3bdisclose registered 169 -> 170 on local/d3b-20260926; at the merge onto D3C
    // (2026-09-26) renumbered openly 170 -> 173.
// D4: six recorded-witness gates registered openly as 174-179.
function exactGateSet(names) { return JSON.stringify(names) === JSON.stringify(EXPECTED_GATES) && names.length === 179; }
    ok('ORIGINAL_GATE_SET_MATCH', exactGateSet(EXPECTED_GATES));
    ok('MUTANT 11 KILLED: deleting namepresence breaks the exact 179-name contract', !exactGateSet(EXPECTED_GATES.filter((name) => name !== 'namepresence')));
    ok('MUTANT 12 KILLED: deleting guardhonesty breaks the exact 179-name contract', !exactGateSet(EXPECTED_GATES.filter((name) => name !== 'guardhonesty')));
