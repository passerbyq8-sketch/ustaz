...
function exactGateSet(names) { return JSON.stringify(names) === JSON.stringify(EXPECTED_GATES) && names.length === 141; }
    ok('ORIGINAL_GATE_SET_MATCH', exactGateSet(EXPECTED_GATES));
    ok('MUTANT 11 KILLED: deleting namepresence breaks the exact 141-name contract', !exactGateSet(EXPECTED_GATES.filter((name) => name !== 'namepresence')));
    ok('MUTANT 12 KILLED: deleting guardhonesty breaks the exact 141-name contract', !exactGateSet(EXPECTED_GATES.filter((name) => name !== 'guardhonesty')));
