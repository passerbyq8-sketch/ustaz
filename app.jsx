...
// ============================================================
const EZ_LANG_KEY = 'ezik_ui_lang_v1';
// THE LANGUAGES THIS INTERFACE OFFERS. A list, not a pair of hardcoded buttons: a third
...
const EZ_LANG_DIR = EZ_LANGUAGES.reduce((m, l) => { m[l.code] = l.dir; return m; }, {});

function ezLangValid(v) { return EZ_LANGS.indexOf(v) !== -1; }

// The stored choice. Anything that is not exactly 'ar' or 'en' -- a truncated write, a value
// from a future build, an object, a quota failure -- is not a choice and is ignored.
function ezLangStored() {
  try {
    const v = localStorage.getItem(EZ_LANG_KEY);
    return ezLangValid(v) ? v : null;
  } catch (e) { return null; }
}
...
function ezLangResolve() { return ezLangStored() || EZ_LANG_FALLBACK; }

let EZ_LANG = ezLangResolve();
const EZ_LANG_SUBS = new Set();

function ezLangApply(v) {
  try {
...
// The <head> boot script normally settles <html> before the first paint. This repeats it once at
...
function ezLangSet(v) {
  ...
  ezLangApply(v);
  ezLangRelabel();
  EZ_LANG_SUBS.forEach((f) => { try { f(v); } catch (e) {} });
}
...
function ezLangRelabel() {
  try {
    ...
    EZIK_FAV_ADD = ezT("favorites.add");
    EZIK_FAV_DEL = ezT("favorites.remove");
    EZIK_FAV_REMOVE = ezT("common.remove");
    EZIK_FAV_OPEN_CHAT = ezT("favorites.openChat");
    EZIK_FAV_CHAT_GONE = ezT("favorites.chatGone");
    EZIK_BACK = ezT("common.backArrow");
    EZIK_SEARCH_PH = ezT("chat.searchPlaceholder");
    EZIK_SEARCH_ARIA = ezT("chat.searchAria");
    EZIK_SEARCH_RESULTS = ezT("chat.searchResults");
    EZIK_SEARCH_NONE = ezT("common.noMatches");
    EZIK_FAV_SEARCH_PH = ezT("favorites.searchPlaceholder");
    EZIK_FAV_SEARCH_ARIA = ezT("favorites.searchAria");
    EZIK_CHATS_EMPTY = ezT("chat.noConversations");
    EZIK_QUOTE_ARIA = ezT("chat.quoteAria");
    EZIK_SHARE_LABEL = ezT("chat.share");
    EZIK_SHARE_ARIA = ezT("chat.shareAria");
    EZIK_SHARE_COPIED = ezT("common.copied");
    EZIK_SHARE_FAIL = ezT("common.copyFailed");
    for (let i = 0; i < EZIK_QUICK_ACTIONS.length; i++) {
      EZIK_QUICK_ACTIONS[i] = { ...EZIK_QUICK_ACTIONS[i], label: ezT('chat.qa.' + EZIK_QUICK_ACTIONS[i].key) };
    }
    EZIK_A11Y_TITLE = ezT("a11y.title");
    EZIK_A11Y_FS_LABEL = ezT("a11y.fontSize");
    EZIK_A11Y_FS_NORMAL = ezT("a11y.fontNormal");
    EZIK_A11Y_FS_LARGE = ezT("a11y.fontLarge");
    EZIK_A11Y_FS_XLARGE = ezT("a11y.fontXLarge");
    EZIK_A11Y_READ = ezT("a11y.reading");
    EZIK_A11Y_READ_HINT = ezT("a11y.readingHint");
    EZIK_A11Y_MOTION = ezT("a11y.motion");
    EZIK_A11Y_MOTION_HINT = ezT("a11y.motionHint");
    EZIK_A11Y_RESET = ezT("a11y.reset");
...
// Subscribing in App() is what makes the switch instant: App owns the whole tree, so one
// state change there redraws every screen without a reload and without a route change.
function useEzLang() {
  const [v, setV] = useState(EZ_LANG);
  useEffect(() => {
    const f = (n) => setV(n);
    EZ_LANG_SUBS.add(f);
    if (EZ_LANG !== v) setV(EZ_LANG);
    return () => { EZ_LANG_SUBS.delete(f); };
  }, []);
  return v;
}
...
const EZ_I18N = {
  ar: {
    ...
    'navigation.account': '\u{0627}\u{0644}\u{062D}\u{0633}\u{0627}\u{0628}',
    'language.label': 'اللغة',
    'language.change': 'تغيير لغة الواجهة',
    'language.current': 'اللغة الحالية: {lang}',
    ...
    'settings.enter.newlineHint': 'Enter سطر جديد، والإرسال بالزر. ولا يُرسل أثناء تركيب الحروف.',
    'settings.watermark': 'وضوح العلامة المائية',
    'settings.watermark.none': 'بلا',
    'settings.watermark.light': 'خفيفة',
    'settings.watermark.medium': 'متوسطة',
    'settings.watermark.bold': 'قوية',
    'settings.watermark.fine': 'ضبط دقيق',
    'settings.watermark.quiet': 'إخفاؤها أثناء ورود الرد',
    'settings.watermark.quietHint': 'تختفي العلامة ما دام الرد يُكتب، ثم تعود كما كانت.',
    'chat.newConversation': '\u0645\u062d\u0627\u062f\u062b\u0629 \u062c\u062f\u064a\u062f\u0629',
    'chat.send': 'إرسال',
    'chat.stop': 'إيقاف',
    'chat.placeholder': 'اكتب سؤالك…',
    // XI-02: NEUTRAL BY MEASUREMENT. This hint is a CLIENT-SIDE TIMER, not a server signal —
    ...
    'chat.searchingSources': 'يُحضِّرُ الجوابَ…',
    'chat.quickActions': 'إجراءات سريعة على آخر رد',
    ...
    'module.treasure': '\u{0631}\u{062D}\u{0644}\u{0629}\u{064F} \u{0627}\u{0644}\u{0643}\u{0646}\u{0648}\u{0632}',
    'module.fatwa': 'فتاوى',
    ...
    'fatwa.scholarLabel': 'اسم الشيخ',
    'fatwa.defaultScholar': 'ابن باز',
    'fatwa.allScholars': 'كل الفتاوى',
    'fatwa.browseLoading': 'جارٍ فتح قائمة الفتاوى…',
    'fatwa.browseSummary': 'الفتاوى مرتبة أبجديًّا — {total} فتوى، الصفحة {page} من {pages}.',
    'fatwa.browseEmpty': 'لا توجد فتاوى في هذه الصفحة.',
    'fatwa.browseAria': 'قائمة الفتاوى مرتبة أبجديًّا',
    'fatwa.openFatwa': 'افتح الفتوى كاملة',
    'fatwa.closeFatwa': 'أغلق الفتوى',
    'fatwa.backToBrowse': 'العودة إلى تصفح الفتاوى',
    'fatwa.searchPlaceholder': 'ابحث في الفتاوى',
    'fatwa.searchAria': 'ابحث في نصوص الفتاوى الرسمية',
    'fatwa.searchButton': 'بحث',
    'fatwa.loading': 'جارٍ البحث في النصوص الرسمية…',
    'fatwa.error': 'تعذّر الاتصال بخادم الفتاوى. حاول مرة أخرى.',
    'fatwa.none': 'لم نجد نتيجة في النسخة الرسمية الحالية.',
    'fatwa.summary': 'وجدنا {total} نتيجة — الصفحة {page} من {pages}.',
    'fatwa.question': 'السؤال',
    'fatwa.answer': 'الجواب',
    'fatwa.officialText': 'النص المنشور في موقع الشيخ',
    'fatwa.audioAvailable': 'يتوفر صوت رسمي',
    'fatwa.textOnly': 'نص رسمي بلا صوت',
    'fatwa.audioAria': 'التسجيل الرسمي للفتوى',
    'fatwa.transcriptTag': 'تفريغ آلي من مقطع رسمي',
    'fatwa.transcriptNotice': 'هذا تفريغٌ آليٌّ من الصوت أو الفيديو الرسميِّ لقناة الشيخ، وليس نصًّا مكتوبًا راجعه الشيخ. والأصلُ هو المقطعُ المرئيُّ.',
    'fatwa.transcriptBody': 'نص التفريغ',
    'fatwa.watchVideo': 'فتح المقطع الرسمي',
    'fatwa.source': 'فتح المصدر الرسمي',
    'fatwa.previous': 'السابق',
    'fatwa.next': 'التالي',
    'fatwa.page': '{page} / {pages}',
    'fatwa.action.simplify': 'بسّط',
    'fatwa.action.example': 'مثال',
    'fatwa.action.explain': 'اشرح لي',
    'fatwa.action.quiz': 'اختبرني',
    'fatwa.action.disabled': 'يتاح بعد تفعيل الشرح المحكوم بالنص الأصلي.',
    'favorites.title': 'المفضلة',
    'favorites.heading': 'الردود المفضلة',
    'favorites.add': 'أضف إلى المفضلة',
    'favorites.remove': 'إزالة من المفضلة',
    'favorites.addFatwa': 'احفظ الفتوى',
    'favorites.addAyah': 'احفظ الآية',
    'favorites.saved': 'محفوظة',
    'favorites.kindAria': 'نوع المحفوظات',
    'favorites.kind.all': 'الكل',
    'favorites.kind.reply': 'الردود',
    'favorites.kind.fatwa': 'الفتاوى',
    'favorites.kind.ayah': 'الآيات',
    'common.remove': 'إزالة',
    'favorites.openChat': 'افتح المحادثة الأصلية',
    'favorites.chatGone': 'المحادثة الأصلية محذوفة',
    'chat.searchPlaceholder': 'ابحث في محادثاتك',
    'chat.searchAria': 'ابحث في محادثاتك المحفوظة على هذا الجهاز',
    'chat.searchResults': 'نتائج البحث',
    'common.noMatches': 'لا توجد نتائج مطابقة',
    'favorites.searchPlaceholder': 'ابحث في المفضلة',
    'favorites.searchAria': 'ابحث في الردود المفضلة',
    'chat.quoteAria': 'اقتباس الرد في خانة الكتابة',
    'chat.foldShow': 'عرض بقية الرد',
    'chat.foldHide': 'إخفاء التفاصيل',
    'chat.qa.simplify': 'بسّط',
    'chat.qa.example': 'مثال',
    'chat.qa.quiz': 'اختبرني',
    'chat.qa.shorten': 'اختصر',
    ...
    'pin.changed': '\u062A\u0645 \u062A\u063A\u064A\u064A\u0631 \u0627\u0644\u0631\u0645\u0632.',
    'settings.themeLight': '\u0641\u0627\u062a\u062d',
    'settings.themeDark': '\u062f\u0627\u0643\u0646',
    'pin.groupTitle': '\u062A\u063A\u064A\u064A\u0631 \u0631\u0645\u0632 \u0627\u0644\u0641\u062A\u062D',
    'pin.new': '\u0627\u0644\u0631\u0645\u0632 \u0627\u0644\u062C\u062F\u064A\u062F',
    'pin.confirm': '\u062A\u0623\u0643\u064A\u062F \u0627\u0644\u0631\u0645\u0632',
    'a11y.title': 'سهولة الاستخدام',
    'a11y.fontSize': 'حجم الخط',
    'a11y.fontNormal': 'عادي',
    'a11y.fontLarge': 'كبير',
    'a11y.fontXLarge': 'كبير جدًّا',
    'a11y.reading': 'وضع القراءة',
    'a11y.readingHint': 'تباعد أوسع بين السطور وراحة أكبر في النصوص الطويلة.',
    'a11y.motion': 'تقليل الحركة',
    'a11y.motionHint': 'إيقاف الحركات الزخرفية والتمرير المتحرّك.',
    'a11y.reset': 'إعادة الإعدادات الافتراضية',
    'visualTheme.title': '\u{0627}\u{0644}\u{0647}\u{0648}\u{064A}\u{0629} \u{0627}\u{0644}\u{0628}\u{0635}\u{0631}\u{064A}\u{0629}',
    'visualTheme.active': '\u{0627}\u{0644}\u{062A}\u{0635}\u{0645}\u{064A}\u{0645} \u{0627}\u{0644}\u{062D}\u{0627}\u{0644}\u{064A}',
    'visualTheme.soon': '\u{0642}\u{0631}\u{064A}\u{0628}\u{0627}\u{064B}',
    'chat.attach': '\u0625\u0631\u0641\u0627\u0642 \u0645\u0644\u0641 \u0623\u0648 \u0635\u0648\u0631\u0629',
    'chat.dictate': '\u0625\u0645\u0644\u0627\u0621 \u0635\u0648\u062a\u064a',
    'chat.call': '\u0645\u0643\u0627\u0644\u0645\u0629 \u0635\u0648\u062a\u064a\u0629 \u0645\u0628\u0627\u0634\u0631\u0629',
    'chat.attachImage': '\u0635\u0648\u0631\u0629',
    'chat.attachFile': '\u0645\u0644\u0641',
    'chat.depthHint': '\u0627\u0636\u063a\u0637 \u0644\u062a\u063a\u064a\u064a\u0631 \u0627\u0644\u0639\u0645\u0642 \u0627\u0644\u0645\u0639\u0631\u0641\u064a',
    'chat.depthBrief': '\u0645\u0648\u062c\u0632',
    'chat.depthDetailed': '\u0645\u0641\u0635\u0651\u0644',
    'chat.depthScholar': '\u0637\u0627\u0644\u0628 \u0627\u0644\u0639\u0644\u0645',
    'chat.standingNotice': 'عزك ذكاءٌ اصطناعيّ وقد يُخطئ — راجِعْ ما يهمُّك مع والديك أو مع أهل العلم.',
    'chat.standingNoticeAdult': 'عزك ذكاءٌ اصطناعيّ وقد يُخطئ — راجِعْ ما يهمُّك مع أهل العلم.',
    'navigation.settings2': '\u0627\u0644\u0625\u0639\u062f\u0627\u062f\u0627\u062a',
    'home.hadithOfDay': 'حديث اليوم',
    'home.verseOfDay': 'آية اليوم',
    'home.hadithOfDay2': '\u062D\u062F\u064A\u062B \u0627\u0644\u064A\u0648\u0645',
    'home.verseOfDay2': '\u0622\u064A\u0629 \u0627\u0644\u064A\u0648\u0645',
    'home.greet.dhikr': 'اذكرِ الله',
    'home.greet.morning.1': 'صباحك طاعة — هل قرأتَ أذكار الصباح؟',
    'home.greet.morning.2': 'بكّر بذكر الله، أذكار الصباح بانتظارك.',
    'home.greet.morning.3': 'افتتحْ يومك بأذكار الصباح.',
    'home.greet.morning.4': 'صباح الخير، هل بدأتَ بأذكار الصباح؟',
    'home.greet.morning.5': 'استقبلِ الصباح بذكر الله.',
    'home.greet.morning.6': 'أذكار الصباح تُعينك على يومك.',
    'home.greet.morning.7': 'طابَ صباحك — أذكار الصباح بانتظارك.',
    'home.greet.morning.8': 'ابدأ نهارك بحصنٍ من الأذكار.',
    'home.greet.evening.1': 'مساؤك ذكرٌ — هل قرأتَ أذكار المساء؟',
    'home.greet.evening.2': 'اختم نهارك بأذكار المساء.',
    'home.greet.evening.3': 'قبل أن يهدأ اليوم، أذكار المساء.',
    'home.greet.evening.4': 'مساء الخير، هل قرأتَ أذكار المساء؟',
    'home.greet.evening.5': 'استقبلِ المساء بذكر الله.',
  ...
  en: {
    ...
    'language.change': 'Change the interface language',
    ...
    'settings.enter.newlineHint': 'Enter makes a new line, the button sends. It never sends while a word is being composed.',
    'settings.watermark': 'Watermark strength',
    'settings.watermark.none': 'None',
    'settings.watermark.light': 'Light',
    'settings.watermark.medium': 'Medium',
    'settings.watermark.bold': 'Bold',
    'settings.watermark.fine': 'Fine adjustment',
    'settings.watermark.quiet': 'Hide it while a reply arrives',
    'settings.watermark.quietHint': 'The mark goes while the reply is being written, then comes back as it was.',
    'chat.newConversation': 'New conversation',
    'chat.send': 'Send',
    'chat.stop': 'Stop',
    'chat.placeholder': 'Type your question…',
    'chat.searchingSources': 'Preparing the answer…',
    'chat.quickActions': 'Quick actions on the latest reply',
    'chat.removeImage': 'Remove the image',
    ...
    'module.treasure': 'Treasure journey',
    'module.fatwa': 'Fatwas',
    ...
    'fatwa.scholarLabel': 'Scholar',
    'fatwa.defaultScholar': 'Ibn Baz',
    'fatwa.allScholars': 'All fatwas',
    'fatwa.browseLoading': 'Opening the list of fatwas…',
    'fatwa.browseSummary': 'Fatwas in alphabetical order — {total} of them, page {page} of {pages}.',
    'fatwa.browseEmpty': 'There are no fatwas on this page.',
    'fatwa.browseAria': 'The list of fatwas in alphabetical order',
    'fatwa.openFatwa': 'Open the whole fatwa',
    'fatwa.closeFatwa': 'Close the fatwa',
    'fatwa.backToBrowse': 'Back to browsing the fatwas',
    'fatwa.searchPlaceholder': 'Search fatwas',
    'fatwa.searchAria': 'Search the official fatwa texts',
    'fatwa.searchButton': 'Search',
    'fatwa.loading': 'Searching the official texts…',
    'fatwa.error': 'Could not reach the fatwa service. Please try again.',
    'fatwa.none': 'No result was found in the current official snapshot.',
    'fatwa.summary': 'Found {total} results — page {page} of {pages}.',
    'fatwa.question': 'Question',
    'fatwa.answer': 'Answer',
    'fatwa.officialText': 'Text published on the scholar’s website',
    'fatwa.audioAvailable': 'Official audio available',
    'fatwa.textOnly': 'Official text without audio',
    'fatwa.audioAria': 'Official fatwa recording',
    'fatwa.transcriptTag': 'Automatic transcript of an official clip',
    'fatwa.transcriptNotice': 'This is an automatic transcript of the official audio or video on the scholar’s channel. It is not text the scholar wrote or reviewed. The clip itself is the original.',
    'fatwa.transcriptBody': 'Transcript',
    'fatwa.watchVideo': 'Open the official clip',
    'fatwa.source': 'Open the official source',
    'fatwa.previous': 'Previous',
    'fatwa.next': 'Next',
    'fatwa.page': '{page} / {pages}',
    'fatwa.action.simplify': 'Simplify',
    'fatwa.action.example': 'Example',
    'fatwa.action.explain': 'Explain',
    'fatwa.action.quiz': 'Quiz me',
    'fatwa.action.disabled': 'Available after the source-bound explanation layer is enabled.',
    'favorites.title': 'Favourites',
    'favorites.heading': 'Saved replies',
    'favorites.add': 'Add to favourites',
    'favorites.remove': 'Remove from favourites',
    'favorites.addFatwa': 'Save this fatwa',
    'favorites.addAyah': 'Save this verse',
    'favorites.saved': 'Saved',
    'favorites.kindAria': 'Kind of saved item',
    'favorites.kind.all': 'All',
    'favorites.kind.reply': 'Replies',
    'favorites.kind.fatwa': 'Fatwas',
    'favorites.kind.ayah': 'Verses',
    'common.remove': 'Remove',
    'favorites.openChat': 'Open the original conversation',
    'favorites.chatGone': 'The original conversation was deleted',
    'chat.searchPlaceholder': 'Search your conversations',
    'chat.searchAria': 'Search the conversations saved on this device',
    'chat.searchResults': 'Search results',
    'common.noMatches': 'No matching results',
    'favorites.searchPlaceholder': 'Search the favourites',
    'favorites.searchAria': 'Search the saved replies',
    'chat.quoteAria': 'Quote the reply in the composer',
    'chat.foldShow': 'Show the rest of the reply',
    'chat.foldHide': 'Hide the details',
    'chat.qa.simplify': 'Simplify',
    'chat.qa.example': 'Example',
    'chat.qa.quiz': 'Quiz me',
    ...
    'pin.changed': 'The code was changed.',
    'settings.themeLight': 'Light',
    'settings.themeDark': 'Dark',
    'pin.groupTitle': 'Change the unlock code',
    'pin.new': 'The new code',
    'pin.confirm': 'Confirm the code',
    'a11y.title': 'Ease of use',
    'a11y.fontSize': 'Text size',
    'a11y.fontNormal': 'Normal',
    'a11y.fontLarge': 'Large',
    'a11y.fontXLarge': 'Very large',
    'a11y.reading': 'Reading mode',
    'a11y.readingHint': 'Wider line spacing, and an easier read on long passages.',
    'a11y.motion': 'Reduce motion',
    'a11y.motionHint': 'Turns off decorative animation and animated scrolling.',
    'a11y.reset': 'Restore the default settings',
    'visualTheme.title': 'Visual identity',
    'visualTheme.active': 'The current design',
    'visualTheme.soon': 'Coming soon',
    'chat.attach': 'Attach a file or an image',
    'chat.dictate': 'Voice dictation',
    'chat.call': 'Live voice call',
    'chat.attachImage': 'Image',
    'chat.attachFile': 'File',
    'chat.depthHint': 'Press to change the depth of the answer',
    'chat.depthBrief': 'Brief',
    'chat.depthDetailed': 'Detailed',
    'chat.depthScholar': 'Student of knowledge',
    'chat.standingNotice': 'Ezik is an artificial intelligence and can be wrong — check what matters to you with your parents or with people of knowledge.',
    'chat.standingNoticeAdult': 'Ezik is an artificial intelligence and can be wrong — check what matters to you with people of knowledge.',
    'navigation.settings2': 'Settings',
    'home.hadithOfDay': 'Hadith of the day',
    'home.verseOfDay': 'Verse of the day',
    'home.hadithOfDay2': 'Hadith of the day',
    'home.verseOfDay2': 'Verse of the day',
    'home.greet.dhikr': 'Remember God',
    'home.greet.morning.1': 'A morning of worship — have you read the morning adhkar?',
    'home.greet.morning.2': 'Start early with the remembrance of God; the morning adhkar are waiting.',
    'home.greet.morning.3': 'Open your day with the morning adhkar.',
    'home.greet.morning.4': 'Good morning — have you begun with the morning adhkar?',
    'home.greet.morning.5': 'Meet the morning with the remembrance of God.',
    'home.greet.morning.6': 'The morning adhkar will carry you through your day.',
    'home.greet.morning.7': 'A good morning to you — the morning adhkar are waiting.',
    'home.greet.morning.8': 'Begin your day with a fortress of adhkar.',
    'home.greet.evening.1': 'An evening of remembrance — have you read the evening adhkar?',
    'home.greet.evening.2': 'Close your day with the evening adhkar.',
    'home.greet.evening.3': 'Before the day settles, the evening adhkar.',
    'home.greet.evening.4': 'Good evening — have you read the evening adhkar?',
    'home.greet.evening.5': 'Meet the evening with the remembrance of God.',
...
function ezT(key, vars) {
  const k = String(key);
  const active = EZ_I18N[EZ_LANG];
  let out = (active && Object.prototype.hasOwnProperty.call(active, k)) ? active[k] : null;
  if (out == null) {
    const base = EZ_I18N[EZ_LANG_FALLBACK];
    out = (base && Object.prototype.hasOwnProperty.call(base, k)) ? base[k] : null;
  }
  if (typeof out !== 'string') return '';
  if (!vars) return out;
  return out.replace(/\{([A-Za-z0-9_]+)\}/g, (m, name) =>
    (Object.prototype.hasOwnProperty.call(vars, name) && vars[name] != null) ? String(vars[name]) : m);
}

// The globe. One bounded glyph, no text inside it, and it is decorative -- the button carries
// the accessible name.
const EZ_LANG_GLOBE = (
...
function EzLangControl({ variant }) {
  const lang = useEzLang();
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      if (e.key === 'Escape') {
        setOpen(false);
        try { if (btnRef.current) btnRef.current.focus(); } catch (err) {}
      }
    };
    const onDown = (e) => {
      const w = wrapRef.current;
      if (w && e.target && typeof w.contains === 'function' && !w.contains(e.target)) setOpen(false);
    };
    try {
  ...
  const pick = (code) => { ezLangSet(code); setOpen(false); };
  const here = ezLangEntry(lang);
  const onb = variant === 'onboarding';
  return (
    <div className={onb ? 'ezlang-wrap is-onb' : 'ezlang-wrap'} ref={wrapRef}>
      <button
        type="button"
        ref={btnRef}
        className={onb ? 'ezlang-btn is-onb' : 'ezlang-btn is-row'}
        data-ez-lang-toggle="1"
        aria-haspopup="listbox"
        aria-expanded={open ? 'true' : 'false'}
        aria-label={ezT('language.change')}
        onClick={() => setOpen((o) => !o)}
      >
        {onb ? EZ_LANG_GLOBE : null}
        <span>{here.nativeName}</span>
        {onb ? null : <span className="ezlang-caret" aria-hidden="true">{'\u25BE'}</span>}
      </button>
      {open && (
        <div className="ezlang-menu" role="listbox" aria-label={ezT('language.label')}>
          {EZ_LANGUAGES.map((l) => (
            <button
              key={l.code}
              type="button"
...
const LEGACY_PIN_HASH_KEY = 'parent_pin_hash';
const readLegacyParentHash = () => {
  try { const v = localStorage.getItem(LEGACY_PIN_HASH_KEY); return /^[0-9a-f]{64}$/.test(v || '') ? v : null; }
  catch (e) { return null; }
};
const clearLegacyParentHash = () => { try { localStorage.removeItem(LEGACY_PIN_HASH_KEY); } catch (e) {} };
// نداءٌ واحدٌ للخادم، ومعرّفُ الجهازِ يُضافُ هنا لا في كلِّ موضعِ نداء.
const parentCodeCall = (payload) => fetch('/api/parent-code', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  credentials: 'same-origin',
  body: JSON.stringify(Object.assign({}, payload, { deviceId: getDeviceId() })),
});

// يبقى لقفلِ الإنفاقِ وحدَه (SPEND_GATE_SHA256) — قرارُ مالكٍ منفصلٌ ومعطَّلٌ بمفتاحِه الموثَّق.
...
const SPEND_GATE_KEY = 'spend_gate_unlock';
const SPEND_GATE_DISABLED = SPEND_GATE_SHA256 === '0'.repeat(64);
function spendGateOpen() {
  if (SPEND_GATE_DISABLED) return true;                          // القفل مُعطَّل ⇐ مفتوح دائماً (فشلٌ آمن للفتح)
...
const EZIK_WM_KEY = 'ezik_watermark_opacity_v1';
const EZIK_WM_DEFAULT = 0.5;
// light / medium / bold, as the brief names them. The slider is the same number, finer.
const EZIK_WM_PRESETS = [0.2, 0.5, 0.8];
const EZIK_WM_MIN = 0.05;
// ZERO IS A CHOICE, and it was not one. `0` fell into the `n < EZIK_WM_MIN` arm with every
...
const ezikWmClamp = (v) => {
  ...
  if (!Number.isFinite(n)) return EZIK_WM_DEFAULT;
  if (n === 0) return 0;
  if (n < EZIK_WM_MIN || n > 1) return EZIK_WM_DEFAULT;
  return Math.round(n * 100) / 100;
};
const readWatermarkOpacity = () => {
  try { const raw = localStorage.getItem(EZIK_WM_KEY); return raw === null ? EZIK_WM_DEFAULT : ezikWmClamp(raw); }
  catch (e) { return EZIK_WM_DEFAULT; }
};
// LIVE, and live is the whole point: the property is on <html>, so a chat mounted behind the
...
const applyWatermarkOpacity = (v) => {
  const n = ezikWmClamp(v);
  try { document.documentElement.style.setProperty('--ezwm-o', String(n)); } catch (e) {}
  // «إزالةٌ تامّة» has to mean removed, not invisible. An element at opacity 0 is still in the
  ...
  try { document.documentElement.setAttribute('data-ezwm', n === 0 ? 'off' : 'on'); } catch (e) {}
  try { localStorage.setItem(EZIK_WM_KEY, String(n)); } catch (e) {}
  return n;
};
(function () {
  try {
    if (localStorage.getItem(EZIK_WM_KEY) === null) return;      // never chosen -> the stylesheet answers
    const n0 = readWatermarkOpacity();
    document.documentElement.style.setProperty('--ezwm-o', String(n0));
    document.documentElement.setAttribute('data-ezwm', n0 === 0 ? 'off' : 'on');
  } catch (e) {}
})();

// WHAT `Enter` DOES, AND IT IS THE READER'S TO SET. Same shape as the watermark above: one
...
const EZIK_ENTER_KEY = 'ezik_enter_sends_v1';
const EZIK_ENTER_AUTO = 'auto';
const EZIK_ENTER_EVENT = 'ezik:enter-pref';
const readEnterPref = () => {
  try {
    const raw = localStorage.getItem(EZIK_ENTER_KEY);
    return (raw === 'send' || raw === 'newline') ? raw : EZIK_ENTER_AUTO;
  } catch (e) { return EZIK_ENTER_AUTO; }
};
const writeEnterPref = (v) => {
  const w = (v === 'send' || v === 'newline') ? v : EZIK_ENTER_AUTO;
  try {
    if (w === EZIK_ENTER_AUTO) localStorage.removeItem(EZIK_ENTER_KEY);
    else localStorage.setItem(EZIK_ENTER_KEY, w);
  } catch (e) {}
  // ANNOUNCED, because the chat is NOT re-mounted when الإعدادات closes. That was measured on
  ...
  try { window.dispatchEvent(new CustomEvent(EZIK_ENTER_EVENT, { detail: w })); } catch (e) {}
  return w;
};
// THE ONE RESOLVER, and every reader of this preference goes through it -- the handler, the
...
const ezikComposerIsTouch = () => {
  try { return !!(window.matchMedia && window.matchMedia('(pointer: coarse)').matches); }
  catch (e) { return false; }
};
const ezikEnterSends = (pref, isTouch) => {
  if (pref === 'send') return true;
  if (pref === 'newline') return false;
  return !isTouch;
};

// THE MARK STEPS BACK WHILE A REPLY IS ARRIVING -- AND ONLY IF ASKED. This is an OPTION and
...
const EZIK_WM_HIDE_KEY = 'ezik_watermark_autohide_v1';
const EZIK_WM_HIDE_EVENT = 'ezik:watermark-autohide';
const readWatermarkAutoHide = () => {
  try { return localStorage.getItem(EZIK_WM_HIDE_KEY) === 'on'; } catch (e) { return false; }
};
const writeWatermarkAutoHide = (on) => {
  try {
    if (on) localStorage.setItem(EZIK_WM_HIDE_KEY, 'on');
    else localStorage.removeItem(EZIK_WM_HIDE_KEY);
  } catch (e) {}
  // Announced for the reason item 65 measured: الإعدادات does not unmount the chat, so a
  // switch that only wrote the key would not change anything until a reload.
  try { window.dispatchEvent(new CustomEvent(EZIK_WM_HIDE_EVENT, { detail: !!on })); } catch (e) {}
  return !!on;
};

const THEME_KEY = 'murabbi_theme_v1';
const readStoredTheme = () => { try { const t = localStorage.getItem(THEME_KEY); return (t === 'dark' || t === 'light') ? t : 'light'; } catch (e) { return 'light'; } };
const applyTheme = (t) => {
  const v = (t === 'dark') ? 'dark' : 'light';                     // anything unrecognised falls to light
  try { document.documentElement.setAttribute('data-theme', v); } catch (e) {}
  // S96: the boot script paints <html> INLINE to kill the cold-start white flash, and an inline
  ...
  try { document.documentElement.style.background = (v === 'dark') ? '#0E1116' : '#FFFFFF'; } catch (e) {}
  // ITEM 95: the light value is #FFFFFF on both of the two lines above, and on the boot
  ...
  try { const m = document.querySelector('meta[name="theme-color"]'); if (m) m.setAttribute('content', v === 'dark' ? '#0E1116' : '#1D4ED8'); } catch (e) {}
...
const SURAH_NUMBERS = {
  ...
  // أسماء بديلة شائعة (قد يستخدمها النموذج)
  'براءة': 9, 'بني إسرائيل': 17, 'الملائكة': 35, 'المؤمن': 40,
  'حم السجدة': 41, 'القتال': 47, 'الدهر': 76, 'الانشراح': 94,
  'الزلزال': 99, 'اللهب': 111, 'تبت': 111, 'التوحيد': 112,
};

// تطبيع عربي خفيف: إزالة التشكيل + توحيد الألف بأنواعها + إزالة التطويل
...
// tashkeel/tajweed.
const normalizeForRecite = (str) => (str || '')
  .replace(/[ً-ٰٟۖ-ۭـ]/g, '')
  .replace(/[آأإٱ]/g, 'ا')
  .replace(/ة/g, 'ه')
  .replace(/ى/g, 'ي')
  .replace(/\s+/g, ' ')
  .trim();
// Tokenize on whitespace AFTER normalizing — standalone pause-mark glyphs normalize to '' and
// are dropped by filter(Boolean), so they never count as words.
const tokenizeForRecite = (str) => normalizeForRecite(str).split(/\s+/).filter(Boolean);
  ...
  while (changed && guard++ < 12) {
    ...
    for (let w = 1; w <= 4; w++) {
      ...
      while (i < words.length) {
        let reps = 1;
        while (i + (reps + 1) * w <= words.length) {
          let same = true;
          for (let j = 0; j < w; j++) {
            if (words[i + j] !== words[i + reps * w + j]) { same = false; break; }
          }
          if (!same) break;
          reps++;
        }
        if (reps >= 3) {
          for (let r = 0; r < 2; r++)
            for (let j = 0; j < w; j++) out.push(words[i + j]);
          i += reps * w;
          changed = true;
        } else {
          out.push(words[i]);
...
// Lenient per-word similarity: a heard word this close to an expected word counts as correct.
// False red in the Quran is worse than a missed slip (Musaed), so the bar is forgiving.
const RECITE_LENIENT_THRESHOLD = 0.8;

// Normalized Levenshtein ratio (1 - dist/maxLen) on two already-normalized tokens. Cheap
...
const wordSim = (a, b) => {
  ...
  return maxLen === 0 ? 1 : 1 - prev[n] / maxLen;
};

// Word-level alignment of the child's heard tokens against the expected ayah tokens, via LCS
...
const alignRecite = (expected, heard) => {
  const m = expected.length, n = heard.length;
  if (m === 0) return [];
  const close = (e, h) => wordSim(e, h) >= RECITE_LENIENT_THRESHOLD;
  // dp[i][j] = LCS length of expected[i..], heard[j..]
  const dp = Array.from({ length: m + 1 }, () => new Int32Array(n + 1));
  for (let i = m - 1; i >= 0; i--) {
    for (let j = n - 1; j >= 0; j--) {
      dp[i][j] = close(expected[i], heard[j])
        ? dp[i + 1][j + 1] + 1
  ...
  let i = 0, j = 0;
  while (i < m && j < n) {
    if (close(expected[i], heard[j])) { matched[i] = true; i++; j++; }
  ...
  for (let k = 0; k < m; k++) states[k] = matched[k] ? 'matched' : (k < furthest ? 'mismatch' : 'pending');
  return states;
};
...
for (const _k in SURAH_NUMBERS) { SURAH_NUMBERS_NORM[normalizeArabic(_k)] = SURAH_NUMBERS[_k]; }

// خريطة عكسية رقم→اسم (أول اسم لكل رقم = الاسم الأساسي) — لعنوان بطاقة الآية
// عندما يرسل النموذج surah_num فقط بلا اسم سورة.
const SURAH_NAMES = {};
for (const _name in SURAH_NUMBERS) { const _n = SURAH_NUMBERS[_name]; if (!SURAH_NAMES[_n]) SURAH_NAMES[_n] = _name; }
...
const loadQuran = () => {
  if (__quranData) return Promise.resolve(__quranData);
  if (!__quranPromise) {
    // Promise.resolve().then(...) يضمن أنّ أيّ خطأ (حتى غياب fetch) يصبح رفضاً
    // لا رميةً متزامنة — فلا ينكسر استدعاء loadQuran().catch(...) أبداً.
    __quranPromise = Promise.resolve()
      .then(() => fetch('/quran-uthmani.json'))
      .then((r) => { if (!r.ok) throw new Error('quran fetch ' + r.status); return r.json(); })
      .then((d) => { __quranData = d; return d; })
      .catch((e) => { __quranPromise = null; throw e; }); // اسمح بإعادة المحاولة لاحقاً
  }
  return __quranPromise;
};
// نصّ الآية الكنسي أو null (لا يقع أبداً على نصّ النموذج)
const getVerseText = (sNum, aNum) => {
  if (!__quranData || !(sNum >= 1) || !(aNum >= 1)) return null;
  return __quranData[`${sNum}:${aNum}`] || null;
};
// ============================================================
...
let __adhkarPromise = null;
const loadAdhkar = () => {
  if (__adhkarData) return Promise.resolve(__adhkarData);
  if (!__adhkarPromise) {
    __adhkarPromise = Promise.resolve()
      .then(() => fetch('/adhkar.json'))
      .then((r) => { if (!r.ok) throw new Error('adhkar fetch ' + r.status); return r.json(); })
      .then((raw) => {
        const byId = {}, byCat = {};
        (raw.adhkar || []).forEach((d) => {
          byId[d.id] = d;
          (byCat[d.categoryId] = byCat[d.categoryId] || []).push(d);
        });
        __adhkarData = { byId, byCat, categories: raw.categories || [] };
        return __adhkarData;
      })
...
let __worshipPromise = null;
const loadWorship = () => {
  if (__worshipData) return Promise.resolve(__worshipData);
  if (!__worshipPromise) {
    __worshipPromise = Promise.resolve()
      .then(() => fetch('/worship-display.json'))
      .then((r) => { if (!r.ok) throw new Error('worship fetch ' + r.status); return r.json(); })
      .then((raw) => {
        __worshipData = raw;
        return __worshipData;
      })
      .catch((e) => { __worshipPromise = null; throw e; });
  }
  return __worshipPromise;
};

const resolveWorshipTags = async (text, band) => {
  if (!text || text.indexOf('<worship') === -1) return text;
  let data = null;
  try { data = await loadWorship(); } catch (e) { data = null; }
  const arm = (band === 'adult' || band === 'teen') ? 'adult' : 'young';
  return text.replace(/<worship([^>]*)>[\s\S]*?<\/worship>/g, (_w, attrs) => {
    const m = attrs.match(/id=["']([^"']+)["']/);
    const cell = m && data && data.cells && data.cells[m[1] + ':' + arm];
    const canonical = cell && typeof cell.text === 'string' && cell.text.trim()
      ? cell.text
      : ezT('errors.generic');
    return ' ' + canonical + ' ';
  });
};
// كل أذكار بابٍ بعددِ التكرار والصوت، أو [] (لا يقع أبداً على نصّ النموذج)
const getDhikrByCategory = (catId) => {
  if (!__adhkarData) return [];
  return __adhkarData.byCat[parseInt(catId, 10)] || [];
};
// عدد آيات سورة من المصحف المُحمَّل (عدّ المفاتيح ذات البادئة `${num}:`)؛ 0 إن لم يُحمَّل.
const getSurahAyahCount = (sNum) => {
  if (!__quranData || !(sNum >= 1)) return 0;
  const prefix = `${sNum}:`;
  let c = 0;
  for (const k in __quranData) if (k.startsWith(prefix)) c++;
  return c;
};
...
const resolveSurahNumber = (surahName, surahNum) => {
  ...
  const raw = String(surahName).trim();
  if (SURAH_NUMBERS[raw]) return SURAH_NUMBERS[raw];
  // إزالة بادئة "سورة" إن وُجدت ثم البحث في الخريطة المطبَّعة
  const norm = normalizeArabic(raw).replace(/^سوره?\s+/, '').replace(/^سورة\s+/, '').trim();
  return SURAH_NUMBERS_NORM[norm] || null;
};
...
let __layoutData = null;
let __layoutPromise = null;
const loadLayout = () => {
  if (__layoutData) return Promise.resolve(__layoutData);
  if (!__layoutPromise) {
    __layoutPromise = Promise.resolve()
      .then(() => fetch('/mushaf-layout.json'))
      .then((r) => { if (!r.ok) throw new Error('layout fetch ' + r.status); return r.json(); })
      .then((d) => { __layoutData = d; return d; })
      .catch((e) => { __layoutPromise = null; throw e; }); // اسمح بإعادة المحاولة
  }
  return __layoutPromise;
};

// "حرفٌ عربيّ" — لا علامة. هذه القاعدةُ بعينها هي التي بُرهن بها الرباط في
...
const __isArabicLetter = (cp) =>
  (cp >= 0x0621 && cp <= 0x063A) ||   // الهمزة .. الغين
  (cp >= 0x0641 && cp <= 0x064A) ||   // الفاء .. الياء (التطويل 0x0640 مستبعَد)
  (cp >= 0x0671 && cp <= 0x06D3);     // ألف الوصل والحروف الممتدّة
const __hasLetter = (t) => { for (const ch of t) if (__isArabicLetter(ch.codePointAt(0))) return true; return false; };

// الوصلات الأربع: خانةٌ واحدة في المصحف تبتلع كلمتين («بَعْدَ مَا» و«إِلْ يَاسِينَ»).
// ليست عطلاً — المصدر يقولها بنفسه. الرقم = رقم الخانة التي تبتلع التي بعدها.
const __LIGATURES = { '2:181': 3, '8:6': 4, '13:37': 8, '37:130': 3 };

// تقطيع الآية إلى كلمات بحيث كلمة[i] == خانة[i+1] في التخطيط.
...
const __wordCache = {};
const wordsOfAyah = (sNum, aNum) => {
  const key = sNum + ':' + aNum;
  if (__wordCache[key]) return __wordCache[key];
  const text = getVerseText(sNum, aNum);
  if (!text) return [];
  const out = [];
  let lead = '';
  for (const tok of text.split(/\s+/)) {
    if (!tok) continue;
    if (__hasLetter(tok)) { out.push(lead ? lead + ' ' + tok : tok); lead = ''; }
    else if (out.length) out[out.length - 1] += ' ' + tok;   // وقف/سجدة → إلى الخلف
    else lead = lead ? lead + ' ' + tok : tok;               // ۞ → إلى الأمام
  }
  if (key in __LIGATURES) { const i = __LIGATURES[key] - 1; out.splice(i, 2, out[i] + ' ' + out[i + 1]); }
  __wordCache[key] = out;
  return out;
};

// فهرسٌ يُبنى مرّةً واحدة من التخطيط:
...
let __layoutIndex = null;
const buildLayoutIndex = () => {
  if (__layoutIndex) return __layoutIndex;
  const startPage = {}, headerSurah = {};
  const flat = [];
  for (const pg of __layoutData.p) for (const ln of pg.l) flat.push({ p: pg.n, ln });
  for (let i = 0; i < flat.length; i++) {
    const F = flat[i];
    if (F.ln.t === 't') {
      for (const loc of F.ln.w) {
        const a = loc.split(':');
        if (a[1] === '1' && a[2] === '1') { const sn = parseInt(a[0], 10); if (!startPage[sn]) startPage[sn] = F.p; }
      }
    } else if (F.ln.t === 'h') {
      for (let j = i + 1; j < flat.length; j++) {
        const nx = flat[j].ln;
        if (nx.t !== 't' || !nx.w || !nx.w.length) continue;
        const a = nx.w[0].split(':');
        if (a[1] === '1' && a[2] === '1') headerSurah[F.p + ':' + F.ln.n] = parseInt(a[0], 10);
        break;
      }
    }
  }
  /* 14.2f3 */
  // ماذا أسقط المصدر؟ نمشي من أوّل كلمةٍ في كلّ سورة إلى الوراء فوق أسطر الرأس
  ...
  for (let i = 0; i < flat.length; i++) {
    const F = flat[i];
    if (F.ln.t !== 't') continue;
    for (const loc of F.ln.w) {
      const a = loc.split(':');
      if (a[1] === '1' && a[2] === '1') { const sn = parseInt(a[0], 10); if (firstLine[sn] === undefined) firstLine[sn] = i; }
    }
  }
  const needHeader = {}, needBasmala = {};
  for (let sn = 1; sn <= 114; sn++) {
    const i = firstLine[sn];
    if (i === undefined) continue;
    let hasH = false, hasB = false, j = i - 1;
    while (j >= 0 && (flat[j].ln.t === 'h' || flat[j].ln.t === 'b')) {
      if (flat[j].ln.t === 'h') hasH = true;
      if (flat[j].ln.t === 'b') hasB = true;
      j--;
    }
    if (!hasH) needHeader[sn] = 1;
    if (!hasB && sn !== 1 && sn !== 9) needBasmala[sn] = 1;
  }
  __layoutIndex = { startPage, headerSurah, needHeader, needBasmala };
  return __layoutIndex;
};
...
// صفوفُ الفهرس مرتَّبةً: بالصفحة، ثمّ الجزءُ قبل السورة عند التساوي، ثمّ بالرقم.
// تُبنى مرّةً واحدة — لا في كلّ رسم.
let __mushafNav = null;
const buildMushafNav = () => {
  if (__mushafNav) return __mushafNav;
  const startPage = buildLayoutIndex().startPage;
  const wordPage = {};
  for (const pg of __layoutData.p) for (const ln of pg.l) {
    if (ln.t !== 't' || !ln.w) continue;
    for (const loc of ln.w) if (!(loc in wordPage)) wordPage[loc] = pg.n;
  }
  const rows = [];
  for (let sn = 1; sn <= 114; sn++) {
    const p = startPage[sn];
    if (!p) return null;
    rows.push({ k: 's', n: sn, p });
  }
  let prev = 0;
  for (let jz = 1; jz <= 30; jz++) {
    const ref = JUZ_FIRST_AYAH[jz];
    const p = wordPage[ref + ':1'];
    if (!p || p <= prev) return null;
    prev = p;
    rows.push({ k: 'j', n: jz, p, s: parseInt(ref.split(':')[0], 10) });
  }
  if (rows.length !== 144) return null;
  rows.sort((a, b) => a.p - b.p || (a.k === b.k ? a.n - b.n : (a.k === 'j' ? -1 : 1)));
  __mushafNav = rows;
  return rows;
};

// ما يُرسَم قبل وصول التخطيط: السورُ وحدها بلا رقمِ صفحة. خانةُ الرقم محجوزةٌ في
...
const sliceHistoryForAPI = (messages) => {
  if (!messages || messages.length === 0) return [];
  const maxMessages = MAX_HISTORY_PAIRS * 2;
  let sliced = messages.slice(-maxMessages);
  // الـ Anthropic API يتطلب أن يبدأ السجلّ برسالة user — احذف أيّ assistant في البداية
...
const SERVER_MAX_CHAT_BODY_BYTES = 2 * 1024 * 1024;
// The client stays strictly UNDER the server: 64 KB headroom absorbs re-stringify slack + headers,
...
// Byte-driven history fit. mkBody(msgs) assembles the REAL outgoing body; drop the OLDEST messages
...
const fitMessagesToBudget = (mkBody, messages) => {
  let msgs = messages.map((m) => ((m && m.role === 'assistant' && typeof m.content === 'string')
    ? { ...m, content: ezikStripIncomplete(m.content) } : m));
  while (msgs.length > 1 && bodyByteSize(mkBody(msgs)) > CLIENT_BUDGET) {
    msgs = msgs.slice(1); // drop oldest turn; keep the system prompt + the newest user message
  }
  return msgs;
};

// File gate, budget half (defect 45): the per-type cap is not the only ceiling. Once the ~111KB
...
const attachOverBudget = (attachBlock, messages, profile) => {
  const p = profile || {};
  const band = deriveCaps(p.age).band;
  // D02ب: the posted body no longer carries the ~50KB system prompt — the server builds it from
  ...
  const mk = (msgs) => ({ max_tokens: 4096, stream: true, name: p.name, age: p.age, gender: p.gender, mode: 'chat', messages: msgs, band });
  const toApi = (arr) => sliceHistoryForAPI(arr).map(m => ({ role: m.role, content: m.content }));
  const userWith = { role: 'user', content: [attachBlock, { type: 'text', text: 'اشرح لي هذا' }] };
  const baseBytes = bodyByteSize(mk(toApi(messages || [])));
  const withBytes = bodyByteSize(mk(toApi([...(messages || []), userWith])));
  if (withBytes <= CLIENT_BUDGET) return null;
  const room = Math.max(0, CLIENT_BUDGET - baseBytes); // bytes that were free for this attachment
  const need = withBytes - baseBytes;                  // bytes this attachment actually adds
  const kb = (n) => Math.max(0, Math.round(n / 1024));
  return 'المحادثة طويلة ولم يبقَ متّسعٌ كافٍ لهذا الملف في هذه الرسالة (المتبقّي نحو ' + kb(room) + ' ك.ب، والملف يحتاج نحو ' + kb(need) + ' ك.ب). ابدأ محادثةً جديدة أو أرسل ملفًّا أصغر.';
};

// Item A: the largest RAW file of a base64-sent type that could ever fit CLIENT_BUDGET on an EMPTY
...
const deriveTypeCeiling = (kind, policyBytes, profile) => {
  const BASE64_MEDIA = { pdf: 'application/pdf', image: 'image/jpeg' };
  const media = BASE64_MEDIA[kind];
  if (!media) return { ceilingBytes: policyBytes, bound: 'policy' }; // clipped text: policy IS the true ceiling
  const p = profile || {};
  const band = deriveCaps(p.age).band;
  const block = { type: kind === 'pdf' ? 'document' : 'image', source: { type: 'base64', media_type: media, data: '' } };
  // D02ب: same correction as attachOverBudget above — model the body that is posted, which no
  // longer contains the system prompt.
  const body = { max_tokens: 4096, stream: true, name: p.name, age: p.age, gender: p.gender, mode: 'chat', band, messages: [{ role: 'user', content: [block, { type: 'text', text: 'اشرح لي هذا' }] }] };
  const overhead = bodyByteSize(body);                          // the whole empty-convo body minus the base64 data
  ...
  return theoreticalRaw < policyBytes ? { ceilingBytes: theoreticalRaw, bound: 'budget' } : { ceilingBytes: policyBytes, bound: 'policy' };
};

// Item A: render a byte count as an Arabic size string (Arabic-Indic digits) -- computed at the
// moment it is shown, never a literal.
const humanBytesAr = (n) => toArabicDigits(n >= 1024 * 1024 ? (n / (1024 * 1024)).toFixed(1) + ' ميغابايت' : Math.max(1, Math.round(n / 1024)) + ' كيلوبايت');

// Item A: a per-type "too big" refusal naming the THREE things a trustworthy limit must state:
...
const fileTooBigMsg = (typeLabel, fileSize, ceilingBytes, bound) => {
  const why = bound === 'budget' ? 'سعة الرسالة' : 'حدّ النوع';
  return typeLabel + ' أكبر من المسموح به الآن. الحدّ الحاليّ نحو ' + humanBytesAr(ceilingBytes) +
    ' (السقف من: ' + why + ')، وحجم ملفك نحو ' + humanBytesAr(fileSize) + '. الرجاء ملفًّا أصغر.';
};

// Plain-text view of a message's content (string, or [image,text] array) for logs.
const contentToText = (content) => {
  if (typeof content === 'string') return content;
  if (Array.isArray(content)) {
    const t = content.find(b => b && b.type === 'text');
    const img = content.some(b => b && b.type === 'image');
    return (img ? '📷 ' : '') + (t ? t.text : '');
  }
  return '';
};

// Read an image File → downscale to ≤1568px long edge, normalize to JPEG, return
...
const FRIENDLY_ERRORS = {
  rateLimit: {
    male: 'عُذْرًا، انْشَغَلْتُ قَلِيلًا. هَلْ تُعِيدُ مَا قُلْتَ بَعْدَ لَحْظَة؟',
    female: 'عُذْرًا، انْشَغَلْتُ قَلِيلًا. هَلْ تُعِيدِينَ مَا قُلْتِ بَعْدَ لَحْظَة؟',
  },
  server: {
    male: 'يَبْدُو أَنَّ الخَادِمَ مَشْغُولٌ الآن. حَاوِلْ مَرَّةً ثَانِيَة بَعْدَ قَلِيل.',
    female: 'يَبْدُو أَنَّ الخَادِمَ مَشْغُولٌ الآن. حَاوِلِي مَرَّةً ثَانِيَة بَعْدَ قَلِيل.',
  },
  general: {
    male: 'عُذْرًا، لَمْ أَفْهَمْ سُؤَالَكَ تَمَامًا. أَعِدْ كَلَامَكَ مِنْ فَضْلِك.',
    female: 'عُذْرًا، لَمْ أَفْهَمْ سُؤَالَكِ تَمَامًا. أَعِيدِي كَلَامَكِ مِنْ فَضْلِكِ.',
  },
  // A NON-2xx answer from our own proxy. The request never became an answer, so nothing about
  ...
  technical: {
    male: 'حَدَثَ خَلَلٌ تِقْنِيٌّ عِنْدِي، لَا فِي سُؤَالِكَ. حَاوِلْ مَرَّةً ثَانِيَةً بَعْدَ قَلِيل.',
    female: 'حَدَثَ خَلَلٌ تِقْنِيٌّ عِنْدِي، لَا فِي سُؤَالِكِ. حَاوِلِي مَرَّةً ثَانِيَةً بَعْدَ قَلِيل.',
  },
  network: {
    male: 'يَبْدُو أَنَّ الاتِّصَالَ ضَعِيفٌ الآن. تَحَقَّقْ مِنَ الإِنْتَرْنِت وَحَاوِلْ ثَانِيَة.',
    female: 'يَبْدُو أَنَّ الاتِّصَالَ ضَعِيفٌ الآن. تَحَقَّقِي مِنَ الإِنْتَرْنِت وَحَاوِلِي ثَانِيَة.',
  },
};
...
const DEVICE_ID_KEY = 'mrb_device_v1';
const FOUNDER_TOKEN_KEY = 'mrb_founder_v1';
// Must satisfy the server's /^[A-Za-z0-9_-]{8,64}$/ (lib/daycap.js safeId) or the server
...
const getDeviceId = () => {
  try {
    ...
    // Private mode / storage blocked: send nothing and let the cookie counter carry the cap.
    return null;
  }
};

// Extra headers for a capped request. The founder TOKEN may be present; the founder PIN
...
// Unlock token (directive 82). Kept in the SAME key capHeaders() already reads
...
const founderTokenAlive = (t) => {
  if (typeof t !== 'string') return false;
  const p = t.split('.');
  if (p.length !== 4 || p[0] !== 'v2') return false;
  const exp = Number.parseInt(p[1], 10);
  return Number.isFinite(exp) && exp * 1000 > Date.now();
};
const hasFounderToken = () => { try { return founderTokenAlive(localStorage.getItem(FOUNDER_TOKEN_KEY)); } catch (e) { return false; } };
const storeFounderToken = (t) => { try { localStorage.setItem(FOUNDER_TOKEN_KEY, t); } catch (e) {} };
...
const EZ_AI_CONSENT_KEY = 'ezik_ai_consent_v1';
const EZ_AI_CONSENT_VERSION = '2026-08-06-1';
// The header every AI route re-checks server-side (lib/ai-consent.js). Hiding a button is not a
// gate; the server refuses 403 without this, so a hand-made POST reaches no vendor either.
const EZ_AI_CONSENT_HEADER = 'x-ezik-ai-consent';
...
// THE ONE PREDICATE. Read at the moment of sending, never from a flag captured earlier.
const hasValidAIConsent = () => {
  const c = readAIConsent();
  return !!c && c.status === EZ_AI_CONSENT_GRANTED && c.version === EZ_AI_CONSENT_VERSION;
};

// "Has this reader answered THIS version of the question?" A recorded refusal counts as an
...
const aiConsentAnswered = () => {
  const c = readAIConsent();
  return !!c && c.version === EZ_AI_CONSENT_VERSION;
};

const aiConsentStatus = () => {
  const c = readAIConsent();
  return (c && c.version === EZ_AI_CONSENT_VERSION) ? c.status : null;
};
const aiConsentGrantedBy = () => {
  const c = readAIConsent();
  return (c && c.version === EZ_AI_CONSENT_VERSION) ? (c.grantedBy || null) : null;
};

// The only writer. `by` is 'user' (13+) or 'guardian' (under 13, after the adult barrier).
const writeAIConsent = (status, by) => {
  const rec = {
  ...
  try { localStorage.setItem(EZ_AI_CONSENT_KEY, JSON.stringify(rec)); } catch (e) {}
  return rec;
};

// Headers carried by an AI request. Empty when there is no consent -- and aiFetch never gets
// that far, so an un-headed AI POST cannot be produced by this client at all.
const aiConsentHeaders = () => (hasValidAIConsent() ? { [EZ_AI_CONSENT_HEADER]: EZ_AI_CONSENT_VERSION } : {});

// Which POSTs are AI sends. /api/unlock and /api/report are NOT: no vendor sees them.
const EZ_AI_ENDPOINTS = ['/api/ask', '/api/chat', '/api/chat-fast', '/api/tashkeel', '/api/tts', '/api/stt'];

// THE CHOKE POINT. Every send to an AI route goes through here and nothing else calls fetch()
...
class EzAIConsentError extends Error {
  constructor() { super('ai-consent-missing'); this.name = 'EzAIConsentError'; this.aiConsentMissing = true; }
}
...
const EZ_LIVE_RECOGNIZERS = new Set();

const ezSpeechEngine = () => {
  try { return window.SpeechRecognition || window.webkitSpeechRecognition || null; } catch (e) { return null; }
};
// "Does this browser have an engine at all?" -- a capability question, asked so the UI can show
// the honest "your browser cannot do this" message. It constructs nothing and sends nothing.
const ezSpeechAvailable = () => !!ezSpeechEngine();

// Detach every handler and stop the engine. Called on withdrawal and whenever a start is refused,
// so a recognizer can never be left holding a live onend that would re-arm it.
const ezKillRecognizer = (rec) => {
  if (!rec) return;
  try { rec.onresult = null; } catch (e) {}
  try { rec.onend = null; } catch (e) {}      // FIRST: an abort() fires onend, and a live onend restarts
  try { rec.onerror = null; } catch (e) {}
  try { rec.onstart = null; } catch (e) {}
  try { if (typeof rec.abort === 'function') rec.abort(); } catch (e) {}
  try { if (typeof rec.stop === 'function') rec.stop(); } catch (e) {}
  try { EZ_LIVE_RECOGNIZERS.delete(rec); } catch (e) {}
};

// THE ONLY CONSTRUCTOR. Returns null without consent -- and a null recognizer is exactly what
// every caller already handles as "this browser has no engine", so the refusal needs no new
// failure path anywhere.
const ezNewRecognition = () => {
  if (!hasValidAIConsent()) return null;
  const SR = ezSpeechEngine();
  if (!SR) return null;
  let rec = null;
  try { rec = new SR(); } catch (e) { return null; }
  try { EZ_LIVE_RECOGNIZERS.add(rec); } catch (e) {}
  return rec;
};

// THE ONLY START. Every call site -- first start and every auto-restart -- goes through it.
// Returns true only if the engine actually began listening.
const ezStartRecognition = (rec) => {
  if (!rec) return false;
  if (!hasValidAIConsent()) { ezKillRecognizer(rec); return false; }
  try { rec.start(); return true; } catch (e) { return false; }
};

// Withdrawal, applied to speech. Every live engine is torn down handlers-first so nothing can
// re-arm, and the registry is emptied. Safe to call when nothing is running.
const ezStopAllRecognition = () => {
  let n = 0;
  try {
    for (const rec of Array.from(EZ_LIVE_RECOGNIZERS)) { ezKillRecognizer(rec); n++; }
    EZ_LIVE_RECOGNIZERS.clear();
  } catch (e) {}
  return n;
};
...
const MicOffIcon = (p) => <Icon {...p}><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></Icon>;
const ResetIcon = (p) => <Icon {...p}><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></Icon>;
const MoonStarsIcon = (p) => <Icon {...p}><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/><circle cx="17.5" cy="6.5" r="0.8" fill="currentColor"/><circle cx="20" cy="9.5" r="0.6" fill="currentColor"/></Icon>;
const PlayIcon = (p) => <Icon {...p}><polygon points="6 4 20 12 6 20 6 4" fill="currentColor"/></Icon>;
...
const DAILY_VERSES = [
  { kind: 'ayah', ref: '94:6', text: 'إِنَّ مَعَ ٱلْعُسْرِ يُسْرًا', surah: 'الشرح', ayah: '٦' },
  { kind: 'ayah', ref: '57:4', text: 'وَهُوَ مَعَكُمْ أَيْنَ مَا كُنتُمْ', surah: 'الحديد', ayah: '٤' },
  { kind: 'ayah', ref: '2:153', text: 'إِنَّ ٱللَّهَ مَعَ ٱلصَّـٰبِرِينَ', surah: 'البقرة', ayah: '١٥٣' },
  { kind: 'ayah', ref: '2:152', text: 'فَٱذْكُرُونِىٓ أَذْكُرْكُمْ', surah: 'البقرة', ayah: '١٥٢' },
  { kind: 'ayah', ref: '65:2', text: 'وَمَن يَتَّقِ ٱللَّهَ يَجْعَل لَّهُۥ مَخْرَجًا', surah: 'الطلاق', ayah: '٢' },
  { kind: 'ayah', ref: '7:56', text: 'إِنَّ رَحْمَتَ ٱللَّهِ قَرِيبٌ مِّنَ ٱلْمُحْسِنِينَ', surah: 'الأعراف', ayah: '٥٦' },
  { kind: 'ayah', ref: '2:186', text: 'وَإِذَا سَأَلَكَ عِبَادِى عَنِّى فَإِنِّى قَرِيبٌ', surah: 'البقرة', ayah: '١٨٦' },
  { kind: 'ayah', ref: '2:195', text: 'إِنَّ ٱللَّهَ يُحِبُّ ٱلْمُحْسِنِينَ', surah: 'البقرة', ayah: '١٩٥' },
  { kind: 'ayah', ref: '12:87', text: 'يَـٰبَنِىَّ ٱذْهَبُوا۟ فَتَحَسَّسُوا۟ مِن يُوسُفَ وَأَخِيهِ وَلَا تَا۟يْـَٔسُوا۟ مِن رَّوْحِ ٱللَّهِ ۖ إِنَّهُۥ لَا يَا۟يْـَٔسُ مِن رَّوْحِ ٱللَّهِ إِلَّا ٱلْقَوْمُ ٱلْكَـٰفِرُونَ', surah: 'يوسف', ayah: '٨٧' },
  { kind: 'ayah', ref: '2:155', text: 'وَبَشِّرِ ٱلصَّـٰبِرِينَ', surah: 'البقرة', ayah: '١٥٥' },
  { kind: 'ayah', ref: '13:28', text: 'أَلَا بِذِكْرِ ٱللَّهِ تَطْمَئِنُّ ٱلْقُلُوبُ', surah: 'الرعد', ayah: '٢٨' },
  { kind: 'ayah', ref: '11:88', text: 'وَمَا تَوْفِيقِىٓ إِلَّا بِٱللَّهِ', surah: 'هود', ayah: '٨٨' },
  { kind: 'ayah', ref: '3:173', text: 'حَسْبُنَا ٱللَّهُ وَنِعْمَ ٱلْوَكِيلُ', surah: 'آل عمران', ayah: '١٧٣' },
  { kind: 'ayah', ref: '2:20', text: 'إِنَّ ٱللَّهَ عَلَىٰ كُلِّ شَىْءٍ قَدِيرٌ', surah: 'البقرة', ayah: '٢٠' },
  { kind: 'ayah', ref: '3:139', text: 'وَلَا تَهِنُوا۟ وَلَا تَحْزَنُوا۟ وَأَنتُمُ ٱلْأَعْلَوْنَ', surah: 'آل عمران', ayah: '١٣٩' },
  { kind: 'other', text: 'إِنَّمَا الْأَعْمَالُ بِالنِّيَّاتِ', surah: 'حديث', ayah: 'البخاري' },
  { kind: 'ayah', ref: '2:43', text: 'وَأَقِيمُوا۟ ٱلصَّلَوٰةَ وَءَاتُوا۟ ٱلزَّكَوٰةَ', surah: 'البقرة', ayah: '٤٣' },
  { kind: 'ayah', ref: '20:114', text: 'وَقُل رَّبِّ زِدْنِى عِلْمًا', surah: 'طه', ayah: '١١٤' },
  { kind: 'ayah', ref: '2:173', text: 'إِنَّ ٱللَّهَ غَفُورٌ رَّحِيمٌ', surah: 'البقرة', ayah: '١٧٣' },
  { kind: 'ayah', ref: '2:45', text: 'وَٱسْتَعِينُوا۟ بِٱلصَّبْرِ وَٱلصَّلَوٰةِ', surah: 'البقرة', ayah: '٤٥' },
  { kind: 'ayah', ref: '2:201', text: 'رَبَّنَآ ءَاتِنَا فِى ٱلدُّنْيَا حَسَنَةً', surah: 'البقرة', ayah: '٢٠١' },
  { kind: 'ayah', ref: '9:120', text: 'إِنَّ ٱللَّهَ لَا يُضِيعُ أَجْرَ ٱلْمُحْسِنِينَ', surah: 'التوبة', ayah: '١٢٠' },
  { kind: 'ayah', ref: '4:32', text: 'وَلَا تَتَمَنَّوْا۟ مَا فَضَّلَ ٱللَّهُ بِهِۦ بَعْضَكُمْ عَلَىٰ بَعْضٍ ۚ لِّلرِّجَالِ نَصِيبٌ مِّمَّا ٱكْتَسَبُوا۟ ۖ وَلِلنِّسَآءِ نَصِيبٌ مِّمَّا ٱكْتَسَبْنَ ۚ وَسْـَٔلُوا۟ ٱللَّهَ مِن فَضْلِهِۦٓ ۗ إِنَّ ٱللَّهَ كَانَ بِكُلِّ شَىْءٍ عَلِيمًا', surah: 'النساء', ayah: '٣٢' },
  { kind: 'ayah', ref: '33:3', text: 'وَتَوَكَّلْ عَلَى ٱللَّهِ ۚ وَكَفَىٰ بِٱللَّهِ وَكِيلًا', surah: 'الأحزاب', ayah: '٣' },
  { kind: 'ayah', ref: '53:39', text: 'وَأَن لَّيْسَ لِلْإِنسَـٰنِ إِلَّا مَا سَعَىٰ', surah: 'النجم', ayah: '٣٩' },
  { kind: 'ayah', ref: '3:146', text: 'وَٱللَّهُ يُحِبُّ ٱلصَّـٰبِرِينَ', surah: 'آل عمران', ayah: '١٤٦' },
  { kind: 'ayah', ref: '2:143', text: 'إِنَّ ٱللَّهَ بِٱلنَّاسِ لَرَءُوفٌ رَّحِيمٌ', surah: 'البقرة', ayah: '١٤٣' },
  { kind: 'ayah', ref: '65:3', text: 'وَمَن يَتَوَكَّلْ عَلَى ٱللَّهِ فَهُوَ حَسْبُهُۥٓ', surah: 'الطلاق', ayah: '٣' },
  { kind: 'ayah', ref: '26:62', text: 'إِنَّ مَعِىَ رَبِّى سَيَهْدِينِ', surah: 'الشعراء', ayah: '٦٢' },
  { kind: 'ayah', ref: '12:64', text: 'قَالَ هَلْ ءَامَنُكُمْ عَلَيْهِ إِلَّا كَمَآ أَمِنتُكُمْ عَلَىٰٓ أَخِيهِ مِن قَبْلُ ۖ فَٱللَّهُ خَيْرٌ حَـٰفِظًا ۖ وَهُوَ أَرْحَمُ ٱلرَّٰحِمِينَ', surah: 'يوسف', ayah: '٦٤' },
];
...
const rescueTruncated = (text) => String(text || '')
  .replace(VERSE_TAG_EXTENT, ' ')
  .replace(/<[^>]*>/g, ' ')
  // §٤ (D): the same correction as in `stripIncompleteTags` below, and for the same reason —
...
// opts.rescue — للقُرّاءِ الذين يقرأون النصَّ النهائيَّ وحدَهم (العرضُ والصوتُ وسجلُّ الأهل).
// المسارُ المتدفّقُ لا يمرّره: الوسمُ الناقصُ أثناءَ التدفّقِ حالةٌ طبيعيّةٌ والقصُّ فيه صواب.
const stripIncompleteTags = (text, opts) => {
  if (!text || typeof text !== 'string') return text || '';
  let t = text;
  // ٠. طَبِّع وسمَي الآية/السورة ذاتيَّي الإغلاق (<verse .../>، <surah .../>) إلى صيغةٍ
  //    مُغلَقة حتى لا يَعدّهما كاشفُ الوسوم الناقصة وسماً مقطوعاً فيحذفهما. (كلاهما فارغ بلا نصّ.)
  t = t.replace(/<(verse|surah)([^>]*?)\s*\/>/g, '<$1$2></$1>');
  // ١. أول وسم معروف مفتوح بلا إغلاق مطابق → احذف من بدايته حتى نهاية النص.
  const openRe = new RegExp(`<(${KNOWN_TAGS})\\b`, 'g');
  let m, cut = -1;
  while ((m = openRe.exec(t)) !== null) {
    const closeRe = new RegExp(`</${m[1]}>`, 'g');
    closeRe.lastIndex = m.index;
    if (!closeRe.test(t)) { cut = m.index; break; }
  }
  if (cut >= 0) t = t.slice(0, cut);
  // ٢. «<» مقطوعٌ في آخرِ النصِّ بلا «>» مطابق (مثل «<ver» أو «<verse surah=») → يُحذَف.
  //
  ...
  const out = t;
  if (opts && opts.rescue && out.trim() === '' && text.trim() !== '') {
    const tagMatch = new RegExp(`<(${KNOWN_TAGS})\\b`).exec(text);
    const rescued = rescueTruncated(text);
    // التسجيلُ دائمًا (أ-٦/٢). كان `if (rescued)` يحرسُ الدفعَ في السجلِّ كما يحرسُ العودة، فحين
    ...
    EZIK_TAG_RESCUES.push({
      tag: tagMatch ? tagMatch[1] : 'unknown',
      reason: rescued ? 'truncated-tag-emptied-reply' : 'truncated-tag-emptied-reply-unrescuable',
      chars: rescued.length,
...
const readStepsTitle = (attrsStr) => {
  const m = String(attrsStr || '').match(/title=["']([^"']*)["']/);
  return m ? m[1].trim() : '';
};
...
const normalizeAttribution = (s) => String(s || '')
  .replace(/[ً-ْٰـ]/g, '')   // تشكيل وتطويل
  .replace(/[أإآٱ]/g, 'ا').replace(/ى/g, 'ي').replace(/ة/g, 'ه')
  .replace(/[^ء-ي\s]/g, ' ')
  .replace(/\s+/g, ' ')
  .trim();
// ألفاظُ الدرجاتِ التي شوهدت — أو تُشبِهُ ما شوهد — في خانةِ المخرِّج. مطابقةٌ تامّةٌ بعد
// التطبيع لا احتواء: «الحسن البصريّ» مخرِّجٌ لا درجة، و«حسن» وحدَها درجة.
const RULING_PHRASES = new Set([
  'متفق عليه', 'متفق علي صحته', 'رواه الشيخان', 'اخرجه الشيخان',
  'صحيح', 'حسن', 'ضعيف', 'حسن صحيح', 'صحيح لغيره', 'حسن لغيره',
  'ضعيف جدا', 'موضوع', 'منكر', 'مرسل', 'متواتر', 'شاذ',
  'اسناده صحيح', 'اسناده حسن', 'اسناده ضعيف',
].map(normalizeAttribution));
// «حسّنه الألباني» / «صحّحه ابن باز» / «ضعّفه الألباني» — درجةٌ منسوبةٌ لا مخرِّج.
const RULING_VERB_RE = /^(حسنه|صححه|ضعفه|حسن|صحح|ضعف)\s/;
const isRulingPhrase = (s) => {
  const n = normalizeAttribution(s);
  if (!n) return false;
  return RULING_PHRASES.has(n) || RULING_VERB_RE.test(n);
};
// ── «رَوَى رواه البخاري» (قرار ١١) ─────────────────────────────────────────
...
const NARRATION_VERBS = new Set(['رواه', 'أخرجه', 'خرّجه'].map(normalizeAttribution));
// القرارُ يُتَّخَذُ على الصورةِ المطبَّعةِ والقصُّ يقعُ على الأصل، فيحتفظُ الاسمُ برسمِه
// وتشكيلِه كما ورد: «رواه البخاريُّ» ⟹ «البخاريُّ» لا «البخاري».
const stripNarrationVerb = (s) => {
  const raw = String(s || '').trim();
  const sp = raw.search(/\s/);
  // كلمةٌ واحدةٌ لا تكونُ فعلًا واسمًا، فلا يُنزَعُ منها شيءٌ — وإلّا أفرغنا الخانةَ كلَّها.
  if (sp === -1) return raw;
  if (!NARRATION_VERBS.has(normalizeAttribution(raw.slice(0, sp)))) return raw;
 ...
 */
const resolveHadithAttribution = (narrator, ruling) => {
  let n = String(narrator || '').trim();
  let r = String(ruling || '').trim();
  if (n && isRulingPhrase(n)) { if (!r) r = n; n = ''; }
  else {
    // النزعُ بعدَ فحصِ الدرجةِ لا قبلَه، والترتيبُ مقصود: «رواه الشيخان» لفظُ درجةٍ مسجَّلٌ
    // في RULING_PHRASES، ونزعُ بادئتِه أوّلًا كان يُحيلُه اسمًا «الشيخان» ويُبطِلُ قاعدةً قائمة.
    n = stripNarrationVerb(n);
    if (n && r && normalizeAttribution(n) === normalizeAttribution(r)) { n = ''; }
  }
  return { narrator: n, ruling: r };
};

// مفتاح تجريبي (A/B) — صحّحه إلى true لمقارنة النطق بلا تشكيل في المتصفح:
...
const STRIP_TASHKEEL_FOR_TTS = false;
const TTS_TASHKEEL_ENABLED = false; // false = skip Haiku tashkeel add-call for speech (pre-diacritized canonical text unaffected); flip true to restore original behavior
...
const stripTashkeelOutsideQuran = (s) => {
  ...
  let last = 0;
  let m;
  QURAN_SPAN_RE.lastIndex = 0;
  while ((m = QURAN_SPAN_RE.exec(src)) !== null) {
    out += src.slice(last, m.index).replace(HARAKAT_RE, '');
    out += m[0];
    last = m.index + m[0].length;
  }
  return out + src.slice(last).replace(HARAKAT_RE, '');
};
const miniBtnStyle = { display: 'inline-flex', alignItems: 'center', gap: 5, background: 'none', border: '1px solid var(--line)', borderRadius: 8, padding: '4px 8px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit', color: 'inherit', opacity: 0.7 };
...
const ezikLegacyCopy = (payload) => {
  try {
    const ta = document.createElement('textarea');
    ta.value = payload;
    ta.setAttribute('readonly', '');               // keeps the mobile keyboard shut
    ...
    ta.focus(); ta.select();
    try { ta.setSelectionRange(0, String(payload).length); } catch (e) {}   // iOS needs the range
    const done = document.execCommand('copy') === true;
    document.body.removeChild(ta);
    return done;
  } catch (e) { return false; }
};
const ezikWriteClipboard = async (payload) => {
  ...
  } catch (e) { wrote = false; }                   // a refusal is not the end -- fall through
  if (!wrote) wrote = ezikLegacyCopy(payload);
  return wrote;
};
const CopyReplyButton = ({ text, getText }) => {
  const [flash, setFlash] = useState('');            // '' | 'ok' | 'fail'
  if (!text) return null;
  const doCopy = async () => {
    const payload = (typeof getText === 'function') ? getText() : text;
    if (!payload) { setFlash('fail'); setTimeout(() => setFlash(''), 1500); return; }
    const wrote = await ezikWriteClipboard(payload);
...
const ShareReplyButton = ({ getText }) => {
  const [flash, setFlash] = useState('');            // '' | 'copied' | 'fail'
  const say = (v) => { setFlash(v); setTimeout(() => setFlash(''), 1500); };
  const doShare = async () => {
    const payload = (typeof getText === 'function') ? getText() : '';
    if (!payload) { say('fail'); return; }
    try {
      if (navigator.share) {
    ...
    say(await ezikWriteClipboard(payload) ? 'copied' : 'fail');
  };
  return (
    <button type="button" onClick={doShare} aria-label={EZIK_SHARE_ARIA} className="ezik-focus" style={miniBtnStyle}>
...
// يهرّب رموز HTML كي لا يُحقَن ترميزٌ من المحتوى (احتياطًا)
const escapeHtml = (s) => (s || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

const toPlainText = (s) => String(s || '')
  .replace(/<[^>]+>/g, ' ')                 // any leftover angle tag
...
const ExportPdfReplyButton = ({ getText }) => {
  ...
  // ONE PRESS, ONE FILE. State alone could not promise it: two taps dispatched inside one task
  ...
  const doExport = async () => {
    ...
    setFlash('wait');
    try {
      await printAsPdf(EZIK_PDF_TITLE, docToHtml(payload));
      setFlash('');
    } catch (e) {
      setFlash('fail');
      setTimeout(() => setFlash(''), 1500);
  ...
  return (
    <button type="button" onClick={doExport} aria-label={EZIK_PDF_ARIA} className="ezik-focus" style={miniBtnStyle}>
      {flash === 'wait' ? EZIK_PDF_WAIT : flash === 'fail' ? EZIK_PDF_FAIL : EZIK_PDF_LABEL}
    </button>
  );
};
...
const ezikCardWrap = (ctx, text, maxWidth) => {
  const out = [];
  for (const para of String(text || '').replace(/\r\n?/g, '\n').split('\n')) {
    ...
    for (const w of words) {
      ...
      if (line && ctx.measureText(next).width > maxWidth) { out.push(line); line = w; }
      else line = next;
    }
...
const ezikDrawReplyCard = (opts) => {
  ...
  // THE CUT IS SAID, not merely marked with an ellipsis a reader can mistake for the author's.
  if (cut) {
    ctx.save();
...
const docToHtml = (md) => {
  ...
  const out = [];
  let i = 0;
  const isSep = (s) => /^[\s|:-]+$/.test(s.trim()) && s.includes('-') && s.includes('|');
  ...
  while (i < lines.length) {
    ...
    if (t.includes('|') && i + 1 < lines.length && isSep(lines[i + 1])) {
      ...
      i += 2;
      const rows = [];
      while (i < lines.length && lines[i].trim() && lines[i].includes('|')) { rows.push(parseRow(lines[i])); i++; }
      let tbl = '<table><thead><tr>' + headers.map((h) => '<th>' + inlineFmt(h) + '</th>').join('') + '</tr></thead>';
      if (rows.length) tbl += '<tbody>' + rows.map((r) => '<tr>' + r.map((c) => '<td>' + inlineFmt(c) + '</td>').join('') + '</tr>').join('') + '</tbody>';
      tbl += '</table>';
      out.push(tbl); continue;
    }
    let m;
    if ((m = t.match(/^####\s+(.*)$/))) { out.push('<h4>' + inlineFmt(m[1]) + '</h4>'); i++; continue; }
    if ((m = t.match(/^###\s+(.*)$/))) { out.push('<h3>' + inlineFmt(m[1]) + '</h3>'); i++; continue; }
    if ((m = t.match(/^##\s+(.*)$/)))  { out.push('<h2>' + inlineFmt(m[1]) + '</h2>'); i++; continue; }
    if ((m = t.match(/^#\s+(.*)$/)))   { out.push('<h1>' + inlineFmt(m[1]) + '</h1>'); i++; continue; }
    if (/^[-•*]\s+/.test(t)) {
      const items = [];
      while (i < lines.length && /^[-•*]\s+/.test(lines[i].trim())) { items.push('<li>' + inlineFmt(lines[i].trim().replace(/^[-•*]\s+/, '')) + '</li>'); i++; }
...
function deriveCaps(age) {
  ...
  return {
    ...
    memorize: true,
    recite:   true,
    upload:   band !== 'young',   // hidden for young by BAND (no parent override)
...
// ============================================================
function standingNoticeKey(band) {
...
// ============================================================
function childVoiceBlocked() {
  if (CHILD_VOICE_ENABLED) return false;
  try {
    const p = voiceProfileRef.current;
    if (!p) return true;                       // fail-closed
    return deriveCaps(p.age).band === 'young';
...
function showChildVoiceNotice(setMsg) {
  try {
    setMsg(CHILD_VOICE_NOTICE);
    setTimeout(() => { try { setMsg(''); } catch (e) {} }, 6000);
  } catch (e) {}
}
...
const EZIK_NOTICE_ALL = /【([^】\n]{1,80})】/gu;

const ezikLiftNotices = (segments) => {
  const out = [];
  (segments || []).forEach((seg) => {
    if (!seg || seg.type !== 'text' || !EZIK_NOTICE_RE.test(seg.content || '')) { out.push(seg); return; }
    const prose = [];
    const notices = [];
    String(seg.content).split('\n').forEach((line) => {
      if (!EZIK_NOTICE_RE.test(line)) { prose.push(line); return; }
      const labels = [];
      const stripped = line
        .replace(EZIK_NOTICE_ALL, (all, label) => { labels.push(label.trim()); return ''; })
        .replace(/[ \t]{2,}/g, ' ')
        .trim();
      const opensLine = /^\s*【/.test(line);
      labels.forEach((label, i) => notices.push({
        type: 'notice', label, content: (opensLine && i === 0) ? stripped : '',
      }));
      if (!opensLine && stripped) prose.push(stripped);
    });
    const kept = prose.join('\n').replace(/\n{3,}/g, '\n\n').trim();
    if (kept) out.push({ type: 'text', content: kept });
    notices.forEach((notice) => out.push(notice));
  });
  return out;
};

const parseRichMessage = (text, viewerAge) => {
  // Tolerate a <document> whose </document> never arrived (stream/token cutoff): close it so it still renders as a card.
  { const _d = text.search(/<document\b[^>]*>/); if (_d !== -1 && text.indexOf('</document>', _d) === -1) text = text + '</document>'; }
  if (!text || typeof text !== 'string') {
    return { segments: [{ type: 'text', content: text || '' }], suggestions: [] };
  }
  ...
  while ((match = tagPattern.exec(text)) !== null) {
    ...
    if (tagName === 'verse') {
      const surahMatch = attrsStr.match(/surah=["']([^"']+)["']/);
      const surahNumMatch = attrsStr.match(/surah_num=["']([^"']+)["']/);
      const ayahMatch = attrsStr.match(/ayah=["']([^"']+)["']/);
      segments.push({
        type: 'verse',
        content,
      ...
      const numMatch = attrsStr.match(/num=["']([^"']+)["']/);
      const fromMatch = attrsStr.match(/from=["']([^"']+)["']/);
      const toMatch = attrsStr.match(/to=["']([^"']+)["']/);
      segments.push({
        type: 'surah',
      ...
      const narratorMatch = attrsStr.match(/narrator=["']([^"']+)["']/);
      const rulingMatch = attrsStr.match(/ruling=["']([^"']+)["']/);
      segments.push({
        type: 'hadith',
        content,
      ...
      segments.push({
        type: 'dhikr',
        catId: dhikrIdMatch ? dhikrIdMatch[1] : '',
      });
    } else if (tagName === 'worship') {
      const worshipIdMatch = attrsStr.match(/id=["']([^"']+)["']/);
      segments.push({
        type: 'worship',
        id: worshipIdMatch ? worshipIdMatch[1] : '',
      });
    } else if (tagName === 'source') {
      const siteMatch = attrsStr.match(/site=["']([^"']+)["']/);
      ...
      const items = content.split('\n')
        ...
        .filter(l => l);
      segments.push({ type: 'steps', items, title: readStepsTitle(attrsStr) });
    } else if (tagName === 'suggestions') {
      const items = content.split('\n')
        .map(l => l.trim())
        .filter(l => l)
        .map(l => l.replace(/^[-•*]\s*/, '').trim())
      ...
      if (deriveCaps(viewerAge).export) {
        segments.push({ type: 'document', title: titleMatch ? titleMatch[1] : 'مستند', content });
      } else {
        segments.push({ type: 'text', content: toPlainText((titleMatch ? titleMatch[1] + '\n\n' : '') + content) });
      }
    }
  ...
  // إن لم نجد وسوماً، النص كله نصّ عادي
  if (segments.length === 0 && !suggestions.length) {
    segments.push({ type: 'text', content: text.trim() });
  }
  // XI-04: the review mark leaves the prose here, LAST — after every card has been separated, so
...
const formatForTTS = (text) => {
  ...
  // وكذلك السورة الكاملة: يتلوها القارئ الحقيقي عبر الزرّ، لا ينطقها المربّي
  t = t.replace(/<surah[^>]*>[\s\S]*?<\/surah>/g, ' ');
  // الذكر: يُقرأ من صوت حصن المسلم عبر زرّ البطاقة، لا يُنطَق من المربّي
  t = t.replace(/<dhikr[^>]*>[\s\S]*?<\/dhikr>/g, ' ');
  t = t.replace(/<dhikr[^>]*\/?>/g, ' ');
  // اللوح: يُعرَض مكتوبًا ولا يُنطَق (كالآيات) — يبقى الشرحُ المنطوقُ كلماتٍ سهلة
  t = t.replace(/<board[^>]*>[\s\S]*?<\/board>/g, ' ');
  // المستند: يُقرأ ويُصدَّر مكتوبًا، فلا يُنطَق كاملًا — نستبدله بعبارةٍ قصيرة
  t = t.replace(/<document[^>]*>[\s\S]*?<\/document>/g, ' هذا مستندٌ مكتوبٌ، يمكن قراءتُه وتصديرُه. ');
  t = t.replace(/<document[^>]*>[\s\S]*$/g, ' هذا مستندٌ مكتوبٌ، يمكن قراءتُه وتصديرُه. ');
  // تحويل الأحاديث إلى نص طبيعي للنطق — يجب أن يسبق تجريدَ الأقواس الزاويّة أدناه، وإلا فُقِد الوسم.
  // متسامحٌ مع ترتيب الخصائص وغيابها: نستخرج narrator وruling كلاًّ على حدة، ولا نلفظ أسماء الوسوم نفسها.
  t = t.replace(/<hadith([^>]*)>([\s\S]*?)<\/hadith>/g, (_, attrs, content) => {
    const nm = attrs.match(/narrator=["']([^"']+)["']/);
    const rm = attrs.match(/ruling=["']([^"']+)["']/);
    // الدرجةُ لا تُنطَقُ مرّتين، ولا يُنطَقُ «رَوَى متفق عليه».
    const att = resolveHadithAttribution(nm ? nm[1] : '', rm ? rm[1] : '');
    const body = content.trim();
    let out = att.narrator ? ` رَوَى ${att.narrator}: ${body}.` : ` ${body}.`;
    if (att.ruling) out += ` ${att.ruling}.`;
    return out + ' ';
  });
  // تحويل الخطوات إلى نص طبيعي للنطق — كذلك قبل تجريد الأقواس. نسمح بخصائص اختياريّة على الوسم المفتوح.
  t = t.replace(/<steps([^>]*)>([\s\S]*?)<\/steps>/g, (_, attrs, content) => {
    const items = content.split('\n').map(l => l.trim()).filter(l => l).map(l => l.replace(/^[-•*]\s*/, '').trim()).filter(l => l);
    if (!items.length) return '';
    // العنوانُ من الجواب إن وُجد، وإلّا فالخطواتُ تُنطَقُ بلا عنوانٍ أصلًا.
    const title = readStepsTitle(attrs);
    return (title ? ' ' + title + ': ' : ' ') + items.join(' ثُمَّ ') + '. ';
  });
  // <worship> is resolved to the frozen cell text up-front (resolveWorshipTags).
  // Any tag surviving to here -> a single space (silence): never the raw tag, never model text.
  t = t.replace(/<worship[^>]*>[\s\S]*?<\/worship>/g, ' ');
  t = t.replace(/<worship[^>]*\/?>/g, ' ');
  // ── §٤ — الوسمُ لا يُنطَق (the review mark is a label, not a sentence) ──────
  ...
  t = t
       .replace(/(^|[^\p{L}\p{N}_])(cos|sin|tan)(?=$|[^\p{L}\p{N}_])/giu,
         (_all, left, name) => left + trigNames[name.toLowerCase()])
       .replace(/π/g, ' باي ').replace(/°/g, ' درجة ').replace(/√/g, ' الجذر التربيعيّ لـ ')
       .replace(/×/g, ' في ').replace(/÷/g, ' على ').replace(/\//g, ' على ')
       .replace(/\+/g, ' زائد ').replace(/−/g, ' ناقص ').replace(/=/g, ' يساوي ')
       .replace(/≤/g, ' أصغر أو يساوي ').replace(/≥/g, ' أكبر أو يساوي ').replace(/≠/g, ' لا يساوي ')
       .replace(/²/g, ' تربيع ').replace(/³/g, ' تكعيب ').replace(/\^/g, ' أُس ').replace(/%/g, ' بالمئة ')
       .replace(/½/g, ' نصف ').replace(/⅓/g, ' ثلث ').replace(/⅔/g, ' ثلثين ').replace(/¼/g, ' ربع ').replace(/¾/g, ' ثلاثة أرباع ');
  // شبكة أمان أخيرة (دفاعٌ في العمق): أيّ اسمِ وسمٍ إنجليزيّ ناجٍ — بقايا وسمٍ مشوّهٍ فلَتَ من
  ...
  /* tts-num-words */
  // الأعداد المكتوبة رقمًا في النثر المنطوق (خارج <board> المجرَّد أعلاه) → كلماتٍ عربيّةً كي
  ...
  // وأرقامُ <board> لا تُقرأ أصلًا لأنّها حُذِفت قبل هذا السطر.
  {
    ...
    const _huns  = ['', 'مئة', 'مئتان', 'ثلاثمئة', 'أربعمئة', 'خمسمئة', 'ستمئة', 'سبعمئة', 'ثمانمئة', 'تسعمئة'];
    const _digitWord = ['صفر', 'واحد', 'اثنان', 'ثلاثة', 'أربعة', 'خمسة', 'ستة', 'سبعة', 'ثمانية', 'تسعة'];
    const _three = (n) => {
      const out = [];
      const h = Math.floor(n / 100), r = n % 100;
      if (h) out.push(_huns[h]);
      if (r) {
        if (r < 10) out.push(_ones[r]);
        else if (r < 20) out.push(_teens[r - 10]);
        else { const u = r % 10, tn = Math.floor(r / 10); out.push(u ? _ones[u] + ' و' + _tens[tn] : _tens[tn]); }
      }
      return out.join(' و');
    };
    const _int = (n) => {
      ...
      if (th) out.push(th === 1 ? 'ألف' : th === 2 ? 'ألفان' : _three(th) + ' آلاف');
      if (r) out.push(_three(r));
      return out.join(' و');
    };
    const _dbd  = (s) => s.split('').map((d) => _digitWord[+d]).join(' ');
    const _norm = (s) => s.replace(/[٠-٩]/g, (d) => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d)));
    t = t.replace(/[٠-٩0-9]+(?:[.٫‚][٠-٩0-9]+)?/g, (tok) => {
      const m = tok.match(/^([٠-٩0-9]+)(?:[.٫‚]([٠-٩0-9]+))?$/);
      if (!m) return tok;
      const ip = _norm(m[1]), fp = m[2] ? _norm(m[2]) : null;
      const n = parseInt(ip, 10);
      const iw = (ip.length <= 4 && n <= 9999) ? _int(n) : _dbd(ip);
...
const formatForLog = (text) => {
  ...
  t = t.replace(/<verse([^>]*)>([\s\S]*?)<\/verse>/g, (_, attrs) => {
    const sNum = resolveSurahNumber(
      (attrs.match(/surah=["']([^"']+)["']/) || [])[1],
      (attrs.match(/surah_num=["']([^"']+)["']/) || [])[1]
    );
    const aNum = parseInt((attrs.match(/ayah=["']([^"']+)["']/) || [])[1], 10);
    const name = sNum ? SURAH_NAMES[sNum] : '';
    const ref = [name && `سورة ${name}`, aNum >= 1 && `آية ${aNum}`].filter(Boolean).join('، ');
    const canon = (sNum && aNum >= 1) ? getVerseText(sNum, aNum) : null;
    return canon ? ` «${canon}»${ref ? ` (${ref})` : ''} ` : (ref ? ` [${ref}] ` : ' ');
  });
  // السورة: مرجعٌ مختصرٌ للأهل (الاسم + المدى إن كان جزئياً) — لا نصّ خام
  t = t.replace(/<surah([^>]*)>([\s\S]*?)<\/surah>/g, (_, attrs) => {
    const sNum = resolveSurahNumber(undefined, (attrs.match(/num=["']([^"']+)["']/) || [])[1]);
    const name = sNum ? SURAH_NAMES[sNum] : '';
    const from = parseInt((attrs.match(/from=["']([^"']+)["']/) || [])[1], 10);
    const to = parseInt((attrs.match(/to=["']([^"']+)["']/) || [])[1], 10);
    const rangePart = (from >= 1 && to >= 1) ? `، الآيات ${from}–${to}` : '';
    return name ? ` [تلاوة سورة ${name}${rangePart}] ` : ' ';
  });
  // الحديث والخطوات: نفس تحويل الصوت كي يبقى السجلّ مقروءاً
  ...
  // كان يطابقُ `<steps>` عاريةً فقط — فأيُّ خاصّيّةٍ عليها (وعنوانُ الخطواتِ خاصّيّةٌ الآن)
  ...
  // ── §٣ — الوسمُ شارةٌ في السجلِّ أيضًا، لا محارفُ خامّ (the mark is a badge here too) ──
  //
...
// ============================================================
const formatForStreamPreview = (text) => {
  if (!text) return '';
  let t = stripIncompleteTags(text); // drops the first unclosed known tag + any truncated trailing "<..."
  t = t.replace(/<suggestions[^>]*>[\s\S]*?<\/suggestions>/g, '');
  t = t.replace(/<verse[^>]*>[\s\S]*?<\/verse>/g, ' ');
  t = t.replace(/<surah[^>]*>[\s\S]*?<\/surah>/g, ' ');
...
const SUGGESTIONS = [
  'أبغى أحكي لك عن يومي',
  'علمني دعاء جميل',
  'احكي لي قصة من السيرة',
  'كيف أصلي صح؟',
  'ساعدني أحفظ سورة قصيرة',
];
...
const EZIK_VISUAL_THEME_KEY = 'ezik_visual_theme_v2';
const EZIK_VISUAL_THEME_KEY_V1 = 'ezik_visual_theme_v1';   // read by nothing; removed on erase
...
const EZIK_VISUAL_THEME_ISTANA = 'istana_33';
const EZIK_VISUAL_THEME_DEFAULT = EZIK_VISUAL_THEME_ISTANA;
const EZIK_VISUAL_THEME_EVENT = 'ezik-visual-theme';
const EZIK_VISUAL_THEME_ATTR = 'data-ezik-visual-theme';

function readEzikVisualTheme() {
  let raw = null;
  try { raw = localStorage.getItem(EZIK_VISUAL_THEME_KEY); } catch (e) { return EZIK_VISUAL_THEME_DEFAULT; }
  if (raw === EZIK_VISUAL_THEME_ISTANA) return raw;
  return EZIK_VISUAL_THEME_DEFAULT;
}
function applyEzikVisualTheme(v) {
  const next = (v === EZIK_VISUAL_THEME_ISTANA) ? v : EZIK_VISUAL_THEME_DEFAULT;
  try { document.documentElement.setAttribute(EZIK_VISUAL_THEME_ATTR, next); } catch (e) {}
  return next;
}
function writeEzikVisualTheme(v) {
  // paint first, save second: the attribute is what the user sees, and a storage that refuses
  // the write must not also refuse the repaint.
  const next = applyEzikVisualTheme(v);
  try { localStorage.setItem(EZIK_VISUAL_THEME_KEY, next); } catch (e) {}
  try { window.dispatchEvent(new CustomEvent(EZIK_VISUAL_THEME_EVENT, { detail: next })); } catch (e) {}
  return next;
}
function useEzikVisualTheme() {
  const [vtheme, setVtheme] = useState(readEzikVisualTheme);
  useEffect(() => {
    // Both listeners re-READ rather than trust what they were handed, so an event carrying a
    // value this build does not accept still lands on istana_33 instead of on that value.
    const onLocal = () => setVtheme(readEzikVisualTheme());
    const onStorage = (e) => { if (!e || !e.key || e.key === EZIK_VISUAL_THEME_KEY) setVtheme(readEzikVisualTheme()); };
    window.addEventListener(EZIK_VISUAL_THEME_EVENT, onLocal);
    window.addEventListener('storage', onStorage);
    return () => {
      window.removeEventListener(EZIK_VISUAL_THEME_EVENT, onLocal);
      window.removeEventListener('storage', onStorage);
    };
  }, []);
  useEffect(() => { applyEzikVisualTheme(vtheme); }, [vtheme]);
  return vtheme;
}

// ITEM 97 -- HOME_GREETINGS_GENERAL STOOD HERE, sixteen keys deep, and the day branch below
...
const HOME_GREETINGS_MORNING = [
    'home.greet.morning.1',
    'home.greet.morning.2',
    'home.greet.morning.3',
    'home.greet.morning.4',
    'home.greet.morning.5',
    'home.greet.morning.6',
    'home.greet.morning.7',
    'home.greet.morning.8',
  ];
const HOME_GREETINGS_EVENING = [
    'home.greet.evening.1',
    'home.greet.evening.2',
    'home.greet.evening.3',
    'home.greet.evening.4',
    'home.greet.evening.5',
    'home.greet.evening.6',
    'home.greet.evening.7',
    'home.greet.evening.8',
  ];
// ITEM 97 -- THE DHIKR ITSELF, AND THE CYCLE THAT ORDERS IT.
...
// The label comes back after five seconds. Said once, read twice: the timer and the guard.
const HOME_DHIKR_MS = 5000;
// A COUNTER, not a random pick. Random repeats, and two identical presses in a row is exactly
...
const EZIK_DHIKR_CYCLE_KEY = 'ezik_dhikr_cycle_v1';
const nextDhikrSeq = () => {
  let n = 0;
  try {
    const raw = parseInt(localStorage.getItem(EZIK_DHIKR_CYCLE_KEY), 10);
    if (Number.isFinite(raw) && raw >= 0) n = raw;
  } catch (e) {}
  // wraps well below Number.MAX_SAFE_INTEGER and stays a small string in storage.
  try { localStorage.setItem(EZIK_DHIKR_CYCLE_KEY, String((n + 1) % 1000000)); } catch (e) {}
  return n;
};
...
const getHomeGreeting = () => {
  ...
  const pick = (arr) => arr[(day * 5) % arr.length];
  if (h >= 5 && h < 11) return { text: ezT(pick(HOME_GREETINGS_MORNING)), period: 'morning', tappable: true };
  if (h >= 16 && h < 19) return { text: ezT(pick(HOME_GREETINGS_EVENING)), period: 'evening', tappable: true };
  // Outside the two windows: one line, the dhikr label, and it is TAPPABLE -- but pressing it
...
const EZH_ICON_TREASURE = (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 4L3 6v14l6-2 6 2 6-2V4l-6 2-6-2z" /><path d="M9 4v14" /><path d="M15 6v14" /></svg>
);
...
const EZH_ICON_SUN = (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" /></svg>
);
const EZH_ICON_MOON = (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z" /></svg>
);
// S118: EZH_ICON_PROFILE, EZH_ICON_NAV_HOME, EZH_ICON_NAV_CHAT_LINE and EZH_ICON_NAV_SET stood
...
// S118: the three bars. The SAME three lines the chat's own menu row already draws, at the
...
// THE MASTHEAD. The arch is the approved signature radius and nothing else; the tulip is three
...
function EzistMasthead({ name, g, hijri, onOpenAdhkar }) {
  const tap = !!(g && g.tappable);
  // ITEM 97 -- THE PRESS STAYS ON THIS SCREEN. The morning and evening lines are untouched:
  ...
  const isDay = !!(g && g.period === 'day');
  const [dhikr, setDhikr] = useState(null);          // null = the line is showing its label
  ...
  useEffect(() => () => { if (dhikrTimer.current) clearTimeout(dhikrTimer.current); }, []);
  const showNextDhikr = () => {
    if (dhikrTimer.current) { clearTimeout(dhikrTimer.current); dhikrTimer.current = null; }
    setDhikr(HOME_DHIKR[nextDhikrSeq() % HOME_DHIKR.length]);
    dhikrTimer.current = setTimeout(() => { dhikrTimer.current = null; setDhikr(null); }, HOME_DHIKR_MS);
  };
  const go = isDay ? showNextDhikr : onOpenAdhkar;
  // THE LINE ON SCREEN. One expression, so the button, its accessible name and its text can
  // never disagree about what it is saying.
  const line = (isDay && dhikr) ? dhikr : (g ? g.text : '');
  return (
    <section className="ezist-masthead">
      <div className="ezist-hello">
        ...
        {/* ITEM 109: the date, on the screen that already speaks about the day — MEASURED
            ...
            it does for the wird, and this component keeps drawing only what it is handed. */}
        {hijri ? <div style={s.ezistHijri}>{hijri}</div> : null}
      </div>
      <span className="ezist-rule-short" aria-hidden="true" />
      {tap ? (
        <button type="button" className="ezhome-focus" onClick={go}
          style={isDay ? { ...s.ezistPrompt, ...s.ezistPromptDay } : s.ezistPrompt} aria-label={line}>
          <span style={s.ezistPromptIcon} aria-hidden="true">{g.period === 'morning' ? EZH_ICON_SUN : EZH_ICON_MOON}</span>
          <span style={s.ezistPromptText}>{line}</span>
        </button>
      ) : (
        <div style={s.ezistPromptFlat}>
          <span style={s.ezistPromptIcon} aria-hidden="true">{g.period === 'morning' ? EZH_ICON_SUN : EZH_ICON_MOON}</span>
          <span style={s.ezistPromptText}>{g.text}</span>
        </div>
      )}
    </section>
  );
...
// THE CHAT ENTRY. There is still exactly ONE, and it is no longer on this screen's chrome at
...
function Home({ profile, onOpenMenu, onOpenMemorize, onOpenAdhkar, onOpenMushaf, onOpenFatwa, onOpenSettings }) {
  ...
  // GENUINE local progress, or nothing at all. These are the MUSHAF'S OWN helpers reading the
  ...
  // ITEM 109: the same discipline as the wird above — the OWNER reads the device, the
...
const EZIK_FATWA_API_BASE = '';

async function ezikFatwaFetch(path, signal) {
  const response = await fetch(EZIK_FATWA_API_BASE + path, {
    method: 'GET',
  ...
  if (!response.ok || !payload || payload.ok !== true) {
    ...
    throw new Error(code || ('fatwa_http_' + response.status));
  }
  return payload;
}
...
const EZIK_FATWA_ACTIONS = [
  ...
  ['explain', 'fatwa.action.explain'],
  ['quiz', 'fatwa.action.quiz'],
];
...
function ezikFatwaPublicUrl(value) {
  try {
...
// Item 86: the card can now be SAVED. It writes through the module-level store rather than
...
function EzikFatwaResult({ fatwa, favPk }) {
  const content = (fatwa && fatwa.content) || {};
  const question = content.question || '';
  const answer = content.answer || content.answerExcerpt || '';
  const audio = (fatwa && fatwa.audio) || {};
  const scholar = (fatwa && fatwa.scholar) || {};
  const source = (fatwa && fatwa.source) || {};
  const sourceUrl = ezikFatwaPublicUrl(source.canonicalUrl || source.url);
  const audioUrl = ezikFatwaPublicUrl(audio.url);
  // A machine transcript of the scholar's own clip is neither a written fatwa nor
  ...
  const isTranscript = content.type === 'auto_transcript_official_video';
  // WHAT IS SAVED IS WHAT IS SHOWN: the question and the answer as this card has them, so a
  ...
  const favRec = React.useMemo(
    () => ezikMakeFavOf('fatwa', favPk, fatwa && fatwa.uid, fatwa && fatwa.title, favText),
    [favPk, fatwa, favText]);
  const [saved, setSaved] = useState(false);
  useEffect(() => {
    const read = () => { try { setSaved(ezikReadFavs().some((f) => f.id === favRec.id)); } catch (e) {} };
    read();
    try { window.addEventListener(EZIK_FAVS_EVENT, read); } catch (e) {}
    return () => { try { window.removeEventListener(EZIK_FAVS_EVENT, read); } catch (e) {} };
  }, [favRec.id]);
  return (
    <article className="ezf-card">
      <header className="ezf-card-head">
        <h2 className="ezf-title">{fatwa.title}</h2>
        <div className="ezf-meta">
          <span className="ezf-tag">{scholar.shortName || scholar.name || ezT('fatwa.defaultScholar')}</span>
          <span>{isTranscript
            ? ezT('fatwa.transcriptTag')
            : (audio.available ? ezT('fatwa.audioAvailable') : ezT('fatwa.textOnly'))}</span>
        </div>
      </header>
      {isTranscript ? (
        <p className="ezf-notice">{ezT('fatwa.transcriptNotice')}</p>
      ) : null}
      {isTranscript ? (
        <h3 className="ezf-section-title">{ezT('fatwa.transcriptBody')}</h3>
      ) : question ? (
        <React.Fragment>
          <h3 className="ezf-section-title">{ezT('fatwa.question')}</h3>
          <p className="ezf-copy">{question}</p>
          <h3 className="ezf-section-title">{ezT('fatwa.answer')}</h3>
        </React.Fragment>
      ) : (
        <h3 className="ezf-section-title">{ezT('fatwa.officialText')}</h3>
      )}
      <p className="ezf-copy">{answer}</p>
      {audio.available && audioUrl ? (
        <audio className="ezf-audio" controls preload="none" src={audioUrl}
          aria-label={ezT('fatwa.audioAria')} />
      ) : null}
      {sourceUrl ? (
        <a className="ezhome-focus ezf-source" href={sourceUrl} target="_blank"
          rel="noopener noreferrer">
          {isTranscript ? ezT('fatwa.watchVideo') : ezT('fatwa.source')}</a>
      ) : null}
      <div className="ezf-actions">
        {/* Item 86: the one action on this card that is not disabled. It saves a FATWA, as a
            fatwa -- not as a reply, and not into adhkar_favorites_v1, which is a different
            store this screen never touches. */}
        {favPk ? (
        <button type="button" className="ezf-action ezik-focus"
          aria-pressed={saved ? 'true' : 'false'}
          aria-label={saved ? ezT('favorites.remove') : ezT('favorites.addFatwa')}
...
// THE WHOLE SHELF, NOT ONLY WHAT WAS SEARCHED FOR. Picking a scholar -- or "all fatwas",
...
const EZIK_FATWA_ALL = 'all';
const EZIK_FATWA_PAGE_SIZE = '10';

// THE SELECTOR REMEMBERS WHO IT LISTED LAST TIME. The registry answer is one network
...
const EZIK_FATWA_SCHOLARS_KEY = 'ezik_fatwa_scholars_v1';

function readCachedScholars() {
  let raw = null;
  try { raw = localStorage.getItem(EZIK_FATWA_SCHOLARS_KEY); } catch (e) { return null; }
  if (!raw) return null;
  let o = null;
  ...
  for (const item of o) {
    ...
    if (typeof item.shortName !== 'string' || !item.shortName) return null;
    list.push({ id: item.id, shortName: item.shortName });
  }
  return list;
}

function writeCachedScholars(list) {
  try { localStorage.setItem(EZIK_FATWA_SCHOLARS_KEY, JSON.stringify(list)); } catch (e) {}
}

function FatwaScreen({ onBack, favPk }) {
  const [scholars, setScholars] = useState(
    () => readCachedScholars() || [{ id: 'binbaz', shortName: '' }]);
  // THE SCREEN OPENS ON EVERYTHING. "All fatwas" is the opening selection, so the first thing
  ...
  const [browsePages, setBrowsePages] = useState(null);
  const [browseLoading, setBrowseLoading] = useState(false);
  const [browseFailed, setBrowseFailed] = useState(false);
  const [openUid, setOpenUid] = useState('');
  const browseRequest = useRef(null);

  // The selector is fed by the registry so Ibn Uthaymeen can be added server-side later. Until
  ...
  useEffect(() => {
    ...
    ezikFatwaFetch('/api/v1/scholars', controller.signal).then((payload) => {
      const list = Array.isArray(payload.scholars)
        ? payload.scholars.filter((item) => item && item.id && (item.shortName || item.name))
        : [];
      if (!list.length) return;
      const next = list.map((item) => ({ id: String(item.id), shortName: String(item.shortName || item.name) }));
      setScholars(next);
      writeCachedScholars(next);
      setScholar((current) => (current === EZIK_FATWA_ALL || list.some((item) => String(item.id) === current))
  ...
  // The shelf is fetched for whatever is selected, including the moment the screen opens, so
  ...
  const runBrowse = async (page) => {
    ...
    const params = new URLSearchParams({
      scholar,
      page: String(page || 1),
      limit: EZIK_FATWA_PAGE_SIZE,
      view: 'full',
    });
    try {
      const payload = await ezikFatwaFetch('/api/v1/fatwas/browse?' + params.toString(), controller.signal);
      if (browseRequest.current !== controller) return;
      setBrowsing(Array.isArray(payload.results) ? payload.results : []);
      setBrowsePages(payload.pagination || { page: 1, total: 0, totalPages: 0, hasPrevious: false, hasNext: false });
    } catch (error) {
      if (browseRequest.current !== controller) return;
      setBrowsing([]);
      setBrowsePages(null);
  ...
  useEffect(() => { runBrowse(1); }, [scholar]);

  const runSearch = async (page, fixedQuery) => {
    const term = String(fixedQuery == null ? query : fixedQuery).trim();
    ...
    try {
      const payload = await ezikFatwaFetch('/api/v1/fatwas/search?' + params.toString(), controller.signal);
      if (searchRequest.current !== controller) return;
      setResults(Array.isArray(payload.results) ? payload.results : []);
      setPagination(payload.pagination || { page: 1, total: 0, totalPages: 0, hasPrevious: false, hasNext: false });
    } catch (error) {
      if (searchRequest.current !== controller) return;
      setFailed(true);
    } finally {
  ...
  // Leaving a search restores the shelf rather than reloading it: the page the reader was on
  ...
  if (searched) {
    if (loading) status = ezT('fatwa.loading');
    else if (failed) { status = ezT('fatwa.error'); statusFailed = true; }
    else if (pagination && pagination.total === 0) status = ezT('fatwa.none');
    else if (pagination && pagination.total > 0) {
      status = ezT('fatwa.summary', {
        total: ezikFatwaNumber(pagination.total),
        page: ezikFatwaNumber(pagination.page),
        pages: ezikFatwaNumber(pagination.totalPages),
      });
    }
  } else if (browseLoading) status = ezT('fatwa.browseLoading');
  else if (browseFailed) { status = ezT('fatwa.error'); statusFailed = true; }
  else if (browsePages && browsePages.total === 0) status = ezT('fatwa.browseEmpty');
  else if (browsePages) {
    status = ezT('fatwa.browseSummary', {
      total: ezikFatwaNumber(browsePages.total),
      page: ezikFatwaNumber(browsePages.page),
      pages: ezikFatwaNumber(browsePages.totalPages),
    });
  }

  return (
    <div className="theme-dark ezhome ezf" style={s.ezfContainer}>
      <div className="ezsh-nav">
        <div className="ezsh-nav-inner">
          <div className="ezsh-nav-side">
            <button type="button" className="ezhome-focus" onClick={onBack}
              style={s.ezshNavBtn} aria-label={ezT('common.back')}>{A2_ICON_BACK}</button>
          </div>
          <span aria-hidden="true" />
          <div className="ezsh-nav-side is-end" />
        </div>
      </div>
      <div style={s.ezshScroll}>
        <main className="ezf-wrap">
          <form className="ezf-controls" role="search" onSubmit={(event) => {
            event.preventDefault();
            runSearch(1);
          }}>
            <label className="ezf-field" htmlFor="ezf-scholar">
              <span className="ezf-label">{ezT('fatwa.scholarLabel')}</span>
              <select id="ezf-scholar" className="ezhome-focus ezf-select"
                value={scholar} onChange={selectScholar}>
                <option value={EZIK_FATWA_ALL}>{ezT('fatwa.allScholars')}</option>
                {scholars.map((item) => (
                  <option key={item.id} value={item.id}>{item.shortName || ezT('fatwa.defaultScholar')}</option>
                ))}
              </select>
            </label>
            <div className="ezf-search">
              <input className="ezhome-focus ezf-input" type="search" dir="auto" required maxLength="180"
                value={query} onChange={(event) => setQuery(event.target.value)}
                placeholder={ezT('fatwa.searchPlaceholder')} aria-label={ezT('fatwa.searchAria')} />
              <button type="submit" className="ezhome-focus ezf-submit"
                disabled={loading}>{ezT('fatwa.searchButton')}</button>
            </div>
          </form>

          {status ? <div className={'ezf-status' + (statusFailed ? ' is-error' : '')}
            role={statusFailed ? 'alert' : 'status'} aria-live="polite">{status}</div> : null}

          {searched ? (
            <React.Fragment>
              <button type="button" className="ezhome-focus ezf-back"
                onClick={backToBrowse}>{ezT('fatwa.backToBrowse')}</button>
              {results.length ? (
                <section className="ezf-results" aria-label={ezT('fatwa.searchAria')}>
                  {results.map((fatwa) => <EzikFatwaResult key={fatwa.uid} fatwa={fatwa} favPk={favPk} />)}
                </section>
              ) : null}
            </React.Fragment>
          ) : browsing.length ? (
            <section className="ezf-list" aria-label={ezT('fatwa.browseAria')}>
              {browsing.map((fatwa) => {
                const isOpen = openUid === fatwa.uid;
                const named = (fatwa.scholar && (fatwa.scholar.shortName || fatwa.scholar.name)) || '';
                return (
                  <React.Fragment key={fatwa.uid}>
                    <button type="button" aria-expanded={isOpen}
                      className={'ezhome-focus ezf-row' + (isOpen ? ' is-open' : '')}
                      title={isOpen ? ezT('fatwa.closeFatwa') : ezT('fatwa.openFatwa')}
                      onClick={() => setOpenUid(isOpen ? '' : fatwa.uid)}>
                      <span className="ezf-row-title">{fatwa.title}</span>
                      {scholar === EZIK_FATWA_ALL && named
                        ? <span className="ezf-row-mark">{named}</span> : null}
                    </button>
                    {isOpen ? <div className="ezf-open"><EzikFatwaResult fatwa={fatwa} favPk={favPk} /></div> : null}
                  </React.Fragment>
                );
              })}
            </section>
          ...
          {searched && pagination && pagination.totalPages > 1 ? (
            <nav className="ezf-pager" aria-label={ezT('fatwa.page', {
              page: ezikFatwaNumber(pagination.page), pages: ezikFatwaNumber(pagination.totalPages)
            })}>
              <button type="button" className="ezhome-focus ezf-page-btn"
                disabled={loading || !pagination.hasPrevious}
                onClick={() => runSearch(pagination.page - 1, lastQuery)}>{ezT('fatwa.previous')}</button>
              <span className="ezf-page-text">{ezT('fatwa.page', {
                page: ezikFatwaNumber(pagination.page), pages: ezikFatwaNumber(pagination.totalPages)
              })}</span>
              <button type="button" className="ezhome-focus ezf-page-btn"
                disabled={loading || !pagination.hasNext}
                onClick={() => runSearch(pagination.page + 1, lastQuery)}>{ezT('fatwa.next')}</button>
            </nav>
          ) : null}

          {!searched && browsePages && browsePages.totalPages > 1 ? (
            <nav className="ezf-pager" aria-label={ezT('fatwa.page', {
              page: ezikFatwaNumber(browsePages.page), pages: ezikFatwaNumber(browsePages.totalPages)
            })}>
              <button type="button" className="ezhome-focus ezf-page-btn"
                disabled={browseLoading || !browsePages.hasPrevious}
                onClick={() => runBrowse(browsePages.page - 1)}>{ezT('fatwa.previous')}</button>
              <span className="ezf-page-text">{ezT('fatwa.page', {
                page: ezikFatwaNumber(browsePages.page), pages: ezikFatwaNumber(browsePages.totalPages)
              })}</span>
              <button type="button" className="ezhome-focus ezf-page-btn"
                disabled={browseLoading || !browsePages.hasNext}
                onClick={() => runBrowse(browsePages.page + 1)}>{ezT('fatwa.next')}</button>
            </nav>
          ) : null}
        </main>
      </div>
...
const readAdhkarUiFlag = () => {
  let p = null;
  try { p = new URLSearchParams(window.location.search).get('adhkarui'); } catch (e) { p = null; }
  if (p === '1' || p === '0') {
    try { localStorage.setItem(ADHKAR_UI_V2_KEY, p); } catch (e) {}
    return p === '1';
  }
  // No parameter: the default is V2, and only a stored, well-formed refusal turns it off.
  try { return localStorage.getItem(ADHKAR_UI_V2_KEY) !== '0'; } catch (e) { return true; }
};
const ADHKAR_UI_V2_ON = readAdhkarUiFlag();
...
// A module-level COPY of the V1 search normaliser, so V2 filters identically without reaching
// into V1's closure and without V1 changing. Same replacements, same order, same result.
const adhkarNormAr = (str) => String(str || '')
  .replace(/[ؐ-ًؚ-ٰٟ]/g, '')
  .replace(/[أإآ]/g, 'ا')
  .replace(/ى/g, 'ي')
  .replace(/ؤ/g, 'و')
...
const ADHKAR_PROGRESS_KEY = 'adhkar_daily_progress_v1';
const ADHKAR_FAVORITES_KEY = 'adhkar_favorites_v1';
const ADHKAR_USAGE_KEY = 'adhkar_usage_v1';
const ADHKAR_DAILY_GOAL = 8;

// ── ITEM 43-أ: THE ADHKAR CHAIN, AND THE NUMBER THE READER CHOSE ────────────────────────────
...
const ADHKAR_STREAK_KEY = 'ezik_adhkar_streak_v1';
// A day's dhikr, not a mountain: one is a real day and twenty is already more than the whole of
...
const ADHKAR_GOAL_MIN = 1;
const ADHKAR_GOAL_MAX = 20;
const ADHKAR_GOAL_PRESETS = [3, 5, 8, 12, 20];
...
// THE COUNT THE DHIKR ITSELF STATES.
//
...
const A2_TASHKEEL_RE = /[\u064B-\u0652\u0670\u06D6-\u06ED\u0640]/g;
const A2_TAIL_PAREN_RE = /\(([^()]{1,30})\)\s*[.\u060C\u061B]?\s*$/;
const A2_REPEAT_UNIT_RE = /^مر(?:ة|ات)$/;
// Only words that name a number greater than one: a group that resolves to 1 is no instruction at
// all, and this must never be able to LOWER a target.
const A2_REPEAT_WORDS = {
  'مرتين': 2,
  'ثلاث': 3, 'ثلاثا': 3,
  'أربع': 4, 'اربع': 4,
  'خمس': 5, 'ست': 6, 'سبع': 7, 'ثمان': 8, 'تسع': 9,
  'عشر': 10, 'عشرا': 10,
  'مائة': 100, 'مئة': 100,
};
const adhkarTextRepeat = (text) => {
  const t = String(text || '').replace(A2_TASHKEEL_RE, '');
  const m = A2_TAIL_PAREN_RE.exec(t);
  if (!m) return 0;
  const parts = m[1].split(/\s+/).filter(Boolean);
  if (parts.length === 1) return parts[0] === 'مرتين' ? 2 : 0;
  if (parts.length !== 2 || !A2_REPEAT_UNIT_RE.test(parts[1])) return 0;
  const n = A2_REPEAT_WORDS[parts[0]];
  return (n && n > 1) ? n : 0;
};

// The item's REAL repetition target. THE STORED NUMBER STILL WINS whenever it says anything at
...
const adhkarTarget = (d) => {
  const n = parseInt(d && d.repeat, 10);
  if (Number.isFinite(n) && n > 1) return n;
  const fromText = adhkarTextRepeat(d && d.text);
  if (fromText > 1) return fromText;
  return (Number.isFinite(n) && n > 0) ? n : 1;
};

// { d: 'YYYY-MM-DD', n: { '<catId>:<index>': count } }. Anything that is not TODAY'S well-formed
...
function readAdhkarProgress() {
  const today = adhkarDayKey();
  const empty = { d: today, n: {} };
  let raw = null;
  try { raw = localStorage.getItem(ADHKAR_PROGRESS_KEY); } catch (e) { return empty; }
  if (!raw) return empty;
  let o = null;
  try { o = JSON.parse(raw); } catch (e) { return empty; }
  if (!o || typeof o !== 'object' || o.d !== today) return empty;
  if (!o.n || typeof o.n !== 'object') return empty;
  const n = {};
  for (const k of Object.keys(o.n)) {
    if (!A2_ID_RE.test(k)) continue;
    const v = o.n[k];
...
const adhkarCountOf = (rec, catId, i) => (rec && rec.n && rec.n[adhkarItemKey(catId, i)]) || 0;
// The ONLY writer of the progress key. One press is one increment: it adds exactly 1 and it
...
function bumpAdhkarCount(catId, i, target) {
  const rec = readAdhkarProgress();
  const key = adhkarItemKey(catId, i);
  if (!A2_ID_RE.test(key)) return rec;
  const cap = (Number.isInteger(target) && target > 0) ? target : 1;
  const cur = rec.n[key] || 0;
  if (cur >= cap) return rec;
...
function adhkarCatDone(rec, catId, items) {
  if (!rec || !Array.isArray(items)) return 0;
  let done = 0;
  for (let i = 0; i < items.length; i++) {
    if (adhkarCountOf(rec, catId, i) >= adhkarTarget(items[i])) done += 1;
  }
  return done;
}

// The day BEFORE a 'YYYY-MM-DD' key, computed on the string alone. No second Date is built and
...
function adhkarPrevDayKey(key) {
  ...
  return String(py) + '-' + (pm < 10 ? '0' : '') + String(pm) + '-' + (pd < 10 ? '0' : '') + String(pd);
}
// Anything that is not a well-formed record reads as the DEFAULT record -- a missing key, damaged
...
function readAdhkarStreak() {
  const empty = { goal: ADHKAR_DAILY_GOAL, last: '', run: 0 };
  let raw = null;
  try { raw = localStorage.getItem(ADHKAR_STREAK_KEY); } catch (e) { return empty; }
  if (!raw) return empty;
  let o = null;
  try { o = JSON.parse(raw); } catch (e) { return empty; }
  if (!o || typeof o !== 'object') return empty;
  const goal = (Number.isInteger(o.goal) && o.goal >= ADHKAR_GOAL_MIN && o.goal <= ADHKAR_GOAL_MAX)
    ? o.goal : ADHKAR_DAILY_GOAL;
  const last = (typeof o.last === 'string' && o.last.length === 10 && adhkarPrevDayKey(o.last)) ? o.last : '';
  const run = (last && Number.isInteger(o.run) && o.run > 0) ? o.run : 0;
  return { goal: goal, last: last, run: run };
}
// THE RUN AS OF A GIVEN DAY. A chain whose last credited day is today, or yesterday, is still
...
function adhkarRunAsOf(rec, today) {
  if (!rec || !rec.run || !rec.last) return 0;
  if (rec.last === today) return rec.run;
  if (rec.last === adhkarPrevDayKey(today)) return rec.run;
  return 0;
}
// The goal is the reader's. Out-of-range writes nothing and returns the record unchanged, so the
// caller always renders a number the store would actually accept.
function writeAdhkarGoal(n) {
  const rec = readAdhkarStreak();
  if (!Number.isInteger(n) || n < ADHKAR_GOAL_MIN || n > ADHKAR_GOAL_MAX) return rec;
  const next = { goal: n, last: rec.last, run: rec.run };
  try { localStorage.setItem(ADHKAR_STREAK_KEY, JSON.stringify(next)); } catch (e) {}
  return next;
}
// THE ONLY WRITER OF THE CHAIN. It is handed today's real completion count and does nothing at
...
function markAdhkarDayMet(done) {
  const rec = readAdhkarStreak();
  if (!Number.isInteger(done) || done < rec.goal) return rec;
  const today = adhkarDayKey();
  if (rec.last === today) return rec;
  const run = (rec.run > 0 && rec.last === adhkarPrevDayKey(today)) ? rec.run + 1 : 1;
  const next = { goal: rec.goal, last: today, run: run };
  try { localStorage.setItem(ADHKAR_STREAK_KEY, JSON.stringify(next)); } catch (e) {}
  return next;
}

function adhkarCompletedToday(rec, byCat) {
  if (!rec || !rec.n || !byCat) return 0;
  let done = 0;
  for (const k of Object.keys(rec.n)) {
    const parts = k.split(':');
    const list = byCat[parseInt(parts[0], 10)];
    if (!Array.isArray(list)) continue;
    const item = list[parseInt(parts[1], 10)];
    if (!item) continue;
    if (rec.n[k] >= adhkarTarget(item)) done += 1;
  }
  return done;
}

// Favourites: an ARRAY OF IDENTIFIERS. No text, no title, no audio URL, no timestamp. Entries
// that are not well-formed identifiers, and duplicates, are dropped on read.
function readAdhkarFavorites() {
  let raw = null;
  try { raw = localStorage.getItem(ADHKAR_FAVORITES_KEY); } catch (e) { return []; }
  if (!raw) return [];
  let o = null;
  ...
  const out = [];
  for (const v of o) {
    if (typeof v !== 'string' || !A2_ID_RE.test(v)) continue;
    if (out.indexOf(v) !== -1) continue;
    out.push(v);
  }
  return out;
}
function toggleAdhkarFavorite(key) {
  const cur = readAdhkarFavorites();
...
// Usage: { '<catId>': timesOpened }. The single call site is the category-open handler, so the
...
function readAdhkarUsage() {
  let raw = null;
  try { raw = localStorage.getItem(ADHKAR_USAGE_KEY); } catch (e) { return {}; }
  if (!raw) return {};
  let o = null;
  try { o = JSON.parse(raw); } catch (e) { return {}; }
...
// Most-used, resolved against the REAL category list: a stored id with no category is skipped,
...
const A3_WINDOW = 7;
function adhkarWindow(len, idx, size) {
  const n = Math.max(0, parseInt(len, 10) || 0);
  const w = Math.min(n, Math.max(1, parseInt(size, 10) || 1));
  if (w <= 0) return [];
  const at = Math.min(Math.max(0, parseInt(idx, 10) || 0), n - 1);
  const start = Math.max(0, Math.min(at - Math.floor(w / 2), n - w));
  const out = [];
...
const A3_ALL          = '\u{0627}\u{0644}\u{0623}\u{0642}\u{0633}\u{0627}\u{0645}';
// S100 -- three more chrome strings, escaped for the same reason every string above is: a
...
const EZ_VT_ISTANA = '\u{0625}\u{0633}\u{062A}\u{0627}\u{0646}\u{0629} \u{0663}\u{0663}';
// S102 -- the state words for the active-design card. Chrome, and neither is a choice.
...
function AdhkarScreenV1({ onBack }) {
  const [cats, setCats] = useState(null);
  const [query, setQuery] = useState('');
  const [selected, setSelected] = useState(null);
  useEffect(() => {
    let alive = true;
    loadAdhkar().then((db) => { if (alive) setCats(db.categories || []); }).catch(() => { if (alive) setCats([]); });
    return () => { alive = false; };
  }, []);
  ...
  const list = (cats || []).filter((c) => !q || normAr(c.title).includes(q));
  // S87 -- the ONE line this rollback screen gains, and it renders nothing: while a category is
  ...
  if (selected) {
    return (
      <div style={s.adhkarContainer}>
        <div style={s.adhkarHeader}>
          {/* S91: the layer owns a history entry, so its back is the same pop the device makes. */}
          <button onClick={ezikGoBack} style={s.adhkarBackBtn}>رجوع</button>
          <div style={s.adhkarTitle}>{selected.title}</div>
        </div>
        <div style={s.adhkarScroll}>
          <DhikrCard catId={selected.id} />
        </div>
      </div>
    );
  }
  return (
    <div style={s.adhkarContainer}>
      {/* S13.3-adhkar */}
      <div style={s.adhkarHeader}>
        <button onClick={onBack} style={s.adhkarBackBtn}>رجوع</button>
        <div style={s.adhkarTitle}>الأذكار</div>
      </div>
      <div style={s.adhkarScroll}>
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="ابحث عن ذكر" style={s.welcomeInput} />
        {cats === null ? (
          <div style={s.adhkarEmpty}>...</div>
        ) : list.length === 0 ? (
          <div style={s.adhkarEmpty}>لا نتائج</div>
        ) : (
          <div style={s.adhkarList}>
            {list.map((c) => (
              <button key={c.id} onClick={() => setSelected(c)} style={s.adhkarRow}>
                <div style={s.adhkarRowTitle}>{c.title}</div>
                <span style={s.adhkarCount}>{c.count}</span>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--nav-off)', flexShrink: 0 }}><path d="M15 18l-6-6 6-6" /></svg>
              </button>
            ))}
          </div>
        )}
...
// RTL: the page runs right to left, so "back" points RIGHT and "onward" points LEFT. These
// two are written as the glyphs the direction calls for, not mirrored by the browser.
const A2_ICON_BACK = (
  <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6" /></svg>
);
const A2_ICON_ONWARD = (
...
const A2_ICON_SEARCH = (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7" /><path d="M20 20l-3.2-3.2" /></svg>
);
// The controls the second pass adds. All four are drawn in the SAME line-icon language as the
...
const A2_ICON_HEART = (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.8 5.9a5 5 0 0 0-7.1 0L12 7.6l-1.7-1.7a5 5 0 1 0-7.1 7.1l8.8 8.7 8.8-8.7a5 5 0 0 0 0-7.1z" /></svg>
);
// The filled heart is the SAME path with a fill -- the favourite state is a solid shape, not a
// second drawing, so on/off cannot drift apart.
const A2_ICON_HEART_ON = (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.8 5.9a5 5 0 0 0-7.1 0L12 7.6l-1.7-1.7a5 5 0 1 0-7.1 7.1l8.8 8.7 8.8-8.7a5 5 0 0 0 0-7.1z" /></svg>
);
const A2_ICON_SHARE = (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3" /><circle cx="6" cy="12" r="3" /><circle cx="18" cy="19" r="3" /><path d="M8.6 10.6l6.8-4M8.6 13.4l6.8 4" /></svg>
);
const A2_ICON_CHECK = (
...
function AdhkarScreenV2({ onBack }) {
  // Session 85. THIS COMPONENT IS STILL THE ONLY OWNER OF BROWSE STATE. The store, the query,
  ...
  const [usage, setUsage] = useState(readAdhkarUsage);
  ...
  const q = adhkarNormAr(query).trim();
  const list = (cats || []).filter((c) => !q || adhkarNormAr(c.title).includes(q));
  // THE ONLY usage write in the app, and it is still on the OPEN -- one increment per opened
  ...
  const open = (c, at) => {
    setUsage(bumpAdhkarUsage(c.id));
    const n = parseInt(at, 10);
    setStartAt(Number.isFinite(n) && n > 0 ? n : 0);
    setSelected(c);
  };
  // ITEM 97 REVOKED -- a deep link stood here, and it is gone with the navigation that fed it.
  ...
  const closeAdhkarDetail = () => {
    setSelected(null);
    setProg(readAdhkarProgress());
    setFavs(readAdhkarFavorites());
    setUsage(readAdhkarUsage());
  };
  useEzikBackLayer(!!selected, closeAdhkarDetail);
  // COMPLETED means target reached, counted over distinct items, and capped for DISPLAY at the
  // goal -- min(done, 8) of 8. A ninth completion is real and is not shown as a ninth eighth.
  const done = adhkarCompletedToday(prog, db && db.byCat);
  // ITEM 43-أ. THE ONE PLACE THE CHAIN IS CREDITED, and it is keyed on the real completion count
  ...
  useEffect(() => { setStreak(markAdhkarDayMet(done)); }, [done]);
  ...
  if (selected) return <AdhkarCategoryV2 cat={selected} startAt={startAt} onBack={ezikGoBack} />;
  // ONE view object, handed to whichever home is drawn. Both receive the same store, the same
  ...
  const view = {
    ...
    query: query,
    setQuery: setQuery,
    // The row is open while it is asked for OR while a query is live, so a filter can never be
    ...
    searchOpen: searchOpen || query.length > 0,
    onToggleSearch: () => {
      const next = !(searchOpen || query.length > 0);
      setSearchOpen(next);
      if (!next) setQuery('');
    },
    onOpen: open,
  ...
  return <IstanaAdhkarBrowse {...view} />;
}
...
function A3Search({ query, setQuery }) {
  return (
    <div style={s.a3SearchRow}>
      <span style={s.a3SearchIcon}>{A2_ICON_SEARCH}</span>
      <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={A2_SEARCH}
        style={s.a3SearchInput} aria-label={A2_SEARCH} />
    </div>
  );
}
...
const EZIA_FEATURED = 3;

// One catalogue card. The treatment varies with the category's own standing and position --
...
function IstanaAdhkarCard({ c, st, i, onOpen }) {
  const coral = (i % 3) === 2;
  return (
    <button type="button" className="adhkar2-focus" onClick={() => onOpen(c)} data-ezia-cat={c.id}
      style={{ ...s.eziaCard, ...(st.full ? s.eziaCardDone : null) }}>
      <span className={coral ? 'ezia-crest ezia-crest-coral' : 'ezia-crest'} aria-hidden="true" />
      <span style={s.eziaCardHead}>
        <span style={st.full ? { ...s.eziaEmblem, ...s.eziaEmblemDone } : s.eziaEmblem} aria-hidden="true">
          <span className="ezia-star" />
        </span>
        <span style={s.eziaCount}>{toArabicDigits(c.count)}</span>
      </span>
      <span style={s.eziaCardTitle}>{c.title}</span>
      <span style={s.eziaCardFoot}>
        <span style={s.eziaStanding}>
          {st.total > 0 && st.done > 0 ? (toArabicDigits(st.done) + ' / ' + toArabicDigits(st.total)) : ''}
        </span>
        <span style={s.eziaGo} aria-hidden="true">{st.full ? A2_ICON_CHECK : EZH_ICON_GO}</span>
      </span>
    </button>
  );
}

// A featured card: the same data, the same handler, the same id attribute -- a larger arch, a
// bigger emblem and room for the standing line. It is not a second copy of anything.
function IstanaAdhkarFeature({ c, st, onOpen }) {
  return (
    <button type="button" className="adhkar2-focus" onClick={() => onOpen(c)} data-ezia-cat={c.id}
      style={{ ...s.eziaFeature, ...(st.full ? s.eziaCardDone : null) }}>
      <span style={s.eziaFeatureTop}>
        <span style={st.full ? { ...s.eziaEmblemLg, ...s.eziaEmblemDone } : s.eziaEmblemLg} aria-hidden="true">
          <span className="ezia-star" />
        </span>
        <span style={s.eziaCount}>{toArabicDigits(c.count)}</span>
      </span>
      <span style={s.eziaFeatureTitle}>{c.title}</span>
      <span style={s.eziaCardFoot}>
        <span style={s.eziaStanding}>
          {st.total > 0 && st.done > 0 ? (toArabicDigits(st.done) + ' / ' + toArabicDigits(st.total)) : A2_SOURCE}
        </span>
        <span style={s.eziaGo} aria-hidden="true">{st.full ? A2_ICON_CHECK : EZH_ICON_GO}</span>
      </span>
    </button>
...
function IstanaAdhkarBrowse({ onBack, cats, list, query, setQuery, searchOpen, onToggleSearch, onOpen, prog, byCat, done, goal, run, onGoal }) {
  ...
  // the real ring, unchanged: min(completions, goal) over the goal, and nothing else.
  const CIRC = 2 * Math.PI * 30;
  const off = CIRC * (1 - (shown / goal));
  const featured = list.slice(0, EZIA_FEATURED);
  const rest = list.slice(EZIA_FEATURED);
  return (
    <div className="theme-dark adhkar3" style={s.eziaContainer}>
      ...
      <div className="ezia-nav">
        <div className="ezia-nav-inner">
          ...
          <button type="button" className="adhkar2-focus" onClick={onToggleSearch} aria-label={A2_SEARCH}
            aria-pressed={searchOpen ? 'true' : 'false'}
            style={searchOpen ? { ...s.eziaNavBtn, ...s.eziaNavBtnOn } : s.eziaNavBtn}>{A2_ICON_SEARCH}</button>
        </div>
      </div>
      ...
      <div style={s.eziaScroll}>
        <div className="ezia-wrap">
          <section className="ezia-masthead">
            <span style={s.eziaRingBox} role="progressbar" aria-label={A2_GOAL_TITLE}
              aria-valuemin={0} aria-valuemax={goal} aria-valuenow={shown}>
              <svg width="76" height="76" viewBox="0 0 76 76" aria-hidden="true">
                <circle cx="38" cy="38" r="30" fill="none" stroke="var(--a3-surface)" strokeWidth="7" />
                <circle cx="38" cy="38" r="30" fill="none" stroke="var(--a3-blue)" strokeWidth="7" strokeLinecap="round"
                  strokeDasharray={CIRC} strokeDashoffset={off} transform="rotate(-90 38 38)" />
              </svg>
              <span style={s.eziaRingNums}>{toArabicDigits(shown)} / {toArabicDigits(goal)}</span>
            </span>
            <span style={s.eziaMastText}>
              <span style={s.eziaMastTitle}>{A2_GOAL_TITLE}</span>
              <span style={s.eziaMastHint}>{A2_SOURCE}</span>
            </span>
          </section>
            {/* ITEM 43-أ. The chain and the goal, on the masthead the ring already sits in.
                The number is drawn the same way whether the chain stands or has lapsed. */}
            <div style={s.eziaChainRow}>
              <span style={s.eziaChainWord}>{A3_CHAIN_TITLE}</span>
              <span style={s.eziaChainNum}>{toArabicDigits(run)}</span>
              <span style={s.eziaChainWord}>{run > 0 ? A3_CHAIN_DAYS : A3_CHAIN_START}</span>
            </div>
            <div style={s.eziaGoalRow} role="group" aria-label={A3_GOAL_PICK}>
              <span style={s.eziaGoalLabel}>{A3_GOAL_PICK}</span>
              {ADHKAR_GOAL_PRESETS.map((n) => (
                <button key={n} type="button" className="adhkar2-focus" onClick={() => onGoal(n)}
                  aria-pressed={goal === n ? 'true' : 'false'}
                  style={goal === n ? { ...s.eziaGoalChip, ...s.eziaGoalChipOn } : s.eziaGoalChip}>
                  {toArabicDigits(n)}
                </button>
              ))}
            </div>
          {cats === null ? (
            ...
            <>
              <div className="ezia-featured">
                {featured.map((c) => (
                  <IstanaAdhkarFeature key={c.id} c={c} st={a3CatStanding(prog, c, byCat)} onOpen={onOpen} />
                ))}
              </div>
              {rest.length > 0 && (
                <div className="ezia-sec">
                  <span style={s.eziaSecTitle}>{A3_ALL}</span>
                  <span className="ezia-sec-rule" aria-hidden="true" />
                </div>
              )}
              <div className="ezia-catalogue">
                {rest.map((c, i) => (
                  <IstanaAdhkarCard key={c.id} c={c} i={i} st={a3CatStanding(prog, c, byCat)} onOpen={onOpen} />
                ))}
              </div>
            </>
          )}
...
function IstanaAdhkarReader(v) {
  ...
  const win = adhkarWindow(len, v.idx, A3_WINDOW);
  return (
    <div className="theme-dark adhkar3" style={s.eziaReadContainer}>
      {/* S103-adhkar-istana-reader */}
      <div className="ezia-nav">
        <div className="ezia-nav-inner">
          ...
          <button type="button" className="adhkar2-focus" onClick={v.onFav}
            ...
            style={v.isFav ? { ...s.eziaNavBtn, ...s.eziaNavBtnOn } : s.eziaNavBtn}>
            {v.isFav ? A2_ICON_HEART_ON : A2_ICON_HEART}
          </button>
        </div>
      </div>
          ...
          <div style={s.eziaReadScroll}>
            <div className="ezia-read-wrap">
              ...
              <div style={s.eziaRail} role="progressbar" aria-label={v.cat.title}
                ...
                {win.map((i) => {
                  const doneHere = adhkarCountOf(v.prog, v.cat.id, i) >= adhkarTarget(items[i]);
                  return <span key={i} style={i === v.idx ? { ...s.eziaRailMark, ...s.eziaRailMarkNow } : (doneHere ? { ...s.eziaRailMark, ...s.eziaRailMarkDone } : s.eziaRailMark)} />;
                })}
              </div>
              <div style={s.eziaNote} role="status" aria-live="polite">{v.note}</div>
              {/* REACHING THE TARGET IS AN EVENT, so it is said in words and not only by a
                  button changing colour. The strip appears when the count reaches the target and
                  offers the next dhikr beside it -- the SAME v.onNext the dock's own forward
                  control calls, so there is one way forward and not two. On the last item of a
      ...
      {items !== null && len > 0 && d && (
        <div style={s.eziaDock}>
          <div className="ezia-dock-inner">
            <button type="button" className="adhkar2-focus" onClick={v.onPrev} disabled={v.idx <= 0}
              style={v.idx <= 0 ? { ...s.eziaDockBtn, ...s.eziaDockBtnOff } : s.eziaDockBtn} aria-label={A2_PREV}>{A2_ICON_BACK}</button>
            <A3AudioBtn d={d} playing={v.playing} onAudio={v.onAudio} style={s.eziaDockBtn} styleOn={s.eziaDockBtnOn} />
            <button type="button" className="adhkar2-focus" onClick={v.onCount}
              style={v.full ? { ...s.eziaCountBtn, ...s.eziaCountBtnDone } : s.eziaCountBtn}
              aria-label={A2_COUNT_ARIA} aria-disabled={v.full ? 'true' : 'false'}>
              <span style={s.eziaCountNums}>{toArabicDigits(v.count)} / {toArabicDigits(v.target)}</span>
              <span style={s.eziaCountLabel}>{v.full ? A2_ICON_CHECK : A2_ICON_PLUS}<span>{v.full ? A2_DONE : A2_COUNT}</span></span>
            </button>
            <button type="button" className="adhkar2-focus" onClick={v.onShare} style={s.eziaDockBtn} aria-label={A2_SHARE}>{A2_ICON_SHARE}</button>
            <button type="button" className="adhkar2-focus" onClick={v.onNext} disabled={v.idx >= len - 1}
              style={v.idx >= len - 1 ? { ...s.eziaDockBtn, ...s.eziaDockBtnOff } : s.eziaDockBtn} aria-label={A2_NEXT}>{A2_ICON_ONWARD}</button>
          </div>
        </div>
      )}
    </div>
...
function AdhkarCategoryV2({ cat, startAt, onBack }) {
  // Session 85. THE LOGIC BELOW IS UNCHANGED and stays here: one loader, one audio element,
  ...
  const [idx, setIdx] = useState(0);                       // the item being read, 0-based
  const [prog, setProg] = useState(readAdhkarProgress);
  const [favs, setFavs] = useState(readAdhkarFavorites);
  ...
  useEffect(() => {
    let alive = true;
    // The opening position is CLAMPED to the real list: a saved favourite opens at its own
  ...
  // Moving between items stops what is playing: the recitation belongs to the dhikr on screen.
  const go = (n) => {
    if (!items || items.length === 0) return;
    const next = Math.min(items.length - 1, Math.max(0, n));
    if (next === idx) return;
    if (audioRef.current) { audioRef.current.pause(); audioRef.current = null; }
    setPlayingId(null);
    setNote('');
  ...
  // SHARING. Nothing is transmitted until this handler runs, and it runs only from the share
  ...
  const onShare = () => {
    const cur = (items && items[idx]) || null;
    const text = (cur && typeof cur.text === 'string') ? cur.text : '';
    if (!text) return;
    const fail = (e) => { if (e && e.name === 'AbortError') { setNote(''); return; } setNote(A2_SHARE_FAIL); };
    try {
      if (navigator.share) { navigator.share({ text: text }).then(() => setNote(A2_SHARE_OK)).catch(fail); return; }
      if (navigator.clipboard && navigator.clipboard.writeText) { navigator.clipboard.writeText(text).then(() => setNote(A2_COPY_OK)).catch(fail); return; }
    } catch (e) { setNote(A2_SHARE_FAIL); return; }
    setNote(A2_SHARE_FAIL);
  };
  const d = (items && items[idx]) || null;
  const target = adhkarTarget(d);
  // Clamped for DISPLAY as well as on write, so a stored count larger than a target that has
  ...
  // S103: one reader shell, and every protected action of the deleted two survives in it.
  return <IstanaAdhkarReader {...view} />;
}

// The audio control, shared by both readers so the contract cannot drift between them: it is
...
function A3AudioBtn({ d, playing, onAudio, style, styleOn }) {
  if (!d || !d.audio) return null;
  return (
    <button type="button" className="adhkar2-focus" onClick={onAudio}
      style={playing ? { ...style, ...styleOn } : style}
      aria-label={playing ? A2_STOP : A2_LISTEN}>
...
const histDepthRef = { current: 0 };
function ezikHistPush(state) {
  try { window.history.pushState(state, ''); histDepthRef.current++; return true; } catch (e) { return false; }
}
// A pop already happened in the browser: just stop counting the entry it spent. Never negative.
function ezikHistSpend() { if (histDepthRef.current > 0) histDepthRef.current--; }
// Ask the browser to go back, and ONLY while an entry of ours is on the stack -- so a back can
// never reach past the app's first entry and the platform's exit at the root is untouched.
function ezikHistBack() {
  if (histDepthRef.current <= 0) return false;
  try { window.history.back(); return true; } catch (e) { return false; }
}
// The one back the WHOLE app presses. Module scope so a nested layer's own visible back button
...
let ezikBackHandler = null;
function ezikGoBack() { return typeof ezikBackHandler === 'function' ? ezikBackHandler() : false; }
const ezikBackLayers = [];
function ezikCloseDeepestLayer() {
  for (let i = ezikBackLayers.length - 1; i >= 0; i--) {
    const close = ezikBackLayers[i];
    if (typeof close === 'function' && close() !== false) return true;
  }
  return false;
}
// The registration hook. `open` says whether this feature currently HAS a nested layer; while it
...
function useEzikBackLayer(open, close) {
  const closeRef = useRef(close);
  useEffect(() => { closeRef.current = close; });
  useEffect(() => {
    if (!open) return undefined;
    // S91: THE LAYER TAKES ITS ENTRY HERE, on the closed->open edge and nowhere else. `open` is a
    ...
    ezikHistPush({ layer: true });
    const entry = () => { const f = closeRef.current; if (typeof f !== 'function') return false; f(); return true; };
    ezikBackLayers.push(entry);
    return () => { const i = ezikBackLayers.indexOf(entry); if (i !== -1) ezikBackLayers.splice(i, 1); };
  }, [open]);
}
// THE DESTINATION TABLE, and it is the whole of the policy. It is a pure function of the screen
...
const EZIK_ROOT_SCREENS = ['loading', 'onboarding', 'chat'];
// S98: «المفضلة» joins the sheets. It is opened from the chat's own drawer through openEzikSheet,
...
const EZIK_SHEET_SCREENS = ['parentGate', 'parentDashboard', 'settings', 'favorites'];
function ezikBackTarget(cur, sheetOrigin) {
  if (EZIK_ROOT_SCREENS.indexOf(cur) !== -1) return null;
  // The call keeps its own documented exit contract: ending a call returns to the chat, and the
  // hardware button now agrees with the call screen's own button instead of contradicting it.
  if (cur === 'call') return 'chat';
  // Home is the root of the feature sections. Leaving home is the platform-level exit the app
  ...
  // A sheet returns to the screen that opened it, and to home when nothing valid was recorded.
  ...
  if (EZIK_SHEET_SCREENS.indexOf(cur) !== -1) {
    const known = (sheetOrigin === 'chat' || sheetOrigin === 'home' || sheetOrigin === 'settings');
    return (known && sheetOrigin !== cur) ? sheetOrigin : 'home';
  }
...
const EZIK_CHATS_KEY = 'ezik_chats_v1';
const EZIK_CHAT_PREFIX = 'ezik_chat_v1_';
const EZIK_CHAT_TITLE_MAX = 40;
const EZIK_CHATS_MAX = 60;          // oldest UNPINNED conversations are dropped past this
const EZIK_LEGACY_MSGS_KEY = 'messages';

// A random, opaque identifier — used for both conversation ids and profile ids. crypto when the
...
function ezikMintId() {
  try {
    ...
    let out = '';
    for (let i = 0; i < b.length; i++) out += (b[i] + 0x100).toString(16).slice(1);
    return out;
  } catch (e) {
...
// The profile identity a conversation is filed under. Minted ONCE and stored ON the profile
...
function ezikProfileKey(p) {
  return (p && typeof p.pid === 'string' && p.pid) ? p.pid : 'anon';
}

function ezikReadChatIndex() {
  let raw;
  try { raw = localStorage.getItem(EZIK_CHATS_KEY); } catch (e) { return []; }
  if (!raw) return [];
  let list;
...
function ezikWriteChatIndex(list) {
  try { localStorage.setItem(EZIK_CHATS_KEY, JSON.stringify(list)); } catch (e) {}
}

// THE ORDER THE MENU SHOWS, and the only place it is decided: pinned first, then the most
...
function ezikSortChats(list) {
  return list.slice().sort((a, b) =>
    ((b && b.pinned) ? 1 : 0) - ((a && a.pinned) ? 1 : 0) || ((b && b.at) || 0) - ((a && a.at) || 0));
}
function ezikListChats(pk) {
  return ezikSortChats(ezikReadChatIndex().filter((r) => r.pk === pk));
}

function ezikReadChatMessages(id) {
  let raw;
  try { raw = localStorage.getItem(EZIK_CHAT_PREFIX + id); } catch (e) { return []; }
  if (!raw) return [];
  try { const m = JSON.parse(raw); return Array.isArray(m) ? m : []; } catch (e) { return []; }
}

// The readable text of a message, whatever shape its content has. A plain turn is a string; an
...
// The title, and it is taken from the FIRST QUESTION the child asked — never from a reply, so a
...
function ezikChatTitle(msgs) {
  let first = null;
  for (let i = 0; i < (msgs || []).length; i++) {
    if (msgs[i] && msgs[i].role === 'user') { first = msgs[i]; break; }
  }
  let t = ezikMessageText(first).replace(/\s+/g, ' ').trim();
  if (!t) return 'محادثة';
  if (t.length > EZIK_CHAT_TITLE_MAX) {
    const cut = t.slice(0, EZIK_CHAT_TITLE_MAX);
    const sp = cut.lastIndexOf(' ');
    t = (sp > EZIK_CHAT_TITLE_MAX - 14 ? cut.slice(0, sp) : cut).trim() + '…';
  }
  return t;
}

// Past the cap, the oldest UNPINNED conversations are dropped — a pinned one is never dropped by
...
function ezikTrimChats(list) {
  if (list.length <= EZIK_CHATS_MAX) return list;
  const sorted = ezikSortChats(list);          // pinned first, so the head is never a pinned drop
  const head = sorted.slice(0, EZIK_CHATS_MAX);
  const tail = sorted.slice(EZIK_CHATS_MAX);
  const keptTail = tail.filter((r) => r.pinned);
  tail.filter((r) => !r.pinned).forEach((r) => {
    try { localStorage.removeItem(EZIK_CHAT_PREFIX + r.id); } catch (e) {}
  });
  return head.concat(keptTail);
}

// Writing the body, and the one place a quota failure is handled. A thread carrying a photo or a
...
function ezikWriteChatBody(id, msgs, idx) {
  const payload = JSON.stringify(msgs);
  const pool = ezikSortChats(idx.filter((r) => r.id !== id && !r.pinned));
  for (;;) {
    try { localStorage.setItem(EZIK_CHAT_PREFIX + id, payload); return true; } catch (e) {}
    const victim = pool.pop();                 // sorted newest-first, so pop() is the oldest
    if (!victim) return false;
    try { localStorage.removeItem(EZIK_CHAT_PREFIX + victim.id); } catch (e2) {}
    const i = idx.indexOf(victim);
    if (i !== -1) idx.splice(i, 1);
  }
}

// THE AUTOSAVE. Called after every turn the chat commits, and it is a NO-OP until the thread
...
function ezikSaveChat(id, msgs, pk) {
  const list = (msgs || []).filter(Boolean);
  let hasQuestion = false;
  for (let i = 0; i < list.length; i++) { if (list[i].role === 'user') { hasQuestion = true; break; } }
  if (!hasQuestion) return null;
  const cid = id || ezikMintId();
  const idx = ezikReadChatIndex();
  let prev = null;
  for (let i = 0; i < idx.length; i++) { if (idx[i].id === cid) { prev = idx[i]; break; } }
  // The title is minted once, from the first question, and a later turn never renames it.
  const rec = {
    id: cid,
    pk: pk,
    title: (prev && prev.title) ? prev.title : ezikChatTitle(list),
    pinned: !!(prev && prev.pinned),
    at: Date.now(),
  };
  const rest = idx.filter((r) => r.id !== cid);
  if (!ezikWriteChatBody(cid, list, rest)) return null;   // store refused: file nothing
  ezikWriteChatIndex(ezikTrimChats([rec].concat(rest)));
  return cid;
}
...
function ezikToggleChatPin(id) {
  ezikWriteChatIndex(ezikReadChatIndex().map((r) =>
    (r.id === id ? { id: r.id, pk: r.pk, title: r.title, pinned: !r.pinned, at: r.at } : r)));
}
// "Delete all my data" has to mean the history too — every body and the index with them.
function ezikClearAllChats() {
  ezikReadChatIndex().forEach((r) => { try { localStorage.removeItem(EZIK_CHAT_PREFIX + r.id); } catch (e) {} });
  try { localStorage.removeItem(EZIK_CHATS_KEY); } catch (e) {}
  try { localStorage.removeItem(EZIK_LEGACY_MSGS_KEY); } catch (e) {}
}

// THE PARENTS' LOG. Before this session the thread on screen WAS the child's whole record, so the
...
function ezikProfileTranscript(pk) {
  const rows = ezikReadChatIndex().filter((r) => r.pk === pk).sort((a, b) => ((a && a.at) || 0) - ((b && b.at) || 0));
  let out = [];
  for (let i = 0; i < rows.length; i++) out = out.concat(ezikReadChatMessages(rows[i].id));
  return out;
}

// ONE-SHOT migration of the single legacy thread. 'messages' held whatever the child was in the
...
function ezikMigrateLegacyThread(pk) {
  let raw;
  try { raw = localStorage.getItem(EZIK_LEGACY_MSGS_KEY); } catch (e) { return; }
  if (!raw) return;
  let msgs = null;
  try { msgs = JSON.parse(raw); } catch (e) { msgs = null; }
  if (Array.isArray(msgs) && msgs.length) ezikSaveChat(null, msgs, pk);
  try { localStorage.removeItem(EZIK_LEGACY_MSGS_KEY); } catch (e) {}
}
...
const EZIK_A11Y_KEY = 'ezik_reading_prefs_v1';
const EZIK_A11Y_DEFAULTS = { fontSize: 'normal', reading: false, reduceMotion: false };

function ezikReadA11yAll() {
  let raw;
  try { raw = localStorage.getItem(EZIK_A11Y_KEY); } catch (e) { return {}; }
  if (!raw) return {};
  let map;
...
// One profile's preferences, with every field validated. An unknown size, a non-boolean flag, a
...
function ezikReadA11y(pk) {
  const map = ezikReadA11yAll();
  const raw = map && map[pk];
  const out = { fontSize: EZIK_A11Y_DEFAULTS.fontSize, reading: false, reduceMotion: false };
  if (!raw || typeof raw !== 'object') return out;
  if (typeof raw.fontSize === 'string' && Object.prototype.hasOwnProperty.call(EZIK_FS_SCALES, raw.fontSize)) out.fontSize = raw.fontSize;
  out.reading = raw.reading === true;
  out.reduceMotion = raw.reduceMotion === true;
  return out;
}
// Writing returns whether the store actually took it, so the UI never claims a preference was
...
function ezikWriteA11y(pk, prefs) {
  if (!pk) return false;
  const map = ezikReadA11yAll();
  const isDefault = prefs.fontSize === EZIK_A11Y_DEFAULTS.fontSize && !prefs.reading && !prefs.reduceMotion;
  if (isDefault) delete map[pk];
  else map[pk] = { fontSize: prefs.fontSize, reading: !!prefs.reading, reduceMotion: !!prefs.reduceMotion };
  try { localStorage.setItem(EZIK_A11Y_KEY, JSON.stringify(map)); return true; } catch (e) { return false; }
}
function ezikClearAllA11y() { try { localStorage.removeItem(EZIK_A11Y_KEY); } catch (e) {} }

// THE ONE PLACE THE PREFERENCES BECOME VISIBLE. Three attributes and one custom property on the
...
let ezikLocalMotionPref = false;
function ezikApplyA11y(prefs) {
  const p = prefs || EZIK_A11Y_DEFAULTS;
  try {
    const el = document.documentElement;
    const scale = EZIK_FS_SCALES[p.fontSize] || 1;
    el.style.setProperty('--ez-fs', String(scale));
    el.setAttribute('data-ez-fs', p.fontSize || 'normal');
    if (p.reading) el.setAttribute('data-ez-read', '1'); else el.removeAttribute('data-ez-read');
    // EITHER SOURCE, ONE ATTRIBUTE. `data-ez-motion` is what every reduced-motion rule reads,
    ...
    ezikLocalMotionPref = !!p.reduceMotion;
    if (ezikMotionReduced(p.reduceMotion)) el.setAttribute('data-ez-motion', 'reduce');
    else el.removeAttribute('data-ez-motion');
  } catch (e) {}
}
// THE OS CAN CHANGE ITS MIND WHILE THE APP IS OPEN. The rules read the attribute, so the
...
try {
  const ezMq = window.matchMedia('(prefers-reduced-motion: reduce)');
  const ezSyncMotion = () => {
    try {
      const el = document.documentElement;
      if (ezikMotionReduced(ezikLocalMotionPref)) el.setAttribute('data-ez-motion', 'reduce');
      else el.removeAttribute('data-ez-motion');
    } catch (e) {}
  };
  if (ezMq.addEventListener) ezMq.addEventListener('change', ezSyncMotion);
  else if (ezMq.addListener) ezMq.addListener(ezSyncMotion);
} catch (e) {}
// Does the app owe this user stillness? EITHER the local preference OR the platform one. The
...
function ezikMotionReduced(localPref) {
  if (localPref) return true;
  try { return !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches); }
  catch (e) { return false; }
}
// The same question for a component that holds no preference of its own. ezikApplyA11y has already
...
function ezikMotionReducedNow() {
  try { if (document.documentElement.getAttribute('data-ez-motion') === 'reduce') return true; } catch (e) {}
  return ezikMotionReduced(false);
}

let EZIK_A11Y_TITLE = ezT("a11y.title");
let EZIK_A11Y_FS_LABEL = ezT("a11y.fontSize");
let EZIK_A11Y_FS_NORMAL = ezT("a11y.fontNormal");
let EZIK_A11Y_FS_LARGE = ezT("a11y.fontLarge");
let EZIK_A11Y_FS_XLARGE = ezT("a11y.fontXLarge");
let EZIK_A11Y_READ = ezT("a11y.reading");
let EZIK_A11Y_READ_HINT = ezT("a11y.readingHint");
let EZIK_A11Y_MOTION = ezT("a11y.motion");
let EZIK_A11Y_MOTION_HINT = ezT("a11y.motionHint");
let EZIK_A11Y_RESET = ezT("a11y.reset");
...
const EZIK_FAV_KINDS = ['reply', 'fatwa', 'ayah'];
const EZIK_FAV_KIND_DEFAULT = 'reply';
const ezikFavKind = (k) => (EZIK_FAV_KINDS.indexOf(k) === -1 ? EZIK_FAV_KIND_DEFAULT : k);
// THE kind of a record, and the only place that decides it. A record with no kind is a reply --
// which is every reply ever saved, before this item and after it.
const ezikKindOf = (f) => ezikFavKind(f && f.kind);
const EZIK_FAVS_EVENT = 'ezik:favourites';

const EZIK_FAVS_KEY = 'ezik_favorite_replies_v1';
const EZIK_FAVS_MAX = 200;
const EZIK_FAV_SNIPPET_MAX = 140;

// FNV-1a over a string. Cheap, stable across reloads and devices, and it needs no store of its own.
function ezikHashText(t) {
  ...
  return 'r' + h.toString(16) + '-' + str.length.toString(36);
}

// THE IDENTITY OF A SAVED REPLY, and it is a POSITION rather than a text.
//
...
function ezikFavId(pk, chatId, idx, text) {
  return String(pk || '') + '|' + String(chatId || '-') + '|' + String(idx) + '|' + ezikHashText(text);
}

// THE IDENTITY OF A SAVED FATWA OR AYAH. Not a position -- neither has one -- but the thing
...
const ezikAyahFavKey = (surahNum, surahName, ayah) => {
  const sn = resolveSurahNumber(surahName, surahNum);
  const an = parseInt(ayah, 10);
  return (sn && an >= 1) ? (String(sn) + ':' + String(an)) : '';
};

function ezikFavRefId(kind, pk, ref, text) {
  return String(pk || '') + '|' + ezikFavKind(kind) + '|' + String(ref || '-') + '|' + ezikHashText(text);
}

// EVERY read is defensive, because this is a file on a device: it can be absent, it can be
...
function ezikReadFavs() {
  let raw;
  try { raw = localStorage.getItem(EZIK_FAVS_KEY); } catch (e) { return []; }
  if (!raw) return [];
  let list;
  try { list = JSON.parse(raw); } catch (e) { return []; }
  if (!Array.isArray(list)) return [];
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!text) continue;                                  // a record with no reply in it is not one
    ...
    const rec = {
      id: r.id,
      pk: r.pk,
      chatId: (typeof r.chatId === 'string' && r.chatId) ? r.chatId : null,
      idx: (typeof r.idx === 'number' && isFinite(r.idx)) ? r.idx : -1,
      at: (typeof r.at === 'number' && isFinite(r.at)) ? r.at : 0,
      snippet: (typeof r.snippet === 'string' && r.snippet) ? r.snippet : ezikFavSnippet(text),
      text: text,
    };
    // ONLY IF THE RECORD HAS THEM. A reply has no kind, no title and no ref and never did;
    ...
    if (EZIK_FAV_KINDS.indexOf(r.kind) !== -1 && r.kind !== EZIK_FAV_KIND_DEFAULT) rec.kind = r.kind;
    if (typeof r.title === 'string' && r.title) rec.title = r.title;
    if (typeof r.ref === 'string' && r.ref) rec.ref = r.ref;
    out.push(rec);
  }
  return out;
}

// Writing, and the one place a full store is handled. The list is newest-first, so the oldest
...
function ezikWriteFavs(list) {
  let l = (list || []).slice(0, EZIK_FAVS_MAX);
  for (;;) {
    try {
      localStorage.setItem(EZIK_FAVS_KEY, JSON.stringify(l));
      // ANNOUNCED. A fatwa is saved from a screen the chat does not own, and the chat is NOT
      ...
      try { window.dispatchEvent(new CustomEvent(EZIK_FAVS_EVENT)); } catch (e2) {}
      return l;
    } catch (e) {}
...
// The list line. Built from the SAME serializer the clipboard uses, so no card tag can reach it.
function ezikFavSnippet(text) {
  let clean = '';
  try {
    const parsed = parseRichMessage(String(text == null ? '' : text), 30);
    clean = serializeReply(parsed.segments, { tashkeel: true, band: 'adult' });
  } catch (e) { clean = String(text == null ? '' : text); }
  clean = clean.replace(/\s+/g, ' ').trim();
  if (clean.length > EZIK_FAV_SNIPPET_MAX) {
    clean = clean.slice(0, EZIK_FAV_SNIPPET_MAX).replace(/\s+\S*$/, '').replace(/\s+$/, '') + ' …';
  }
  return clean;
}

function ezikMakeFav(text, pk, chatId, idx) {
  const t = String(text == null ? '' : text);
  return { id: ezikFavId(pk, chatId, idx, t), pk: pk || null, chatId: chatId || null, idx: idx, at: Date.now(), snippet: ezikFavSnippet(t), text: t };
}

// A FATWA OR AN AYAH, made the same way and stored in the same list. `title` is what the card
...
function ezikMakeFavOf(kind, pk, ref, title, text) {
  const t = String(text == null ? '' : text);
  return { id: ezikFavRefId(kind, pk, ref, t), kind: ezikFavKind(kind), pk: pk || null,
    chatId: null, idx: -1, at: Date.now(), snippet: ezikFavSnippet(t), text: t,
    title: String(title == null ? '' : title), ref: String(ref == null ? '' : ref) };
}

// THE TOGGLE ANY SCREEN CAN CALL. الفتاوى does not own the chat's state and must not be given
...
function ezikToggleFavRecord(rec) {
  if (!rec || !rec.id || !rec.pk || !rec.text) return false;
  const cur = ezikReadFavs();
  const exists = cur.some((f) => f.id === rec.id);
  const next = exists ? cur.filter((f) => f.id !== rec.id) : [rec].concat(cur.filter((f) => f.id !== rec.id));
  const written = ezikWriteFavs(next);
  if (written === null) return exists;            // the store refused: nothing changed
  return !exists;
}

// "Delete all my data" has to mean the favourites too.
function ezikClearAllFavs() { try { localStorage.removeItem(EZIK_FAVS_KEY); } catch (e) {} }

// The labels, in one block, referenced through JS expressions rather than written into quoted JSX
// attributes — the convention SettingsSheet already follows for the same reason.
let EZIK_FAV_TITLE = ezT("favorites.title");
let EZIK_FAV_HEADING = ezT("favorites.heading");
let EZIK_FAV_ADD = ezT("favorites.add");
let EZIK_FAV_DEL = ezT("favorites.remove");
let EZIK_FAV_REMOVE = ezT("common.remove");
let EZIK_FAV_OPEN_CHAT = ezT("favorites.openChat");
let EZIK_FAV_CHAT_GONE = ezT("favorites.chatGone");
const EZIK_FAV_EMPTY = 'لا توجد ردود محفوظة بعد. اضغط النجمة تحت أي رد لتحفظه هنا.';
let EZIK_BACK = ezT("common.backArrow");
let EZIK_SEARCH_PH = ezT("chat.searchPlaceholder");
let EZIK_SEARCH_ARIA = ezT("chat.searchAria");
let EZIK_SEARCH_RESULTS = ezT("chat.searchResults");
let EZIK_SEARCH_NONE = ezT("common.noMatches");
let EZIK_FAV_SEARCH_PH = ezT("favorites.searchPlaceholder");
let EZIK_FAV_SEARCH_ARIA = ezT("favorites.searchAria");
// S112: what the menu says when this profile has saved nothing yet. It is a STATEMENT about the
// store, drawn where the list would be; it reads nothing, writes nothing and offers no action.
let EZIK_CHATS_EMPTY = ezT("chat.noConversations");

// The saved date, rendered from the record's own timestamp. Arabic-Kuwait with a Gregorian
// calendar, and a record whose `at` was lost simply shows no date rather than 1970.
function ezikFavDate(at) {
  if (!at) return '';
  try { return new Date(at).toLocaleDateString('ar-KW', { year: 'numeric', month: 'long', day: 'numeric' }); }
  catch (e) { try { return new Date(at).toISOString().slice(0, 10); } catch (e2) { return ''; } }
}
// The compact form, for the dense search-results list where a spelled-out month wraps.
function ezikShortDate(at) {
  if (!at) return '';
  try { return new Date(at).toLocaleDateString('ar-KW', { year: 'numeric', month: 'numeric', day: 'numeric' }); }
  catch (e) { return ezikFavDate(at); }
}
...
function ezikSearchPlain(t) {
  return String(t == null ? '' : t)
    .replace(EZIK_CARD_TAG_RE, ' ')
    .replace(/<[^>\n]{0,160}>/g, ' ')
    .replace(/[ \t ]+/g, ' ');
}

// One pass that produces BOTH the normalized text and, for each of its characters, the index it
...
function ezikSearchNormalize(src) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    // 064B..0652 harakat · 0670 dagger alef · 0640 tatweel · 0653..065F extra marks · 06D6..06ED
    if ((code >= 0x064B && code <= 0x065F) || code === 0x0670 || code === 0x0640
      || (code >= 0x06D6 && code <= 0x06ED)) continue;
    let ch = s.charAt(i);
    if (ch === ' ' || ch === '\t' || ch === '\n' || ch === '\r' || ch === ' ') {
      ...
      prevSpace = false;
      if (ch === 'آ' || ch === 'أ' || ch === 'إ' || ch === 'ٱ') ch = 'ا';
      else if (ch === 'ى') ch = 'ي';
      else if (code >= 0x41 && code <= 0x5A) ch = ch.toLowerCase();
    }
    out += ch;
    map.push(i);
  }
...
const ezikSearchQuery = (q) => ezikSearchNormalize(q).text.trim();

// The line under a result. Cut from the ORIGINAL text around the hit, walked out to word
// boundaries so it never starts or ends mid-word, and marked with ellipses where it was cut.
function ezikSearchSnippet(original, map, at, qLen) {
  const src = String(original == null ? '' : original);
  if (!src) return '';
  if (at < 0 || !map.length) return src.slice(0, EZIK_SNIPPET_RADIUS * 2).replace(/\s+/g, ' ').trim();
  const oStart = map[Math.min(map.length - 1, Math.max(0, at))];
  const oEnd = map[Math.min(map.length - 1, Math.max(0, at + qLen - 1))] + 1;
  let from = Math.max(0, oStart - EZIK_SNIPPET_RADIUS);
  let to = Math.min(src.length, oEnd + EZIK_SNIPPET_RADIUS);
  ...
  if (from > 0) out = '… ' + out;
  if (to < src.length) out = out + ' …';
  return out;
}

// One conversation, made searchable. It reads the body the store already holds; it writes nothing.
function ezikBuildSearchRow(rec) {
  const msgs = ezikReadChatMessages(rec.id);
  const parts = [];
  for (let i = 0; i < msgs.length; i++) {
    const t = ezikSearchPlain(ezikMessageText(msgs[i])).trim();
    if (t) parts.push(t);
  }
  const body = parts.join('\n');
  const norm = ezikSearchNormalize(body);
  return {
    id: rec.id, title: rec.title, at: rec.at, pinned: rec.pinned,
    body: body, hay: norm.text, map: norm.map,
    titleHay: ezikSearchNormalize(rec.title || '').text,
  };
}

// The match. Order is the store's own — pinned first, then most recent — because the rows arrive
// already sorted and nothing here re-ranks them.
function ezikSearchChats(rows, q) {
  const out = [];
  if (!q) return out;
  for (let i = 0; i < (rows || []).length; i++) {
    const r = rows[i];
    const inTitle = r.titleHay.indexOf(q);
    const at = r.hay.indexOf(q);
    if (inTitle === -1 && at === -1) continue;
    out.push({
      id: r.id,
      title: r.title,
      at: r.at,
      pinned: r.pinned,
      snippet: at !== -1 ? ezikSearchSnippet(r.body, r.map, at, q.length) : ezikSearchSnippet(r.body, r.map, 0, 1),
    });
  }
  return out;
}

// The same match over the favourites already in memory. No store is touched at all here.
function ezikSearchFavs(favs, q) {
  const out = [];
  if (!q) return out;
  for (let i = 0; i < (favs || []).length; i++) {
    const f = favs[i];
    const plain = ezikSearchPlain(f.text);
    const norm = ezikSearchNormalize(plain);
    const at = norm.text.indexOf(q);
    if (at === -1) continue;
    out.push(Object.assign({}, f, { hit: ezikSearchSnippet(plain, norm.map, at, q.length) }));
  }
  return out;
}
...
function App() {
  ...
  // THE COMPOSER'S FOCUS, and it exists for exactly one reason: the section suggestions below
  ...
  // S98: THE FAVOURITES, READ ONCE. This is the whole of the performance contract for the feature:
  ...
  const [favs, setFavs] = useState(ezikReadFavs);
  const favsRef = useRef(favs);
  // S98: WHICH REPLIES ARRIVED LIVE IN THIS THREAD, by position. A long reply is watched in full
  ...
  const [a11y, setA11yState] = useState(EZIK_A11Y_DEFAULTS);
  const a11yRef = useRef(a11y);
  const setA11y = (next) => {
    const merged = Object.assign({}, a11yRef.current, next);
    // The first time a larger size is chosen, the styles object becomes scalable. From then on the
    // multiplier alone moves it, so switching between large and كبير جدًّا costs nothing more.
    if (merged.fontSize !== 'normal') ezikEnsureScalableStyles();
    const pk = ezikProfileKey(profileRef.current || profile);
    const stored = ezikWriteA11y(pk, merged);
    // A refused store must not leave the screen claiming a preference was saved. It IS applied for
    ...
    setA11yState(merged);
    ezikApplyA11y(merged);
    return stored;
  };
  const resetA11y = () => setA11y(EZIK_A11Y_DEFAULTS);
  const [streamedOpen, setStreamedOpen] = useState(() => new Set());
  // WHICH THREAD IS ON SCREEN, as a number that changes every time the thread is REPLACED — a
  ...
  const newThreadEpoch = () => setThreadEpoch((n) => n + 1);
  const markStreamedOpen = (idx) => setStreamedOpen((prev) => {
    if (prev.has(idx)) return prev;
    const next = new Set(prev);
    next.add(idx);
    return next;
  });
  // D88: seeded from the SAME reader the boot script used, so the first render already agrees
  ...
  const [reportFor, setReportFor] = useState(null); // {ai,user} of the reply being flagged, or null
  ...
  const [aiConsent, setAiConsent] = useState(() => aiConsentStatus());
  const [aiConsentReview, setAiConsentReview] = useState(false);   // "review the choice" reopens the screen
  ...
  const grantAIConsent = (by) => { writeAIConsent(EZ_AI_CONSENT_GRANTED, by); setAiConsent(aiConsentStatus()); setAiConsentReview(false); };
  // Declining, and withdrawing, are the SAME write -- there is one refused state, not two. The
  ...
  const declineAIConsent = () => {
    writeAIConsent(EZ_AI_CONSENT_DECLINED, 'user');
    try { if (abortRef.current) abortRef.current.abort(); } catch (e) {}
    try { cancelAudio(); } catch (e) {}
    try { stopCloudAll(); } catch (e) {}
    try { stopDictAll(); } catch (e) {}
    // EVERY live Web Speech engine, wherever it was built -- the chat's, the call's, and the
    ...
    try { ezStopAllRecognition(); } catch (e) {}
    try { shouldListenRef.current = false; } catch (e) {}
    try { callActiveRef.current = false; callGenRef.current++; } catch (e) {}
    setIsListening(false);
    setIsLoading(false);
    setCallState('idle');
    setAiConsent(aiConsentStatus());
    setAiConsentReview(false);
  };
  // قفل الإنفاق: مفتوحٌ دائماً حين يكون القفل مُعطَّلاً (٦٤ صفراً) فلا يتغيّر سلوك اليوم إطلاقاً.
  // spendGateRef مرآةٌ متزامنة يقرؤها callAI/fetchSpeechAudio دون إغلاقٍ على قيمةٍ بائتة.
  const [spendGateOpenState, setSpendGateOpenState] = useState(spendGateOpen());
  const spendGateRef = useRef(spendGateOpenState);
  useEffect(() => { spendGateRef.current = spendGateOpenState; }, [spendGateOpenState]);
  const unlockSpendGate = () => { try { localStorage.setItem(SPEND_GATE_KEY, SPEND_GATE_SHA256); } catch (e) {} setSpendGateOpenState(true); };
  const [voiceMode, setVoiceMode] = useState(false); // chat TTS OFF by default (opt-in): silent unless the user taps a message’s listen button or turns on the header speaker toggle
  ...
  const [directConvoLocked, setDirectConvoLocked] = useState(localStorage.getItem('directConvoLocked') === 'true');
  const [isListening, setIsListening] = useState(false);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [voiceError, setVoiceError] = useState('');
  const messagesEndRef = useRef(null);
  // S97: the scroll CONTAINER, which nothing used to hold a handle on -- the only scrolling the
  // chat did was scrollIntoView on the sentinel above, and that cannot be made instant-before-paint.
  const messagesAreaRef = useRef(null);
  // true from the moment a STORED conversation is loaded until its height has settled. While it is
  ...
  // false while the user has deliberately scrolled up to read something older. Nothing may drag
  // them back down while it is false.
  const stickToEndRef = useRef(true);
  // How many upcoming runs of the follow-the-reply effect belong to an OPEN rather than to a new
  ...
  const skipFollowRef = useRef(0);
  // ── STREAM-P4 §٣/١: THE QUESTION STAYS AT THE TOP ─────────────────────────
  ...
  const [pinnedAskIndex, setPinnedAskIndex] = useState(null);
  const pinAnchorRef = useRef(null);
  // Live mirror of the state above, for the scroll handlers and the follow effect: those run
  // outside React's render and must not read a value a re-render has not delivered yet.
  const pinActiveRef = useRef(false);
  // Circuit breaker for a device geometry whose rendered spacer never converges. It is reset
  // for every turn and bounds the number of state-writing layout passes that turn may make.
  const pinPassRef = useRef(0);
  // ITEM 102-ب. The streamed length the PREVIOUS pin pass measured, so a pass can tell «the
  ...
  const askPadElRef = useRef(null);
  // The last scroll position the pin itself wrote, read back from the element. Anything else is
  // the reader's hand. -1 means the pin has not placed anything yet this turn.
  const pinScrollTopRef = useRef(-1);
  // ── STREAM-P4 §٣/٢: `Enter` DOES NOT SEND ON A PHONE ──────────────────────
  ...
  // WHETHER THE COMPOSER IS BEING DRIVEN BY TOUCH, and «التمييزُ بنوعِ الإدخالِ لا بعرضِ
  ...
  const [inputIsTouch, setInputIsTouch] = useState(() => {
    try { return !!(window.matchMedia && window.matchMedia('(pointer: coarse)').matches); }
    catch (e) { return false; }
  });
  // The stored choice, read at mount. الإعدادات unmounts this tree while it is on screen, so
  ...
  const enterSends = ezikEnterSends(enterPref, inputIsTouch);
  const [wmAutoHide, setWmAutoHide] = useState(readWatermarkAutoHide);
  useEffect(() => {
    const onWm = () => setWmAutoHide(readWatermarkAutoHide());
    try { window.addEventListener(EZIK_WM_HIDE_EVENT, onWm); } catch (e) {}
    return () => { try { window.removeEventListener(EZIK_WM_HIDE_EVENT, onWm); } catch (e) {} };
  }, []);
  useEffect(() => {
    const onPref = () => setEnterPref(readEnterPref());
    try { window.addEventListener(EZIK_ENTER_EVENT, onPref); } catch (e) {}
    return () => { try { window.removeEventListener(EZIK_ENTER_EVENT, onPref); } catch (e) {} };
  }, []);
  ...
  const setStreamingText = (next) => {
    if (typeof next !== 'string' || next === '') {
      // null (retire / abort) and '' (arm): both empty the queue and paint immediately. This is
    ...
    // A cursor can never sit past what has arrived. It cannot happen with an append-only
    ...
    if (revealAtRef.current > next.length) revealAtRef.current = next.length;
  ...
  const [founderUnlocked, setFounderUnlocked] = useState(() => hasFounderToken());
  const [tashkeelOn, setTashkeelOn] = useState(() => { try { return localStorage.getItem('tashkeel_v1') === '1'; } catch (e) { return false; } });
  const toggleTashkeel = () => setTashkeelOn((v) => { const n = !v; try { localStorage.setItem('tashkeel_v1', n ? '1' : '0'); } catch (e) {} return n; });
  const CALL_RESTART_GRACE_MS = 3000; // deaf window BETWEEN SR sessions - must never count as user silence
  ...
  const sttErrorMessage = (status) => {
    if (status === 429) return '⏳ تجاوزنا حدّ الاستماع المسموح الآن. انتظر قليلاً ثم أعد المحاولة.';
    if (status === 413) return '🎤 المقطع طويل جدًّا. تكلّم بمقاطع أقصر.';
    if (status === 403) return '🚫 خدمة تحويل الكلام غير متاحة لهذا الحساب.';
    if (status >= 500) return '🛠️ خدمة تحويل الكلام متوقّفة مؤقّتًا. حاول بعد قليل.';
    return '🛠️ تعذّر تحويل كلامك إلى نصّ (رمز ' + status + '). حاول مرّة أخرى.';
  };
  const cloudStreamRef = useRef(null);
  ...
  const pickRecMime = () => {
    ...
    for (let i = 0; i < cands.length; i++) {
      try { if (window.MediaRecorder && MediaRecorder.isTypeSupported(cands[i])) return { mimeType: cands[i] }; } catch (e) {}
    }
    return {};
  };
  const blobToBase64 = (blob) => new Promise((resolve) => {
    const fr = new FileReader();
    fr.onloadend = () => { const s = String(fr.result || ''); const i = s.indexOf(','); resolve(i >= 0 ? s.slice(i + 1) : ''); };
    fr.onerror = () => resolve('');
  ...
  const stopCloudAll = () => {
    ...
    try { cloudStreamRef.current?.getTracks().forEach((tr) => tr.stop()); } catch (e) {}
  ...
  const startCloudListening = async () => {
    ...
    try {
      ...
      const mr = new MediaRecorder(cloudStreamRef.current, pickRecMime());
      mr.ondataavailable = (e) => { if (e.data && e.data.size) cloudChunksRef.current.push(e.data); };
      ...
      if (!vadCtxRef.current) {
        ...
        vadCtxRef.current = new Ctx();
        vadAnalyserRef.current = vadCtxRef.current.createAnalyser();
      ...
      // The microphone never opened. A SILENT fall to 'idle' here was the whole bug: on a denied
      // permission the ring simply stopped and the call looked like it was working but deaf.
      console.error('[Al-Murabbi] call mic error:', e);
  ...
  const stopCloudTurn = async () => {
    ...
    if (mr && mr.state !== 'inactive') {
      blob = await new Promise((resolve) => {
        mr.onstop = () => { try { resolve(new Blob(cloudChunksRef.current, { type: mr.mimeType || 'audio/webm' })); } catch (e) { resolve(null); } };
    ...
    let sttError = '';
    try {
      const band = (() => { try { return deriveCaps(profileRef.current?.age).band; } catch (e) { return 'young'; } })();
      const r = await aiFetch('/api/stt', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ audio: b64, mime: blob.type, band }) });
      if (r.ok) { const d = await r.json(); text = String(d.text || '').trim(); }
      else {
        const body = await r.text().catch(() => '');
        console.error('[Al-Murabbi] /api/stt ' + r.status + ':', body.slice(0, 300));
        sttError = sttErrorMessage(r.status);
      }
    } catch (e) {
      console.error('[Al-Murabbi] /api/stt network error:', e);
      sttError = '📡 تعذّر الوصول إلى خدمة تحويل الكلام. تحقّق من الاتصال ثم أعد المحاولة.';
    }
    if (callGenRef.current !== myGen) { stopCloudAll(); return; }
    if (sttError) {
      // A FAILED transcription is not silence, so it must not be answered with more listening.
  ...
  const VAD_SILENCE_MS = 1200;           // real acoustic silence (ms) that ends a turn
  ...
  const vadGenRef = useRef(-1);
  const vadLastVoiceRef = useRef(0);
  const stopVad = () => {
    vadOkRef.current = false;
    try { vadStreamRef.current?.getTracks().forEach((tr) => tr.stop()); } catch (e) {}
    try { vadCtxRef.current?.close(); } catch (e) {}
    vadStreamRef.current = null;
    vadCtxRef.current = null;
  };
  // The Android recognizer is DEAF between sessions, so SR events cannot measure user silence.
  // This loop reads microphone energy directly and is the SOLE turn-ender while it lives.
  const ensureVad = async () => {
    // A microphone is a microphone even when nothing is transcribed from it: this loop opens one
    ...
    if (!hasValidAIConsent()) { stopVad(); return; }
    if (vadCtxRef.current) { vadGenRef.current = callGenRef.current; vadLastVoiceRef.current = Date.now(); return; }
    try {
      ...
      vadGenRef.current = callGenRef.current;
      vadLastVoiceRef.current = Date.now();
      vadOkRef.current = true;
      const buf = new Uint8Array(an.fftSize);
      const tick = () => {
        if (!vadCtxRef.current) return;
        if (vadGenRef.current !== callGenRef.current) { stopVad(); return; }
        an.getByteTimeDomainData(buf);
        let sum = 0;
        for (let i = 0; i < buf.length; i++) { const v = (buf[i] - 128) / 128; sum += v * v; }
        const now = Date.now();
        if (Math.sqrt(sum / buf.length) > VAD_RMS_ON) {
          vadLastVoiceRef.current = now;
          if (callActiveRef.current) armInactivityTimer();
        } else if (callActiveRef.current && (now - vadLastVoiceRef.current) > VAD_SILENCE_MS && (callBaseTextRef.current + callTranscriptRef.current).trim()) {
          endCallTurnNow();
        }
        setTimeout(tick, 100);
      };
      tick();
    } catch (e) { vadOkRef.current = false; }
  };
  // Layer 3 (continuous call) — anti-leak + auto-end machinery.
  ...
  useEffect(() => {
    try {
      // مؤقت: التخزين مُعطَّل — تصرّف كأنه تثبيت جديد في كل فتح: امسح الملف والمحادثة معاً،
      ...
      const profileData = localStorage.getItem('child_profile');
      if (profileData) {
        ...
        // S92 -- one-shot migration, beside the birthYear one above: a profile created before
        ...
        setProfile(p);
        // S92: the app ALWAYS opens on a NEW, EMPTY thread. What the child wrote before is not
        ...
        ezikMigrateLegacyThread(ezikProfileKey(p));
        // S99: this profile's reading preferences, read once here — the same single read the boot
        ...
        {
          const prefs = ezikReadA11y(ezikProfileKey(p));
          a11yRef.current = prefs;
          setA11yState(prefs);
          ezikApplyA11y(prefs);
        }
  ...
  // تحميل المصحف الكنسي مسبقاً بعد الإقلاع كي تظهر أول آية فوراً (يُتجاهَل الفشل بهدوء)
  useEffect(() => {
    // S117 PERF. The prefetch stays; it no longer races the first paint. Measured cold against
    ...
    const go = () => { loadQuran().catch(() => {}); };
    if (typeof requestIdleCallback === 'function') {
      const h = requestIdleCallback(go, { timeout: 3000 });
      return () => { try { cancelIdleCallback(h); } catch (e) {} };
    }
    const t = setTimeout(go, 1200);
    return () => clearTimeout(t);
  }, []);

  // زر الرجوع (أندرويد/المتصفح) — خطأ ١٥: بلا هذا يُغلق التطبيق فوراً من أي شاشة فرعية.
  ...
  // The nested layer, if any, and it is asked FIRST by both back paths. The registry answers for
  ...
  const closeEzikNested = () => {
    if (ezikCloseDeepestLayer()) return true;
    if (screenRef.current === 'mushaf' && selectedSurahRef.current != null) { setSelectedSurah(null); return true; }
    return false;
  };

  // ===== goEzikBack -- THE ONE APPLICATION BACK RESOLVER =====
  ...
  const resolveEzikBack = (viaPop) => {
    const cur = screenRef.current;
    // S92: the layer registry is asked FIRST on EVERY screen, root or not -- the `!rooted` guard
    ...
    if (closeEzikNested()) {
      // S91: the entry the pop just spent BELONGED to the layer that has now closed, so there is
    ...
    // 2. The screen itself leaves, to its own parent from the table -- never to the chat by default.
    const dest = ezikBackDestination(cur);
    if (!dest || dest === cur) return false;
    if (EZIK_SHEET_SCREENS.indexOf(cur) !== -1) sheetOriginRef.current.pop();   // spend this sheet's opener, not another's
  ...
  // The destination this screen's back resolves to, read without spending anything -- both the
  // resolver above and the visible-press path below ask it, so they cannot disagree.
  const ezikBackDestination = (cur) => {
    const origins = sheetOriginRef.current;
    const isSheet = EZIK_SHEET_SCREENS.indexOf(cur) !== -1;
    return ezikBackTarget(cur, isSheet ? origins[origins.length - 1] : null);
  };
  // ===== goEzikBack -- WHAT EVERY VISIBLE BACK CONTROL PRESSES =====
  ...
  const goEzikBack = () => {
    const cur = screenRef.current;
    if (EZIK_ROOT_SCREENS.indexOf(cur) === -1) {
      // S91: a REGISTERED layer owns an entry, so its visible back takes the same route the
      ...
      if (ezikBackLayers.length > 0 && ezikHistBack()) return true;
      if (closeEzikNested()) return true;
    }
    const dest = ezikBackDestination(cur);
    if (!dest || dest === cur) return false;
    if (ezikHistBack()) return true;
    return resolveEzikBack(false);
  };
  // Sheets remember their opener so their back returns THERE: home when home opened them, the
  ...
  const openEzikSheet = (target) => { sheetOriginRef.current.push(screenRef.current); setScreen(target); };
  // S90: the PIN gate and the parental screen behind it are ONE layer, not two -- passing the
  ...
  useEzikBackLayer(drawerOpen, () => {
    setDrawerOpen(false);
    setChatPendingDelete(null);        // a half-asked delete never survives the menu closing
    const run = drawerNavRef.current;
  ...
  useEffect(() => {
    ...
    const rooted = EZIK_ROOT_SCREENS.indexOf(screen) !== -1;
    if (histReplaceRef.current) {
      histReplaceRef.current = false;
      if (!rooted) { try { window.history.replaceState({ screen }, ''); } catch (e) {} }
      return;
    }
    if (!rooted) ezikHistPush({ screen });
  }, [screen]);

  // Stop audio when leaving a screen. App never unmounts on a screen switch, so playing
  ...
  useEffect(() => {
    ...
    return () => {
      if (leaving === 'chat') { try { abortRef.current?.abort(); } catch (e) {} }
      cancelAudio();
    };
  }, [screen]);

  // Mobile autoplay fix: with chat voice ON by default, the first spoken reply (the greeting) plays
  ...
  const unlockAudioPlayback = () => {
    if (audioUnlockedRef.current) return; // one-time only
    ...
    try {
      ...
      if (Ctx) {
        ...
        // A 1-sample silent buffer started within the gesture satisfies the user-activation requirement.
        const src = ctx.createBufferSource();
        src.buffer = ctx.createBuffer(1, 1, 22050);
        src.connect(ctx.destination);
        src.start(0);
      }
    } catch (e) { /* fail silent — never block onboarding or audio */ }
    // iOS: HTML5 <audio> has a SEPARATE activation gate from AudioContext. The blob-play path
    ...
    try {
      const wav = new Uint8Array([82,73,70,70,38,0,0,0,87,65,86,69,102,109,116,32,16,0,0,0,1,0,1,0,64,31,0,0,128,62,0,0,2,0,16,0,100,97,116,97,2,0,0,0,0,0]);
      const url = URL.createObjectURL(new Blob([wav], { type: 'audio/wav' }));
      const u = audioElRef.current || (audioElRef.current = new Audio()); u.src = url;
      const pr = u.play();
      if (pr && pr.then) pr.then(() => { try { u.pause(); } catch (e) {} URL.revokeObjectURL(url); }).catch(() => { try { URL.revokeObjectURL(url); } catch (e) {} audioUnlockedRef.current = false; }); // play() rejected -> not actually unlocked; allow a later gesture to retry
    } catch (e) { audioUnlockedRef.current = false; /* prime threw -> allow a later gesture to retry */ }
  };

  useEffect(() => {
    const handler = () => { unlockAudioPlayback(); };
    const opts = { capture: true };
    // Stay armed: after a real success the ref guard makes each call a no-op; before success we retry on every gesture.
  ...
  useEffect(() => {
    const recognition = ezNewRecognition();
    ...
    recognition.onresult = (event) => {
      ...
      for (let i = 0; i < event.results.length; i++) {
        ...
        if (event.results[i].isFinal) {
          const seg = t.trim();
          if (!seg) continue;
          if (!finalText) finalText = seg;
          else if (seg.startsWith(finalText)) finalText = seg;        // cumulative restatement -> replace
          else if (finalText.startsWith(seg)) { /* shorter prefix already covered -> skip */ }
          else finalText = finalText + ' ' + seg;                     // genuine new segment -> append
        } else {
          interim += t;
        }
      }
    ...
    recognition.onend = () => {
      ...
      // THE AUTO-RESTART IS THE DANGEROUS ONE. This loop was armed while consent was held; if it
      ...
      // Browsers end recognition periodically even in continuous mode. If the user still
      ...
      shouldListenRef.current = false;
      setIsListening(false);
      // Transcribed text stays in the box for the owner to review/edit/send. No auto-send.
    };
    recognition.onerror = (event) => {
      // Fatal errors end dictation; transient ones (e.g. no-speech) let onend restart it.
      const fatal = ['not-allowed', 'audio-capture', 'service-not-allowed', 'network'];
      if (fatal.includes(event.error)) shouldListenRef.current = false;
      const errorMap = {
        'not-allowed': '🚫 لم يتم السماح بالميكروفون. افتح إعدادات المتصفح واسمح بالميكروفون.',
        'no-speech': '🤫 لم أسمع شيئاً، جرب مرة أخرى.',
        'audio-capture': '🎤 الميكروفون غير متصل.',
        'network': '📡 خطأ في الشبكة.',
        'aborted': '',
      };
      const msg = errorMap[event.error] !== undefined ? errorMap[event.error] : `خطأ: ${event.error}`;
      if (msg) {
        setVoiceError(msg);
    ...
    // Teardown on rebuild AND on unmount. Handlers first, then abort: an abort() fires onend, and
  ...
  // (2) HOLDING while the first layout settles. Some height changes never go through React -- an
  ...
  useEffect(() => {
    if (!jumpToEndRef.current) return undefined;
    const el = messagesAreaRef.current;
    if (!el) { jumpToEndRef.current = false; return undefined; }
    el.scrollTop = el.scrollHeight;
    let ro = null, timer = null, settled = 0, lastH = -1;
    const release = () => {
    ...
    try {
      ro = new ResizeObserver(() => {
        ...
        el.scrollTop = h;
        // two consecutive observations at the same height means the layout has stopped moving
        if (h === lastH) { if (++settled >= 2) release(); } else { settled = 0; lastH = h; }
      });
      ro.observe(el);
      for (let i = 0; i < el.children.length; i++) ro.observe(el.children[i]);
    } catch (e) { ro = null; }
    // the user reaching for the scroll ends the pin immediately -- they own it from then on
    el.addEventListener('wheel', release, { passive: true });
    el.addEventListener('touchstart', release, { passive: true });
    timer = setTimeout(release, ro ? 1200 : 300);
    return release;
  }, [chatId]);

  // (3) FOLLOWING a live reply. Unchanged behaviour -- smooth, as before -- but only while the
  ...
  useEffect(() => {
    if (skipFollowRef.current > 0) { skipFollowRef.current--; return; }  // this run IS an open
    ...
    if (!stickToEndRef.current) return;        // the user is reading further up
    ...
    messagesEndRef.current?.scrollIntoView({ behavior: ezikMotionReduced(a11yRef.current.reduceMotion) ? 'auto' : 'smooth' });
  }, [messages, streamingText]);

  // Who is in charge of the scroll: recomputed on every scroll event, cheap, no state churn.
  const onMessagesScroll = () => {
    const el = messagesAreaRef.current;
    if (!el) return;
    stickToEndRef.current = (el.scrollHeight - el.scrollTop - el.clientHeight) < 80;
    // ── STREAM-P4 §٣/١: «حرّكَ بيدِه ⟹ يتوقّفُ التثبيت» ──────────────────────
    ...
    if (pinActiveRef.current && pinScrollTopRef.current >= 0
      && Math.abs(el.scrollTop - pinScrollTopRef.current) > 2) {
      pinActiveRef.current = false;
    }
  };

  // ── STREAM-P4 §٣/١: THE QUESTION SETTLES AT THE TOP, THE ANSWER GROWS UNDER IT ──
  //
  ...
  React.useLayoutEffect(() => {
    if (pinnedAskIndex == null || !pinActiveRef.current) return;
    const el = messagesAreaRef.current;
    const anchor = pinAnchorRef.current;
    if (!el || !anchor) return;

    // Where the question starts, in the container's own scroll coordinates.
    const anchorTop = anchor.getBoundingClientRect().top
      - el.getBoundingClientRect().top + el.scrollTop;

    // ── ITEM 102-ب: THE BREAKER NOW COUNTS STUCKNESS, NOT WORK ───────────────────────────
    ...
    const streamLen = streamingText == null ? -1 : streamingText.length;
    if (streamLen !== pinLenRef.current) {
      pinLenRef.current = streamLen;
      pinPassRef.current = 0;
    }

    // (1) ROOM. The shortfall against one viewport is the room still needed, and it therefore
    ...
    const tail = askPadElRef.current || messagesEndRef.current;
    if (!tail) return;
    // The scroller carries its own bottom padding (.ezc-scroll), and that padding is real room
    ...
    if (Math.abs(need - askPinPad) > 1) {
      ...
      if (pinPassRef.current > 8) {
        pinActiveRef.current = false;
        setAskPinPad(0);
        return;
      }
      setAskPinPad(need);
      // The spacer lands on the next commit, and this effect runs again then. Taking the
      // position now, against a height that is about to change, would pin to a stale number.
      return;
    }

    // (2) POSITION. The written value is read back rather than assumed: the browser clamps
    ...
    if (Math.abs(el.scrollTop - anchorTop) > 1) el.scrollTop = anchorTop;
  ...
  const saveMessages = (msgs) => {
    if (!PERSIST_CONVERSATION) return; // مؤقت: لا تكتب المحادثة أثناء مرحلة التجارب.
    const cid = ezikSaveChat(chatIdRef.current, msgs, ezikProfileKey(profileRef.current));
    if (!cid) return;
    if (cid !== chatIdRef.current) { chatIdRef.current = cid; setChatId(cid); }
    refreshChatList();
  };

  // The menu's list, re-read from the store rather than patched in memory, so what the menu shows
  ...
  const refreshChatList = () => setChatList(ezikListChats(ezikProfileKey(profileRef.current)));

  // Put the thread back to the empty state: stop anything in flight, drop the conversation
  ...
  const resetThread = () => {
    ...
    cancelAudio();
    // S97: an empty thread has no end to land on, so the opening pin is disarmed rather than
    ...
    setChatId(null);
    setMessages([]);
    setStreamedOpen(new Set());   // S98: a new thread inherits nobody's expanded replies
  ...
  const newChat = () => { resetThread(); };
  // S118: what «محادثة جديدة» calls. newChat only RESETS the thread, which was enough while the
  ...
  const favPk = ezikProfileKey(profileRef.current || profile);
  // THIS PROFILE'S FAVOURITES, and only this profile's. The match is exact: a record whose owner
  ...
  const myFavs = React.useMemo(
    () => favs.filter((f) => f.pk === favPk).sort((a, b) => (b.at || 0) - (a.at || 0)),
    [favs, favPk]);
  // The lookup a bubble is asked for. A Set, rebuilt once per change — never a scan per bubble.
  const favIdSet = React.useMemo(() => {
    const set = new Set();
    for (let i = 0; i < myFavs.length; i++) set.add(myFavs[i].id);
    return set;
  }, [myFavs]);
  // WHICH BUBBLE WEARS A FILLED STAR, decided ONCE per change of the thread, the conversation, the
  ...
  const favFlags = React.useMemo(
    () => messages.map((m, i) => !!(m && m.role === 'assistant' && typeof m.content === 'string' && m.content
      && favIdSet.has(ezikFavId(favPk, chatId, i, m.content)))),
    [messages, favIdSet, favPk, chatId]);
  // The ONE writer. It reads the ref, not the state, so a fast double tap toggles twice rather
  ...
  const applyFavs = (next) => {
    const written = ezikWriteFavs(next);
    const settled = written === null ? favsRef.current : written;   // store refused: change nothing
    favsRef.current = settled;
    setFavs(settled);
    return written !== null;
  };
  // The ONE writer's toggle. `idx` is the bubble's own position, handed down from the map that
  ...
  const toggleFavoriteReply = (msg, idx) => {
    const text = (msg && typeof msg.content === 'string') ? msg.content : '';
    if (!text.trim()) return;
    const id = ezikFavId(favPk, chatIdRef.current, idx, text);
    const cur = favsRef.current;
    const exists = cur.some((f) => f.id === id);
    applyFavs(exists
      ? cur.filter((f) => f.id !== id)
      : [ezikMakeFav(text, favPk, chatIdRef.current, idx)].concat(cur.filter((f) => f.id !== id)));
  };
  const removeFavorite = (id) => applyFavs(favsRef.current.filter((f) => f.id !== id));
  // Item 86: THE AYAH STAR. A set of REFERENCES, not of ids -- a bubble knows which verse it is
  ...
  const ayahFavIds = React.useMemo(() => {
    const set = new Set();
    for (let i = 0; i < myFavs.length; i++) if (ezikKindOf(myFavs[i]) === 'ayah' && myFavs[i].ref) set.add(myFavs[i].ref);
    return set;
  }, [myFavs]);
  // The verse card hands up what it is showing -- including the CANONICAL text it fetched from
  // the guarded mushaf, never the model's -- and this makes the record from it.
  const toggleFavoriteAyah = (v) => {
    const ref = ezikAyahFavKey(v && v.surahNum, v && v.surah, v && v.ayah);
    const text = String((v && v.text) || '').trim();
    if (!ref || !text) return;
    const title = (v && v.surah) ? ('سورة ' + v.surah + '، آية ' + toArabicDigits(v.ayah)) : ref;
    const rec = ezikMakeFavOf('ayah', favPk, ref, title, text);
    const cur = favsRef.current;
    const exists = cur.some((f) => f.id === rec.id);
    applyFavs(exists ? cur.filter((f) => f.id !== rec.id) : [rec].concat(cur.filter((f) => f.id !== rec.id)));
  };
  // ...and the store may now be written from a screen this component does not own (الفتاوى), so
  // it is re-read on the store's own announcement rather than only on this component's writes.
  useEffect(() => {
    const onFavs = () => { const l = ezikReadFavs(); favsRef.current = l; setFavs(l); };
    try { window.addEventListener(EZIK_FAVS_EVENT, onFavs); } catch (e) {}
    return () => { try { window.removeEventListener(EZIK_FAVS_EVENT, onFavs); } catch (e) {} };
  }, []);
  ...
  const searchCorpusRef = useRef({ key: null, rows: null });
  const getSearchCorpus = () => {
    let key = '';
    for (let i = 0; i < chatList.length; i++) key += chatList[i].id + ':' + (chatList[i].at || 0) + '|';
    if (searchCorpusRef.current.key === key && searchCorpusRef.current.rows) return searchCorpusRef.current.rows;
    // A plain loop rather than a mapped one, deliberately: chat-history-guard counts the mapping
    ...
    const rows = [];
    for (let i = 0; i < chatList.length; i++) rows.push(ezikBuildSearchRow(chatList[i]));
    searchCorpusRef.current = { key: key, rows: rows };
    return rows;
  };
  const [chatQuery, setChatQuery] = useState('');
  const [favQuery, setFavQuery] = useState('');
  // Item 86: WHICH KIND THE VIEWER IS SHOWING. 'all' is the opening state, so the screen still
  ...
  // null means "no search is running" — the menu then shows its ordinary list, untouched.
  const chatResults = React.useMemo(() => {
    const q = ezikSearchQuery(chatQuery);
    if (!q) return null;
    return ezikSearchChats(getSearchCorpus(), q);
  }, [chatQuery, chatList]);
  const favResults = React.useMemo(() => {
    const q = ezikSearchQuery(favQuery);
    if (!q) return null;
    return ezikSearchFavs(myFavs, q);
  }, [favQuery, myFavs]);
  // The kind narrows FIRST and the search runs over what is left, so a query and a kind mean
  // "this word, among these" rather than two independent lists fighting over one screen.
  const kindFavs = favKind === 'all' ? myFavs : myFavs.filter((f) => ezikKindOf(f) === favKind);
  const shownFavs = favResults === null ? kindFavs : favResults.filter((f) => favKind === 'all' || ezikKindOf(f) === favKind);
  // What each tab would hold, counted once for the whole screen rather than per tab.
  const favCounts = React.useMemo(() => {
    const c = { all: myFavs.length, reply: 0, fatwa: 0, ayah: 0 };
    for (let i = 0; i < myFavs.length; i++) { const k = ezikKindOf(myFavs[i]); c[k] = (c[k] || 0) + 1; }
    return c;
  }, [myFavs]);
  // Which saved conversations still exist, computed ONCE for the whole favourites screen rather
  // than once per card — the screen asks it for every row.
  const liveChatIds = React.useMemo(() => {
    const set = new Set();
    if (screen !== 'favorites') return set;               // only the screen that needs it pays for it
    ezikReadChatIndex().forEach((r) => set.add(r.id));
    return set;
  }, [screen, chatList]);
  const openFavoriteChat = (id) => {
    if (!id || !liveChatIds.has(id)) return;
    openSavedChat(id);
    goEzikBack();                                          // spend the sheet's own history entry
  };
  ...
  const deleteSavedChat = (id) => {
    ezikDeleteChat(id);
    if (chatIdRef.current === id) resetThread();
    setChatPendingDelete(null);
    refreshChatList();
  };
  ...
  const takeAudioFocus = () => {
    audioPlayTokenRef.current++;                 // stale TTS events become no-ops
    const f = audioDoneRef.current;
    audioDoneRef.current = null;
    if (f) { try { f(); } catch (e) {} }          // resolve+detach the in-flight TTS playback
    if (audioRef.current) {
      try { audioRef.current.pause(); } catch (e) {}   // recitation path: pause fires its onpause -> resolves
      audioRef.current = null;
    }
    try { if (audioElRef.current) audioElRef.current.pause(); } catch (e) {}  // actually stop the reused TTS element
  };
  ...
  const stopSpeaking = cancelAudio;

  // ===== خطّ أنابيب الصوت: نفصل "جلب" المقطع الكلاميّ عن "تشغيله" =====
  ...
  const fetchSpeechAudio = async (rawText, myId) => {
    ...
    // إزالة الوسوم وتحويلها لنص طبيعي للنطق (الآية محذوفة هنا — تُتلى لا تُنطق)
    const ttsText = formatForTTS(rawText);
    const cleanText = ttsText.replace(SPEAK_EMOJI_RE, '').trim();
    if (!cleanText || !isCurrent()) return null;

    // فَحص ذكيّ للتشكيل — نتخطّى Haiku إن كان تشكيل Opus كافياً (٦٠٪+)
    const diacriticCount = (cleanText.match(/[ً-ْٰ]/g) || []).length;
    const letterCount = (cleanText.match(/[ء-ي]/g) || []).length;
    const diacriticRatio = letterCount > 0 ? diacriticCount / letterCount : 0;
    let textForTTS = cleanText;
    const bandForVoice = (() => { try { return deriveCaps(profileRef.current?.age).band; } catch (e) { return 'young'; } })();
    if (diacriticRatio >= 0.6 || !TTS_TASHKEEL_ENABLED) {
      // النصّ مُشكَّل جيداً من Opus — نَتخطّى Haiku، نوفِّر ثانيةً كاملة
      console.log(`[المربّي] تشكيل Opus كافٍ (${Math.round(diacriticRatio * 100)}٪) — تخطّينا Haiku`);
    } else {
      // التشكيل ناقص — نَستدعي Haiku للضبط
      console.log(`[المربّي] تشكيل ناقص (${Math.round(diacriticRatio * 100)}٪) — نَستدعي Haiku للضبط`);
      try {
        const tashkeelResponse = await aiFetch('/api/tashkeel', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (tashkeelResponse.ok) {
          ...
          if (tashkeelData.text && tashkeelData.text.trim()) {
            textForTTS = tashkeelData.text;
          }
        }
      } catch (e) {
        console.warn('[المربّي] Tashkeel failed, using original text:', e);
        // نُكمل بالنص الأصلي بدون توقف
      }
    }
    if (!isCurrent()) return null; // أُلغِيَ أثناء التشكيل

    // مفتاح A/B: تجريد التشكيل من النصّ المُرسَل للنطق فقط (العرض يبقى مُشَكَّلاً)
    const spokenText = STRIP_TASHKEEL_FOR_TTS ? stripAllTashkeel(textForTTS) : textForTTS;

    // ElevenLabs TTS — نُحضّر الـ blob فقط، ولا نُشغّل ولا نسحب التركيز هنا
    try {
      const response = await aiFetch('/api/tts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        if (!isCurrent()) { URL.revokeObjectURL(url); return null; } // مُلغى → لا تُسرّب blob
        return { kind: 'blob', url, consumed: false };
      }
      console.warn('ElevenLabs TTS failed (HTTP ' + response.status + ') — failing safe: no audio, no system-voice fallback');
    } catch (e) {
      console.warn('ElevenLabs TTS error — failing safe: no audio, no system-voice fallback:', e);
    }
    // فشل ElevenLabs → فشل آمن: لا صوت إطلاقاً. تطبيق الأطفال يجب ألّا يُصدر صوت نظامٍ
    ...
    if (isCurrent()) {
      setVoiceError('🔇 تعذّر تشغيل الصوت — تحقّق من الاتصال وحاول مرة أخرى.');
      setTimeout(() => setVoiceError(''), 6000);
    }
    return null;
  };
  ...
  const playPreparedSpeech = async (prepPromise, myId) => {
    ...
    let r = null;
    try { r = await prepPromise; } catch (e) { r = null; }
    if (!r) return;
    if (!isCurrent()) { // أُلغِيَ قبل أن نُشغّله — تخلّص من blob المُحضَّر دون تشغيلٍ بائت
      if (r.kind === 'blob' && r.url) { r.consumed = true; URL.revokeObjectURL(r.url); }
      return;
    }
    r.consumed = true;
    takeAudioFocus();
    if (r.kind === 'blob') {
      const audio = audioElRef.current || (audioElRef.current = new Audio());  // REUSE the unlocked element (iOS)
      audioRef.current = audio;
      const myPlay = ++audioPlayTokenRef.current;
      await new Promise((resolve) => {
        ...
        audioDoneRef.current = finish;                                   // lets takeAudioFocus cancel cleanly
    ...
    // فشل آمن: لم يَعُد هناك احتياط صوت نظام. أيّ نتيجةٍ غير 'blob' لا تُنطَق إطلاقاً
  ...
  const playRecitationUrl = (url, myId) => new Promise((resolve) => {
    if (myId !== undefined && myId !== sequenceIdRef.current) return resolve({ failed: false });
    takeAudioFocus();
    const audio = audioElRef.current || (audioElRef.current = new Audio()); // العنصر المفتوح (iOS)
    audioRef.current = audio;
    const myPlay = ++audioPlayTokenRef.current;                              // يُبطِل أحداثَ التشغيل البائتة
    let settled = false;
    const finish = (failed) => {
      if (settled) return;
      settled = true;
      audio.onended = null; audio.onerror = null; audio.onpause = null;
      if (audioRef.current === audio) audioRef.current = null;
      if (audioDoneRef.current === finishExternal) audioDoneRef.current = null;
      resolve({ failed: !!failed });
    };
    const finishExternal = () => finish(false);   // takeAudioFocus/إيقاف خارجيّ: يُنهي المقطعَ مباشرةً
    audioDoneRef.current = finishExternal;
    audio.onended = () => { if (myPlay === audioPlayTokenRef.current) finish(false); };
    audio.onerror = () => { if (myPlay === audioPlayTokenRef.current) finish(true); };
    audio.onpause = () => { if (myPlay === audioPlayTokenRef.current) finish(false); }; // سحبُ تركيزٍ/إيقافٌ يدويّ
    audio.src = url;
    const p = audio.play();
    if (p && p.catch) p.catch((e) => {
      // ⛔ لا نبتلع الفشل صامتاً — نُسجّل سببَ الرفض (تفعيلٌ صوتيّ محجوب / شبكة) بدل تمريره كنجاح
      console.warn('[المربّي] تعذّر تشغيل التلاوة:', (e && (e.name || e.message)) || e);
      if (myPlay === audioPlayTokenRef.current) finish(true);
    });
  });

  // تشغيل تلاوة آيةٍ مفردةٍ بصوت القارئ — awaitable: يُحَلّ {failed} عند انتهاء/خطأ/إيقاف الصوت
  const playRecitation = (sNum, aNum, myId) => {
    if (myId !== undefined && myId !== sequenceIdRef.current) return Promise.resolve({ failed: false });
    const pad3 = (x) => String(x).padStart(3, '0');
    const url = `https://everyayah.com/data/${QURAN_RECITER}/${pad3(sNum)}${pad3(aNum)}.mp3`;
    return playRecitationUrl(url, myId);
  };

  // بناء تسلسل الصوت من الردّ: مقاطع كلام + تلاوات (آية مفردة <verse> أو سورة/مدى <surah>).
  ...
  const splitSpeechIntoSentences = (prose) => {
    const clean = formatForTTS(prose).trim();
    if (!clean) return [];
    const SENT = '\uE000'; // private-use sentinel; never appears in real text
    const marked = clean
      .replace(/([.!\u061F?\u061B])\s+/g, '$1' + SENT) // after . ! ؟ ? ؛ + space (won't split "3.5")
      .replace(/\n+/g, SENT);                           // and at line breaks
    const MIN_CHARS = 40; // merge tiny fragments to avoid choppy micro-requests
    const chunks = [];
    for (let s of marked.split(SENT)) {
      s = s.trim();
      if (!s) continue;
      const n = chunks.length;
      if (n && chunks[n - 1].length < MIN_CHARS) chunks[n - 1] += ' ' + s;
      else chunks.push(s);
    }
    return chunks.length ? chunks : [clean];
  };
  const buildAudioSequence = (text) => {
    // نُنظّف الترميز الناقص أوّلاً كي لا يُرسَل "<verse surah=..." خاماً إلى ElevenLabs
    text = stripIncompleteTags(text, { rescue: true });
    const parts = [];
    const blockRe = /<(verse|surah|dhikr)([^>]*)>[\s\S]*?<\/\1>/g;
    let last = 0, m;
    while ((m = blockRe.exec(text)) !== null) {
      const before = text.slice(last, m.index);
      for (const chunk of splitSpeechIntoSentences(before)) parts.push({ kind: 'speak', text: chunk });
      const tag = m[1], attrs = m[2] || '';
      if (tag === 'dhikr') { // dhikr - recorded Hisn al-Muslim recitation, auto-played inline (empty <dhikr id="N">)
        const catId = (attrs.match(/id=["']([^"']+)["']/) || [])[1];
        if (catId) parts.push({ kind: 'reciteDhikr', catId });
      } else if (tag === 'verse') {
        const sNum = resolveSurahNumber(
          (attrs.match(/surah=["']([^"']+)["']/) || [])[1],
          (attrs.match(/surah_num=["']([^"']+)["']/) || [])[1]
        ...
        const aNum = parseInt((attrs.match(/ayah=["']([^"']+)["']/) || [])[1], 10);
        if (sNum && aNum >= 1) parts.push({ kind: 'recite', sNum, aNum });
      } else { // surah — تلاوة متتابعة لمدًى من الآيات
        const sNum = resolveSurahNumber(undefined, (attrs.match(/num=["']([^"']+)["']/) || [])[1]);
        const from = parseInt((attrs.match(/from=["']([^"']+)["']/) || [])[1], 10);
        const to = parseInt((attrs.match(/to=["']([^"']+)["']/) || [])[1], 10);
        if (sNum) parts.push({ kind: 'reciteSurah', sNum, from: from >= 1 ? from : 1, to: to >= 1 ? to : null });
      }
      last = m.index + m[0].length;
    }
    const tail = text.slice(last);
    for (const chunk of splitSpeechIntoSentences(tail)) parts.push({ kind: 'speak', text: chunk });
    return parts;
  };

  // تلاوة سورة/مدًى متتابعاً بصوت القارئ، مع تحميلٍ مُسبقٍ للآية التالية → بلا فجوات.
  // يحترم الإلغاء (sequenceIdRef/takeAudioFocus). يُرجع {failed}.
  const playSurahRecitation = async (sNum, from, to, myId) => {
    const isCurrent = () => myId === undefined || myId === sequenceIdRef.current;
    if (!sNum) return { failed: true };
    let f = from >= 1 ? from : 1;
    let t = to;
    if (!(t >= 1)) { // المدى مفتوح → السورة كاملة: نحتاج عدد الآيات من المصحف
      try { await loadQuran(); } catch (e) {}
      t = getSurahAyahCount(sNum);
    }
    if (!(t >= f)) return { failed: true };
    const pad3 = (x) => String(x).padStart(3, '0');
    const mkUrl = (a) => `https://everyayah.com/data/${QURAN_RECITER}/${pad3(sNum)}${pad3(a)}.mp3`;
    // كلُّ آيةٍ تُشغَّل على العنصر المفتوح (playRecitationUrl) بدل new Audio() بعد await، فينجو
    ...
    for (let a = f; a <= t; a++) {
      if (!isCurrent()) break;
      if (a + 1 <= t) preloadRecitation(sNum, a + 1);
      const res = await playRecitationUrl(mkUrl(a), myId);
      if (res.failed) failedAny = true;
      if (!isCurrent()) break;
    }
    return { failed: failedAny };
  };

  // إعادة تشغيل تلاوة السورة يدوياً من زرّ البطاقة — يُوقف كلام المربّي أولاً. awaitable.
  const playSurahManual = async (sNum, from, to) => {
    const myId = ++sequenceIdRef.current; // يُلغي أي تسلسل/كلام جارٍ
    takeAudioFocus();
    setIsSpeaking(true);
    const res = await playSurahRecitation(sNum, from, to, myId);
    if (myId === sequenceIdRef.current) setIsSpeaking(false);
    return res;
  };

  // تحميل مسبق رخيص لتلاوةٍ قادمة (تسخين كاش المتصفح) — أفضل جهد، يُتجاهَل أيّ فشل.
  const __preloadedRecitations = new Set();
  const preloadRecitation = (sNum, aNum) => {
    if (!sNum || !(aNum >= 1)) return;
    const pad3 = (x) => String(x).padStart(3, '0');
    const url = `https://everyayah.com/data/${QURAN_RECITER}/${pad3(sNum)}${pad3(aNum)}.mp3`;
    if (__preloadedRecitations.has(url)) return;
    __preloadedRecitations.add(url);
    try { fetch(url, { mode: 'no-cors' }).catch(() => {}); } catch (e) {}
  };

  // تشغيل الردّ كاملاً بالتنسيق: كلام-قبل → تلاوة → كلام-بعد، مع إمكان الإلغاء.
  ...
  const speakReply = async (text) => {
    if (!text) return;
    text = await resolveWorshipTags(text, deriveCaps(profileRef.current?.age).band);
    const parts = buildAudioSequence(text);
    if (!parts.length) return;
    const myId = ++sequenceIdRef.current; // يُلغي أي تسلسل سابق
    takeAudioFocus();
    setIsSpeaking(true);

    // وعود تحضير المقاطع الكلامية (index → Promise<{kind,...}|null>)
    const prep = new Array(parts.length).fill(null);
    const prepareSpeak = (i) => {
      if (i < 0 || i >= parts.length || parts[i].kind !== 'speak') return null;
      if (!prep[i]) prep[i] = fetchSpeechAudio(parts[i].text, myId);
      return prep[i];
    };
    const nextSpeakIndex = (from) => {
      for (let j = from; j < parts.length; j++) if (parts[j].kind === 'speak') return j;
      return -1;
    };

    // ابدأ تحضير أوّل مقطعٍ كلاميّ فوراً (لا شيء نُوازيه قبله — تأخيره الابتدائيّ كاليوم).
    prepareSpeak(nextSpeakIndex(0));

    for (let i = 0; i < parts.length; i++) {
      if (myId !== sequenceIdRef.current) break; // أُلغِيَ
      const part = parts[i];
      // أثناء تشغيل المقطع الحاليّ: حضّر المقطع الكلاميّ القادم، وسخّن التلاوة التالية إن وُجدت.
      const nx = nextSpeakIndex(i + 1);
      if (nx >= 0) prepareSpeak(nx);
      if (i + 1 < parts.length) {
        const np = parts[i + 1];
        if (np.kind === 'recite') preloadRecitation(np.sNum, np.aNum);
        else if (np.kind === 'reciteSurah') preloadRecitation(np.sNum, np.from || 1);
      }
      try {
        if (part.kind === 'speak') await playPreparedSpeech(prepareSpeak(i), myId);
        else if (part.kind === 'reciteSurah') await playSurahRecitation(part.sNum, part.from || 1, part.to, myId);
        else if (part.kind === 'reciteDhikr') await playDhikrRecitation(part.catId, myId);
        else await playRecitation(part.sNum, part.aNum, myId);
      } catch (e) {}
      if (myId !== sequenceIdRef.current) break;
    }
    if (myId === sequenceIdRef.current) setIsSpeaking(false);

    // نظافة: حرّر أيّ صوتٍ كلاميّ مُحضَّرٍ لم يُشغَّل (إلغاء/خطأ) كي لا تتسرّب blob URLs.
    for (let i = 0; i < prep.length; i++) {
      const pr = prep[i];
      if (!pr) continue;
      Promise.resolve(pr).then((r) => {
        if (r && r.kind === 'blob' && r.url && !r.consumed) URL.revokeObjectURL(r.url);
      }).catch(() => {});
    }
  };

  // إعادة تشغيل التلاوة يدوياً من زر البطاقة — يُوقف كلام المربّي أولاً. awaitable.
  const playVerseManual = async (sNum, aNum) => {
    const myId = ++sequenceIdRef.current; // يُلغي أي تسلسل/كلام جارٍ
    takeAudioFocus();
    setIsSpeaking(true);
    const res = await playRecitation(sNum, aNum, myId);
    if (myId === sequenceIdRef.current) setIsSpeaking(false);
    return res;
  };

  // Auto-play the recorded Hisn al-Muslim recitation for a dua/dhikr category, inline in the
  ...
  const playDhikrRecitation = async (catId, myId) => {
    const isCurrent = () => myId === undefined || myId === sequenceIdRef.current;
    if (!catId) return { failed: true };
    let items = [];
    try {
      const db = await loadAdhkar();
      items = (db && db.byCat && db.byCat[parseInt(catId, 10)]) || [];
    } catch (e) { return { failed: true }; }
    const urls = items.map((d) => d && d.audio).filter(Boolean);
    if (!urls.length) return { failed: false };
    // كلُّ ذِكرٍ يُشغَّل على العنصر المفتوح (playRecitationUrl) بدل new Audio() بعد await.
    let failedAny = false;
    for (const url of urls) {
      if (!isCurrent()) break;
      const res = await playRecitationUrl(url, myId);
      if (res.failed) failedAny = true;
      if (!isCurrent()) break;
    }
    return { failed: failedAny };
  ...
  // CALL_STREAM_SPEECH streaming speech. Speaks COMPLETE prose sentences that arrive
  ...
  const createCallSpeechStream = () => {
    const myId = ++sequenceIdRef.current;   // one session; cancelAudio()/barge-in bump this to stop us
    takeAudioFocus();
    setIsSpeaking(true);
    const queue = [];
    let consuming = false, inputDone = false, hitTag = false, consumedLen = 0;
    let resolveDone;
    ...
    const pump = async () => {
      ...
      consuming = true;
      while (true) {
        if (!isCurrent()) { consuming = false; finishUp(); return; }         // barge-in / superseded
        if (queue.length === 0) {
          if (inputDone) { consuming = false; finishUp(); return; }
          consuming = false; return;                                          // idle; re-kicked by enqueue/finish
        }
    ...
    const feed = (full) => {
      ...
      if (tm) hitTag = true;   // reached a tag -> stop streaming; finish() plays the rest via buildAudioSequence
    };
    const finish = async (fullReply) => {
      let rest = (fullReply || '').slice(consumedLen);      // tags + any trailing prose not yet spoken
      rest = await resolveWorshipTags(rest, deriveCaps(profileRef.current?.age).band);
      if (rest.trim()) enqueue(buildAudioSequence(rest));
      inputDone = true;
      pump();
      await donePromise;
    };
  ...
  // Chat dictation goes through api/stt (Scribe) too: tap to record, tap again to transcribe.
  ...
  const dictStreamRef = useRef(null);
  const dictRecRef = useRef(null);
  const dictChunksRef = useRef([]);
  const stopDictAll = () => {
    try { if (dictRecRef.current && dictRecRef.current.state !== 'inactive') dictRecRef.current.stop(); } catch (e) {}
    dictRecRef.current = null;
    try { dictStreamRef.current?.getTracks().forEach((tr) => tr.stop()); } catch (e) {}
    dictStreamRef.current = null;
  };
  const startCloudDictation = async () => {
    // The cloud dictation recorder, on the same terms as everything else: no consent, no
    // getUserMedia. startListening already refuses above this, so this is the second barrier.
    if (!hasValidAIConsent()) { setIsListening(false); setVoiceError(EZ_SPEECH_NO_CONSENT); setTimeout(() => setVoiceError(''), 6000); return; }
    try {
      dictStreamRef.current = await navigator.mediaDevices.getUserMedia({ audio: true });
      dictChunksRef.current = [];
      const mr = new MediaRecorder(dictStreamRef.current, pickRecMime());
      mr.ondataavailable = (e) => { if (e.data && e.data.size) dictChunksRef.current.push(e.data); };
      dictRecRef.current = mr;
      mr.start(250);
      setIsListening(true);
    } catch (e) {
      stopDictAll();
      setIsListening(false);
      setVoiceError('\u0644\u0645 \u0623\u062A\u0645\u0643\u0651\u0646 \u0645\u0646 \u0641\u062A\u062D \u0627\u0644\u0645\u0627\u064A\u0643.');
      setTimeout(() => setVoiceError(''), 6000);
    }
  };
  const stopCloudDictation = async () => {
    const mr = dictRecRef.current;
    setIsListening(false);
    if (!mr) { stopDictAll(); setVoiceError('DBG A no-recorder'); setTimeout(() => setVoiceError(''), 8000); return; }
    const blob = await new Promise((resolve) => {
      mr.onstop = () => { try { resolve(new Blob(dictChunksRef.current, { type: mr.mimeType || 'audio/webm' })); } catch (e) { resolve(null); } };
      try { mr.stop(); } catch (e) { resolve(null); }
    });
    stopDictAll();
    if (!blob || blob.size < 1500) { setVoiceError('DBG B bytes=' + (blob ? blob.size : -1)); setTimeout(() => setVoiceError(''), 8000); return; }
    const b64 = await blobToBase64(blob);
    let text = '';
    try {
      const band = (() => { try { return deriveCaps(profileRef.current?.age).band; } catch (e) { return 'young'; } })();
      const r = await aiFetch('/api/stt', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ audio: b64, mime: blob.type, band }) });
      if (r.ok) { const d = await r.json(); text = String(d.text || '').trim(); } else { setVoiceError('DBG C http=' + r.status); setTimeout(() => setVoiceError(''), 8000); return; }
    } catch (e) { setVoiceError('DBG E ' + String(e && e.message).slice(0, 40)); setTimeout(() => setVoiceError(''), 8000); return; }
    if (!text) {
      setVoiceError('\u0644\u0645 \u0623\u0633\u0645\u0639\u0643\u060C \u062D\u0627\u0648\u0644 \u0645\u0631\u0651\u0629\u064B \u0623\u062E\u0631\u0649.');
      setTimeout(() => setVoiceError(''), 5000);
      return;
    }
    setInput((baseTextRef.current || '') + text);
  };  const startListening = async () => {
    if (childVoiceBlocked()) {                                   // غ‑٣: لا getUserMedia ولا recognition.start
      showChildVoiceNotice(setVoiceError);
      return;
    }
    // بلا موافقة: لا يُفتح الميكروفونُ إطلاقاً — لا تسجيلٌ يُرسَل إلى ElevenLabs ولا جلسةُ
    // تعرُّفٍ يفتحها المتصفّح. فشلٌ آمنٌ صامتٌ إلى الوضعِ المحلّيّ.
    if (!hasValidAIConsent()) { setAiConsent(aiConsentStatus()); return; }
    if (DICTATE_CLOUD) {
      if (isSpeaking) stopSpeaking();
      baseTextRef.current = input && !/\s$/.test(input) ? input + ' ' : input;
      setVoiceError('');
      await startCloudDictation();
      return;
    }    if (!recognitionRef.current) {
      // Two different truths, and they must not be told as one. No engine in the browser is the
      ...
      setVoiceError(hasValidAIConsent()
        ? '🚫 متصفحك لا يدعم التعرف على الصوت. استخدم Chrome أو Safari.'
        : EZ_SPEECH_NO_CONSENT);
      setTimeout(() => setVoiceError(''), 6000);
      return;
    }
    try {
      ...
      setVoiceError('🚫 لم يُمنح إذن الميكروفون.');
      setTimeout(() => setVoiceError(''), 6000);
      return;
    }
    if (isSpeaking) stopSpeaking();
    // Snapshot the current box (typed or previously dictated, incl. hand-edits) as the base.
    ...
    setVoiceError('');
    setIsListening(true);
    // Re-read at the instant of starting: the getUserMedia permission prompt above can sit open
  ...
  const stopListening = () => {
    if (DICTATE_CLOUD) { stopCloudDictation(); return; }
    // User tapped the mic off: prevent the onend auto-restart and keep the text in the box.
    shouldListenRef.current = false;
    if (recognitionRef.current) recognitionRef.current.stop();
  };

  const startChat = async (name, age, gender) => {
    ...
    // S92: pid is the identity this profile's saved conversations are filed under. It is minted
    ...
    const p = { name, age, gender, birthYear, pid: ezikMintId(), createdAt: new Date().toISOString() };
    profileRef.current = p;
    voiceProfileRef.current = p;                // غ‑٣: مرآةُ حاجزِ الصوت — قبل أيّ نطقٍ أو تحيّة
    setProfile(p);
    // D85: after the age step the landing screen is the chat, and it opens EMPTY. The boot
    // greeting is NOT sent -- nothing sits in the thread until the child writes the first line.
    chatIdRef.current = null;
    setChatId(null);
    setMessages([]);
    setChatList([]);                           // S92: a new profile starts with an empty menu
    setScreen('chat');
    try { localStorage.setItem('child_profile', JSON.stringify(p)); } catch (e) {}
  };
  ...
  const callAI = async (history, p, { onDelta, signal, mode = 'chat', endpoint = '/api/ask' } = {}) => {
    ...
    if (!hasValidAIConsent()) return '';
    // Text and call share the same deterministic server router and evidence path. The thin
    ...
    const FAST_CHANNEL_ENABLED = false;
    const __classifyFast = async () => {
      try {
        const __ex = (m) => (m && typeof m.content === 'string') ? m.content : (m && typeof m.text === 'string') ? m.text : (m && Array.isArray(m.content)) ? m.content.map(c => (c && typeof c.text === 'string') ? c.text : '').join(' ') : '';
        const __n = (history || []).length;
        const __curText = __ex(__n ? history[__n - 1] : null).trim();
        if (!__curText) return 'DEEN';
        const __prev = __n >= 2 ? history[__n - 2] : null;
        const __prevText = (__prev && __prev.role === 'assistant') ? __ex(__prev).trim() : '';
        const __userContent = __prevText ? ('السياق السابق: ' + __prevText + ' | الرسالة الحاليّة: ' + __curText) : __curText;
        const __resp = await aiFetch('/api/chat-fast', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          signal,
          // D02ب: the classifier's own instructions are the server's too (max_tokens 8 is what
          ...
          body: JSON.stringify({ max_tokens: 8, stream: true, name: p.name, age: p.age, gender: p.gender, mode: 'call', band: deriveCaps(p.age).band, messages: [{ role: 'user', content: __userContent }] })
        });
        if (!__resp.ok || !__resp.body) return 'DEEN';
        const __reader = __resp.body.getReader();
        const __decoder = new TextDecoder();
        let __buf = '', __out = '';
        while (true) {
          const __chunk = await __reader.read();
          if (__chunk.done) break;
          __buf += __decoder.decode(__chunk.value, { stream: true });
          const __lines = __buf.split('\n');
          __buf = __lines.pop();
          for (let __i = 0; __i < __lines.length; __i++) {
            const __s = __lines[__i].trim();
            if (__s.indexOf('data:') !== 0) continue;
            const __data = __s.slice(5).trim();
            if (__data === '[DONE]') continue;
            try {
              const __evt = JSON.parse(__data);
              if (__evt.type === 'content_block_delta' && __evt.delta && __evt.delta.type === 'text_delta') { __out += __evt.delta.text; }
            } catch (__pe) {}
          }
        }
        return (__out.trim().toUpperCase() === 'GEN') ? 'GEN' : 'DEEN';
      } catch (__e) { return 'DEEN'; }
    };
    // D02ب: NO PROMPT IS BUILT HERE ANY MORE. The routing decision is unchanged — a call turn the
    ...
    try {
      ...
      const __extra = {
        ...
        ...(mode === 'chat' && endpoint === '/api/ask' && deriveCaps(p.age).band === 'adult' && depthMode !== 'brief' ? { depth: depthMode === 'scholar' ? 'scholar' : 'deep' } : {}),
        // band: so the server restricts RAG sources by age (khilaf-policy §6: under-18 is served
        ...
        band: deriveCaps(p.age).band,
      };
      // D02ب: the four reader fields REPLACE `system`. The server builds the prompt from them
      ...
      const __mkBody = (msgs) => ({ max_tokens: 4096, stream: true, name: p.name, age: p.age, gender: p.gender, mode, messages: msgs, ...__extra });
      // Item 1 / defect 44: drop OLDEST turns until the body fits CLIENT_BUDGET, measured the SERVER's
      // way -- never shipping a body the server would 413. Keeps the system prompt + the newest turn.
      const __apiMessages = fitMessagesToBudget(__mkBody, history.map(m => ({ role: m.role, content: m.content })));
      const response = await aiFetch(endpoint, {
        method: 'POST',
        // capHeaders() carries the device id (and a founder token when one exists) for the
        ...
        credentials: 'same-origin',
        signal,
        body: JSON.stringify(__mkBody(__apiMessages)),
      });
      // NOTE: the server still reports the remaining allowance in a response header. That is
      ...
      if (!response.ok) {
        // Upstream error via the proxy — distinct from a network drop. Logged to console
        // (for parents via devtools), never shown raw to the child.
        const errText = await response.text().catch(() => '');
        console.error(`[Al-Murabbi] API error ${response.status}:`, errText.slice(0, 500));
        if (response.status === 429) {
          // Day cap (directive 78): the SERVER owns the wording, so the child is told the
          ...
          return getFriendlyError('rateLimit', p.gender);
        }
        if (response.status >= 500) return getFriendlyError('server', p.gender);
        // Every remaining non-2xx is a TECHNICAL failure of ours (400 bad body, 403, 413 too
        ...
        return getFriendlyError('technical', p.gender);
      }

      // Read the SSE stream: accumulate text_delta and notify onDelta on every update.
      ...
      const handleEvent = (block) => {
        ...
        // message_stop / ping / message_start / ... -> ignore
      };
      ...
      return full || getFriendlyError('technical', p.gender);
    } catch (e) {
  ...
  const onPickImage = async (e) => {
    ...
    if (!hasValidAIConsent()) { e.target.value = ''; setAiConsent(aiConsentStatus()); return; }
    const rawName = (file.name || '').trim();
    const lowerName = rawName.toLowerCase();
    const ext = (lowerName.match(/\.([a-z0-9]+)\s*$/) || [,''])[1]; // extension without dot, or ''
    const mime = file.type || '';
    const isImage = mime.startsWith('image/') || ['png','jpg','jpeg','gif','webp','bmp','heic','heif'].includes(ext);
    ...
    const fail = (msg) => { alert(msg); e.target.value = ''; };
    try {
      if (isImage) {
        ...
        try {
          ...
          // UTF-8 first; if it produces nothing usable, fall back to Windows-1256 (common for Arabic .txt from Notepad)
          raw = new TextDecoder('utf-8', { fatal: false }).decode(buf);
          if (!raw.trim() || raw.indexOf('\uFFFD') !== -1) {
            const alt = new TextDecoder('windows-1256', { fatal: false }).decode(buf);
            if (alt.trim() && (alt.match(/\uFFFD/g) || []).length < (raw.match(/\uFFFD/g) || []).length + 1) raw = alt;
          }
        } catch (tErr) {
          return fail('تعذّرت قراءة الملف النصّي. حاول حفظه بترميز UTF-8.');
        }
        const text0 = raw.replace(/^\uFEFF/, '').trim(); // strip BOM + trim
        if (!text0) return fail('الملف النصّي فارغ أو بترميز غير مدعوم. احفظه بترميز UTF-8 ثمّ أعد رفعه.');
        const MAX = 100000;
        ...
        // S97: mammoth is warmed on idle after the first paint rather than blocking boot. The
        ...
        if (window.__ezikVendor) { try { await window.__ezikVendor('mammoth'); } catch (e) {} }
        if (!window.mammoth) return fail('تعذّر تحميل أداة قراءة ملفات Word. تحقّق من الاتصال وأعد المحاولة.');
        let extracted = '';
        try {
          ...
          return fail('تعذّرت قراءة ملف Word. تأكّد أنّه بصيغة ‎.docx حديثة.');
        }
        if (!extracted) return fail('لم يُعثَر على نصٍّ في ملف Word. قد يكون فارغًا أو صورًا فقط.');
        const MAXD = 100000;
        ...
        return fail('نوع الملف غير مدعوم: ' + (rawName || 'بلا اسم') + ' | ' + (mime || 'بلا نوع'));
      }
    } catch (err) {
      return fail('تعذّرت قراءة الملف. حاول ملفًّا آخر.');
    }
  ...
  const sendMessage = async (text) => {
    ...
    // S92: the conversation is filed HERE, on the QUESTION -- not after the reply lands. "Saved
    ...
    const apiHistory = sliceHistoryForAPI(updated);
    const controller = new AbortController();
    abortRef.current = controller;
    // Arm the "searching sources…" hint: if no delta has arrived after ~4s (fast non-search
    ...
    try {
      ...
      if (e.name === 'AbortError') {
        // Aborted (newer message, or leaving chat via the [screen] cleanup). Clear the
        ...
        if (abortRef.current === controller) { clearSearchingHint(); setIsLoading(false); setStreamingText(null); }
        return;
      }
  ...
  const sectionSuggestions = [
    { id: 'fatwa', label: EZH_FATWA, icon: EZH_ICON_FATWA, go: () => setScreen('fatwa') },
    { id: 'treasure', label: EZH_TREASURE, icon: EZH_ICON_TREASURE, go: () => { window.location.href = '/quest.html'; } },
    { id: 'mushaf', label: EZH_MUSHAF, icon: EZH_ICON_MUSHAF, go: () => setScreen('mushaf') },
  ];
  // THE THIRD STATE IS ABSENCE. One expression decides it, and each term is a separate promise
  ...
  // ── STREAM-P4 §٣/٢: THE COMPOSER'S `Enter`, IN ONE PLACE ──────────────────
  //
  ...
  const onComposerKeyDown = (e) => {
    ...
    if (composingRef.current || e.nativeEvent?.isComposing
      || e.which === 229 || e.keyCode === 229) return;
  ...
  // The reader's actual device, recorded from the event rather than inferred from the viewport.
  // Guarded so an unchanged value costs no render: this fires on every press of the field.
  const onComposerPointerDown = (e) => {
    const touch = e.pointerType === 'touch' || e.pointerType === 'pen';
    if (touch !== inputIsTouch) setInputIsTouch(touch);
  };
  // Where the quick actions may appear, and it is deliberately ONE place: under the newest reply,
  ...
  const quickActionsVisible = !!lastMsg
    && lastMsg.role === 'assistant'
    && !isLoading
    && streamingText === null
    && !ezikIsErrorReply(lastMsg.content)
    && messages.some((m) => m && m.role === 'user');
  ...
  const openReport = (aiMsg, prevMsg) => {
    const aiText = aiMsg && typeof aiMsg.content === 'string' ? aiMsg.content : '';
    let userText = '';
    if (prevMsg && prevMsg.role === 'user') {
      const c = prevMsg.content;
      if (typeof c === 'string') userText = c;
      else if (Array.isArray(c)) { const t = c.find(b => b && b.type === 'text'); userText = t ? t.text : ''; }
    }
    setReportFor({ ai: aiText, user: userText });
  };

  const bubbleFnRef = useRef({});
  useEffect(() => {
    bubbleFnRef.current = {
      send: sendMessage,
      ...
      speak: speakReply,
      toggleTashkeel: toggleTashkeel,
      quote: quoteReply,
      favorite: toggleFavoriteReply,
      favoriteAyah: toggleFavoriteAyah,
      report: openReport,
      messages: messages,
    };
  });
  ...
  const cbPlayMessage = React.useCallback((t) => bubbleFnRef.current.speak(t), []);
  const cbToggleTashkeel = React.useCallback(() => bubbleFnRef.current.toggleTashkeel(), []);
  const cbQuote = React.useCallback((t) => bubbleFnRef.current.quote(t), []);
  const cbFavorite = React.useCallback((m, idx) => bubbleFnRef.current.favorite(m, idx), []);
  const cbFavoriteAyah = React.useCallback((v) => bubbleFnRef.current.favoriteAyah(v), []);
  // The report needs the reply AND the question before it, which used to be closed over per bubble.
  // The index is a stable prop; the pair is looked up on the tap, off the current thread.
  const cbReport = React.useCallback((idx) => {
    const ms = bubbleFnRef.current.messages || [];
    bubbleFnRef.current.report(ms[idx], idx > 0 ? ms[idx - 1] : null);
  }, []);

  // THE QUOTE. It writes to the composer and moves the caret there — no send, no store, and the
  ...
  const quoteReply = (clean) => {
    const block = ezikBuildQuote(clean);
    if (!block) return;
    setInput((prev) => ezikComposeWithQuote(prev, block));
    // After the value lands, not before — otherwise the caret is placed in the old text.
    setTimeout(() => {
      const el = inputElRef.current;
      if (!el) return;
      try { el.focus(); const n = String(el.value || '').length; el.setSelectionRange(n, n); } catch (e) {}
    }, 0);
  };
  ...
  // Silent auto-end. setScreen('chat') fires the call useEffect cleanup, which stops rec,
  // aborts the in-flight callAI, cancels audio, and bumps callGenRef (invalidating continuations).
  const endCallSilently = () => {
  ...
  // (Re)start the 45s idle clock. Armed on a FRESH listening turn and on detected speech only —
  ...
  const armInactivityTimer = () => {
    // The idle clock runs ONLY while we are actually listening. A late SR result arriving during
    // thinking/playback used to re-arm it, and 45s later it exited the call MID-ANSWER.
    if (!callActiveRef.current) return;
    clearInactivityTimer();
    const genAtArm = callGenRef.current;
    inactivityTimerRef.current = setTimeout(() => {
      if (callGenRef.current !== genAtArm) return; // call ended/changed → bail
      endCallSilently();
    }, 45000);
  };

  // Start (or re-arm) a listen turn. armIdleClock=true resets the 45s idle clock (a fresh turn);
  // the silence self-close re-arm passes false so silence keeps counting toward the auto-end.
  const startCallListening = (armIdleClock = true) => {
    if (callMutedRef.current) return; // muted = do not listen
    ...
    if (CALL_STT_CLOUD) {
      ...
      setCallState('listening');
      startCloudListening();
      if (armIdleClock) armInactivityTimer();
      return;
    }
    ...
    if (ezStartRecognition(rec)) {
      if (CALL_VAD) { vadLastVoiceRef.current = Date.now(); ensureVad(); }
      if (armIdleClock) armInactivityTimer();
    } else { callActiveRef.current = false; setCallState('idle'); }
  };

  // End the active listen turn and route it to the brain. Guarded so the silence timer and
  // recognition.onend can never both fire a turn (double-send guard via callActiveRef).
  const endCallTurnNow = () => {
    if (!callActiveRef.current) return;
    callActiveRef.current = false;
    if (silenceTimerRef.current) { clearTimeout(silenceTimerRef.current); silenceTimerRef.current = null; }
    try { callRecognitionRef.current?.stop(); } catch (e) {}
    const text = collapseRepeats((callBaseTextRef.current + ' ' + callTranscriptRef.current).trim());
    if (!text) { setCallState('idle'); return; } // never send an empty turn
    if (callTurnRef.current) callTurnRef.current(text);
  };
  ...
  const runCallTurn = async (text) => {
    ...
    const updated = [...messages, userMsg];
    // Do NOT commit the user msg to messages here. It is written together with the reply on
    ...
    abortRef.current = controller;
    let reply;
    let callStream = null; // CALL_STREAM_SPEECH: lazily-created streaming session (off -> stays null; classic speakReply used)
    try {
      reply = await callAI(apiHistory, profile, {
        signal: controller.signal, mode: 'call',
        ...(CALL_STREAM_SPEECH ? { onDelta: (full) => { if (abortRef.current !== controller) return; if (!callStream) { callStream = createCallSpeechStream(); setCallState('speaking'); } callStream.feed(full); } } : {}),
      });
    } catch (e) {
      if (e.name === 'AbortError') return; // exited mid-turn — do not write state
      reply = getFriendlyError('network', profile?.gender);
    }
    if (abortRef.current !== controller) return; // superseded or call exited
    abortRef.current = null;
    const aiMsg = { role: 'assistant', content: reply, timestamp: new Date().toISOString() };
    const final = [...updated, aiMsg];
    setMessages(final);
  ...
  useEffect(() => { callTurnRef.current = runCallTurn; });

  // Mute toggle — actually gates the mic: muted = stop/suppress listening (not just visual).
  const toggleCallMute = () => {
    setIsCallMuted((m) => {
      ...
      if (next) {
        callActiveRef.current = false;
        if (silenceTimerRef.current) { clearTimeout(silenceTimerRef.current); silenceTimerRef.current = null; }
        clearInactivityTimer(); // leaving listening (muted) — stop the idle clock; user re-arms manually
        try { callRecognitionRef.current?.stop(); } catch (e) {}
        if (CALL_STT_CLOUD) stopCloudAll(); // mute must STOP capturing, not merely ignore the audio
        setCallState((csNow) => (csNow === 'listening' ? 'idle' : csNow));
      }
      return next;
    });
  };

  // Avatar tap = "my turn". Layer 3 makes it state-aware: it is the manual interrupt (no voice
  // barge-in). Tapping during think/speak cancels the tutor and opens the mic; from idle it starts.
  const onCallTalk = () => {
    if (callMutedRef.current) return;
    if (callState === 'thinking' || callState === 'speaking') {
  ...
  useEffect(() => {
    ...
    if (childVoiceBlocked()) return; // غ‑٣: لا يُبنى معرِّفُ كلامٍ أصلاً — الرسمُ يعرض التنبيهَ بدل الشاشة
    ...
    if (!hasValidAIConsent()) return;
    if (!hasFounderToken()) return; // directive 82: no token => the call session is never built
    callGenRef.current++; // ENTER: open a new call session (invalidates any stale continuation)
    const SR = ezSpeechEngine();
    // CLOUD STT DOES NOT USE SpeechRecognition AT ALL. It records with MediaRecorder and
    ...
    if (!SR && !CALL_STT_CLOUD) {
      showCallError('🚫 متصفحك لا يدعم التعرف على الصوت. استخدم Chrome أو Safari.');
      return;
    }
    setVoiceError(''); // entering a call clears any stale banner from the chat screen
    ...
    if (rec) {
      rec.lang = 'ar-SA';
      rec.continuous = true;     // interim events keep resetting the silence timer (smoother end-of-turn)
      rec.interimResults = true;
    }
    const onRecResult = (event) => {
      // This ar-SA engine emits multiple isFinal results where each later final RE-INCLUDES the
      ...
      callTranscriptRef.current = finalText;
      setCallHeard((finalText + interim).trim()); // feedback only — NEVER setInput
      // Layer 3: actual speech this event → child present → reset the 45s idle clock.
      if ((finalText + interim).trim()) armInactivityTimer();
      // Reset the end-of-turn debounce on any speech activity.
      if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
      silenceTimerRef.current = setTimeout(() => {
        // Fire only with a non-empty transcript; silence on an empty turn keeps listening.
        if (callActiveRef.current && (callBaseTextRef.current + callTranscriptRef.current).trim()) endCallTurnNow();
      }, vadOkRef.current ? 86400000 : CALL_SILENCE_MS);
    };
    const onRecEnd = () => {
      // Chrome owns onend and fires it on its own schedule (short Arabic pauses, engine timeouts).
      ...
      callBaseTextRef.current = (callBaseTextRef.current + ' ' + callTranscriptRef.current).trim();
      callTranscriptRef.current = '';                      // new SR session => results[] restarts at 0
      ...
      silenceTimerRef.current = setTimeout(() => {
        if (callActiveRef.current && (callBaseTextRef.current + callTranscriptRef.current).trim()) endCallTurnNow();
      }, vadOkRef.current ? 86400000 : CALL_RESTART_GRACE_MS);
      const genAtEnd = callGenRef.current;
      const tryStart = (attempt) => {
        if (childVoiceBlocked()) return;                 // غ‑٣: الحجبُ يقطع محاولاتِ إعادةِ التشغيل الخمس
        ...
        if (!hasValidAIConsent()) { ezKillRecognizer(rec); callActiveRef.current = false; setCallState('idle'); return; }
        if (callGenRef.current !== genAtEnd) return;       // call ended/changed
    ...
    const onRecError = (event) => {
      ...
      setCallHeard('');
      setCallState('idle');
      // A denied mic is fatal and MUST be said. 'no-speech'/'aborted' are routine and stay quiet.
      const fatal = ['not-allowed', 'audio-capture', 'service-not-allowed'];
      if (fatal.includes(event.error)) {
        showCallError(event.error === 'audio-capture'
          ? '🎤 الميكروفون غير متاح على هذا الجهاز.'
          : '🚫 لم يُمنح إذن الميكروفون. افتح إعدادات التطبيق واسمح بالميكروفون ثم أعد الدخول للمكالمة.');
      } else if (event.error === 'network') {
        showCallError('📡 انقطع الاتّصال بمحرّك التعرّف على الصوت. تحقّق من الإنترنت ثم أعد المحاولة.');
      }
    };
    ...
    return () => {
      ...
      if (silenceTimerRef.current) { clearTimeout(silenceTimerRef.current); silenceTimerRef.current = null; }
      clearInactivityTimer();
      // Clear the banner WITH its dismiss timer. Cancelling the timer alone would strand a
      ...
      setVoiceError('');
      ezKillRecognizer(rec);   // handlers first, then abort+stop, and drop it from the live registry
  ...
  // Toggle the parental direct-conversation lock. Locking writes 'true'; unlocking REMOVES the key
  // (absence = allowed default — we never persist 'false').
  const toggleDirectConvoLock = () => setDirectConvoLocked(prev => {
    const next = !prev;
    try { if (next) localStorage.setItem('directConvoLocked', 'true'); else localStorage.removeItem('directConvoLocked'); } catch (e) {}
    return next;
  });
  ...
  const resetAll = () => {
    if (confirm('هل أنت متأكد من حذف كل البيانات؟')) {
      ...
      ezikClearAllChats();
      // S98 -- and the saved replies, which live in their own key precisely BECAUSE they survive a
      // deleted conversation. Nothing else clears them, so this line is the only way they go.
      ezikClearAllFavs();
      // S99 -- and the reading preferences, which are per-profile device settings and go with
      // everything else this button clears.
      ezikClearAllA11y();
      localStorage.removeItem('directConvoLocked');
      localStorage.removeItem(MUSHAF_BOOKMARK_KEY);
      // ITEM 87: `mushaf_pos_v1` IS A DEAD NAME. This sweep is its ONLY appearance in the file
      ...
      try { localStorage.removeItem(EZIK_UI_STYLE_KEY_DEAD); } catch (e) {}
      // S100 -- and the VISUAL IDENTITY, on the same terms. Removing the key is enough: the
      ...
      try { localStorage.removeItem(EZIK_VISUAL_THEME_KEY); } catch (e) {}
  ...
  const aiConsentDecided = aiConsent === EZ_AI_CONSENT_GRANTED || aiConsent === EZ_AI_CONSENT_DECLINED;
  if (profile && (!aiConsentDecided || aiConsentReview)) return (
    <AIConsentGate
      age={profile.age}
      current={aiConsent}
      onGrant={grantAIConsent}
      onDecline={declineAIConsent}
      onBack={aiConsentDecided ? () => setAiConsentReview(false) : null}
    />
  );
  // S118 -- ONE MENU, DRAWN BY TWO SCREENS. This block used to sit inside the chat's own
  ...
  const ezikDrawer = () => drawerOpen && (
      <div className="ezhome ezc">
        <div onClick={() => closeDrawerWith(null)} className="ezc-drawer-ov" />
        {/* S112: the panel is a side drawer on a phone and an INSET, bounded panel from a desk
            upward -- it never becomes a full-screen sheet over the conversation behind it. The
            order of the entries, the handlers they call, the ids they carry and the store they
            read are untouched; only the surface they sit on is new. */}
        <div className="ezc-drawer" role="dialog" aria-modal="true">
          ...
          <div style={s.drawerTop}>
            {/* S98: the search box. Local only — what is typed here is matched against the
                conversations already on this device and is never sent anywhere. It is rendered
                before the entries so it is the first thing under the thumb. */}
            <div style={s.drawerSearchWrap}>
              <input
                type="search"
                value={chatQuery}
                onChange={(e) => setChatQuery(e.target.value)}
                placeholder={EZIK_SEARCH_PH}
                aria-label={EZIK_SEARCH_ARIA}
                className="ezik-focus"
                style={s.drawerSearch}
              />
            </div>
            <button onClick={() => closeDrawerWith(startChatFromMenu)} style={s.drawerItem}>
              ...
              <span>{ezT('chat.newConversation') || '\u0645\u062d\u0627\u062f\u062b\u0629 \u062c\u062f\u064a\u062f\u0629'}</span>
            </button>
            <button onClick={() => closeDrawerWith(() => setScreen('home'))} style={s.drawerItem}>
              <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="var(--ink)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <line x1="3" y1="6" x2="21" y2="6" />
                <line x1="3" y1="12" x2="21" y2="12" />
                <line x1="3" y1="18" x2="21" y2="18" />
              </svg>
              <span>{ezT('navigation.menu') || '\u0627\u0644\u0642\u0627\u0626\u0645\u0629'}</span>
            </button>
            {/* S98: \u00ab\u0627\u0644\u0645\u0641\u0636\u0644\u0629\u00bb. The count comes from state that was read once at mount, so
                opening the menu reads no store and parses no JSON. */}
            <button onClick={() => closeDrawerWith(() => openEzikSheet('favorites'))} style={s.drawerItem} className="ezik-focus">
              <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="var(--ink)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <path d="M12 17.3l-5.4 3 1-6-4.4-4.3 6-.9L12 3.5l2.8 5.6 6 .9-4.4 4.3 1 6z" />
              </svg>
            ...
            {/* S92: THE HISTORY. \u00ab\u0645\u062d\u0627\u062f\u062b\u0629 \u062c\u062f\u064a\u062f\u0629\u00bb stays the one and only new-chat entry -- this
                ...
                S112: it is still the store's order and still ONE mapping of it -- a pinned
                conversation is MARKED (.ezc-row.is-pinned) rather than moved into a second
                list, so nothing here re-sorts, filters or slices what the store returned.
                The section itself is absent while there is nothing to show; the empty line
                below takes its place. */}
            {/* S98: WHILE A SEARCH IS RUNNING the results replace the conversation list and
                nothing else on the menu moves. Opening a result goes through the SAME
                closeDrawerWith(openSavedChat) the ordinary row uses \u2014 so it spends the menu's
                history entry and lands at the end of the conversation exactly as S97 requires,
                with no second navigation path and no new scroll behaviour. */}
            {chatResults !== null && (
              <>
                <div style={s.drawerSectionLabel}>{EZIK_SEARCH_RESULTS}</div>
                {chatResults.length === 0 && <div style={s.drawerEmpty}>{EZIK_SEARCH_NONE}</div>}
                {chatResults.map((r) => (
                  <div key={r.id} className={r.id === chatId ? 'ezc-row is-on' : 'ezc-row'} style={s.drawerResultRow}>
                    <button onClick={() => closeDrawerWith(() => openSavedChat(r.id))} style={s.drawerResultBtn} title={r.title} className="ezik-focus">
                      <span style={s.drawerResultTitle}>{r.title}</span>
                      {r.snippet && <span style={s.drawerSnippet}>{r.snippet}</span>}
                      {r.at ? <span style={s.drawerResultDate}>{ezikShortDate(r.at)}</span> : null}
                    </button>
                  </div>
                ))}
              </>
            )}
            {chatResults === null && chatList.length > 0 && (
              <>
                <div style={s.drawerSectionLabel}>{'\u0627\u0644\u0645\u062d\u0627\u062f\u062b\u0627\u062a'}</div>
                {chatList.map((c) => (
                  <div key={c.id} className={'ezc-row' + (c.id === chatId ? ' is-on' : '') + (c.pinned ? ' is-pinned' : '')} style={s.drawerChatRow}>
                    {chatPendingDelete === c.id ? (
                      /* The confirmation, and it replaces the row it belongs to rather than
                         covering the menu -- so what is about to be deleted stays readable. */
                      <>
                        <span style={s.drawerConfirmText}>{'\u062d\u0630\u0641 \u0647\u0630\u0647 \u0627\u0644\u0645\u062d\u0627\u062f\u062b\u0629\u061f'}</span>
                        <button onClick={() => deleteSavedChat(c.id)} style={s.drawerConfirmYes}>{'\u062d\u0630\u0641'}</button>
                        <button onClick={() => setChatPendingDelete(null)} style={s.drawerConfirmNo}>{'\u0625\u0644\u063a\u0627\u0621'}</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => closeDrawerWith(() => openSavedChat(c.id))} style={s.drawerChatOpen} title={c.title}>
                          {c.pinned && (
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--red)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                              <path d="M12 17v5" /><path d="M9 10.8V5h6v5.8l2.5 3.2h-11L9 10.8z" />
                            </svg>
                          )}
                          <span style={s.drawerChatTitle}>{c.title}</span>
                        </button>
                        <button onClick={() => pinSavedChat(c.id)} style={s.drawerChatIcon} aria-label={c.pinned ? '\u0625\u0644\u063a\u0627\u0621 \u0627\u0644\u062a\u062b\u0628\u064a\u062a' : '\u062a\u062b\u0628\u064a\u062a'}>
                          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={c.pinned ? 'var(--red)' : 'var(--muted)'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M12 17v5" /><path d="M9 10.8V5h6v5.8l2.5 3.2h-11L9 10.8z" />
                          </svg>
                        </button>
                        <button onClick={() => setChatPendingDelete(c.id)} style={s.drawerChatIcon} aria-label={'\u062d\u0630\u0641'}>
            ...
            {/* S112: and when there is nothing saved the menu SAYS so, instead of ending in a
                blank space that reads as a list that failed to load. It is one line of the
                store's own vocabulary; no row, no id and no write. */}
            {chatResults === null && chatList.length === 0 && (
              <div style={s.drawerEmpty}>{EZIK_CHATS_EMPTY}</div>
            )}
          </div>
          {/* THE PINNED FOOTER, and it is ONE row now. It used to be two: the profile entry and,
              beside it, a small round button drawn as a ringed disc with eight rays. MEASURED:
              that button was not a theme toggle and touched no theme key -- its handler was
              closeDrawerWith(() => openEzikSheet('settings')), character for character the
              profile row's own handler, so the two controls opened the identical destination
              and the reader had to choose between them for nothing. The second one is gone.
  
              THE ACCESSIBLE NAME MOVED WITH IT. «الإعدادات» was that button's aria-label, and
              it is the name of what this row DOES -- it is the app's one door to the account
              area. It now sits on the row that survived, which is why a reader (and every
              guard that reaches settings by accessible name) still finds the entry exactly
  ...
  // S115: the boot mark is a bounded arch instead of a 48px emoji. The animation, its keyframe,
  ...
  // REFUSED, or a disclosure the reader has not answered: the two screens that SEND are replaced
  ...
  if ((screen === 'chat' || screen === 'call') && aiConsent !== EZ_AI_CONSENT_GRANTED) return (
    <AILocalModeNotice
      onReview={() => openEzikSheet('settings')}
      onMushaf={() => setScreen('mushaf')}
      onAdhkar={() => setScreen('adhkar')}
      onTreasure={() => { window.location.href = '/quest.html'; }}
      onBack={() => setScreen('home')}
    />
  );
  // S87: the home's chat entry is the FAB in its own bottom bar and nothing else -- the chat is
  ...
  // S87: the three sheets resolve their back through goEzikBack, which returns them to the screen
  ...
  if (screen === 'parentGate') return <ParentGate pinInput={pinInput} setPinInput={setPinInput} pinError={pinError} setPinError={setPinError} onSuccess={() => { setPinInput(''); setPinError(false); replaceEzikScreen('parentDashboard'); }} onBack={() => { setPinInput(''); setPinError(false); goEzikBack(); }} />;
  // S92: the dashboard reads the SAVED conversations, not the thread that happens to be open --
  ...
  if (screen === 'parentDashboard') return <ParentDashboard profile={profile} messages={ezikProfileTranscript(ezikProfileKey(profileRef.current))} onBack={goEzikBack} onReset={resetAll} directConvoLocked={directConvoLocked} onToggleDirectConvo={toggleDirectConvoLock} />;
  // D88/S90: the settings sheet, and it is now the ONE door to the account area -- the home
  ...
  if (screen === 'call' && childVoiceBlocked()) return <ChildVoiceNotice onBack={goEzikBack} />;
  // CALL UNLOCK (directive 82). Sits at the SCREEN transition, immediately AFTER the child
  ...
  const caps = deriveCaps(profile?.age);
  const directConvoAllowed = caps.band === 'young' ? !directConvoLocked : true;

  // S112: what the top rail says. It is the REAL title -- the one the store filed for the
  ...
  const ezcOpenChat = chatId ? chatList.find((c) => c.id === chatId) : null;
  // ITEM 94, THE REST OF IT. This line used to fall back to the app's own name, so an empty
  ...
  const ezcTitle = (ezcOpenChat && ezcOpenChat.title) ? ezcOpenChat.title : null;

  // Report button (step 2b). Opens the flag modal for a given assistant reply, capturing the
  ...
  const submitReport = async (reason, note) => {
    ...
    while (enc.encode(body).length > 20000 && n < 40) {
      if (n % 2 === 0) a = a.slice(0, Math.floor(a.length / 2));
      else u = u.slice(0, Math.floor(u.length / 2));
      body = build();
      n++;
    }
    try {
      const resp = await fetch('/api/report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
  ...
  return (
    <div className="theme-dark ezhome ezc" style={s.chatContainer}>
      {/* THE WATERMARK. One element, behind everything, reachable by nothing: aria-hidden so no
          reader is told about it, pointer-events:none so no tap can land on it, and z-index:-1
          inside this root's stacking context so it paints over the page colour and under every
          piece of chrome. The small crest that used to sit in the empty state is gone with it --
          the mark is the same mark, drawn once, large, and behind the whole conversation. */}
      {/* Item 62: `is-quiet` is added ONLY while the reader asked for it AND a reply is
          actually arriving. isLoading covers the wait before the first token and
          streamingText covers the writing itself, so the mark is back the moment the answer
          is finished -- it is a retreat, not a second stored strength. */}
      <div className={'ezwm' + ((wmAutoHide && (isLoading || streamingText !== null)) ? ' is-quiet' : '')} aria-hidden="true" />
      {/* D85: the chat top bar carries exactly ONE control -- the drawer menu. The old home,
          call, speaker and lock buttons are gone. Per-reply audio already sits beneath every
          reply, so the header speaker was redundant; the other three moved into the drawer
          (القائمة and the pinned profile/settings row). */}
      {/* S112: and it is a BOUNDED CARD on the page now, not the navy gradient slab that ran the
          full width of the viewport. Not one control was added or removed by that move: the same
          single button, the same openDrawer handler, the same accessible name. What it gained is
          a title, and the title is the real one -- see ezcTitle above. */}
      <div className="ezc-rail">
        <div className="ezc-rail-inner">
          <button onClick={openDrawer} className="ezc-icon" aria-label={'\u0641\u062a\u062d \u0627\u0644\u0642\u0627\u0626\u0645\u0629 \u0627\u0644\u062c\u0627\u0646\u0628\u064a\u0629'}
            {...(uiLang === 'ar' ? null : { 'aria-label': ezT('navigation.openMenu') })}>
      ...
      <div ref={messagesAreaRef} onScroll={onMessagesScroll} className={'ezc-scroll' + (pinnedAskIndex == null ? '' : ' ezc-askpinned')} style={s.messagesArea}>
        {/* S112 -- THE EMPTY STATE. A NEW thread shows this and nothing else IN THE TRANSCRIPT:
            no old messages, no history list, no card of any kind and no devotional text.
            ITEM 94: and no WORD either. It drew the app’s own name in 21px above the
            watermark, and the watermark behind the transcript already carries that name inside
            its arch -- the same name twice on one empty screen is once too often. The block
            itself stays exactly where it was, centred in the space that is already there, so the
            composer does not move and the screen never turns back into a second home. It is gone
            the moment there is anything to read, including the first token of a stream.
            The section suggestions added later are NOT here and are not part of this: they live
            in the composer dock, below the transcript, and they carry no devotional text. */}
        ...
        {/* S98: the quick actions. They are rendered ONCE, here, under the newest reply — not
            inside MessageBubble — so an old conversation does not carry a strip beneath every one
            of its 60 answers, and a keystroke in the composer cannot re-render 60 of them. They
            are laid out IN the scroller, above the end sentinel, so they scroll with the thread
            and can never cover the composer or the tool row. */}
        {/* §٢ (C): THE READER IS TOLD THE ANSWER STOPPED SHORT, IN A LINE, BESIDE «كمّل».
            It sits immediately above the strip that carries that button, so the fact and the act
            are one glance apart. `role="status"` because it is news about what is already on the
            screen — a screen reader announces it without being moved to it, and it is never
            focusable, because there is nothing here to press. Rendered from the SERVER'S signal
        ...
        {quickActionsVisible && (
          <div className="ez-hit" style={s.quickRow} role="group" aria-label={ezT("chat.quickActions")}>
            {EZIK_QUICK_ACTIONS.map((qa) => (
              <button
                key={qa.key}
                type="button"
                ...
                disabled={isLoading}
                className="ezik-focus"
                style={{ ...s.quickBtn, opacity: isLoading ? 0.5 : 1 }}
              >
                {qa.label}
              </button>
            ))}
        ...
        {/* STREAM-P4 §٣/١: the room that lets a SHORT answer still carry its question to the top.
            It is zero except while a turn is live, and it shrinks to zero by itself as a long
            answer fills the viewport, so on the common case it never exists at all. It sits
            ABOVE the end sentinel so that scrollIntoView on that sentinel — which is what the
            follow effect uses once the pin has let go — still lands at the true end. */}
        {/* `flexShrink: 0` is not decoration. This scroller is a COLUMN FLEXBOX — that is what the
            auto margin above it works through — and a flex item with a height and no content is
            shrinkable by default. MEASURED: the pin computed the room correctly and asked for
            410px, React rendered it, and flexbox took every pixel back the same frame; the
            element existed with an offsetHeight of 0 while `scrollHeight` never moved, so the
            spacer «did not work» in a way no amount of arithmetic would have explained. */}
        {askPinPad > 0 && <div ref={askPadElRef} aria-hidden="true" data-ezik-ask-pad="" style={{ height: askPinPad, flexShrink: 0 }} />}
        <div ref={messagesEndRef} />
      </div>

      {/* S112 -- THE COMPOSER DOCK. One bounded card on the same measure as the transcript,
          holding the pieces that were four separate full-width strips: the error banner, the
          attachment chip, the composer row, the tool row and the standing disclaimer. It is IN
          FLOW, exactly as those strips were, so it consumes its own height and can never cover
          the last reply -- there is no fixed positioning and no floating control on this screen.
          Nothing inside it was rewired: every handler, every disabled test and the Enter /
          Shift+Enter contract are the ones that shipped. */}
      <div className="ezc-dock">
        <div className="ezc-dock-inner">
      {/* ONE LIST, TWO SHAPES, and the shape is a class on the container -- the entries below are
          mapped ONCE whichever state is on, so a card and a pill can never drift apart or offer
          different destinations. Pressing one is a navigation and nothing else: no send, no
          write to the composer, no request. onMouseDown is defaulted away so that pressing a
          pill cannot blur the composer first, re-lay the strip as cards, and move the control
          out from under the pointer before the click lands. */}
      {sectionSuggestVisible && (
        <div className={composerFocused ? 'ezsug is-row' : 'ezsug'} role="group" aria-label={ezT('navigation.menu')}>
          {sectionSuggestions.map((sg) => (
            <button
              ...
              onClick={sg.go}
              className={composerFocused ? 'ezsug-pill ezik-focus' : 'ezsug-card ezik-focus'}
              data-ezik-section-suggestion={sg.id}
            >
              <span className="ezsug-mark" aria-hidden="true">{sg.icon}</span>
              {composerFocused ? <span>{sg.label}</span> : (
                <>
                  <span className="ezsug-body">
                    <span className="ezsug-title">{sg.label}</span>
                    <span className="ezsug-sub">{EZIST_SUB[sg.id]}</span>
                  </span>
                  <span className="ezsug-go" aria-hidden="true">{EZH_ICON_GO}</span>
                </>
              )}
            </button>
          ))}
      ...
      {voiceError && <div style={s.errorBanner}>{voiceError}</div>}

      {pendingImage && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px' }}>
          {(pendingImage.kind === 'image' || pendingImage.kind === undefined) ? (
            <img src={`data:${pendingImage.media_type};base64,${pendingImage.data}`} style={{ height: 56, borderRadius: 8, border: '1px solid var(--line)' }} />
          ) : (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 8, border: '1px solid var(--line)', background: 'var(--tint)', fontSize: 14 }}>
              <span>📄</span>
      ...
      <div style={s.toolBar}>
        <div style={s.toolGroup}>
          ...
          {/* Model selector. RENDERED FOR EVERY BAND so موجز is always available; the two deep
              ...
              one that shipped, except that item 84 lifted the token requirement from it. */}
          <button
            onClick={() => {
              if (caps.band !== 'adult') return;                  // 18+ visibility on the deep tiers is unchanged
              const next = depthMode === 'brief' ? 'detailed' : depthMode === 'detailed' ? (SCHOLAR_ENABLED ? 'scholar' : 'brief') : 'brief';
              // THE CLIENT NO LONGER ASKS FOR THE TOKEN HERE. The removed line tested the two
            ...
            disabled={isLoading || caps.band !== 'adult'}
      ...
      {/* S118: the one menu, defined once above the screen ladder. */}
      {ezikDrawer()}
      {reportFor && <ReportModal onClose={() => setReportFor(null)} onSubmit={submitReport} />}
    </div>
  );
}

// Per-message listen button (opt-in TTS). Mirrors VerseCard's play/stop pattern:
...
function MessageListenButton({ text, onPlayMessage, onStopAudio }) {
  const [playing, setPlaying] = useState(false);
  if (!text || !onPlayMessage) return null;
  const toggle = () => {
    if (playing) { if (onStopAudio) onStopAudio(); setPlaying(false); return; }
    setPlaying(true);
    Promise.resolve(onPlayMessage(text))
  ...
  return (
    <button
      type="button"
      onClick={toggle}
      aria-label={playing ? 'إيقاف الصوت' : 'استمع للرد'}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'transparent', color: 'var(--red)', border: '1px solid var(--line)', borderRadius: 8, padding: '4px 10px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}
    >
...
const REPLY_SERIALIZERS = {
  text: (sg, ctx) => (ctx.tashkeel ? sg.content : stripTashkeelOutsideQuran(sg.content)),
  verse: (sg) => {
    const sNum = resolveSurahNumber(sg.surah, sg.surahNum);
    const aNum = parseInt(sg.ayah, 10);
    const surahName = sg.surah || (sNum ? SURAH_NAMES[sNum] : '');
    const body = (sNum && aNum >= 1) ? getVerseText(sNum, aNum) : null;   // canonical only
    const meta = [surahName ? '\u0633\u0648\u0631\u0629 ' + surahName : '', sg.ayah ? '\u0622\u064A\u0629 ' + sg.ayah : '']
      .filter(Boolean).join('\u060C ');
    return REPLY_LINE(['\u0642\u064E\u0627\u0644\u064E \u0627\u0644\u0644\u0647\u064F \u062A\u064E\u0639\u064E\u0627\u0644\u064E\u0649', body, meta]);
  },
  surah: (sg) => {
    const sNum = resolveSurahNumber(undefined, sg.num);
    const surahName = sNum ? SURAH_NAMES[sNum] : '';
    const head = '\u0642\u064E\u0627\u0644\u064E \u0627\u0644\u0644\u0647\u064F \u062A\u064E\u0639\u064E\u0627\u0644\u064E\u0649' + (surahName ? ' \u2014 \u0633\u064F\u0648\u0631\u064E\u0629\u064F ' + surahName : '');
    const count = sNum ? getSurahAyahCount(sNum) : 0;
    let body = '', meta = '';
    if (count) {
      let f = parseInt(sg.from, 10); if (!(f >= 1)) f = 1;
      let t = parseInt(sg.to, 10); if (!(t >= 1) || t > count) t = count;
      if (f <= t) {
        const pieces = [];
        if (f === 1 && sNum !== 1 && sNum !== 9) { const b = getVerseText(1, 1); if (b) pieces.push(b); }
        let missing = false;
        for (let a = f; a <= t; a++) {
    ...
    return REPLY_LINE([head, body, meta]);
  },
  hadith: (sg) => {
    const att = resolveHadithAttribution(sg.narrator, sg.ruling);
    return REPLY_LINE([att.narrator ? '\u0631\u064E\u0648\u064E\u0649 ' + att.narrator : '\u0645\u0646 \u0627\u0644\u0633\u0646\u0629 \u0627\u0644\u0646\u0628\u0648\u064A\u0629', sg.content, att.ruling]);
  },
  dhikr: (sg) => {
    const db = (typeof __adhkarData !== 'undefined') ? __adhkarData : null;
    const list = (db && db.byCat && db.byCat[parseInt(sg.catId, 10)]) || [];
    if (!list.length) return '';
    const head = ((list[0] && list[0].category) || '\u0630\u0643\u0631') + ' \u2014 \u062D\u0635\u0646 \u0627\u0644\u0645\u0633\u0644\u0645';
    return REPLY_LINE([head].concat(list.map((d) => REPLY_LINE([
      d.text, d.repeat > 1 ? '\u062A\u064F\u0642\u0627\u0644 ' + toArabicDigits(d.repeat) + ' \u0645\u0631\u0651\u0627\u062A' : '',
    ]))));
  },
  worship: (sg, ctx) => {
    const d = (typeof __worshipData !== 'undefined') ? __worshipData : null;
    const arm = (ctx.band === 'young') ? 'young' : 'adult';   // byte-for-byte WorshipCard's ternary
    const cell = d && d.cells && d.cells[sg.id + ':' + arm];
    return (cell && cell.text) ? cell.text : '';
  },
  source: (sg) => {
    let host = (sg.site || '').trim();
    if (!host && sg.url) { try { host = new URL(sg.url).hostname.replace(/^www\./, ''); } catch (e) { host = ''; } }
    const label = host || '\u0627\u0644\u0645\u0635\u062F\u0631';
    const text = (sg.content || '').trim();
    // The url is the REFERENCE. A fiqh answer pasted into a message without it is incomplete,
  ...
  steps: (sg) => REPLY_LINE(((sg.title || '').trim() ? [(sg.title || '').trim()] : []).concat(
    (sg.items || []).map((it, i) => (i + 1) + '. ' + it))),   // matches the card's <ol> decimal marks
  board: (sg) => sg.content,
  // XI-04: THE CLIPBOARD CARRIES THE MEANING WITHOUT THE BRACKETS. The reader copied 【】 verbatim
...
const ezikCardSourceLine = (segments) => {
  const names = [];
  for (const sg of (segments || [])) {
    if (!sg || sg.type !== 'source') continue;
    let host = (sg.site || '').trim();
    if (!host && sg.url) { try { host = new URL(sg.url).hostname.replace(/^www\./, ''); } catch (e) { host = ''; } }
...
const serializeReply = (segments, ctx) => {
  const out = [];
  (segments || []).forEach((sg) => {
    const fn = sg && REPLY_SERIALIZERS[sg.type];
    if (!fn) {
      try { console.error('[copy] no serializer for segment type:', sg && sg.type); } catch (e) {}
      out.push('[' + ((sg && sg.type) || 'unknown') + ': not serialized]');
      return;
    }
    const piece = fn(sg, ctx || {});
    if (piece && String(piece).trim()) out.push(String(piece).trim());
  });
  return out.join('\n\n');
};
...
function ezikProseLength(segments) {
  let n = 0;
  for (let i = 0; i < (segments || []).length; i++) {
    if (segments[i] && segments[i].type === 'text') n += String(segments[i].content || '').length;
  }
  return n;
}

// Cut markdown at or before `budget`, on the safest boundary available. The 0.4 floors stop a
...
function ezikFoldCut(src, budget) {
  const t = String(src == null ? '' : src);
  if (t.length <= budget) return t;
  const window = t.slice(0, budget);
  let at = window.lastIndexOf('\n\n');
  if (at < budget * 0.4) { const nl = window.lastIndexOf('\n'); if (nl > at) at = nl; }
  if (at < budget * 0.4) { const sp = window.lastIndexOf(' '); if (sp > at) at = sp; }
  let cut = (at > 0 ? window.slice(0, at) : window).replace(/\s+$/, '');
  if (!cut) cut = window;
  // An odd number of fence lines means the cut fell INSIDE a code block: close it, or every
  // following character in the bubble is drawn as code.
  if (((cut.match(/^\s*```/gm) || []).length) % 2 === 1) cut += '\n```';
  return cut + ' …';
}

// The folded view, or null when there is nothing worth folding. Null is the signal the bubble
// reads to decide whether the toggle exists at all — a short reply never grows one.
function ezikFoldSegments(segments, minChars, headChars) {
  const list = segments || [];
  if (ezikProseLength(list) <= minChars) return null;
  let budget = headChars;
  let changed = false;
  const out = [];
  for (let i = 0; i < list.length; i++) {
    const sg = list[i];
    if (!sg || sg.type !== 'text') { out.push(sg); continue; }   // every card is kept, always
    const body = String(sg.content == null ? '' : sg.content);
    // A hidden segment leaves a HOLE rather than closing the gap: the bubble keys its children by
    ...
    if (budget <= 0) { changed = true; out.push(null); continue; }
    if (body.length <= budget) { out.push(sg); budget -= body.length; continue; }
    out.push({ type: 'text', content: ezikFoldCut(body, budget) });
    budget = 0;
    changed = true;
  }
...
let EZIK_QUICK_ACTIONS = [
  { key: 'simplify', label: ezT('chat.qa.simplify'),    prompt: 'بسّط لي الإجابة السابقة بكلمات أسهل، مع الحفاظ على المعنى والمصدر الشرعي إن وُجد.' },
  { key: 'example', label: ezT('chat.qa.example'),    prompt: 'أعطني مثالًا واضحًا ومختصرًا على الإجابة السابقة.' },
  { key: 'quiz', label: ezT('chat.qa.quiz'), prompt: 'اختبرني بسؤال واحد عن الإجابة السابقة، ولا تعرض الحل حتى أجيب.' },
...
const EZIK_ERROR_REPLIES = (function () {
  const out = [];
  Object.keys(FRIENDLY_ERRORS).forEach((k) => {
    const b = FRIENDLY_ERRORS[k];
    if (b && b.male) out.push(b.male);
    if (b && b.female) out.push(b.female);
  });
  return out;
})();
...
let EZIK_SHARE_ARIA = ezT("chat.shareAria");
let EZIK_SHARE_COPIED = ezT("common.copied");
let EZIK_SHARE_FAIL = ezT("common.copyFailed");
let EZIK_QUOTE_ARIA = ezT("chat.quoteAria");
function ezikBuildQuote(clean) {
  let t = String(clean == null ? '' : clean).replace(/\s+/g, ' ').trim();
  if (!t) return '';
  if (t.length > EZIK_QUOTE_MAX) {
    t = t.slice(0, EZIK_QUOTE_MAX).replace(/\s+\S*$/, '').replace(/\s+$/, '') + ' …';
  }
  return '« ' + t + ' »';
}
// WHERE A QUOTE LANDS. Kept as a pure rule rather than a line buried inside the handler, because
...
function ezikComposeWithQuote(prev, block) {
  const had = String(prev == null ? '' : prev);
  if (!block) return had;
  return had.trim() ? had.replace(/\s+$/, '') + '\n\n' + block + '\n' : block + '\n';
}
...
const EZ_MD_INLINE = [
  // EVERY delimiter must HUG its content: an opener is followed by a non-space and a closer is
  ...
  { re: /__(\S|\S[^\n]*?\S)__/, tag: 'strong' },
  { re: /~~(\S|\S[^\n]*?\S)~~/, tag: 'del' },
  // A single * is italic only between word boundaries, so a stray asterisk stays a stray asterisk.
  { re: /(^|[\s(\[«"'،؛:.!?؟])\*(\S|\S[^*\n]*?\S)\*(?=$|[\s)\]»"'،؛:.!?؟])/, tag: 'em', group: 2, pre: 1 },
];

// Inline pass: emit the EARLIEST match, recurse into its content, continue after it. Anything the
// rules do not claim is returned as a plain string child, which React escapes.
function ezMdInline(text, keyPrefix) {
  const out = [];
  ...
  while (rest && guard++ < 600) {
    let best = null;
    for (let i = 0; i < EZ_MD_INLINE.length; i++) {
      const rule = EZ_MD_INLINE[i];
      const mm = rule.re.exec(rest);
      if (!mm) continue;
      const at = mm.index + (rule.pre ? mm[rule.pre].length : 0);
      if (!best || at < best.at) best = { at: at, mm: mm, rule: rule };
    }
    if (!best) break;
    const head = rest.slice(0, best.at);
    if (head) out.push(head);
    const rule = best.rule;
    const inner = best.mm[rule.group || 1];
    const key = keyPrefix + 'i' + (k++);
    const consumed = best.mm[0].length - (rule.pre ? best.mm[rule.pre].length : 0);
    // Code spans are literal by definition — nothing inside them is a rule.
    out.push(React.createElement(rule.tag, { key: key, style: rule.literal ? s.mdCode : undefined },
      rule.literal ? inner : ezMdInline(inner, key)));
    rest = rest.slice(best.at + consumed);
  }
  if (rest) out.push(rest);
  return out;
}

// One paragraph line, and the <br/> that separates it from the next — a single newline inside a
// paragraph is a line break here, because a reply written for a child means it as one.
function ezMdLines(lines, keyPrefix) {
  const out = [];
  for (let i = 0; i < lines.length; i++) {
    if (i) out.push(React.createElement('br', { key: keyPrefix + 'br' + i }));
    const parts = ezMdInline(lines[i], keyPrefix + 'l' + i);
    for (let j = 0; j < parts.length; j++) out.push(parts[j]);
  }
  return out;
}

const EZ_MD_HR = /^\s*(?:-{3,}|\*{3,}|_{3,})\s*$/;
const EZ_MD_HEAD = /^\s*(#{1,6})\s+(.+?)\s*#*\s*$/;
const EZ_MD_UL = /^(\s*)[-*+]\s+(.*)$/;
const EZ_MD_OL = /^(\s*)(\d{1,9})[.)]\s+(.*)$/;
const EZ_MD_QUOTE = /^\s*>\s?(.*)$/;
const EZ_MD_FENCE = /^\s*```+\s*[A-Za-z0-9+#_-]*\s*$/;
// The delimiter row is what PROMOTES a line of pipes into a table. Until it arrives the header
// line is still an ordinary paragraph, which is exactly what a half-streamed table should be.
const EZ_MD_TABLE_SEP = /^\s*\|?\s*:?-{1,}:?\s*(\|\s*:?-{1,}:?\s*)*\|?\s*$/;

function ezMdIsBlockStart(line) {
  return !line.trim() || EZ_MD_HR.test(line) || EZ_MD_HEAD.test(line) || EZ_MD_UL.test(line) ||
         EZ_MD_OL.test(line) || EZ_MD_QUOTE.test(line) || EZ_MD_FENCE.test(line);
}
function ezMdSplitRow(row) {
  let r = String(row).trim();
  if (r.charAt(0) === '|') r = r.slice(1);
  if (r.charAt(r.length - 1) === '|') r = r.slice(0, -1);
  return r.split('|').map((c) => c.trim());
}
// LOGICAL alignment, not physical. The page is RTL, so a leading colon means "the edge the text
...
// A table only exists once the DELIMITER ROW has arrived AND declares the same number of columns
...
function ezMdTableAt(lines, i) {
  if (i + 1 >= lines.length) return null;
  const head = lines[i];
  const sep = lines[i + 1];
  if (head.indexOf('|') === -1 || sep.indexOf('-') === -1 || !EZ_MD_TABLE_SEP.test(sep)) return null;
  const cells = ezMdSplitRow(head);
  const align = ezMdSplitRow(sep).map(ezMdAlign);
  if (align.length !== cells.length) return null;
  return { head: cells, align: align };
}

// The block pass. Returns an array of React elements — never a string of markup.
function ezMdBlocks(text) {
  ...
  while (i < lines.length) {
    ...
    // fenced code — the body is literal, and an unclosed fence still renders what has arrived
    if (EZ_MD_FENCE.test(line)) {
      const body = [];
      i++;
      while (i < lines.length && !EZ_MD_FENCE.test(lines[i])) { body.push(lines[i]); i++; }
      if (i < lines.length) i++;
      const k = K();
      out.push(React.createElement('pre', { key: k, style: s.mdPre },
        React.createElement('code', null, body.join('\n'))));
      continue;
    }

    if (EZ_MD_HR.test(line)) { out.push(React.createElement('hr', { key: K(), style: s.mdHr })); i++; continue; }

    const h = line.match(EZ_MD_HEAD);
    if (h) {
      const lvl = h[1].length;
      const k = K();
      out.push(React.createElement('div', { key: k, style: (lvl <= 1 ? s.mdH1 : lvl === 2 ? s.mdH2 : s.mdH3), role: 'heading', 'aria-level': lvl },
        ezMdInline(h[2], k)));
      i++;
      continue;
    }

    // table — only once the delimiter row confirms it
    const tbl = ezMdTableAt(lines, i);
    if (tbl) {
      const head = tbl.head;
      const align = tbl.align;
      i += 2;
      const rows = [];
      while (i < lines.length && lines[i].trim() && lines[i].indexOf('|') !== -1 && !EZ_MD_HR.test(lines[i])) {
        rows.push(ezMdSplitRow(lines[i]));
        i++;
      }
      const k = K();
      out.push(React.createElement('div', { key: k, style: s.mdTableWrap },
        React.createElement('table', { style: s.mdTable },
          React.createElement('thead', null,
            React.createElement('tr', null, head.map((c, j) =>
              React.createElement('th', { key: j, style: Object.assign({}, s.mdTh, { textAlign: align[j] || 'right' }) },
                ezMdInline(c, k + 'h' + j))))),
          React.createElement('tbody', null, rows.map((r, ri) =>
            React.createElement('tr', { key: ri }, head.map((_, j) =>
              React.createElement('td', { key: j, style: Object.assign({}, s.mdTd, { textAlign: align[j] || 'right' }) },
                ezMdInline(r[j] == null ? '' : r[j], k + 'r' + ri + 'c' + j)))))))));
      continue;
    }

    // blockquote
    if (EZ_MD_QUOTE.test(line)) {
      const body = [];
      while (i < lines.length && EZ_MD_QUOTE.test(lines[i])) { body.push(lines[i].match(EZ_MD_QUOTE)[1]); i++; }
      const k = K();
      out.push(React.createElement('div', { key: k, style: s.mdQuote }, ezMdLines(body, k)));
      continue;
    }

    // lists — a following indented line continues the item it belongs to
    if (EZ_MD_UL.test(line) || EZ_MD_OL.test(line)) {
      // A line begins with either a digit or a bullet, never both, so the two cannot disagree.
      const isOrdered = EZ_MD_OL.test(line);
      const items = [];
      const startAt = isOrdered ? parseInt(line.match(EZ_MD_OL)[2], 10) : 1;
      while (i < lines.length) {
        const cur = lines[i];
        const mu = cur.match(EZ_MD_UL);
        const mo = cur.match(EZ_MD_OL);
        const sameKind = isOrdered ? (!!mo && !mu) : !!mu;
        if (sameKind) { items.push([isOrdered ? mo[3] : mu[2]]); i++; continue; }
        if (items.length && cur.trim() && /^\s{2,}/.test(cur) && !ezMdIsBlockStart(cur)) {
          items[items.length - 1].push(cur.trim()); i++; continue;
        }
        break;
      }
      const k = K();
      out.push(React.createElement(isOrdered ? 'ol' : 'ul',
        { key: k, style: isOrdered ? s.mdOl : s.mdUl, start: isOrdered && startAt !== 1 ? startAt : undefined },
        items.map((it, ii) => React.createElement('li', { key: ii, style: s.mdLi }, ezMdLines(it, k + 'i' + ii)))));
      continue;
    }
    ...
    const para = [line];
    i++;
    while (i < lines.length && lines[i].trim() && !ezMdIsBlockStart(lines[i]) && !ezMdTableAt(lines, i)) {
      para.push(lines[i]); i++;
    }
    const k = K();
    out.push(React.createElement('div', { key: k, style: s.mdP }, ezMdLines(para, k)));
  }
  return out;
}

// The component the bubble renders. Memoised on the string, because the whole thread re-renders
// on every streaming delta and a settled reply must not be re-parsed for each one.
const EzikMarkdown = React.memo(function EzikMarkdown({ text }) {
  const t = String(text == null ? '' : text);
  if (!t.trim()) return null;
  const blocks = ezMdBlocks(t);
  if (!blocks.length) return null;
  return React.createElement(React.Fragment, null, blocks);
});

// THE ONE PLACE A SEGMENT BECOMES AN ELEMENT.
...
function ezikRenderSegments(segments, ctx) {
  const { tashkeel, age, onPlayVerse, onPlaySurah, onStopAudio, onFavoriteAyah, ayahFavIds } = (ctx || {});
  return (segments || []).map((seg, i) => {
    if (!seg) return null;   // S98: a folded-away prose segment, holding its index open
    if (seg.type === 'text') {
      // S93: PROSE ONLY, and the last thing that happens to it. The cards were separated above,
    ...
    if (seg.type === 'worship') {
      return <WorshipCard key={i} id={seg.id} band={deriveCaps(age).band} />;
    }
    if (seg.type === 'steps') {
    ...
    if (seg.type === 'document') {
      return <DocumentCard key={i} title={seg.title} content={seg.content} age={age} />;
    }
    if (seg.type === 'notice') {
      return <ReviewNotice key={i} label={seg.label} content={seg.content} />;
    }
    return null;
  });
}

// S98 PERF: MEMOISED. Every prop below is either a primitive, the message object itself (whose
...
const MessageBubble = React.memo(function MessageBubble({ message, index, onSuggestionClick, onPlayVerse, onPlaySurah, onStopAudio, onPlayMessage, age, onReport, tashkeel, onToggleTashkeel, onQuote, onFavorite, isFavorite, onFavoriteAyah, ayahFavIds, defaultOpen, foldEpoch }) {
  ...
  const foldView = React.useMemo(
    () => (isUser ? null : ezikFoldSegments(segments, EZIK_FOLD_MIN_CHARS, EZIK_FOLD_HEAD_CHARS)),
    [isUser, segments]);
  // S112: THE TWO TURNS ARE DIFFERENT STRUCTURES, not two colours of one. A user turn is a
  ...
  if (isUser) {
    ...
    if (Array.isArray(c)) {
      const imgBlock = c.find(b => b && b.type === 'image');
      const txtBlock = c.find(b => b && b.type === 'text');
      return (
        <div className="ezc-turn is-user">
        <div className="ez-anim" style={{ ...s.messageBubble, ...s.userBubble, animation: 'fadeIn 0.3s ease-out' }}>
          {imgBlock && <img src={`data:${imgBlock.source.media_type};base64,${imgBlock.source.data}`} style={{ maxWidth: '100%', borderRadius: 10, marginBottom: txtBlock ? 8 : 0, display: 'block' }} />}
          {txtBlock && txtBlock.text}
        </div>
        </div>
      );
    }
    return <div className="ezc-turn is-user"><div className="ez-anim" style={{ ...s.messageBubble, ...s.userBubble, animation: 'fadeIn 0.3s ease-out' }}>{c}</div></div>;
  }
  // رسالة الأستاذ: بطاقاتها مفصولةٌ أعلاه قبل أن يعمل عارضُ Markdown
  ...
  const buildCopyText = () => serializeReply(segments, { tashkeel, band: deriveCaps(age).band });
  ...
  const canFold = !!foldView;
  const overrideActive = !!foldOverride && foldOverride.epoch === foldEpoch;
  const foldOpen = canFold && (overrideActive ? foldOverride.open : !!defaultOpen);
  const shownSegments = (canFold && !foldOpen) ? foldView : segments;
  return (
    <div className="ez-anim" style={{ animation: 'fadeIn 0.3s ease-out', display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: 8, maxWidth: '100%' }}>
      {/* S112: the reading sheet. Every segment the reply produced is still drawn here, in the
          order ezikRenderSegments returns them and by the SAME single renderer -- the prose, the
      ...
      <div className="ezc-ans" style={{ ...s.assistantBubble, maxWidth: '100%', /* 13.4-b2 */ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {canFold && (
          <button
            ...
            aria-label={ezT(foldOpen ? EZIK_FOLD_HIDE : EZIK_FOLD_SHOW)}
            className="ezik-focus"
            style={s.foldToggle}
          >
            <span>{ezT(foldOpen ? EZIK_FOLD_HIDE : EZIK_FOLD_SHOW)}</span>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, transform: foldOpen ? 'rotate(180deg)' : 'none' }}>
      ...
      <div className="ezc-acts" style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-start', padding: '0 2px' }}>
        ...
        <CopyReplyButton text={String(message.content || '').trim()} getText={buildCopyText} />
        {/* ITEM 42-أ: SHARE. The same payload the clipboard gets, the same rail, the same
            44x44 hit area every .ezc-acts button already takes from 44-ج. It calls nothing. */}
        <ShareReplyButton getText={buildCopyText} />
        {/* ITEM 42-ب: the same buildCopyText payload, rendered by the document card's own
        ...
        {onQuote && (
          <button type="button" onClick={() => onQuote(buildCopyText())} aria-label={EZIK_QUOTE_ARIA} className="ezik-focus" style={miniBtnStyle}>{EZIK_QUOTE_LABEL}</button>
        )}
        {/* S98: the star. `isFavorite` is a boolean handed DOWN — this component never reads a
            store, never parses JSON and never scans a list to draw it. */}
        {onFavorite && (
          <button
            type="button"
            onClick={() => onFavorite(message, index)}
            aria-pressed={isFavorite ? 'true' : 'false'}
            aria-label={isFavorite ? EZIK_FAV_DEL : EZIK_FAV_ADD}
            className="ezik-focus"
            style={{ ...miniBtnStyle, opacity: isFavorite ? 1 : 0.7, borderColor: isFavorite ? 'var(--red)' : 'var(--line)', color: isFavorite ? 'var(--red)' : 'inherit' }}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 17.3l-5.4 3 1-6-4.4-4.3 6-.9L12 3.5l2.8 5.6 6 .9-4.4 4.3 1 6z" />
            </svg>
          </button>
        )}
        <button type="button" onClick={onToggleTashkeel} aria-label={'\u062A\u0634\u0643\u064A\u0644'} style={{ ...miniBtnStyle, opacity: tashkeel ? 1 : 0.7, borderColor: tashkeel ? 'var(--red)' : 'var(--line)' }}>{'\u062A\u0634\u0643\u064A\u0644'}</button>
        <button type="button" onClick={() => onReport && onReport(index)} aria-label={ezT("chat.report")} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'transparent', color: 'var(--muted)', border: '1px solid var(--line)', borderRadius: 8, padding: '4px 10px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      {suggestions && suggestions.length > 0 && (
        <div style={s.suggestionsInline}>
          {suggestions.map((sg, i) => (
            <button key={i} onClick={() => onSuggestionClick && onSuggestionClick(sg)} style={s.suggestionChipSmall}>
              {sg}
            </button>
          ))}
        </div>
...
function FavoriteReplyBody({ segments, age, tashkeel }) {
  const [open, setOpen] = useState(false);
  const folded = React.useMemo(
    () => ezikFoldSegments(segments, EZIK_FOLD_MIN_CHARS, EZIK_FOLD_HEAD_CHARS),
    [segments]);
  const shown = (folded && !open) ? folded : segments;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {ezikRenderSegments(shown, { tashkeel, age })}
      {folded && (
        <button
          type="button"
          onClick={() => setOpen(!open)}
          aria-expanded={open ? 'true' : 'false'}
          aria-label={ezT(open ? EZIK_FOLD_HIDE : EZIK_FOLD_SHOW)}
          className="ezik-focus"
          style={{ ...s.foldToggle, alignSelf: 'flex-start' }}
        >
          <span>{ezT(open ? EZIK_FOLD_HIDE : EZIK_FOLD_SHOW)}</span>
        </button>
      )}
    </div>
  );
...
// S114: the same component, on istana chrome. It is handed the SAME props, in the SAME order the
...
function EzikFavKindNote({ f }) {
  const kind = ezikKindOf(f);
  if (kind === 'reply') return null;
  return (
    <React.Fragment>
      <div style={s.ezistMeta}>{ezT('favorites.kind.' + kind)}</div>
      {(f.title || f.ref) ? <div style={s.favMeta}>{f.title || f.ref}</div> : null}
    </React.Fragment>
  );
}
function EzikFavKindTabs({ kind, onKind, counts }) {
  return (
      <div style={s.themeRow} role="radiogroup" aria-label={ezT('favorites.kindAria')}>
        {[['all', ezT('favorites.kind.all')], ['reply', ezT('favorites.kind.reply')],
          ['fatwa', ezT('favorites.kind.fatwa')], ['ayah', ezT('favorites.kind.ayah')]].map(([v, label]) => (
          <button
            key={v}
            type="button"
            role="radio"
            aria-checked={kind === v ? 'true' : 'false'}
            onClick={() => onKind(v)}
            className="ezik-focus"
            style={{ ...s.a11yOpt, ...(kind === v ? s.themeOptActive : {}) }}
          >{label} {toArabicDigits((counts && counts[v]) || 0)}</button>
        ))}
      </div>
  );
}
function FavoritesScreen({ items, liveChatIds, onBack, onOpenChat, onRemove, age, tashkeel, query, onQuery, searching, total, kind, onKind, counts }) {
  return (
    <div className="theme-dark ezhome ezfav" style={s.favScreen}>
      <div className="ezfav-rail">
        <div className="ezfav-rail-inner">
          <button onClick={onBack} className="ezfav-back ezik-focus">{EZIK_BACK}</button>
          <span className="ezfav-brand">
            <span className="ezfav-brand-arch" aria-hidden="true" />
            <span className="ezfav-brand-text">{EZIK_FAV_HEADING}</span>
          </span>
        </div>
      </div>
      <div style={s.favBody}>
       <div className="ezfav-wrap">
        {/* S98/S114: the same local search over the records ALREADY IN MEMORY, in the masthead. */}
        {total > 0 && (
          <div className="ezfav-masthead">
            <span className="ezfav-crest" aria-hidden="true" />
            <EzikFavKindTabs kind={kind} onKind={onKind} counts={counts} />
            <div className="ezfav-mast-field">
            <input
              type="search"
              value={query}
              onChange={(e) => onQuery(e.target.value)}
              placeholder={EZIK_FAV_SEARCH_PH}
              aria-label={EZIK_FAV_SEARCH_ARIA}
              className="ezik-focus"
              style={s.drawerSearch}
            />
            </div>
        ...
        {total === 0 && (
          <div className="ezfav-empty">
            <span className="ezfav-empty-crest" aria-hidden="true"><span className="ezfav-empty-in" /></span>
            {ezLangGet() === 'ar'
              ? <div className="ezfav-empty-text">{EZIK_FAV_EMPTY}</div>
              : <div className="ezfav-empty-text">{ezT('favorites.empty')}</div>}
          </div>
        )}
        {total > 0 && searching && items.length === 0 && <div style={s.drawerEmpty}>{EZIK_SEARCH_NONE}</div>}
        <div className="ezfav-cat">
        {items.map((f) => {
          const parsed = parseRichMessage(f.text, age);
          const alive = !!f.chatId && liveChatIds.has(f.chatId);
          const when = ezikFavDate(f.at);
          return (
            <div key={f.id} className="ezfav-card" style={s.favCard}>
              <div className="ezfav-head">
                <span className="ezfav-head-mark" aria-hidden="true" />
                {when && <div style={s.favMeta}>{when}</div>}
              </div>
                <EzikFavKindNote f={f} />
              <div className="ezfav-read">
              <FavoriteReplyBody segments={parsed.segments} age={age} tashkeel={tashkeel} />
              </div>
              <div className="ezfav-foot" style={s.favRow}>
                <CopyReplyButton
                  text={String(f.text || '').trim()}
                  getText={() => serializeReply(parsed.segments, { tashkeel, band: deriveCaps(age).band })}
                />
                <button type="button" onClick={() => onRemove(f.id)} aria-label={EZIK_FAV_DEL} className="ezik-focus" style={s.favBtn}>
                  {EZIK_FAV_REMOVE}
                </button>
                {/* The original conversation. When it is gone the row SAYS so instead of
                    offering a button that would do nothing — and nothing here can throw on a
                    missing conversation, because the only thing consulted is a Set of ids.
                    Item 86: a fatwa and an ayah never had a conversation, so they are offered
                    neither the button nor the «المحادثة الأصلية محذوفة» line, which would be a
                    false statement about a record that never came from one. */}
                {ezikKindOf(f) !== 'reply' ? null : alive ? (
                  <button type="button" onClick={() => onOpenChat(f.chatId)} aria-label={EZIK_FAV_OPEN_CHAT} className="ezik-focus" style={s.favBtn}>
                    {EZIK_FAV_OPEN_CHAT}
                  </button>
                ) : (
                  <span style={{ ...s.favBtn, ...s.favBtnOff }}>{EZIK_FAV_CHAT_GONE}</span>
                )}
              </div>
            </div>
          );
...
// Flag-a-reply modal (step 2b). Copies AdultGate's overlay+card pattern (the app's one existing
...
function ReportModal({ onClose, onSubmit }) {
  ...
  const [status, setStatus] = useState('idle'); // idle | sending | ok | rate | fail
  const REASON_OPTIONS = [
    { key: 'wrong_info', label: 'معلومةٌ خاطئة' },
    { key: 'wrong_ruling', label: 'حكمٌ شرعيٌّ خاطئ' },
    { key: 'inappropriate', label: 'محتوًى غيرُ مناسب' },
    { key: 'other', label: 'شيءٌ آخر' },
  ];
  const send = async () => {
    if (!reason || status === 'sending') return;
    setStatus('sending');
    const r = await onSubmit(reason, note);
    setStatus(r);
  };
  const overlay = { position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'rgba(30,30,30,0.6)' };
  if (status === 'ok') {
    return (
      <div style={overlay}>
        <div style={{ ...s.onboardingCard, maxWidth: 360 }}>
          <div style={s.bigEmoji}>✅</div>
          <div style={s.onboardingTitle}>وصل بلاغُك. شكرًا لك.</div>
          <button onClick={onClose} style={s.primaryBtn}>إغلاق</button>
        </div>
      </div>
    );
  }
  return (
    <div style={overlay}>
      <div style={{ ...s.onboardingCard, maxWidth: 360 }}>
        <div style={s.bigEmoji}>🚩</div>
        <div style={s.onboardingTitle}>بلّغ عن هذا الردّ</div>
        <div style={s.onboardingSubtitle}>اختر السبب، وأضِف ملاحظةً إن شئت.</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%', margin: '4px 0 10px' }}>
          {REASON_OPTIONS.map(o => (
            <button key={o.key} type="button" onClick={() => setReason(o.key)}
              style={{ textAlign: 'right', padding: '10px 14px', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', fontSize: 15,
                border: reason === o.key ? '1px solid var(--red)' : '1px solid var(--line)',
                background: reason === o.key ? 'var(--tint)' : 'var(--white)', color: 'var(--ink)' }}>
              {o.label}
            </button>
          ))}
        </div>
        <textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={500} rows={3}
          placeholder="ملاحظة (اختياري)" style={{ ...s.onboardingInput, resize: 'none', textAlign: 'right', marginBottom: 12 }} />
        {status === 'rate' && <div style={{ color: 'var(--ink)', fontSize: 14, marginBottom: 10 }}>أرسلتَ بلاغاتٍ كثيرةً الآن. حاول بعد قليل.</div>}
        {status === 'fail' && <div style={{ color: 'var(--ink)', fontSize: 14, marginBottom: 10 }}>لم يصل بلاغُك. حاول لاحقًا.</div>}
        <button onClick={send} disabled={!reason || status === 'sending'} style={{ ...s.primaryBtn, opacity: (!reason || status === 'sending') ? 0.5 : 1 }}>
          {status === 'sending' ? '…' : 'أرسِل'}
        </button>
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 14, marginTop: 10, cursor: 'pointer', fontFamily: 'inherit' }}>إلغاء</button>
      </div>
    </div>
  );
}

// Item 86: the ayah is savable in its own right. `onFavorite` and `isFavorite` are handed
...
function VerseCard({ surah, surahNum, ayah, onPlayVerse, onStopAudio, onFavorite, isFavorite }) {
  ...
  // نصّ الآية يأتي حصراً من المصحف الكنسي المُحمَّل، لا من النموذج.
  const [verseText, setVerseText] = useState('');
  const [textState, setTextState] = useState('loading'); // loading | ok | fail
  const audioRef = useRef(null); // fallback محلي فقط (إن لم تُمرَّر دوال المنسِّق)

  const sNum = resolveSurahNumber(surah, surahNum);
  const aNum = parseInt(ayah, 10);
  const canPlay = !!sNum && aNum >= 1;
  const surahName = surah || (sNum ? SURAH_NAMES[sNum] : '');
  const pad3 = (x) => String(x).padStart(3, '0');
  const audioUrl = canPlay
    ? `https://everyayah.com/data/${QURAN_RECITER}/${pad3(sNum)}${pad3(aNum)}.mp3`
    : null;

  // جلب نصّ الآية الكنسي مرة واحدة (مُخزَّن في الذاكرة). عند الفشل: لا نصّ —
  // نعرض المرجع وزرّ التلاوة فقط، ولا نقع أبداً على نصّ النموذج أو ترميز خام.
  useEffect(() => {
    let alive = true;
    if (!canPlay) { setTextState('fail'); return; }
    loadQuran()
      .then(() => {
        if (!alive) return;
        const t = getVerseText(sNum, aNum);
        if (t) { setVerseText(t); setTextState('ok'); }
        else { setTextState('fail'); }
      })
      .catch(() => { if (alive) setTextState('fail'); });
    return () => { alive = false; };
  }, [sNum, aNum, canPlay]);

  // إيقاف صوت fallback المحلي وتنظيفه عند إزالة البطاقة
  useEffect(() => () => { if (audioRef.current) { audioRef.current.pause(); audioRef.current = null; } }, []);

  // إعادة تشغيل يدوية — عبر منسِّق App: يُوقف كلام المربّي أولاً ثم يُعيد التلاوة.
  // الزر الآن «تكرار» يدوي؛ التلاوة تُشغَّل تلقائياً ضمن تسلسل الردّ في App.
  const togglePlay = () => {
    // مسار المنسِّق الموحَّد (الاستخدام الفعلي في المحادثة)
    if (onPlayVerse) {
      if (playing) {
      ...
      setPlaying(true);
      Promise.resolve(onPlayVerse(sNum, aNum))
        .then((res) => { setPlaying(false); if (res && res.failed) setAudioFailed(true); })
        .catch(() => { setPlaying(false); setAudioFailed(true); });
      return;
    }
    // fallback محلي (لو رُكِّبت البطاقة خارج المحادثة بدون دوال المنسِّق)
    setAudioFailed(false);
    if (!audioRef.current) {
      const audio = new Audio(audioUrl);
      audio.onended = () => setPlaying(false);
      audio.onerror = () => { setPlaying(false); setAudioFailed(true); };
      audioRef.current = audio;
    }
    ...
    if (playing) {
      ...
      setPlaying(true);
      const p = audio.play();
      if (p && p.catch) p.catch(() => { setPlaying(false); setAudioFailed(true); });
    }
  };

  return (
    <div style={s.verseCard}>
      <div style={s.verseCardLabel}>
        <span>قَالَ اللهُ تَعَالَى</span>
      </div>
      {textState === 'ok' && <div style={s.verseText}>{verseText}</div>}
      <div style={s.verseFooter}>
        ...
        {onFavorite && textState === 'ok' && (
          <button
            ...
            className="ezik-focus"
            aria-pressed={isFavorite ? 'true' : 'false'}
            aria-label={isFavorite ? ezT('favorites.remove') : ezT('favorites.addAyah')}
            onClick={() => onFavorite({ surah: surahName, surahNum: sNum, ayah: aNum, text: verseText })}
            style={{ ...s.versePlayBtn, opacity: isFavorite ? 1 : 0.75 }}
          >
            <svg width="13" height="13" viewBox="0 0 24 24" fill={isFavorite ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              <path d="M12 17.3l-5.4 3 1-6-4.4-4.3 6-.9L12 3.5l2.8 5.6 6 .9-4.4 4.3 1 6z" />
            </svg>
            <span>{isFavorite ? ezT('favorites.saved') : ezT('favorites.addAyah')}</span>
          </button>
        )}
        {canPlay && (
          <button
            type="button"
            onClick={togglePlay}
            style={s.versePlayBtn}
            aria-label={playing ? 'إيقاف التلاوة' : 'تشغيل التلاوة'}
          >
            {playing ? <PauseIcon size={14} /> : <PlayIcon size={14} />}
            <span>{playing ? 'إيقاف' : 'استمع للتلاوة'}</span>
          </button>
        )}
      </div>
      {audioFailed && <div style={s.verseAudioError}>تعذّر تحميل التلاوة</div>}
    </div>
  );
}

// بطاقة السورة الكاملة (أو مدًى متّصل): نصّ واحد متّصل بأرقام آيات ۝، وزرّ تلاوة واحد.
// النصّ من المصحف الكنسي حصراً؛ عند الفشل نعرض الاسم والمدى فقط (لا نصّ نموذج ولا ترميز خام).
function SurahCard({ num, from, to, onPlaySurah, onStopAudio }) {
  const [playing, setPlaying] = useState(false);
  const [audioFailed, setAudioFailed] = useState(false);
  const [body, setBody] = useState('');
  const [state, setState] = useState('loading'); // loading | ok | fail
  const [range, setRange] = useState({ from: 0, to: 0, whole: false });

  const sNum = resolveSurahNumber(undefined, num);
  const surahName = sNum ? SURAH_NAMES[sNum] : '';

  useEffect(() => {
    let alive = true;
    if (!sNum) { setState('fail'); return; }
    loadQuran()
      .then(() => {
        if (!alive) return;
        const count = getSurahAyahCount(sNum);
        if (!count) { setState('fail'); return; }
        let f = parseInt(from, 10); if (!(f >= 1)) f = 1;
        let t = parseInt(to, 10); if (!(t >= 1) || t > count) t = count;
        if (f > t) { setState('fail'); return; }
        setRange({ from: f, to: t, whole: f === 1 && t === count });
        const pieces = [];
        // البسملة (للسورة كاملة من الآية ١) — غير مرقّمة، إلا الفاتحة (آية ١) والتوبة (لا بسملة).
        // نُعيد استخدام نصّ البسملة الكنسي getVerseText(1,1) — لا نكتب نصّاً قرآنياً يدوياً.
        if (f === 1 && sNum !== 1 && sNum !== 9) {
          const basmala = getVerseText(1, 1);
          if (basmala) pieces.push(basmala);
        }
        let missing = false;
        for (let a = f; a <= t; a++) {
          const vt = getVerseText(sNum, a);
  ...
  const togglePlay = () => {
    ...
    setAudioFailed(false);
    setPlaying(true);
    Promise.resolve(onPlaySurah(sNum, range.from || 1, range.to || null))
      .then((res) => { setPlaying(false); if (res && res.failed) setAudioFailed(true); })
      .catch(() => { setPlaying(false); setAudioFailed(true); });
  };
  ...
  return (
    <div style={s.verseCard}>
      <div style={s.verseCardLabel}>
        <span>قَالَ اللهُ تَعَالَى{surahName ? ` — سُورَةُ ${surahName}` : ''}</span>
      </div>
      {state === 'ok' && <div style={s.surahText}>{body}</div>}
      <div style={s.verseFooter}>
        {metaLabel && <div style={s.verseMeta}>{metaLabel}</div>}
        {sNum && (
          <button
            type="button"
            onClick={togglePlay}
            style={s.versePlayBtn}
            aria-label={playing ? 'إيقاف التلاوة' : 'تشغيل التلاوة كاملة'}
          >
            {playing ? <PauseIcon size={14} /> : <PlayIcon size={14} />}
            <span>{playing ? 'إيقاف' : 'استمع للتلاوة (كاملة)'}</span>
          </button>
        )}
      </div>
      {audioFailed && <div style={s.verseAudioError}>تعذّر تحميل التلاوة</div>}
...
const QURAN_ORNATE_SPAN_RE = /\uFD3F[\s\S]*?\uFD3E/;
...
const readStatedAyahRef = (t) => {
  const src = String(t || '');
  const found = [];
  const re = /سورة\s+([^\s،,:()\[\]\uFD3E\uFD3F]+(?:\s+[^\s،,:()\[\]\uFD3E\uFD3F]+)?)\s*[،,:]?\s*(?:الآية|آية)?\s*[:\s]*([٠-٩0-9]+)/g;
  let m;
  while ((m = re.exec(src)) !== null) {
    let name = m[1];
    if (SURAH_NUMBERS[name] === undefined) {
      // «سورة آل عمران، آية ١٧٣» captures two words; «سورة البقرة» captures one. Try the
      // shorter reading before giving up, so a two-word name does not swallow the next word.
      name = name.split(/\s+/)[0];
      if (SURAH_NUMBERS[name] === undefined) continue;
    }
    found.push({ surah: name, ayah: m[2] });
  }
...
// بطاقة المصدر: شريحةُ عزوٍ قابلةٌ للنقر في نهاية الجواب — أوّلُ رابطٍ خارجيٍّ في التطبيق.
// تُظهِر اسمَ النطاق (site) ثمّ وصفاً عربيّاً موجزاً (المحتوى)، وتفتح url في تبويبٍ جديد.
function SourceCard({ site, url, content }) {
  // D92: this chip is the app's ONLY external navigation -- tapping it leaves for the system
  ...
  // اسمُ النطاق للعرض: نُفضّل site، وإلا نشتقّه من رابط الصفحة إن أمكن.
  let host = (site || '').trim();
  if (!host && url) {
    try { host = new URL(url).hostname.replace(/^www\./, ''); } catch (e) { host = ''; }
  }
  const label = host || 'المصدر';
  const text = (content || '').trim() || host || url || '';
  const body = [
    <span key="site" style={s.sourceChipSite}>{label}</span>,
    text ? <span key="text" style={s.sourceChipText}>{text}</span> : null,
    <span key="arrow" style={s.sourceChipArrow} aria-hidden="true">↗</span>,
  ];
  // حمايةٌ من رابطٍ مفقود: بلا url نرسم شريحةً نصّيّةً غير قابلةٍ للنقر — لا <a href=""> أبداً.
  if (!url) {
    return <div style={s.sourceChip}>{body}</div>;
  }
  const chipStyle = { ...s.sourceChip, ...s.sourceChipLink };
  if (!(PARENTAL_GATE_ENABLED && childProfileActive())) {
    return (
      <a href={url} target="_blank" rel="noopener noreferrer" style={chipStyle}>
        {body}
      </a>
    );
  }
  // The href stays real, so long-press / copy-link still behave. Only the tap is intercepted:
  // the navigation happens in onPass, which is itself a button click, so no popup blocker fires.
  return (
    <>
      <a href={url} target="_blank" rel="noopener noreferrer" style={chipStyle}
        onClick={(e) => { e.preventDefault(); setGate({ a: 2 + Math.floor(Math.random() * 8), b: 2 + Math.floor(Math.random() * 8) }); }}>
        {body}
      </a>
      {gate && <AdultGate a={gate.a} b={gate.b}
...
function DhikrCard({ catId }) {
  ...
  useEffect(() => {
    ...
    loadAdhkar()
      .then((db) => {
        if (!alive) return;
        const list = (db.byCat && db.byCat[parseInt(catId, 10)]) || [];
        setItems(list);
        if (list[0]) setTitle(list[0].category || '');
      })
  ...
  if (items === null) return <div style={s.hadithCard}><div style={s.hadithText}>…</div></div>;
  if (items.length === 0) return null;
  return (
    <div style={s.hadithCard}>
      <div style={s.hadithCardLabel}>
        <span>{title || 'ذكر'} — حصن المسلم</span>
      </div>
      {items.map((d, i) => (
        <div key={d.id} style={{ marginBottom: i < items.length - 1 ? 10 : 0 }}>
          <div style={s.hadithText}>{d.text}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4, flexWrap: 'wrap' }}>
            {d.repeat > 1 && <span style={s.hadithMeta}>تُقال {toArabicDigits(d.repeat)} مرّات</span>}
            {d.audio && (
              <button type="button" onClick={() => toggle(d)} style={s.versePlayBtn} aria-label={playingId === d.id ? 'إيقاف' : 'استماع'}>
                {playingId === d.id ? <PauseIcon size={14} /> : <PlayIcon size={14} />}
                <span>{playingId === d.id ? 'إيقاف' : 'استماع'}</span>
              </button>
            )}
          </div>
        </div>
...
function WorshipCard({ id, band }) {
  const [display, setDisplay] = useState(null);
  const [failed, setFailed] = useState(false);
  useEffect(() => {
    let alive = true;
    loadWorship()
      .then((d) => { if (alive) setDisplay(d); })
      .catch(() => { if (alive) setFailed(true); });
    return () => { alive = false; };
  }, []);
  // arm: byte-for-byte the prompt's own ternary (band === 'young'). A teen (13-17) takes the
  // ADULT arm. That is TODAY's behaviour — this commit changes ZERO policy.
  const arm = (band === 'young') ? 'young' : 'adult';
  // FAIL CLOSED: a missing cell renders a VISIBLE error, never model text, never empty silence.
  const errorCard = (why) => (
    <div style={{ background: 'var(--tint)', border: '1px solid var(--red)', borderRadius: 10, padding: '12px 14px', color: 'var(--red)', direction: 'ltr', textAlign: 'left', fontSize: 13, fontFamily: 'monospace' }}>
      worship card unavailable — {why}
    </div>
  );
  if (failed) return errorCard('display fetch failed');
  if (display === null) return <div style={s.hadithCard}><div style={s.hadithText}>…</div></div>;
  const cell = display.cells && display.cells[id + ':' + arm];
  if (!cell || !cell.text) return errorCard('no cell for ' + id + ':' + arm);
  // Render cell.text VERBATIM: no markdown pass, no tashkeel pass, no age simplification,
  // no truncation, line breaks preserved (pre-wrap). The text is authored+guarded, not model output.
  return (
    <div style={s.stepsCard}>
      <div style={s.stepsCardLabel}>
        <ListCheckIcon size={12} color="var(--red)" />
      </div>
      <div style={{ ...s.hadithText, whiteSpace: 'pre-wrap' }}>{cell.text}</div>
    </div>
  );
}

// العنوانُ يأتي من الجوابِ نفسِه (خاصّيّةُ title على الوسم) — وإن غابَ فلا عنوانَ أصلًا،
...
function StepsCard({ items, title }) {
  ...
  return (
    <div style={s.stepsCard}>
      ...
      <ol style={s.stepsList}>
        {items.map((item, i) => (
          <li key={i} style={s.stepsItem}>{item}</li>
        ))}
      </ol>
    </div>
  );
...
// XI-04 — الشارةُ التي حلّت محلَّ المحارفِ الخام. It is drawn like the cards beside it and it
...
function ReviewNotice({ label, content }) {
  const body = String(content || '').trim();
  return (
    <div role="note" style={s.reviewNotice}>
      <span style={s.reviewNoticeLabel}>{label}</span>
      {body ? <span style={s.reviewNoticeText}>{body}</span> : null}
    </div>
  );
}

function BoardCard({ content }) {
  return (
    <div style={{
      direction: 'ltr', textAlign: 'left', background: 'var(--tint)',
      border: '1px solid var(--line)', borderRadius: 10, padding: '12px 14px',
      fontFamily: "'Courier New', monospace", fontSize: 15, lineHeight: 1.9,
      color: 'var(--ink)', whiteSpace: 'pre-wrap', overflowX: 'auto', width: '100%',
    }}>
      {content}
    </div>
  );
...
function DocumentCard({ title, content, age }) {
  ...
  return (
    <div style={{ border: '1px solid var(--line)', borderRadius: 12, padding: '14px 16px', background: 'var(--white)', width: '100%' }}>
      <div style={{ fontWeight: 'bold', color: 'var(--ink)', fontSize: 17, marginBottom: 8 }}>{title}</div>
      <div className="doc-rendered" style={{ fontSize: 14.5, lineHeight: 1.9, color: 'var(--ink)', maxHeight: 320, overflowY: 'auto' }} dangerouslySetInnerHTML={{ __html: docToHtml(content) }} />
      {canExport && (<>
      <button onClick={exportDoc} style={{ marginTop: 12, background: 'var(--red)', color: 'var(--on-accent)', border: 'none', borderRadius: 10, padding: '9px 16px', fontSize: 15, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--on-accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M12 18v-6M9 15h6"/></svg>
        صدِّر كملف Word
      </button>
      <button onClick={exportPdf} style={{ marginTop: 12, marginRight: 8, background: 'transparent', color: 'var(--red)', border: '1px solid var(--red)', borderRadius: 10, padding: '9px 16px', fontSize: 15, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
...
// childProfileActive -- is the ACTIVE stored profile a child (young band)? SourceCard is a leaf
...
function childProfileActive() {
  try {
    const raw = localStorage.getItem('child_profile');
    if (!raw) return true;
    return deriveCaps(JSON.parse(raw).age).band === 'young';
  } catch (e) { return true; }
}

const PARENTAL_GATE_ENABLED = true; // Session 08: gate off during testing - MUST be true before store release
function AdultGate({ a, b, onPass, onCancel }) {
  const [answer, setAnswer] = useState("");
  const [error, setError] = useState(false);
  const submit = () => {
    const v = parseInt(String(answer).replace(/[٠-٩]/g, x => x.charCodeAt(0) - 0x0660), 10);
    if (v === a * b) onPass();
    else { setError(true); setTimeout(() => setError(false), 1500); }
  };
  return (
    <div className="theme-dark ezhome ezgate ezgate-scrim">
      <div className="ezgate-card is-modal" style={s.onboardingCard}>
        <div className="ezgate-crest" aria-hidden="true"><span style={s.bigEmoji}>🔒</span></div>
        <div style={s.onboardingTitle}>تأكيد ولي الأمر</div>
        <div style={s.onboardingSubtitle}>هذا القسم مخصّص للكبار. للتأكيد أنّ وليًّا حاضر، احسب الناتج:</div>
        <div className="ezgate-sum">{a} × {b}</div>
        <input type="text" inputMode="numeric" value={answer} onChange={(e) => setAnswer(e.target.value)} onKeyDown={(e) => e.key === "Enter" && submit()} placeholder="الجواب" autoFocus style={{ ...s.onboardingInput, textAlign: "center", borderColor: error ? "var(--a3-cyan)" : "var(--a3-line)" }} />
        {error && <div className="ezgate-err">جواب غير صحيح، حاول مرة أخرى</div>}
        <button onClick={submit} style={s.primaryBtn}>تأكيد</button>
...
const EZ_AIC_DATA = [
  ...
  'الصور أو الملفات التي تختار رفعها.',
...
const EZ_AIC_PROVIDERS = [
  ...
  ['Brave Search', 'إرسال عبارات بحث مشتقّة من السؤال للوصول إلى مصادر الويب.'],
];
// The first line was «لا تُستخدم هذه البيانات للإعلانات ولا لتتبُّعك» -- an absolute passive that
...
const EZ_AIC_ASSURANCES = [
  'لا يستخدم عزك هذه البيانات للإعلانات أو لتتبعك، ولا يرسلها إلى مزودي الخدمة إلا لتشغيل الميزات التي تختار استخدامها.',
...
function AIConsentVersion({ style }) {
  return (
    <div style={style}>
      {EZ_AIC_VERSION_LABEL + ' '}
      <span style={s.aicVersionNum}>{EZ_AI_CONSENT_VERSION}</span>
    </div>
  );
}

// The three policy links, drawn identically wherever they appear.
function AIConsentLinks() {
  return (
    <div style={s.aicLinks}>
      <a href="/privacy.html" target="_blank" rel="noopener noreferrer" style={s.aicLink}>{EZ_AIC_LINK_PRIVACY}</a>
      <a href="/delete.html" target="_blank" rel="noopener noreferrer" style={s.aicLink}>{EZ_AIC_LINK_DELETE}</a>
      <a href="/support.html" target="_blank" rel="noopener noreferrer" style={s.aicLink}>{EZ_AIC_LINK_SUPPORT}</a>
    </div>
  );
}
...
function AIConsentGate({ age, current, onGrant, onDecline, onBack }) {
  ...
  const [guard, setGuard] = useState(null);   // the arithmetic challenge, or null
  const agree = () => {
    if (!isUnder13) { onGrant('user'); return; }
    setGuard({ a: 2 + Math.floor(Math.random() * 8), b: 2 + Math.floor(Math.random() * 8) });
  };
  if (guard) {
    return <AdultGate a={guard.a} b={guard.b}
      onPass={() => { setGuard(null); onGrant('guardian'); }}
    ...
    <div className="theme-dark ezhome ezgate" style={s.onboardingContainer}>
      <div className="ezgate-wrap"><div className="ezgate-card" style={s.onboardingCard}>
        ...
        <div style={s.aicBody}>
          ...
          <div style={s.aicSubhead}>{EZ_AIC_PROVIDERS_TITLE}</div>
          <ul style={s.aicList}>
            {EZ_AIC_PROVIDERS.map(([n, d]) => (
              <li key={n} style={s.aicItem}><span style={s.aicProvider}>{n}</span>{': ' + d}</li>
            ))}
          </ul>
          <ul style={s.aicList}>
            {EZ_AIC_ASSURANCES.map((t) => <li key={t} style={s.aicItem}>{t}</li>)}
          </ul>
...
function AILocalModeNotice({ onReview, onMushaf, onAdhkar, onTreasure, onBack }) {
  return (
    <div className="theme-dark ezhome ezgate" style={s.onboardingContainer}>
      <div className="ezgate-wrap"><div className="ezgate-card" style={s.onboardingCard}>
        <div className="ezgate-crest" aria-hidden="true"><span style={s.bigEmoji}>🔒</span></div>
        <div style={s.onboardingTitle}>{EZ_AILM_TITLE}</div>
        <div style={{ ...s.onboardingSubtitle, lineHeight: 1.9, marginBottom: 20 }}>{EZ_AILM_BODY}</div>
        <button type="button" onClick={onReview} style={s.primaryBtn}>{EZ_AILM_REVIEW}</button>
        <button type="button" onClick={onMushaf} style={s.secondaryBtn}>{EZ_AILM_MUSHAF}</button>
        <button type="button" onClick={onAdhkar} style={s.secondaryBtn}>{EZ_AILM_ADHKAR}</button>
        <button type="button" onClick={onTreasure} style={s.secondaryBtn}>{EZ_AILM_TREASURE}</button>
        <button type="button" onClick={onBack} style={s.secondaryBtn}>{A2_BACK}</button>
      </div></div>
    </div>
  );
}

// غ‑٣ — بديلُ شاشةِ المكالمة حين يكون الصوتُ مقفولاً. يحتذي شكلَ DisclosureGate بالضبط:
// رموزُ التنسيق القائمة فقط، بلا أيّ لونٍ جديد. نصٌّ واحدٌ ثابت + زرُّ رجوعٍ ظاهر.
function ChildVoiceNotice({ onBack }) {
  return (
    <div className="theme-dark ezhome ezgate" style={s.onboardingContainer}>
      <div className="ezgate-wrap"><div className="ezgate-card" style={s.onboardingCard}>
        <div className="ezgate-crest" aria-hidden="true"><span style={s.bigEmoji}>🔈</span></div>
        <div style={s.onboardingTitle}>الصوتُ قيدَ التجهيز</div>
        <div style={{ ...s.onboardingSubtitle, lineHeight: 1.9, marginBottom: 20 }}>{CHILD_VOICE_NOTICE}</div>
        <button onClick={onBack} style={s.primaryBtn}>رجوع</button>
      </div></div>
    </div>
  );
}

// ONE PIN sheet, reached from two places: selecting a locked depth tier, and entering the
...
function UnlockSheet({ onUnlocked, onBack }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (r.ok && d && d.token) { storeFounderToken(d.token); setPin(''); onUnlocked(); return; }
      // The SERVER owns the wording. A wrong code, a day lockout and an unreachable store are
      // three different truths and only it knows which -- so show its message verbatim and
      // never an invented generic line.
      setMsg((d && d.message) || '');
    } catch (e) {
      // A network failure carries no server message; leave the sheet up rather than invent a
  ...
  return (
    <div className="theme-dark ezhome ezgate" style={s.onboardingContainer}>
      <div className="ezgate-wrap"><div className="ezgate-card" style={s.onboardingCard}>
        <div className="ezgate-crest" aria-hidden="true"><span style={s.bigEmoji}>{'\uD83D\uDD10'}</span></div>
        <div style={s.onboardingTitle}>{'\u0627\u0644\u0631\u0645\u0632 \u0645\u0637\u0644\u0648\u0628'}</div>
        <div style={s.onboardingSubtitle}>{'\u0623\u062F\u062E\u0644 \u0631\u0645\u0632 \u0627\u0644\u0641\u062A\u062D \u0644\u0644\u0645\u062A\u0627\u0628\u0639\u0629.'}</div>
        <input type="password" inputMode="numeric" value={pin} onChange={(e) => setPin(e.target.value)}
          onKeyDown={(e) => e.key === 'Enter' && submit()} placeholder="••••" autoFocus
          style={{ ...s.onboardingInput, textAlign: 'center', letterSpacing: '0.5em', borderColor: msg ? 'var(--a3-cyan)' : 'var(--a3-line)' }} />
        {msg && <div className="ezgate-err">{msg}</div>}
        <button onClick={submit} disabled={busy} style={{ ...s.primaryBtn, opacity: busy ? 0.5 : 1 }}>{'\u0641\u062A\u062D'}</button>
        <button onClick={onBack} style={s.secondaryBtn}>{'\u0631\u062C\u0648\u0639'}</button>
      </div></div>
    </div>
  );
}
...
function ParentGate({ pinInput, setPinInput, pinError, setPinError, onSuccess, onBack }) {
  ...
  const [busy, setBusy] = useState(false);
  const [errMsg, setErrMsg] = useState('رمز خاطئ');
  const fail = (msg) => { setErrMsg(msg); setPinError(true); setTimeout(() => setPinError(false), 1800); };
  useEffect(() => {
    let alive = true;
    (async () => {
      let serverHas = null;                               // null = لم نعرف ⇐ نُعامِلُه معاملةَ «عنده رمز»
      try {
      ...
      // جهازٌ يحملُ بصمةً قديمةً ولا سجلَّ له ⇐ وضعُ تحقُّقٍ لتجريَ الهجرةُ الصامتة، لا وضعُ
      // إنشاءٍ يطلبُ رمزًا جديدًا من أهلٍ يحفظونَ رمزَهم.
      const needVerify = serverHas !== false || !!readLegacyParentHash();
      if (alive) setMode(needVerify ? 'verify' : 'create');
    })();
    return () => { alive = false; };
  }, []);
  const verify = async () => {
    if (busy || mode === 'probing') return;
    setBusy(true);
    try {
      const legacyHash = readLegacyParentHash();
      const r = await parentCodeCall(Object.assign({ action: 'verify', pin: pinInput }, legacyHash ? { legacyHash } : {}));
      const d = await r.json().catch(() => ({}));
      setBusy(false);
      if (r.ok && d && d.ok) { clearLegacyParentHash(); setPinInput(''); onSuccess(); return; }
      // الخادمُ يملكُ الصياغة: رمزٌ خاطئ، وقفلُ محاولاتِ اليوم، وتعذُّرُ التحقُّقِ ثلاثُ حقائقَ
      // مختلفة، وهو وحدَه يعرفُ أيَّها وقع — فتُعرَضُ رسالتُه كما هي.
      fail((d && d.message) || 'رمز خاطئ');
    } catch (e) {
      setBusy(false);
      // انقطاعُ الشبكةِ لا يحملُ رسالةَ خادم، ولومُ الأهلِ بـ«رمزٍ خاطئ» كذبٌ عليهم.
      fail('تعذّر التحقّق الآن. جرّب بعد قليل.');
    }
  };
  const create = async () => {
    if (busy) return;
    if (!/^[0-9]{4,}$/.test(pinInput)) return fail('اختر ٤ أرقام على الأقل');
    if (pinInput !== confirmPin) return fail('الرمزان غير متطابقان');
    setBusy(true);
    try {
      const r = await parentCodeCall({ action: 'set', pin: pinInput });
      const d = await r.json().catch(() => ({}));
      setBusy(false);
      if (r.ok && d && d.ok) { clearLegacyParentHash(); setConfirmPin(''); onSuccess(); return; }
      fail((d && d.message) || 'تعذّر الحفظ');
    } catch (e) { setBusy(false); fail('تعذّر الحفظ'); }
  };
  const hasPin = mode !== 'create';
  ...
  if (mode === 'create' && !adultOk && PARENTAL_GATE_ENABLED && childProfileActive()) return <AdultGate a={challenge.a} b={challenge.b} onPass={() => setAdultOk(true)} onCancel={onBack} />;
  return (
    <div className="theme-dark ezhome ezgate" style={s.onboardingContainer}>
      <div className="ezgate-wrap"><div className="ezgate-card" style={s.onboardingCard}>
        <div className="ezgate-crest" aria-hidden="true"><span style={s.bigEmoji}>🔒</span></div>
        ...
        {/* مُعطَّلٌ ريثما نعرفُ من الخادمِ أيَّ وضعٍ نحن فيه، وريثما يعودُ نداءٌ جارٍ: ضغطةٌ
            مزدوجةٌ على «دخول» محاولتان تُحسَبان على الجهازِ والعنوانِ معًا. */}
        <button onClick={hasPin ? verify : create} disabled={busy || mode === 'probing'} style={{ ...s.primaryBtn, opacity: (busy || mode === 'probing') ? 0.5 : 1 }}>{hasPin ? 'دخول' : 'حفظ الرمز'}</button>
        <button onClick={onBack} style={s.secondaryBtn}>رجوع</button>
      </div></div>
    </div>
  );
...
// قفل الإنفاق — مطالبةٌ بسيطة على نمط لوحة الأهل. تُقارَن بصمة SHA-256 للرمز المُدخَل
// بالثابت SPEND_GATE_SHA256؛ لا يُخزَّن الرمز نفسه أبداً. النجاح يفتح القفل ويُحفَظ لهذا الجهاز.
function SpendGate({ onUnlock, onExit }) {
  const [code, setCode] = useState('');
  const [err, setErr] = useState(false);
  const submit = async () => {
    try { if ((await hashPin(code)) === SPEND_GATE_SHA256) { onUnlock(); return; } } catch (e) {}
  ...
  return (
    <div className="theme-dark ezhome ezgate" style={s.onboardingContainer}>
      <div className="ezgate-wrap"><div className="ezgate-card" style={s.onboardingCard}>
        <div className="ezgate-crest" aria-hidden="true"><span style={s.bigEmoji}>🔒</span></div>
        <div style={s.onboardingTitle}>رمز الدخول</div>
        <div style={s.onboardingSubtitle}>أدخل رمز الدخول لتفعيل المحادثة</div>
        <input type="password" inputMode="numeric" value={code} onChange={(e) => setCode(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit()} placeholder="••••" style={{ ...s.onboardingInput, textAlign: 'center', letterSpacing: '0.5em', borderColor: err ? 'var(--a3-cyan)' : 'var(--a3-line)' }} autoFocus />
        {err && <div className="ezgate-err">رمز خاطئ</div>}
        <button onClick={submit} style={s.primaryBtn}>دخول</button>
        <button onClick={onExit} style={s.secondaryBtn}>رجوع</button>
      </div></div>
    </div>
  );
}

// D88 -- the settings sheet. It carries .theme-dark itself so it is themed by the same scoped
    ...
    <div className="theme-dark ezhome" style={s.ezshContainer}>
      <div className="ezsh-nav">
        <div className="ezsh-nav-inner">
          ...
          <div className="ezsh-nav-side is-end">{actions || null}</div>
        </div>
      </div>
      <div style={s.ezshScroll}>
        <div className="ezsh-wrap">{children}</div>
      </div>
    </div>
  );
}
...
const PRAYER_OFFSETTABLE = ['fajr', 'dhuhr', 'asr', 'maghrib', 'isha'];
...
function prayerMethodTable() {
  return {
    ...
    karachi: { name: 'جامعة العلوم الإسلاميّة بكراتشي', fajr: 18, isha: 18, ishaMin: 0 },
    isna: { name: 'أمريكا الشماليّة (ISNA)', fajr: 15, isha: 15, ishaMin: 0 },
    qatar: { name: 'قطر', fajr: 18, isha: 0, ishaMin: 90 },
  };
}
function prayerMethodIds() {
  const t = prayerMethodTable();
  const out = [];
  for (const k in t) if (Object.prototype.hasOwnProperty.call(t, k)) out.push(k);
  return out;
}
function prayerMethodOf(id) {
  const t = prayerMethodTable();
  return Object.prototype.hasOwnProperty.call(t, id) ? t[id] : t[PRAYER_METHOD_DEFAULT];
}

// The sun, at a Julian date. Declination in degrees, equation of time in hours wrapped into
// (−12, 12] so that a mean longitude near the wrap cannot produce a day-long error.
function prayerSunPosition(jd) {
  const R = Math.PI / 180;
  const D = jd - 2451545.0;
  const g = ((357.529 + 0.98560028 * D) % 360 + 360) % 360;
  const q = ((280.459 + 0.98564736 * D) % 360 + 360) % 360;
  const L = ((q + 1.915 * Math.sin(g * R) + 0.020 * Math.sin(2 * g * R)) % 360 + 360) % 360;
  const e = 23.439 - 0.00000036 * D;
  let ra = Math.atan2(Math.cos(e * R) * Math.sin(L * R), Math.cos(L * R)) / R / 15;
  ra = ((ra % 24) + 24) % 24;
  const decl = Math.asin(Math.sin(e * R) * Math.sin(L * R)) / R;
  let eqt = q / 15 - ra;
  eqt = ((eqt + 12) % 24 + 24) % 24 - 12;
  return { decl: decl, eqt: eqt };
}
// Hours from solar noon at which the sun stands at «angle» below the horizon, or null where it
// never does. The null is the whole high-latitude answer and it is deliberate.
function prayerSunAngleTime(angle, decl, lat) {
  const R = Math.PI / 180;
  const den = Math.cos(decl * R) * Math.cos(lat * R);
  if (den === 0) return null;
  const c = (-Math.sin(angle * R) - Math.sin(decl * R) * Math.sin(lat * R)) / den;
  if (!isFinite(c) || c > 1 || c < -1) return null;
  return Math.acos(c) / R / 15;
}
// The shadow rule, expressed as the altitude it corresponds to.
function prayerAsrAngle(factor, decl, lat) {
  const R = Math.PI / 180;
  return -Math.atan(1 / (factor + Math.tan(Math.abs(lat - decl) * R))) / R;
}
// EVERY TIME, IN MINUTES FROM LOCAL MIDNIGHT, or null. The offsets are applied here, each to its
// own prayer and to nothing else, and each clamped on the way out as well as on the way in.
function prayerTimesFor(y, m, d, lat, lng, tzMinutes, methodId, asrMode, offsets) {
  const out = { fajr: null, sunrise: null, dhuhr: null, asr: null, maghrib: null, isha: null };
  if (typeof lat !== 'number' || typeof lng !== 'number') return out;
  if (!isFinite(lat) || !isFinite(lng)) return out;
  if (lat < -90 || lat > 90 || lng < -180 || lng > 180) return out;
  const tz = (typeof tzMinutes === 'number' && isFinite(tzMinutes)) ? tzMinutes : 0;
  const M = prayerMethodOf(methodId);
  const factor = asrMode === 'hanafi' ? 2 : 1;
  const jd0 = hijriJdnFromCivil(y, m, d) - 0.5 - lng / 360;
  const shift = tz / 60 - lng / 15;
  const at = (hours) => prayerSunPosition(jd0 + hours / 24);
  // Two passes: the guess only has to be inside the right half of the day.
  const solve = (angleAt, before, guess) => {
    let t = guess;
    for (let i = 0; i < 2; i++) {
      const p = at(t);
      const h = prayerSunAngleTime(angleAt(p.decl), p.decl, lat);
      if (h === null) return null;
      t = before ? (12 - p.eqt) - h : (12 - p.eqt) + h;
    }
    return t;
  };
  const noon = at(12);
  const dhuhr = 12 - noon.eqt;
  const fajr = solve(() => M.fajr, true, 5);
  const sunrise = solve(() => PRAYER_HORIZON, true, 6);
  const maghrib = solve(() => PRAYER_HORIZON, false, 18);
  const asr = solve((decl) => prayerAsrAngle(factor, decl, lat), false, 15);
  const isha = (M.ishaMin > 0)
    ? (maghrib === null ? null : maghrib + M.ishaMin / 60)
    : solve(() => M.isha, false, 19);
  const raw = { fajr: fajr, sunrise: sunrise, dhuhr: dhuhr, asr: asr, maghrib: maghrib, isha: isha };
  for (let i = 0; i < PRAYER_KEYS.length; i++) {
    const k = PRAYER_KEYS[i];
    if (raw[k] === null || !isFinite(raw[k])) { out[k] = null; continue; }
    let off = 0;
    if (offsets && PRAYER_OFFSETTABLE.indexOf(k) !== -1) {
      const v = offsets[k];
...
// A twelve-hour clock in Arabic-Indic digits. Null draws an em dash, never a zero.
function prayerClock(mins) {
  if (typeof mins !== 'number' || !isFinite(mins)) return '—';
...
function readPrayerPrefs() {
  ...
  if (o && typeof o === 'object' && !Array.isArray(o)) {
    for (let i = 0; i < PRAYER_OFFSETTABLE.length; i++) {
      ...
      const v = o[k];
      if (typeof v === 'number' && isFinite(v) && Math.trunc(v) === v
        && v >= PRAYER_OFFSET_MIN && v <= PRAYER_OFFSET_MAX) out.off[k] = v;
    }
  }
  return out;
}
function writePrayerPrefs(next) {
  ...
  if (next.off && typeof next.off === 'object') {
    for (let i = 0; i < PRAYER_OFFSETTABLE.length; i++) {
      ...
      const v = next.off[k];
      if (typeof v === 'number' && isFinite(v) && Math.trunc(v) === v
        && v >= PRAYER_OFFSET_MIN && v <= PRAYER_OFFSET_MAX) rec.off[k] = v;
    }
  }
...
function prayerNudgeOffset(prefs, key, step) {
  const base = readPrayerPrefs();
  const cur = (prefs && prefs.off && typeof prefs.off[key] === 'number') ? prefs.off[key] : base.off[key];
  if (PRAYER_OFFSETTABLE.indexOf(key) === -1) return base;
  let v = (typeof cur === 'number' && isFinite(cur) ? Math.trunc(cur) : 0) + (Math.trunc(Number(step)) || 0);
  if (v < PRAYER_OFFSET_MIN) v = PRAYER_OFFSET_MIN;
  if (v > PRAYER_OFFSET_MAX) v = PRAYER_OFFSET_MAX;
  const off = {};
  off[key] = v;
...
// ITEM 121: preferences move as one intact unit. The storage record, option builders and
    ...
    <>
      ...
      <div className="ez-hit" style={s.prayerOptRow} role="radiogroup" aria-label={PRAYER_METHOD_LABEL}>
        {prayerMethodIds().map((id) => (
          <button key={id} type="button" role="radio" aria-checked={prefs.method === id ? 'true' : 'false'}
            onClick={() => setPrefs(writePrayerPrefs({ method: id }))}
            data-ezik-prayer-setting="method" className="ezik-focus" style={prefs.method === id ? { ...s.prayerOpt, ...s.themeOptActive } : s.prayerOpt}>
            {prayerMethodOf(id).name}
          </button>
        ))}
      </div>
      <div style={s.a11yGroupLabel}>{PRAYER_ASR_LABEL}</div>
      <div className="ez-hit" style={s.prayerOptRow} role="radiogroup" aria-label={PRAYER_ASR_LABEL}>
        {['standard', 'hanafi'].map((mode) => (
          <button key={mode} type="button" role="radio" aria-checked={prefs.asr === mode ? 'true' : 'false'}
            onClick={() => setPrefs(writePrayerPrefs({ asr: mode }))}
            data-ezik-prayer-setting="asr" className="ezik-focus" style={prefs.asr === mode ? { ...s.prayerOpt, ...s.themeOptActive } : s.prayerOpt}>
            {PRAYER_ASR_LABELS[mode]}
          </button>
        ))}
      </div>
    </>
...
// Degrees clockwise from true north, 0 <= b < 360, or null when the position is not a position.
function qiblaBearing(lat, lng) {
  ...
  const R = Math.PI / 180;
  const p1 = lat * R;
  const p2 = KAABA_LAT * R;
  const dl = (KAABA_LNG - lng) * R;
  const y = Math.sin(dl) * Math.cos(p2);
  const x = Math.cos(p1) * Math.sin(p2) - Math.sin(p1) * Math.cos(p2) * Math.cos(dl);
  if (x === 0 && y === 0) return null;
  return ((Math.atan2(y, x) / R) + 360) % 360;
}
// The eight-point name, so the number is never the only thing said. The bands are 45° wide and
// centred on their point, which is why the offset is 22.5 rather than 0.
function qiblaDirName(deg) {
  if (typeof deg !== 'number' || !isFinite(deg)) return '';
  const i = Math.floor((((deg % 360) + 360) % 360 + 22.5) / 45) % 8;
  return QIBLA_DIRS[i];
}
function qiblaDegreeText(deg) {
  if (typeof deg !== 'number' || !isFinite(deg)) return '';
...
function qiblaHeadingOf(ev) {
  if (!ev) return null;
  const wk = ev.webkitCompassHeading;
  if (typeof wk === 'number' && isFinite(wk) && wk >= 0 && wk <= 360) return ((wk % 360) + 360) % 360;
  if (ev.absolute !== true) return null;
  const a = ev.alpha;
  if (typeof a !== 'number' || !isFinite(a)) return null;
  return ((360 - (a % 360)) % 360 + 360) % 360;
}
// How far to turn the needle: the qibla, seen from a device that is itself pointing somewhere.
function qiblaNeedleAngle(bearing, heading) {
...
function readQiblaLoc() {
  let raw = null;
  try { raw = localStorage.getItem(QIBLA_LOC_KEY); } catch (e) { raw = null; }
  ...
  return { lat: QIBLA_DEFAULT_LAT, lng: QIBLA_DEFAULT_LNG, by: 'default' };
}
function writeQiblaLoc(lat, lng) {
  ...
  const askLocation = () => {
    ...
    try {
      navigator.geolocation.getCurrentPosition((pos) => {
        ...
        if (!c) { setLocState('denied'); return; }
        setLoc(writeQiblaLoc(c.latitude, c.longitude));
        setLocState('');
      }, () => { setLocState('denied'); }, { enableHighAccuracy: false, timeout: 10000, maximumAge: 600000 });
    } catch (e) { setLocState('denied'); }
  };
  const useDefault = () => { setLoc(clearQiblaLoc()); setLocState(''); };

  const startCompass = () => {
    if (compass === 'wait' || compass === 'live') return;
    setCompass('wait');
    const onEvent = (ev) => {
      const h = qiblaHeadingOf(ev);
      if (h === null) return;              // a reading with nothing in it is not a reading
      setHeading(h);
      setCompass('live');
    };
    let timer = null;
    const attach = () => {
      ...
      } catch (e) { setCompass('none'); return; }
      timer = setTimeout(() => { setCompass((c) => (c === 'live' ? c : 'none')); }, QIBLA_NEEDLE_MS);
      stopRef.current = () => {
    ...
    // The gesture requirement, satisfied where it exists: this whole function runs inside the
    ...
    if (DOE && typeof DOE.requestPermission === 'function') {
      let p = null;
      try { p = DOE.requestPermission(); } catch (e) { setCompass('none'); return; }
      if (p && typeof p.then === 'function') {
        p.then((r) => { if (r === 'granted') attach(); else setCompass('none'); }, () => setCompass('none'));
        return;
      }
      setCompass('none');
      return;
...
const HIJRI_FALLBACK_CALENDAR = 'tabular-civil-IIa';
const HIJRI_OFFSET_KEY = 'ezik_hijri_offset_v1';
...
function hijriJdnFromCivil(y, m, d) {
...
function hijriCivilFromJdn(jdn) {
  const a = jdn + 32044;
  ...
  return {
    y: 100 * b + dd - 4800 + Math.floor(mi / 10),
    m: mi + 3 - 12 * Math.floor(mi / 10),
    d: e - Math.floor((153 * mi + 2) / 5) + 1,
  };
}
// THE ARITHMETICAL CALENDAR, and it is the fallback rather than the answer.
function hijriTabularFromJdn(jdn) {
  ...
  return { y: 30 * n + j - 30, m: m, d: l - Math.floor((709 * m) / 24) };
}
// Umm al-Qura, through the engine's own tables. It is handed a TIMESTAMP rather than a Date, so
// this function needs no clock at all: noon UTC of the civil day the JDN names.
function hijriUmalquraFromJdn(jdn) {
  try {
    if (typeof Intl === 'undefined' || typeof Intl.DateTimeFormat !== 'function') return null;
    const fmt = new Intl.DateTimeFormat('en-u-ca-' + HIJRI_CALENDAR,
      { day: 'numeric', month: 'numeric', year: 'numeric', timeZone: 'UTC' });
    if (typeof fmt.resolvedOptions !== 'function') return null;
    if (fmt.resolvedOptions().calendar !== HIJRI_CALENDAR) return null;
    if (typeof fmt.formatToParts !== 'function') return null;
    const parts = fmt.formatToParts((jdn - 2440588) * 86400000 + 43200000);
    let y = 0, m = 0, d = 0;
    for (let i = 0; i < parts.length; i++) {
      const p = parts[i];
      if (p.type === 'year') y = parseInt(String(p.value).replace(/[^0-9]/g, ''), 10);
      else if (p.type === 'month') m = parseInt(String(p.value).replace(/[^0-9]/g, ''), 10);
      else if (p.type === 'day') d = parseInt(String(p.value).replace(/[^0-9]/g, ''), 10);
    }
    if (!(y > 0 && m >= 1 && m <= 12 && d >= 1 && d <= 30)) return null;
    return { y: y, m: m, d: d };
  } catch (e) { return null; }
}
function hijriFromJdn(jdn) {
  const named = hijriUmalquraFromJdn(jdn);
  if (named) { named.by = HIJRI_CALENDAR; return named; }
  const t = hijriTabularFromJdn(jdn);
  t.by = HIJRI_FALLBACK_CALENDAR;
  return t;
}
// THE OFFSET IS CLAMPED HERE TOO. Not only when it is written: a store edited by hand, or one
// carrying a value from a future version, may not move the date further than the pair allows.
function hijriForCivilDay(y, m, d, offset) {
  let k = Math.trunc(Number(offset));
  if (!isFinite(k)) k = 0;
  if (k < HIJRI_OFFSET_MIN) k = HIJRI_OFFSET_MIN;
  if (k > HIJRI_OFFSET_MAX) k = HIJRI_OFFSET_MAX;
  const out = hijriFromJdn(hijriJdnFromCivil(y, m, d) + k);
  out.offset = k;
  return out;
}
// ANYTHING UNUSABLE READS AS ZERO AND NEVER THROWS. A storage that denies reads, a value that is
// not a number, a number outside the pair — all of them are "no offset", which is the app as it
// was before this item.
function readHijriOffset() {
  let raw = null;
  try { raw = localStorage.getItem(HIJRI_OFFSET_KEY); } catch (e) { return 0; }
  if (typeof raw !== 'string' || !/^-?[0-9]+$/.test(raw)) return 0;
  const n = Number(raw);
  if (n < HIJRI_OFFSET_MIN || n > HIJRI_OFFSET_MAX) return 0;
  return n;
}
// IT RETURNS WHAT IS NOW IN EFFECT, never what was asked for. A refused value and a store that
...
function hijriTodayLabel() {
  try {
    const now = new Date();
    return hijriLabel(hijriForCivilDay(now.getFullYear(), now.getMonth() + 1, now.getDate(), readHijriOffset()));
  } catch (e) { return ''; }
}
...
function HijriOffsetControl() {
  ...
  const choose = (v) => { setOff(writeHijriOffset(v)); };
  const today = (() => {
    try {
      const now = new Date();
      return hijriLabel(hijriForCivilDay(now.getFullYear(), now.getMonth() + 1, now.getDate(), off));
    } catch (e) { return ''; }
  })();
  const opts = [];
  for (let v = HIJRI_OFFSET_MIN; v <= HIJRI_OFFSET_MAX; v++) opts.push(v);
  return (
    <>
      <div style={s.a11yGroupLabel}>{HIJRI_SET_LABEL}</div>
      <div style={s.themeRow} role="radiogroup" aria-label={HIJRI_SET_LABEL}>
        {opts.map((v) => (
          <button
            key={v}
            type="button"
            role="radio"
            aria-checked={off === v ? 'true' : 'false'}
            onClick={() => choose(v)}
            data-ezik-prayer-setting="hijri"
            className="ezik-focus"
            style={{ ...s.a11yOpt, ...(off === v ? s.themeOptActive : {}) }}
          >
            {v === 0 ? toArabicDigits(0) : (v > 0 ? '+' : '−') + toArabicDigits(Math.abs(v))}
          </button>
        ))}
      </div>
      <div style={s.hijriNow}>{HIJRI_SET_NOW} {today}</div>
    </>
  );
}
...
function ezikPrecacheNotice(report) {
  if (!report || typeof report !== 'object') return null;
  if (report.ezik !== EZIK_SW_REPORT_TAG) return null;
  if (report.skipped === 'quota') return EZIK_SW_MSG_NONE;
  const failed = report.failed;
  if (typeof failed !== 'number' || !isFinite(failed) || failed <= 0) return null;
  return EZIK_SW_MSG_PARTIAL;
}

let ezikSwNoticeSpent = false;

function EzikPrecacheNotice() {
  const [text, setText] = useState(null);
  useEffect(() => {
    if (typeof navigator === 'undefined' || !navigator.serviceWorker) return undefined;
    const onMessage = (e) => {
      if (ezikSwNoticeSpent) return;
      const line = ezikPrecacheNotice(e && e.data);
      if (!line) return;
      ezikSwNoticeSpent = true;
      setText(line);
    };
    try { navigator.serviceWorker.addEventListener('message', onMessage); } catch (err) { return undefined; }
    return () => {
      try { navigator.serviceWorker.removeEventListener('message', onMessage); } catch (err) {}
    };
  }, []);
  if (!text) return null;
  return (
    <div role="status" aria-label={EZIK_SW_ARIA} style={s.swNote}>
      <span style={s.swNoteText}>{text} {EZIK_SW_MSG_TAIL}</span>
      <span className="ez-hit" style={s.swNoteHit}>
        <button type="button" onClick={() => setText(null)} className="ezik-focus" style={s.swNoteBtn}>{EZIK_SW_OK}</button>
      </span>
    </div>
  );
}

function EzShellGroup({ title, hint, wide, children }) {
  return (
    <section className={wide ? 'ezsh-group is-wide' : 'ezsh-group'}>
      <div className="adhkar3" style={s.ezshGroup}>
        <div style={s.ezshGroupHead}>
          <span style={s.ezshGroupMark} aria-hidden="true" />
          <div style={s.settingsLabel}>{title}</div>
        </div>
        {children}
        {hint ? <div style={s.settingsHint}>{hint}</div> : null}
      </div>
    </section>
  );
}
// ---- S105 ISTANA SHELL END ----------------------------------------------------------------
...
function SettingsSheet({ theme, onTheme, onBack, onOpenControl, a11y, onA11y, onA11yReset, aiConsent, aiConsentBy, onWithdrawAI, onReviewAI }) {
  ...
  const A_CONTROL = ezT('settings.control');
  // S105: the screen title and the appearance heading, lifted to constants so the shell and
  ...
  const canSetPin = hasFounderToken();
  // Read ONCE, at mount, from the one reader -- the same rule pin1/pin2 below follow. The value
  ...
  const savePin = async () => {
    if (pinBusy) return;
    // The ONLY check made here, because the server is sent one value and never sees the second
    // field. Length, shape and authorisation are the server's to judge and the server's to word.
    if (pin1 !== pin2) { setPinMsg(ezT('pin.mismatch')); return; }
    setPinBusy(true); setPinMsg('');
    try {
      const r = await fetch('/api/unlock', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...capHeaders() },   // carries the founder token
        credentials: 'same-origin',
        body: JSON.stringify({ action: 'set-pin', pin: pin1, deviceId: getDeviceId() }),
      });
      const d = await r.json().catch(() => ({}));
      const good = r.ok && d && d.ok === true;
      // The SERVER owns every refusal's wording: a missing token, a short code and an
      // unreachable store are three different truths and only it knows which.
      setPinMsg(good ? ezT('pin.changed') : ((d && d.message) || ''));
      if (good) { setPin1(''); setPin2(''); }
    } catch (e) {
      setPinMsg('');   // a network failure carries no server message; do not invent one
    }
    setPinBusy(false);
  };
  const Opt = ({ value, label }) => (
    <button
      ...
      style={{ ...s.themeOpt, ...(theme === value ? s.themeOptActive : {}) }}
    >
      <span style={{ ...s.themeSwatch, background: value === 'dark' ? '#171B24' : '#FFFFFF', borderColor: value === 'dark' ? '#2A313D' : '#E5EBF5' }} />
      <span>{label}</span>
    </button>
  );
  // S102 -- THE ACTIVE APPLICATION DESIGN. This is a STATEMENT, not a chooser: there is one
      ...
      <div className="ezsh-grid">
        <EzShellGroup title={A_APPEARANCE} hint={ezT('settings.savedOnDevice')}>
          <div style={s.themeRow}>
            ...
            <Opt value="dark" label={ezT('settings.themeDark')} />
          </div>
        </EzShellGroup>
        {/* S116 -- the interface language. ONE compact row inside the shell every other
            setting uses: the group's own heading is the label, and the control sits at the
            inline end showing the current choice. No second design, no pair of full-width
            buttons, and the theme, the reading preferences and the control row are untouched. */}
        <EzShellGroup title={ezT('settings.language')}>
          <div className="ezlang-row"><EzLangControl variant="settings" /></div>
        </EzShellGroup>
        {/* S102 -- the active application design. One statement, one disabled line. */}
        <EzShellGroup title={EZ_VT_TITLE}>
          {visualTheme === EZIK_VISUAL_THEME_ISTANA && (
          <div style={s.vtActiveRow}>
            <span style={s.vtActiveMark} aria-hidden="true" />
            <span style={s.vtActiveBody}>
              <span style={s.vtActiveName}>{EZ_VT_ISTANA}</span>
              <span style={s.vtActiveState}>{EZ_VT_ACTIVE}</span>
            </span>
          </div>
          )}
          <div style={s.vtSoonRow} aria-disabled="true">
            <span style={s.vtSoonName}>{EZ_VT_QIBLA}</span>
            <span style={s.vtSoonTag}>{EZ_VT_SOON}</span>
          </div>
        </EzShellGroup>
        {/* S99 -- the three accessibility preferences, byte-for-byte the controls that shipped:
            same roles, same aria-checked, same handlers, same profile-scoped writes. */}
        {/* NOT wide: measured at 1440 and 2048, spanning both columns stretched each switch
            row to about 800px with the label at one edge and the toggle at the other. A settings
            row read across a desk is a worse control, not a bigger one. */}
        <EzShellGroup title={EZIK_A11Y_TITLE} hint={ezT('settings.savedOnDevice')}>
          <div style={s.a11yGroupLabel}>{EZIK_A11Y_FS_LABEL}</div>
          <div style={s.themeRow} role="radiogroup" aria-label={EZIK_A11Y_FS_LABEL}>
            {[['normal', EZIK_A11Y_FS_NORMAL], ['large', EZIK_A11Y_FS_LARGE], ['xlarge', EZIK_A11Y_FS_XLARGE]].map(([v, label]) => (
              <button
                key={v}
                type="button"
                role="radio"
                aria-checked={a11y.fontSize === v ? 'true' : 'false'}
                onClick={() => onA11y({ fontSize: v })}
                className="ez-a11y-opt"
                style={{ ...s.a11yOpt, ...(a11y.fontSize === v ? s.themeOptActive : {}) }}
              >
                {label}
              </button>
            ))}
          ...
          <button
            type="button"
            role="switch"
            aria-checked={a11y.reading ? 'true' : 'false'}
            onClick={() => onA11y({ reading: !a11y.reading })}
            className="ez-a11y-opt"
            style={s.a11ySwitchRow}
          >
            <span style={s.a11ySwitchText}>
              <span style={s.a11ySwitchTitle}>{EZIK_A11Y_READ}</span>
              <span style={s.a11ySwitchHint}>{EZIK_A11Y_READ_HINT}</span>
            </span>
            <span style={{ ...s.a11ySwitch, ...(a11y.reading ? s.a11ySwitchOn : {}) }} aria-hidden="true">
              <span style={{ ...s.a11yKnob, ...(a11y.reading ? s.a11yKnobOn : {}) }} />
            </span>
          </button>
          <button
            type="button"
            role="switch"
            aria-checked={a11y.reduceMotion ? 'true' : 'false'}
            onClick={() => onA11y({ reduceMotion: !a11y.reduceMotion })}
            className="ez-a11y-opt"
            style={s.a11ySwitchRow}
          >
            <span style={s.a11ySwitchText}>
              <span style={s.a11ySwitchTitle}>{EZIK_A11Y_MOTION}</span>
              <span style={s.a11ySwitchHint}>{EZIK_A11Y_MOTION_HINT}</span>
            </span>
            <span style={{ ...s.a11ySwitch, ...(a11y.reduceMotion ? s.a11ySwitchOn : {}) }} aria-hidden="true">
              <span style={{ ...s.a11yKnob, ...(a11y.reduceMotion ? s.a11yKnobOn : {}) }} />
            </span>
          </button>
          <button type="button" onClick={onA11yReset} className="ez-a11y-opt" style={s.a11yReset}>{EZIK_A11Y_RESET}</button>
        </EzShellGroup>
        {/* WHAT `Enter` DOES. One switch over the one preference, and it writes an EXPLICIT
            word: flipping it stores send/newline rather than leaving the device to be asked
            again. The hint line states the rule in full, including the half that is not
            negotiable -- an Enter the keyboard is using to finish a word never sends. */}
        <EzShellGroup title={ezT('settings.enter')} hint={ezT('settings.savedOnDevice')}>
          <button
            type="button"
            role="switch"
            aria-checked={enterSendsNow ? 'true' : 'false'}
            onClick={() => setEnterPref(writeEnterPref(enterSendsNow ? 'newline' : 'send'))}
            className="ezik-focus"
            style={s.a11ySwitchRow}
          >
            <span style={s.a11ySwitchText}>
              <span style={s.a11ySwitchTitle}>{ezT('settings.enter.sends')}</span>
              <span style={s.a11ySwitchHint}>{enterSendsNow ? ezT('settings.enter.sendsHint') : ezT('settings.enter.newlineHint')}</span>
            </span>
            <span style={{ ...s.a11ySwitch, ...(enterSendsNow ? s.a11ySwitchOn : {}) }} aria-hidden="true">
              <span style={{ ...s.a11yKnob, ...(enterSendsNow ? s.a11yKnobOn : {}) }} />
            </span>
          </button>
        </EzShellGroup>
        {/* THE WATERMARK'S STRENGTH. Three presets and a fine slider over the SAME number, so
            there is one value and not two: a preset writes 0.2 / 0.5 / 0.8 and the slider writes
            anything between; whichever moved last is what both controls then show. It is applied
            LIVE by writing --ezwm-o on the document root, and persisted per device by the one
            writer above -- this screen owns no copy of the value and reads the store once, at
            mount, exactly as the sheet's other local state does. Nothing here is sent anywhere.
            The buttons deliberately reuse the reading-preference option styling rather than
            adding a colour of their own; the group sits AFTER the reading preferences, which is
            also what keeps it outside the region the visual-identity gate reads. */}
        <EzShellGroup title={ezT('settings.watermark')} hint={ezT('settings.savedOnDevice')}>
          <div style={s.themeRow} role="radiogroup" aria-label={ezT('settings.watermark')}>
            {[[0, ezT('settings.watermark.none')],
              [EZIK_WM_PRESETS[0], ezT('settings.watermark.light')],
              [EZIK_WM_PRESETS[1], ezT('settings.watermark.medium')],
              [EZIK_WM_PRESETS[2], ezT('settings.watermark.bold')]].map(([v, label]) => (
              <button
                key={v}
                type="button"
                role="radio"
                aria-checked={wmOpacity === v ? 'true' : 'false'}
                onClick={() => setWmOpacity(applyWatermarkOpacity(v))}
                className="ezik-focus"
                style={{ ...s.a11yOpt, ...(wmOpacity === v ? s.themeOptActive : {}) }}
              >
                {label}
              </button>
            ))}
          </div>
          <div style={s.a11yGroupLabel}>{ezT('settings.watermark.fine')}</div>
          {/* The slider keeps its floor. Zero is not the bottom of a range -- it is a
              different answer, and it has its own control above rather than an end of the
              slider a thumb can land on by accident. */}
          <input
            type="range"
            min={EZIK_WM_MIN}
            max={1}
            step={0.01}
            value={wmOpacity}
            onChange={(e) => setWmOpacity(applyWatermarkOpacity(e.target.value))}
            aria-label={ezT('settings.watermark.fine')}
            className="ezik-focus"
            style={s.wmSlider}
          />
          {/* THE OPTIONAL RETREAT. Off by default: the item asks for a choice, not a new
              behaviour every reader is given. */}
          <button
            type="button"
            role="switch"
            aria-checked={wmHide ? 'true' : 'false'}
            onClick={() => setWmHide(writeWatermarkAutoHide(!wmHide))}
            className="ezik-focus"
            style={s.a11ySwitchRow}
          >
            <span style={s.a11ySwitchText}>
              <span style={s.a11ySwitchTitle}>{ezT('settings.watermark.quiet')}</span>
              <span style={s.a11ySwitchHint}>{ezT('settings.watermark.quietHint')}</span>
            </span>
            <span style={{ ...s.a11ySwitch, ...(wmHide ? s.a11ySwitchOn : {}) }} aria-hidden="true">
              <span style={{ ...s.a11yKnob, ...(wmHide ? s.a11yKnobOn : {}) }} />
            </span>
          </button>
        </EzShellGroup>
        {/* الخصوصية والذكاء الاصطناعي -- the consent decision, stated and reversible. It is NOT
            behind the PIN gate: a reader must be able to withdraw without an adult's code, and
            Apple requires the withdrawal to be as reachable as the grant. */}
        <EzShellGroup title={EZ_AIS_TITLE}>
          <div style={s.aicStatusRow}>{aiConsent === EZ_AI_CONSENT_GRANTED ? EZ_AIS_ON : EZ_AIS_OFF}</div>
          {aiConsent === EZ_AI_CONSENT_GRANTED && (
            <div style={s.aicNote}>{aiConsentBy === 'guardian' ? EZ_AIS_BY_GUARDIAN : EZ_AIS_BY_USER}</div>
          )}
          <div style={s.aicNote}>{EZ_AIS_PROVIDERS_LABEL + ' '}<span style={s.aicProvider}>{EZ_AIS_PROVIDERS_NAMES}</span></div>
          <AIConsentVersion style={s.aicNote} />
          <AIConsentLinks />
          {aiConsent === EZ_AI_CONSENT_GRANTED
            ? (<>
                <button type="button" onClick={onWithdrawAI} style={s.aicWithdrawBtn}>{EZ_AIS_WITHDRAW}</button>
                <div style={s.aicNote}>{EZ_AIS_KEEP}</div>
              </>)
            : <button type="button" onClick={onReviewAI} style={s.aicWithdrawBtn}>{EZ_AIS_REVIEW}</button>}
        </EzShellGroup>
        {/* S90 -- the one row that opens the PIN gate. Same handler, same accessible name. */}
        <EzShellGroup title={A_CONTROL}>
          <button type="button" onClick={onOpenControl} style={s.settingsNavBtn} aria-label={A_CONTROL}>
            <span>{A_CONTROL}</span>
            <span style={s.settingsNavChev} aria-hidden="true">{'\u2039'}</span>
          </button>
        </EzShellGroup>
        {/* D89 -- change the unlock PIN. Rendered ONLY with a founder token in hand. */}
        {canSetPin && (
        <EzShellGroup title={ezT('pin.groupTitle')}>
          <input type="password" inputMode="numeric" autoComplete="new-password" value={pin1}
            onChange={(e) => setPin1(e.target.value)} placeholder={ezT('pin.new')} style={s.settingsInput} />
          <input type="password" inputMode="numeric" autoComplete="new-password" value={pin2}
            onChange={(e) => setPin2(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter') savePin(); }}
            placeholder={ezT('pin.confirm')} style={s.settingsInput} />
          {pinMsg && <div style={s.settingsMsg}>{pinMsg}</div>}
          <button onClick={savePin} disabled={pinBusy || !pin1 || !pin2}
        ...
        <EzShellGroup title={PRAYER_SETTINGS_TITLE} hint={PRAYER_HINT}>
          <PrayerSettingsControl />
          <div style={s.ezshGroupHead}>
            <span style={s.ezshGroupMark} aria-hidden="true" />
            <div style={s.settingsLabel}>{HIJRI_SET_TITLE}</div>
          </div>
          <HijriOffsetControl />
          <div style={s.settingsHint}>{HIJRI_SET_HINT}</div>
        </EzShellGroup>
      </div>
    </EzShell>
  );
...
function ParentDashboard({ profile, messages, onBack, onReset, directConvoLocked, onToggleDirectConvo }) {
  ...
  const isYoung = deriveCaps(profile?.age).band === 'young';
  return (
    <div className="theme-dark ezhome ezparent" style={s.dashboardContainer}>
      {/* S115: a bounded rail in place of the strip header. The back control is the shipped one,
          text and all; every row below shows exactly what it always showed, read from exactly
          where it always read it, and nothing that was hidden is now revealed. */}
      <div className="ezparent-rail">
        <div className="ezparent-rail-inner">
          <button onClick={onBack} className="ezparent-back">← رجوع</button>
          <span className="ezparent-brand">
            <span className="ezparent-arch" aria-hidden="true" />
            <span>لوحة الأهل</span>
          </span>
        </div>
      </div>
      <div style={s.dashboardContent}>
       <div className="ezparent-wrap">
        <div style={s.dashboardCard}>
          <div className="ezparent-head"><span className="ezparent-mark" aria-hidden="true" /><div style={s.dashboardLabel}>الطفل</div></div>
          <div style={s.dashboardValue}>{profile?.name} — {profile?.age} سنة</div>
        </div>
        <div style={s.dashboardCard}>
          <div className="ezparent-head"><span className="ezparent-mark" aria-hidden="true" /><div style={s.dashboardLabel}>عدد الرسائل</div></div>
          <div style={s.dashboardValue}>{messages.length} رسالة</div>
        </div>
        {isYoung && (
        <div style={s.dashboardCard}>
          <div className="ezparent-head"><span className="ezparent-mark" aria-hidden="true" /><div style={s.dashboardLabel}>المحادثة المباشرة</div></div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            <div style={s.dashboardValue}>{directConvoLocked ? '🔒 مقفلة' : '🔓 مفتوحة'}</div>
            <button
              onClick={onToggleDirectConvo}
              style={{ background: directConvoLocked ? 'var(--red-lift)' : 'var(--accent-fill)', color: directConvoLocked ? 'var(--a3-navy)' : 'var(--on-accent)', border: 'none', borderRadius: 12, padding: '10px 16px', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
            >
              {directConvoLocked ? 'فتح المحادثة المباشرة' : 'قفل المحادثة المباشرة'}
            </button>
          </div>
        </div>
        )}
        <div style={s.dashboardCard}>
          <div className="ezparent-head"><span className="ezparent-mark" aria-hidden="true" /><div style={s.dashboardLabel}>سجل المحادثات</div></div>
          <div style={s.conversationLog}>
            {messages.length === 0 ? (
              <div style={s.dashboardEmpty}>لا توجد محادثات بعد</div>
            ) : (
              messages.map((m, i) => (
                <div key={i} style={{ ...s.logEntry, backgroundColor: m.role === 'user' ? 'var(--a3-ice)' : 'var(--a3-soft)' }}>
                  <div style={s.logRole}>{m.role === 'user' ? `👶 ${profile?.name}` : (profile?.gender === 'female' ? '👩‍🏫 عزك' : '👨‍🏫 عزك')}</div>
                  <div style={s.logContent}>{m.role === 'assistant' ? formatForLog(m.content) : contentToText(m.content)}</div>
                  <div style={s.logTime}>{new Date(m.timestamp).toLocaleString('ar-SA')}</div>
                </div>
              ))
            )}
          </div>
...
function CallScreen({ profileName, gender, callState, heard, isMuted, error, onToggleMute, onTalk, onExit }) {
  ...
  // S113: the four labels are the ones that shipped. They were COMPUTED and never drawn -- `cs`
  ...
  const STATES = {
    idle:      { label: 'اضغط للتحدّث',       ring: 'var(--a3-line)',  speed: '3.6s' },
    listening: { label: 'أستمع إليك...',     ring: 'var(--a3-cyan)',  speed: '2.4s' },
    thinking:  { label: 'لحظة...',            ring: 'var(--a3-muted)', speed: '3.2s' },
    speaking:  { label: gender === 'female' ? 'عزك تتحدّث...' : 'عزك يتحدّث...',  ring: 'var(--a3-blue)',  speed: '1.2s' },
  };
  const cs = STATES[callState] || STATES.idle;

  const ringStyle = {
    ...s.callRing,
    borderColor: cs.ring,
    animation: reduceMotion ? 'none' : `callPulse ${cs.speed} ease-in-out infinite`,
  };

  // Layer 3: the avatar stays tappable while the tutor is thinking/speaking — tapping there is the
  // manual interrupt ("my turn"). Disabled only when muted, or during 'listening' (no-op as before).
  const canTalk = !isMuted && (callState === 'idle' || callState === 'thinking' || callState === 'speaking');
  // The one line under the emblem, and it is the SAME line that shipped: the interim transcript
  ...
  const hint = callState === 'listening' && heard ? heard : (isMuted ? CALL_TXT.MUTED_HINT : '');

  return (
    <div className="theme-dark ezhome ezcall" style={s.callContainer}>
      {/* S113 -- THE TOP RAIL. Bounded and centred, carrying the screen's own name and the REAL
          way out: onExit, the same handler the «إنهاء» control below has always called, which
          resolves through goEzikBack. No handler was invented and none was removed. */}
      <div className="ezcall-rail">
        <div className="ezcall-rail-inner">
          <button onClick={onExit} className="ezcall-icon" aria-label={CALL_TXT.END}>{A2_ICON_BACK}</button>
          <span className="ezcall-brand">
            <span className="ezcall-brand-arch" aria-hidden="true" />
            <span className="ezcall-brand-text">{CALL_TXT.TITLE}</span>
          </span>
        </div>
      </div>
      <div className="ezcall-body">
        <div className="ezcall-stage">
          <div style={{ ...s.callAvatarWrap, cursor: canTalk ? 'pointer' : 'default' }} onClick={canTalk ? onTalk : undefined}>
            <div style={ringStyle} />
            <div style={s.callAvatar}><MoonStarsIcon size={56} color="var(--a3-blue)" /></div>
          </div>
          ...
          <div className="ezcall-note">{CALL_TXT.DISCLAIMER}</div>
          {/* A failure that cannot be seen is a failure that reads as "the app just does nothing".
              It keeps its own bounded panel inside the stage, and says exactly what it always said. */}
          {error ? <div style={s.callErrorBanner} className="ezcall-alert" role="alert">{error}</div> : null}
        </div>
      </div>
      <div className="ezcall-dock">
        <div className="ezcall-dock-inner" style={s.callControls}>
          <button onClick={onToggleMute} className="ezcall-btn" style={s.callMuteBtn}>
            {isMuted ? <MicOffIcon size={22} color="var(--a3-blue)" /> : <MicIcon size={22} color="var(--a3-blue)" />}
            <span style={s.callBtnLabel}>{isMuted ? CALL_TXT.MUTE_ON : CALL_TXT.MUTE_OFF}</span>
          </button>
          <button onClick={onExit} className="ezcall-btn" style={s.callEndBtn}>
            <PhoneOffIcon size={22} color="var(--a3-navy)" />
            <span style={{ ...s.callBtnLabel, color: 'var(--a3-navy)' }}>{CALL_TXT.END}</span>
          </button>
        </div>
      </div>
    </div>
...
const MEM = {
  TITLE: 'المُحفّظ',
  PICKER_HEADING: 'اختر سورةً لتبدأ الحفظ',
  START_LABEL: 'ابدأ من آية',
  START_BTN: 'ابدأ',
  REVEAL_BTN: 'افتح عليّ',
  LISTEN_BTN: 'استمع',
  LISTEN_RANGE_BTN: 'استمع للمقطع',
  REVEAL_LABEL: 'الكشف',
  GRAN_AYAH: 'آية',
  GRAN_WORD: 'كلمة',
  CHANGE_SURAH: 'سورة أخرى',
  HIDE: 'إخفاء',
  BACK_BTN: 'رجوع',
  LOADING: 'لحظة، يُحضَّر المصحف…',
  PRAISE_M: 'أحسنتَ، تابِع.',
  PRAISE_F: 'أحسنتِ، تابِعي.',
  FINISH_M: 'أتممتَ المقطع، بارك الله فيك.',
  FINISH_F: 'أتممتِ المقطع، بارك الله فيكِ.',
  MODE_MANUAL: 'تلقين يدوي',
  MODE_ADNAN: 'استماع وترديد',
  ADNAN_START: 'ابدأ',
  ADNAN_STOP: 'إيقاف',
  ADNAN_REPEAT: 'أعد الآية',
  ADNAN_AYAH_LABEL: 'آية',
  MODE_RECITE: 'سمِّعني',
  RECITE_HINT: 'هذا تمرينٌ يعينك على الحفظ، والتسميع الحقيقيّ يكون عند شيخك أو والديك.',
  RECITE_START: 'ابدأ التسميع',
  RECITE_STOP: 'إيقاف',
  RECITE_DONE: 'أحسنت، تابِع حفظك.',
  RECITE_NEXT: 'الآية التالية',
};

/* 14.1 */
// Standalone mushaf reader. NOT part of MemorizeScreen -- separate tile, separate screen.
...
/* 14.2 */
// صفحةٌ واحدةٌ من المصحف. كلُّ خانةٍ في التخطيط تُجلب كلمتَها من المصحف المحروس.
/* 14.2f2 */
// ضبطُ السطر — وهو ما يجعل الصفحة مصحفاً لا نصّاً.
...
const PG_FONT = "'Amiri Quran', 'Amiri', serif";

/* Session 51 -- ONE font size for all 604 pages.
 * ---------------------------------------------------------------------------
...
const pgAdvChars = (t) => {
  ...
  for (const ch of t) {
    ...
    if (!((u >= 0x064b && u <= 0x065f) || u === 0x0670 || (u >= 0x06d6 && u <= 0x06ed))) n++;
  }
  return n;
};

// خاناتُ السطر ← ما يُرسم فيه فعلاً. مصدرٌ واحدٌ للتصيير وللقياس معاً: لو افترقا
// لقِسنا سطراً غيرَ الذي نرسم. كلُّ حرفٍ من __quranData وحده — لا نصَّ بأيدينا.
const pgLineTokens = (ln) => {
  const parts = [];
  if (!ln || !ln.w) return parts;
  for (const loc of ln.w) {
    const a = loc.split(':');
    const sn = parseInt(a[0], 10), an = parseInt(a[1], 10), wi = parseInt(a[2], 10);
    const words = wordsOfAyah(sn, an);
    const w = words[wi - 1];
...
// (أ) المرشّحون — من البيانات وحدها، بلا DOM. كلفةُ السطر = محارفُه ذواتُ العرض
...
let __pgCands = null;
const pgCandidates = () => {
  if (__pgCands) return __pgCands;
  if (!__layoutData || !__layoutData.p || !__quranData) return null;
  const gapCh = PG_GAP_EM / PG_AVG_CH;
  const all = [];
  for (const pg of __layoutData.p) {
    for (const ln of pg.l) {
      if (ln.t !== 't') continue;
      const parts = pgLineTokens(ln);
      if (!parts.length) continue;
      let cost = (parts.length - 1) * gapCh;
      for (const p of parts) cost += pgAdvChars(p);
      all.push({ cost, parts });
    }
  }
  if (!all.length) return null;
  all.sort((a, b) => b.cost - a.cost);
  __pgCands = all.slice(0, PG_CAND_N).map((x) => x.parts);
  return __pgCands;
};

// (ب) القياسُ الحقيقيّ — مرّةً واحدة. المرشّحون يُصيَّرون خارج الشاشة بعرض الصفحة
...
const pgMeasureK = (box) => {
  if (PG_K !== null && PG_K_BOX === box) return PG_K;
  if (!(box > 0) || typeof document === 'undefined') return null;
  const cands = pgCandidates();
  if (!cands) return null;
  const t0 = (typeof performance !== 'undefined' && performance.now) ? performance.now() : 0;
  const host = document.createElement('div');
  host.setAttribute('aria-hidden', 'true');
  host.style.cssText = 'position:absolute;left:-99999px;top:0;visibility:hidden;pointer-events:none;width:' + box + 'px;';
  const rows = [];
  for (let i = 0; i < cands.length; i++) {
    const row = document.createElement('div');
    row.style.cssText = 'display:flex;flex-direction:row;align-items:baseline;justify-content:space-between;width:100%;direction:rtl;white-space:nowrap;line-height:' + PG_LINE_H + ';font-size:' + PG_BASE_FS + 'px;font-family:' + PG_FONT + ';';
    const parts = cands[i];
    for (let j = 0; j < parts.length; j++) {
      const sp = document.createElement('span');
      sp.style.cssText = 'flex-shrink:0;white-space:nowrap;';
  ...
  try {
    for (let i = 0; i < rows.length; i++) {
      ...
      let content = 0;
      for (let j = 0; j < kids.length; j++) content += kids[j].offsetWidth;
      content += (kids.length - 1) * (PG_BASE_FS * PG_GAP_EM);
      if (content > 0) { const r = box / content; if (r < k) k = r; }
    }
  } finally {
  ...
  PG_K = k;
  PG_K_BOX = box;
  PG_K_MS = ((typeof performance !== 'undefined' && performance.now) ? performance.now() : 0) - t0;
  return k;
};

// إبطالٌ صريح: تدويرُ الشاشة (تغيّرُ العرض) أو وصولُ الخطّ بعد قياسٍ بخطٍّ بديل.
// كلاهما يجعل K المخزَّن كذباً — والكذبُ هنا يعمّ الـ٦٠٤ لا صفحةً واحدة.
const pgInvalidateK = () => { PG_K = null; PG_K_BOX = 0; PG_GUARD_PAGES.clear(); };
// نافذةُ فحصٍ للمالك وحدها: قراءةٌ محضة، لا يقرأها منطقُ التطبيق ولا يتفرّع عليها.
try { window.__mushafFit = { k: () => PG_K, box: () => PG_K_BOX, ms: () => PG_K_MS, guard: () => Array.from(PG_GUARD_PAGES) }; } catch (e) {}

/* 14.6 */
// Session 74 -- PHASE ONE: the official Madinah mushaf page, drawn as one image.
...
const MUSHAF_SVG_ORIGIN = 'https://mushaf.almurabbi.app';
const MUSHAF_SVG_KEY = 'mushaf_svg_v1';   // device key. '0' = off, absent/anything = on
...
const MUSHAF_SVG_RATIO = 382.68 / 547.09;
const MUSHAF_SVG_AR = '382.68 / 547.09';
const MUSHAF_VB_W = 382.68;
const MUSHAF_VB_H = 547.09;

// Session 75/C -- THE SAFE CROP. The pages carry a wide inner margin of their own, and that
...
const MUSHAF_CROP_X0 = 26.73;
const MUSHAF_CROP_Y0 = 11.91;
const MUSHAF_CROP_X1 = 355.75;
const MUSHAF_CROP_Y1 = 524.11;
const MUSHAF_CROP_W = MUSHAF_CROP_X1 - MUSHAF_CROP_X0;   // 329.02 viewBox units
const MUSHAF_CROP_H = MUSHAF_CROP_Y1 - MUSHAF_CROP_Y0;   // 512.20 viewBox units
const MUSHAF_CROP_AR = MUSHAF_CROP_W + ' / ' + MUSHAF_CROP_H;

// Session 75/D -- OUR OWN ORNAMENT. Nothing here is copied, traced or derived from the King
...
const MUSHAF_FRAME_INSET = 2;
const MUSHAF_FRAME_INNER = 0.75;
const MUSHAF_FRAME_GAP = 2;
const MUSHAF_FRAME_OUTER = 1.5;
const MUSHAF_FRAME_BAND = MUSHAF_FRAME_INSET + MUSHAF_FRAME_INNER + MUSHAF_FRAME_GAP + MUSHAF_FRAME_OUTER;
// Where the inner rule hangs: just outside the breathing room.
const MUSHAF_FRAME_INNER_AT = MUSHAF_FRAME_INSET + MUSHAF_FRAME_INNER;
// The two warm tones, and they are the only new colours. The app palette is cool blue
...
const MUSHAF_CORNER_PX = 10;

// Session 75/E -- ZOOM, AND IT IS CONTAINER-LOCAL. The app-wide viewport meta at line 5 is NOT
...
const mushafClampPan = (tx, ty, k, fw, fh, sw, sh) => {
  const mx = Math.max(0, (fw * k - sw) / 2);
  const my = Math.max(0, (fh * k - sh) / 2);
  return { tx: Math.min(mx, Math.max(-mx, tx)), ty: Math.min(my, Math.max(-my, ty)) };
};
// Upstream naming, verified against 604/604 files: three digits always, 001.svg .. 604.svg.
const mushafSvgUrl = (n) => MUSHAF_SVG_ORIGIN + '/pages/' + String(n).padStart(3, '0') + '.svg';
...
const readMushafSvgFlag = () => {
  try {
    ...
    if (p === '1' || p === '0') localStorage.setItem(MUSHAF_SVG_KEY, p);
  } catch (e) {}
  try { return localStorage.getItem(MUSHAF_SVG_KEY) !== '0'; } catch (e) { return true; }
};
const MUSHAF_SVG_ON = readMushafSvgFlag();

/* 14.7 */
// Session 79 -- THE WHOLE MUSHAF: the real printed Madina page, as a page image, on
...
const MADINA_PAGE_CACHE_CAP = 60;
// Root-absolute, like the icons and the manifest: the app is served from '/' and nothing
...
const readMadinaImgFlag = () => {
  let p = null;
  try { p = new URLSearchParams(window.location.search).get('madinaimg'); } catch (e) { p = null; }
  if (p === '1' || p === '0') {
    try { localStorage.setItem(MADINA_IMG_KEY, p); } catch (e) {}
    return p === '1';
  }
  // No parameter: the default is ON, and only a stored, well-formed refusal turns it off.
  try { return localStorage.getItem(MADINA_IMG_KEY) !== '0'; } catch (e) { return true; }
};
const MADINA_IMG_ON = readMadinaImgFlag();
const madinaImgUrl = (n) => {
  if (!MADINA_IMG_ON) return null;
  // A page outside 1..604 is not touched by any of this: this returns null, MushafSheet
  ...
  if (!(n >= 1 && n <= MADINA_IMG_PAGES)) return null;
  return '/assets/madina-hafs/page-' + String(n).padStart(3, '0') + '.webp';
};
// The surface behind the printed page. The scanned page's own margin is #FDFDFD, so on any
...
const MADINA_SHEET_ST = { flex: 1, minHeight: 0, width: '100%', display: 'flex', background: MADINA_DESK };
const MADINA_IMG_ST = { width: '100%', height: '100%', objectFit: 'fill', display: 'block', pointerEvents: 'none', userSelect: 'none', WebkitUserSelect: 'none' };
// The same box, the same element, one property different. The element still fills the
...
// A portrait phone, and nothing else: portrait alone would catch a tall desktop window, and
...
const madinaFillNow = () => {
  try { return window.matchMedia(MADINA_FILL_Q).matches; } catch (e) { return true; }
};
// Rotation must not need a reload: matchMedia fires `change` when the device turns and when
...
function useMadinaFill() {
  const [fill, setFill] = useState(madinaFillNow);
  useEffect(() => {
    if (!MADINA_IMG_ON) return;
    let mq = null;
    try { mq = window.matchMedia(MADINA_FILL_Q); } catch (e) { return; }
    const on = () => setFill(mq.matches);
    on();                                  // the orientation may have turned before this ran
...
// The reading box, inset by the REAL safe area. The app-wide viewport meta is not
...
const MADINA_SAFE_PAD = 'env(safe-area-inset-top, 0px) env(safe-area-inset-right, 0px) env(safe-area-inset-bottom, 0px) env(safe-area-inset-left, 0px)';

// Latency, not bytes: transfer is 0.01-0.05s and time-to-first-byte is ~0.30s, so the win
...
let __mushafSvgWarm = 0;
const prefetchMushafSvg = (n) => {
  if ((!MUSHAF_SVG_ON && !MADINA_IMG_ON) || n === __mushafSvgWarm) return;
  __mushafSvgWarm = n;
  try {
    // ITEM 33. The worker keeps at most MADINA_PAGE_CACHE_CAP pages and evicts the least
    ...
    let warmed = 0;
    for (const d of [1, -1]) {
      if (warmed >= MADINA_PAGE_CACHE_CAP) break;
      const t = n + d;
      if (t < 1 || t > 604) continue;
      const url = madinaImgUrl(t) || (MUSHAF_SVG_ON ? mushafSvgUrl(t) : null);
      if (!url) continue;
      const im = new Image();
      im.decoding = 'async';
      im.src = url;
...
function SheetCorners() {
  const at = (extra) => (
    <svg viewBox="0 0 16 16" width={MUSHAF_CORNER_PX} height={MUSHAF_CORNER_PX} aria-hidden="true" style={{ position: 'absolute', pointerEvents: 'none', ...extra }}>
      <path d="M8 2 L14 8 L8 14 L2 8 Z" fill={MUSHAF_PAPER} stroke="var(--red-deep)" strokeWidth="1.4" />
      <path d="M8 5.6 L10.4 8 L8 10.4 L5.6 8 Z" fill="var(--red-lift)" />
    </svg>
  );
  // Centre the motif on the outer rule's corner: half its own size beyond the band.
  const off = -(MUSHAF_FRAME_BAND + MUSHAF_CORNER_PX / 2);
  return (
    <React.Fragment>
      {at({ top: off, left: off })}
      {at({ top: off, right: off })}
      {at({ bottom: off, left: off })}
      {at({ bottom: off, right: off })}
    </React.Fragment>
  );
}

function MushafSheet({ page, headerSurah, needHeader, needBasmala, onSheetLoad }) {
  const [broke, setBroke] = useState(false);
  // 78 -- the prototype's own latch, and it is a SECOND one on purpose. The fallback chain
  ...
  // 81 -- unconditional, and before every branch, exactly like the two latches above: the
  // hook order is identical on all three renderers whichever one this sheet takes.
  const fill = useMadinaFill();
  const madina = madinaImgUrl(page.n);
  if (madina && !imgBroke)
    return (
      <div style={MADINA_SHEET_ST}>
        {/* S108 -- data-mushaf-page is a POSITIVE handle, and the only reason it exists. The
            live page image has no class and no id, so no rule in the stylesheet can reach it --
            which means its safety today rests on ABSENCE rather than on exclusion, and a guard
            could only ever check that nothing NAMES it. This attribute lets the guard identify
            every mounted page deterministically and freeze the measured geometry instead. It is
            a data attribute and nothing else: no class, no selector, no layout, no paint. No CSS
            in this file may target it, and the guard fails if any does. */}
        <img
          src={madina}
          alt=""
          decoding="async"
          loading="lazy"
          draggable="false"
          data-mushaf-page={page.n}
          style={fill ? MADINA_IMG_ST : MADINA_IMG_ST_FIT}
          onLoad={() => { if (onSheetLoad) onSheetLoad(page.n); }}
          onError={() => setImgBroke(true)}
        />
      </div>
    );
  if (!MUSHAF_SVG_ON || broke)
    return <MushafPage page={page} headerSurah={headerSurah} needHeader={needHeader} needBasmala={needBasmala} />;
  // No interaction of any kind: pointerEvents none lets the strip keep the drag (the touch
  ...
  return (
    <div className="mushaf-paper" style={s.svgFrame}>
      <div style={s.svgRuleOuter} />
      <div style={s.svgRuleInner} />
      <div style={s.svgSheet}>
        <img
          src={mushafSvgUrl(page.n)}
          alt=""
          decoding="async"
          draggable="false"
          style={s.svgImg}
...
function MushafPage({ page, headerSurah, needHeader, needBasmala }) {
  const ref = useRef(null);
  const [fit, setFit] = useState(null);   // null = لم يُقَس بعد

  React.useLayoutEffect(() => {
    if (fit) return;
    const el = ref.current;
    if (!el) return;
    // (أ) عرضُ الخانة من الصفحة نفسِها، ثمّ الثابتُ العالميّ. لا يُقاس هنا سطرٌ
    // واحدٌ لأجل الحجم: K محسوبٌ مرّةً لكلّ الجلسة، وفتحُ الصفحة يقرأه فحسب.
    const rows = el.querySelectorAll('[data-l]');
    let box = 0;
    for (let r = 0; r < rows.length; r++) { const b = rows[r].clientWidth; if (b > box) box = b; }
    if (!box) return;                                  // لم تُقَس خانةٌ ← ابقَ على الأساس
    const K = pgMeasureK(box);
    if (K === null) return;                            // عجز القياسُ ← ابقَ على الأساس
    // (ب) حجمٌ واحدٌ للمصحف كلِّه. الـ٠٫٩٩ هامشُ التقريب نفسُه الذي كان.
    let uni = Math.max(8, Math.min(40, Math.floor(PG_BASE_FS * K * 0.99)));
    // (ج) حارسُ الفيض — الثابتُ الذي لا يُخرَق. أضيقُ نسبةٍ في هذه الصفحة بعينها،
    ...
    for (let r = 0; r < rows.length; r++) {
      ...
      const b = rows[r].clientWidth;
      if (!b || !kids.length) continue;
      let content = 0;
      for (let k = 0; k < kids.length; k++) content += kids[k].offsetWidth;
      content += (kids.length - 1) * (PG_BASE_FS * PG_GAP_EM);
      if (!(content > 0)) continue;
      const f = PG_BASE_FS * (b / content);
      if (f < guard) guard = f;
    }
    if (isFinite(guard) && guard > 0) {
      const g = Math.max(8, Math.min(40, Math.floor(guard * 0.99)));
      if (g < uni) { uni = g; PG_GUARD_PAGES.add(page.n); }
    }
    // (د) الملاءمة الرأسيّة: مجموعُ الأسطر بالحجم الواحد، مضافاً إليه ارتفاعُ ما ليس
    // سطراً (رأسٌ · بسملةٌ · فاصل) كما قيس من الـDOM. فإن فاض ضُرب الحجمُ بنسبةٍ
    // واحدة — والصفحةُ تصغر معاً ولا ينكسر تناسبُها.
    const kidsPage = el.children;
    let otherH = 0;
    for (let c = 0; c < kidsPage.length; c++)
      if (!kidsPage[c].hasAttribute('data-l')) otherH += kidsPage[c].offsetHeight;
    const room = el.clientHeight;
    const total = rows.length * uni * PG_LINE_H * 1.06 + otherH;
    if (room > 0 && total > room)
      uni = Math.max(8, Math.min(40, Math.floor(uni * (room / total) * 0.99)));
    // (هـ) الفائضُ الرأسيّ — كما استقرّ في c4393fe بلا تغيير في جوهره. الحجمُ يُقاس
    ...
    const avail = room - pad;
    const textH = rows.length * uni * PG_LINE_H * 1.06 + otherH;
    if (avail > 0 && textH > 0 && textH < avail) lead = Math.min(PG_LINE_H * 2.0, PG_LINE_H * (avail / textH));
    // البسملةُ تتبع الحجمَ الواحد — ولم يعد ثَمَّ وسيطٌ تتبعه، فالأسطرُ كلُّها سواء.
    setFit({ uni, lead, basmala: Math.round(uni * 0.92) });
  }, [fit]);

  // قبل القياس تُرسم الصفحة كلُّها بحجم الأساس مفرودةً — وهي حالةُ القياس نفسها.
  const basmalaFs = fit ? fit.basmala : Math.round(PG_BASE_FS * 0.92);
  const out = [];
  const done = {};   /* 14.2f4 */  // رأسٌ حُقن لهذه السورة على هذه الصفحة — فلا يُحقن مرّتين
  for (let i = 0; i < page.l.length; i++) {
    const ln = page.l[i];
    if (ln.t === 't') {
      // سطرٌ يفتتح سورةً بلا بسملةٍ في المصدر (٨١ و٨٥) ← الرأسُ ثمّ البسملةُ فوقه.
      ...
      if (op[1] === '1' && op[2] === '1') {
        const sn0 = parseInt(op[0], 10);
        if (needHeader && needHeader[sn0] && !done[sn0]) {
          done[sn0] = 1;
          out.push(<div key={'h' + i} style={s.pgHeader}>{'سُورَةُ ' + SURAH_NAMES[sn0]}</div>);
        }
        if (needBasmala && needBasmala[sn0])
          out.push(<div key={'b' + i} style={{ ...s.pgBasmala, fontSize: basmalaFs }}>{getVerseText(1, 1) || ''}</div>);
      }
      // نفسُ الدالّة التي بها اختير المرشّحون وقيسوا — فلا يفترق المرسومُ عن المقيس.
      const parts = pgLineTokens(ln);
      // حجمٌ واحدٌ لكلّ سطرٍ في الصفحة، وهو نفسُه في الـ٦٠٤ إلا حيث عمل الحارس.
      // و space-between دائماً بلا استثناء: السطرُ يمتدّ من حافةٍ لحافة كالمطبوع.
      const lfs = fit ? fit.uni : PG_BASE_FS;
      out.push(
        <div key={i} data-l={i} style={{ ...s.pgLine, fontSize: lfs, lineHeight: fit ? fit.lead : PG_LINE_H, gap: PG_GAP_EM + 'em' }}>
          {parts.map((p, j) => <span key={j} style={s.pgWord}>{p}</span>)}
        </div>
      );
    } else if (ln.t === 'h') {
      const sn = headerSurah[page.n + ':' + ln.n];
      out.push(<div key={i} style={sn ? s.pgHeader : s.pgBlank}>{sn ? 'سُورَةُ ' + SURAH_NAMES[sn] : ''}</div>);
    } else {
      // ترتيبُ المصحف لا يُقلب: الرأسُ، ثمّ البسملةُ، ثمّ أوّلُ آية.
      ...
      let sb = 0;
      for (let j = i + 1; j < page.l.length; j++) {
        const nx = page.l[j];
        if (nx.t !== 't' || !nx.w || !nx.w.length) continue;
        const b0 = nx.w[0].split(':');
        if (b0[1] === '1' && b0[2] === '1') sb = parseInt(b0[0], 10);
        break;
      }
      if (sb && needHeader && needHeader[sb] && !done[sb]) {
        done[sb] = 1;
        out.push(<div key={'h' + i} style={s.pgHeader}>{'سُورَةُ ' + SURAH_NAMES[sb]}</div>);
      }
      // البسملة من getVerseText(1,1) — لا نكتب نصّاً قرآنياً بيدنا أبداً.
      out.push(<div key={i} style={{ ...s.pgBasmala, fontSize: basmalaFs }}>{getVerseText(1, 1) || ''}</div>);
    }
  }
  // 14.5 — الورقةُ تملأ خانتَها ولا تعانق نصَّها. عناقُ الأمس كان يهبط بالإطار إلى طول
  ...
  const frameSt = s.pgFrame;
  const pageSt = s.pgPage;
  return (
    <div style={frameSt}>
      <div ref={ref} style={pageSt}>{out}</div>
      <PageDecor n={page.n} />
    </div>
  );
}

// Session 51 -- the page dressing: four corner brackets and the page medallion.
...
function PageDecor({ n }) {
  const corner = (extra) => (
    <svg viewBox="0 0 14 14" width="14" height="14" fill="none" stroke="var(--red-lift)" strokeWidth="1.2" strokeLinecap="round" aria-hidden="true" style={{ position: 'absolute', pointerEvents: 'none', ...extra }}>
      <path d="M13 1H4a3 3 0 0 0-3 3v9" />
    </svg>
  );
  return (
    <React.Fragment>
      {corner({ top: 3, left: 3 })}
      {corner({ top: 3, right: 3, transform: 'scaleX(-1)' })}
      {corner({ bottom: 3, left: 3, transform: 'scaleY(-1)' })}
      {corner({ bottom: 3, right: 3, transform: 'scale(-1,-1)' })}
      <div style={s.pgMedal}>{toArabicDigits(n)}</div>
    </React.Fragment>
  );
}

// Session 51 -- the mushaf bookmark (the thread). One flat device key: the child profile
...
const MUSHAF_BOOKMARK_KEY = 'mushaf_bookmark_v1';
// Returns null on the slightest doubt -- missing key, damaged JSON, non-integer or
// out-of-range page or surah -- and never surfaces an error to the reader.
function readMushafBookmark() {
  try {
    const raw = localStorage.getItem(MUSHAF_BOOKMARK_KEY);
    if (!raw) return null;
    const o = JSON.parse(raw);
    if (!o || typeof o !== 'object') return null;
    if (!Number.isInteger(o.p) || o.p < 1 || o.p > 604) return null;
    if (!Number.isInteger(o.s) || o.s < 1 || o.s > 114) return null;
    return { p: o.p, s: o.s };
  } catch (e) { return null; }
}
function writeMushafBookmark(pg, sr) {
  try { localStorage.setItem(MUSHAF_BOOKMARK_KEY, JSON.stringify({ p: pg, s: sr })); } catch (e) {}
}
...
// ---------------------------------------------------------------------------
const MUSHAF_LAST_PAGE_KEY = 'mushaf_last_page_v1';
// Same shape as the bookmark and the same refusal to guess: any doubt at all -- no key,
...
function readMushafLastPage() {
  let raw = null;
  try { raw = localStorage.getItem(MUSHAF_LAST_PAGE_KEY); } catch (e) { return null; }
  if (!raw) return null;
  let o = null;
  try { o = JSON.parse(raw); } catch (e) { return null; }
  if (!o || typeof o !== 'object') return null;
...
// Validated BEFORE the write, so a bad pair is never persisted for the reader above to have
// to reject later. A storage that throws costs nothing: the page is still on the screen.
function writeMushafLastPage(pg, sr) {
  if (!Number.isInteger(pg) || pg < 1 || pg > 604) return;
  if (!Number.isInteger(sr) || sr < 1 || sr > 114) return;
  try { localStorage.setItem(MUSHAF_LAST_PAGE_KEY, JSON.stringify({ p: pg, s: sr })); } catch (e) {}
}
...
function readWirdTarget() {
  let raw = null;
  try { raw = localStorage.getItem(WIRD_TARGET_KEY); } catch (e) { return null; }
...
function writeWirdTarget(n) {
  if (!Number.isInteger(n) || n < 1 || n > 604) return;
  try { localStorage.setItem(WIRD_TARGET_KEY, String(n)); } catch (e) {}
}
function clearWirdTarget() {
  try { localStorage.removeItem(WIRD_TARGET_KEY); } catch (e) {}
}

// Today's completed pages: { d: 'YYYY-MM-DD', pages: [distinct 1..604] }.
const WIRD_DAY_KEY = 'mushaf_wird_day_v1';
// 8 seconds on the page, continuously, or it did not count. Short enough that ordinary
...
const WIRD_DWELL_MS = 8000;
const WIRD_TARGET_PRESETS = [1, 2, 5, 10, 20];
// DEVICE-LOCAL, and deliberately not toISOString. The day boundary a reader lives by is
...
// Anything that is not today's well-formed record reads as TODAY'S EMPTY RECORD -- a record
...
function readWirdDay() {
  const today = wirdDayKey();
  let raw = null;
  try { raw = localStorage.getItem(WIRD_DAY_KEY); } catch (e) { return { d: today, pages: [] }; }
  if (!raw) return { d: today, pages: [] };
  let o = null;
  try { o = JSON.parse(raw); } catch (e) { return { d: today, pages: [] }; }
  if (!o || typeof o !== 'object') return { d: today, pages: [] };
  if (o.d !== today) return { d: today, pages: [] };
  if (!Array.isArray(o.pages)) return { d: today, pages: [] };
  const seen = Object.create(null);
  const pages = [];
  for (const v of o.pages) {
    if (!Number.isInteger(v) || v < 1 || v > 604) continue;
    if (seen[v]) continue;
    seen[v] = 1;
    pages.push(v);
  }
  return { d: today, pages: pages };
}
// The only writer of the daily key. A page already credited today writes NOTHING, so
...
function markWirdPageRead(pg) {
  const rec = readWirdDay();
  if (!Number.isInteger(pg) || pg < 1 || pg > 604) return rec;
  if (rec.pages.indexOf(pg) !== -1) return rec;
  const next = { d: rec.d, pages: rec.pages.concat([pg]) };
  try { localStorage.setItem(WIRD_DAY_KEY, JSON.stringify(next)); } catch (e) {}
  return next;
}
// The picker's own normalisation. Character for character the acceptance the page-jump box
...
function wirdNormalizeDigits(v) {
...
function PagedMushaf({ startSurah, startPage, onExit }) {
  ...
  // 75/B2 -- immersive chrome: two bars the page can take the whole screen from.
  ...
  // The one-shot latch is a REF, not state: a re-render cannot re-fire it, and it is never
  ...
  const chromeAuto = useRef(false);
  // 78 -- the image prototype is a page reader too, so it collapses the bars after the
  // first turn exactly as the SVG reader does. With the prototype off the condition is
  // character for character what it was.
  const readerTurnedPage = () => {
    if ((!MUSHAF_SVG_ON && !MADINA_IMG_ON) || chromeAuto.current) return;
    chromeAuto.current = true;
    setChromeOn(false);
  };
  // 75/E -- null means FIT, and fit means no transform on the element at all rather than a
  ...
  const tapRef = useRef({ t: 0, x: 0, y: 0, timer: null });
  const slotRef = useRef(null);
  const touch = useRef({ x: 0, y: 0, on: false, lock: null });
  const timer = useRef(null);
  // الثابتُ العالميّ يُبطَل في حالتين لا ثالثة، وكلتاهما تجعله كذباً يعمّ الـ٦٠٤:
  ...
  useEffect(() => {
    ...
    const redo = () => { if (!alive) return; pgInvalidateK(); setEpoch((e) => e + 1); };
    const onResize = () => { if (window.innerWidth !== w) { w = window.innerWidth; redo(); } };
    window.addEventListener('resize', onResize);
    // لو كان الخطُّ حاضراً سلفاً (مخزَّناً) فلا إبطالَ ولا إعادةَ قياسٍ بلا سبب.
    try {
  ...
  useEffect(() => () => { if (timer.current) clearTimeout(timer.current); if (tapRef.current.timer) clearTimeout(tapRef.current.timer); }, []);

  // 75/E -- the zoom belongs to the page being looked at and never outlives it. Paging, jumping
  ...
  useEffect(() => { setZoom(null); }, [page]);

  // Session 51 -- the bookmark is placed by hand and only by hand: this button is the sole
  ...
  useEffect(() => {
    if (state !== 'ok') return;
    writeMushafLastPage(page, startSurah);
  }, [state, page, startSurah]);

  // THE DWELL. A page is credited only after it has been the current page continuously for
  ...
  useEffect(() => {
    ...
    const arm = () => {
      disarm();
      if (!visible()) return;
      t = setTimeout(() => { t = null; setWirdDay(markWirdPageRead(page)); }, WIRD_DWELL_MS);
    };
    const onVis = () => {
      if (!visible()) { disarm(); return; }
      // A tab left open across midnight comes back to today's empty record, not yesterday's.
      setWirdDay(readWirdDay());
      arm();
    };
    arm();
  ...
  if (state !== 'ok') {
    return (
      <div style={s.memContainer}>
        {/* S110 -- the same rail as the reading render, in its static variant: there is no page
            under it yet, so it is an ordinary flex child instead of an overlay. */}
        <div className="ezhome ezmr-rail is-static">
          <div className="ezmr-bar">
            <div className="ezmr-title" style={s.ezmrTitle}>المصحف</div>
            <button onClick={onExit} className="ezmr-btn" style={s.ezmrJump}>السور</button>
          </div>
        </div>
        <div style={s.mushafHint}>{state === 'fail' ? 'تعذّر فتح المصحف' : 'جارٍ فتح المصحف…'}</div>
      </div>
    );
  }

  const L = __layoutData.p;
  const cur = L[page - 1];
  const nxt = page < 604 ? L[page] : null;         // التالية — يسارَ الحالية
  ...
  const land = (sl) => {
    ...
    setAnim(false);
    // 77 -- inside the `if`, so a swipe that bounced at page 1 or 604 arrives with sl === 0
    // and is correctly not a page turn. commit() is what the prev/next buttons call too, so
    // both reader-driven paths arrive here.
    if (sl) { setPage(page + sl); setSlide(0); readerTurnedPage(); }
  };
  const commit = (d) => {
    const n = page + d;
    const eff = (n >= 1 && n <= 604) ? d : 0;   // خارج المدى ← ارتدّ في مكانك
    setDrag(0);
    setAnim(true);
    setSlide(eff);
  ...
  // The window the pan is clamped against: the slot's CONTENT box, and the frame's unscaled box.
  ...
  const panBox = () => {
    ...
    const fr = el && el.firstElementChild;
    if (!el || !fr) return null;
    // 78 -- the image path pays no frame band, and its slot padding is the safe area rather
    ...
    if (MADINA_IMG_ON) {
      ...
      return { sw: w, sh: h, fw: w, fh: h };
    }
    return {
      sw: el.clientWidth - 2 * MUSHAF_FRAME_BAND,
      sh: el.clientHeight - 2 * MUSHAF_FRAME_BAND,
      fw: fr.offsetWidth, fh: fr.offsetHeight,
    };
  };

  const onStart = (e) => {
    if (slide) return;                           // ورقةٌ في الطيران — لا تلمسها
    const t = e.touches[0];
    touch.current = { x: t.clientX, y: t.clientY, on: true, lock: null };
    setAnim(false);
  };
  const onMove = (e) => {
    ...
    if (!T.on) return;
    const t = e.touches[0];
    // 75/E -- magnified: one finger pans, clamped, and the page flip does not get a look in.
    ...
    if (zoom) {
      const b = panBox();
      const ddx = t.clientX - T.x, ddy = t.clientY - T.y;
      setZoom((z) => {
        if (!z) return z;
        if (!b) return z;
        const p = mushafClampPan(z.tx + ddx, z.ty + ddy, z.k, b.fw, b.fh, b.sw, b.sh);
        return { k: z.k, tx: p.tx, ty: p.ty };
      });
      T.x = t.clientX; T.y = t.clientY;
      T.lock = 'z';
      return;
    }
    const dx = t.clientX - T.x, dy = t.clientY - T.y;
    if (T.lock === null && (Math.abs(dx) > 8 || Math.abs(dy) > 8)) T.lock = Math.abs(dx) > Math.abs(dy) ? 'x' : 'y';
    if (T.lock === 'x') setDrag(dx);
  };
  const onEnd = () => {
    const T = touch.current;
    if (!T.on) return;
    T.on = false;
    // 75/B2 -- lock is set on the first move past 8px in either axis, so lock === null means the
    ...
    if (T.lock === null) { setDrag(0); if (MUSHAF_SVG_ON || MADINA_IMG_ON) onTap(T.x, T.y); return; }   /* 78 -- the tap gestures belong to the image path too */
    if (T.lock !== 'x') { setDrag(0); return; }
    if (drag > 55) commit(1);
    else if (drag < -55) commit(-1);
    else { setDrag(0); setAnim(true); }
  };

  /* 14.2f1 */
  // الشريط الآن موضوعٌ وضعاً مطلقاً، والخانات بإزاحاتٍ صريحة (-100% / 0 / +100%).
  ...
  const slotSt = MADINA_IMG_ON
    ? { ...s.pgSlot, padding: MADINA_SAFE_PAD }
    : MUSHAF_SVG_ON ? { ...s.pgSlot, padding: MUSHAF_FRAME_BAND } : s.pgSlot;
  // 75/D -- the warm surface the page sits on. pgViewport is SHARED with the fallback reader, so
  ...
  const vpSt = MADINA_IMG_ON
    ? { ...s.pgViewport, background: MADINA_DESK, ...(zoom ? { touchAction: 'none' } : {}) }
    : MUSHAF_SVG_ON ? { ...s.pgViewport, background: MUSHAF_DESK, ...(zoom ? { touchAction: 'none' } : {}) } : s.pgViewport;
  // 78 -- THE FULL-SCREEN CONTRACT, and all of it is these three objects.
  ...
  const contSt = MADINA_IMG_ON ? { ...s.memContainer, position: 'relative' } : s.memContainer;
  // S111 -- the rollback branch reads memHeaderFb, whose BOX is memHeader's box property for
  ...
  const headSt = MADINA_IMG_ON
    ? { ...s.memHeader, position: 'absolute', top: 0, left: 0, right: 0, zIndex: 4,
  ...
  const zoomSt = zoom ? { transform: 'translate(' + zoom.tx + 'px, ' + zoom.ty + 'px) scale(' + zoom.k + ')', willChange: 'transform' } : null;
  const strip = {
    ...s.pgStrip,
    transform: 'translateX(calc(' + (slide * 100) + '% + ' + drag + 'px))',
    transition: anim ? 'transform .26s cubic-bezier(.22,.61,.36,1)' : 'none',
  };
  const firstText = cur.l.find((x) => x.t === 't' && x.w && x.w.length);
  const curSurah = firstText ? parseInt(firstText.w[0].split(':')[0], 10) : null;
  const hs = idx.current.headerSurah;
  const nh = idx.current.needHeader;
  const nb = idx.current.needBasmala;

  return (
    <div className="ezmr-scope" style={contSt}>
      {/* S110 -- THE TOP READER RAIL. Same gate, same handlers, same accessible names: only the
          furniture changed. The bookmark keeps its two titles and its two aria-labels, and the
          way back is still the same text on the same onExit. The rollback reader (?madinaimg=0)
          keeps the bars it was measured with -- its page is a FLEX CHILD of this column, so
          replacing its in-flow header would move the paper, and the paper is frozen. */}
      {chromeOn && (MADINA_IMG_ON ? (
      <div className="ezhome ezmr-rail">
        <div className="ezmr-bar">
          <button onClick={putMark} title={marked ? 'علامتك هنا' : 'ضع العلامة'} aria-label={marked ? 'علامتك هنا' : 'ضع العلامة'} className={marked ? 'ezmr-btn is-on' : 'ezmr-btn'}>
            <svg viewBox="0 0 24 24" width="20" height="20" fill={marked ? 'var(--red)' : 'none'} stroke={marked ? 'var(--red-deep)' : 'currentColor'} strokeWidth={marked ? 1.5 : 1.8} strokeLinejoin="round" aria-hidden="true"><path d="M7 3h10a1 1 0 0 1 1 1v17l-6-4-6 4V4a1 1 0 0 1 1-1z" /></svg>
          </button>
          <div className="ezmr-title" style={s.ezmrTitle}>{curSurah ? SURAH_NAMES[curSurah] : 'المصحف'} · صفحة {toArabicDigits(page)}</div>
          <button onClick={onExit} className="ezmr-btn" style={s.ezmrJump}>السور</button>
        </div>
      </div>
      ) : (
      <div className="ezhome" style={headSt}>
        <div className="ezmr-fb-inner">
          <div style={s.memTitleFb}>{curSurah ? SURAH_NAMES[curSurah] : 'المصحف'}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={putMark} title={marked ? 'علامتك هنا' : 'ضع العلامة'} aria-label={marked ? 'علامتك هنا' : 'ضع العلامة'} className={marked ? 'ezmr-fb-btn is-on' : 'ezmr-fb-btn'} style={{ ...s.memBtnFb, padding: '0 10px', display: 'inline-flex', alignItems: 'center' }}>
              <svg viewBox="0 0 24 24" width="20" height="20" fill={marked ? 'var(--red)' : 'none'} stroke={marked ? 'var(--red-deep)' : 'currentColor'} strokeWidth={marked ? 1.5 : 1.8} strokeLinejoin="round" aria-hidden="true"><path d="M7 3h10a1 1 0 0 1 1 1v17l-6-4-6 4V4a1 1 0 0 1 1-1z" /></svg>
            </button>
            <button onClick={onExit} className="ezmr-fb-btn" style={s.memBtnFb}>السور</button>
          </div>
        </div>
      </div>
      ))}
      <div style={vpSt} onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd} onTouchCancel={onEnd}>
        <div style={strip} onTransitionEnd={onTransEnd}>
          {/* Session 74 -- MushafSheet is MushafPage when the flag is off (and when the image
              fails). Same three slots, same keys, same props. Only the current sheet carries
              onSheetLoad, so the neighbour prefetch fires once the visible page has loaded. */}
          <div style={{ ...slotSt, left: '-100%' }}>{nxt && <MushafSheet key={nxt.n + ':' + epoch} page={nxt} headerSurah={hs} needHeader={nh} needBasmala={nb} />}</div>
          <div ref={slotRef} style={{ ...slotSt, left: '0%', ...zoomSt }}><MushafSheet key={cur.n + ':' + epoch} page={cur} headerSurah={hs} needHeader={nh} needBasmala={nb} onSheetLoad={prefetchMushafSvg} /></div>
...
function MushafScreen({ selected, setSelected, onBack, onPlaySurah, onStopAudio }) {
  ...
  const [openAt, setOpenAt] = useState(null);       // set only by tapping the bar
  // 14.3 — صفوفُ الفهرس (١١٤ سورة + ٣٠ جزءاً) حين يصل التخطيط، وإلّا null.
  const [nav, setNav] = useState(__mushafNav);
  const openBookmark = () => { if (bookmark) { setOpenAt(bookmark); setSelected(bookmark.s); } };
  const clearBookmark = () => { try { localStorage.removeItem(MUSHAF_BOOKMARK_KEY); } catch (e) {} setBookmark(null); };
  // Session 82 -- the last page, alongside the bookmark and never instead of it. Its own
  ...
  // خطأ ٤٦: `selected` (السورة المفتوحة) رُفعت إلى App لتُقشَر بزر الرجوع؛ تصل عبر props بلا تغيير سلوك
  ...
  useEffect(() => {
    ...
    loadQuran()
      .then(() => {
        if (!alive) return;
        const c = {};
        for (const k in __quranData) {
          const n = parseInt(k.split(':')[0], 10);
          c[n] = (c[n] || 0) + 1;
        }
        setCounts(c);
        setReady(true);
  ...
  // 14.3 — التخطيطُ لا يحبس الفهرس: السورُ تُرسَم فوراً بلا أرقام، ثمّ تُحقن أرقامُ
  // الصفحات وتُدسّ صفوفُ الأجزاء حين يصل. وإن لم يصل بقيت القائمةُ كما كانت.
  useEffect(() => {
    let alive = true;
    loadLayout().then(() => { if (alive) setNav(buildMushafNav()); }).catch(() => {});
    return () => { alive = false; };
  }, []);

  // Session 51 -- MushafScreen never unmounts when a surah is left (only `selected` drops to
  ...
  const leaveSurah = () => { if (onStopAudio) onStopAudio(); setOpenAt(null); setSelected(null); };
  const leaveScreen = () => { if (onStopAudio) onStopAudio(); onBack(); };
  // S87: the open surah IS this screen's nested layer, so a hardware/browser back peels it back
  ...
  useEzikBackLayer(selected != null, leaveSurah);

  // 14.2 — الفهرس يفتح على صفحة السورة في المصحف المُصفَّح.
  ...
  const navRows = nav || MUSHAF_NAV_FALLBACK;
  const juzRows = navRows.filter((r) => r.k === 'j');
  const surahRows = navRows.filter((r) => r.k === 's');

  return (
    // S109 -- the INDEX only. The reader below is untouched: pressing a surah still runs the
    ...
    <EzShell title={'\u0627\u0644\u0645\u0635\u062d\u0641'} onBack={leaveScreen} backLabel={'\u0631\u062c\u0648\u0639'}>
      <div>
        ...
        {/* the bounded masthead: only state this screen already holds -- the last page it
            wrote and the bookmark it stored. Nothing is invented and nothing new is read. */}
        {(lastPage || bookmark) && (
        <section className="ezq-masthead is-strip">
        {lastPage && (
          <button onClick={openLastPage} style={s.mushafRow}>
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="var(--red-deep)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ flexShrink: 0 }}><path d="M3.5 12a8.5 8.5 0 1 0 2.7-6.2" /><path d="M3.1 4.4v4.3h4.3" /><path d="M12 7.5V12l3 1.8" /></svg>
            <div style={s.mushafName}>تابِع القراءة · صفحة {toArabicDigits(lastPage.p)}</div>
          </button>
        )}
        {bookmark && (
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={openBookmark} style={{ ...s.mushafRow, flex: 1 }}>
              <svg viewBox="0 0 24 24" width="18" height="18" fill="var(--red)" stroke="var(--red-deep)" strokeWidth="1.5" strokeLinejoin="round" aria-hidden="true" style={{ flexShrink: 0 }}><path d="M7 3h10a1 1 0 0 1 1 1v17l-6-4-6 4V4a1 1 0 0 1 1-1z" /></svg>
              <div style={s.mushafName}>علامتك · صفحة {toArabicDigits(bookmark.p)}</div>
            </button>
            <button onClick={clearBookmark} title="إزالة العلامة" style={{ ...s.pgNavBtn, height: 'auto' }}>×</button>
          </div>
        )}
        </section>
        )}
        ...
        {/* S110 -- the juz, in a DENSER grid of their own above the surahs: .ezm-juzgrid is not
            .ezq-cat and shares none of its columns. Same rows, same opener, same style keys. */}
        {ready && juzRows.length > 0 && (
          <React.Fragment>
            <h2 className="ezm-sec">الانتقال إلى جزء</h2>
            <div className="ezm-juzgrid">
              {juzRows.map((r) => (
                <button key={'j' + r.n} onClick={() => { setOpenAt({ p: r.p, s: r.s }); setSelected(r.s); }} style={s.mushafJuzRow}>
                  <div style={s.mushafJuzName}>الجزء {toArabicDigits(r.n)}</div>
                  <div style={s.mushafJuzPage}>صفحة {toArabicDigits(r.p)}</div>
                </button>
              ))}
            </div>
          </React.Fragment>
        )}
        {ready && <h2 className="ezm-sec">السور</h2>}
        {ready && (
          <div className="ezq-cat">
            {surahRows.map((r) => (
              <button key={'s' + r.n} className="ezhome-focus ezq-card" data-ezm-surah={r.n}
                onClick={() => { setOpenAt(null); setSelected(r.n); }} style={s.ezmCard}>
                <span className="ezq-crest" aria-hidden="true" />
                <span style={s.ezmTop}>
                  <span style={s.ezmNum}>{toArabicDigits(r.n)}</span>
                  <span style={s.ezmReveal}>{revelationLabel(r.n)}</span>
                </span>
                <span style={s.ezmName}>{SURAH_NAMES[r.n]}</span>
                <span style={s.ezmFoot}>
                  <span style={s.ezmCount}>{toArabicDigits(counts && counts[r.n] ? counts[r.n] : 0)} آية</span>
                  <span style={s.ezmGo} aria-hidden="true">{EZH_ICON_GO}</span>
                </span>
                <span style={s.ezmPage}>{r.p ? 'صفحة ' + toArabicDigits(r.p) : ''}</span>
              </button>
            ))}
          </div>
        )}
...
function MemorizeScreen({ profile, onExit, onPlayVerse, onPlaySurah, onStopAudio }) {
  ...
  // Layer C — عدنان listen-and-repeat audio talqin (alongside the manual reveal).
  ...
  const [adnanRunning, setAdnanRunning] = useState(false);
  const adnanTimeoutRef = useRef(null);                 // pending repeat-pause timer
  ...
  const [reciteStates, setReciteStates] = useState([]); // per-expected-word: matched|mismatch|pending
  ...
  // Load the mushaf once, then tally ayah counts per surah in a SINGLE pass over all keys
    ...
    loadQuran()
      .then(() => {
        ...
        for (const k in __quranData) {
          ...
          if (colon < 1) continue;
          const sn = parseInt(k.slice(0, colon), 10);
          if (sn >= 1) c[sn] = (c[sn] || 0) + 1;
        }
        setCounts(c);
        setReady(true);
      })
      .catch(() => { /* fail-safe: stay on the loading string; never fall back to model text */ });
    return () => { alive = false; };
  }, []);

  // Layer C teardown — runs on MemorizeScreen unmount (leaving the memorize screen). cancelAudio
  ...
  useEffect(() => () => {
    runIdRef.current++;
    if (adnanTimeoutRef.current) clearTimeout(adnanTimeoutRef.current);
    // Layer B: stop the recite recognizer on unmount (the App [screen] cleanup doesn't know it).
    reciteRunIdRef.current++;
    ezKillRecognizer(reciteRecognitionRef.current);
    reciteRecognitionRef.current = null;
  }, []);

  const surahCount = (counts && selectedSurah) ? (counts[selectedSurah] || 0) : 0; // last ayah of range
  ...
  const buildUnits = () => {
    if (!ready || !selectedSurah || !surahCount) return [];
    const out = [];
    for (let a = startAyah; a <= surahCount; a++) {
      const text = getVerseText(selectedSurah, a) || '';
      if (granularity === 'word') {
        const words = text.split(/\s+/).filter(Boolean);
        words.forEach((w, wi) => out.push({ ayah: a, text: w, firstInAyah: wi === 0 }));
      } else {
        out.push({ ayah: a, text, firstInAyah: true });
      }
    }
    return out;
  };
  const units = view === 'drill' ? buildUnits() : [];
  const allRevealed = units.length > 0 && revealedCount >= units.length;
  // Focus ayah for the single-ayah "listen": the ayah holding the next hidden unit, else the last.
  const focusAyah = units.length === 0 ? startAyah
    : (revealedCount < units.length ? units[revealedCount].ayah : units[units.length - 1].ayah);
  // Light, occasional praise — every few units while drilling, NOT on every press.
  const showPraise = !allRevealed && granularity === 'ayah' && revealedCount > 0 && revealedCount % 4 === 0;

  const openPicker = () => { setView('picker'); setRevealedCount(0); };
  const startDrill = () => { setView('drill'); setRevealedCount(0); };
  const setGran = (g) => { setGranularity(g); setRevealedCount(0); }; // switching re-hides the range
  ...
  const stopAdnan = () => {
    runIdRef.current++;                       // invalidate the running loop so it bails at its next guard
    if (adnanTimeoutRef.current) { clearTimeout(adnanTimeoutRef.current); adnanTimeoutRef.current = null; }
    if (onStopAudio) onStopAudio();           // App cancelAudio: pause current ayah + bump sequenceIdRef
    setAdnanRunning(false);
  };

  const startAdnan = async () => {
    if (!ready || !selectedSurah || !surahCount) return;
    if (onStopAudio) onStopAudio();           // clear any prior audio before starting fresh
    if (adnanTimeoutRef.current) { clearTimeout(adnanTimeoutRef.current); adnanTimeoutRef.current = null; }
    const myRun = ++runIdRef.current;         // local cancel token for THIS run
    setAdnanRunning(true);
    for (let a = startAyah; a <= surahCount; a++) {
      if (runIdRef.current !== myRun) return; // stopped / mode switched / screen left / restarted
      setAdnanAyah(a);
      const t0 = performance.now();
      try { await onPlayVerse(selectedSurah, a); } catch (e) {} // resolves when the ayah audio ENDS
      if (runIdRef.current !== myRun) return;
      const elapsed = performance.now() - t0;
      const pause = Math.min(ADNAN_PAUSE_CEIL, Math.max(ADNAN_PAUSE_FLOOR, elapsed * ADNAN_PAUSE_FACTOR));
      await new Promise((r) => { adnanTimeoutRef.current = setTimeout(r, pause); });
      if (runIdRef.current !== myRun) return;
    }
    if (runIdRef.current === myRun) { setAdnanRunning(false); setAdnanAyah(null); } // natural completion
  };

  const repeatAyah = async () => {
    const a = adnanAyah || startAyah;
    stopAdnan();                              // halt the loop + audio + pending pause first
  ...
  const stopRecite = () => {
    reciteRunIdRef.current++;            // invalidate: a stale onend can't restart the mic
    setReciteListening(false);
    ezKillRecognizer(reciteRecognitionRef.current);
    reciteRecognitionRef.current = null;
    // leave reciteStates as-is so the last result stays visible after stop
  };

  const startRecite = () => {
    if (childVoiceBlocked()) { setReciteErr(CHILD_VOICE_NOTICE); return; } // غ‑٣: لا ميكروفونَ لسمِّعني
    ...
    if (!hasValidAIConsent()) { setReciteErr(EZ_SPEECH_NO_CONSENT); return; }
    if (!ezSpeechAvailable()) { setReciteErr(RECITE_NO_SR); return; } // graceful fail, same message as chat/call
    setReciteErr('');
    setReciteFinished(false);
    setReciteAyah(startAyah);
    reciteAyahRef.current = startAyah;
    reciteHeardRef.current = '';
    setReciteStates([]);
    // Cumulative EXPECTED (Option C): expected = tokens of ayat startAyah..targetAyah, with the
    ...
    rec.continuous = true;
    rec.interimResults = true;
    const myRun = ++reciteRunIdRef.current;
    rec.onresult = (event) => {
      // Cumulative-finals prefix-merge — identical to the call recognizer.
      let finalText = '', interim = '';
      for (let i = 0; i < event.results.length; i++) {
        const t = event.results[i][0].transcript;
        if (event.results[i].isFinal) {
          ...
          if (!seg) continue;
          if (!finalText) finalText = seg;
          else if (seg.startsWith(finalText)) finalText = seg;
          else if (finalText.startsWith(seg)) { /* covered */ }
          else finalText = finalText + ' ' + seg;
        } else { interim += t; }
      }
      reciteHeardRef.current = (finalText + ' ' + interim).trim();
      ...
      if (allMatched && cur < surahCount) {
        ...
        const next = cur + 1;
        reciteAyahRef.current = next;
        setReciteAyah(next);
        setReciteStates([]);
      } else {
        setReciteStates(shown);
        if (allMatched && cur >= surahCount) { setReciteFinished(true); stopRecite(); } // last ayah → finish
      }
    };
    rec.onend = () => {
      // Auto-restart only if this run is still current (stopRecite/unmount bumped the id otherwise).
      if (childVoiceBlocked()) {                         // غ‑٣: لا إعادةَ فتحٍ بعد الحجب — أوقفِ الحلقة
        reciteRunIdRef.current++;
        setReciteListening(false);
        return;
      }
      // Withdrawal mid-recitation ends the loop here as well as at the registry: this callback
      ...
      if (!hasValidAIConsent()) {
        reciteRunIdRef.current++;
        setReciteListening(false);
        setReciteErr(EZ_SPEECH_NO_CONSENT);
        ezKillRecognizer(rec);
        return;
      }
      if (reciteRunIdRef.current === myRun) { ezStartRecognition(rec); }
    };
    rec.onerror = (e) => {
      const fatal = ['not-allowed', 'audio-capture', 'service-not-allowed'];
      if (fatal.includes(e.error)) { reciteRunIdRef.current++; setReciteListening(false); setReciteErr(RECITE_NO_SR); }
    };
    reciteRecognitionRef.current = rec;
    setReciteListening(true);
    if (!ezStartRecognition(rec)) { reciteRunIdRef.current++; setReciteListening(false); }
  };

  // Manual advance — freeze-proof fallback so a missed/substituted last word can't lock progress.
  const advanceReciteManual = () => {
    const cur = reciteAyahRef.current || startAyah;
    if (cur < surahCount) {
      const next = cur + 1;
      reciteAyahRef.current = next;
      setReciteAyah(next);
      setReciteStates([]); // new ayah starts dim; recomputed on the next onresult
    } else {
  ...
  const setMode = (m) => {
    if (m === drillMode) return;
    stopAdnan();
    stopRecite();
    if (m === 'recite') { // fresh recite session from the start of the range
      setReciteStates([]); reciteHeardRef.current = ''; setReciteErr('');
      setReciteAyah(startAyah); reciteAyahRef.current = startAyah; setReciteFinished(false);
    }
    setDrillMode(m);
  };

  // ===== THE MEMORIZER'S NESTED CLOSER =====
  ...
  const leaveDrill = () => { stopAdnan(); stopRecite(); openPicker(); };
  useEzikBackLayer(view === 'drill', leaveDrill);

  // ---------- PICKER ----------
  if (view === 'picker') {
    return (
      <EzShell title={MEM.TITLE} onBack={onExit} backLabel={MEM.BACK_BTN}>
        {/* S106 -- the bounded masthead. It carries the screen's OWN heading and, when the
            reader has already chosen a surah, the real selection -- nothing is invented and no
            progress is claimed that this screen does not already hold. */}
        <section className="ezq-masthead">
          <div style={s.ezqMastTitle}>{MEM.PICKER_HEADING}</div>
          {selectedSurah ? (
            <div style={s.ezqMastPick}>{SURAH_NAMES[selectedSurah]}</div>
          ) : null}
          {!ready && <div style={s.ezqMastHint}>{MEM.LOADING}</div>}
        </section>
        {/* THE CANONICAL 114, from the same source and in the same order as before: the range
            1..114 mapped once, SURAH_NAMES for the name, the single-pass `counts` tally for the
            ayah count and revelationLabel for the Meccan/Medinan word. No list is re-derived,
            re-sorted or filtered here, and no metadata is written down in this file. */}
        <div className="ezq-cat">
          {Array.from({ length: 114 }, (_, i) => i + 1).map((n) => {
            const active = selectedSurah === n;
            return (
              <button
                key={n}
                type="button"
                className="ezhome-focus ezq-card"
                data-ezq-surah={n}
                aria-pressed={active ? 'true' : 'false'}
                onClick={() => { setSelectedSurah(n); setStartAyah(1); }}
                style={{ ...s.ezqCard, ...(active ? s.ezqCardOn : null) }}
              >
                <span className="ezq-crest" aria-hidden="true" />
                <span style={s.ezqCardTop}>
                  <span style={active ? { ...s.ezqNum, ...s.ezqNumOn } : s.ezqNum}>{toArabicDigits(n)}</span>
                  <span style={s.ezqReveal}>{revelationLabel(n)}</span>
                </span>
                <span style={s.ezqName}>{SURAH_NAMES[n]}</span>
                <span style={s.ezqCount}>{counts && counts[n] ? (toArabicDigits(counts[n]) + ' ' + MEM.GRAN_AYAH) : ''}</span>
              </button>
            );
          })}
        </div>
        ...
        {selectedSurah && (
          <div style={s.ezqStartBar}>
            <span style={s.ezqStartLabel}>{MEM.START_LABEL}</span>
            <select
              value={startAyah}
              onChange={(e) => setStartAyah(parseInt(e.target.value, 10) || 1)}
              style={s.memAyahSelect}
              disabled={!ready || !surahCount}
            >
              {Array.from({ length: surahCount || 1 }, (_, i) => i + 1).map((a) => (
                <option key={a} value={a}>{toArabicDigits(a)}</option>
              ))}
            </select>
            <button onClick={startDrill} style={{ ...s.memStartBtn, opacity: (!ready || !surahCount) ? 0.5 : 1 }} disabled={!ready || !surahCount}>{MEM.START_BTN}</button>
          </div>
        )}
      </EzShell>
    );
  ...
  // ---------- DRILL ----------
  const praiseLine = isFemale ? MEM.PRAISE_F : MEM.PRAISE_M;
  const finishLine = isFemale ? MEM.FINISH_F : MEM.FINISH_M;
  return (
    // S107 -- the drill is on the SAME shell as the picker, and its back is the SAME back:
    ...
    <EzShell title={MEM.TITLE} onBack={ezikGoBack} backLabel={MEM.BACK_BTN}>
      {/* the bounded masthead: the SAME surah, the SAME ayah badge and the SAME change-surah
          control, which still leaves by the same door so the drill's history entry is spent. */}
      <section className="ezq-masthead">
        <div style={s.ezqDrillSurah}>{SURAH_NAMES[selectedSurah]}</div>
        <div style={s.ezqDrillRow}>
          <span style={s.ezqDrillBadge}>{`${MEM.GRAN_AYAH} ${toArabicDigits(startAyah)}`}</span>
          <button onClick={ezikGoBack} style={s.ezqDrillChange}>{MEM.CHANGE_SURAH}</button>
        </div>
      </section>
      {/* Drill mode switch: manual reveal · عدنان listen-and-repeat (Layer C) · سمِّعني recite (Layer B). */}
      <div style={s.memToggleRow}>
        <button onClick={() => setMode('manual')} style={{ ...s.memGranBtn, ...(drillMode === 'manual' ? s.memGranBtnActive : {}) }}>{MEM.MODE_MANUAL}</button>
        <button onClick={() => setMode('adnan')} style={{ ...s.memGranBtn, ...(drillMode === 'adnan' ? s.memGranBtnActive : {}) }}>{MEM.MODE_ADNAN}</button>
        <button onClick={() => setMode('recite')} style={{ ...s.memGranBtn, ...(drillMode === 'recite' ? s.memGranBtnActive : {}) }}>{MEM.MODE_RECITE}</button>
      </div>
      {drillMode === 'manual' && (
      <div style={s.memToggleRow}>
        <span style={s.memToggleLabel}>{MEM.REVEAL_LABEL}</span>
        <button onClick={() => setGran('ayah')} style={{ ...s.memGranBtn, ...(granularity === 'ayah' ? s.memGranBtnActive : {}) }}>{MEM.GRAN_AYAH}</button>
        <button onClick={() => setGran('word')} style={{ ...s.memGranBtn, ...(granularity === 'word' ? s.memGranBtnActive : {}) }}>{MEM.GRAN_WORD}</button>
      </div>
      )}
      {drillMode === 'manual' && (
      <div className="ezq-read" style={s.memDrillArea}>
        <div style={s.memDrillText}>
          {units.map((u, i) => {
            const revealed = i < revealedCount;
            const isNext = i === revealedCount;
            if (!revealed && !isNext) return null; // beyond the next unit stays fully hidden
            return (
              <span key={i} style={s.memUnit}>
                {u.firstInAyah && <span style={s.memAyahBadge}>{toArabicDigits(u.ayah)}</span>}
                {revealed
                  ? <span style={s.memWord}>{u.text}</span>
                  : <span style={s.memPlaceholder}>{granularity === 'word' ? '—' : '— — —'}</span>}
              </span>
            );
          })}
        </div>
        {showPraise && <div style={s.memMsg}>{praiseLine}</div>}
        {allRevealed && <div style={s.memMsg}>{finishLine}</div>}
      </div>
      )}
      {drillMode === 'manual' && (
      <div className="ezq-drill-acts" style={s.memControls}>
        <button onClick={() => onPlayVerse && onPlayVerse(selectedSurah, focusAyah)} style={s.memListenBtn}>{MEM.LISTEN_BTN}</button>
        <button onClick={() => setRevealedCount((c) => Math.min(c + 1, units.length))} style={{ ...s.memRevealBtn, opacity: allRevealed ? 0.5 : 1 }} disabled={allRevealed}>{MEM.REVEAL_BTN}</button>
        {/* Re-hide/reset the same range. */}
        <button onClick={() => setRevealedCount(0)} style={s.memListenBtn}>{MEM.HIDE}</button>
      </div>
      )}
      {drillMode === 'adnan' && (
      <div className="ezq-read" style={s.memDrillArea}>
        <div style={s.memSurahTitle}>{`${SURAH_NAMES[selectedSurah]} · ${MEM.ADNAN_AYAH_LABEL} ${toArabicDigits(adnanAyah || startAyah)}`}</div>
        <div style={s.memDrillText}>
          <span style={s.memWord}>{getVerseText(selectedSurah, adnanAyah || startAyah) || ''}</span>
        </div>
      </div>
      )}
      {drillMode === 'adnan' && (
      <div className="ezq-drill-acts" style={s.memControls}>
        <button onClick={() => (adnanRunning ? stopAdnan() : startAdnan())} style={s.memRevealBtn}>{adnanRunning ? MEM.ADNAN_STOP : MEM.ADNAN_START}</button>
        <button onClick={repeatAyah} style={s.memListenBtn}>{MEM.ADNAN_REPEAT}</button>
      </div>
      )}
      {drillMode === 'recite' && (
      <div className="ezq-read" style={s.memDrillArea}>
        <div style={s.memReciteHint}>{MEM.RECITE_HINT}</div>
        <div style={s.memSurahTitle}>{`${SURAH_NAMES[selectedSurah]} · ${MEM.ADNAN_AYAH_LABEL} ${toArabicDigits(reciteAyah || startAyah)}`}</div>
        <div style={s.memDrillText}>
          {(() => {
            // Display the CURRENT recite ayah (original diacritized words), colored by its slice of
            ...
            const displayWords = (getVerseText(selectedSurah, reciteAyah || startAyah) || '').split(/\s+/).filter(Boolean);
            let wordIdx = 0;
            return displayWords.map((dw, i) => {
              if (!normalizeForRecite(dw)) return <span key={i} style={s.memWordPending}>{dw}</span>;
              const st = reciteStates[wordIdx]; wordIdx++;
              const wstyle = st === 'matched' ? s.memWord : st === 'mismatch' ? s.memWordWrong : s.memWordPending;
              return <span key={i} style={wstyle}>{dw}</span>;
            });
          })()}
        </div>
        {reciteErr && <div style={s.memMsg}>{reciteErr}</div>}
        {reciteFinished && <div style={s.memMsg}>{finishLine}</div>}
      </div>
      )}
      {drillMode === 'recite' && (
      <div className="ezq-drill-acts" style={s.memControls}>
        <button onClick={() => (reciteListening ? stopRecite() : startRecite())} style={s.memRevealBtn}>{reciteListening ? MEM.RECITE_STOP : MEM.RECITE_START}</button>
        <button onClick={advanceReciteManual} style={s.memListenBtn}>{MEM.RECITE_NEXT}</button>
      </div>
      )}
    </EzShell>
  );
...
// ============================================================
const s = {
  // ITEM 107: the times table and its two choosers. Type and tokens only.
  prayerRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '7px 0', borderBottom: '1px solid var(--line)' },
  prayerName: { fontSize: 15, lineHeight: 1.7, color: 'var(--ink)' },
  prayerTime: { fontSize: 16, lineHeight: 1.7, fontWeight: 700, color: 'var(--ink)' },
  prayerOptRow: { display: 'flex', flexWrap: 'wrap', gap: 6, margin: '2px 0 4px' },
  prayerOpt: { background: 'none', border: '1px solid var(--line)', borderRadius: 10, padding: '6px 10px', fontSize: 13, fontFamily: 'inherit', color: 'inherit', cursor: 'pointer' },
  prayerStep: { display: 'inline-flex', alignItems: 'center', gap: 8 },
  prayerStepBtn: { background: 'none', border: '1px solid var(--line)', borderRadius: 8, padding: '2px 10px', fontSize: 15, fontFamily: 'inherit', color: 'inherit', cursor: 'pointer' },
  prayerOffVal: { minWidth: 28, textAlign: 'center', fontSize: 14, color: 'var(--muted)' },
  // ITEM 108-أ: the qibla panel. Type and one dial, every colour a token.
  ...
  hijriNow: { marginTop: 8, fontSize: 13, lineHeight: 1.7, color: 'var(--muted)' },
  // ITEM 93-ج: the offline-store notice. Every colour is a token, so it follows the theme and
  ...
  swNote: { position: 'fixed', insetInlineStart: 12, insetInlineEnd: 12, insetBlockEnd: 12, zIndex: 60, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', boxSizing: 'border-box', padding: '10px 14px', borderRadius: 12, border: '1px solid var(--warn-line)', background: 'var(--warn-bg)', color: 'var(--warn-ink)', boxShadow: 'var(--a3-lift)' },
  swNoteText: { flex: '1 1 200px', minWidth: 0, fontSize: 13, lineHeight: 1.7 },
  swNoteHit: { display: 'inline-flex', flexShrink: 0 },
  swNoteBtn: { background: 'none', border: '1px solid var(--warn-line)', borderRadius: 8, padding: '4px 14px', fontSize: 13, fontFamily: 'inherit', color: 'inherit', cursor: 'pointer' },
  // S115: the boot mark keeps its key and its animation -- only the 48px emoji it used to size is
  // gone, so reduced motion, the keyframe and the duration are all untouched.
  loadingScreen: { minHeight: '100vh', background: 'var(--boot-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  loadingSpinner: { animation: 'pulse 1.5s ease-in-out infinite' },
  // S95. Onboarding was written entirely in literals, so it stayed light for a user who had
  ...
  onboardingContainer: { height: '100dvh', overflowY: 'auto', WebkitOverflowScrolling: 'touch', boxSizing: 'border-box', background: 'var(--boot-bg)' },
  onboardingInner: { minHeight: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'safe center', alignItems: 'center', padding: 20 },
  // S115: THE CARD FAMILY. The rounded white box with the 20px navy drop shadow and the 64px
  ...
  onboardingCard: { padding: 0, textAlign: 'center', background: 'transparent', border: 'none' },
  bigEmoji: { fontSize: 26, lineHeight: 1, animation: 'fadeIn 0.5s ease-out' },
  onboardingTitle: { fontSize: 24, fontWeight: 800, color: 'var(--a3-ink)', marginBottom: 10 },
  onboardingSubtitle: { fontSize: 15.5, color: 'var(--a3-muted)', marginBottom: 20, lineHeight: 1.8 },
  // AI-CONSENT SCREEN. Reading text, not chrome: it is read start-aligned rather than centred,
  ...
  aicItem: { fontSize: 14.5, color: 'var(--a3-muted)', lineHeight: 1.9, marginBottom: 4 },
  aicProvider: { fontWeight: 800, color: 'var(--a3-ink)', direction: 'ltr', unicodeBidi: 'isolate' },
  aicGuardian: { fontSize: 14.5, fontWeight: 700, color: 'var(--a3-ink)', lineHeight: 1.9, border: '1px solid var(--a3-line)', borderRadius: 12, padding: '10px 12px', margin: '14px 0 4px' },
  aicLinks: { display: 'flex', flexWrap: 'wrap', gap: 14, margin: '14px 0 6px' },
  aicLink: { fontSize: 14.5, fontWeight: 700, color: 'var(--a3-ink)', textDecoration: 'underline' },
  aicVersion: { fontSize: 13, color: 'var(--a3-muted)', lineHeight: 1.8, direction: 'rtl' },
  // The version identifier itself, isolated LTR. Without this the neutral hyphens let the digit
  // groups reorder inside the RTL paragraph and "2026-08-06-1" is displayed as "1-06-08-2026".
  aicVersionNum: { direction: 'ltr', unicodeBidi: 'isolate', fontWeight: 700 },
  // The Settings privacy group: the current answer, stated as a sentence, above the one control
  // that changes it.
  aicStatusRow: { fontSize: 14.5, fontWeight: 700, color: 'var(--a3-ink)', lineHeight: 1.9, marginBottom: 4, textAlign: 'start' },
  aicNote: { fontSize: 13.5, color: 'var(--a3-muted)', lineHeight: 1.9, textAlign: 'start' },
  aicWithdrawBtn: { width: '100%', boxSizing: 'border-box', marginTop: 12, padding: '12px', borderRadius: 12, border: '1px solid var(--a3-line)', background: 'transparent', color: 'var(--a3-ink)', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, cursor: 'pointer' },
  onboardingInput: { width: '100%', padding: '15px 18px', fontSize: 18, borderRadius: 16, border: '2px solid var(--a3-line)', background: 'var(--a3-ice)', color: 'var(--a3-ink)', fontFamily: 'var(--ez-ui-font)', direction: 'rtl', marginBottom: 16, boxSizing: 'border-box' },
  ageGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 },
  ageBtn: { padding: '14px', fontSize: 20, fontWeight: 700, borderRadius: 14, border: '2px solid var(--line)', background: 'var(--page)', color: 'var(--accent-ink)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  ageBtnActive: { background: 'var(--red-deep)', color: 'var(--on-accent)', borderColor: 'var(--red-deep)', transform: 'scale(1.05)' },
  ageBtnWide: { gridColumn: '1 / -1', fontSize: 17 },
  // Session 13.1 - Welcome/Onboarding reskin (Boubyan black+red). Scoped to the first-run flow only.
  welcomeContainer: { minHeight: '100dvh', height: '100dvh', overflowY: 'auto', WebkitOverflowScrolling: 'touch', boxSizing: 'border-box', background: 'var(--welcome-bg)' },
  welcomeInner: { minHeight: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'safe center', alignItems: 'center', padding: 24 },
  welcomeCard: { width: '100%', maxWidth: 380, textAlign: 'center' },
  // S115: the red square is an arch now, and .ezonb-crest in <style> owns its geometry -- the key
  // stays because it is what the mark is spread from, and it declares no colour any more.
  welcomeLogoSquare: { display: 'flex', alignItems: 'center', justifyContent: 'center' },
  welcomeTitle: { fontSize: 32, fontWeight: 800, color: 'var(--welcome-ink)', marginBottom: 14, letterSpacing: '-0.5px' },
  welcomeGreeting: { fontSize: 16, fontWeight: 500, color: 'var(--welcome-ink2)', marginBottom: 28 },
  welcomeEmoji: { fontSize: 56, marginBottom: 14, animation: 'fadeIn 0.5s ease-out' },
  welcomeStepTitle: { fontSize: 25, fontWeight: 800, color: 'var(--welcome-ink)', marginBottom: 12 },
  welcomeSubtitle: { fontSize: 15.5, color: 'var(--welcome-ink2)', marginBottom: 22, lineHeight: 1.6 },
  welcomeInput: { width: '100%', padding: '15px 18px', fontSize: 18, borderRadius: 16, border: '1px solid var(--a3-line)', background: 'var(--a3-ice)', color: 'var(--a3-ink)', fontFamily: 'var(--ez-ui-font)', direction: 'rtl', marginBottom: 16, boxSizing: 'border-box' },
  welcomePrimaryBtn: { width: '100%', padding: '15px', fontSize: 17, fontWeight: 700, borderRadius: '22px 22px 15px 15px', background: 'var(--accent-fill)', color: 'var(--on-accent)', border: '1px solid var(--a3-line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  welcomeAgeGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 },
  welcomeAgeBtn: { padding: '14px', fontSize: 20, fontWeight: 700, borderRadius: 14, border: '1px solid var(--line)', background: 'var(--white)', color: 'var(--ink)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  welcomeAgeBtnActive: { background: 'var(--red)', color: 'var(--on-accent)', borderColor: 'var(--red)', transform: 'scale(1.05)' },
  welcomeAgeBtnWide: { gridColumn: '1 / -1', fontSize: 17 },
  // ============================================================
  ...
  // ============================================================
  ezistContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--a3-page)' },
  // S105 -- the shared shell. Every colour is an --a3-* token, so a tenant screen inherits the
  // identity and the dark palette without declaring one; sizes are numbers so text scaling
  // reaches them; nothing animates.
  ezshContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--a3-page)' },
  // The fatwa search is a shell tenant with its own reachable-screen key. One numeric base size
  // lets the existing accessibility scaler resize every em-based label inside it as one unit.
  ezfContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--a3-page)', fontSize: 15 },
  // S106 -- the Quran catalogue. --a3-* tokens only, numeric font sizes, no animation.
  ezqMastTitle: { fontSize: 17, fontWeight: 800, color: 'var(--a3-ink)', lineHeight: 1.6 },
  ezqMastPick: { marginTop: 6, fontSize: 14, fontWeight: 700, color: 'var(--a3-blue)' },
  ezqMastHint: { marginTop: 6, fontSize: 12.5, fontWeight: 600, color: 'var(--a3-muted)' },
  ezqCard: { display: 'flex', flexDirection: 'column', gap: 4, width: '100%', minHeight: 96, padding: '26px 10px 10px', borderRadius: 14, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  ezqCardOn: { border: '1px solid var(--a3-blue)', background: 'var(--a3-ice)' },
  ezqCardTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 },
  ezqNum: { minWidth: 26, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 6px', borderRadius: 999, background: 'var(--a3-ice)', color: 'var(--a3-blue)', fontSize: 12, fontWeight: 800 },
  ezqNumOn: { background: 'var(--a3-blue)', color: 'var(--a3-on-blue)' },
  ezqReveal: { flexShrink: 0, fontSize: 10.5, fontWeight: 700, color: 'var(--a3-cyan)' },
  ezqName: { minWidth: 0, fontSize: 14.5, fontWeight: 800, color: 'var(--a3-ink)', lineHeight: 1.5 },
  ezqCount: { fontSize: 11.5, fontWeight: 600, color: 'var(--a3-muted)' },
  ezqStartBar: { position: 'sticky', bottom: 0, display: 'flex', alignItems: 'center', gap: 10, marginTop: 14, padding: '10px 12px', borderRadius: 16, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-lift)' },
  ezqStartLabel: { flexShrink: 0, fontSize: 13, fontWeight: 700, color: 'var(--a3-ink)' },
  // S109 -- the mushaf surah card. Tokens only; no colour varies by surah.
  ezmCard: { display: 'flex', flexDirection: 'column', gap: 4, width: '100%', minHeight: 104, padding: '26px 10px 10px', borderRadius: 14, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  ezmTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 },
  ezmNum: { minWidth: 26, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0 6px', borderRadius: 999, background: 'var(--a3-ice)', color: 'var(--a3-blue)', fontSize: 12, fontWeight: 800 },
  ezmReveal: { flexShrink: 0, fontSize: 10.5, fontWeight: 700, color: 'var(--a3-cyan)' },
  ezmName: { minWidth: 0, fontSize: 14.5, fontWeight: 800, color: 'var(--a3-ink)', lineHeight: 1.5 },
  ezmFoot: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 },
  ezmCount: { fontSize: 11.5, fontWeight: 600, color: 'var(--a3-muted)' },
  ezmGo: { flexShrink: 0, display: 'inline-flex', color: 'var(--a3-muted)' },
  ezmPage: { fontSize: 11, fontWeight: 600, color: 'var(--a3-blue)' },
  // S107 -- the drill masthead. Layout only; it sits ABOVE the reading area and never over it.
  ezqDrillSurah: { fontSize: 19, fontWeight: 800, color: 'var(--a3-ink)' },
  ezqDrillRow: { display: 'inline-flex', alignItems: 'center', gap: 10, marginTop: 10 },
  ezqDrillBadge: { padding: '4px 10px', borderRadius: 999, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', color: 'var(--a3-blue)', fontSize: 12.5, fontWeight: 800 },
  ezqDrillChange: { minHeight: 36, padding: '0 12px', borderRadius: 12, background: 'transparent', border: '1px solid var(--a3-line)', color: 'var(--a3-muted)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 12.5, fontWeight: 700 },

  ezshScroll: { flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' },
  ezshNavBtn: { width: 44, height: 44, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: 0, borderRadius: 14, background: 'transparent', border: '1px solid transparent', color: 'var(--a3-muted)', cursor: 'pointer' },
  ezshGroup: { display: 'flex', flexDirection: 'column', gap: 10, padding: '14px 14px 16px', borderRadius: 18, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)' },
  ezshGroupHead: { display: 'flex', alignItems: 'center', gap: 9 },
  ezshGroupMark: { width: 12, height: 15, flexShrink: 0, border: '2px solid var(--a3-blue)', borderRadius: '60% 60% 12% 12%', borderBottomColor: 'transparent' },
  ...
  // ============================================================
  eziaContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--a3-page)' },
  // the reader paints the same page; it is its own key so each screen has its own root.
  eziaReadContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--a3-page)' },
  eziaScroll: { flex: 1, minHeight: 0, overflowY: 'auto', WebkitOverflowScrolling: 'touch' },
  eziaNavBtn: { width: 44, height: 44, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: 0, borderRadius: 14, background: 'transparent', border: '1px solid transparent', color: 'var(--a3-muted)', cursor: 'pointer' },
  eziaNavBtnOn: { background: 'var(--a3-ice)', border: '1px solid var(--a3-line)', color: 'var(--a3-blue)' },
  eziaRingBox: { position: 'relative', width: 76, height: 76, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
  eziaRingNums: { position: 'absolute', fontSize: 14, fontWeight: 800, color: 'var(--a3-ink)', fontFamily: 'var(--ez-ui-font)' },
  eziaMastText: { display: 'flex', flexDirection: 'column', gap: 4, minWidth: 0, textAlign: 'right' },
  eziaMastTitle: { fontSize: 19, fontWeight: 800, color: 'var(--a3-ink)' },
  eziaMastHint: { fontSize: 12.5, fontWeight: 600, color: 'var(--a3-muted)' },
  // ITEM 43-أ. The chain row and the goal chooser. Every colour is a token this screen already
  ...
  eziaChainRow: { display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginTop: 12 },
  eziaChainNum: { fontSize: 19, fontWeight: 800, color: 'var(--a3-ink)', fontFamily: 'var(--ez-ui-font)' },
  eziaChainWord: { fontSize: 12.5, fontWeight: 600, color: 'var(--a3-muted)' },
  eziaGoalRow: { display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginTop: 10 },
  eziaGoalLabel: { fontSize: 12.5, fontWeight: 700, color: 'var(--a3-muted)' },
  eziaGoalChip: { minWidth: 44, minHeight: 44, padding: '0 12px', borderRadius: 14, border: '1px solid var(--a3-line)', background: 'var(--a3-surface)', color: 'var(--a3-ink)', fontSize: 14, fontWeight: 800, fontFamily: 'var(--ez-ui-font)', cursor: 'pointer' },
  eziaGoalChipOn: { background: 'var(--a3-blue)', color: 'var(--a3-on-blue)', border: '1px solid var(--a3-blue)' },
  eziaSecTitle: { flexShrink: 0, fontSize: 13, fontWeight: 800, color: 'var(--a3-blue)', fontFamily: 'var(--ez-ui-font)' },
  // the catalogue card: a crest, an emblem row, the title, then the standing and the chevron.
  eziaCard: { position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 8, width: '100%', minHeight: 132, padding: '32px 12px 12px', borderRadius: 16, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  eziaCardDone: { background: 'var(--a3-ice)' },
  eziaCardHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 },
  eziaEmblem: { width: 34, height: 34, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 10, background: 'var(--a3-ice)', color: 'var(--a3-blue)' },
  eziaEmblemLg: { width: 44, height: 44, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 12, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', color: 'var(--a3-blue)' },
  eziaEmblemDone: { color: 'var(--a3-cyan)' },
  eziaCount: { flexShrink: 0, padding: '3px 9px', borderRadius: 999, background: 'var(--a3-ice)', color: 'var(--a3-blue)', fontSize: 12, fontWeight: 800 },
  eziaCardTitle: { flex: 1, minWidth: 0, fontSize: 14, fontWeight: 700, lineHeight: 1.65, color: 'var(--a3-ink)' },
  eziaCardFoot: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 },
  eziaStanding: { minWidth: 0, fontSize: 12, fontWeight: 700, color: 'var(--a3-muted)' },
  eziaGo: { flexShrink: 0, display: 'inline-flex', color: 'var(--a3-muted)' },
  // the featured card: the same parts, an arch crest and more room.
  eziaFeature: { position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 10, width: '100%', minHeight: 148, padding: '16px 14px 14px', borderRadius: '44px 44px 16px 16px', background: 'var(--a3-ice)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  eziaFeatureTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 },
  eziaFeatureTitle: { flex: 1, minWidth: 0, fontSize: 16, fontWeight: 800, lineHeight: 1.6, color: 'var(--a3-ink)' },
  // the reader shell
  ...
  eziaRailMarkDone: { background: 'var(--a3-cyan)' },
  eziaNote: { minHeight: 20, textAlign: 'center', fontSize: 12.5, fontWeight: 600, color: 'var(--a3-blue)' },
  // The completion strip. Ice under blue is exactly the pair eziaReadRepeat above it already
  ...
  eziaDockBtnOff: { color: 'var(--a3-muted)', cursor: 'default' },
  eziaCountBtn: { flex: 1, minWidth: 0, maxWidth: 240, minHeight: 52, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, padding: '4px 10px', borderRadius: 16, background: 'var(--a3-blue)', color: 'var(--a3-on-blue)', border: 'none', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  ...
  ezistSalam: { fontSize: 11.5, fontWeight: 700, color: 'var(--a3-muted)', letterSpacing: '.2px', lineHeight: 1.4 },
  ezistName: { fontSize: 17, fontWeight: 800, color: 'var(--a3-ink)', margin: 0, lineHeight: 1.35, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  // The prompt chip keeps its 44px tap target -- a strip is not a reason to shrink a thing a
  // thumb has to hit -- and loses the 14px that pushed it away from the line above it, because
  // there is no line above it any more.
  ezistPrompt: { display: 'inline-flex', alignItems: 'center', gap: 8, minHeight: 44, marginTop: 0, padding: '6px 12px', borderRadius: 999, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', color: 'var(--a3-ink)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', textAlign: 'right', maxWidth: '100%', flexShrink: 0 },
  // ITEM 97 -- THE FLOOR THAT STOPS THE PILL SHIVERING. Only the DAY line is given it, so the
  ...
  ezistPromptDay: { minWidth: 158, justifyContent: 'center' },
  ezistPromptFlat: { display: 'inline-flex', alignItems: 'center', gap: 8, minHeight: 44, marginTop: 0, padding: '6px 12px', borderRadius: 999, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', color: 'var(--a3-ink)', maxWidth: '100%', flexShrink: 0 },
  ezistPromptIcon: { display: 'inline-flex', flexShrink: 0, color: 'var(--a3-blue)' },
  ezistPromptText: { minWidth: 0, fontSize: 13, fontWeight: 700, color: 'var(--a3-ink)', lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  // the chat entry: a surface panel with one bounded accent affordance, never a filled slab.
  ezistAsk: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, width: '100%', minHeight: 76, padding: '14px 16px', borderRadius: 18, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  ezistAskGo: { width: 46, height: 46, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 14, background: 'var(--a3-blue)', color: 'var(--a3-on-blue)' },
  // the five module cards. One base, four id-specific accents, one wide feature.
  ezistCard: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, width: '100%', minHeight: 84, padding: '14px', borderRadius: 18, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  ezistCard_adhkar: { borderTop: '3px solid var(--a3-cyan)' },
  ezistCard_memorize: { borderRight: '3px solid var(--a3-blue)' },
  ezistCard_treasure: { background: 'var(--a3-ice)' },
  ezistCard_fatwa: { borderLeft: '3px solid var(--a3-cyan)' },
  // Item 64: padding and gap are the MOSAIC'S values, not this card's own. Measured at 430x932
  ...
  // 105: the dot that used to lead this row is gone. `paddingInlineStart` is exactly what it
  ...
  ezistQuranHead: { display: 'flex', alignItems: 'center', gap: 8, paddingInlineStart: 16 },
  ezistQuranLabel: { fontSize: 12.5, fontWeight: 800, color: 'var(--a3-blue)', letterSpacing: '.2px' },
  // ITEM 99 -- THE TOP MARGIN, AND ONLY THE TOP MARGIN. Measured at 430x932 dpr 2 on cfb73ca:
  ...
  ezistQuranText: { color: 'var(--a3-ink)', fontSize: 19, lineHeight: 2.1, textAlign: 'center', fontFamily: "'Amiri', serif", margin: '0 0 6px' },
  ezistQuranMeta: { color: 'var(--a3-muted)', fontSize: 12.5, fontWeight: 600, textAlign: 'center' },
  ...
  adhkarContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--page)' },
  adhkarHeader: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: 'var(--white)', borderBottom: '1px solid var(--line)' },
  adhkarBackBtn: { height: 36, padding: '0 14px', borderRadius: 'var(--radius-btn)', background: 'var(--white)', color: 'var(--ink)', border: '1px solid var(--line)', cursor: 'pointer', fontSize: 14, fontWeight: 600, fontFamily: 'var(--ez-ui-font)', flexShrink: 0 },
  adhkarTitle: { flex: 1, textAlign: 'center', fontSize: 18, fontWeight: 800, color: 'var(--ink)' },
  adhkarScroll: { flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '16px 16px 24px', boxSizing: 'border-box' },
  adhkarList: { display: 'flex', flexDirection: 'column', gap: 10 },
  adhkarRow: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 'var(--radius-card)', background: 'var(--white)', border: '1px solid var(--line)', cursor: 'pointer', width: '100%', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  ...
  adhkar2Scroll: { flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '16px 16px calc(28px + env(safe-area-inset-bottom, 0px))', boxSizing: 'border-box' },
  adhkar2List: { display: 'flex', flexDirection: 'column', gap: 10 },
  adhkar2Row: { display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '13px 14px', borderRadius: 18, background: 'var(--white)', border: '1px solid var(--line)', boxShadow: '0 2px 10px rgba(18,50,122,0.05)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  adhkar2RowIcon: { width: 42, height: 42, flexShrink: 0, borderRadius: 13, background: 'var(--red-soft)', color: 'var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  adhkar2RowTitle: { flex: 1, minWidth: 0, fontSize: 16, fontWeight: 700, lineHeight: 1.55, color: 'var(--ink)' },
  adhkar2Count: { minWidth: 30, height: 26, flexShrink: 0, padding: '0 9px', borderRadius: 999, background: 'var(--tint)', color: 'var(--accent-ink)', fontSize: 13, fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
  adhkar2Chevron: { display: 'flex', flexShrink: 0, color: 'var(--nav-off)' },
  adhkar2Empty: { textAlign: 'center', color: 'var(--muted)', fontSize: 15, padding: '32px 0' },
  adhkar2Cards: { display: 'flex', flexDirection: 'column', gap: 12 },
  adhkar2Card: { position: 'relative', overflow: 'hidden', background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 20, padding: '14px 16px', boxShadow: '0 3px 14px rgba(18,50,122,0.06)' },
  // The accent rule down the START edge of the card. RTL, so the start edge is the right one,
  // and `right` here is the physical side the reading eye meets first.
  adhkar2CardEdge: { position: 'absolute', top: 14, bottom: 14, right: 0, width: 3, borderRadius: '3px 0 0 3px', background: 'linear-gradient(180deg, var(--red) 0%, var(--red-lift) 100%)' },
  adhkar2CardTop: { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 },
  adhkar2Index: { width: 28, height: 28, flexShrink: 0, borderRadius: 10, background: 'var(--tint)', color: 'var(--accent-ink)', fontSize: 13, fontWeight: 800, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
  adhkar2Rule: { flex: 1, height: 1, background: 'var(--line)' },
  // Amiri, already loaded in <head> for the ayah text, at a reading size and a wide leading:
  ...
  adhkar2Text: { fontFamily: "'Amiri', serif", fontSize: 21, lineHeight: 2.05, color: 'var(--ink)' },
  adhkar2CardBottom: { display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginTop: 12 },
  adhkar2Repeat: { display: 'inline-flex', alignItems: 'center', padding: '6px 12px', borderRadius: 999, background: 'var(--tint)', color: 'var(--accent-ink)', fontSize: 12.5, fontWeight: 700 },
  adhkar2PlayBtn: { display: 'inline-flex', alignItems: 'center', gap: 7, padding: '8px 16px', borderRadius: 999, background: 'var(--accent-fill)', color: 'var(--on-accent)', border: 'none', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' },
  adhkar2PlayBtnOn: { background: 'var(--red-deep)' },
  // Session 84, second pass. Additive again: the keys above are untouched, and V1's are three
  ...
  adhkar2Goal: { background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 18, padding: '13px 15px', marginBottom: 14, boxShadow: '0 2px 10px rgba(18,50,122,0.05)' },
  adhkar2GoalTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 9 },
  adhkar2GoalTitle: { fontSize: 14.5, fontWeight: 800, color: 'var(--ink)' },
  adhkar2GoalCount: { fontSize: 13.5, fontWeight: 800, color: 'var(--red)' },
  // RTL flex: the fill is the first child, so it grows from the RIGHT edge -- the direction the
  // reader's eye already runs. No transform, no absolute positioning to get it there.
  adhkar2Bar: { display: 'flex', height: 8, borderRadius: 999, background: 'var(--tint)', overflow: 'hidden' },
  adhkar2BarFill: { height: '100%', borderRadius: 999, background: 'linear-gradient(90deg, var(--red-lift) 0%, var(--red) 100%)', transition: 'width 200ms ease' },
  adhkar2Section: { marginBottom: 14 },
  adhkar2SectionTitle: { fontSize: 13, fontWeight: 800, color: 'var(--muted)', marginBottom: 8 },
  adhkar2Chips: { display: 'flex', flexWrap: 'wrap', gap: 8 },
  adhkar2Chip: { display: 'inline-flex', alignItems: 'center', gap: 8, minHeight: 44, maxWidth: '100%', padding: '0 14px', borderRadius: 999, background: 'var(--white)', border: '1px solid var(--line)', color: 'var(--ink)', fontFamily: 'var(--ez-ui-font)', fontSize: 14, fontWeight: 700, cursor: 'pointer', textAlign: 'right' },
  adhkar2ChipTitle: { minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  adhkar2ChipNum: { minWidth: 22, height: 22, flexShrink: 0, padding: '0 7px', borderRadius: 999, background: 'var(--tint)', color: 'var(--accent-ink)', fontSize: 12, fontWeight: 800, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' },
  // The grid TILE. The grid itself is the .adhkar2-grid class in <style> (a media query cannot
  // live in an inline style); the tile is the cell that class lays out.
  adhkar2Tile: { display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: 10, minHeight: 116, padding: '13px 13px', borderRadius: 18, background: 'var(--white)', border: '1px solid var(--line)', boxShadow: '0 2px 10px rgba(18,50,122,0.05)', cursor: 'pointer', textAlign: 'right', fontFamily: 'var(--ez-ui-font)' },
  adhkar2TileTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 },
  adhkar2TileIcon: { width: 38, height: 38, flexShrink: 0, borderRadius: 12, background: 'var(--red-soft)', color: 'var(--red)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  adhkar2TileTitle: { fontSize: 14.5, fontWeight: 700, lineHeight: 1.55, color: 'var(--ink)' },
  adhkar2Pos: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 8 },
  adhkar2PosText: { fontSize: 14, fontWeight: 800, color: 'var(--ink)' },
  adhkar2PosDone: { fontSize: 12.5, fontWeight: 700, color: 'var(--muted)' },
  adhkar2Slim: { display: 'flex', height: 5, borderRadius: 999, background: 'var(--tint)', overflow: 'hidden', marginBottom: 14 },
  adhkar2SlimFill: { height: '100%', borderRadius: 999, background: 'var(--red)', transition: 'width 200ms ease' },
  // Session 85. THE IVORY READING PAGE IS REMOVED. Its six keys pinned #FBF6EA paper, #EFE4CC
  ...
  adhkar2Counter: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, width: '100%', minHeight: 68, marginTop: 14, padding: '10px 16px', borderRadius: 22, background: 'var(--accent-fill)', color: 'var(--on-accent)', border: 'none', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  adhkar2CounterDone: { background: 'var(--red-deep)', cursor: 'default' },
  adhkar2CounterNums: { fontSize: 24, fontWeight: 800 },
  adhkar2CounterLabel: { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12.5, fontWeight: 700, color: 'rgba(255,255,255,0.88)' },
  adhkar2Actions: { display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginTop: 12 },
  adhkar2Act: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, minHeight: 44, minWidth: 44, padding: '0 15px', borderRadius: 999, background: 'var(--white)', border: '1px solid var(--line)', color: 'var(--ink)', fontFamily: 'var(--ez-ui-font)', fontSize: 13, fontWeight: 700, cursor: 'pointer' },
  adhkar2ActOn: { background: 'var(--red-soft)', border: '1px solid var(--red)', color: 'var(--red)' },
  adhkar2Note: { minHeight: 19, marginTop: 10, textAlign: 'center', fontSize: 12.5, fontWeight: 700, color: 'var(--red)' },
  adhkar2Nav: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginTop: 12 },
  adhkar2NavBtn: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 7, minHeight: 44, minWidth: 44, padding: '0 16px', borderRadius: 999, background: 'var(--white)', border: '1px solid var(--line)', color: 'var(--ink)', fontFamily: 'var(--ez-ui-font)', fontSize: 13.5, fontWeight: 700, cursor: 'pointer' },
  adhkar2NavBtnOff: { opacity: 0.42, cursor: 'default' },
  ...
  a3Bar: { flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8, padding: 'calc(6px + env(safe-area-inset-top, 0px)) 10px 6px', background: 'var(--a3-surface)', borderBottom: '1px solid var(--a3-line)' },
  a3BarBtn: { width: 44, height: 44, flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: 0, borderRadius: 14, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', color: 'var(--a3-ink)', cursor: 'pointer' },
  a3BarBtnOn: { background: 'var(--a3-soft)', border: '1px solid var(--a3-blue)', color: 'var(--a3-blue)' },
  // The category title is content and may run long: it wraps rather than being truncated.
  a3SearchRow: { flexShrink: 0, display: 'flex', alignItems: 'center', gap: 9, height: 46, margin: '10px 12px 0', padding: '0 14px', borderRadius: 999, background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)' },
  a3SearchIcon: { display: 'flex', flexShrink: 0, color: 'var(--a3-muted)' },
  a3SearchInput: { flex: 1, minWidth: 0, padding: 0, border: 'none', background: 'transparent', outline: 'none', fontFamily: 'var(--ez-ui-font)', fontSize: 15.5, color: 'var(--a3-ink)', direction: 'rtl' },
  // DESKTOP: the column stops widening, so a journey path and a deck keep a phone's reading
  // measure on a large display instead of stretching across it.
  a3Empty: { flex: 1, width: '100%', textAlign: 'center', color: 'var(--a3-muted)', fontSize: 15, padding: '32px 0' },
  // The daily ring. One stroked circle over one track circle; the digits inside are the real
  ...
  vtSoonRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, minHeight: 48, marginTop: 8, padding: '10px 12px', borderRadius: 14, background: 'var(--a3-surface)', border: '1px dashed var(--a3-line)' },
  vtSoonName: { fontSize: 14, fontWeight: 700, color: 'var(--a3-muted)' },
  vtSoonTag: { fontSize: 12, fontWeight: 700, color: 'var(--a3-muted)' },
  // S100 -- the identity previews. Every colour is a --vtp-* token declared in <style>, not a
  // literal, and none of these keys carries a text colour: they are swatches, not text.
  vtpBox: { position: 'relative', display: 'block', width: '100%', height: 62, borderRadius: 12, overflow: 'hidden', border: '1px solid var(--line)' },
  vtpQBox: { background: 'var(--vtp-q-page)' },
  vtpIBox: { background: 'var(--vtp-i-page)' },
  vtpQBar: { position: 'absolute', top: 7, left: 9, right: 9, height: 7, borderRadius: 4, background: 'var(--vtp-q-accent)' },
  vtpIBar: { position: 'absolute', top: 7, left: 9, right: 9, height: 7, borderRadius: 4, background: 'var(--vtp-i-accent)' },
  vtpQCard: { position: 'absolute', top: 20, left: 9, right: 9, bottom: 8, borderRadius: 10, background: 'var(--vtp-q-surface)', border: '1px solid var(--vtp-q-line)' },
  // the signature arch, shown rather than described.
  vtpICard: { position: 'absolute', top: 20, left: 9, right: 9, bottom: 8, borderRadius: '22px 22px 10px 10px', background: 'var(--vtp-i-surface)', border: '1px solid var(--vtp-i-line)' },
  // S115: the gradient pill and its 24px navy glow are gone. One flat accent surface, an arch at
  // its outer edge, and a quiet second action beneath it -- both tokens, both in every mode.
  primaryBtn: { width: '100%', padding: '15px', fontSize: 17, fontWeight: 700, borderRadius: '22px 22px 15px 15px', background: 'var(--accent-fill)', color: 'var(--on-accent)', border: 'none', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  secondaryBtn: { width: '100%', padding: '13px', fontSize: 15.5, fontWeight: 700, borderRadius: 15, background: 'transparent', color: 'var(--a3-muted)', border: '1px solid var(--a3-line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', marginTop: 8 },

  // ===== شاشة الدردشة — S112 ISTANA =====
  ...
  chatContainer: { position: 'relative', zIndex: 0, height: '100vh', height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--page)' },
  // ===== S98 — the reply controls, the favourites screen and the local search =====
  ...
  foldToggle: { alignSelf: 'flex-start', display: 'inline-flex', alignItems: 'center', gap: 6, minHeight: 40, marginTop: 2, padding: '8px 12px', background: 'var(--tint)', color: 'var(--red)', border: '1px solid var(--line)', borderRadius: 999, fontSize: 13, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' },
  // The quick-action strip. It WRAPS rather than scrolling sideways, so five labels cannot push
  ...
  quickRow: { display: 'flex', flexWrap: 'wrap', gap: 8, alignSelf: 'stretch', padding: '2px 0 4px' },
  ...
  drawerSearchWrap: { padding: '2px 2px 8px' },
  drawerSearch: { width: '100%', minHeight: 40, padding: '9px 12px', background: 'var(--white)', color: 'var(--ink)', border: '1px solid var(--line)', borderRadius: 12, fontSize: 14.5, fontFamily: 'inherit', textAlign: 'right', outline: 'none' },
  drawerEmpty: { padding: '10px 12px', fontSize: 13, color: 'var(--muted)' },
  // A RESULT IS A BLOCK, not a one-line row. It carries a title, a snippet and a date, and the
  ...
  drawerResultRow: { borderBottom: '1px solid var(--line)' },
  drawerResultBtn: { display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: 3, width: '100%', minWidth: 0, padding: '10px', background: 'none', border: 'none', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14.5, color: 'var(--ink)', textAlign: 'right' },
  drawerResultTitle: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%', fontWeight: 600, lineHeight: 1.5 },
  drawerResultDate: { fontSize: 11.5, lineHeight: 1.5, color: 'var(--muted)' },
  drawerSnippet: { display: 'block', fontSize: 12, lineHeight: 1.6, color: 'var(--muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' },
  drawerBadge: { minWidth: 22, padding: '1px 7px', borderRadius: 999, background: 'var(--tint)', color: 'var(--red)', fontSize: 12, fontWeight: 700, textAlign: 'center' },
  // ===== شاشة المفضلة — S114 ISTANA =====
  ...
  favScreen: { height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--page)', color: 'var(--a3-ink)' },
  favBody: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '10px 14px calc(18px + env(safe-area-inset-bottom, 0px))' },
  favCard: { display: 'block', background: 'var(--a3-surface)', border: '1px solid var(--a3-line)', borderRadius: '24px 24px 16px 16px', boxShadow: 'var(--a3-shadow)' },
  favMeta: { flex: 1, minWidth: 0, fontSize: 12, fontWeight: 700, color: 'var(--a3-muted)' },
  favText: { fontSize: 14.5, lineHeight: 1.85, color: 'var(--a3-ink)', whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' },
  favRow: { display: 'flex', flexWrap: 'wrap', gap: 8 },
  favBtn: { minHeight: 40, padding: '8px 12px', background: 'var(--a3-surface)', color: 'var(--a3-blue)', border: '1px solid var(--a3-line)', borderRadius: 12, fontSize: 13, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' },
  favBtnOff: { color: 'var(--a3-muted)', cursor: 'default', opacity: 0.5 },
  // S112: the scrim and the panel are .ezc-drawer-ov / .ezc-drawer in <style>, because the panel
  ...
  drawerTop: { display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minHeight: 0, overflowY: 'auto' },
  drawerItem: { display: 'flex', alignItems: 'center', gap: 12, width: '100%', minHeight: 44, padding: '12px', background: 'none', border: 'none', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', fontSize: 15.5, color: 'var(--a3-ink)', textAlign: 'right' },
  // S92: the saved-conversation rows. Every colour is an EXISTING variable -- --ink, --muted,
  // --line, --tint, --red -- so both palettes already answer for them and no theme value is
  // added, changed or overridden by any of this.
  drawerSectionLabel: { padding: '14px 12px 6px', fontSize: 12, color: 'var(--muted)', borderTop: '1px solid var(--line)', marginTop: 6 },
  drawerChatRow: { display: 'flex', alignItems: 'center', gap: 2, borderRadius: 12, minHeight: 44 },
  drawerChatOpen: { display: 'flex', alignItems: 'center', gap: 7, flex: 1, minWidth: 0, padding: '11px 10px', background: 'none', border: 'none', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14.5, color: 'var(--ink)', textAlign: 'right' },
  drawerChatTitle: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, minWidth: 0 },
  drawerChatIcon: { width: 32, height: 32, flexShrink: 0, borderRadius: 10, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  ...
  // the file reads it.
  drawerGear: { width: 40, height: 40, flexShrink: 0, borderRadius: 12, background: 'var(--tint)', border: '1px solid var(--line)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  // D88 settings sheet. Every colour here is a token, so the sheet themes with the chat.
  settingsContainer: { display: 'flex', flexDirection: 'column', height: '100dvh', background: 'var(--page)' },
  settingsHeader: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', background: 'var(--white)', borderBottom: '1px solid var(--line)' },
  settingsTitle: { fontSize: 17, fontWeight: 800, color: 'var(--ink)' },
  settingsBody: { flex: 1, overflowY: 'auto', padding: '16px 14px' },
  settingsCard: { background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 'var(--radius-card)', padding: '14px 16px' },
  settingsLabel: { fontSize: 13, fontWeight: 700, color: 'var(--muted)', marginBottom: 10 },
  settingsHint: { fontSize: 12, color: 'var(--muted)', marginTop: 10 },
  // S90 -- the التحكم row. Every value here is one the settings card above already uses: the same
  ...
  settingsNavBtn: { width: '100%', boxSizing: 'border-box', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginTop: 12, background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 'var(--radius-card)', padding: '14px 16px', color: 'var(--ink)', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, cursor: 'pointer', textAlign: 'right' },
  settingsNavChev: { fontSize: 18, lineHeight: 1, color: 'var(--muted)' },
  // ===== S99 — سهولة الاستخدام. Every value is a token; every tappable row is at least 44px on
  // its short side, which is the target size the rest of the settings screen already uses. =====
  a11yGroupLabel: { fontSize: 13, fontWeight: 700, color: 'var(--muted)', margin: '2px 0 8px' },
  // The three size options share themeOpt's shape so the card reads as one family, but carry
  // their own key: themeOpt is the theme control's and must not drift with this one.
  a11yOpt: { flex: 1, minWidth: 0, minHeight: 44, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 8px', borderRadius: 14, border: '1px solid var(--line)', background: 'var(--tint)', color: 'var(--ink)', fontFamily: 'inherit', fontSize: 15, fontWeight: 600, cursor: 'pointer' },
  a11ySwitchRow: { display: 'flex', alignItems: 'center', gap: 12, width: '100%', minHeight: 56, marginTop: 12, padding: '10px 2px', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'right', color: 'var(--ink)' },
  a11ySwitchText: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 },
  a11ySwitchTitle: { fontSize: 15, fontWeight: 600, color: 'var(--ink)' },
  a11ySwitchHint: { fontSize: 12.5, lineHeight: 1.6, color: 'var(--muted)' },
  a11ySwitch: { flexShrink: 0, width: 46, height: 28, borderRadius: 999, background: 'var(--tint)', border: '1px solid var(--line)', display: 'flex', alignItems: 'center', padding: 2 },
  a11ySwitchOn: { background: 'var(--red)', border: '1px solid var(--red)' },
  a11yKnob: { width: 22, height: 22, borderRadius: 999, background: 'var(--white)', marginInlineStart: 0, marginInlineEnd: 'auto' },
  a11yKnobOn: { marginInlineStart: 'auto', marginInlineEnd: 0, background: 'var(--on-accent)' },
  a11yReset: { width: '100%', minHeight: 44, marginTop: 14, padding: '11px 12px', borderRadius: 14, border: '1px solid var(--line)', background: 'var(--white)', color: 'var(--red)', fontFamily: 'inherit', fontSize: 14.5, fontWeight: 600, cursor: 'pointer' },
  // The slider carries no colour PAIR of its own -- accentColor tints the shipped native
  // control from the identity's accent, and the track stays whatever the platform draws.
  wmSlider: { width: '100%', margin: '2px 0 4px', accentColor: 'var(--red)', cursor: 'pointer' },
  themeRow: { display: 'flex', gap: 10 },
  themeOpt: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 10px', borderRadius: 14, border: '1px solid var(--line)', background: 'var(--tint)', color: 'var(--ink)', fontFamily: 'inherit', fontSize: 15, fontWeight: 600, cursor: 'pointer' },
  themeOptActive: { border: '2px solid var(--red)', background: 'var(--red-soft)', color: 'var(--red)' },
  themeSwatch: { width: 18, height: 18, borderRadius: '50%', border: '1px solid', flexShrink: 0 },
  settingsInput: { width: '100%', boxSizing: 'border-box', padding: '12px 14px', marginBottom: 8, borderRadius: 12, border: '1px solid var(--line)', background: 'var(--tint)', color: 'var(--ink)', fontFamily: 'inherit', fontSize: 15, textAlign: 'center', letterSpacing: '0.4em', direction: 'ltr' },
  settingsMsg: { fontSize: 13, lineHeight: 1.8, color: 'var(--red)', margin: '2px 0 8px' },
  settingsSaveBtn: { width: '100%', padding: '12px', borderRadius: 12, border: 'none', background: 'var(--accent-fill)', color: 'var(--on-accent)', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, cursor: 'pointer' },
  ...
  messagesArea: { flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 },
  ...
  messageBubble: { maxWidth: '78%', padding: '12px 16px', borderRadius: 'var(--radius-card)', fontSize: 16, lineHeight: 1.7, wordWrap: 'break-word', whiteSpace: 'pre-wrap' },
  // ~0.78 alpha on both bubbles, so the watermark behind the thread stays visible THROUGH a
  ...
  userBubble: { background: 'color-mix(in srgb, var(--a3-ice) 78%, transparent)', color: 'var(--a3-ink)', border: '1px solid var(--a3-line)', borderInlineStart: '3px solid var(--a3-blue)', borderRadius: 16, borderBottomRightRadius: 6, padding: '10px 14px', fontSize: 15 },
  assistantBubble: { maxWidth: '100%', background: 'color-mix(in srgb, var(--a3-surface) 78%, transparent)', color: 'var(--answer-ink)', borderRadius: 'var(--radius-card)', boxShadow: 'var(--a3-shadow)', border: '1px solid var(--a3-line)', fontSize: 15 },
  bubbleText: { overflowWrap: 'anywhere', /* 13.4-b2 */ lineHeight: 1.85, fontSize: 15, color: 'var(--answer-ink)' },
  // S93: the Markdown blocks. Every colour is an EXISTING variable -- --answer-ink, --muted,
  // --line, --tint, --red -- so both palettes already answer for them and no theme value is
  ...
  mdP: { margin: 0 },
  mdH1: { margin: '2px 0 2px', fontSize: 18, fontWeight: 700, lineHeight: 1.6, color: 'var(--answer-ink)' },
  mdH2: { margin: '2px 0 2px', fontSize: 16.5, fontWeight: 700, lineHeight: 1.6, color: 'var(--answer-ink)' },
  mdH3: { margin: '2px 0 2px', fontSize: 15.5, fontWeight: 700, lineHeight: 1.6, color: 'var(--answer-ink)' },
  mdHr: { border: 'none', borderTop: '1px solid var(--line)', margin: '10px 0' },
  mdUl: { margin: '2px 0', paddingInlineStart: 22, listStyleType: 'disc' },
  mdOl: { margin: '2px 0', paddingInlineStart: 22 },
  mdLi: { margin: '2px 0' },
  mdQuote: { margin: '2px 0', padding: '6px 12px', borderInlineStart: '3px solid var(--line)', color: 'var(--muted)' },
  mdCode: { fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace', fontSize: '0.92em', background: 'var(--tint)', borderRadius: 6, padding: '1px 5px', direction: 'ltr', unicodeBidi: 'embed', display: 'inline-block' },
  mdPre: { margin: '4px 0', padding: '10px 12px', background: 'var(--tint)', border: '1px solid var(--line)', borderRadius: 10, overflowX: 'auto', direction: 'ltr', textAlign: 'left', fontSize: 13.5, lineHeight: 1.6, whiteSpace: 'pre' },
  // A wide table scrolls INSIDE its own box; the bubble itself must never scroll sideways.
  mdTableWrap: { margin: '4px 0', overflowX: 'auto', maxWidth: '100%' },
  mdTable: { borderCollapse: 'collapse', width: '100%', fontSize: 14.2 },
  mdTh: { border: '1px solid var(--line)', background: 'var(--tint)', padding: '7px 10px', fontWeight: 700, whiteSpace: 'nowrap' },
  mdTd: { border: '1px solid var(--line)', padding: '7px 10px', verticalAlign: 'top' },
  typingDots: { display: 'flex', gap: 4, padding: '4px 0' },
  searchingHint: { display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', fontSize: 14, color: 'var(--red)' },
  dot: { fontSize: 10, color: 'var(--red)', animation: 'typing 1.4s infinite' },

  // ===== بطاقة الآية =====
  verseCard: { background: 'var(--red-soft)', border: '1px solid var(--line)', borderRadius: 14, padding: '12px 14px' },
  verseCardLabel: { display: 'inline-flex', alignItems: 'center', gap: 5, color: 'var(--red)', fontSize: 12, fontWeight: 600, marginBottom: 6 },
  verseText: { fontSize: 19, color: 'var(--verse-ink)', textAlign: 'center', fontFamily: "'Amiri Quran', 'Amiri', 'Tajawal', serif", lineHeight: 2.1, margin: '6px 0 6px' },
  surahText: { fontSize: 19, color: 'var(--verse-ink)', textAlign: 'justify', textAlignLast: 'center', fontFamily: "'Amiri Quran', 'Amiri', 'Tajawal', serif", lineHeight: 2.3, margin: '6px 2px 8px', direction: 'rtl' },
  verseMeta: { fontSize: 12, textAlign: 'center', color: 'var(--muted)', fontWeight: 500 },
  verseFooter: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, flexWrap: 'wrap', marginTop: 8 },
  versePlayBtn: { display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--accent-fill)', color: 'var(--on-accent)', border: 'none', borderRadius: 999, padding: '6px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  verseAudioError: { fontSize: 11, textAlign: 'center', color: '#B4453A', marginTop: 6 },

  // ===== بطاقة الحديث =====
  hadithCard: { background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 14, padding: '12px 14px' },
  ...
  reviewNotice: { display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: 6, alignSelf: 'flex-start', maxWidth: '100%', background: 'var(--tint)', border: '1px solid var(--line)', borderRadius: 12, padding: '8px 12px', fontSize: 12.5, lineHeight: 1.75, fontFamily: 'var(--ez-ui-font)', boxSizing: 'border-box' },
  reviewNoticeLabel: { color: 'var(--red)', fontWeight: 600, flexShrink: 0 },
  reviewNoticeText: { minWidth: 0, color: 'var(--muted)', fontWeight: 500 },

  // ===== بطاقة الخطوات =====
  stepsCard: { background: 'var(--red-soft)', border: '1px solid var(--line)', borderRadius: 14, padding: '12px 14px' },
  stepsCardLabel: { display: 'inline-flex', alignItems: 'center', gap: 5, color: 'var(--red)', fontSize: 12, fontWeight: 600, marginBottom: 8 },
  stepsList: { margin: 0, padding: '0 18px 0 0', color: 'var(--ink)', listStyle: 'decimal' },
  stepsItem: { marginBottom: 6, fontSize: 14.5, lineHeight: 1.8 },
  ...
  dashboardContainer: { height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--page)' },
  dashboardEmpty: { color: 'var(--a3-muted)', textAlign: 'center', padding: 20 },
  // `dashboardHeader`, `backBtn` and `dashboardTitle` are GONE with the strip they drew. They were
  ...
  dashboardContent: { flex: 1, minHeight: 0, overflowY: 'auto', padding: '10px 14px calc(18px + env(safe-area-inset-bottom, 0px))' },
  dashboardCard: { background: 'var(--a3-surface)', borderRadius: '24px 24px 16px 16px', padding: 18, border: '1px solid var(--a3-line)', boxShadow: 'var(--a3-shadow)', marginBottom: 12 },
  dashboardLabel: { fontSize: 13, color: 'var(--a3-muted)', fontWeight: 700 },
  dashboardValue: { fontSize: 17, fontWeight: 700, color: 'var(--a3-ink)' },
  conversationLog: { maxHeight: 400, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10, marginTop: 10 },
  logEntry: { padding: 12, borderRadius: 12, fontSize: 14 },
  ...
  /* 14.1 -- Session 86: homeIconSquareDeep stood here and drew the old home screen's filled
     ...
     other home* keys. No mushaf key was touched. */
  mushafBody: { flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '14px 14px 24px', boxSizing: 'border-box' },
  mushafHint: { textAlign: 'center', color: 'var(--muted)', fontSize: 13, padding: '28px 0' },
  mushafList: { display: 'flex', flexDirection: 'column', gap: 8 },
  mushafRow: { display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '12px 14px', borderRadius: 'var(--radius-btn)', background: 'var(--white)', border: '1px solid var(--line)', cursor: 'pointer', textAlign: 'right', fontFamily: 'inherit' },
  mushafNum: { flexShrink: 0, width: 34, height: 34, borderRadius: 10, background: 'var(--tint)', color: 'var(--accent-ink)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700 },
  mushafRowMain: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 },
  mushafName: { fontSize: 16, fontWeight: 700, color: 'var(--ink)' },
  mushafSub: { fontSize: 12, color: 'var(--muted)', fontWeight: 500 },
  mushafPage: { flexShrink: 0, minWidth: 56, textAlign: 'left', fontSize: 11.5, fontWeight: 600, color: 'var(--muted)' },
  // S110 -- a juz is a CONTROL in a dense grid, no longer a full-width bar: the label sits over
  // its page number instead of beside it, so the cell reads at a third of a phone's width.
  mushafJuzRow: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, width: '100%', padding: '8px 4px', borderRadius: 'var(--radius-btn)', background: 'var(--tint)', border: 'none', cursor: 'pointer', textAlign: 'center', fontFamily: 'inherit' },
  mushafJuzName: { minWidth: 0, fontSize: 12.5, fontWeight: 700, color: 'var(--accent-ink)', whiteSpace: 'nowrap' },
  mushafJuzPage: { minWidth: 0, fontSize: 11, fontWeight: 600, color: 'var(--accent-ink)', opacity: 0.75, whiteSpace: 'nowrap' },
  // 14.2f1 — النافذة والشريط ltr قسراً: موضعةٌ مطلقة بإزاحاتٍ فيزيائيّة صريحة، لا
  ...
  pgViewport: { flex: 1, minHeight: 0, overflow: 'hidden', position: 'relative', touchAction: 'pan-y', background: 'var(--tint)', direction: 'ltr' },
  pgStrip: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, willChange: 'transform', direction: 'ltr' },
  // 14.5 — لا حشوةَ رأسيّة: الورقةُ تملأ ما بين الرأس والشريط، فلا يبقى شريطُ لونٍ
  // فوقها ولا تحتها. والحشوةُ الأفقيّةُ ٦ = مدى حلقات الإطار الخمس وزيادةُ شعرة،
  // فالإطارُ يُرسم خارج صندوقه ولا يقصّه فيضُ الخانة.
  pgSlot: { position: 'absolute', top: 0, bottom: 0, width: '100%', boxSizing: 'border-box', padding: '0 6px', direction: 'rtl', display: 'flex', flexDirection: 'column', justifyContent: 'center' },
  // maxWidth — صفحةُ المصحف لها نسبةٌ. بلا سقفٍ تتمدّد على الشاشة العريضة فتصير شريطاً.
  ...
  pgPage: { height: '100%', width: '100%', maxWidth: 560, marginLeft: 'auto', marginRight: 'auto', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'center', background: 'var(--white)', borderRadius: 6, boxShadow: '0 0 0 1px var(--red-lift), 0 0 0 4px var(--white), 0 0 0 5px var(--red-deep)', padding: '20px 5px', overflow: 'hidden', direction: 'rtl' },
  // 51 -- the frame wrapper carries the ornaments so pgPage's child count stays exact.
  pgFrame: { position: 'relative', height: '100%', width: '100%', maxWidth: 560, marginLeft: 'auto', marginRight: 'auto' },
  // Session 74 -- the flagged sheet only. Deliberately NOT added to pgFrame or pgPage: the
  ...
  svgFrame: { position: 'relative', width: '100%', maxWidth: 560, marginLeft: 'auto', marginRight: 'auto', aspectRatio: MUSHAF_CROP_AR, maxHeight: '100%' },
  // The outer rule, heavier, and the paper the whole band is painted on. The shadow lifts the
  ...
  svgRuleOuter: { position: 'absolute', top: -MUSHAF_FRAME_BAND, right: -MUSHAF_FRAME_BAND, bottom: -MUSHAF_FRAME_BAND, left: -MUSHAF_FRAME_BAND, border: MUSHAF_FRAME_OUTER + 'px solid var(--red-deep)', background: MUSHAF_PAPER, boxShadow: '0 1px 2px rgba(18,50,122,0.10), 0 8px 24px rgba(18,50,122,0.12)', pointerEvents: 'none' },
  // The inner rule, lighter, hanging just outside the breathing room.
  svgRuleInner: { position: 'absolute', top: -MUSHAF_FRAME_INNER_AT, right: -MUSHAF_FRAME_INNER_AT, bottom: -MUSHAF_FRAME_INNER_AT, left: -MUSHAF_FRAME_INNER_AT, border: MUSHAF_FRAME_INNER + 'px solid var(--red-lift)', pointerEvents: 'none' },
  // The crop window itself: it fills the frame exactly, and its overflow:hidden is what crops.
  ...
  svgSheet: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, background: MUSHAF_PAPER, overflow: 'hidden' },
  // 75/C -- NOT object-fit: cover. Cover crops by ratio, blindly, and would cut letters on the
  ...
  svgImg: { position: 'absolute', display: 'block', width: (100 * MUSHAF_VB_W / MUSHAF_CROP_W) + '%', height: (100 * MUSHAF_VB_H / MUSHAF_CROP_H) + '%', left: (-100 * MUSHAF_CROP_X0 / MUSHAF_CROP_W) + '%', top: (-100 * MUSHAF_CROP_Y0 / MUSHAF_CROP_H) + '%', pointerEvents: 'none', userSelect: 'none', WebkitUserSelect: 'none' },
  pgMedal: { position: 'absolute', left: '50%', bottom: 4, transform: 'translateX(-50%)', color: 'var(--red-deep)', fontSize: 11, fontWeight: 700, lineHeight: 1, pointerEvents: 'none' },
  // السطر: عنصرُ flex أفقيّ. space-between يفرد ما بين الكلمات فيمتدّ من حافةٍ لحافة.
  // fontSize و gap يُحقنان في الرسم — يُشتقّان من قياس أعرضِ سطرٍ في الصفحة.
  pgLine: { display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline', width: '100%', direction: 'rtl', fontFamily: PG_FONT, lineHeight: PG_LINE_H, color: 'var(--ink)', whiteSpace: 'nowrap' },
  pgWord: { flexShrink: 0, whiteSpace: 'nowrap' },
  pgHeader: { fontFamily: "'Amiri', serif", fontSize: 15, fontWeight: 700, color: 'var(--accent-ink)', textAlign: 'center', background: 'linear-gradient(180deg, var(--red-soft), var(--white))', borderTop: '1px solid var(--red-lift)', borderBottom: '1px solid var(--red-lift)', borderRadius: 4, padding: '3px 0', margin: '2px 0' },
  pgBasmala: { fontFamily: "'Amiri Quran', 'Amiri', serif", color: 'var(--accent-ink)', direction: 'rtl', textAlign: 'center', whiteSpace: 'nowrap' },
  pgBlank: { minHeight: 4 },
  pgBar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '8px 14px', background: 'var(--white)', borderTop: '1px solid var(--line)' },
  // direction:'ltr' — الحرفان › ‹ من المنعكسات ثنائيّة الاتّجاه (Bidi_Mirrored),
  // فيقلبهما المتصفّح داخل سياق rtl ويشير كلٌّ منهما إلى عكس وجهته. العزلُ يوقف القلب.
  pgNavBtn: { width: 44, height: 36, borderRadius: 10, background: 'var(--tint)', color: 'var(--accent-ink)', border: '1px solid var(--line)', cursor: 'pointer', fontSize: 20, fontWeight: 700, fontFamily: 'inherit', lineHeight: 1, direction: 'ltr' },
  pgNavOff: { opacity: 0.32, cursor: 'default' },
  // S110 -- the reader chrome's type, and ONLY its type. Everything else about .ezmr-* lives in
  // the stylesheet; these are numbers here so the reading-size preference scales them, which a
  // px in a CSS rule cannot do.
  ezmrTitle: { fontSize: 15 },
  ezmrNav: { fontSize: 20 },
  ezmrJump: { fontSize: 13 },
  // S111 -- THE ROLLBACK READER'S CHROME, repainted and not remeasured. ?madinaimg=0 draws its
  ...
  memHeaderFb: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', background: 'var(--a3-surface)', borderBottom: '1px solid var(--a3-line)' },
  memTitleFb: { fontSize: 18, fontWeight: 800, color: 'var(--a3-ink)', letterSpacing: '0.3px' },
  memBtnFb: { height: 36, padding: '0 16px', borderRadius: 12, background: 'transparent', color: 'var(--a3-ink)', border: '1px solid var(--a3-line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 14, fontWeight: 600 },
  pgBarFb: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '8px 14px', background: 'var(--a3-surface)', borderTop: '1px solid var(--a3-line)' },
  pgNavBtnFb: { width: 44, height: 36, borderRadius: 10, background: 'transparent', color: 'var(--a3-ink)', border: '1px solid var(--a3-line)', cursor: 'pointer', fontSize: 20, fontWeight: 700, fontFamily: 'inherit', lineHeight: 1, direction: 'ltr' },
  pgMetaFb: { fontSize: 13, fontWeight: 600, color: 'var(--a3-muted)' },
  pgMeta: { fontSize: 13, fontWeight: 600, color: 'var(--muted)' },
  // 14.4 — غلافُ القفز: ارتفاعُه ٣٦ = ارتفاعُ زرّي التصفّح، فلا يتغيّر علوُّ الشريط بين اللافتة والخانة.
  pgJumpWrap: { minWidth: 128, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  pgJumpBtn: { background: 'none', border: 0, padding: 0, cursor: 'pointer', fontFamily: 'inherit', lineHeight: 1 },
  pgJumpInput: { width: 96, height: 32, boxSizing: 'border-box', textAlign: 'center', fontSize: 16, fontWeight: 600, fontFamily: 'inherit', color: 'var(--ink)', background: 'var(--page)', border: '1px solid var(--line)', borderRadius: 10, padding: 0, direction: 'rtl' },
  // ===== 82: شريطُ الورد اليوميّ =====
  ...
  wirdFill: { display: 'block', height: '100%', borderRadius: 999, background: 'var(--red)' },
  wirdBack: { position: 'fixed', inset: 0, zIndex: 9, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18, direction: 'rtl' },
  wirdSheet: { width: '100%', maxWidth: 330, background: 'var(--white)', border: '1px solid var(--line)', borderRadius: 16, padding: 14, display: 'flex', flexDirection: 'column', gap: 12, boxSizing: 'border-box' },
  wirdSheetHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 },
  wirdSheetTitle: { fontSize: 16, fontWeight: 800, color: 'var(--ink)' },
  wirdChips: { display: 'flex', flexWrap: 'wrap', gap: 8 },
  wirdChip: { minWidth: 46, height: 36, borderRadius: 10, background: 'var(--tint)', color: 'var(--accent-ink)', border: '1px solid var(--line)', cursor: 'pointer', fontSize: 15, fontWeight: 700, fontFamily: 'inherit', padding: '0 12px' },
  ...
  memBackBtn: { height: 36, padding: '0 16px', borderRadius: 12, background: 'rgba(255,255,255,0.12)', color: 'var(--on-accent)', border: '1px solid rgba(255,255,255,0.2)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 14, fontWeight: 600 },
  memHeading: { fontSize: 16, fontWeight: 700, color: 'var(--ink)', textAlign: 'center', padding: '14px 16px 6px' },
  memLoading: { fontSize: 14, color: 'var(--muted)', textAlign: 'center', padding: 8 },
  memList: { flex: 1, overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 8, padding: '8px 14px 16px', alignContent: 'start' },
  memRow: { display: 'flex', alignItems: 'center', gap: 8, minHeight: 52, padding: '8px 10px', borderRadius: 14, background: 'var(--white)', border: '1px solid var(--line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', textAlign: 'right' },
  memRowActive: { background: 'var(--red)', borderColor: 'var(--red)' },
  memRowNum: { flexShrink: 0, minWidth: 32, height: 32, borderRadius: 10, background: 'var(--red-soft)', color: 'var(--red)', fontSize: 13, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  memRowNameWrap: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 1, overflow: 'hidden' },
  memRowName: { fontSize: 15, fontWeight: 600, color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  memRowCount: { fontSize: 11, color: 'var(--muted)', fontWeight: 500 },
  memRowReveal: { flexShrink: 0, fontSize: 11, fontWeight: 600, color: 'var(--red)', background: 'var(--red-soft)', borderRadius: 8, padding: '2px 8px', whiteSpace: 'nowrap' },
  memStartBar: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'var(--white)', borderTop: '1px solid var(--line)' },
  memStartLabel: { fontSize: 14, fontWeight: 600, color: 'var(--ink)', whiteSpace: 'nowrap' },
  memAyahSelect: { flex: 1, height: 40, borderRadius: 12, border: '2px solid var(--line)', background: 'var(--page)', color: 'var(--ink)', fontFamily: 'var(--ez-ui-font)', fontSize: 15, padding: '0 10px' },
  memStartBtn: { height: 40, padding: '0 22px', borderRadius: 12, background: 'linear-gradient(135deg, var(--red) 0%, var(--red-deep) 100%)', color: 'var(--on-accent)', border: 'none', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 15, fontWeight: 700 },
  memSubBar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '10px 16px', background: 'var(--red-soft)', borderBottom: '1px solid var(--line)' },
  memSurahTitle: { fontSize: 16, fontWeight: 700, color: 'var(--ink)' },
  memRangeBadge: { fontSize: 12, fontWeight: 600, color: 'var(--red)', background: 'var(--white)', borderRadius: 8, padding: '3px 10px' },
  memChangeBtn: { height: 34, padding: '0 14px', borderRadius: 10, background: 'transparent', color: 'var(--red)', border: '1px solid rgba(29,78,216,0.35)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 13, fontWeight: 600 },
  memToggleRow: { display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px' },
  memToggleLabel: { fontSize: 14, fontWeight: 600, color: 'var(--muted)' },
  memGranBtn: { height: 34, padding: '0 18px', borderRadius: 10, background: 'var(--tint)', color: 'var(--red)', border: '1px solid var(--line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 14, fontWeight: 600 },
  memGranBtnActive: { background: 'var(--red)', color: 'var(--on-accent)', borderColor: 'var(--red)' },
  memDrillArea: { flex: 1, overflowY: 'auto', padding: '12px 18px 20px', display: 'flex', flexDirection: 'column', gap: 14 },
  memDrillText: { display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', justifyContent: 'center', direction: 'rtl', lineHeight: 2.2 },
  memUnit: { display: 'inline-flex', alignItems: 'center', gap: 6 },
  memAyahBadge: { minWidth: 26, height: 26, borderRadius: 8, background: 'var(--red-soft)', color: 'var(--red)', fontSize: 12, fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--ez-ui-font)' },
  memWord: { fontFamily: "'Amiri', 'Tajawal', serif", fontSize: 26, color: 'var(--ink)', lineHeight: 2 },
  // Layer B — سمِّعني word states: wrong (red), not-yet-reached (dim), and the soft-framing hint.
  memWordWrong: { fontFamily: "'Amiri', 'Tajawal', serif", fontSize: 26, color: '#C0392B', fontWeight: 600, lineHeight: 2 },
  memWordPending: { fontFamily: "'Amiri', 'Tajawal', serif", fontSize: 26, color: 'var(--nav-off)', lineHeight: 2 },
  memReciteHint: { fontSize: 13, color: 'var(--muted)', textAlign: 'center', padding: '0 8px 6px', lineHeight: 1.7 },
  memPlaceholder: { fontFamily: 'var(--ez-ui-font)', fontSize: 18, color: 'var(--nav-off)', background: 'var(--tint)', borderRadius: 8, padding: '2px 14px', letterSpacing: '2px' },
  memMsg: { fontSize: 15, fontWeight: 700, color: 'var(--red)', textAlign: 'center' },
  memControls: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderTop: '1px solid var(--line)', background: 'var(--white)' },
  memRevealBtn: { flex: 1, height: 50, borderRadius: 14, background: 'linear-gradient(135deg, var(--red) 0%, var(--red-deep) 100%)', color: 'var(--on-accent)', border: 'none', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 17, fontWeight: 700 },
  memListenBtn: { height: 50, padding: '0 16px', borderRadius: 14, background: 'var(--tint)', color: 'var(--red)', border: '1px solid var(--line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)', fontSize: 14, fontWeight: 600 },
  memResetBtn: { width: 50, height: 50, borderRadius: 14, background: 'var(--tint)', color: 'var(--red)', border: '1px solid var(--line)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' },

  // ===== شاشة المكالمة — S113 ISTANA =====
  ...
  callContainer: { height: '100vh', height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--page)', direction: 'rtl' },
  // `callTopLabel` and `callCenter` are GONE with the slab they belonged to: the first was the
  ...
  callAvatarWrap: { position: 'relative', width: 152, height: 172, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' },
  callRing: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, borderRadius: '60% 60% 14% 14%', border: '3px solid var(--a3-blue)' },
  callAvatar: { width: 108, height: 122, borderRadius: '60% 60% 14% 14%', background: 'var(--a3-ice)', border: '1px solid var(--a3-line)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  ...
  callControls: { display: 'flex', gap: 16, alignItems: 'center', justifyContent: 'center' },
  callMuteBtn: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, width: 56, height: 56, borderRadius: '18px 18px 14px 14px', background: 'var(--a3-ice)', border: '1px solid var(--a3-line)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  callEndBtn: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 3, width: 56, height: 56, borderRadius: '18px 18px 14px 14px', background: 'var(--red-lift)', border: '1px solid var(--red-lift)', cursor: 'pointer', fontFamily: 'var(--ez-ui-font)' },
  callBtnLabel: { fontSize: 11.5, fontWeight: 700, color: 'var(--a3-blue)' },
};
...
const EZIK_FS_SCALES = { normal: 1, large: 1.14, xlarge: 1.28 };
function ezikScaleStyleObject(obj) {
  if (!obj || typeof obj !== 'object') return;
  const fixed = (typeof obj.width === 'number') || (typeof obj.height === 'number');
  for (const k of Object.keys(obj)) {
    const v = obj[k];
    if (v && typeof v === 'object') { ezikScaleStyleObject(v); continue; }
    if (k === 'fontSize' && typeof v === 'number' && !fixed) {
      obj[k] = 'calc(' + v + 'px * var(--ez-fs, 1))';
    }
  }
}
let ezikStylesAreScalable = false;
function ezikEnsureScalableStyles() {
  if (ezikStylesAreScalable) return false;
  ezikStylesAreScalable = true;
  Object.keys(s).forEach((k) => ezikScaleStyleObject(s[k]));
  return true;
}
// The boot script has already put data-ez-fs on <html> from the stored preference, so this reads
// the decision that was made before the first paint rather than opening the store a second time.
(function () {
  try {
    const chosen = document.documentElement.getAttribute('data-ez-fs');
    if (chosen && chosen !== 'normal') ezikEnsureScalableStyles();
  } catch (e) {}
})();
...
ErrorBoundary.prototype.errorParts = function () {
  var error = this.state.error;
  var message = '';
  var stack = '';
  try {
    message = error && typeof error.message !== 'undefined' ? String(error.message) : String(error || '');
  } catch (ignored) {
    message = '(unavailable)';
  }
  try {
...
ErrorBoundary.prototype.legacyCopy = function (text) {
  var area = null;
  try {
...
// ── ITEM 103: WHAT HAPPENED BEFORE REACT EXISTED, MADE COPYABLE ────────────────────────────
...
// as it did before.
ErrorBoundary.prototype.preBootDetails = function () {
  try {
    var diag = window.__ezikDiag;
    if (!diag || typeof diag.text !== 'function') return '';
    var body = diag.text();
    if (!body) return '';
    var n = typeof diag.count === 'function' ? diag.count() : 0;
    return '\n\nPRE-BOOT DIAGNOSTIC (' + String(n) + ' of ' + String(diag.max) + ')\n' + body;
  } catch (ignored) {
    return '';
  }
};
ErrorBoundary.prototype.copyDetails = function () {
  var parts = this.errorParts();
  var nav = window.navigator;
  var userAgent = nav && nav.userAgent ? String(nav.userAgent) : '';
  var text = parts.message + '\n' + parts.stack + '\n' + userAgent + this.preBootDetails();
  try {
    if (nav && nav.clipboard && typeof nav.clipboard.writeText === 'function') {
      var operation = nav.clipboard.writeText(text);
      if (operation && typeof operation.then === 'function') {
        var self = this;
        operation.then(function () {}, function () { self.legacyCopy(text); });
      }
    } else {
...
ErrorBoundary.prototype.render = function () {
  ...
  return React.createElement('main', {
    ...
    dir: 'rtl',
    style: {
      minHeight: '100vh', height: '100dvh', display: 'flex', alignItems: 'center',
      justifyContent: 'center', padding: 24, background: 'var(--page)', color: 'var(--ink)',
      fontFamily: 'var(--ez-ui-font)'
    }
  }, React.createElement('section', {
    style: {
      width: '100%', maxWidth: 680, padding: 24, borderRadius: 18,
      border: '1px solid var(--line)', background: 'var(--white)', color: 'var(--ink)',
      boxShadow: '0 12px 40px rgba(0,0,0,0.12)'
    }
  },
  React.createElement('h1', { style: { margin: '0 0 20px', fontSize: 22, lineHeight: 1.5 } },
    '\u062a\u0639\u0630\u0651\u0631\u0020\u0639\u0631\u0636\u0020\u0627\u0644\u0645\u062d\u0627\u062f\u062b\u0629'),
  React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 10 } },
    React.createElement('button', {
      type: 'button', onClick: this.retry,
      style: {
        minHeight: 44, padding: '10px 16px', border: '1px solid var(--red)', borderRadius: 12,
        background: 'var(--red)', color: 'var(--on-accent)', cursor: 'pointer', fontWeight: 700
      }
    }, '\u0623\u0639\u062f\u0020\u0627\u0644\u0645\u062d\u0627\u0648\u0644\u0629'),
    React.createElement('button', {
      type: 'button', onClick: this.copyDetails,
      style: {
        minHeight: 44, padding: '10px 16px', border: '1px solid var(--line)', borderRadius: 12,
        background: 'var(--page)', color: 'var(--ink)', cursor: 'pointer', fontWeight: 700
      }
    }, '\u0627\u0646\u0633\u062e\u0020\u0627\u0644\u062a\u0641\u0627\u0635\u064a\u0644')),
  React.createElement('pre', {
    dir: 'ltr',
    style: {
      margin: '20px 0 0', padding: 12, borderRadius: 10, overflow: 'auto',
      whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', textAlign: 'left',
      background: 'var(--page)', color: 'var(--ink)', fontFamily: 'monospace', fontSize: 12,
      lineHeight: 1.5
    }
  }, parts.message + '\n\n' + parts.stack)));
};
