...
head('14) GATE ROSTER (single source: gates.json)');
{
  ...
  // a SCORE floor on the lessons row -- the owner ruled on 2026-09-09 that there is none. His
  // ruling of 2026-09-24 (program order م٣-ز) adds a route and title filter on the server behind
  // FULL_ANSWER_V1, which is gate falessons's.
  const GATES_EXPECTED = 179;  // 174-179: D4 S1-S6 (d4s1-d4s6); 173rd: d3bdisclose -- D3B F5 an honest statement of what is missing survives the delivery filter on the before-writing fiqh path and is not repeated in the line (BEFORE_WRITING_V1); 172nd: d3bnotfound -- D3B F4 the one lam-aqif line from whole sentences: no cut, scope kept, what was not found named, no kept sentence negated (BEFORE_WRITING_V1); 171st: d3breviewer -- D3B F3 the sentence reviewer in batches under its cap, partial verdicts kept, one retry, unreviewed counted (BEFORE_WRITING_V1); 170th: d3bdepends -- D3B F2 a removed sentence takes its dependents; 169th: d3bquotes -- D3B F1 correct sentences not deleted: waw/fa quote start, one re-check with the correct quote, contradiction only within one proposition (BEFORE_WRITING_V1); 168th: d3c-frame -- D3C-C3; 167th: d3c-takhrij -- D3C-C2; 166th: d3c-cards -- D3C-C1; 165th: faparentheticalunique -- D3A H2: a shared opening cannot combine citations across different hadiths (FULL_ANSWER_V1); 164th: fanarratorframe -- D3A H1: narrator speech cannot keep an unproved prophetic frame (FULL_ANSWER_V1); 163rd: d2rejecttail -- D2 E8 keep complete answer tail after rejected rewrite; 162nd: d2notfound -- D2 E7 one terminal deduplicated absence line; 161st: d2citations -- D2 E5 pinned single bracket citations without changing streaming; 160th: d2sections -- D2 E4 full encyclopedia articles and relevant pinned sections; 159th: d2schools -- D2 E3 majority from three named schools and truthful absence wording; 158th: d2rowids -- D2 E1 distinct reviewer row ids and checked quote recovery; 157th: diagtrace -- the diagnostic trace of one turn: preview only, never production, the reply byte-identical (DIAG_TRACE_V1); 156th: navnoauthorask -- C5 (order C): the not-found reply never asks for the author's name, on the quote road and the page road (LIB_NAV_V1); 155th: facompanionunique -- C4 (order C): no Companion is named where the quoted words match more than one man's hadith (FULL_ANSWER_V1); 154th: fadoorclock -- C3 (order C): every whole-answer rewrite door asks the clock on every turn, with the draft's own speed (FULL_ANSWER_V1); 153rd: faascriptionlift -- C2 (order C): what the ascription door cannot correct is lifted whole, the sentence door's prophet line in its place (FULL_ANSWER_V1); 152nd: farejectlift -- C1 (order C): the reject door refused for time lifts each rejected sentence whole, the D13 line in its place, no marker for the lift alone (FULL_ANSWER_V1); 151st: navauthorbook -- order B item 6: a quote request without the word kitab is recognised by a title of the index by the author named in it, never without one; 150th: navglue -- order B item 5: an author glued to a title with a plain lam is separated only where the index holds the title and that book's author; 149th: navsummary -- order B item 4: the quote-then-summary reply carries the quote's own cursor, so a continue after it goes on with the book; 148th: falessonsage -- order B item 3: the related-lessons strip is an adult's only, and an absent age is young; 147th: facompanion -- order B item 2: the Companion next to the Prophet is named from the cited book's own text with the prayer, and never from the model; 146th: fadoors -- order B item 1: a capped whole-answer rewrite of the four doors is continued, and one still cut does not replace a whole answer; 145th: harvesttools -- م٦: أدواتُ الحصاد — المحوِّل وفاحصُ الروابط والجلبُ الدوريّ، بلا جلب; 144th: libsectionui -- م٥: «المكتبة» في العميل وبابُ البطاقة; 143rd: libsection -- م٥: «المكتبة» — الرفّ وقراءةُ الكتاب والوكيل api/lib-nav.js; 142nd: libsearchcap -- م٤-هـ: سقفُ الخادمِ العامِّ للمكتبة (api/lib-search.js); 141st: navform -- م٤-د: صيغة «لـ» ولا طلبَ لمؤلّفٍ مذكور; 140th: navexplain -- م٤-ج: «اشرح» بعد نقل، و«ثمّ لخّصه»; 139th: navcontinue -- M4-b LIB_NAV_V1: «كمّل» after a quotation gives what follows, never an atom twice; one navigation door; 138th: navpage -- M4-a LIB_NAV_V1: a book's page by its number, the real range from its atoms, each class by its rule; 137th: falessons -- M3-g FULL_ANSWER_V1: related lessons follow the question: none under a general question, a shared issue word, each title once; 136th: fahead -- M3-f FULL_ANSWER_V1: the takhrij head stands down when the answer's first sentence already says it; 135th: famechanism -- M3-e FULL_ANSWER_V1: the answer does not narrate its machinery; the takhrij heads say «لم أقف»; 134th: fagrade -- M3-d FULL_ANSWER_V1: al-Tirmidhi's own verdict fills the bracket, and no NO_RULING bracket stands beside a grade the prose states; 133rd: falongmatn -- M3-c FULL_ANSWER_V1: a long matn is carried by its head and its tail at the bracket's evidence only; 132nd: fabracket -- M3-b FULL_ANSWER_V1: a governed matn takes its bracket at its closing mark, and the seal and the seat keep it; 131st: facontinue -- M3-a FULL_ANSWER_V1: a capped write is continued from its last whole sentence on the handler clock; 130th: bwencyc -- the encyclopedia: warmed at the head of an adult's sharia turn so a cold instance catches its first question, named once as card or footer, rows named by source (BEFORE_WRITING_V1, program order 2026-09-24, M2-i); 129th: bwreconnect -- reconnected where safe: the reviewer with every pinned text, the khilaf from two verified quotes, and no word attributed to the Prophet without a retrieved hadith text (BEFORE_WRITING_V1, program order 2026-09-24, M2-h); 128th: bwspeaker -- on a before-writing fiqh turn no speaker is generalised into «بعض أهل العلم»; a named scholar's statement is the sentence door's (BEFORE_WRITING_V1, program order 2026-09-24, M2-g); 127th: bwissue -- the evidence matched to the reader's issue, not his words: a resolver writes the searches, a judge keeps what answers the issue, both with a deterministic fallback (BEFORE_WRITING_V1, program order 2026-09-24, M2-f); 126th: bwmadhhab -- according to the madhhabs: one library call per madhhab to its own book, each madhhab from its own text, a madhhab with no text named (BEFORE_WRITING_V1, program order 2026-09-24, M2-e); 125th: bwnotext -- no ruling without a text: nothing gathered, the writer is told to rule nothing from memory and the sentence door removes any ruling (BEFORE_WRITING_V1, program order 2026-09-24, M2-d); 124th: bwruling -- every madhhab/majority/consensus/council/book claim and absolute ruling read sentence by sentence against every retrieved text, by a reader that must quote, the quote checked in code (BEFORE_WRITING_V1, program order 2026-09-24, M2-c); 123rd: bwsinglewrite -- one writing round from the pinned evidence, and the reader's widen chip when the texts do not suffice (BEFORE_WRITING_V1, program order 2026-09-24, M2-b); 122nd: bwevidence -- the sharia question's evidence gathered and pinned before the first character; a hadith question gets no fiqh books nor encyclopedia (BEFORE_WRITING_V1, program order 2026-09-24, M2-a); 121st: quotetashkeel -- a quotation from a book keeps its harakat in the chat bubble (LIB_QUOTE_V1, 2026-09-24); 120th: encycbrain -- the Kuwaiti encyclopedia in the first second, and its reader card (ENCYC_V1, 2026-09-24); 119th: libquote; 118th: libmujaz
  ...
  // the guard rules and does not edit -- a cut answer is rewritten, never delivered sutured.  // 106th: nameform -- guards/name-form-match-guard.cjs. One man
                               ...
                               //       actually wrote. The guard pins the closed nine-man table,
                               //       drives eleven true pairs and four look-alike impostors
                               //       through the shipped reviewer, and kills three mutants.
                               //       105th: layer1human -- guards/layer1-human-capture-guard.cjs. A
                               //       captured authority must be a human name before any evidence
                               //       verdict can cut, mark, keep, or annotate it. The guard pins
                               //       measured nonhuman drops, human-name survival, and a bypass
                               //       mutant at the production seam.
                               //       104th: streamkeep -- guards/stream-partial-keep-guard.cjs. A
                               //       turn that has already delivered prose may not be erased by
                               //       its own failure. Witnessed twice live: five lines of Arabic
                               //       on screen, the stream broke, and the client replaced the
                               //       whole turn with its friendly connection sentence. The guard
                               //       runs the shipped exit code against a stubbed stream that
                               //       breaks in each way production breaks, and six mutants restore
                               //       the old behaviour one aspect at a time.
                               ...
                               // 91st: lessonssearch -- guards/lessons-search-guard.cjs. The lessons
