...
function LessonsScreen({onBack}){const[query,setQuery]=useState('');const[state,setState]=useState(EZIK_LESSONS_IDLE);const[rows,setRows]=useState([]);// The in-flight request, so a second search cuts the first instead of racing it.
