...
'chat.searchingSources':'يُحضِّرُ الجوابَ…','chat.quickActions':'إجراءات سريعة على آخر رد','chat.removeImage':'إزالة الصورة','chat.listen':'استمع للرد','chat.stopAudio':'إيقاف الصوت','chat.report':'بلّغ عن هذا الردّ',// ITEM 24-A: the heading of the related-lessons list under a settled reply. It names
...
// nothing to draw.
'chat.lessons':'دروسٌ ذاتُ صلة',// ITEM 24-B: the lessons section. The title is the module's own name, so the home card
// and the screen it opens cannot drift apart -- they read the same key.
'module.lessons':'الدروس','module.lessons.sub':'بحثٌ في دروسِ العلماء','lessons.searchPlaceholder':'ابحثْ في الدروس…','lessons.searchAria':'البحثُ في الدروس','lessons.searchButton':'ابحثْ','lessons.minChars':'اكتبْ ثلاثةَ محارفَ فأكثرَ.','lessons.loading':'يبحثُ في الدروس…','lessons.empty':'لا نتائجَ لهذا البحث.','lessons.error':'تعذّرَ البحثُ الآنَ.','lessons.retry':'أعِدِ المحاولة','lessons.resultsAria':'نتائجُ البحثِ في الدروس',// ITEM 24-C: the browse pane. Two tabs, three levels, a pager, and the bucket the wire has
...
const SURAH_ORDER=Array.from({length:114},(_,i)=>i+1);const MEDINAN_SURAHS=new Set([2,3,4,5,8,9,13,22,24,33,47,48,49,55,57,58,59,60,61,62,63,64,65,66,76,98,99,110]);const revelationLabel=n=>MEDINAN_SURAHS.has(n)?'مدنية':'مكية';// ============================================================
...
const CLIENT_BODY_HEADROOM=64*1024;const CLIENT_BUDGET=SERVER_MAX_CHAT_BODY_BYTES-CLIENT_BODY_HEADROOM;// Size a candidate outgoing body EXACTLY as the server does (api/ask.js, beside MAX_CHAT_BODY_BYTES):
...
const EZIK_CARD_W=1080;// ITEM 42-C, THE OWNER'S CALL. The height was the literal 1350 and the reply was cut to fit it.
...
const EZIK_CARD_H_MIN=1350;const EZIK_CARD_H_MAX=2700;const EZIK_CARD_PAD=84;const EZIK_CARD_BODY_SIZE=40;const EZIK_CARD_LINE=62;// The three offsets the footer is built from. They were three bare numbers inside the draw
...
const EZIK_CARD_ATTR_LINE=40;const EZIK_CARD_ATTR_GAP=26;// between the body block and the attribution
const EZIK_CARD_NOTICE_SIZE=26;// the reply's own notice, in the reply's own words
const EZIK_CARD_NOTICE_LINE=36;const EZIK_CARD_NOTICE_GAP=22;const EZIK_CARD_LINKS_LABEL='روابط';// The card is a fixed size, so the text it can hold is a fixed number of lines -- but that
...
const EZIK_CARD_BODY_LINES=Math.max(1,Math.floor((EZIK_CARD_H_MAX-EZIK_CARD_PAD-EZIK_CARD_SITE_SIZE-EZIK_CARD_FOOT_GAP-(EZIK_CARD_LINE+EZIK_CARD_NOTE_DROP)// the slot the cut note occupies
...
const ezikCardWrap=(ctx,text,maxWidth)=>{const out=[];for(const para of String(text||'').replace(/\r\n?/g,'\n').split('\n')){const words=para.trim().split(/\s+/).filter(Boolean);if(!words.length)continue;let line='';for(const w of words){const next=line?line+' '+w:w;if(line&&ctx.measureText(next).width>maxWidth){out.push(line);line=w;}else line=next;}if(line)out.push(line);}return out;};// ITEM 42-C. THE TAIL, BUILT BEFORE THE BODY IS MEASURED.
//
...
const ezikCardParts=source=>{if(source&&typeof source==='object'){return{links:String(source.links==null?'':source.links).trim(),attribution:String(source.attribution==null?'':source.attribution).trim(),notice:String(source.notice==null?'':source.notice).trim()};}return{links:String(source==null?'':source).trim(),attribution:'',notice:''};};const ezikCardTail=(ctx,o,inner)=>{const parts=ezikCardParts(o.source);const wrapAt=(text,size)=>{const s=String(text==null?'':text).trim();if(!s)return[];ctx.font='400 '+size+'px system-ui, sans-serif';return ezikCardWrap(ctx,s,inner);};const items=[];for(const[i,t]of wrapAt(parts.attribution,EZIK_CARD_ATTR_SIZE).entries()){items.push({t:t,size:EZIK_CARD_ATTR_SIZE,weight:'600',alpha:0.95,step:i===0?EZIK_CARD_ATTR_GAP+EZIK_CARD_ATTR_LINE:EZIK_CARD_ATTR_LINE});}for(const[i,t]of wrapAt(parts.notice,EZIK_CARD_NOTICE_SIZE).entries()){items.push({t:t,size:EZIK_CARD_NOTICE_SIZE,weight:'400',alpha:0.7,step:i===0?EZIK_CARD_NOTICE_GAP+EZIK_CARD_NOTICE_LINE:EZIK_CARD_NOTICE_LINE});}// THE LINKS ARE LABELLED AS LINKS. A bare domain printed where a reader looks for a source is a
...
const parts0=ezikCardParts(o.source);const reserved=(parts0.attribution+'\n\n'+parts0.notice).split('\n\n').map(p=>p.trim()).filter(Boolean);const bodyText=String(o.text==null?'':o.text).replace(/\r\n?/g,'\n').split('\n\n').filter(p=>reserved.indexOf(p.trim())===-1).join('\n\n');ctx0.font='400 '+EZIK_CARD_BODY_SIZE+'px system-ui, sans-serif';const allLines=ezikCardWrap(ctx0,bodyText,innerW);const firstBaseline=EZIK_CARD_PAD+EZIK_CARD_LINE;// What the card would have to be to hold the whole reply, with nothing cut.
const wanted=firstBaseline+Math.max(0,allLines.length-1)*EZIK_CARD_LINE+EZIK_CARD_FOOT_GAP+tail.h+EZIK_CARD_PAD;const fits=wanted<=EZIK_CARD_H_MAX;const cardH=fits?Math.max(EZIK_CARD_H_MIN,wanted):EZIK_CARD_H_MAX;// At the ceiling the cut note takes a slot of its own, and the body gets the remainder.
...
ctx.direction='rtl';const inner=innerW;ctx.fillStyle='#0E1512';ctx.fillRect(0,0,EZIK_CARD_W,cardH);// The watermark: the wordmark, large, faint, behind everything. Drawn, never fetched.
...
ctx.fillStyle='#EAF3EE';ctx.textAlign='right';ctx.textBaseline='alphabetic';ctx.font='400 '+EZIK_CARD_BODY_SIZE+'px system-ui, sans-serif';const all=allLines;// already stripped of what the tail carries
...
let ty=cardH-EZIK_CARD_PAD-tail.h;for(const it of tail.items){ty+=it.step;ctx.save();ctx.globalAlpha=it.alpha;ctx.font=it.weight+' '+it.size+'px system-ui, sans-serif';ctx.fillText(it.t,EZIK_CARD_W-EZIK_CARD_PAD,ty);ctx.restore();}return{url:canvas.toDataURL('image/png'),cut:cut,lines:lines.length,w:EZIK_CARD_W,h:cardH};};const SaveReplyImageButton=({getText,getSource})=>{const[flash,setFlash]=useState('');// ONE PRESS, ONE FILE -- the same ref latch the PDF control uses, and for the same reason.
...
let EZH_FATWA=ezT("module.fatwa");// ITEM 24-B: the lessons module reads its name from the dictionary exactly as the five
// beside it do, so the home card and the screen title can never disagree.
let EZH_LESSONS=ezT("module.lessons");// THE REAL MODULE ICONS. Every path below is the path the home screen already drew, moved here
...
const EZH_ICON_LESSONS=/*#__PURE__*/React.createElement("svg",{width:"22",height:"22",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"1.8",strokeLinecap:"round",strokeLinejoin:"round"},/*#__PURE__*/React.createElement("rect",{x:"3",y:"4",width:"18",height:"12",rx:"2"}),/*#__PURE__*/React.createElement("path",{d:"M7 9h10"}),/*#__PURE__*/React.createElement("path",{d:"M7 12h6"}),/*#__PURE__*/React.createElement("path",{d:"M12 16v4"}),/*#__PURE__*/React.createElement("path",{d:"M8 20h8"}));// The callout, the profile entry and the three navigation icons -- the same paths as before,
...
function ezHomeModules(v){const wird=v.wird?toArabicDigits(v.wird.done)+' / '+toArabicDigits(v.wird.target):null;return[{id:'memorize',label:EZH_MEMORIZE,icon:EZH_ICON_MEMORIZE,onClick:v.onOpenMemorize,meta:null},{id:'adhkar',label:EZH_ADHKAR,icon:EZH_ICON_ADHKAR,onClick:v.onOpenAdhkar,meta:null},{id:'mushaf',label:EZH_MUSHAF,icon:EZH_ICON_MUSHAF,onClick:v.onOpenMushaf,meta:wird},{id:'treasure',label:EZH_TREASURE,icon:EZH_ICON_TREASURE,onClick:v.onOpenTreasure,meta:null},{id:'fatwa',label:EZH_FATWA,icon:EZH_ICON_FATWA,onClick:v.onOpenFatwa,meta:null},{id:'lessons',label:EZH_LESSONS,icon:EZH_ICON_LESSONS,onClick:v.onOpenLessons,meta:null},{id:'prayer',label:EZH_PRAYER,icon:EZH_ICON_PRAYER,onClick:v.onOpenPrayer,meta:null}];}// ---- S101 ISTANA HOME START --------------------------------------------------------------
...
let EZIST_SUB_FATWA=ezT("module.fatwa.sub");let EZIST_SUB_LESSONS=ezT("module.lessons.sub");let EZIST_SUB_PRAYER='المواقيت والقبلة، محسوبةً على هذا الجهاز';let EZIST_SUB_LIBRARY='بحثٌ في نصوص المكتبة، بمصادرها';let EZIST_SUB={memorize:EZIST_SUB_MEMORIZE,adhkar:EZIST_SUB_ADHKAR,mushaf:EZIST_SUB_MUSHAF,treasure:EZIST_SUB_TREASURE,fatwa:EZIST_SUB_FATWA,lessons:EZIST_SUB_LESSONS,prayer:EZIST_SUB_PRAYER,library:EZIST_SUB_LIBRARY};// THE TOP NAVIGATION. TWO ELEMENTS AND NO THIRD -- the daily verse, and the menu button.
//
...
function ezWidgetRegistry(){return[{id:'prayer',title:'widget.prayer.title',order:1,shown:false,draw:nav=>/*#__PURE__*/React.createElement(EzWidgetPrayer,{nav:nav})},{id:'adhkar',title:'widget.adhkar.title',order:2,shown:false,draw:nav=>/*#__PURE__*/React.createElement(EzWidgetAdhkar,{nav:nav})},{id:'verse',title:'widget.verse.title',order:3,shown:false,draw:()=>/*#__PURE__*/React.createElement(EzWidgetVerse,null)}];}function ezWidgetIds(){return ezWidgetRegistry().map(w=>w.id);}function ezWidgetById(id){const reg=ezWidgetRegistry();for(let i=0;i<reg.length;i++)if(reg[i].id===id)return reg[i];return null;}// THE DEFAULT ARRANGEMENT, derived from the register and from nothing else, so it cannot drift
...
function readHomeWidgets(){const def=ezWidgetDefaults();const known=ezWidgetIds();let raw=null;try{raw=localStorage.getItem(EZWID_KEY);}catch(e){return def;}if(typeof raw!=='string'||!raw)return def;let o=null;try{o=JSON.parse(raw);}catch(e){return def;}if(!o||typeof o!=='object'||Array.isArray(o))return def;if(o.v!==EZWID_V)return def;if(!Array.isArray(o.order)||!Array.isArray(o.hidden))return def;const mentioned=ezWidgetMentioned(o.order,o.hidden,known);const order=[];for(let i=0;i<o.order.length;i++){const id=o.order[i];if(typeof id!=='string')continue;if(known.indexOf(id)===-1)continue;if(order.indexOf(id)!==-1)continue;order.push(id);}const hidden=[];for(let i=0;i<o.hidden.length;i++){const id=o.hidden[i];if(typeof id!=='string')continue;if(known.indexOf(id)===-1)continue;if(hidden.indexOf(id)===-1)hidden.push(id);}ezWidgetAdmitUnmentioned(order,hidden,mentioned,def);return{v:EZWID_V,order:order,hidden:hidden};}// THE ONE PLACE THAT DECIDES WHAT "THE READER HAS NOT RULED ON THIS" MEANS, so the store reader
// and the panel's normaliser cannot come to two different answers about the same record.
function ezWidgetMentioned(order,hidden,known){const seen=[];const take=list=>{if(!Array.isArray(list))return;for(let i=0;i<list.length;i++){const id=list[i];if(typeof id!=='string'||known.indexOf(id)===-1)continue;if(seen.indexOf(id)===-1)seen.push(id);}};take(order);take(hidden);return seen;}// Appends, IN PLACE and in the register's own order, every registered id the record did not rule
...
function ezWidgetAdmitUnmentioned(order,hidden,mentioned,def){for(let i=0;i<def.order.length;i++){const id=def.order[i];if(mentioned.indexOf(id)!==-1)continue;if(order.indexOf(id)===-1)order.push(id);if(def.hidden.indexOf(id)!==-1&&hidden.indexOf(id)===-1)hidden.push(id);}}// THE ONLY WRITER, and it REPORTS. A full store, a private window that refuses, a storage that
...
function readHomeWidgetsShape(state){const def=ezWidgetDefaults();if(!state||typeof state!=='object'||Array.isArray(state))return def;const known=ezWidgetIds();const osrc=Array.isArray(state.order)?state.order:def.order;const hsrc=Array.isArray(state.hidden)?state.hidden:def.hidden;const mentioned=ezWidgetMentioned(osrc,hsrc,known);const order=[];for(let i=0;i<osrc.length;i++){if(known.indexOf(osrc[i])===-1||order.indexOf(osrc[i])!==-1)continue;order.push(osrc[i]);}const hidden=[];for(let i=0;i<hsrc.length;i++){if(known.indexOf(hsrc[i])===-1||hidden.indexOf(hsrc[i])!==-1)continue;hidden.push(hsrc[i]);}ezWidgetAdmitUnmentioned(order,hidden,mentioned,def);return{v:EZWID_V,order:order,hidden:hidden};}// THE SHELL every widget is drawn in, so three widgets are one design and not three. The head is
...
function EzWidgetShell({title,openLabel,onOpen,children}){return/*#__PURE__*/React.createElement("section",{style:s.ezwidCard},/*#__PURE__*/React.createElement("div",{style:s.ezwidHead},/*#__PURE__*/React.createElement("span",{style:s.ezwidTitle},title),onOpen?/*#__PURE__*/React.createElement("span",{className:"ez-hit"},/*#__PURE__*/React.createElement("button",{type:"button",onClick:onOpen,className:"ezhome-focus",style:s.ezwidOpen},openLabel)):null),children);}// THE PRAYER-TIMES WIDGET -- ITEMS 107, 121 AND 119 REUSED, NOT REWRITTEN.
//
...
function EzWidgetPrayer({nav}){useEzLang();const loc=readQiblaLoc();const prefs=readPrayerPrefs();const now=new Date();const t=prayerTimesFor(now.getFullYear(),now.getMonth()+1,now.getDate(),loc.lat,loc.lng,-now.getTimezoneOffset(),prefs.method,prefs.asr,prefs.off);// A time that could not be computed at this latitude draws an em dash through prayerClock and
// is SAID rather than left as a dash the reader has to interpret.
const missing=PRAYER_KEYS.some(k=>t[k]===null);return/*#__PURE__*/React.createElement(EzWidgetShell,{title:ezT('widget.prayer.title'),openLabel:ezT('widget.prayer.open'),onOpen:nav?nav.onOpenPrayer:null},PRAYER_KEYS.map(k=>/*#__PURE__*/React.createElement("div",{key:k,style:s.ezwidRow},/*#__PURE__*/React.createElement("span",{style:s.ezwidName},ezT('widget.prayer.'+k)),/*#__PURE__*/React.createElement("span",{style:s.ezwidValue},prayerClock(t[k])))),missing?/*#__PURE__*/React.createElement("div",{style:s.ezwidNote},ezT('widget.prayer.none')):null,/*#__PURE__*/React.createElement("div",{style:s.ezwidNote},ezT('widget.prayer.note')));}// THE ADHKAR WIDGET -- ITEMS 12 AND 43-أ, READ AND NOT RECOUNTED.
//
...
function EzWidgetAdhkar({nav}){useEzLang();const rec=readAdhkarStreak();const today=adhkarDayKey();const run=adhkarRunAsOf(rec,today);const met=rec.last===today;return/*#__PURE__*/React.createElement(EzWidgetShell,{title:ezT('widget.adhkar.title'),openLabel:ezT('widget.adhkar.open'),onOpen:nav?nav.onOpenAdhkar:null},/*#__PURE__*/React.createElement("div",{style:s.ezwidNumRow},/*#__PURE__*/React.createElement("span",{style:s.ezwidName},ezT('widget.adhkar.chain')),/*#__PURE__*/React.createElement("span",{style:s.ezwidNum},toArabicDigits(run)),/*#__PURE__*/React.createElement("span",{style:s.ezwidName},run>0?ezT('widget.adhkar.days'):ezT('widget.adhkar.start'))),/*#__PURE__*/React.createElement("div",{style:s.ezwidNote},ezT('widget.adhkar.goal',{n:toArabicDigits(rec.goal)})),/*#__PURE__*/React.createElement("div",{style:s.ezwidNote},met?ezT('widget.adhkar.met'):ezT('widget.adhkar.notMet')));}// THE DAILY-VERSE WIDGET -- ITEMS 56 AND 120-أ, THE SAME SOURCE AND THE SAME VERBATIM TEXT.
//
...
function EzWidgetVerse(){useEzLang();const v=getDailyVerse();const isHadith=v.surah==='\u062D\u062F\u064A\u062B';return/*#__PURE__*/React.createElement(EzWidgetShell,{title:isHadith?ezT('widget.verse.hadithTitle'):ezT('widget.verse.title')},/*#__PURE__*/React.createElement("div",{style:s.ezwidVerseText},v.text),/*#__PURE__*/React.createElement("div",{style:s.ezwidVerseMeta},isHadith?ezT('widget.verse.narrated',{by:v.ayah}):ezT('widget.verse.surah',{surah:v.surah,ayah:v.ayah})));}// THE ARRANGING PANEL. Buttons, and deliberately NOT drag and drop: a drag target is not a 44px
...
function EzikHomeArrange({widgets,onWidgets,onClose}){useEzLang();const[saveFailed,setSaveFailed]=useState(false);const state=readHomeWidgetsShape(widgets);const rows=state.order.map(id=>ezWidgetById(id)).filter(Boolean);// ONE PLACE APPLIES A CHANGE, so the store and the screen can never be given different records.
const apply=next=>{setSaveFailed(!writeHomeWidgets(next));onWidgets(next);};return/*#__PURE__*/React.createElement("section",{style:s.ezwidPanel,"aria-label":ezT('home.arrange.title')},/*#__PURE__*/React.createElement("div",{style:s.ezwidHead},/*#__PURE__*/React.createElement("span",{style:s.ezwidTitle},ezT('home.arrange.title')),/*#__PURE__*/React.createElement("span",{className:"ez-hit"},/*#__PURE__*/React.createElement("button",{type:"button",onClick:onClose,className:"ezhome-focus",style:s.ezwidOpen},ezT('home.arrange.close')))),/*#__PURE__*/React.createElement("div",{style:s.ezwidNote},ezT('home.arrange.hint')),rows.map((w,i)=>{const hidden=state.hidden.indexOf(w.id)!==-1;const title=ezT(w.title);const showHide=hidden?ezT('home.arrange.show'):ezT('home.arrange.hide');return/*#__PURE__*/React.createElement("div",{key:w.id,style:s.ezwidPanelRow},/*#__PURE__*/React.createElement("span",{style:s.ezwidPanelName},title),/*#__PURE__*/React.createElement("span",{className:"ez-hit",style:s.ezwidPanelActs},/*#__PURE__*/React.createElement("button",{type:"button",onClick:()=>apply(ezWidgetToggle(state,w.id)),"aria-pressed":hidden?'false':'true',"aria-label":showHide+' '+title,className:"ezhome-focus",style:hidden?s.ezwidActOff:s.ezwidAct},showHide),/*#__PURE__*/React.createElement("button",{type:"button",onClick:()=>apply(ezWidgetMove(state,w.id,-1)),disabled:i===0,"aria-label":ezT('home.arrange.up')+' '+title,className:"ezhome-focus",style:i===0?s.ezwidActOff:s.ezwidAct},ezT('home.arrange.up')),/*#__PURE__*/React.createElement("button",{type:"button",onClick:()=>apply(ezWidgetMove(state,w.id,1)),disabled:i===rows.length-1,"aria-label":ezT('home.arrange.down')+' '+title,className:"ezhome-focus",style:i===rows.length-1?s.ezwidActOff:s.ezwidAct},ezT('home.arrange.down'))));}),/*#__PURE__*/React.createElement("span",{className:"ez-hit"},/*#__PURE__*/React.createElement("button",{type:"button",onClick:()=>apply(ezWidgetDefaults()),className:"ezhome-focus",style:s.ezwidReset},ezT('home.arrange.reset'))),saveFailed?/*#__PURE__*/React.createElement("div",{role:"status",style:s.swNote},/*#__PURE__*/React.createElement("span",{style:s.swNoteText},ezT('home.arrange.saveFailed'))):null);}// THE REGION. It maps the reader's list ONCE, in the reader's own order, so the reading order,
...
//
// 🔴 ONE HOOK, FIRST STATEMENT, NO RETURN ABOVE IT.
function EzikHomeWidgetArea({widgets,nav,arrangeOpen,onArrange,onWidgets}){useEzLang();const list=ezWidgetVisible(widgets);return/*#__PURE__*/React.createElement(React.Fragment,null,list.length?/*#__PURE__*/React.createElement("div",{style:s.ezwidStack},list.map(w=>/*#__PURE__*/React.createElement("div",{key:w.id,"data-ezik-home-widget":w.id},w.draw(nav)))):null,/*#__PURE__*/React.createElement("div",{style:s.ezwidBar},/*#__PURE__*/React.createElement("button",{type:"button",onClick:()=>onArrange(!arrangeOpen),"aria-expanded":arrangeOpen?'true':'false',className:"ezhome-focus",style:s.ezwidBarBtn},arrangeOpen?ezT('home.arrange.close'):ezT('home.arrange.open'))),arrangeOpen?/*#__PURE__*/React.createElement(EzikHomeArrange,{widgets:widgets,onWidgets:onWidgets,onClose:()=>onArrange(false)}):null);}function Home({profile,onOpenMenu,onOpenMemorize,onOpenAdhkar,onOpenMushaf,onOpenFatwa,onOpenLessons,onOpenSettings}){// THE OWNER. Every prop it was handed is handed straight on, and the treasure entry resolves
...
const EZIK_LESSONS_BROWSE_ROUTE='/api/lessons-browse';// Twelve seconds, the same patience the search screen has, for the same reason: this is a place
// the reader CHOSE to be and is watching.
const EZIK_BROWSE_TIMEOUT_MS=12000;// THE THREE LEVELS. The value is the wire's own word for the level, so the request body carries
// the constant rather than a literal typed twice.
const EZIK_BROWSE_SCHOLARS='scholars';const EZIK_BROWSE_SERIES='series';const EZIK_BROWSE_LESSONS='lessons';// THE THREE STATES of the result area. There is no idle: the pane asks the moment it mounts, so
// a reader never sees a browse screen that has not tried.
const EZIK_BROWSE_LOADING='loading';const EZIK_BROWSE_READY='ready';const EZIK_BROWSE_FAILED='failed';// A count is a count. Anything that is not a finite number at or above zero is nothing, not a
// guess and not NaN drawn onto the glass.
function ezikBrowseCount(value){return typeof value==='number'&&isFinite(value)&&value>=0?Math.floor(value):0;}// A page number is at least one. The server states both numbers; a body that states neither
// leaves a single page, which is the honest reading of "there is no more".
function ezikBrowsePageNo(value){return typeof value==='number'&&isFinite(value)&&value>=1?Math.floor(value):1;}// Digits follow the reader's language rather than the file's. The rest of the app writes
...
function ezikBrowseNum(value){return ezLangGet()==='ar'?toArabicDigits(value):String(value);}// WHITELIST 1 of 3 -- THE SCHOLARS. Two named properties and no third. A row without a name is
// dropped rather than drawn as an empty button a reader could press.
function ezikBrowseScholarRows(rows){if(!Array.isArray(rows))return[];const out=[];for(const row of rows){if(!row||typeof row!=='object')continue;const scholar=typeof row.scholar_id==='string'?row.scholar_id.trim():'';if(!scholar)continue;out.push({scholar:scholar,count:ezikBrowseCount(row.count)});}return out;}// WHITELIST 2 of 3 -- THE SERIES. Two named properties and no third. THE EMPTY `series` IS NOT
...
function ezikBrowseSeriesRows(rows){if(!Array.isArray(rows))return[];const named=[];const loose=[];for(const row of rows){if(!row||typeof row!=='object')continue;const series=typeof row.series==='string'?row.series.trim():'';const entry={series:series,count:ezikBrowseCount(row.count),misc:series===''};if(entry.misc)loose.push(entry);else named.push(entry);}return named.concat(loose);}// WHITELIST 3 of 3 -- THE LESSONS. Three named properties and no fourth. `unit_id` is read for
...
function ezikBrowseLessonRows(rows){if(!Array.isArray(rows))return[];const out=[];for(const row of rows){if(!row||typeof row!=='object')continue;const unit=typeof row.unit_id==='string'?row.unit_id.trim():'';const title=typeof row.title==='string'?row.title.trim():'';const url=typeof row.url==='string'?row.url.trim():'';if(!title||!/^https?:\/\//i.test(url))continue;out.push({unit:unit,title:title,url:url});}return out;}// THE REQUEST BODY, WRITTEN OUT PER LEVEL. The scholars level takes no parameter at all and no
...
function ezikBrowseRequest(level,scholar,series,pageNo){if(level===EZIK_BROWSE_SCHOLARS)return{level:EZIK_BROWSE_SCHOLARS};if(level===EZIK_BROWSE_SERIES){return{level:EZIK_BROWSE_SERIES,scholar_id:scholar,page:pageNo};}return{level:EZIK_BROWSE_LESSONS,scholar_id:scholar,series:series,page:pageNo};}// THE CALL. Like the search screen and unlike the tail card, this one must be able to TELL a
...
async function ezikBrowseFetch(request,signal){try{const response=await fetch(EZIK_LESSONS_BROWSE_ROUTE,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(request),signal});if(response.status!==200)return{ok:false,payload:null};const payload=await response.json();return{ok:true,payload:payload&&typeof payload==='object'?payload:{}};}catch(e){return{ok:false,payload:null};}}// THE PANE. It draws inside the section's shell and declares none of its own -- no colour, no
...
function LessonsBrowsePane({backRef}){const[level,setLevel]=useState(EZIK_BROWSE_SCHOLARS);const[scholar,setScholar]=useState('');const[series,setSeries]=useState('');const[pageNo,setPageNo]=useState(1);const[attempt,setAttempt]=useState(0);const[state,setState]=useState(EZIK_BROWSE_LOADING);const[rows,setRows]=useState([]);const[pages,setPages]=useState(1);const[total,setTotal]=useState(0);// The in-flight request, and the generation that owns the pane. The abort covers the wire;
...
const browseAbortRef=useRef(null);const browseGenRef=useRef(0);// ONE EFFECT ASKS, AND EVERY MOVE IS A DEPENDENCY OF IT: choosing a scholar, choosing a
...
useEffect(()=>{const controller=new AbortController();browseAbortRef.current=controller;browseGenRef.current+=1;const generation=browseGenRef.current;setState(EZIK_BROWSE_LOADING);const timer=setTimeout(()=>{try{controller.abort();}catch(e){}},EZIK_BROWSE_TIMEOUT_MS);ezikBrowseFetch(ezikBrowseRequest(level,scholar,series,pageNo),controller.signal).then(outcome=>{clearTimeout(timer);if(browseGenRef.current!==generation)return;// a later move owns the pane
browseAbortRef.current=null;if(!outcome.ok){setRows([]);setPages(1);setTotal(0);setState(EZIK_BROWSE_FAILED);return;}const body=outcome.payload||{};if(level===EZIK_BROWSE_SCHOLARS)setRows(ezikBrowseScholarRows(body.rows));else if(level===EZIK_BROWSE_SERIES)setRows(ezikBrowseSeriesRows(body.rows));else setRows(ezikBrowseLessonRows(body.rows));setPages(level===EZIK_BROWSE_SCHOLARS?1:ezikBrowsePageNo(body.pages));setTotal(ezikBrowseCount(body.total));setState(EZIK_BROWSE_READY);});return()=>{clearTimeout(timer);try{controller.abort();}catch(e){}};},[level,scholar,series,pageNo,attempt]);// THE LADDER, ONE RUNG AT A TIME. Lessons fall back to the series of the same scholar, series
...
const ezikBrowseStepBack=()=>{if(level===EZIK_BROWSE_LESSONS){setSeries('');setPageNo(1);setLevel(EZIK_BROWSE_SERIES);return true;}if(level===EZIK_BROWSE_SERIES){setScholar('');setPageNo(1);setLevel(EZIK_BROWSE_SCHOLARS);return true;}return false;};// Parked on every render, so the section always calls the rung the reader is actually on, and
// cleared on unmount, so a section whose pane has gone cannot call a dead closure.
useEffect(()=>{backRef.current=ezikBrowseStepBack;return()=>{backRef.current=null;};});const heading=level===EZIK_BROWSE_SCHOLARS?ezT('lessons.scholarsTitle'):level===EZIK_BROWSE_SERIES?scholar:series||ezT('lessons.miscSeries');return/*#__PURE__*/React.createElement("div",{style:s.lsbPane},/*#__PURE__*/React.createElement("div",{style:s.lsbHead},/*#__PURE__*/React.createElement("span",{style:s.lsbHeadName},heading),state===EZIK_BROWSE_READY&&total>0?/*#__PURE__*/React.createElement("span",{style:s.lsbHeadCount},ezT('lessons.countLessons',{n:ezikBrowseNum(total)})):null),state===EZIK_BROWSE_LOADING?/*#__PURE__*/React.createElement("div",{role:"status","aria-live":"polite",style:s.lsnNote},/*#__PURE__*/React.createElement("span",{style:s.dot},"●"),/*#__PURE__*/React.createElement("span",{style:{...s.dot,animationDelay:'0.2s'}},"●"),/*#__PURE__*/React.createElement("span",{style:{...s.dot,animationDelay:'0.4s'}},"●"),/*#__PURE__*/React.createElement("span",null,ezT('lessons.browseLoading'))):null,state===EZIK_BROWSE_READY&&rows.length===0?/*#__PURE__*/React.createElement("div",{role:"status",style:s.lsnNote},ezT('lessons.browseEmpty')):null,state===EZIK_BROWSE_FAILED?/*#__PURE__*/React.createElement("div",{role:"alert",style:s.lsnError},/*#__PURE__*/React.createElement("span",null,ezT('lessons.browseError')),/*#__PURE__*/React.createElement("button",{type:"button",className:"ezhome-focus",style:s.lsnRetry,onClick:()=>setAttempt(attempt+1)},ezT('lessons.browseRetry'))):null,state===EZIK_BROWSE_READY&&rows.length>0&&level===EZIK_BROWSE_SCHOLARS?/*#__PURE__*/React.createElement("section",{style:s.lsnList,"aria-label":ezT('lessons.scholarsAria')},rows.map((row,i)=>/*#__PURE__*/React.createElement("button",{key:i,type:"button",className:"ezhome-focus",style:s.lsbRow,onClick:()=>{setScholar(row.scholar);setSeries('');setPageNo(1);setLevel(EZIK_BROWSE_SERIES);}},/*#__PURE__*/React.createElement("span",{style:s.lsnItemTitle},row.scholar),/*#__PURE__*/React.createElement("span",{style:s.lsnItemScholar},ezT('lessons.countLessons',{n:ezikBrowseNum(row.count)}))))):null,state===EZIK_BROWSE_READY&&rows.length>0&&level===EZIK_BROWSE_SERIES?/*#__PURE__*/React.createElement("section",{style:s.lsnList,"aria-label":ezT('lessons.seriesAria')},rows.map((row,i)=>/*#__PURE__*/React.createElement("button",{key:i,type:"button",className:"ezhome-focus",style:s.lsbRow,onClick:()=>{setSeries(row.series);setPageNo(1);setLevel(EZIK_BROWSE_LESSONS);}},/*#__PURE__*/React.createElement("span",{style:s.lsnItemTitle},row.misc?ezT('lessons.miscSeries'):row.series),/*#__PURE__*/React.createElement("span",{style:s.lsnItemScholar},ezT('lessons.countLessons',{n:ezikBrowseNum(row.count)}))))):null,state===EZIK_BROWSE_READY&&rows.length>0&&level===EZIK_BROWSE_LESSONS?/*#__PURE__*/React.createElement("section",{style:s.lsnList,"aria-label":ezT('lessons.lessonsAria')},rows.map((row,i)=>/*#__PURE__*/React.createElement("a",{key:row.unit||i,href:row.url,target:"_blank",rel:"noopener noreferrer",className:"ezhome-focus",style:s.lsnItem},/*#__PURE__*/React.createElement("span",{style:s.lsnItemTitle},row.title)))):null,state===EZIK_BROWSE_READY&&rows.length>0&&level!==EZIK_BROWSE_SCHOLARS?/*#__PURE__*/React.createElement("nav",{style:s.lsbPager,"aria-label":ezT('lessons.pagerAria')},/*#__PURE__*/React.createElement("button",{type:"button",className:"ezhome-focus",style:s.lsbPageBtn,disabled:pageNo<=1,onClick:()=>setPageNo(pageNo-1)},ezT('lessons.pagePrev')),/*#__PURE__*/React.createElement("span",{role:"status",style:s.lsbPageOf},ezT('lessons.pageOf',{n:ezikBrowseNum(pageNo),of:ezikBrowseNum(pages)})),/*#__PURE__*/React.createElement("button",{type:"button",className:"ezhome-focus",style:s.lsbPageBtn,disabled:pageNo>=pages,onClick:()=>setPageNo(pageNo+1)},ezT('lessons.pageNext'))):null);}// ITEM 24-C -- END OF THE LESSONS BROWSE PANE
// ============================================================
...
const EZIK_LESSONS_SCREEN_ENDPOINT='/api/lessons-search';// The service ceiling, asked for in full. Ten is the most that can ever come back.
const EZIK_LESSONS_SCREEN_LIMIT=10;// Below three characters after trimming, no request leaves the screen.
const EZIK_LESSONS_SCREEN_MIN_Q=3;// Twelve seconds. This screen is a place the reader CHOSE to be and is watching, so it waits
...
const EZIK_LESSONS_SCREEN_TIMEOUT_MS=12000;// The four states of the result area, named rather than spelled at each site.
const EZIK_LESSONS_IDLE='idle';const EZIK_LESSONS_LOADING='loading';const EZIK_LESSONS_DONE='done';const EZIK_LESSONS_FAILED='failed';// THE WHITELIST. Three named properties per hit and no fourth; a hit without a title or
// without an http(s) url is dropped rather than drawn as an empty row.
function ezikLessonsScreenRows(hits){if(!Array.isArray(hits))return[];const rows=[];for(const hit of hits){if(rows.length>=EZIK_LESSONS_SCREEN_LIMIT)break;if(!hit||typeof hit!=='object')continue;const title=typeof hit.title==='string'?hit.title.trim():'';const url=typeof hit.url==='string'?hit.url.trim():'';const scholar=typeof hit.scholar_id==='string'?hit.scholar_id.trim():'';if(!title||!/^https?:\/\//i.test(url))continue;rows.push({title,url,scholar});}return rows;}// THE CALL. Unlike the tail card, this screen must be able to TELL a failure from an empty
...
async function ezikLessonsScreenSearch(q,signal){const query=typeof q==='string'?q.trim():'';if(query.length<EZIK_LESSONS_SCREEN_MIN_Q)return{ok:false,rows:[]};try{const response=await fetch(EZIK_LESSONS_SCREEN_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({q:query,limit:EZIK_LESSONS_SCREEN_LIMIT}),signal});if(response.status!==200)return{ok:false,rows:[]};const payload=await response.json();return{ok:true,rows:ezikLessonsScreenRows(payload&&payload.hits)};}catch(e){return{ok:false,rows:[]};}}// THE TWO TABS. Item 24-B shipped one pane -- a box and a list of links. Item 24-C puts a
...
const EZIK_LESSONS_TAB_BROWSE='browse';const EZIK_LESSONS_TAB_SEARCH='search';// THE SCREEN. It mounts on EzShell -- the shared istana shell that الإعدادات, المحفّظ and the
...
function LessonsScreen({onBack}){return/*#__PURE__*/React.createElement(LessonsSection,{onHome:onBack});}// THE SECTION. Two panes under one shell and one back button between them.
function LessonsSection({onHome}){// Browsing opens first: a reader who has typed nothing yet still has something to read.
const[tab,setTab]=useState(EZIK_LESSONS_TAB_BROWSE);// What the browse pane parks here is the rung it is standing on -- see item 24-C above.
const browseBackRef=useRef(null);const[query,setQuery]=useState('');const[state,setState]=useState(EZIK_LESSONS_IDLE);const[rows,setRows]=useState([]);// The in-flight request, so a second search cuts the first instead of racing it.
const lessonsAbortRef=useRef(null);// What «أعِدِ المحاولة» re-runs: the query that failed, not whatever is in the box now.
const lastQueryRef=useRef('');const trimmed=query.trim();const tooShort=trimmed.length>0&&trimmed.length<EZIK_LESSONS_SCREEN_MIN_Q;const runLessonsSearch=raw=>{const q=typeof raw==='string'?raw.trim():'';if(q.length<EZIK_LESSONS_SCREEN_MIN_Q)return;if(lessonsAbortRef.current){try{lessonsAbortRef.current.abort();}catch(e){}}const controller=new AbortController();lessonsAbortRef.current=controller;lastQueryRef.current=q;setState(EZIK_LESSONS_LOADING);const timer=setTimeout(()=>{try{controller.abort();}catch(e){}},EZIK_LESSONS_SCREEN_TIMEOUT_MS);ezikLessonsScreenSearch(q,controller.signal).then(outcome=>{clearTimeout(timer);if(lessonsAbortRef.current!==controller)return;// a newer search owns the screen
lessonsAbortRef.current=null;if(!outcome.ok){setRows([]);setState(EZIK_LESSONS_FAILED);return;}setRows(outcome.rows);setState(EZIK_LESSONS_DONE);});};// Leaving the screen mid-request cuts it; nothing is left to settle into a dead component.
useEffect(()=>()=>{if(lessonsAbortRef.current){try{lessonsAbortRef.current.abort();}catch(e){}}},[]);// THE BACK BUTTON IS ONE STEP AND NEVER TWO. The shell's back is offered to the browse pane
...
const onBack=()=>{const step=browseBackRef.current;if(tab===EZIK_LESSONS_TAB_BROWSE&&step&&step()===true)return;onHome();};// The search pane, exactly as item 24-B wrote it, lifted into a value so that a tab can
// choose it. Not a line of it changed.
const searchPane=/*#__PURE__*/React.createElement(React.Fragment,null,/*#__PURE__*/React.createElement("form",{role:"search",style:s.lsnForm,onSubmit:event=>{event.preventDefault();runLessonsSearch(query);}},/*#__PURE__*/React.createElement("input",{className:"ezhome-focus",type:"search",dir:"auto",required:true,maxLength:"180",value:query,onChange:event=>setQuery(event.target.value),placeholder:ezT('lessons.searchPlaceholder'),"aria-label":ezT('lessons.searchAria'),style:s.lsnInput}),/*#__PURE__*/React.createElement("button",{type:"submit",className:"ezhome-focus",disabled:state===EZIK_LESSONS_LOADING||trimmed.length<EZIK_LESSONS_SCREEN_MIN_Q,style:s.lsnSubmit},ezT('lessons.searchButton'))),tooShort?/*#__PURE__*/React.createElement("div",{role:"status",style:s.lsnNote},ezT('lessons.minChars')):null,state===EZIK_LESSONS_LOADING?/*#__PURE__*/React.createElement("div",{role:"status","aria-live":"polite",style:s.lsnNote},/*#__PURE__*/React.createElement("span",{style:s.dot},"●"),/*#__PURE__*/React.createElement("span",{style:{...s.dot,animationDelay:'0.2s'}},"●"),/*#__PURE__*/React.createElement("span",{style:{...s.dot,animationDelay:'0.4s'}},"●"),/*#__PURE__*/React.createElement("span",null,ezT('lessons.loading'))):null,state===EZIK_LESSONS_DONE&&rows.length===0?/*#__PURE__*/React.createElement("div",{role:"status",style:s.lsnNote},ezT('lessons.empty')):null,state===EZIK_LESSONS_FAILED?/*#__PURE__*/React.createElement("div",{role:"alert",style:s.lsnError},/*#__PURE__*/React.createElement("span",null,ezT('lessons.error')),/*#__PURE__*/React.createElement("button",{type:"button",className:"ezhome-focus",style:s.lsnRetry,onClick:()=>runLessonsSearch(lastQueryRef.current)},ezT('lessons.retry'))):null,state===EZIK_LESSONS_DONE&&rows.length>0?/*#__PURE__*/React.createElement("section",{style:s.lsnList,"aria-label":ezT('lessons.resultsAria')},rows.map((row,i)=>/*#__PURE__*/React.createElement("a",{key:i,href:row.url,target:"_blank",rel:"noopener noreferrer",className:"ezhome-focus",style:s.lsnItem},/*#__PURE__*/React.createElement("span",{style:s.lsnItemTitle},row.title),row.scholar?/*#__PURE__*/React.createElement("span",{style:s.lsnItemScholar},row.scholar):null))):null);return/*#__PURE__*/React.createElement(EzShell,{title:ezT('module.lessons'),onBack:onBack,backLabel:A2_BACK},/*#__PURE__*/React.createElement("div",{role:"tablist","aria-label":ezT('lessons.tabsAria'),style:s.lsbTabs},/*#__PURE__*/React.createElement("button",{type:"button",role:"tab",id:"lsn-tab-browse","aria-controls":"lsn-pane-browse","aria-selected":tab===EZIK_LESSONS_TAB_BROWSE,className:"ezhome-focus",style:tab===EZIK_LESSONS_TAB_BROWSE?s.lsbTabOn:s.lsbTab,onClick:()=>setTab(EZIK_LESSONS_TAB_BROWSE)},ezT('lessons.tabBrowse')),/*#__PURE__*/React.createElement("button",{type:"button",role:"tab",id:"lsn-tab-search","aria-controls":"lsn-pane-search","aria-selected":tab===EZIK_LESSONS_TAB_SEARCH,className:"ezhome-focus",style:tab===EZIK_LESSONS_TAB_SEARCH?s.lsbTabOn:s.lsbTab,onClick:()=>setTab(EZIK_LESSONS_TAB_SEARCH)},ezT('lessons.tabSearch'))),/*#__PURE__*/React.createElement("div",{role:"tabpanel",id:"lsn-pane-browse","aria-labelledby":"lsn-tab-browse",hidden:tab!==EZIK_LESSONS_TAB_BROWSE},tab===EZIK_LESSONS_TAB_BROWSE?/*#__PURE__*/React.createElement(LessonsBrowsePane,{backRef:browseBackRef}):null),/*#__PURE__*/React.createElement("div",{role:"tabpanel",id:"lsn-pane-search","aria-labelledby":"lsn-tab-search",hidden:tab!==EZIK_LESSONS_TAB_SEARCH},tab===EZIK_LESSONS_TAB_SEARCH?searchPane:null));}// ITEM 24-B -- END OF THE LESSONS SECTION
// ============================================================
...
const[dailyWird,setDailyWird]=useState(readDailyWird);useEffect(()=>{let alive=true;loadAdhkar().then(d=>{if(alive)setDb(d||{categories:[],byCat:{}});}).catch(()=>{if(alive)setDb({categories:[],byCat:{}});});return()=>{alive=false;};},[]);const cats=db?db.categories||[]:null;// Same filter as V1: normalise both sides, substring, and an empty query keeps EVERY
...
searchOpen:searchOpen||query.length>0,onToggleSearch:()=>{const next=!(searchOpen||query.length>0);setSearchOpen(next);if(!next)setQuery('');},onOpen:open,prog:prog,byCat:db&&db.byCat||{},usage:usage,favs:favs,done:done,goal:streak.goal,run:adhkarRunAsOf(streak,adhkarDayKey()),onGoal:n=>setStreak(writeAdhkarGoal(n)),// A-3: the reader's daily dhikr. The title travels with the id so the home card can name
// it without loading the store a second time.
dailyCat:dailyWird.adhkar.cat,onDailyCat:id=>{const hit=(cats||[]).filter(c=>c.id===id)[0];setDailyWird(writeDailyWird({adhkar:{cat:hit?hit.id:'',title:hit?hit.title:''}}));}};// The whole of the style switch on this screen: which component receives the state above.
...
const[lessonRows,setLessonRows]=useState(null);const lessonsAbortRef=useRef(null);const lessonsSeqRef=useRef(0);// Called at the START of every question: the card goes at once, the pending call is aborted,
// and the generation moves on so a late landing can recognise itself as stale.
const resetLessons=()=>{lessonsSeqRef.current+=1;if(lessonsAbortRef.current){try{lessonsAbortRef.current.abort();}catch(e){}lessonsAbortRef.current=null;}setLessonRows(null);};// Called AFTER the answer is on the screen, and never awaited by the send path -- it cannot
// delay a single character of the reply, the stream, or the paint that follows it.
const startLessonsSearch=(q,seq)=>{const query=typeof q==='string'?q.trim():'';if(query.length<EZIK_LESSONS_MIN_Q)return;const controller=new AbortController();lessonsAbortRef.current=controller;const timer=setTimeout(()=>{try{controller.abort();}catch(e){}},EZIK_LESSONS_TIMEOUT_MS);ezikFetchLessonRows(query,controller.signal).then(rows=>{clearTimeout(timer);if(lessonsSeqRef.current!==seq)return;// a newer question owns the screen
lessonsAbortRef.current=null;if(rows&&rows.length)setLessonRows(rows);});};// ===== Live voice-call mode (Layer 2) — dedicated recognition + one-turn loop, isolated from the dictation mic =====
...
if(abortRef.current)abortRef.current.abort();// ITEM 24-A: the lessons card belongs to the question that produced it. A new question
...
if(screen==='lessons')return/*#__PURE__*/React.createElement(LessonsScreen,{onBack:goEzikBack});// Single-source capability flags for the chat UI. `directConvoAllowed` layers the parental lock
...
const ezikCardBlockOf=(segments,type)=>{const out=[];for(const sg of segments||[]){if(!sg||sg.type!==type)continue;const fn=REPLY_SERIALIZERS[type];if(!fn)continue;const piece=String(fn(sg,{})||'').trim();if(piece)out.push(piece);}return out.join('\n\n');};const ezikCardAttributionBlock=segments=>ezikCardBlockOf(segments,'source');const ezikCardNoticeBlock=segments=>ezikCardBlockOf(segments,'notice');// Every type MessageBubble can render must have an entry above. A new card added to the renderer
...
const EZIK_LESSONS_ENDPOINT='/api/lessons-search';// Three of the ten the service will return. The other seven are fetched and dropped here.
const EZIK_LESSONS_MAX=3;// Below three characters after trimming there is no call at all.
const EZIK_LESSONS_MIN_Q=3;// Eight seconds, and then the AbortController below cuts the call and nothing is drawn. The
...
const PRAYER_SCHEDULE_KEY='ezik_prayer_schedule_v1';const PRAYER_SCHEDULE_DAYS=30;// how many days ahead one generation covers
...
function prayerDayKey(dt){const m=dt.getMonth()+1,d=dt.getDate();return String(dt.getFullYear())+'-'+(m<10?'0':'')+m+'-'+(d<10?'0':'')+d;}// EVERY INPUT THE TABLE WAS COMPUTED FROM, IN ONE STRING. If any of them moves, the stored table
...
function prayerScheduleStamp(loc,prefs,tz){const off=[];for(let i=0;i<PRAYER_OFFSETTABLE.length;i++){const k=PRAYER_OFFSETTABLE[i];off.push(k+':'+(typeof prefs.off[k]==='number'?Math.trunc(prefs.off[k]):0));}return[Math.round(loc.lat*1000)/1000,Math.round(loc.lng*1000)/1000,tz,prefs.method,prefs.asr,off.join(','),readHijriOffset()].join('|');}// Thirty rows from `startDt` forward. new Date(y, monthIndex, d + i) is what rolls the month and
// the year over, so the last day of a 31-day month needs no special case here.
function buildPrayerSchedule(startDt,loc,prefs,tz){const hOff=readHijriOffset();const days=[];for(let i=0;i<PRAYER_SCHEDULE_DAYS;i++){const dt=new Date(startDt.getFullYear(),startDt.getMonth(),startDt.getDate()+i);const y=dt.getFullYear(),m=dt.getMonth()+1,d=dt.getDate();const t=prayerTimesFor(y,m,d,loc.lat,loc.lng,tz,prefs.method,prefs.asr,prefs.off);const row={day:prayerDayKey(dt),hijri:hijriLabel(hijriForCivilDay(y,m,d,hOff))};for(let j=0;j<PRAYER_KEYS.length;j++)row[PRAYER_KEYS[j]]=t[PRAYER_KEYS[j]];days.push(row);}return days;}// A STORE THAT IS WRONG IN ANY WAY READS AS ABSENT, which rebuilds it. A half-validated table
// would put times on a screen that nothing in this file computed.
function readPrayerSchedule(){let raw=null;try{raw=localStorage.getItem(PRAYER_SCHEDULE_KEY);}catch(e){return null;}if(typeof raw!=='string'||!raw)return null;let rec=null;try{rec=JSON.parse(raw);}catch(e){return null;}if(!rec||typeof rec!=='object'||Array.isArray(rec))return null;if(rec.v!==1||typeof rec.stamp!=='string'||typeof rec.from!=='string')return null;if(!Array.isArray(rec.days)||rec.days.length!==PRAYER_SCHEDULE_DAYS)return null;for(let i=0;i<rec.days.length;i++){const r=rec.days[i];if(!r||typeof r!=='object'||Array.isArray(r))return null;if(typeof r.day!=='string'||!/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/.test(r.day))return null;for(let j=0;j<PRAYER_KEYS.length;j++){const v=r[PRAYER_KEYS[j]];if(v!==null&&!(typeof v==='number'&&isFinite(v)))return null;}}return rec;}// A DENIED WRITE IS NOT AN ERROR ON A SCREEN. The table is still returned and still drawn; it is
// simply recomputed next time, which is the app as it was before this item.
function writePrayerSchedule(rec){try{localStorage.setItem(PRAYER_SCHEDULE_KEY,JSON.stringify(rec));}catch(e){}return rec;}function prayerScheduleRemaining(rec,todayKey){if(!rec||!Array.isArray(rec.days))return 0;let n=0;for(let i=0;i<rec.days.length;i++)if(rec.days[i].day>=todayKey)n++;return n;}// THE ONE ENTRY POINT. It answers with the table that is now in effect and says whether it had
...
function ensurePrayerSchedule(loc,prefs,now){const tz=-now.getTimezoneOffset();const stamp=prayerScheduleStamp(loc,prefs,tz);const todayKey=prayerDayKey(now);const cur=readPrayerSchedule();const remaining=prayerScheduleRemaining(cur,todayKey);let why=null;if(!cur)why='absent';else if(cur.stamp!==stamp)why='inputs';else if(remaining<PRAYER_SCHEDULE_RENEW_AT)why='short';if(!why)return{rec:cur,built:false,why:null,remaining:remaining};const rec=writePrayerSchedule({v:1,stamp:stamp,from:todayKey,days:buildPrayerSchedule(now,loc,prefs,tz)});return{rec:rec,built:true,why:why,remaining:prayerScheduleRemaining(rec,todayKey)};}const PRAYER_TITLE='مواقيت الصلاة';const PRAYER_SETTINGS_TITLE='الصلاة';const PRAYER_METHOD_LABEL='المنهج';const PRAYER_ASR_LABEL='مذهب العصر';const PRAYER_OFFSET_LABEL='إزاحة يدويّة بالدقائق';const PRAYER_HINT='تُحسَب على هذا الجهاز من المنهج والإحداثيّات، بلا إنترنت. قابِلْها بتقويمك وعدِّلْ بالدقائق إن لزم.';const PRAYER_NONE='لا يبلغُ الشفقُ هذه الزاويةَ في هذا الموضع اليوم.';const PRAYER_SCHEDULE_TITLE='جدول ثلاثين يومًا';const PRAYER_SCHEDULE_SHOW='اعرض جدول ثلاثين يومًا';const PRAYER_SCHEDULE_HIDE='اطوِ الجدول';// The renewal rule, said in words, because a rule the reader cannot see is not a rule they can
...
const JUZ_DL_PAGE_BYTES=323956;const JUZ_DL_TIMEOUT_MS=3000;// The pages of a juz, from the SHIPPED layout -- the same nav rows the index draws, so no
// second table of juz boundaries exists in this file to disagree with the first.
function juzPagesFor(jz){const rows=buildMushafNav();if(!rows)return null;const js=rows.filter(r=>r.k==='j').slice().sort((a,b)=>a.n-b.n);if(js.length!==30)return null;let i=-1;for(let k=0;k<js.length;k++)if(js[k].n===jz)i=k;if(i===-1)return null;const from=js[i].p;const to=i<29?js[i+1].p-1:MADINA_IMG_PAGES;if(!(from>=1)||!(to>=from)||to>MADINA_IMG_PAGES)return null;const out=[];for(let p=from;p<=to;p++)out.push(p);return out;}// Which juz a page belongs to: the last juz that starts at or before it.
function juzOfPage(p){const rows=buildMushafNav();if(!rows)return 0;let found=0;for(const r of rows)if(r.k==='j'&&r.p<=p&&r.n>found)found=r.n;return found;}// THE WORKER'S OWN NUMBERS, pulled on the channel item 93-B opened. Nothing here retypes the
...
function swMushafReport(){return new Promise(resolve=>{let settled=false;const finish=v=>{if(!settled){settled=true;resolve(v);}};try{const sw=navigator.serviceWorker;if(!sw||!sw.controller||typeof MessageChannel!=='function'){finish(null);return;}const ch=new MessageChannel();ch.port1.onmessage=e=>finish(e&&e.data||null);sw.controller.postMessage({ezik:'precache-status'},[ch.port2]);setTimeout(()=>finish(null),JUZ_DL_TIMEOUT_MS);}catch(e){finish(null);}});}// The device's free space, taken FRESH at the moment of the press.
function juzFreeSpace(){return new Promise(resolve=>{try{if(!navigator.storage||typeof navigator.storage.estimate!=='function'){resolve(null);return;}navigator.storage.estimate().then(e=>{if(!e||typeof e.quota!=='number'||typeof e.usage!=='number'){resolve(null);return;}resolve(e.quota-e.usage);},()=>resolve(null));}catch(e){resolve(null);}});}// CONDITION 1, as one pure decision so it can be driven by a guard without a browser.
function juzRoomVerdict(policy,free,pageCount){if(!policy||typeof policy.cap!=='number'||typeof policy.minFree!=='number'){return{ok:false,why:'noworker'};}const need=pageCount*JUZ_DL_PAGE_BYTES;if(typeof free!=='number'||!isFinite(free)){return{ok:false,why:'unmeasured',need:need,cap:policy.cap,minFree:policy.minFree};}if(free-need<policy.minFree){return{ok:false,why:'nospace',need:need,free:free,cap:policy.cap,minFree:policy.minFree};}return{ok:true,why:'room',need:need,free:free,cap:policy.cap,minFree:policy.minFree};}const juzMb=n=>toArabicDigits(Math.max(1,Math.round(n/(1024*1024))));const MADINA_IMG_PAGES=604;// printed pages 1..604, and no other page
...
const DAILY_WIRD_KEY='ezik_daily_wird_v1';const DAILY_WIRD_MODES=['pages','surah'];// EVERY FIELD IS CHECKED, and anything unusable reads as "not chosen yet" -- which is the app
// exactly as it was before this item, not an error on a child's screen.
function readDailyWird(){const out={mushaf:{mode:'',surah:0},adhkar:{cat:'',title:''},memorize:{surah:0}};let raw=null;try{raw=localStorage.getItem(DAILY_WIRD_KEY);}catch(e){return out;}if(typeof raw!=='string'||!raw)return out;let rec=null;try{rec=JSON.parse(raw);}catch(e){return out;}if(!rec||typeof rec!=='object'||Array.isArray(rec))return out;const m=rec.mushaf;if(m&&typeof m==='object'&&!Array.isArray(m)){if(typeof m.mode==='string'&&DAILY_WIRD_MODES.indexOf(m.mode)!==-1)out.mushaf.mode=m.mode;const n=Number(m.surah);// An integer, or nothing. Truncating 1.5 to 1 would turn a broken store into a CHOICE
// the reader never made, which is the one thing this record must never do.
if(typeof m.surah==='number'&&isFinite(n)&&Math.trunc(n)===n&&n>=1&&n<=114)out.mushaf.surah=n;}const a=rec.adhkar;if(a&&typeof a==='object'&&!Array.isArray(a)){if(typeof a.cat==='string'&&a.cat.length>0&&a.cat.length<=120)out.adhkar.cat=a.cat;if(typeof a.title==='string'&&a.title.length<=200)out.adhkar.title=a.title;}const z=rec.memorize;if(z&&typeof z==='object'&&!Array.isArray(z)){const n=Number(z.surah);if(typeof z.surah==='number'&&isFinite(n)&&Math.trunc(n)===n&&n>=1&&n<=114)out.memorize.surah=n;}// A mushaf mode of 'surah' with no surah behind it is not a choice; it is half a write.
if(out.mushaf.mode==='surah'&&!out.mushaf.surah){out.mushaf.mode='';}return out;}// IT RETURNS WHAT IS NOW IN EFFECT, never what was asked for -- so a control can set its own
// state from this return and cannot end up showing a choice that was never stored.
function writeDailyWird(next){const cur=readDailyWird();if(!next||typeof next!=='object')return cur;const rec={mushaf:cur.mushaf,adhkar:cur.adhkar,memorize:cur.memorize};if(next.mushaf&&typeof next.mushaf==='object')rec.mushaf={mode:next.mushaf.mode,surah:next.mushaf.surah};if(next.adhkar&&typeof next.adhkar==='object')rec.adhkar={cat:next.adhkar.cat,title:next.adhkar.title};if(next.memorize&&typeof next.memorize==='object')rec.memorize={surah:next.memorize.surah};try{localStorage.setItem(DAILY_WIRD_KEY,JSON.stringify(rec));}catch(e){return readDailyWird();}return readDailyWird();}// The card's lines, built from the record and the reader's OWN page target. Only what was
// actually chosen produces a line: nothing here invents a goal the reader never set.
function dailyWirdLines(dw,pageTarget){const out=[];if(!dw)return out;if(dw.mushaf.mode==='surah'&&dw.mushaf.surah){out.push(DW_LINE_MUSHAF+' '+DW_SURAH_WORD+' '+(SURAH_NAMES[dw.mushaf.surah]||''));}else if(dw.mushaf.mode==='pages'&&pageTarget){out.push(DW_LINE_MUSHAF+' '+toArabicDigits(pageTarget)+' '+DW_PAGES_WORD);}if(dw.adhkar.cat&&dw.adhkar.title)out.push(DW_LINE_ADHKAR+' '+dw.adhkar.title);if(dw.memorize.surah)out.push(DW_LINE_MEMORIZE+' '+DW_SURAH_WORD+' '+(SURAH_NAMES[dw.memorize.surah]||''));return out;}// THE VISIBLE TEXT OF THIS LAYER, all of it, in one place so the guard can scan one region.
// Not one of these strings names a time, an alert, or anything that rings.
const JD_TITLE='العملُ بلا إنترنت';const JD_BTN='نزِّلْ هذا الجزء';const JD_BUSY='ينزلُ الآن';const JD_NO_WORKER='لا يمكن التنزيل الآن: خدمةُ التخزين على هذا الجهاز غير عاملة.';const JD_UNMEASURED='لم يتيسّر قياسُ المساحة على هذا الجهاز، فلم يبدأِ التنزيل.';const JD_NOSPACE_A='المساحةُ لا تكفي: يحتاج هذا الجزء نحو ';const JD_NOSPACE_B=' م.ب، والمتاح ';const JD_NOSPACE_C=' م.ب، ولا بدّ من إبقاء ';const JD_NOSPACE_D=' م.ب حرّة. لم يبدأِ التنزيل.';const JD_OF=' من ';const JD_DONE='تمّ حفظُ الجزء على هذا الجهاز.';const JD_FAILED_A='أخفق ';const JD_FAILED_B=' من الصفحات ولم تُحفَظ.';const JD_DECLINED_A='ولم تُحفَظ ';const JD_DECLINED_B=' صفحة لضيق المساحة.';const JD_RULE_A='يحفظُ الجهازُ ';const JD_RULE_PLAIN='إذا امتلأ مخزنُ الصفحات حُذِفت الأقدمُ استعمالًا أوّلًا.';const JD_RULE_B=' صفحةً من المصحف؛ فإذا امتلأ حُذِفت الأقدمُ استعمالًا أوّلًا.';const DW_CARD_TITLE='وِردي اليوم';const DW_CARD_EMPTY='لم تختر بعد. اختر من المصحف أو الأذكار أو المحفّظ.';const DW_LINE_MUSHAF='المصحف:';const DW_LINE_ADHKAR='الأذكار:';const DW_LINE_MEMORIZE='الحفظ:';const DW_SURAH_WORD='سورة';const DW_PAGES_WORD='صفحة في اليوم';const DW_NONE='بلا اختيار';const DW_MUSHAF_LABEL='اختر وردك من المصحف';const DW_ADHKAR_LABEL='اختر ذكرك اليوميّ';const DW_MEMORIZE_LABEL='اختر ما تحفظه';const WIRD_TARGET_KEY='mushaf_wird_target_v1';// Canonical decimal only. ' 5', '5 ', '5.0', '-5', '05', '', 'abc' and Arabic-Indic digits
...
const[wirdDay,setWirdDay]=useState(readWirdDay);const[wirdTarget,setWirdTarget]=useState(readWirdTarget);// A-3: the reader's own choice of WHAT the mushaf wird is. The page count keeps living in
// its own key; this holds the kind, and the surah when a surah is what was chosen.
const[dailyWird,setDailyWird]=useState(readDailyWird);// A-4: the offline package's whole state. null until the reader presses.
const[juzDl,setJuzDl]=useState(null);// A-4: the worker's own ceiling, pulled when the panel opens so the eviction rule is on
// screen BEFORE the reader presses anything -- and never retyped from sw.js.
const[juzCap,setJuzCap]=useState(0);const[picker,setPicker]=useState(false);const[pickerText,setPickerText]=useState('');useEffect(()=>{if(!picker)return undefined;let alive=true;swMushafReport().then(r=>{const p=r&&r.storage?r.storage.mushafPolicy:null;if(alive&&p&&typeof p.cap==='number')setJuzCap(p.cap);});return()=>{alive=false;};},[picker]);const runJuzDownload=async()=>{if(juzDl&&juzDl.phase==='run')return;// one press cannot become two runs
const jz=juzOfPage(page);const pages=jz?juzPagesFor(jz):null;if(!jz||!pages||!pages.length){setJuzDl({phase:'fail',note:JD_NO_WORKER});return;}setJuzDl({phase:'check',juz:jz,total:pages.length,done:0,failed:0});// CONDITION 1 -- the estimate, before a single byte is fetched.
const report=await swMushafReport();const policy=report&&report.storage?report.storage.mushafPolicy:null;const free=await juzFreeSpace();const verdict=juzRoomVerdict(policy,free,pages.length);if(!verdict.ok){const note=verdict.why==='noworker'?JD_NO_WORKER:verdict.why==='unmeasured'?JD_UNMEASURED:JD_NOSPACE_A+juzMb(verdict.need)+JD_NOSPACE_B+juzMb(verdict.free)+JD_NOSPACE_C+juzMb(verdict.minFree)+JD_NOSPACE_D;setJuzDl({phase:'fail',juz:jz,total:pages.length,done:0,failed:0,note:note,cap:verdict.cap});return;}// CONDITION 2 + 3 -- a visible count as it goes, and every failure counted by name.
let done=0,failed=0;for(let i=0;i<pages.length;i++){const url=madinaImgUrl(pages[i]);if(!url){failed++;setJuzDl({phase:'run',juz:jz,total:pages.length,done:done,failed:failed,cap:verdict.cap});continue;}try{const res=await fetch(url);if(res&&res.ok)done++;else failed++;}catch(e){failed++;// counted, never swallowed: this number reaches the reader below
}setJuzDl({phase:'run',juz:jz,total:pages.length,done:done,failed:failed,cap:verdict.cap});}// A page can fetch perfectly and still not be STORED. Ask the worker what it actually
// wrote, so a full disk is reported rather than covered by a finished progress count.
const after=await swMushafReport();const m=after&&after.storage?after.storage.mushaf:null;const before=report&&report.storage?report.storage.mushaf:null;const declined=m&&before&&typeof m.skipped==='number'&&typeof before.skipped==='number'?Math.max(0,m.skipped-before.skipped):0;const storeFailed=m&&before&&typeof m.failed==='number'&&typeof before.failed==='number'?Math.max(0,m.failed-before.failed):0;const bad=failed+storeFailed;let note=bad?JD_FAILED_A+toArabicDigits(bad)+JD_FAILED_B:JD_DONE;if(declined)note=note+' '+JD_DECLINED_A+toArabicDigits(declined)+JD_DECLINED_B;setJuzDl({phase:bad||declined?'fail':'done',juz:jz,total:pages.length,done:done,failed:bad,declined:declined,note:note,cap:verdict.cap});};const dailyWirdValue=dailyWird.mushaf.mode==='surah'&&dailyWird.mushaf.surah?'surah:'+dailyWird.mushaf.surah:dailyWird.mushaf.mode==='pages'&&wirdTarget?'pages:'+wirdTarget:'';const onDailyWirdPick=raw=>{const v=String(raw||'');if(!v){setDailyWird(writeDailyWird({mushaf:{mode:'',surah:0}}));return;}const bits=v.split(':');const n=Math.trunc(Number(bits[1]));if(bits[0]==='pages'&&isFinite(n)&&n>0){setTarget(n);// the EXISTING helper, writing the EXISTING key
setDailyWird(writeDailyWird({mushaf:{mode:'pages',surah:0}}));}else if(bits[0]==='surah'&&isFinite(n)&&n>=1&&n<=114){setDailyWird(writeDailyWird({mushaf:{mode:'surah',surah:n}}));}};// The measured height of the pager, taken off the pager itself rather than assumed from
...
const[dailyWird,setDailyWird]=useState(readDailyWird);const[selectedSurah,setSelectedSurah]=useState(null);// 1..114
...
ezistQuranText:{color:'var(--a3-ink)',fontSize:19,lineHeight:2.1,textAlign:'center',fontFamily:"'Amiri', serif",margin:'0 0 6px'},ezistQuranMeta:{color:'var(--a3-muted)',fontSize:12.5,fontWeight:600,textAlign:'center'},ezwidStack:{display:'flex',flexDirection:'column',gap:12,marginTop:14},ezwidCard:{padding:'14px',borderRadius:18,background:'var(--a3-surface)',border:'1px solid var(--a3-line)',boxShadow:'var(--a3-shadow)',color:'var(--a3-ink)'},ezwidHead:{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,minHeight:44},ezwidTitle:{fontSize:15.5,fontWeight:800,color:'var(--a3-ink)'},ezwidOpen:{display:'inline-flex',alignItems:'center',justifyContent:'center',minHeight:44,minWidth:44,padding:'6px 12px',borderRadius:999,background:'var(--a3-ice)',border:'1px solid var(--a3-line)',color:'var(--a3-blue)',fontSize:12.5,fontWeight:800,fontFamily:'var(--ez-ui-font)',cursor:'pointer'},ezwidRow:{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10,padding:'7px 0',borderBottom:'1px solid var(--a3-line)'},ezwidName:{fontSize:13.5,fontWeight:600,color:'var(--a3-muted)'},ezwidValue:{fontSize:14.5,fontWeight:800,color:'var(--a3-ink)'},ezwidNote:{marginTop:8,fontSize:12.5,fontWeight:600,color:'var(--a3-muted)',lineHeight:1.6},ezwidNum:{fontSize:19,fontWeight:800,color:'var(--a3-ink)',fontFamily:'var(--ez-ui-font)'},ezwidNumRow:{display:'flex',alignItems:'baseline',flexWrap:'wrap',gap:8,marginTop:10},ezwidVerseText:{color:'var(--a3-ink)',fontSize:19,lineHeight:2.1,textAlign:'center',fontFamily:"'Amiri', serif",margin:'10px 0 6px'},ezwidVerseMeta:{color:'var(--a3-muted)',fontSize:12.5,fontWeight:600,textAlign:'center'},ezwidBar:{display:'flex',justifyContent:'flex-start',marginTop:14},ezwidBarBtn:{display:'inline-flex',alignItems:'center',justifyContent:'center',minHeight:44,padding:'6px 16px',borderRadius:999,background:'var(--a3-surface)',border:'1px solid var(--a3-line)',color:'var(--a3-ink)',fontSize:12.5,fontWeight:800,fontFamily:'var(--ez-ui-font)',cursor:'pointer'},ezwidPanel:{marginTop:14,padding:'14px',borderRadius:18,background:'var(--a3-surface)',border:'1px solid var(--a3-line)',boxShadow:'var(--a3-shadow)',color:'var(--a3-ink)'},ezwidPanelRow:{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,flexWrap:'wrap',padding:'8px 0',borderBottom:'1px solid var(--a3-line)'},ezwidPanelName:{flex:'1 1 120px',minWidth:0,fontSize:14,fontWeight:700,color:'var(--a3-ink)'},ezwidPanelActs:{display:'inline-flex',alignItems:'center',gap:6,flexShrink:0},ezwidAct:{display:'inline-flex',alignItems:'center',justifyContent:'center',minWidth:44,minHeight:44,padding:'0 10px',borderRadius:12,background:'var(--a3-ice)',border:'1px solid var(--a3-line)',color:'var(--a3-blue)',fontSize:12.5,fontWeight:800,fontFamily:'var(--ez-ui-font)',cursor:'pointer'},ezwidActOff:{display:'inline-flex',alignItems:'center',justifyContent:'center',minWidth:44,minHeight:44,padding:'0 10px',borderRadius:12,background:'transparent',border:'1px solid var(--a3-line)',color:'var(--a3-muted)',fontSize:12.5,fontWeight:800,fontFamily:'var(--ez-ui-font)',cursor:'pointer'},ezwidReset:{display:'inline-flex',alignItems:'center',justifyContent:'center',minHeight:44,marginTop:12,padding:'6px 16px',borderRadius:999,background:'var(--a3-ice)',border:'1px solid var(--a3-line)',color:'var(--a3-blue)',fontSize:12.5,fontWeight:800,fontFamily:'var(--ez-ui-font)',cursor:'pointer'},// The compact greeting row. A line profile icon in a white 44px button -- no face, no avatar.
...
suggestionsInline:{display:'flex',flexWrap:'wrap',gap:6,paddingRight:4},suggestionChipSmall:{background:'var(--white)',border:'1px solid var(--line)',color:'var(--red)',borderRadius:999,padding:'6px 12px',fontSize:12.5,fontWeight:500,cursor:'pointer',fontFamily:'var(--ez-ui-font)'},// ITEM 24-A. Existing tokens only -- --tint, --line, --red, --muted, --answer-ink -- so both
...
lessonsBox:{direction:'rtl',display:'flex',flexDirection:'column',gap:6,padding:'10px 12px',background:'var(--tint)',border:'1px solid var(--line)',borderRadius:12},lessonsHead:{fontSize:12.5,fontWeight:700,color:'var(--red)'},lessonsItem:{display:'flex',flexDirection:'column',gap:2,minHeight:44,justifyContent:'center',padding:'6px 8px',background:'var(--white)',border:'1px solid var(--line)',borderRadius:10,textDecoration:'none',color:'var(--answer-ink)'},lessonsTitle:{fontSize:13.5,fontWeight:500,lineHeight:1.6,overflowWrap:'anywhere'},lessonsScholar:{fontSize:12,color:'var(--muted)'},// ITEM 24-B. The lessons SECTION. Every colour is an existing --a3-* token, the set EzShell
// puts in scope through .ezhome, so this screen adds no theme value and overrides none.
lsnForm:{display:'flex',gap:8,alignItems:'stretch',flexWrap:'wrap',marginBottom:12},lsnInput:{flex:'1 1 200px',minWidth:0,minHeight:44,padding:'10px 14px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-surface)',color:'var(--a3-ink)',fontSize:15,fontFamily:'inherit'},lsnSubmit:{minHeight:44,padding:'10px 18px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-ice)',color:'var(--a3-ink)',fontSize:15,fontWeight:700,fontFamily:'inherit',cursor:'pointer'},lsnNote:{display:'flex',alignItems:'center',gap:6,padding:'10px 2px',color:'var(--a3-muted)',fontSize:14},lsnError:{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap',padding:'10px 12px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-surface)',color:'var(--a3-ink)',fontSize:14},lsnRetry:{minHeight:44,padding:'8px 14px',borderRadius:10,border:'1px solid var(--a3-line)',background:'transparent',color:'var(--a3-ink)',fontSize:14,fontFamily:'inherit',cursor:'pointer'},lsnList:{display:'flex',flexDirection:'column',gap:8},lsnItem:{display:'flex',flexDirection:'column',gap:3,minHeight:44,justifyContent:'center',padding:'12px 14px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-surface)',color:'var(--a3-ink)',textDecoration:'none'},lsnItemTitle:{fontSize:15,fontWeight:500,lineHeight:1.7,overflowWrap:'anywhere'},lsnItemScholar:{fontSize:13,color:'var(--a3-muted)'},// ITEM 24-C. The browse pane. The same rule the section above it keeps: every colour is an
...
lsbTabs:{display:'flex',gap:8,marginBottom:12},lsbTab:{flex:'1 1 0',minWidth:0,minHeight:44,padding:'10px 12px',borderRadius:12,border:'1px solid var(--a3-line)',background:'transparent',color:'var(--a3-muted)',fontSize:15,fontWeight:700,fontFamily:'inherit',cursor:'pointer'},lsbTabOn:{flex:'1 1 0',minWidth:0,minHeight:44,padding:'10px 12px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-ice)',color:'var(--a3-ink)',fontSize:15,fontWeight:700,fontFamily:'inherit',cursor:'pointer'},lsbPane:{display:'flex',flexDirection:'column',gap:10},lsbHead:{display:'flex',alignItems:'baseline',gap:8,flexWrap:'wrap',padding:'2px 2px 4px'},lsbHeadName:{fontSize:16,fontWeight:700,color:'var(--a3-ink)',overflowWrap:'anywhere'},lsbHeadCount:{fontSize:13,color:'var(--a3-muted)'},lsbRow:{display:'flex',flexDirection:'column',gap:3,minHeight:44,justifyContent:'center',padding:'12px 14px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-surface)',color:'var(--a3-ink)',textAlign:'start',fontFamily:'inherit',cursor:'pointer'},lsbPager:{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,marginTop:4},lsbPageBtn:{minHeight:44,padding:'10px 16px',borderRadius:12,border:'1px solid var(--a3-line)',background:'var(--a3-surface)',color:'var(--a3-ink)',fontSize:14,fontWeight:700,fontFamily:'inherit',cursor:'pointer'},lsbPageOf:{fontSize:13.5,color:'var(--a3-muted)'},// ===== شريط الإدخال =====
